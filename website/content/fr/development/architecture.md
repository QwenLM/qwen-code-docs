...
3. **Outils (`packages/core/src/tools/`) :**
   - **Objectif :** Ce sont des modules individuels qui étendent les capacités du modèle Gemini, lui permettant d'interagir avec l'environnement local (par exemple, le système de fichiers, les commandes shell, la récupération de données web).
...
2.  **Request to core :** `packages/cli` envoie l'input de l'utilisateur à `packages/core`.
3.  **Request processed :** Le package core :
...
4.  **Model API response :** L'API du modèle traite le prompt et renvoie une réponse. Cette réponse peut être une réponse directe ou une demande d'utilisation d'un des outils disponibles.
5.  **Tool execution (si applicable) :**
    ...
    - Une fois confirmé, ou si aucune confirmation n'est requise, le package core exécute l'action correspondante dans l'outil concerné, et le résultat est renvoyé à l'API du modèle par le package core.
    - L'API du modèle traite le résultat de l'outil et génère une réponse finale.
6.  **Response to CLI :** Le package core renvoie la réponse finale au package CLI.
7.  **Display to user :** Le package CLI formate et affiche la réponse à l'utilisateur dans le terminal.

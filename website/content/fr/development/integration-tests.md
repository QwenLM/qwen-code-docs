...
Les tests d'intégration sont conçus pour valider la fonctionnalité bout-en-bout de Qwen Code. Ils exécutent le binaire compilé dans un environnement contrôlé et vérifient qu'il se comporte comme attendu lors des interactions avec le système de fichiers.
...
Lorsque la sortie est conservée, le test runner affichera le chemin vers le répertoire unique correspondant à l'exécution du test.
...
Afin de garantir la qualité et la cohérence du code, les fichiers de test d'intégration sont analysés par le linter dans le cadre du processus de build principal. Tu peux également exécuter manuellement le linter et le correcteur automatique.

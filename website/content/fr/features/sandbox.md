...
3. **Fichier de configuration** : `"sandbox": true` dans l'objet `tools` de votre fichier `settings.json` (ex. : `{"tools": {"sandbox": true}}`).
...
- `permissive-proxied` : Restrictions en écriture, accès réseau via proxy

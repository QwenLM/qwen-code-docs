...
1. **Fichier de configuration** : Ajoutez `"tavilyApiKey": "votre-clé-ici"` dans votre fichier `settings.json`
2. **Variable d'environnement** : Définissez `TAVILY_API_KEY` dans votre environnement ou dans le fichier `.env`
3. **Ligne de commande** : Utilisez `--tavily-api-key votre-clé-ici` lors de l'exécution du CLI

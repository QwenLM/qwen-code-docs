...
utilisateur. Un message utilisateur en cours de tour s'exécute en premier, les résultats
arrière-plan de retry/continue et connexes conservent le budget de l'étape en cours.
...
| `--local-control`                       | `false`         | Partage le Web Shell authentifié sur chaque interface IPv4 non-loopback avec un token frais par processus, des QR codes de terminal étiquetés, des origins de navigateur exactes, un port fixe et une inhibition de veille au meilleur effort. En conflit avec `--token`, `--allow-origin`, `--no-web`, `--port 0` et `--hostname` non par défaut ; ajoutez `--tls-cert` + `--tls-key` pour les API de navigateur en contexte sécurisé telles que la saisie vocale.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
| `--memory-project-scope <mode>`         | `git-root`      | Mode de partitionnement de la mémoire projet. `git-root` (par défaut) partage la mémoire entre les workspaces résolus vers la même racine Git ; `workspace` clé la mémoire par le répertoire exact de workspace enregistré afin que chaque workspace du démon obtienne sa propre mémoire isolée. Remplace `QWEN_CODE_MEMORY_PROJECT_SCOPE` lorsqu'il est fourni ; une valeur env non reconnue est ignorée avec un avertissement unique et retombe sur `git-root`. Passer à `workspace` ne migre pas la mémoire projet git-root existante — ces entrées cessent d'être visibles jusqu'à ce que vous repassiez à l'autre mode.
...
| `--compacted-replay-max-bytes <n>`      | `4194304`          | Limite d'octets par session live pour les événements de relecture retenus dans le snapshot borné renvoyé par `POST /session/:id/load`. La limite s'applique à `compactedReplay` ; le `liveJournal` en cours est séparément limité par `--max-journal-events` et `--max-journal-bytes`. Les valeurs doivent être des entiers safe positifs ; les valeurs invalides échouent au démarrage, et le plafond dur est 256 MiB. Lorsque de la relecture ancienne retenue est supprimée, le snapshot commence par `history_truncated`. Cela ne limite pas le transcript sur disque.                                                                                                                                                                                                                                                                                                                                                                                                                            |
| `--max-journal-events <n>`              | `10000`            | Limite par session du nombre d'événements bruts retenus dans le journal live en cours (le tour actuel non terminé). Lorsque la limite est dépassée, les entrées de journal les plus anciennes sont supprimées et un marqueur `history_truncated` est ajouté en préfixe. Doit être un entier safe positif.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| `--max-journal-bytes <n>`               | `8388608`          | Limite d'octets par session pour le journal live en cours. Lorsque la limite est dépassée, les entrées de journal les plus anciennes sont supprimées (au moins une entrée est toujours conservée). Doit être un entier safe positif. Par défaut 8 MiB.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
>
> - **Démon vs CLI standalone.** `--memory-project-scope` (et
>   `QWEN_CODE_MEMORY_PROJECT_SCOPE`) n'affecte que les runtimes gérés par le démon.
>   Un TUI `qwen` standalone démarré dans le même répertoire utilise toujours
>   la portée git-root sauf si la variable d'environnement est exportée globalement. Pour maintenir
>   les deux points d'entrée cohérents, épinglez la portée dans le `.env` du workspace ou
>   `settings.env` afin que chaque processus qui lit le workspace soit d'accord.

...
>   `settings.env` ne peut pas la remplacer pour un seul runtime. Un TUI `qwen`
>   standalone possède toujours sa propre portée (par défaut `workspace` depuis la même
>   version qui a changé la valeur par défaut du démon). Pour maintenir les deux points d'entrée
>   cohérents, épinglez la portée dans le `.env` du workspace ou `settings.env`
>   afin que chaque processus qui lit le workspace soit d'accord.

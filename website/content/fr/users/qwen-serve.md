...
Le flag est désactivé par défaut, inclut le comportement d'ouverture du navigateur, et nécessite le Web Shell, les assets Web Shell construits et une liaison loopback. Un simple `qwen serve --open` reste sans token sur le loopback. En mode open authentifié, les routes API normales rejettent les autres clients locaux sans le bearer ; les assets statiques du Web Shell et le `/health` loopback conservent leur comportement pré-authentification existant sauf si `--require-auth` est également défini (le `/health` loopback est pré-auth par défaut ; sur les liaisons non-loopback, `/health` nécessite le token comme les autres routes API normales). Pour plusieurs clients ou un Web Shell réouvrable, utilisez plutôt un token partagé stable :

...
- ✅ **Jetons d'appairage Local Control révocables** — `--local-control` génère un jeton d'appairage LAN distinct appartenant au démon. Le stockage général des jetons du démon reste BYO-token.

**Sécurisation — minimum viable pour le local mono-utilisateur :**

- ✅ Gate de sécurité au démarrage (les liaisons non-loopback embarquent toujours un bearer token — fourni par l'opérateur ou généré au démarrage ; les sources explicitement vides refusent toujours de démarrer, [PR 15 / #4236](https://github.com/QwenLM/qwen-code/pull/4236))
...
### Web Shell distant et accès téléphone

```bash
qwen serve --hostname 0.0.0.0
```

Lorsque ni `--token` ni `QWEN_SERVER_TOKEN` n'est fourni, un écouteur non-loopback génère un bearer éphémère de 128 bits (22 caractères URL-safe) et l'affiche une seule fois au démarrage. Les valeurs de flag explicites prennent le pas sur l'environnement ; sur une liaison non-loopback, une valeur explicitement vide échoue toujours en fail closed et refuse de démarrer, tandis que sur le loopback elle masque `QWEN_SERVER_TOKEN` et laisse la liaison dans le mode de confiance sans token décrit sous `--require-auth`. Le comportement loopback par défaut est inchangé.

Sur les liaisons wildcard, le démarrage liste les URL locales/réseau sans token depuis les interfaces réelles et — pour le token généré, ou pour votre propre token dans un terminal interactif — un **QR code contenant le secret** pour une adresse LAN privée étiquetée. L'énumération wildcard liste uniquement la même population LAN privée/link-local que Local Control annonce (RFC 1918 / RFC 3927 IPv4, plus fc00::/7 ULA IPv6 sur les liaisons dual-stack) ; le filtre d'interface logicielle qui exclut les adaptateurs VPN et les bridges de conteneurs est une **heuristique de nom** ([#9158](https://github.com/QwenLM/qwen-code/issues/9158)), pas une garantie, donc les noms d'overlay k8s (`cbr0`, `cali*`, `vxlan.*`) peuvent passer au travers. Une liaison non-loopback explicite affiche et QR-code exactement l'adresse que vous avez choisie — y compris une adresse routable. Dans un conteneur, l'adresse énumérée est l'interface propre du conteneur, donc atteindre un port publié signifie substituer l'adresse hôte ; un conteneur avec pty alloué compte comme un terminal interactif, donc ses logs restent porteurs de secrets. Scannez le QR pour ouvrir le Web Shell intégré avec le bearer dans le fragment URL. Le navigateur retire l'identifiant de l'URL et le conserve dans le `sessionStorage` par onglet. Alternativement, ouvrez une URL affichée et saisissez le token ; les identifiants manquants ou expirés affichent un formulaire de retry. Les tokens générés tournent au redémarrage. `--no-web` omet le QR, et un stdout pipé/redirigé ne re-publie jamais un token opérateur stable en QR. Les adresses n'impliquent pas l'accessibilité firewall ou NAT ; aucune IP publique n'est découverte automatiquement.

Les requêtes **HTTP** same-origin du Web Shell intégré fonctionnent sans `--allow-origin` ; les autres origins de navigateur ont toujours besoin de cette allowlist, et `Origin: null` reste rejeté. Les fonctionnalités supportées par WebSocket (terminal, voix) et les navigateurs atteignant le démon via un reverse proxy à terminaison TLS nécessitent toujours une entrée `--allow-origin <origin>` explicite — les en-têtes transférés ne sont jamais considérés comme fiables pour établir la même origin. Il en va de même pour tout intermédiaire HTTP en clair qui réécrit l'en-tête `Host` : le `proxy_set_header Host $proxy_host` par défaut de nginx et l'Ingress k8s font tous deux voir au démon un `Host` qui ne correspond plus à l'`Origin` du navigateur. Passez `--allow-origin <origin>` pour l'origin que votre navigateur voit réellement, ou configurez le proxy pour transférer `Host` verbatim. Sur une liaison **non-loopback**, une translation de port seule (`docker -p 8080:4170`) n'a besoin de rien — la vérification same-origin compare `Origin` avec le `Host` transféré et ne consulte jamais le port d'écoute. Sur la liaison **loopback** par défaut, ce n'est pas le cas : la allowlist Host anti-DNS-rebinding n'accepte que le port propre du démon, donc un tunnel avec translation de port (`ssh -L 8080:localhost:4170`) est rejeté avec `403 Invalid Host header` pour chaque requête y compris le document shell, et `--allow-origin` ne peut pas passer outre — transférez le même port (`ssh -L 4170:localhost:4170`) ou liez en non-loopback. Le bearer accorde l'autorité complète de l'API du démon, y compris l'exécution de code. Traitez les tokens de terminal et les captures QR comme des secrets. HTTP n'est pas chiffré ; les options existantes `--tls-cert` et `--tls-key` fournissent du HTTPS avec votre propre certificat.
...
# → qwen serve: mode loopback de confiance ; les appelants locaux ont un accès API complet sans authentification bearer, y compris l'exécution de code en tant qu'utilisateur du démon. Utilisez --require-auth avec QWEN_SERVER_TOKEN sur les hôtes partagés ou non fiables.
```

La liaison par défaut est `127.0.0.1:4170`. L'auth Bearer est **désactivée** et l'écouteur primaire est de confiance, donc tout processus local pouvant atteindre le port peut utiliser l'API opérateur complète, y compris exécuter du code en tant qu'utilisateur du démon. Les vérifications de confiance workspace par route, de propriété de session, `X-Qwen-Client-Id`, de permission, de fonctionnalité, de validation et de ressources s'appliquent toujours. Le démon enregistre le répertoire de travail courant comme workspace primaire ; utilisez un `--workspace /path/to/dir` absolu pour le remplacer, et répétez le flag pour enregistrer des runtimes isolés supplémentaires.
...
Pour un lancement authentifié mono-utilisateur sans créer manuellement un token, optez explicitement :

```bash
qwen serve --open-with-auth
```

Ce mode loopback uniquement génère un bearer token 256 bits lorsque ni `--token` ni `QWEN_SERVER_TOKEN` n'en fournit un, puis le transmet au Web Shell ouvert sous forme de fragment URL `#token=`. Le shell supprime le fragment et conserve l'identifiant dans le `sessionStorage` de cet onglet ; le rafraîchissement fonctionne, mais fermer l'onglet ou redémarrer le démon perd l'identifiant. En CI, SSH ou un autre environnement où l'auto-open n'est pas disponible, le démon démarre et affiche l'URL fragmentée pour ouverture manuelle. L'URL affichée contient des secrets.

Le flag est désactivé par défaut, inclut le comportement d'ouverture du navigateur, et nécessite le Web Shell, les assets Web Shell construits et une liaison loopback. Un simple `qwen serve --open` reste sans token sur le loopback. En mode open authentifié, les routes API normales rejettent les autres clients locaux sans le bearer ; les assets statiques du Web Shell et le `/health` loopback conservent leur comportement pré-authentification existant sauf si `--require-auth` est également défini. Pour plusieurs clients ou un Web Shell réouvrable, utilisez plutôt un token partagé stable :
...
### Capacité d'enregistrement de workspace

La limite d'enregistrement par défaut est 256. Définissez `QWEN_SERVE_MAX_WORKSPACES=25 qwen serve --workspace /path/to/project` pour l'abaisser ; les valeurs acceptées sont des entiers décimaux de 1 à 256. Les valeurs invalides échouent au démarrage. C'est un paramètre de démarrage du démon : les `.env` de projet et `settings.env` de projet ne peuvent pas le modifier. Les appelants embarqués peuvent définir `ServeOptions.maxRegisteredWorkspaces`, qui prend le pas sur l'environnement de démarrage.

La limite compte le workspace primaire, les workspaces secondaires enregistrés et les workspaces scratch créés par l'utilisateur. Le runtime interne Conversations en est exempté. L'enregistrement dynamique renvoie `409 workspace_limit_reached` à capacité. `GET /capabilities` et `GET /daemon/status` exposent `limits.maxRegisteredWorkspaces`.

Avec la capacité d'enregistrement par défaut de 256, le démon standard utilise un plafond global fixe de 800 sessions, y compris un démon démarré avec un seul workspace. Le même défaut de session s'applique à toute capacité d'enregistrement configurée supérieure à 25. `--max-total-sessions` le remplace toujours ; `0` le désactive. À des capacités de 25 ou moins, le défaut existant reste : un workspace démarré ou restauré est illimité, et plusieurs dérivent le total du plafond par workspace et de ce nombre. Les changements d'enregistrement ne recalculent pas le total. Cela ne change pas la politique mémoire des enfants ACP ni ne promet 800 sessions concurrentes sûres.

Le contrôle de canal supporte indépendamment au plus 25 propriétaires de workspace, exposé sous `limits.maxChannelControlWorkspaces`. Une transition compte aussi les propriétaires conservés pour la récupération ; à capacité, arrêtez d'abord tous les canaux sélectionnés sur un ancien propriétaire avant d'ajouter un nouveau propriétaire. Une transition surdimensionnée renvoie `409 channel_control_workspace_limit_reached`. `--channel all` continue de sélectionner uniquement le workspace primaire. Le timeout SDK de canal est inchangé.

Les fichiers d'enregistrement persistés conservent le schéma version 1 et supportent jusqu'à 255 enregistrements secondaires dans 8 MiB. Les enregistrements bruts, y compris les alias obsolètes, consomment des slots persistants jusqu'à être oubliés. Si l'ensemble de démarrage valide complet dépasse la limite configurée, le démarrage échoue et laisse le fichier inchangé. Pour abaisser la capacité, redémarrez avec la capacité précédente, oubliez suffisamment d'enregistrements, et réduisez les arguments `--workspace` explicites avant de redémarrer avec la limite inférieure. Avant de rétrograder vers un binaire plus ancien, sauvegardez le fichier et utilisez le binaire plus récent pour le réduire à au plus 24 enregistrements secondaires et 256 KiB. Les binaires plus anciens rejetteraient sinon ce store et démarreraient avec les workspaces explicites uniquement ; leurs mutations de store échouent avant l'écriture.

### Ajuster le polling de l'état live des sessions

Le Web Shell rafraîchit l'état live des sessions du workspace toutes les **5 secondes** par défaut. Pour utiliser un intervalle différent, définissez la variable d'environnement du démon avant de le démarrer :

```bash
QWEN_SESSION_LIVE_STATE_POLL_INTERVAL_MS=10000 qwen serve --web
```

La valeur est un entier en millisecondes, de `1000` à `2147483647`. Les valeurs manquantes ou invalides reviennent à `5000`. L'intervalle s'applique à tous les workspaces servis par ce démon ; redémarrez le démon et rechargez les pages Web Shell ouvertes après l'avoir modifié. Les actions de session locales et le retour à un onglet visible peuvent toujours rafraîchir immédiatement. Cela ne change que le polling de l'état live, pas le polling complet du catalogue de sessions.

Le démon publie l'intervalle effectif sous `sessionLiveStatePollIntervalMs` dans `/capabilities`. Le Web Shell utilise cette valeur et revient à cinq secondes lors de la connexion à un démon plus ancien qui l'omet. Les consommateurs SDK peuvent lire la valeur lors du choix de leur propre planning de polling.
...
Les `qwen channel set` et `qwen channel reload` gérés par le démon, ainsi que `status` et `stop` avec `--daemon-url`, ne peuvent pas découvrir un token généré par `--open-with-auth`. Utilisez `QWEN_SERVER_TOKEN` et passez la même valeur avec `--token` à ces clients, ou omettez le mode authenticated-open.
...
Le démon expose également des instantanés d'exécution en lecture seule pour les UIs client et les
opérateurs : `GET /daemon/status`, `GET /workspace/mcp`,
...
`GET /session/:id/resources` renvoie les instantanés sanitized de Skill et MCP
depuis la Config propre de cette session live. Utilisez-le lorsqu'une session peut
s'exécuter depuis un worktree ou un autre répertoire effectif dont les ressources
diffèrent du workspace de base. L'authentification MCP, le pool, le budget et les
détails d'erreur de découverte appartenant au workspace sont omis de cette vue
session. Vérifiez la capacité `session_resources` avant de l'appeler.
...
Pour mettre à jour les paramètres de Skill du workspace par nom, appelez `POST /workspace/skills/:name/enable` avec `{ "enabled": true | false }` après avoir vérifié la capacité `workspace_skill_settings_toggle`. Pour modifier plusieurs noms, vérifiez `workspace_skill_settings_batch_toggle` et appelez `POST /workspace/skills/enable` avec `{ "skillNames": ["review", "deploy"], "enabled": false }` ; cela traite chaque nom structurellement valide ensemble, persiste toutes les modifications de déclaration résultantes en au plus une écriture de paramètres verrouillée, et rafraîchit les sessions ACP actives en une fois si quelque chose a changé. Ces routes écrivent `skills.disabled` et `skills.enabled` du workspace sans consulter le catalogue de Skills chargé, donc des noms peuvent être désactivés avant l'installation ou lorsque leur Extension est inactive. Activer supprime une désactivation de workspace correspondante et enregistre un opt-in explicite `skills.enabled`, même avant l'installation, afin que cette entrée puisse overriding une désactivation interne de Skill de l'Extension. Leurs chemins et corps de requête sont inchangés ; les capacités spécifiques aux paramètres remplacent les tags de toggle Skill validés par le catalogue retirés. Le tableau `errors` du batch reste pour la compatibilité wire et est vide pour les noms structurellement valides. Une entrée `skills.disabled` dure héritée d'une portée supérieure reste autoritaire pour la disponibilité effective, mais ne bloque pas le workspace pour enregistrer ou supprimer sa propre déclaration. Les cellules de statut de Skill exposent `disabledReason` (`hard`, `default` ou `inactive_extension`) et un `lockedScope` optionnel. Les écritures de workspace non fiable sont toujours rejetées. Une réponse `deferred` signifie qu'aucun enfant n'était actif lors du contrôle de liveness ou qu'une requête modifiée a perdu son enfant/session lors du rafraîchissement requis ; lorsque `changed` est vrai, la déclaration persistée s'applique au démarrage d'un enfant. Le fait qu'une déclaration ait réellement changé est rapporté par `changed` (sur chaque résultat pour une requête batch). `skills.disabled` désactive à la fois l'utilisation manuelle et par le modèle, contrairement à `disable-model-invocation: true` qui laisse l'invocation directe via `/skill-name` disponible. Pour les lots d'extensions V2, vérifiez `extension_batch_activation_v2` : `PUT /extensions/activation` modifie les valeurs par défaut globales, tandis que `PUT /workspaces/:workspace/extensions/activation` modifie les overrides exactes pour le workspace sélectionné et accepte `"inherit"` pour les effacer. Les deux acceptent des noms dans `extensionNames` ; `enabled` et `disabled` peuvent être déclarés avant l'installation, tandis que `inherit` pour un nom inconnu est une opération vide. Chaque requête retourne une opération à poller. `skills.disabled` désactive à la fois l'utilisation manuelle et par le modèle, contrairement à `disable-model-invocation: true` qui laisse l'invocation directe via `/skill-name` disponible. Pour les lots d'extensions V2, vérifiez `extension_batch_activation_v2` : `PUT /extensions/activation` modifie les valeurs par défaut globales, tandis que `PUT /workspaces/:workspace/extensions/activation` modifie les overrides exactes pour le workspace sélectionné et accepte `"inherit"` pour les effacer. Les deux acceptent des noms dans `extensionNames` ; `enabled` et `disabled` peuvent être déclarés avant l'installation, tandis que `inherit` pour un nom inconnu est une opération vide. Chaque requête retourne une opération à poller.
...
- `GET /file` lit les fichiers texte. Les réponses full-snapshot renvoient un hash raw-byte
  `sha256:<hex>` ; les fenêtres de lignes finies des fichiers au-dessus de 256 KiB l'omettent.
...
Write/edit sont des **routes de mutation strictes** : l'écouteur primaire loopback de confiance sans token accorde l'autorité opérateur ; les embeds non fiables sans identifiant renvoient `token_required`. Les remplacements
...
Les clients envoient ensuite `Authorization: Bearer $QWEN_SERVER_TOKEN` sur les requêtes API normales. `/health` est exempté **uniquement sur les liaisons loopback ordinaires** afin que les sondes de liveness k8s/Compose à l'intérieur du pod n'aient pas besoin d'identifiants. Sur les liaisons non-loopback (`--hostname 0.0.0.0` etc.) `/health` nécessite le token comme les autres routes API normales — sinon un attaquant peut sonder des adresses arbitraires pour confirmer l'existence du démon. L'entrée webhook de canal est séparée dans chaque mode et utilise son `x-qwen-webhook-secret` configuré au lieu du bearer du démon. Utilisez `/capabilities` pour vérifier que votre token est correct de bout en bout :

> **Loopback durci (`--require-auth`).** Le comportement par défaut sans token sur le loopback est adapté pour un laptop mono-utilisateur mais non sécurisé sur les hôtes de dev partagés, les runners CI ou les workstations multi-tenants où n'importe quel utilisateur local peut faire `curl 127.0.0.1:4170`. Passez `--require-auth` pour rendre le bearer token obligatoire sur chaque route API normale — y compris `/health` et `/capabilities` — même lorsqu'il est lié à `127.0.0.1`. L'entrée webhook de canal reste authentifiée de manière indépendante par `x-qwen-webhook-secret`. Les routes de documents et d'assets du Web Shell restent pré-auth ; utilisez `--no-web` pour les supprimer. Le démarrage échoue sans token. Avec le flag activé, un client **non authentifié** ne peut pas lire `/capabilities` pour découvrir que l'auth est requise ; la surface de découverte est le corps de la réponse 401 lui-même. Une fois authentifié, le tag `caps.features.require_auth` est une confirmation post-auth que le déploiement est durci (utile pour les UIs d'audit / conformité) :
>
> ```bash
...
`--open-with-auth` est une commodité appartenant au CLI, pas une autre source de token du démon : il sélectionne `--token` lorsque cette option est définie (même vide), sinon `QWEN_SERVER_TOKEN`, puis tronque la valeur sélectionnée et génère uniquement si le résultat est vide. Le démon ne persiste pas la valeur générée ni ne l'exporte comme `QWEN_SERVER_TOKEN` ; le handoff interne existant pour l'enfant authentifié reste inchangé. Le Web Shell stocke sa copie navigateur uniquement dans le `sessionStorage` de l'onglet récepteur ; ce mode n'ajoute aucun mécanisme de découverte d'identifiant cross-tab ou pour les clients externes. Le token n'est pas indépendamment révocable ni lié à une identité client. La possession accorde la même autorité de démon que n'importe quel autre bearer token. Consultez la [conception du lancement Web Shell authentifié](../design/2026-08-22-serve-open-with-auth.md) et les travaux futurs connexes dans [#4514](https://github.com/QwenLM/qwen-code/issues/4514).
...
- **Les workers de canal rappellent le démon en `https://`** — ils doivent donc aussi faire confiance au certificat de service. Un certificat auto-signé (ou une chaîne complète portant sa propre racine) n'a besoin de rien : le démon l'injecte dans le `NODE_EXTRA_CA_CERTS` de chaque worker. Le flux mkcert ci-dessus est **émis par une CA**, donc la feuille seule ne peut pas ancrer la chaîne — exportez `NODE_EXTRA_CA_CERTS="$(mkcert -CAROOT)/rootCA.pem"` dans l'environnement de lancement du démon avant de démarrer avec `--channel`. Une valeur définie par l'opérateur est _fusionnée_ avec le certificat du démon, pas remplacée. Sans cela, le démon démarre normalement tandis que chaque worker de canal boucle en redémarrage sur `UNABLE_TO_VERIFY_LEAF_SIGNATURE` ; le log du démon nomme ce manque au démarrage.
- **Une liaison wildcard IPv6 est rappelée sur le loopback que cet hôte assigne réellement** — `--hostname ::` (ou `[::]`) lie une socket IPv6, et un `--hostname` vide en lie une aussi lorsque IPv6 est disponible (lorsqu'il ne l'est pas, Node retombe sur la liaison de `0.0.0.0`). Cette socket est dual-stack (Node épingle `IPV6_V6ONLY=0` dessus, donc le sysctl `net.ipv6.bindv6only` ne change pas cela), et les deux loopbacks l'atteignent généralement — mais un hôte sans IPv4 du tout n'a que `::1`, tandis qu'un hôte qui lie `::` mais ne porte pas de `::1` sur sa loopback (par exemple `net.ipv6.conf.lo.disable_ipv6=1`) n'a que `127.0.0.1`. Les workers sont envoyés vers `[::1]` lorsque cet hôte l'assigne et vers `127.0.0.1` sinon. Un certificat de service pour une liaison wildcard IPv6 doit porter les deux loopbacks dans ses SANs (`mkcert localhost 127.0.0.1 ::1` les couvre) ; le diagnostic de confiance au démarrage inspecte l'URL exacte que les workers composeront et nomme l'écart. `--hostname 0.0.0.0` est inchangé et nécessite toujours `127.0.0.1`.
- **La rotation de `--tls-cert` en place nécessite un redémarrage du démon** — le démon sert les octets qu'il a lus au démarrage, donc jusqu'à ce qu'il redémarre, les workers relancés peuvent charger des contenus plus récents que ceux que le démon présente et leurs handshakes échouent.
...
| `--hostname <addr>`                     | `127.0.0.1`     | Interface de liaison. Toute liaison au-delà du loopback nécessite un token ; lorsque ni `--token` ni `QWEN_SERVER_TOKEN` n'en fournit un, un token éphémère est généré et affiché au démarrage.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
| `--token <str>`                         | —               | Bearer token. Se rabat sur la variable d'environnement `QWEN_SERVER_TOKEN` (avec les espaces de début et de fin supprimés — pratique pour `$(cat token.txt)`). Une liaison non-loopback sans aucune source génère un token éphémère. Une valeur explicitement vide refuse toujours de démarrer sur une liaison non-loopback ; sur le loopback, elle masque la variable d'environnement et sélectionne le mode de confiance sans token (ou refuse catégoriquement sous `--require-auth`).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `--require-auth`                        | `false`         | Refuse de démarrer sans bearer token, même sur le loopback. Renforce le comportement par défaut du développeur sur `127.0.0.1` pour les hôtes de développement partagés / les runners CI / les stations de travail multi-locataires où n'importe quel utilisateur local peut atteindre l'écouteur. Ce fail-fast est propre au loopback : le loopback ne démarre qu'avec l'une de ses trois sources de token — `--token`, `QWEN_SERVER_TOKEN`, ou `--open-with-auth`, qui installe son propre token généré avant le démarrage (la génération est supprimée sur le loopback) — tandis que sur une liaison non-loopback, le token éphémère généré satisfait le flag. Place également `/health` derrière le bearer.                                                                                                                                                                                                                                                                                                                                                                                 |
| `--enable-session-shell`                | `false`            | Active l'exécution directe de shell de session. Effectif avec un bearer token configuré ou le mode loopback de confiance ; les appels nécessitent toujours un `X-Qwen-Client-Id` lié à une session valide.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
| `--max-total-sessions <n>`              | `800` ou dérivé | Entier positif ou nul optionnel, plafond à l'échelle du démon pour la création de nouvelles sessions dans tous les runtimes de workspace enregistrés. Il s'applique aux nouvelles sessions enfants, à la restauration de sessions et aux sessions créées par branch/fork ; le rattachement à une session live existante ne consomme pas de slot. Définir à `0` pour illimité. La valeur par défaut est 800 lorsque la capacité d'enregistrement est supérieure à 25 (y compris la valeur par défaut de 256). À des capacités de 25 ou moins, un workspace démarré ou restauré est illimité ; plusieurs dérivent le total de la limite par workspace et de ce nombre. Un enregistrement dynamique ultérieur ne le recalcule pas.                                                                                                                                                                                                                                                                                                                                                                              |
...
| `--session-restore-timeout-ms <n>`      | `60000`            | Deadline de chargement/reprise de session ACP en millisecondes. Doit être un entier positif jusqu'à `2147483647` ; `0` est invalide. Si omis, la valeur par défaut est de 60 secondes, relevée à un `--initialize-timeout-ms` explicitement fourni lorsque cette valeur est plus grande ; un timeout d'initialisation plus court n'abaisse jamais le budget de restauration. Le SDK et le Web Shell ajoutent 10 et 15 secondes de marge client. Un timeout retourne un `504 session_restore_timeout` retryable ; cela n'implique pas que le démon lui-même a quitté.                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `--allow-origin <pat>`                  | —               | T2.4 ([#4514](https://github.com/QwenLM/qwen-code/issues/4514)). Liste blanche cross-origin pour les clients navigateur. Répétable. Chaque valeur est `*` (n'importe quelle origin — le démarrage refuse si aucun bearer token n'est configuré ; `--require-auth` sur le loopback est recommandé pour que `/health` soit également protégé par le bearer, car il est pré-auth sur le loopback par défaut ; les ressources statiques du Web Shell restent pré-auth dans tous les modes, donc passez `--no-web` pour les supprimer) ou une origin URL canonique (`<scheme>://<host>[:<port>]`, sans slash de fin / chemin / userinfo / requête). Sans token, les origins HTTP(S) doivent être loopback ; les origins HTTP(S) distantes nécessitent un token. Les extensions de navigateur conservent leur accès local sans token. **Les wildcards de sous-domaine (`https://*.example.com`) ne sont intentionnellement pas prises en charge** — listez explicitement chaque sous-domaine, ou utilisez `*` avec un token configuré (et `--require-auth` pour un durcissement complet). Les origins correspondantes reçoivent les en-têtes de réponse CORS (`Access-Control-Allow-Origin`, `Vary: Origin`, méthodes, en-têtes, max-age, et `Retry-After` exposé) ; les origins non correspondantes obtiennent toujours un 403 avec la même enveloppe qu'aujourd'hui. `Origin: null` (iframes sandboxées, documents file://) est toujours rejeté, même sous `*`. Pre-flight via `caps.features.allow_origin`. Les hits sur l'origin du loopback lui-même ne sont pas affectés. |
| `--web` / `--no-web`                    | `true`          | Sert le SPA Web Shell construit à la racine du démon (`GET /`, `/assets/*`, et navigations de documents `GET /session/<id>`). Ces points d'entrée sont enregistrés **avant** la porte d'auth par bearer token — un navigateur ne peut pas attacher de token à une sous-ressource `<script>` ou à une navigation dans la barre d'adresse, et le shell ne contient aucun secret. Les requêtes API suivent la politique d'autorité normale du démon : les tokens configurés gardent les routes API normales sauf le `/health` loopback sauf si `--require-auth` est défini, l'entrée webhook de canal utilise toujours son propre secret partagé, et l'écouteur primaire loopback de confiance sans token a un accès opérateur complet. Le fallback de lien profond du SPA pour tous les autres chemins se trouve derrière le middleware bearer. Sur les liaisons non-loopback, un avertissement d'une ligne sur stderr indique que l'UI est accessible sans authentification. Utilisez `--no-web` pour un démon API uniquement. Aucun effet lorsque la build omet les ressources du Web Shell (le démon enregistre un breadcrumb et s'exécute en API uniquement).                                                                                                                                                                                                                                                                                         |
...
| `--open-with-auth`                      | `false`         | Ouvre le Web Shell avec l'authentification bearer sur le loopback. Nécessite un Web Shell activé et des assets construits. Réutilise un token configuré ou génère un bearer 256 bits pour la durée du processus et le délivre dans le fragment du Web Shell. Les environnements incompatibles avec le navigateur affichent l'URL manuelle contenant le secret. Les autres clients ont besoin du même token partagé configuré explicitement.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
- **Loopback uniquement** — `127.0.0.0/8`, `localhost` ou `::1` ; aucune authentification nécessaire.
- **`localhost` est résolu une seule fois, épinglé et vérifié après l'écoute** — si son adresse d'écoute réelle est en dehors de `127.0.0.0/8` ou `::1`, le démarrage sans token ferme l'écouteur et échoue.
- **`--hostname 0.0.0.0` requiert un token** — un token est généré lorsqu'aucune source de token n'est fournie ; les sources explicitement vides échouent toujours en fail closed.
- **`LOOPBACK_BINDS` inclut l'IPv6** — `::1` et `[::1]` comptent comme loopback pour la règle sans token.
- **Liste d'autorisation de l'en-tête Host** — sur les liaisons **loopback**, le démon vérifie que `Host:` correspond à `localhost:port` / `127.0.0.1:port` / `[::1]:port` / `host.docker.internal:port` ou à l'adresse et port loopback liés exacts (insensible à la casse selon la RFC 7230 §5.4) pour se défendre contre le DNS rebinding. Lors de l'écoute sur le port 80 ou 443, les formes sans port correspondantes sont également acceptées car les navigateurs omettent les ports par défaut de schéma. Cela prend en charge la plage loopback IPv4 complète (`127.0.0.0/8`) sans admettre de Hosts non liés. **Les liaisons non-loopback (`--hostname 0.0.0.0`) contournent intentionnellement la liste d'autorisation Host** — l'opérateur a choisi la surface d'exposition, la barrière du bearer-token est donc la seule couche d'authentification ; les reverse proxies / SNI / le pinning de certificats client relèvent de la responsabilité de l'opérateur, et non du daemon. Si vous avez besoin d'une isolation basée sur Host pour une liaison non-loopback, terminez le TLS + vérifiez Host au niveau d'un proxy frontal.
- **CORS refuse toute origine de navigateur par défaut** — renvoie un JSON `403`. Passez **`--allow-origin <pattern>`** (répétable, T2.4 #4514) pour faire passer des origines de navigateur spécifiques. Chaque valeur est soit le littéral `*` (toute origine — sur une liaison **loopback**, le démarrage refuse si aucun bearer token n'est configuré, tandis qu'une liaison non-loopback démarre car la génération de token en fournit un ; `--require-auth` sur le loopback est recommandé pour un durcissement complet car `/health` reste pré-auth sur le loopback par défaut — notez que les assets statiques du Web Shell (`/`, `/assets/*`, navigations de documents `/session/:id`) sont montés avant le bearer dans tous les modes et restent pré-auth même sous `--require-auth`, donc utilisez `--no-web` lorsque la surface navigateur résiduelle compte) soit une origine URL canonique (`<scheme>://<host>[:<port>]`, sans slash de fin / chemin / userinfo). Sur une liaison **loopback** sans token, les entrées HTTP(S) doivent utiliser un hôte loopback ; une liaison non-loopback transporte toujours un token — fourni ou généré — précisément parce que les origines de navigateur hébergées à distance peuvent exécuter du code via l'API opérateur en tant qu'utilisateur du démon. Les origines d'extension de navigateur explicites conservent leur chemin d'automatisation locale sans token. Les origines correspondantes reçoivent des en-têtes de réponse CORS appropriés (`Access-Control-Allow-Origin: <echoed>`, `Vary: Origin`, ainsi que les méthodes / en-têtes / max-age standards et `Retry-After` exposé) ; les origines non correspondantes reçoivent toujours un 403 avec la même enveloppe que le mur par défaut. `caps.features.allow_origin` est annoncé conditionnellement afin que les clients SDK / webui puissent vérifier en amont (pre-flight) si le daemon honore les requêtes cross-origin avant de les émettre. Exemple : `qwen serve --allow-origin http://localhost:3000 --allow-origin http://localhost:5173`. Les requêtes vers l'auto-origin loopback (par ex. l'UI Web Shell) ne sont pas affectées — un shim de suppression d'Origin distinct les gère indépendamment de `--allow-origin`. Les requêtes **HTTP** same-origin du Web Shell distant intégré fonctionnent également sans `--allow-origin` : sur un écouteur primaire non-loopback, une `Origin` égale au schéma direct du socket plus l'autorité `Host` normalisée est authentifiée par bearer puis retirée avant le mur CORS, tandis que ses routes statiques publiques existantes restent non authentifiées. Deux exclusions nécessitent toujours `--allow-origin` — les en-têtes transférés n'établissent jamais la confiance same-origin, donc l'origine de navigateur `https` d'un reverse proxy à terminaison TLS n'est pas same-origin (pas plus qu'un intermédiaire HTTP en clair qui réécrit l'en-tête `Host`, comme le `proxy_set_header Host $proxy_host` par défaut de nginx ou l'Ingress k8s ; le remède est `--allow-origin <origin>` pour l'origin que votre navigateur voit, ou un proxy qui transfère `Host` verbatim — une translation de port seule n'a besoin de rien, car seul le `Host` transféré est comparé, avec uniquement les ports par défaut de schéma `:80`/`:443` retirés), et les routes de mise à niveau WebSocket (terminal, voix) conservent une gate CSWSH séparée qui n'admet que les origins loopback, les origins sur liste d'autorisation et l'origin propre de l'écouteur Local Control.
...
L'issue [#4514](https://github.com/QwenLM/qwen-code/issues/4514) T2.9 introduit deux flags opt-in qui comblent les lacunes des déploiements longue durée / distants que le heartbeat de 15s + AbortSignal ne couvrent pas. Le timeout de réponse de permission partagé est listé ici également. Les trois sont désactivés par défaut.
...
| `--permission-response-timeout-ms <n>` | —                                   | `0`     | Temps mural partagé pour les réponses de permission ordinaires et `ask_user_question` en mode démon. `0` ou un flag omis attend indéfiniment ; un entier positif impose un délai sur les deux. L'annulation de voteur, l'annulation de session et l'arrêt du démon résolvent toujours les interactions en attente lorsque le timer est désactivé. |

Les flags de prompt et de writer acceptent un entier positif en millisecondes ; les valeurs `0`, `NaN`, non entières ou négatives sont rejetées au démarrage avec un message d'erreur clair. Le timeout de réponse de permission accepte `0` ou un entier positif. Pour les deux deadlines supportées par des variables d'env, les champs explicites `ServeOptions` l'emportent sur les valeurs d'env. Les consommateurs SDK doivent vérifier en amont (pre-flight) le tag de capacité correspondant avant de s'appuyer sur les comportements de prompt et de writer.

### Mode de fichier pour les écritures texte de l'agent

Les écritures texte de l'agent (`write_file`, `edit`, `notebook_edit`, et l'éditeur sed simulé de l'outil shell) publient via le writer atomique du démon, qui préserve le mode d'une cible existante et — pour les **nouveaux** fichiers — utilise par défaut `0600` en propriétaire uniquement, ignorant l'umask du processus démon. Ce défaut fail-closed est intentionnel : un fichier fraîchement créé par l'agent n'est jamais lisible par le groupe/le monde par accident, quelle que soit la permissivité de l'umask du superviseur.
...
### Stockage des pièces jointes de session

Les pièces jointes de session (fichiers et images uploadés par le Web Shell via `POST /session/:id/attachments`) sont stockées par défaut sous le répertoire temp runtime du workspace : `<runtimeBaseDir>/tmp/<projectHash>/attachments`, clées par session sous `session-<sessionId>`. Les opérateurs qui souhaitent que les pièces jointes persistent en dehors du répertoire temp runtime (par ex. sur un volume dédié) peuvent remplacer la racine :

| Variable d'env                            | Valeurs | Défaut | Ce que cela fait                                                                                                                                                                                                |
| ------------------------------------- | ------ | ------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `QWEN_SERVE_SESSION_ATTACHMENTS_ROOT` | chemin   | non défini   | Stocke les pièces jointes de session sous ce répertoire au lieu du répertoire temp runtime par défaut. Accepte un chemin absolu, un chemin relatif au cwd du démon, ou `~` / `~/…` étendu par rapport au répertoire home. |

Portée et limites :

- **Migration à sens unique.** Lorsque la variable d'env est définie, les nouvelles pièces jointes sont écrites uniquement sous la racine configurée. Les lectures et suppressions qui manquent la racine configurée se rabattent sur le répertoire temp runtime par défaut, donc les pièces jointes uploadées **avant** le changement restent lisibles, et supprimables tant que le répertoire fallback par défaut reste accessible en écriture — une suppression dont la copie héritée ne peut pas être déliée (par ex. un volume fallback en lecture seule) signale l'erreur plutôt que de rapporter le succès. La direction inverse — supprimer la variable d'env après que des pièces jointes ont été écrites dans la racine configurée — rend ces pièces jointes inaccessibles ; maintenez la variable stable pour un workspace donné.
- **Layout par session.** Les fichiers vivent sous `<root>/<projectHash>/attachments/session-<sessionId>/` dans les deux emplacements, où `<projectHash>` est le même hash de workspace utilisé par le répertoire temp runtime par défaut ; la recherche fallback utilise le même layout de session dans le répertoire par défaut. Deux workspaces pointant vers la même racine configurée restent isolés l'un de l'autre.
- **Nettoyage par suppression.** Lorsqu'une session est supprimée, son répertoire de pièces jointes est retiré à la fois de la racine configurée et du répertoire fallback par défaut. Archiver une session conserve ses pièces jointes afin qu'elles survivent au désarchivage.
- Le démon lit la variable au démarrage ; redémarrez le démon après l'avoir modifiée. Le répertoire doit être accessible en écriture par le processus démon.
...
| `GET /workspaces/:workspace/session/:id/transcript`     | Le client a besoin d'un transcript persisté actif d'un workspace sélectionné. Les pages forward et cursor restent daemon-local ; une première page backward pour une session live peut démarrer ACP et charger les paramètres du workspace pour flusher le tail persisté. Les workspaces secondaires non fiables enregistrés peuvent utiliser ce chemin en lecture seule. Tag de capacité : `workspace_persisted_transcript`. |
...
`limit` est le nombre cible d'enregistrements de chat actifs, pas les frames de relecture émises ; un enregistrement peut produire plusieurs événements `session_update`. Une page backward peut s'étendre à au plus `3 * limit` enregistrements pour préserver les limites de tour et d'appel/résultat d'outil. La première réponse gèle la taille de l'instantané JSONL et renvoie `nextCursor` tant que `hasMore` est vrai. Les pages ultérieures ignorent les ajouts après la page 1, mais renvoient `409` si le fichier est supprimé, tronqué, remplacé, archivé ou autrement en conflit avec le curseur gelé. Les instantanés très volumineux renvoient `413 transcript_too_large` avant l'indexation afin que le démon ne scanne pas des fichiers transcript illimités sur le chemin de la requête.

Pour le paging répété via la route singulière héritée, définissez `--channel-idle-timeout-ms` à une valeur positive. Avec la valeur par défaut `0`, l'enfant ACP d'un workspace inactif — et le cache d'index de transcript in-process qu'il détient — est récupéré après chaque page, donc chaque page re-spaune l'enfant et reconstruit l'index en re-scanant le préfixe gelé entier (`O(snapshotSize)` par page). Un timeout positif maintient l'enfant en vie pendant la parcours du curseur afin qu'il réutilise son index de transcript en cache et sa configuration de relecture. Les pages forward et cursor qualifiées par workspace restent daemon-local ; seule une première page backward pour une session live traverse la barrière de flush.
...
**`--require-auth` et commodité de développement.** Les routes du flux device utilisent la gate de mutation stricte. Le loopback de confiance sans token par défaut peut démarrer et gérer un flux directement ; un embed non fiable sans identifiants retourne `401 token_required`. Définissez un `QWEN_SERVER_TOKEN` géré et passez `--require-auth` sur les hôtes partagés, les runners CI, les machines de développement distantes ou tout endroit où les processus locaux ne sont pas mutuellement de confiance.
...
Soumis à une gate stricte (primaire loopback de confiance ou identifiants opérateur requis). Connecte le serveur immédiatement via le `McpClientManager` actif et découvre ses outils.
...
| `401`  | `token_required`          | Requête non-fiable sans token refusée par la gate stricte                                           |

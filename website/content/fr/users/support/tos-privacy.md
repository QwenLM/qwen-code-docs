...
Qwen Code est un outil d'assistance IA open-source pour le codage, maintenu par l'équipe Qwen Code. Ce document décrit les conditions d'utilisation et les politiques de confidentialité applicables lors de l'utilisation des méthodes d'authentification et des services du modèle IA de Qwen Code.

## Comment déterminer votre méthode d'authentification

Qwen Code prend en charge deux principales méthodes d'authentification pour accéder aux modèles IA. Votre méthode d'authentification détermine les conditions d'utilisation et les politiques de confidentialité qui s'appliquent à votre utilisation :

1. **Qwen OAuth** - Connectez-vous avec votre compte qwen.ai
2. **API compatible OpenAI** - Utilisez des clés API provenant de divers fournisseurs de modèles IA

Pour chaque méthode d'authentification, différentes Conditions d'utilisation et Politiques de confidentialité peuvent s'appliquer selon le fournisseur de service sous-jacent.
...
| API compatible OpenAI      | Divers fournisseurs | Dépend du fournisseur d'API choisi (OpenAI, Alibaba Cloud, ModelScope, etc.)     | Dépend du fournisseur d'API choisi                      |
...
L'utilisation de votre code, y compris des invites et des réponses, pour l'entraînement des modèles d'IA dépend de votre méthode d'authentification et du fournisseur de services IA que vous utilisez :

- **Qwen OAuth** : L'utilisation des données est régie par la [Politique de confidentialité de Qwen](https://qwen.ai/privacy). Veuillez consulter leur politique pour obtenir des précisions sur les pratiques de collecte de données et d'entraînement des modèles.

- **API compatible OpenAI** : L'utilisation des données dépend entièrement du fournisseur d'API choisi. Chaque fournisseur possède ses propres politiques d'utilisation des données. Veuillez examiner la politique de confidentialité et les conditions d'utilisation spécifiques à votre fournisseur.

**Important** : Qwen Code n'utilise pas vos invites, votre code ou vos réponses pour l'entraînement des modèles. Toute utilisation des données à des fins d'entraînement serait régie par les politiques du fournisseur de services IA avec lequel vous vous authentifiez.
...
### 3. Comment basculer d'une méthode d'authentification à une autre ?

Vous pouvez passer de l'authentification OAuth Qwen à l'authentification compatible avec l'API OpenAI à tout moment :
...
3. **Variables d'environnement** : Configurez des fichiers `.env` pour une authentification automatique compatible avec l'API OpenAI

Pour des instructions détaillées, consultez la documentation sur [Configuration de l'authentification](../configuration/settings#environment-variables-for-api-access).
...
**Aone Code :** pour un clone dont l'origine est sur `gitlab.alibaba-inc.com`, exécutez `/review` depuis l'intérieur de ce clone — la plateforme est détectée depuis le remote et les sous-commandes fonctionnent, supportées par le CLI `a1` (au moins 0.1.90 — une installation plus ancienne est refusée au moment de l'authentification avec un message de mise à jour) — le numéro cible est l'id global de MR. `fetch-pr` récupère `refs/merge-requests/<id>/head` et construit le worktree + diff, donc la revue par les agents du worktree est inchangée, et `test-plan` fonctionne aussi — il lit la description de la MR via le même lecteur. Chaque exécution Aone est sans contexte disponible et plusieurs flux sont ignorés (plutôt que de toucher le repo homonyme de github.com) : `pr-context`/`comment-status` n'ont pas de support Aone (le plafond sans contexte disponible maintient un verdict Approve à Comment), `presubmit` n'a de support que pour la détection d'auto-PR (le compte `a1 auth whoami` vs l'auteur de la MR) et la dérive du head — ses vérifications CI et commentaires existants retournent neutres — l'Agent 0 est ignoré, et l'écriture `publish-assets` est ignorée. `--comment` **publie** la revue via le CLI `a1` : un commentaire par résultat inline, puis le commentaire de résumé. Aone n'a pas d'état natif de demande de modifications — sur ce verdict le commentaire de résumé porte un en-tête bloquant, et tout Critical inline effectivement publié bloque le merge via la porte de discussion tant que leurs discussions restent non résolues (lorsqu'aucun Critical inline n'a été publié, l'en-tête est consultatif et rien ne bloque mécaniquement le merge). Les commentaires publiés ne portent pas d'indicateur de commentaire IA — `a1` ne peut pas en définir — donc la gate de merge `ai_comment` dédiée du repo ne les suit pas. Le `a1 repo mr approve` natif est câblé pour un verdict Approve mais ne déclenche pas cette phase : le plafond sans contexte disponible maintient un verdict Approve à Comment (un verdict Request-changes publie toujours son résumé bloquant). Deux mises en garde pour les tours répétés : il n'y a pas encore de support de déduplication, donc un second tour `--comment` re-publie chaque résultat encore valide comme nouveau commentaire (la détection d'auto-PR EST supportée : revoir sa propre MR obtient le même downgrade de verdict que sur GitHub). Voir `docs/design/2026-08-15-review-aone-provider.md`.

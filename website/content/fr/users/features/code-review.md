...
Étape 3A : high, <=500 src ET <=3200 total : 14 agents              [14+ appels LLM]
           |-- Agent 0 : Fidélité aux issues et responsabilité de la cause racine
           |-- Agent 1a : Correction — analyse ligne par ligne
           |     (incl. vérifications de pièges linguistiques et de routage de wrappers)
...
| Agent 1a : Analyse ligne par ligne | Parcourt chaque hunk plus sa fonction englobante : conditions incorrectes, décalage d'un, `await` manquant, pièges spécifiques au langage, routage de wrappers/proxies                                                                                                                        |
...
Les trois agents de Correction sont **procéduraux** : chacun est défini par la façon dont il parcourt le diff (ligne par ligne / lignes supprimées / arêtes inter-fichiers), pas par une taxonomie de bugs — leur couverture est donc complémentaire au lieu de se chevaucher. Le même raisonnement divise **la qualité du code en trois** (3a/3b/3c) : un agent tenant une checklist de six éléments termine un élément — mesuré sur un fichier fortement réécrit, un agent tenant une checklist de huit éléments a trouvé 1 défaut sur 5 et le même modèle divisé en trois a trouvé les 5 — la checklist qualité est donc découpée là où les questions diffèrent réellement. Tous les agents s'exécutent en parallèle (l'Agent 1 lance 3 variantes procédurales, l'Agent 3 lance 3 tranches de checklist, et l'Agent 6 lance 3 variantes de personas simultanément, totalisant 14 tâches parallèles pour les revues de PR dans le même dépôt, plus 0 à 2 détecteurs Agent 8 quand le domaine du diff les appelle — soit 14 à 16 en pratique ; l'Agent 0 est ignoré pour les revues de diff local et de chemin de fichier, qui en exécutent 13 à 15 ; le mode léger inter-repos ignore également les Agents 1c et 7, exécutant 12 à 14).
...
Une fois qu'une PR dépasse 500 lignes de modifications **source** — ou plus de 3 200 lignes de diff au total, seuil au-delà duquel les onze lecteurs de l'ensemble du diff sont chacun trop dilués pour lire attentivement (une limite d'attention, pas une promesse de moins d'appels — les fichiers lourds et les détecteurs spécialisés peuvent faire coûter plus cher la 3B) — ce fan-out par dimension est remplacé par un fan-out **territoire × dimension** : le diff est découpé en chunks d'environ 400 lignes — les limites tombent sur les limites de hunk, et un hunk trop grand pour tenir est découpé uniquement au niveau d'une déclaration de premier niveau, jamais à l'intérieur d'une fonction — et chaque chunk reçoit son propre agent qui applique chaque dimension de revue à ce chunk uniquement.

La porte compte délibérément les lignes source plutôt que les lignes de diff. Le code de test, la prose et les lockfiles dominent la taille du diff — sur les 40 dernières PR fusionnées de ce dépôt, le diff médian est composé à 41 % de tests — donc une porte sur la taille brute découperait un changement de production de 173 lignes en territoires simplement parce qu'il a livré 489 lignes de nouveaux tests, laissant ce code de production avec un seul relecteur au lieu de dix lentilles (les agents de dimension de lecture de diff — douze moins Fidélité aux issues et Build et tests). Le chunking couvre toujours chaque ligne de toute façon, tests inclus ; ce que la porte décide c'est combien de relecteurs il y a et ce qu'on leur demande. Dix lentilles de lecture de diff parcourant toutes un seul grand diff relisent les mêmes premiers hunks dix fois ; un agent par chunk signifie que chaque ligne du diff a exactement un relecteur responsable. Chaque agent de chunk retourne un reçu `Covered:`, et un chunk sans reçu est relu avant que l'exécution ne proceed — ainsi « pas de bloqueurs » ne peut jamais être rapporté pour du code que personne n'a lu.
...
| ------------------------------------------------------------------- | --------- | -------------------------------- |
| Revue LLM (Agents 0, 1a, 1b, 2-6 + vérification + audit inverse itératif) | ✅        | ✅                               |
...
**Aone Code :** pour un clone dont l'origine est sur `gitlab.alibaba-inc.com`, exécutez `/review` depuis l'intérieur de ce clone — la plateforme est détectée depuis le remote et les sous-commandes fonctionnent, supportées par le CLI `a1` — le numéro cible est l'id global de MR. `fetch-pr` récupère `refs/merge-requests/<id>/head` et construit le worktree + diff, donc la revue par les agents du worktree est inchangée. Chaque exécution Aone est sans contexte disponible et plusieurs flux sont ignorés (plutôt que de toucher le repo homonyme de github.com) : `pr-context`/`comment-status` n'ont pas de support Aone (le verdict est plafonné à `COMMENT`), `presubmit` n'a de support que pour la détection d'auto-PR (le compte `a1 auth whoami` vs l'auteur de la MR) et la dérive du head — ses vérifications CI et commentaires existants retournent neutres — `test-plan` n'est pas supporté, l'Agent 0 est ignoré, et l'écriture `publish-assets` est ignorée. `--comment` **publie** la revue via le CLI `a1` : un commentaire par résultat inline, puis le commentaire de résumé. Aone n'a pas d'état natif de demande de modifications — sur ce verdict le commentaire de résumé porte un en-tête bloquant, et tout Critical inline effectivement publié bloque le merge via la porte de discussion tant que leurs discussions restent non résolues (lorsqu'aucun Critical inline n'a été publié, l'en-tête est consultatif et rien ne bloque mécaniquement le merge). Les commentaires publiés ne portent pas d'indicateur de commentaire IA — `a1` ne peut pas en définir — donc la gate de merge `ai_comment` dédiée du repo ne les suit pas. Le `a1 repo mr approve` natif est câblé pour un verdict Approve mais ne déclenche pas cette phase : le plafond sans contexte disponible maintient chaque verdict Aone à Comment. Deux mises en garde pour les tours répétés : il n'y a pas encore de support de déduplication, donc un second tour `--comment` re-publie chaque résultat encore valide comme nouveau commentaire (la détection d'auto-PR EST supportée : revoir sa propre MR obtient le même downgrade de verdict que sur GitHub). Voir `docs/design/2026-08-15-review-aone-provider.md`.
...
| -------------------------------- | ------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Agents de revue (Étape 3)        | 14 (+0-2)                       | Exécutés en parallèle ; inter-repos ignore les Agents 1c et 7 (12), local/fichier ignore l'Agent 0 (13)                                                                                           |
...
| **Total**                        | **~17-28 (~15-27)**             | 3A même dépôt : ~17-28 (typique ~17-19) ; inter-repos ou local/fichier : ~15-27 ; 3B varie avec les chunks (voir DESIGN.md)                                                                         |

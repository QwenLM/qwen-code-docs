...
Étape 5 :  Audit inverse itératif, réparti par chunk ;
           arrêt après 2 tours consécutifs sans résultat (limite 5)
...
**GitHub Enterprise :** la revue d'une URL de PR sur un hôte autre que `github.com` achemine chaque appel GitHub vers cet hôte — les sous-commandes de revue (`fetch-pr`, `pr-context`, `comment-status`, `presubmit`) acceptent `--host` et le définissent en code, de sorte qu'un hôte oublié ne peut pas rediriger silencieusement la revue vers `github.com`.
...
| Audit inverse itératif (Étape 5) | 2-5 (3A) ; tours × chunks (3B) | Deux tours consécutifs sans résultat pour arrêter (limite 5) ; 3B répartit un auditeur par chunk par tour          |
| **Total**                        | **~17-23 (~15-22)**            | 3A même dépôt : ~17-23 (typique ~17-19) ; inter-repos ou local/fichier : ~15-22 ; 3B varie avec les chunks (voir DESIGN.md) |

...
> [!note]
>
> Ce comportement s'applique aux exécutions CLI headless standard. Les sessions pilotées par ACP (intégrations IDE, Web Shell) pilotent le même runtime Goal via la méthode d'extension `sessionGoalControl`, et reçoivent chaque changement de statut sous forme de notification `session/update` portant `_meta.goalState` au lieu d'un événement de stream `goal_state`.
...
| `--output-style`             | Style de sortie pour cette exécution (`Concise`, `Proactive`, `Explanatory`, `Learning`, le nom d'un style personnalisé, ou `default` pour aucun) ; remplace `general.outputStyle`                                                                                                                                                                                                                                                                             | `qwen -p "query" --output-style Concise`                                 |

...
### Livraison des réponses IM

Une réponse de groupe @ ou de message direct terminée est checkpointée avant son premier envoi et retentée en arrière-plan sans réexécuter la tâche. Le backoff commence à cinq secondes, double et plafonne à cinq minutes. Une réponse est abandonnée après 16 tentatives de livraison échouées. Les reports locaux causés par un canal déconnecté ou une approbation d'appairage illisible utilisent une limite séparée de 32 vérifications consécutives, afin de ne pas consommer le budget de tentatives de transport. Une réponse est supprimée immédiatement lorsque l'autorisation de son expéditeur, de son groupe ou de son message direct est définitivement révoquée. La file d'attente contient jusqu'à 100 réponses et évince l'entrée la plus ancienne quel que soit son nombre de tentatives lorsqu'elle est pleine ; changer le profil DWS configuré vide la file d'attente. Ces suppressions sont signalées sur stderr du canal, pas envoyées sous forme de notifications de chat.

Les réponses IM en file d'attente de plus de 12 000 points de code sont tronquées, avec `[Response truncated for DWS delivery.]` ajouté au message que le destinataire voit. Les réponses aux commentaires de document et aux commentaires de todos natifs n'utilisent pas ce chemin de retry en file d'attente ni de troncature : chaque tour effectue un appel d'envoi de commentaire, les résultats inconnus sont ignorés, et les échecs définitifs sont propagés à la politique de retry de tour entrant existante. La réaction de fin marque l'achèvement de la tâche, pas la livraison de la réponse, donc une réponse IM retentée peut arriver après l'apparition de cette réaction.
...
Pour la vérification locale, envoyez un message direct depuis un autre compte, approuvez l'appairage si nécessaire, et vérifiez que la réaction de démarrage configurée apparaît pendant l'exécution de la tâche. Si une réaction de fin est configurée, vérifiez qu'elle remplace la réaction de démarrage après la fin de la tâche ; la livraison de la réponse peut se terminer plus tard comme décrit ci-dessus. Ajoutez ensuite un commentaire de document avec la notification de @mention activée. Le canal devrait réagir au message de notification, lire le document et publier la réponse finale sous le commentaire d'origine. Un commentaire avec la notification désactivée ne devrait produire aucune tâche.

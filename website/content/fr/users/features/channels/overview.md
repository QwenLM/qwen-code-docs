...
| `messagePrefix`          | Non               | Ne distribue que les messages utilisateur commençant par ce préfixe exact, sensible à la casse, après tout ` @mentions` en tête ; le préfixe et les espaces suivants sont retirés avant la distribution |
...
Deux comportements sont délibérés et méritent d'être connus avant d'activer le préfixe. Un message vocal dont DingTalk ou WeCom remplit la transcription compte comme du texte prononcé par l'utilisateur, il doit donc porter le préfixe comme tout autre message et est supprimé sinon — seule une note vocale non transcrite s'exécute comme média sans légende. Et le préfixe est vérifié avant l'appairage, donc le premier contact d'un expéditeur inconnu ou d'un groupe non approuvé doit également porter le préfixe ; sans cet ordre, chaque message sans préfixe dans un groupe occupé déclencherait une réponse d'appairage, ce qui est exactement le bruit que le préfixe est conçu pour supprimer. Communiquez le préfixe aux nouveaux utilisateurs hors bande, ou laissez les canaux d'appairage sans préfixe.

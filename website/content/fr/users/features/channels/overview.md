...
Une tâche reste sélectionnée pour recevoir le prochain message normal, mais d'autres tâches nommées peuvent continuer à s'exécuter simultanément. `/session new <name>` partage le workspace configuré, tandis que `/session new <name> --worktree` crée un checkout isolé pour cette tâche sous le répertoire `.qwen/worktrees/` du workspace du démon. Le démon vérifie le propriétaire du worktree persisté avant de rouvrir la tâche après un redémarrage ; un enregistrement de propriété manquant, modifié ou étranger échoue au lieu de déplacer silencieusement la tâche vers le workspace partagé. Créer ou sélectionner une autre tâche n'annule pas ni ne redirige le travail antérieur, et les résultats tardifs conservent leur étiquette de tâche d'origine. Une tâche occupée ne peut pas être fermée, mais son invite active peut être annulée avec `/session cancel [<name>]` via le comportement d'annulation de canal existant. Les tours mis en file d'attente indépendamment ne sont pas annulés, mais en mode de dispatch `collect`, les suivis mis en mémoire tampon derrière l'invite annulée sont supprimés par ce comportement d'annulation existant. La préparation des médias n'est pas ciblée. Les commandes de permission nues s'appliquent uniquement à la tâche sélectionnée, tandis qu'un ID de requête explicite peut répondre à une tâche inactive possédée. `/clear`, `/new` et `/reset` fonctionnent également sur une tâche worktree sélectionnée : la tâche obtient une nouvelle conversation tandis que son worktree et ses fichiers sont conservés. Une tâche worktree occupée refuse la réinitialisation jusqu'à ce que son invite se termine, et une tâche dont l'enregistrement du worktree a été endommagé signale l'échec sans toucher aux fichiers. La mémoire du canal reste limitée au chat plutôt qu'à une tâche nommée.
...
- `/btw <question>` — Poser une question latérale sans interrompre la tâche en cours ; questions textuelles uniquement jusqu'à 4096 caractères, nécessite une connexion agent avec support des questions latérales
...
- `/session new <name> --worktree` — Créer et sélectionner une tâche dans son propre worktree Git ; mode de tâches nommées gérées par le démon uniquement
...
Ce mode démarre des processus worker de canal groupés par workspace appartenant à `qwen serve`. Les workers se reconnectent au démon via le SDK et utilisent les mêmes adaptateurs de canal. Ils sont distincts du processus démon, ainsi le crash d'un adaptateur de canal ne fait pas planter le démon. Une sélection explicite de `--channel` est prioritaire et fait échouer le démarrage du démon si elle ne peut pas devenir prête. Lors d'un démarrage sans flag, le paramètre `serve.channels` du workspace principal fiable est restauré. Les workspaces secondaires ne restaurent pas indépendamment leur propre `serve.channels`. Sans aucune de ces deux sources, le démon ne charge pas les adaptateurs de canal ni ne réserve le bail jusqu'au premier `qwen channel set`.

La restauration automatique ignore les paramètres de démarrage invalides ainsi que les échecs de validation ou de bail qui surviennent avant le démarrage des workers, tout en préservant les paramètres sans rapport. Après l'échec du démarrage d'un worker, le démon ne continue que lorsque le nettoyage a réussi. Un timeout global de démarrage du runtime ou un arrêt de worker non confirmé suit toujours le chemin normal d'échec de démarrage ; le bail du service reste détenu tant que la terminaison du worker n'est pas confirmée. Consultez le log du démon pour les messages identifiant `serve.channels` lorsqu'un canal ne se restaure pas.

Les noms de démarrage stockés doivent être non vides, ne pas avoir d'espaces en début ou en fin, et ne pas contenir de caractères de contrôle ou invisibles non sûrs. Les entrées invalides sont ignorées individuellement et journalisées par leur index de tableau. Le démarrage ne renomme pas les instances ni ne réécrit la configuration. Le basculeur de démarrage du canal reflète le paramètre sauvegardé ; l'état d'exécution indique si le canal est en cours d'exécution.

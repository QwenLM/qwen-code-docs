...
Qwen Code est livré avec une sélection de thèmes prédéfinis, que vous pouvez lister en utilisant la commande `/theme` dans l'interface CLI :
...
Qwen Code vous permet de créer vos propres thèmes de couleurs personnalisés en les spécifiant dans votre fichier `settings.json`. Cela vous donne un contrôle total sur la palette de couleurs utilisée dans l'interface en ligne de commande.

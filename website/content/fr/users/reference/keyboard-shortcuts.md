...
| `Ctrl+S`    | Permet l'impression complète des réponses longues, désactivant le troncage. Utilisez le défilement arrière de votre terminal pour voir la sortie entière. |
| `Ctrl+T`    | Bascule l'affichage des descriptions des outils.                                                                      |
| `Maj+Tab`   | Parcourt les modes d'approbation (`plan` → `default` → `auto-edit` → `yolo`).                                         |
...
| `Ctrl+V`                                           | Colle le contenu du presse-papiers. Si le presse-papiers contient une image, elle sera sauvegardée et une référence à celle-ci sera insérée dans l'invite. |
...
| (plusieurs chiffres)| Pour les éléments dont le numéro est supérieur à 9, appuyez rapidement sur les chiffres pour sélectionner l'élément correspondant. |

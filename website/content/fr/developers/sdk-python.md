...
| `permission_mode`          | `default`, `plan`, `auto-edit`, `auto`, `yolo`              | Mode d'approbation d'exécution des outils. `auto` laisse un classificateur LLM approuver les appels d'outils ; `yolo` approuve automatiquement tous les outils ; à utiliser uniquement dans des environnements de confiance ou sandboxés. |

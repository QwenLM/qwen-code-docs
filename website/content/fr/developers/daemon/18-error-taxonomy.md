...
| `file_too_large`         | 413       | Un texte volumineux n'a pas de limite de lignes finie, un texte volumineux n'est pas en UTF-8, ou une écriture dépasse `MAX_WRITE_BYTES` (5 Mio). | Utilisez une limite de lignes finie pour du texte UTF-8 volumineux, utilisez une fenêtre d'octets, ou fractionnez l'écriture. |
| `hash_mismatch`          | 409       | Échec de l'`expectedSha256` de concurrence optimiste, ou un fichier a changé pendant une lecture stable. | Relisez le fichier et réessayez avec le nouveau hash/version.                                                                  |
...
| ------------------------------------ | ---- | -------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `SessionNotFoundError`               | 404  | sessionId introuvable dans `byId` (`code: "session_not_found"`) ou la session est en cours de fermeture (`code: "session_closing"`). | Pour `session_not_found` : recréez ou rattachez-vous ; la session a peut-être été récupérée. Pour `session_closing` : attendez et réessayez ; une fermeture simultanée est en cours. Les routes DELETE traitent `session_closing` comme un succès idempotent. |
...
| `RestoreInProgressError`             | 409  | `loadSession`, `resumeSession`, ou un id fourni par l'appelant sur `POST /session` entre en conflit avec un autre enregistrement qui possède déjà le même id. | Attendez le délai annoncé et réessayez la restauration ou le spawn demandé ; le nettoyage abandonné comporte un backoff dérivé du budget.                                                  |
...
| `restore_timeout`          | Le chargement/la reprise de session ACP a dépassé son budget de restauration dédié.          |

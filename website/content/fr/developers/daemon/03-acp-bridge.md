...
Chaque `WorkspaceRuntime` actif possède une instance `HttpAcpBridge`. La production tente de préchauffer l'enfant principal fiable pour la compatibilité ; les secondaires fiables ouvrent leur `AcpChannel` à la première commande runtime ou Session, et les secondaires non fiables ne peuvent pas démarrer ACP. Les routes principales héritées conservent leur comportement de compatibilité existant. Au sein du runtime, le bridge fournit des sessions multiplexées sur le canal, des `EventBus` par session, un `MultiClientPermissionMediator`, un adaptateur `BridgeFileSystem`, et des helpers orientés ACP (`spawnOrAttach`, `loadSession`, `resumeSession`, `sendPrompt`, `cancelSession`, `respondToPermission`, ainsi que des RPC extMethod pour le statut du workspace et le redémarrage MCP). Les bridges et les enfants ne sont jamais partagés entre runtimes de workspace.
...
| Constantes       | —                               | `DEFAULT_INIT_TIMEOUT_MS = 10_000`, `MCP_RESTART_TIMEOUT_MS = 300_000`, `DEFAULT_MAX_SESSIONS = 32`, `MAX_EVENT_RING_SIZE = 1_000_000`, `DEFAULT_PERMISSION_TIMEOUT_MS = 0`, `DEFAULT_MAX_PENDING_PER_SESSION = 64`.                                                                                                                                                                                     |

**Invariant `isDying`** : tout chemin de démontage doit définir `ChannelInfo.isDying = true` de manière synchrone **avant** d'attendre `channel.kill()`. `ensureChannel` traite un canal mourant comme absent et en lance un nouveau. Sans cet indicateur, un `spawnOrAttach` concurrent arrivant pendant la fenêtre de grâce SIGTERM (jusqu'à 10s) s'attacherait à un transport sur le point de se fermer et le sessionId de l'appelant retournerait une 404 à chaque suivi. Le démontage inclut les échecs d'initialisation, les échecs de spawn sur canal vide, l'expiration d'inactivité du workspace, et l'arrêt ; fermer la dernière Session libère uniquement son bail et programme la politique d'inactivité du workspace.
...
- `defaultEntry` est la Session logique réutilisable sous `sessionScope: 'single'`. La fermeture de session retire uniquement son bail de Session. Après que tout le travail de Session, restauration, contrôle de workspace, découverte, authentification et opération runtime soit terminé et qu'aucune fenêtre de keepalive explicite n'est en attente, un `channelIdleTimeoutMs` omis ou à zéro récupère l'enfant immédiatement. Le préchauffage simple est préservé pour la première utilisation et n'arme pas ce récupérateur immédiat à lui seul. Une valeur positive ou une fenêtre de keepalive active retarde la récupération ; le délai restant le plus long l'emporte.
...
- `DEFAULT_PERMISSION_TIMEOUT_MS = 0` permet aux permissions humaines et aux questions d'attendre indéfiniment par défaut. `permissionResponseTimeoutMs` active une limite horloge lorsque les opérateurs en ont besoin ; l'annulation par le votant, l'annulation de session et l'arrêt restent disponibles sans cela.
...
| `@qwen-code/qwen-code-core` — `ApprovalMode`, `TrustGateError`                     | `packages/channels/base/` (prévu, F4)        |
...
| `sessionRestoreTimeoutMs`                     | `60_000`                                           | Timeout de `loadSession` / `unstable_resumeSession` ACP ; par défaut 60s, et un timeout d'initialize configuré explicitement peut l'augmenter mais jamais le diminuer.      |
...
| `permissionResponseTimeoutMs`                 | `DEFAULT_PERMISSION_TIMEOUT_MS = 0`                | Temps horloge par requête pour le médiateur ; `0` le désactive.                                                                                                     |
...
| `channelIdleTimeoutMs`                        | `0`                                                | Délai de récupération automatique ; le préchauffage simple survit à sa première lecture de statut, et le keepalive actif peut l'étendre.                                                           |

Les restaurations expirées ne sont pas annulables dans le SDK ACP actuel. Le bridge maintient donc une barrière de règlement et une admission de capacité jusqu'à ce que la requête réelle se règle ou que son transport se ferme. Un résultat tardif est fermé exactement une fois et jamais enregistré. L'incertitude de nettoyage met en quarantaine uniquement le travail de session frais sur ce workspace ; le trafic de session existant et de contrôle de workspace continue jusqu'à ce que le canal se vide et soit recyclé.

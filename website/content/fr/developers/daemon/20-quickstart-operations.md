...
# 12. Garder un child ACP inactif réutilisable pendant au moins 60s après la fin du travail
...
| `--hostname <host>`                     | string                         | `127.0.0.1`                                      | Le non-loopback requiert un token        | Adresse de liaison. Valeurs loopback : toute adresse `127.0.0.0/8`, `localhost`, `::1`, `[::1]` ; `localhost` est résolu une fois et épinglé, et le mode trusted vérifie l'adresse réelle du listener avant la fin du démarrage. Les crochets de `[::1]` sont retirés automatiquement ; l'entrée `host:port` est rejetée avec une indication pour utiliser `--port`.                                                                                                  |
...
| `--enable-session-shell`                | boolean                        | `false`                                          | Bearer ou loopback trusted               | Active l'exécution directe de `POST /session/:id/shell`. Les appelants doivent également envoyer un `X-Qwen-Client-Id` lié à la session.                                                                                                                                                                                |
...
| `--allow-origin <pattern>`              | repeatable string              | none                                           | -                                        | Liste blanche CORS qui remplace le déni d'Origin par défaut. Les wildcards et les origins HTTP(S) non-loopback requièrent un token.                                                                                                                                                                                                   |
...
| `--channel-idle-timeout-ms <n>`         | non-negative integer           | `0`                                              | -                                        | Délai d'auto-reap du child ACP après la fin du travail du runtime. Le préchauffage simple est conservé pour la première utilisation. Les fenêtres de keepalive actives peuvent étendre le délai configuré ; le délai restant le plus long l'emporte.                                                                                                                                                                     |
| `--initialize-timeout-ms <n>`           | number                         | `10000`                                        | -                                        | Deadline de démarrage du child ACP (channel factory + handshake initialize) et timeout de requête par défaut (ms).                                                                                                                                                                                                          |
...
| Non-loopback HTTP(S) `--allow-origin` sans token                           | `Refusing to start with --allow-origin ... but no bearer token configured`                          |
...
Le shell statique est monté **avant** `bearerAuth` dans tous les modes de lancement — un navigateur ne peut pas attacher un en-tête `Authorization` à une navigation dans la barre d'adresse ou à une sous-ressource `<script src>`, donc le conditionner casserait simplement l'UI. Les appels API utilisent l'autorité du loopback trusted sur l'écouteur primaire sans token ou attachent le bearer configuré dans les modes authentifiés. Sur un bind non-loopback, le shell est en lecture seule sauf si `--allow-origin <origin>` est passé — les POST same-origin portent un en-tête `Origin` que le mur CORS rejette (403) — donc passez `--allow-origin` pour tout bind au-delà du loopback.
...
- **`() => actualPort` est une closure paresseuse (lazy closure).** `actualPort` est assigné dans le callback de `server.listen`. Le middleware `hostAllowlist` le lit à la demande, de sorte que les ports éphémères (`--port 0`) filtrent toujours correctement l'en-tête `Host`.

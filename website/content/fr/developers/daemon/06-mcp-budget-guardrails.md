...
- **`ServeMcpBudgetStatusCell.scope` est piloté par les capacités et compatible vers l'avant.** Les cellules d'instantané exposent `budgets[]`, et non un seul champ `budget?`. Avec `mcp_workspace_pool`, le runtime sélectionné émet `scope: 'workspace'` ; avec le pool désactivé ou indisponible, le gestionnaire hérité émet `scope: 'session'`. Les consommateurs doivent tolérer des valeurs `scope` inconnues supplémentaires en les ignorant plutôt qu'en échouant.

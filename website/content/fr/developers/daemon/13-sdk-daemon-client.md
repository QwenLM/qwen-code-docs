...
Le résultat terminal de l'opération contient les `results` ordonnés. Les cibles n'ont pas besoin d'être installées lors de la définition de `enabled` ou `disabled` : le daemon stocke une déclaration de nom et préserve cette politique d'activation lorsqu'une Extension portant ce nom est installée ultérieurement. Toutes les cibles modifiées partagent une génération de l'Extension Store. Lorsque `extension_activation_explicit_refresh` est annoncé, les opérations d'activation se terminent après le commit durable de la politique sans rafraîchir les sessions actives. Un appelant qui a besoin d'une application immédiate doit alors soumettre `workspace.refreshExtensionRuntime()` pour chaque workspace dont les sessions doivent appliquer le changement immédiatement ; le rafraîchissement est une opération distincte et peut être attendu ou laissé en arrière-plan. Un lot global par défaut modifie l'activation par défaut que chaque workspace enregistré hérite, sauf si ce workspace détient une surcharge exacte pour le nom (ou correspond à une règle de chemin héritée), et il n'a pas de rafraîchissement unique couvrant chaque runtime ; un lot de workspace modifie uniquement le runtime fiable sélectionné. Les anciens démons rafraîchissent déjà à l'intérieur de l'opération d'activation, donc les clients ne doivent pas soumettre le rafraîchissement supplémentaire sauf si la capacité est présente. Le réconciliateur de génération à 30 secondes reste un chemin indépendant de convergence éventuelle pour les workspaces que l'appelant n'a pas rafraîchis. Le `inherit` de workspace efface la surcharge exacte mais ne crée pas de déclaration pour un nom inconnu ; un effacement tout-inconnu réussit comme une opération vide. Les méthodes d'activation singulières restent limitées aux installations.

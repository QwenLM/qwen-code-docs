...
Rafraîchissement explicite de l'activation d'extension : `extension_activation_explicit_refresh` signifie que les opérations d'activation singulières et par lot se terminent au commit de politique durable sans rafraîchir directement les sessions actives. Les clients qui ont besoin d'une application immédiate doivent attendre le succès de l'activation et soumettre l'opération de rafraîchissement de runtime de workspace pour chaque workspace dont les sessions doivent appliquer le changement immédiatement ; un lot par défaut global modifie l'activation par défaut héritée par chaque workspace sauf si ce workspace détient une surcharge exacte pour le nom (ou correspond à une règle de chemin héritée), et il n'a pas de rafraîchissement unique qui couvre chaque runtime, donc les workspaces que l'appelant ne rafraîchit pas convergent uniquement lors du prochain passage du réconciliateur de génération toutes les 30 secondes. Les démons sans ce tag incluent déjà le rafraîchissement dans l'opération d'activation, donc les appelants ne doivent pas soumettre un rafraîchissement de compatibilité à cet endroit.
...
Mutation du workspace (Wave 4+) : `workspace_memory`, `workspace_agents`, `workspace_agent_generate`, `workspace_acp_preheat`, `workspace_tool_toggle`, `workspace_skill_settings_toggle`, `workspace_skill_settings_batch_toggle`, **`workspace_settings`** (conditionnel), `workspace_permissions`, `workspace_init`, `workspace_github_setup`, `workspace_trust`, `workspace_mcp_restart`, `workspace_mcp_manage`, `workspace_file_read`, `workspace_file_bytes`, `workspace_file_read_cursor`, `workspace_file_write`, `workspace_file_upload`, **`workspace_reload`** (conditionnel). Les deux tags de paramètres de Skill remplacent les tags retirés validés par le catalogue `workspace_skill_toggle` et `workspace_skill_batch_toggle`.

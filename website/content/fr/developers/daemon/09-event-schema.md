...
| `turn_error`          | S->C      | Un tour échoue                                                                                                       | `sessionId, message, code?, promptId?` ; même mécanisme de corrélation `promptId`. Pour les échecs `-32603` côté agent, `message` porte le détail propre du fournisseur lorsque l'agent en a fourni un (`data.details` / `data.message` / `data.error.message` imbriqué), et non le texte JSON-RPC générique. |

...
- [`09-event-schema.md`](./09-event-schema.md) - schéma d'événements typés v1 : les 53 types d'événements connus avec leurs payloads, réducteurs, compatibilité vers l'avant.
...
- **Enfant ACP** - l'enfant `qwen --acp` qui héberge le runtime de l'agent d'un workspace. La production tente de préchauffer l'enfant principal fiable pour la compatibilité ; les secondaires fiables démarrent à leur première commande runtime ou Session, et les secondaires non fiables ne démarrent pas ACP. Les routes principales héritées conservent leur comportement de compatibilité existant. Le bridge propriétaire multiplexe les sessions et les clients sur cet enfant.
...
- **Workspace** - un répertoire enregistré au démarrage du daemon, restauré depuis le magasin d'enregistrement, ou ajouté dynamiquement. `workspaceCwd` est le défaut principal hérité ; `workspaces[]` est le catalogue des runtimes isolés et leurs métadonnées de confiance/suppression.
...
| ----------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------- |
| Bootstrap et assemblage HTTP        | `packages/cli/src/serve/run-qwen-serve.ts`, `packages/cli/src/serve/server.ts`, `packages/cli/src/serve/routes/health.ts`, `packages/cli/src/serve/web-shell-static.ts`                                                                                                | [`02`](./02-serve-runtime.md), [`20`](./20-quickstart-operations.md)                                                   |
...
| Couche de transcription UI partagée | `DaemonUiEventType`, `DaemonSessionProvider`, `packages/web-shell/client/daemon/`                                                                                                                                                                                             | [`13`](./13-sdk-daemon-client.md), [`14`](./14-cli-tui-adapter.md), [`../daemon-ui/README.md`](../daemon-ui/README.md) |
...
| Auth / garde-fous réseau  | Le loopback sans token accorde au listener principal l'autorité d'opérateur local ; le non-loopback exige un bearer ; `--require-auth` étend le bearer au loopback et à `/health` ; la liste blanche d'hôtes et le refus CORS par défaut sont actifs. | [`12`](./12-auth-security.md), [`17`](./17-configuration.md)              |
...
| Schéma d'événements | `EVENT_SCHEMA_VERSION = 1` ; 53 types d'événements connus ; trames synthétiques sans ID pour les abonnés ; `_meta.serverTimestamp` horodaté par `EventBus.publish()` (avec `formatSseFrame()` en fallback pour les trames synthétiques). | [`09`](./09-event-schema.md), [`10`](./10-event-bus.md)                                                       |
| Capacités        | `SERVE_PROTOCOL_VERSION = 'v1'` ; les tags enregistrés et conditionnels sont définis par le registre faisant autorité.                                                                                                                      | [`11`](./11-capabilities-versioning.md)                                                                       |
| Shell de session | `POST /session/:id/shell` existe derrière `--enable-session-shell`, l'autorité bearer ou loopback fiable, et un `X-Qwen-Client-Id` lié à la session ; le tag de capacité est conditionnel.                                  | [`11`](./11-capabilities-versioning.md), [`17`](./17-configuration.md), [`20`](./20-quickstart-operations.md) |
...
| Consommateur daemon du Web Shell       | `packages/web-shell/client/daemon/` consomme le store de transcript du SDK via des providers et des adapters React.                                                         | [`14`](./14-cli-tui-adapter.md), [`../daemon-client-adapters/web-shell.md`](../daemon-client-adapters/web-shell.md)                                 |
...
- Le schéma d'événements v1 est additif. Les nouveaux types d'événements connus doivent être ajoutés à `DAEMON_KNOWN_EVENT_TYPE_VALUES` ; les anciens SDK doivent traiter les types inconnus comme compatibles vers l'avant.

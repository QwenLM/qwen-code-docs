...
En mode géré par le daemon, chaque canal mappe le trafic de chat entrant aux sessions du daemon sous un `SessionScope` configurable (`user`, `chat_thread` ou `single`). La valeur héritée Channel `thread` reste lisible et modifiable pour les configurations existantes, mais les nouvelles configurations Web Shell ne la proposent pas ; ceci est distinct du knob de création de session `single`/`thread` propre au bridge du daemon. Avec `sessionScope: "user"` et `multiSession: true`, `ChannelBase` ajoute un catalogue de sessions nommées persisté, indexé par canal, chat et expéditeur, tandis que `SessionRouter` conserve la session sélectionnée comme route de compatibilité. Les chargements de session nommée exacte n'utilisent jamais le chemin hérité de chargement-ou-remplacement. Un tour nommé réserve sa session exacte avant la préparation asynchrone et reste lié après les changements de sélection ultérieurs sans relier la route de compatibilité. Les tâches nommées peuvent s'exécuter concurremment, `/session cancel [<name>]` cible uniquement le prompt actif vérifié, et les commandes de permission en texte brut ne considèrent que la tâche sélectionnée. `/session new <name> --worktree` demande également une isolation Git worktree gérée par le daemon. Le daemon renvoie le chemin canonique du worktree ainsi que `worktreeState: "persisted-v1"` ; le bridge valide cette attestation avant d'enregistrer une route, et le rattachement répète la même vérification. Les tours nommés capturent également une étiquette de source de tâche à livraison uniquement : les messages directs utilisent `[task]`, les groupes utilisent `[sender · task]`, et les prompts de permission en texte exact incluent l'ID de requête. L'étiquette n'est pas ajoutée à la réponse du modèle ni à la transcription. L'adaptateur délègue à `DaemonChannelBridge`, qui délègue au `DaemonSessionClient` du SDK (voir [`13-sdk-daemon-client.md`](./13-sdk-daemon-client.md)). Chaque canal nommé doit résoudre vers exactement un workspace enregistré et fiable. Le worker utilise le cwd canonique de ce runtime, `QWEN_DAEMON_WORKSPACE` et la surcouche d'environnement ; la résolution de propriété ne fallback jamais vers le primary.
...
Le `ChannelWorkerManager` de longue durée possède la sélection commitée du daemon et les superviseurs groupés par workspace. Une sélection explicite `--channel` est prioritaire et reste fail-fast. Lors d'un démarrage sans flag, le paramètre `serve.channels` du workspace primaire fiable est restauré ; cela charge dynamiquement le runtime de canal, réserve le pidfile du service, résout l'appartenance du workspace et démarre les workers sélectionnés. Les workspaces secondaires ne restaurent pas indépendamment leur propre `serve.channels`. Sans aucune de ces deux sources de démarrage, le premier `PUT /workspace/channel` à gating strict effectue une initialisation paresseuse. `GET /workspace/channel` lit le snapshot du manager et `DELETE /workspace/channel` l'arrête de manière idempotente. Les helpers SDK sont `getChannelWorkerControl()`, `setChannelWorkerSelection()` et `stopChannelWorker()` ; l'entrée CLI est `qwen channel set` plus les variantes distantes `status` et `stop`.

Les noms de démarrage stockés doivent être non vides, sans espace blanc en début ou fin, et ne pas contenir de caractères de contrôle ou invisibles non sûrs. Les entrées invalides sont ignorées individuellement et journalisées par index de tableau ; le daemon ne les transforme pas en un autre nom d'instance ni ne réécrit les paramètres. Les arguments des workers utilisent `--channel=<value>`, ce qui conserve un tiret en début de nom. La gestion des canaux continue de rapporter les paramètres de démarrage persistés et l'état runtime réel.

Un champ de démarrage invalide ou une erreur de validation ou de bail avant le démarrage des workers saute la restauration automatique et journalise sa source `serve.channels` sans supprimer les paramètres non liés. Après l'échec d'un démarrage de worker, le daemon continue uniquement si le nettoyage réussit. Les timeouts globaux de démarrage runtime et les arrêts de workers non confirmés suivent le chemin existant d'échec de démarrage ; le bail du service reste détenu tant que la terminaison du worker n'est pas confirmée. Consultez le log du daemon pour les tentatives de restauration ignorées ou échouées.

...
Utilisez `exit_plan_mode` lorsque vous êtes en mode planification et que vous avez terminé de présenter votre plan d'implémentation. Cet outil invite l'utilisateur à approuver ou rejeter le plan et effectue la transition du mode planification vers le mode implémentation.
...
L'outil Exit Plan Mode fait partie du workflow de planification de Qwen Code. Lorsque vous êtes en mode planification (généralement après avoir exploré une base de code et conçu une approche d'implémentation), vous utilisez cet outil pour :

1. Présenter votre plan d'implémentation à l'utilisateur
2. Demander l'approbation pour procéder à l'implémentation
3. Effectuer la transition du mode planification vers le mode implémentation en fonction de la réponse de l'utilisateur
...
1. **Tâches d'implémentation** : Vous planifiez les étapes d'implémentation d'une tâche de codage
2. **Achèvement du plan** : Vous avez terminé l'exploration et la conception de votre approche d'implémentation
3. **Approbation utilisateur nécessaire** : Vous avez besoin de la confirmation de l'utilisateur avant de procéder à des modifications de code
4. **Tâches d'écriture de code** : La tâche implique l'écriture, la modification ou le refactoring de code

### Exemples d'utilisation appropriée :

- "Help me implement user authentication" → Utilisation après avoir planifié l'implémentation du système d'authentification
- "Add a new API endpoint for user management" → Utilisation après avoir conçu la structure du point d'API
- "Refactor the database layer to use TypeORM" → Utilisation après avoir planifié l'approche de refactoring

## Quand ne PAS utiliser cet outil

N'utilisez PAS `exit_plan_mode` pour :

1. **Tâches de recherche** : Tâches axées sur la compréhension ou l'exploration de code existant
2. **Collecte d'informations** : Lorsque vous recherchez, lisez ou analysez sans implémenter
3. **Tâches de documentation** : Lors de la création de documentation sans modifications de code
4. **Tâches d'analyse** : Lorsque vous fournissez des explications ou des révisions sans implémentation
...
### Exemple de planification de refactoring
...
- **Mode planification uniquement** : Cet outil ne doit être utilisé que lorsque vous êtes en mode planification.
- **Focus sur l'implémentation** : Utilisez-le uniquement pour les tâches impliquant l'écriture ou la modification de code.
- **Plans concis** : Gardez les plans ciblés et concis - visez la clarté plutôt que des détails exhaustifs.
...
L'outil Exit Plan Mode fait partie d'un workflow de planification plus large :

1. **Entrer en mode planification** : L'utilisateur demande ou le système détermine qu'une planification est nécessaire
2. **Phase d'exploration** : Analyser la base de code, comprendre les besoins, explorer les options
...
Ce workflow garantit des approches d'implémentation réfléchies et donne aux utilisateurs le contrôle sur les modifications importantes du code.
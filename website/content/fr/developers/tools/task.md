...
- `working_dir` (string, optionnel) : Épingle un agent nommé explicitement, non-fork, à un worktree git enregistré existant dans le dépôt actuel. Les lancements sans nom s'exécutent au premier plan car l'appelant possède le cycle de vie du worktree (voir `run_in_background`) ; un teammate nommé épinglé à un worktree s'exécute en concurrence et doit être arrêté avant que le worktree ne soit supprimé. Si `working_dir` et `isolation` sont tous deux fournis, `working_dir` est prioritaire.

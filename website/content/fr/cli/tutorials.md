...
Ce tutoriel montre comment configurer un serveur MCP, en utilisant le [GitHub MCP server](https://github.com/github/github-mcp-server) comme exemple. Le serveur GitHub MCP fournit des outils pour interagir avec les repositories GitHub, comme créer des issues et commenter des pull requests.
...
> [!CAUTION]
> Utiliser un token d'accès personnel largement scopé qui a accès aux dépôts personnels et privés peut entraîner une fuite d'informations du dépôt privé vers le dépôt public. Nous recommandons d'utiliser un token d'accès finement granulaire qui ne partage pas l'accès aux dépôts publics et privés.
...
Lorsque vous lancez Qwen Code, il lit automatiquement votre configuration et démarre le serveur GitHub MCP en arrière-plan. Vous pouvez ensuite utiliser des prompts en langage naturel pour demander à Qwen Code d'effectuer des actions GitHub. Par exemple :

...
拡張機能のスキルは所有者の名前で登録されます。上記のスキルは `my-first-extension:code-analyzer` となり、`/my-first-extension:code-analyzer` として実行します。`skills.disabled` はその名前または記述通りの `code-analyzer` のどちらでもブロックでき、`skills.enabled` はプレフィックス付きの名前でのみオプトインします。詳しくは [Extension Skills](../features/skills.md#extension-skills) を参照してください。

### 条件付きルールの追加

拡張機能は `rules/` ディレクトリも含めることができます。ルールは Markdown ファイルで、`paths:` フロントマターでどのファイルに適用されるかを指定し、ツール呼び出しが一致するファイルに触れた場合にのみプロンプトに入ります。
...
**拡張機能のルールは条件付きでなければなりません** — `paths:` のないルールはスキップされ、起動時の警告で名前が言及されます。ベースラインルールはすべてのリクエストに含まれることになりますが、それは次のステップの `QWEN.md` の注意で避けるように求めていることそのものです。ルールはプロンプト内で所有者によってラベル付けされて表示されます。`my-first-extension:rules/charting.md` のようになります。詳しくは [Rules](../features/rules.md) を参照してください。
...
> **これが最もコストの高い命令の配置場所です。** 拡張機能のコンテキストファイルは、拡張機能がアクティブな**すべてのセッションのすべてのリクエスト**のシステムプロンプトに連結されます。作業内容が拡張機能と関係があるかどうかに関わりません — 関連性のゲーティングもサイズ制限もありません。ある計測されたセッションでは、9つの拡張機能のコンテキストファイルが 9,989 トークンを占め、会話自体の前にリクエストが持つすべてのトークンの 21% に相当しました。コンテキストファイルは常に真実である少数の事実 — 拡張機能のアイデンティティ、その用語、ハード制約 — に絞り、シナリオ別のガイダンスは代わりに [スキル](../features/skills.md) に配置してください。スキルは名前と説明のみがリストされます — 同じ計測セッションでは、84のスキルが平均してそれぞれ約 55 トークンでした — 呼び出されたときに本文をロードし、[`paths:` でゲーティングされたスキル](../features/skills.md#optional-gate-a-skill-on-file-paths-paths)は一致するファイルに触れられるまでリストされさえしません。`/context detail` は各拡張機能のコンテキストファイルに名前を付けるので、自分のものがどの程度のコストかを確認できます。
...
CLI を再度再起動します。これで、拡張機能がアクティブなすべてのセッションのすべてのリクエストで、モデルが `QWEN.md` ファイルのコンテキストを持つようになります。上記の注意で短く保ち、シナリオ専用のことはスキルに委ねるよう求めている理由がこれです。

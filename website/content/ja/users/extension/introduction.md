...
認証が必要な非 GitHub ソース、ネストされたマーケットプレイス、サブモジュール、Git LFS ソースには Git 2.37 以降が必要です。Qwen Code は `http.curloptResolve` を使用して Git 接続を検証済みの DNS 結果に固定します。古い Git バージョンでは、匿名の公開 `https://github.com/{owner}/{repo}[.git]` ルートリポジトリのみがサポートされます。要求された ref をコミットに解決し、GitHub のソースアーカイブを同じ公開ネットワークおよびアーカイブ安全性チェックでダウンロードします。

古い Git のフォールバックはクローンではなくソースアーカイブからインストールするため、サブモジュールや Git LFS に依存するリポジトリはインストールできません。また、ダウンロードは圧縮状態で 100 MiB、アーカイブは 100,000 エントリ / 展開後 1 GiB / パスメタデータ 8 MiB（最大 100 個のシンボリックリンクから実体化されたファイルを含む）に制限されます。リポジトリ内の通常のファイルを直接対象とするシンボリックリンクは、シンボリックリンクの作成を許可するシステムでサポートされています。Windows では Developer Mode または管理者権限が必要になる場合があります。フォールバックは、ディレクトリを対象とするリンク、チェーンされたリンク、リンク先が存在しないリンク、絶対パスリンク、リポジトリ外を参照するリンク、ハードリンク、および POSIX のリテラルバックスラッシュ対象リンクを拒否します。他の Agent Plugin のインストールパスと同様に、シンボリックリンクは引き続き省略されます。リポジトリがリリースを公開している場合は、リリースベースのインストールが引き続き推奨されます。
...
- `workflows`: ワークフロースクリプトを含むディレクトリ、またはディレクトリと `.js` ファイルのリスト（デフォルト: `workflows`）。[カスタムワークフロー](#カスタムワークフロー) を参照してください。
...
これは 1 つのスキルを提供し、`gcp:pdf-processor` として登録されます — 拡張機能の `name`、コロン、そして `SKILL.md` が定義する名前です。`/gcp:pdf-processor` で実行します。`/skills` は一覧表示し、拡張機能の表示名でラベル付けします。マニフェストに表示名が宣言されていない場合は `name` にフォールバックします。

拡張機能のカスタムコマンド（上記の `/deploy` や `/gcs:sync`、ファイル名に基づいて名付けられ、競合時のみプレフィックスが付けられる — 下記の「競合解決」を参照）とは異なり、拡張機能のスキルは常に所有者を保持します。両方が `pdf-processor` を持つ 2 つの拡張機能は、一方が他方を隠すのではなく 2 つのスキルを提供します。プレフィックスはスキルの読み込み時に追加されるため、`SKILL.md` 内の `name` がディスク上で書き換えられることはありません。

スキルに名前を付ける設定は、2 つの綴形を非対称に扱います。`skills.disabled` はどちらの名前でもスキルをブロックし、`skills.enabled` はプレフィックス付きの名前でのみ有効化します。[拡張機能スキル](../features/skills.md#extension-skills) を参照してください。
...
拡張機能は、`workflows/` サブディレクトリに `.js` ファイルを配置するか、マニフェストの `workflows` にリストされたディレクトリやファイルに配置することで、ワークフロースクリプトを提供できます。ワークフローは、[`tools.workflowsEnabled`](../configuration/settings.md) 設定でワークフローが有効になっている場合にのみ表示されます（デフォルトではオフです）。インストール時の同意プロンプトには、いずれの場合でもワークフローが記載されます。
...
は、スクリプトが `name: 'deep-research'` を持つ静的 `meta` オブジェクトを宣言する場合に 1 つのワークフローを提供し、`gcp:deep-research` として登録されます — 拡張機能の `name`、コロン、そして `meta.name` です。`/gcp:deep-research` で実行し、別のワークフローから `workflow('gcp:deep-research')` で呼び出すか、モデルに `Workflow({ name: 'gcp:deep-research' })` で名前により実行させます。拡張機能スキルと同様に、拡張機能ワークフローは常に所有者を保持するため、プロジェクトやユーザーのワークフローを隠すことはありません。同じ拡張機能がその名前のスキルも提供する場合、スキルは `/gcp:deep-research` を保持し、ワークフローのスラッシュコマンドは `/gcp.gcp:deep-research` にリネームされます（競合する拡張機能コマンドと同様です）。`slashCommands.disabled` に `gcp:deep-research` として書かれたエントリは、両方を削除します。

各スクリプトは、静的な `export const meta = { name, description }` ブロックを宣言する必要があります。`description` はインストール時の同意プロンプトとコマンドリストに表示されます。ファイル名は `meta.name` と異なる場合があります。呼び出しは常にメタデータ名を使用します。複数のスクリプトが同じ `meta.name` を宣言する場合、最初に検出されたスクリプトが保持されます。500 文字を超える `description` は、表示される場所で短縮されます。

スクリプトは、ワークフローがいつ適用されるかを述べる文である `whenToUse` を宣言することもできます。
...
`whenToUse` を宣言したワークフローのみが、その説明とともにモデルに一覧表示されるため、モデルはリクエストが一致したときにそれを開始できます。各実行は引き続きワークフローの承認を経由します。これがない場合、モデルはワークフローを認識せず、あなたがそれを呼び出すか、名前で要求するか、別のワークフローがそれを呼び出すときに実行されます。`whenToUse` は `description` と同様に 500 文字を超えると短縮されます。また、スクリプト内に存在するため、変更すると次の更新で再度同意が求められます。

インタラクティブ UI では、`/gcp:deep-research` がワークフローを直接開始します。ヘッドレスモードおよび ACP 経由では、同じコマンドがモデルに名前で実行するよう要求し、承認が続きます。

モデルに拡張機能のワークフローを開始させつつ、モデル自身がスクリプトを書いて実行することはさせない場合は、[`tools.workflowNameOnly`](../configuration/settings.md)（または `QWEN_CODE_WORKFLOW_NAME_ONLY=1`）でデプロイし、名前でワークフローを許可します。たとえば `Workflow(name:gcp:deep-research)` です。このロックにより、モデルが開始するすべての実行がこのようなルールでアドレス可能になります。それ自体では何も承認しないため、許可していない名前の要求は引き続き行ってください。

検出は意図的に狭く制限されています:

- 各ディレクトリ内 directly にある `.js` ファイルのみが読み込まれます。サブディレクトリは無視されます。
- `meta.name` は小文字、数字、ハイフンを使用し、文字で始まり、最大 41 文字である必要があります。
- 宣言されたすべてのパスは拡張機能ディレクトリ内に残る必要があります。リンクされた拡張機能（`qwen extensions link`）は、シンボリックリンクされたワークフローファイルとディレクトリをスキップします。インストールされた拡張機能はコピーであり、各シンボリックリンクはすでにそれが指すファイルに置き換えられています。
- 256 KiB を超えるスクリプト、または有効な `meta` ブロックがないスクリプトは警告付きでスキップされます。

拡張機能をインストールすると、同意プロンプトにワークフローが一覧表示されます。更新時にワークフローが追加または削除された場合、ワークフローの名前や説明が変更された場合、またはスクリプトのコードが変更された場合は再度同意が求められます。コードのみが変更された場合、プロンプトに変更されたスクリプトが示されます。拡張機能ワークフローは、あなたが保存したワークフローと同じルールに従います。信頼できないフォルダとベアモードでは非表示になり、各実行は通常のワークフロー承認を経由します（スクリプトの開始部分が表示されます）。名前またはパスで実行されるワークフローの「常に許可」は、スクリプトのコンテンツに固定されたルールとして保存されます（`Workflow(name:gcp:deep-research,sha256:3f2a9c1d0b4e5f67)` など）。そのため、スクリプトが変更されると適用されなくなります。`sha256` なしで記述したルール（`Workflow(name:gcp:deep-research)` など）は、スクリプトのすべてのバージョンを許可します。

デフォルトの `workflows/` ディレクトリ内のファイルの編集は自動的に検出されます。他の宣言されたパス以下の変更は、`/reload-plugins` または再起動後に有効になります。

ワークフローを提供する Claude Code プラグインはインストール時に変換されます。`workflows` を宣言しないプラグインは `workflows/` ディレクトリを保持し、これがデフォルトとして検出されます。プラグインが `workflows` を宣言する場合、宣言されたファイルは相対パスを保持し、変換後の拡張機能のマニフェストに明示的に一覧表示されます。それらのファイルのみが検出されます。異なるディレクトリにある同じベース名を持つファイルは、それぞれの `meta.name` 値の下で区別されたままになり、プラグインが提供する `workflows/` ディレクトリはコピーされますが読み込まれません。宣言されたディレクトリ内のシンボリックリンクは、ターゲットがプラグイン内にとどまる場合、通常のファイルとしてコピーされます。

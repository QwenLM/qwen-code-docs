...
Qwen Code がコードレビュー、インシデント調査、監査、本番データの調査などリスクの高い作業を行う場合、自信はあるが誤った結論は、結論を出さないことよりもコストがかかります。チームの検証ポリシーを QWEN.md（[Memory](./features/memory.md) を参照）にエンコードすることで、誰のデフォルトも変更せずに、すべてのセッションに適用できます。

以下のルールは、マルチエージェントデプロイメントの本番レビューから適応されたものです。そこでは、自信はあるが誤った結論が繰り返し同じ失敗モードに起因していました。コードをクエリする代わりにコードから状態を推論すること、1 つの満たされた条件を証明として扱うこと、以前の発言を事実として繰り返すことです。

**1. プロジェクトの QWEN.md に検証ポリシーを追加する**

このテンプレートをリポジトリルートの `QWEN.md` に配置してコミットし、ポリシーをチーム全体に適用します。

```markdown
# 検証ポリシー

調査、トラブルシューティング、レビュー、および結論を導くあらゆるタスクにおいて、これらのルールを必須とします。

1. 結論を出す前に検証する。データまたはシステムの状態に関する主張は、信頼できるソース（データベース、API、ログ、コマンド出力）からのエビデンスで裏付ける。コードの読み取りのみから導かれた結論は「未検証の推論」としてラベル付けする必要があります。
2. チェーン全体を検証する。因果チェーンを列挙し、各リンクを確認します。1 つの必要条件が満たされたとしても、結論を証明するものではありません。
3. 会話履歴をリードとして扱い、事実として扱わない。以前のトランスクリプトの発言（自分自身の以前の主張を含む）はリードです。事実として繰り返す前に、ソースで再検証してください。
4. 順序立ててトラブルシューティングする。まず自分自身の変更、次にドキュメントと既知の問題、最後に外部依存関係を調査します。
5. レビューでは 100% エビデンスルール。レビューおよび監査タスクでは、エビデンスなしに結論を報告しないでください。検証可能なエビデンス（ファイル:行、クエリ結果、ログ抜粋）をすべての主張に添付します。
```

**2. 適切なスコープを選択する**

どのファイルが誰に適用されるかについては、[QWEN.md を作成する場所](./features/memory.md#where-to-create-qwenmd) を参照してください。プロジェクトルートの `QWEN.md` のコピーをコミットして、ポリシーがチーム全体をカバーするようにします。

> [!tip]
>
> - ルールを具体的で実行可能なものにします。「注意深く行う」よりも「すべての主張をクエリ結果で裏付ける」方が効果的です。
> - ポリシーは短く保ちます。QWEN.md は簡潔な場合により確実に遵守されます（[Memory](./features/memory.md) を参照）。
> - ルールをスタックに適応させます。チームが信頼できるソースとして扱う実際のクエリツール、ダッシュボード、またはログコマンドに言及します。
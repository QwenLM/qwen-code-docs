...
`.qwenignore` ファイルにパスを追加すると、このファイルを尊重するツールは、一致するファイルやディレクトリを操作から除外します。例えば、[`read_many_files`](../../developers/tools/multi-file) コマンドを使用する際、`.qwenignore` ファイル内のすべてのパスが自動的に除外されます。

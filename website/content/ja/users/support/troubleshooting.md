...
- **右クリックが機能しない、リンクが開かない、またはターミナルでテキストを選択できない**
  - **問題:** Qwen Code の実行中、ネイティブの右クリックコンテキストメニュー、OSC 8 ハイパーリンククリック（Ctrl+Click または URL のプレーンクリック）、およびターミナルネイティブのテキスト選択が機能しなくなります。
  - **原因:** `ui.mouseTracking` が有効な場合（デフォルト）、Qwen Code は SGR マウス追跡を通じてすべてのマウスイベントをキャプチャし、アプリ内のテキスト選択、クリックでの位置指定、行のホバー、およびビューポートのスクロールを実現します。ターミナルはすべてのマウスイベントをネイティブに処理するのではなく、アプリに転送します。
  - **解決策:** `settings.json` で `"ui.mouseTracking": false` を設定して、ネイティブの右クリックメニューとクリック可能な URL リンクを復元します。これにより、アプリ内のすべてのマウス操作が無効になります。Virtualized History（`ui.useTerminalBuffer: true`、デフォルト）では、ホイールでトランスクリプトをスクロールできなくなります。代わりに `Shift+↑/↓`、`PgUp/PgDn`、または `Ctrl+Home/End` を使用してください。ネイティブのターミナルスクロールバックも復元するには、`"ui.useTerminalBuffer": false` を設定します。再起動が必要です。

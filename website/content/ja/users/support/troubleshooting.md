...
- **tmux でのトラックパッドスクロールが会話のスクロールではなくプロンプト履歴を変更する**
  ...
  - **解決策:** スクリーンリーダーモードが無効になっている場合、`ui.useTerminalBuffer` が有効であることを確認してください。その後、`Shift+Up` / `Shift+Down` を使用するか、tmux がホイールイベントをアプリに転送する場合（`ui.mouseTracking` が必要）はマウスホイールを使用します。ホストのスクロールバックを優先する場合は、ホイールイベントに対する tmux のマウスバインディングを調整してください。

- **右クリックが機能しない、リンクが開かない、またはターミナルでテキストを選択できない**
  - **問題:** Qwen Code の実行中、ネイティブの右クリックコンテキストメニュー、OSC 8 ハイパーリンククリック、およびターミナルネイティブのテキスト選択が機能しなくなります。
  - **原因:** `ui.mouseTracking` が有効な場合（デフォルト）、Qwen Code は SGR マウス追跡を通じてすべてのマウスイベントをキャプチャし、アプリ内のテキスト選択、クリックでの位置指定、行のホバー、履歴項目の切り替え、およびビューポートのスクロールを実現します。ターミナルはすべてのマウスイベントをネイティブに処理するのではなく、アプリに転送します。Qwen Code は追跡中に独自の置き換えを提供します。シングルクリックで http(s) OSC 8 ハイパーリンクが開き、リンクやテキスト選択上の右クリックでアプリ内のコンテキストメニューが開きます。
  - **解決策:** `settings.json` で `"ui.mouseTracking": false` を設定して、ネイティブの右クリックメニューとクリック可能な URL リンクを復元します。これにより、アプリ内のすべてのマウス操作（アプリ内のリンクオープンやコンテキストメニューを含む）が無効になります。Virtualized History（`ui.useTerminalBuffer: true`、デフォルト）では、ホイールでトランスクリプトをスクロールできなくなります。代わりに `Shift+↑/↓`、`PgUp/PgDn`、または `Ctrl+Home/End` を使用してください。ネイティブのターミナルスクロールバックも復元するには、`"ui.useTerminalBuffer": false` を設定します。再起動が必要です。

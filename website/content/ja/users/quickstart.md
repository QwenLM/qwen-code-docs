...
[Node.js 20以上](https://nodejs.org/download)が必要です。`node -v`でバージョンを確認できます。未インストールの場合は、以下のコマンドでインストールしてください。
...
```
変更内容を説明付きのメッセージでコミットしてください
```

さらに複雑なGit操作もプロンプトで指示できます：
...
すべてのコマンドの一覧については、[CLI リファレンス](./features/commands)をご参照ください。

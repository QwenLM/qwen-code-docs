...
| `--memory-project-scope <mode>`         | `git-root`      | プロジェクトメモリのパーティショニングモード。`git-root`（デフォルト）は同じ Git ルートに解決されるワークスペース間でメモリを共有します。`workspace` は正確な登録ワークスペースディレクトリによってメモリをキーイングし、各デーモンワークスペースが独自の隔離されたメモリを持ちます。指定された場合は `QWEN_CODE_MEMORY_PROJECT_SCOPE` をオーバーライドします。認識されない環境変数の値は一度限りの警告とともに無視され、`git-root` にフォールバックします。`workspace` に切り替えても既存の git-root プロジェクトメモリは移行されません。それらのエントリは切り戻すまで表示されなくなります。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
| `--max-journal-events <n>`              | `10000`            | 進行中のライブジャーナル（現在の未完了ターン）に保持される生イベント数のセッションごとの上限。超えた場合、最も古いジャーナルエントリが削除され、`history_truncated` マーカーが先頭に追加されます。正の安全な整数である必要があります。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| `--max-journal-bytes <n>`               | `8388608`          | 進行中のライブジャーナルのセッションごとのバイト上限。超えた場合、最も古いジャーナルエントリが削除されます（少なくとも 1 つのエントリは常に保持されます）。正の安全な整数である必要があります。デフォルトは 8 MiB です。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
> **メモリプロジェクトスコープの注意事項。**
>
> - **デーモン vs. スタンドアロン CLI。** `--memory-project-scope`（および
>   `QWEN_CODE_MEMORY_PROJECT_SCOPE`）はデーモン管理ランタイムにのみ影響します。
>   同じディレクトリで起動されたスタンドアロンの `qwen` TUI は、環境変数がグローバルに
>   エクスポートされていない限り、引き続き git-root スコープを使用します。両方のエントリ
>   ポイントを一貫して保つには、ワークスペースの `.env` または `settings.env` にスコープ
>   をピン留めして、ワークスペースを読み取るすべてのプロセスが同意するようにします。
...
- **エージェントのテキスト読み取りは子プロセスローカルであり、ワークスペースファイルシステムのバウンダリではなく、通常の CLI 権限ルールに従います。** 直接の `read_file` は、登録されたワークスペース外のホストテキストパスに到達できます。外部パスはデフォルトで確認が必要ですが、許可ルールや承認モードによって自動的に承認される場合があります。承認された読み取りは、ワークスペースファイルシステムの返却出力、フルスナップショット、および大型テキストスキャンのキャップではなく、設定可能な CLI 出力制限を使用します。これは共有テキスト読み取りコンシューマーすべてに適用されるため、write、edit、notebook、sed、および artifact 操作が実行するプリリードは、ワークスペースファイルシステムの読み取り監査、シンボリックリンク拒否、および読み取り側の TOCTOU 保護とともにそれらのキャップを失います — 正確なリストについては[デザインドキュメント](../design/daemon-local-text-reads.md)を参照してください。確認ペイロードはファイルを読み取ることで構築されるため、ワークスペース外の差分は誰かが承認する前に**すべての**接続された SSE サブスクライバーにファンアウトされます — インタラクティブ CLI では、その内容はターミナルにいる人にのみ表示されます。認証されたデーモンクライアントも同じセキュリティプリンシパルとして扱ってください。HTTP ファイルシステムルートはワークスペーススコープのままであり、これらのパスを引き続き拒否します。エージェントの検出ツールの動作は変更されず、最終的な ACP `writeTextFile` コンテンツ書き込みはワークスペースファイルシステムを介して継続されます。

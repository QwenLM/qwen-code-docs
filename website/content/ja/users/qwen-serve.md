...
### Remote Web Shell とスマートフォンアクセス

```bash
qwen serve --hostname 0.0.0.0
```

`--token` も `QWEN_SERVER_TOKEN` も指定されていない場合、非ループバックのリスナーは 128 ビットのエフェメラルベアラートークン（22 文字の URL セーフな文字列）を生成し、起動時に一度だけ表示します。明示的なフラグ値が環境変数より優先されます。非ループバックバインドでは、明示的に空の値でも fail closed となり起動を拒否します。一方ループバックでは `QWEN_SERVER_TOKEN` をシャドウし、`--require-auth` の項で説明する信頼されたトークンなしモードのままにします。デフォルトのループバックの動作は変わりません。

ワイルドカードバインドでは、起動時に実際のインターフェースからトークンなしのローカル / ネットワーク URL を一覧表示し、生成されたトークン、またはインタラクティブなターミナルでの独自トークンについては、ラベル付きのプライベート LAN アドレス 1 つの**シークレットを含む QR コード**を表示します。ワイルドカードの列挙は、Local Control が広告するものと同じプライベート / リンクローカルの LAN 母集団のみをリストします（RFC 1918 / RFC 3927 IPv4、およびデュアルスタックバインドでは fc00::/7 ULA IPv6）。VPN アダプターやコンテナブリッジを除外するソフトウェアインターフェースフィルターは**名前ヒューリスティック**（[#9158](https://github.com/QwenLM/qwen-code/issues/9158)）であり、保証ではないため、k8s オーバーレイ名（`cbr0`、`cali*`、`vxlan.*`）がすり抜ける可能性があります。明示的な非ループバックバインドでは、選択したアドレス（ルーティング可能なアドレスを含む）を正確に表示し QR コード化します。コンテナ内では、列挙されたアドレスはコンテナ自身のインターフェースであるため、公開されたポートに到達するにはホストアドレスに置き換える必要があります。pty が割り当てられたコンテナはインタラクティブなターミナルとみなされるため、そのログはシークレットを含んだままです。QR をスキャンすると、URL フラグメントにベアラを含む組み込み Web Shell が開きます。ブラウザは URL から資格情報を削除し、タブごとの `sessionStorage` に保持します。あるいは、表示された URL を開いてトークンを入力します。欠落または期限切れの資格情報はリトライフォームを表示します。生成されたトークンは再起動時にローテーションされます。`--no-web` は QR を省略し、パイプ / リダイレクトされた stdout は安定したオペレータートークンを QR として再公開することはありません。アドレスはファイアウォールや NAT の到達性を意味するものではありません。自動的に公開 IP は検出されません。

組み込み Web Shell の同一オリジンの **HTTP** リクエストは `--allow-origin` なしで動作します。他のブラウザオリジンでは引き続きその許可リストが必要であり、`Origin: null` は引き続き拒否されます。WebSocket バックの機能（ターミナル、音声）および TLS 終端リバースプロキシ経由でデーモンに到達するブラウザは、明示的な `--allow-origin <origin>` エントリを必要とします。転送されたヘッダーが同一オリジンを確立するために信頼されることは決してありません。`Host` ヘッダーを書き換える平文 HTTP の中継も同様です。nginx のデフォルトの `proxy_set_header Host $proxy_host` と k8s Ingress はどちらも、デーモンがブラウザの `Origin` と一致しなくなった `Host` を見るようにします。ブラウザが実際に表示するオリジンに対して `--allow-origin <origin>` を渡すか、プロキシが `Host` をそのまま転送するように設定してください。**非ループバック**バインドでは、ポート変換のみ（`docker -p 8080:4170`）は何も必要としません。同一オリジンチェックは `Origin` を転送された `Host` に対して比較し、リッスンポートは参照しません。デフォルトの**ループバック**バインドではそうではありません。DNS リバンディングの Host 許可リストはデーモン自身のポートのみを受け付けるため、ポート変換するトンネル（`ssh -L 8080:localhost:4170`）はシェルドキュメントを含むすべてのリクエストに対して `403 Invalid Host header` で拒否され、`--allow-origin` でオーバーライドすることはできません。同じポートを転送するか（`ssh -L 4170:localhost:4170`）、非ループバックにバインドしてください。ベアラはデーモン API の完全な権限（コード実行を含む）を付与します。ターミナルのトークンと QR キャプチャはシークレットとして扱ってください。HTTP は暗号化されていません。既存の `--tls-cert` と `--tls-key` オプションは独自の証明書で HTTPS を提供します。
...
デフォルトの登録上限は 256 です。`QWEN_SERVE_MAX_WORKSPACES=25 qwen serve --workspace /path/to/project` を設定して下げてください。受け付けられる値は 1 から 256 までの 10 進整数です。無効な値は起動を失敗させます。これはデーモン起動時の設定です。プロジェクトの `.env` とプロジェクトの `settings.env` は変更できません。埋め込み呼び出し元は `ServeOptions.maxRegisteredWorkspaces` を設定でき、それは起動時環境より優先されます。

上限はプライマリワークスペース、登録されたセカンダリワークスペース、およびユーザー作成のスクラッチワークスペースをカウントします。内部の Conversations ランタイムは対象外です。動的登録は容量到達時に `409 workspace_limit_reached` を返します。`GET /capabilities` と `GET /daemon/status` は `limits.maxRegisteredWorkspaces` を公開します。

デフォルトの登録容量 256 では、標準デーモンは 1 つのワークスペースのみで起動した場合を含め、800 の固定グローバルセッション上限を使用します。同じセッションのデフォルトは 25 を超える任意の登録容量に適用されます。`--max-total-sessions` は引き続きそれをオーバーライドします。`0` は無効にします。25 以下の容量では、既存のデフォルトのままです。1 つの起動または復元されたワークスペースは無制限であり、複数場合はワークスペースごとの上限とその数から合計を導出します。登録の変更は合計を再計算しません。これは ACP 子のメモリポリシーを変更せず、800 の安全な同時セッションを保証するものではありません。

チャネル制御は独立して最大 25 のワークスペースオーナーをサポートし、`limits.maxChannelControlWorkspaces` として公開されます。移行は回復のために保持されるオーナーもカウントします。容量到達時には、新しいオーナーを追加する前に、古いオーナー上の選択されたすべてのチャネルを正常に停止してください。 oversized な移行は `409 channel_control_workspace_limit_reached` を返します。`--channel all` は引き続きプライマリワークスペースのみを選択します。チャネル SDK タイムアウトは変更されません。

永続化された登録ファイルはスキーマバージョン 1 を保持し、8 MiB 以内に最大 255 のセカンダリレコードをサポートします。古いエイリアスを含む生レコードは、忘れられるまで永続スロットを消費します。完全な有効な起動セットが設定された上限を超える場合、起動は失敗し、ファイルは変更されません。容量を下げるには、以前の容量で再起動し、十分な登録を忘れ、低い上限で再起動する前に明示的な `--workspace` 引数を減らしてください。古いバイナリにダウングレードする前に、ファイルをバックアップし、新しいバイナリを使用してセカンダリレコードを最大 24 件、256 KiB 以内に縮小してください。それ以外の場合、古いバイナリはそのストアを拒否し、明示的なワークスペースのみで起動します。それらのストア変更は書き込み前に失敗します。

### セッションのライブ状態ポーリングの調整

Web Shell はワークスペースセッションのライブ状態をデフォルトで **5 秒**ごとにリフレッシュします。異なる間隔を使用するには、デーモンを開始する前にデーモンの環境変数を設定します。

```bash
QWEN_SESSION_LIVE_STATE_POLL_INTERVAL_MS=10000 qwen serve --web
```

値はミリ秒単位の整数で、`1000` から `2147483647` までです。欠落または無効な値は `5000` にフォールバックします。間隔はそのデーモンが提供するすべてのワークスペースに適用されます。変更後はデーモンを再起動し、開いている Web Shell ページをリロードしてください。ローカルのセッションアクションと表示中のタブに戻ることは引き続き即座にリフレッシュできます。これはライブ状態のポーリングのみを変更し、完全なセッションカタログのポーリングは変更しません。

デーモンは有効な間隔を `/capabilities` の `sessionLiveStatePollIntervalMs` として公開します。Web Shell はその値を使用し、それを省略する古いデーモンに接続する場合は 5 秒にフォールバックします。SDK コンシューマーは独自のポーリングスケジュールを選択する際にその値を読み取れます。
...
| `--session-restore-timeout-ms <n>`      | `60000`            | ミリ秒単位の ACP セッションのロード / 再開デッドライン。`2147483647` までの正の整数である必要があります。`0` は無効です。省略した場合、デフォルトは 60 秒です。明示的に指定された `--initialize-timeout-ms` がそれより大きい場合はその値に引き上げられます。短い initialize タイムアウトが復元バジェットを下げることはありません。SDK と Web Shell は 10 秒および 15 秒のクライアント余裕を追加します。タイムアウトはリトライ可能な `504 session_restore_timeout` を返します。デーモン自身が終了したことを意味するものではありません。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| `--allow-origin <pat>`                  | —               | T2.4 ([#4514](https://github.com/QwenLM/qwen-code/issues/4514))。ブラウザクライアントのクロスオリジン許可リスト。繰り返し指定可能です。各値は `*`（任意のオリジン。Bearer トークンが設定されていない場合は起動を拒否。ループバックでの `--require-auth` を推奨。これにより `/health` も Bearer トークンで保護されます。デフォルトではループバックで事前認証のため。Web Shell の静的アセットはすべてのモードで事前認証のままなので、それらを削除するには `--no-web` を渡してください）または正規の URL オリジン（`<scheme>://<host>[:<port>]`。末尾のスラッシュ / パス / userinfo / クエリは不可）です。**サブドメインワイルドカード（`https://*.example.com`）は意図的にサポートされていません**。各サブドメインを明示的にリスト化するか、設定したトークン（および完全な強化のための `--require-auth`）と共に `*` を使用してください。一致したオリジンには CORS レスポンスヘッダー（`Access-Control-Allow-Origin`、`Vary: Origin`、メソッド、ヘッダー、max-age、および公開される `Retry-After`）が送信されます。一致しないオリジンには、現在と同様のエンベロープで 403 が返されます。`Origin: null`（サンドボックス化された iframe、file:// ドキュメント）は `*` を指定した場合でも常に拒否されます。プリフライトは `caps.features.allow_origin` 経由。ループバックのセルフオリジンヒットは影響を受けません。 |
| `--web` / `--no-web`                    | `true`          | ビルドされた Web Shell SPA をデーモンのルート（`GET /`、`/assets/*`、および `GET /session/<id>` のドキュメントナビゲーション）で提供します。これらのエントリポイントは Bearer 認証ゲートの**前**に登録されます。ブラウザは `<script>` サブリソースやアドレスバーのナビゲーションにトークンを付加できないため、シェルには秘密情報が含まれません。API リクエストはデーモンの通常の権限ポリシーに従います。設定されたトークンは `--require-auth` が設定されていない限りループバックの `/health` を除く通常の API ルートをゲートし、チャネル Webhook の ingress は常に独自の共有シークレットを使用し、トークンなしの信頼されたループバックのプライマリリスナーはフルオペレーターアクセスを持ちます。他のすべてのパスの SPA ディープリンクフォールバックは Bearer ミドルウェアの背後にあります。ループバック以外のバインドでは、UI が認証なしでアクセス可能である旨の 1 行の stderr 警告が表示されます。API のみのデーモンとして使用する場合は `--no-web` を使用します。ビルド時に Web Shell アセットが省略されている場合は効果がありません（デーモンはパンくずログを出力し、API のみで実行されます）。                                                                                                                                                                                                                         |
...
- **`--hostname 0.0.0.0` はトークンを要求** — トークンソースがどちらも指定されていない場合はトークンが生成されます。明示的に空のソースでも fail closed となります。
...
Issue [#4514](https://github.com/QwenLM/qwen-code/issues/4514) T2.9 では、15 秒のハートビート + AbortSignal ではカバーできない長時間実行 / リモートデプロイメントのギャップを埋める 2 つのオプトインフラグが提供されます。共有パーミッションレスポンスタイムアウトもここに記載されています。すべてデフォルトでオフになっています。
...
| `GET /workspaces/:workspace/session/:id/transcript`     | クライアントが選択したワークスペースからアクティブな永続化トランスクリプトを必要とする場合。前方およびカーソルページはデーモンローカルのままです。ライブセッションの後方最初のページは、永続化された末尾をフラッシュするために ACP を開始し、ワークスペース設定をロードする場合があります。登録された信頼されていないセカンダリワークスペースもこの読み取り専用パスを使用できます。機能タグ: `workspace_persisted_transcript`。 |
...
登録されたワークスペースの場合は、`client.workspaceById(workspaceId).getSessionTranscriptPage(sessionId, { cursor, limit })` または `/workspaces/:workspace/session/:id/transcript` を使用します。ワークスペース修飾メソッドは、SDK クライアントに置換可能な ACP トランスポートがある場合でも、常にネイティブ REST を使用します。そのカーソルはデーモンの存続期間のみ有効であり、デーモンの再起動後はページ 1 から再開する必要があります。ライブセッションの後方最初のページは、永続化された末尾をフラッシュするために ACP を開始し、ワークスペース設定をロードする場合があります。
...
`limit` はアクティブなチャットレコードをカウントし、出力されたリプレイフレームはカウントしません。1 つのレコードが複数の `session_update` イベントを生成する可能性があります。後方ページは、ターンとツール呼び出し / 結果の境界を保持するために、最大 `3 * limit` レコードに展開される場合があります。最初のレスポンスは JSONL スナップショットサイズをフリーズし、`hasMore` が true の間に `nextCursor` を返します。後のページはページ 1 以降の追加を無視しますが、ファイルが削除、切り詰め、置換、アーカイブされた場合、またはフリーズされたカーソルと競合する場合は `409` を返します。非常に大きなスナップショットは、デーモンがリクエストパスで無制限のトランスクリプトファイルをスキャンしないように、インデックス作成前に `413 transcript_too_large` を返します。

レガシーな単数ルートを通じて繰り返しページングする場合は、`--channel-idle-timeout-ms` を正の値に設定してください。デフォルトの `0` では、アイドル状態のワークスペースの ACP 子 — およびそれが保持するインプロセスのトランスクリプトインデックスキャッシュ — は各ページ後に回収されるため、各ページは子を再スポーンし、フリーズされたプレフィックス全体を再スキャンしてインデックスを再構築します（ページあたり `O(snapshotSize)`）。正のタイムアウトはカーソルウォークをまたいで子を存続させるため、キャッシュされたトランスクリプトインデックスとリプレイ設定を再利用します。ワークスペース修飾の前方およびカーソルページはデーモンローカルのままです。ライブセッションの後方最初のページのみがフラッシュバリアを越えます。
...
厳格なゲート（信頼されたループバックのプライマリまたはオペレーター資格情報が必要）。ライブの `McpClientManager` 経由でサーバーに即座に接続し、そのツールを検出します。
...
| `401`  | `token_required`          | 信頼されていないトークンなしのリクエストが厳格なゲートによって拒否された                                           |

...
`working_dir` の起動はフォアグラウンドで実行されます。Qwen Code はそのワークツリーのライフサイクルを所有していないためです。`subagent_type: "fork"` やバックグラウンド実行と組み合わせることはできません。`working_dir` と `isolation: "worktree"` の両方が指定された場合、Qwen Code は別のワークツリーを作成する代わりに、呼び出し元が所有するワークツリーを再利用します。

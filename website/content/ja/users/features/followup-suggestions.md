...
この機能は CLI と Web Shell の両方でエンドツーエンドで動作します。生成は自動でサーバーサイドで行われます。各ターンの完了後、デーモンがセッションストリーム上で提案を出力します（デフォルトでオン。`ui.enableFollowupSuggestions` を `false` に設定するとオフにできます）。Web Shell の composer はすでに `useDaemonFollowupSuggestion` フックを配線しているため、追加のホスト側の配線なしに提案の表示と受け入れが可能です。

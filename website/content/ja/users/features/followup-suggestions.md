...
提案は CLI の非対話モード（例：ヘッドレス/SDK モード）では表示されません。デーモンでは、生成はサーバーサイドで行われ、上記の条件を満たす毎ターンの後に実行されるため、提案を描画できないヘッドレスや SDK のクライアントは、ターンごとの LLM コストを避けるために `ui.enableFollowupSuggestions` を `false` に設定する必要があります。

...
> [!note]
>
> HTML エクスポートを開くと、`unpkg.com` からその Qwen Code バージョン専用のレンダラーとスタイルシートが読み込まれます。バージョンがまだ公開されていない場合やいずれのアセットにもアクセスできない場合は、ファイルに読み込みエラーが表示されます。Markdown、JSON、JSONL エクスポートは自己完結型です。
...
| `/peers` | 保留中のピアメッセージを確認する。信頼されたコントローラーを管理する | `/peers`、`/peers accept <id>`、`/peers deny all`、`/peers controllers`、`/peers revoke <id>` |
...
| `/output-style` | 応答の書き方を形作る出力スタイルを選択する | `/output-style`（ピッカーを開く）、`/output-style Concise`、`/output-style default`（スタイルなし） |
...
| `/goal` | ゴールを設定する — 検証ツールが確認するまで作業を継続する（[Goals](./goals.md) を参照） | `/goal <objective>`、`/goal edit <objective>`、`/goal pause`、`/goal resume`、`/goal clear` |
...
> [!warning]
>
> 信頼できるソースからのみ拡張機能（`/extensions install`）をインストールしてください。拡張機能には、Qwen Code 自体と同じ権限で実行される MCP サーバー、スキル、コマンドが含まれる場合があります。これらはあなたのファイル、API キー、会話データにアクセスできます。`/extensions install` は確認プロンプトを表示しません。
...
> [!note]
>
> インストールされた拡張機能のスキルもスラッシュコマンドであり、その名前にはオーナーが含まれます。`/pdf` ではなく `/rust:pdf` です。裸の形式はエイリアスではありません — 別のスキルが `pdf` という名前の場合、`/pdf` はそのスキルを実行します。`slashCommands.disabled` はどちらの綴りでもそのようなコマンドをブロックするため、名前がオーナーを含む前に書かれたエントリでも引き続き有効です。[拡張スキルの命名方法](./skills.md#how-extension-skills-are-named)を参照してください。
...
- レビュアーモデルは**ツールを実行できません** — ツールはリストレベルで削除されます（`/btw` と同じメカニズム）。そのため、レビューがコードを書いたりコマンドを実行したりすることはありません。すべての主張は表示されているトランスクリプトに根ざしている必要があります
...
| `qwen sessions controllers` | 信頼されたコントローラートークンを管理する | `qwen sessions controllers add --label <name>`, `qwen sessions controllers list` |
...
以下の列を持つテーブル: NAME, KIND, PID, AGE, DIRECTORY.

KIND はセッションを登録したものを示します — ターミナルで操作している場合は `tui`、Qwen Code セッションではないプログラム（音声フロントエンド、リレー）の場合は `external`、別のプログラムが駆動するセッションの場合は `headless` または `serve` です。複数の `serve` または `headless` 行が1つの PID を共有することがあります。`qwen --acp` の子はすべてのセッションを1つのプロセスでホストします — デーモンが生成した場合は `serve`、クライアントが直接駆動している場合は `headless` です — そしてそれぞれが個別に登録されます。これは NAME や DIRECTORY と同様に自己報告です。ここにあるすべてのフィールドは、それが説明するプロセスによって書き込まれたものであり、セッションが何を許可されているかに依存するものではありません。レコード形式と独自のプログラムを登録する方法については、[Cross-Session Protocol](./cross-session-protocol.md) を参照してください。
...
## 6. 実行中の別セッションへのメッセージ送信

同じマシン上の2つのインタラクティブセッションは互いにメッセージを送信できます。この機能は実験的で、**デフォルトではオフ**です。`settings.json` で有効にして再起動してください。

```json
{ "agents": { "crossSessionMessaging": true } }
```

有効にすると、あるセッションのモデルは `list_agents` で他のセッションを発見できます — 各セッションは `sessions` の下に `qwen sessions ps --json` が記録する `name` で表示されます（テーブルビューでは長い名前が切り詰められる場合があります）— `send_message` でその名前を `to` として指定してアドレス指定します。2つのセッションが同じ名前を共有する場合、`list_agents` はそれぞれに短い `[ref]` を付けて表示し、送信時にはそれを含める必要があります（`name [ref]`）。どちらとも解釈可能な裸の名前は、推測ではなく拒否されます。`list_agents` はセッション自身の名前も `self` の下に報告し、`to: "*"` は引き続き「自分の Agent Team のチームメート」を意味し、他のセッションには届きません。

メッセージは別のセッションから来たものとして受信セッションに届き、そのユーザーからのものではなく、そこであなたの権限を一切持ちません。受信セッションは自身の権限設定の範囲内でのみメッセージを処理します。ユーザーは `agents.crossSessionInbound`（`accept`、`hold`、`refuse`）で受信メッセージの処理方法を選択できます。未設定の場合、メッセージは両方のセッションが同じレビュークラスにある場合にのみ配信されます。つまり、両方が各アクションをレビューするモード（デフォルトまたはプランモード）であるか、両方がアクションごとのレビューなしに一部のアクションを適用するモード（auto-edit、auto、または yolo）である場合です。他のクラスのセッションからのメッセージ、またはどのクラスにいるかを明示しない送信者からのメッセージは、レビューのために保留されます — 双方向です。各アクションをレビューするセッションは、レビューしないセッションからのメッセージを保留します。そのメッセージは誰も監視していないモデルによって書かれたものであり、アクションごとのプロンプトはアクションを保護するものであり、セッションが何を吹き込まれるかを保護するものではないためです。保留されたメッセージは受信セッションで `/peers` を使用して一覧表示および解放され、モードの違いのみが原因で保留されたメッセージは、両者が一致すると自動的に解放されます。

リポジトリは、そこで開かれたセッションをより慎重にすることができます。決して緩くはなりません。ワークスペースの `.qwen/settings.json` は `agents.crossSessionInbound` を `hold` または `refuse` に、または `agents.crossSessionMessaging` を `false` に設定でき、その値はユーザー設定の緩い値に優先します。あなたの設定を緩めるワークスペース値（`accept`、またはスイッチの `true`）は警告とともに無視され、CLI が認識しない値は、それが有効な値である場合は常にすべてのメッセージを保留します。システム設定は、他のすべての設定と同様に、これらすべてに優先します。

保留は永久に待ち続けるわけではありません。誰も判断しないメッセージは `agents.crossSessionHeldExpiry` — `1m`、`5m`、`10m`、または `never`（デフォルトは5分）— 後に期限切れになり、送信セッションには判断がなかったことが通知されます。この設定を短くすると、すでに待機中のメッセージにも適用されます。

セッションがインボックスをバインドできない場合 — ランタイムディレクトリが存在しない、別のユーザーが所有している、または読み取り専用（コンテナ内で起こり得る）— まずテンポラリディレクトリ下のプライベートディレクトリを試み、それも失敗した場合のみインボックスなしで開始します。その場合、セッションは起動時にその旨を表示し、`/peers` は理由と変更方法を繰り返します（通常は `XDG_RUNTIME_DIR` または `TMPDIR`）。

2つのセッションが同じインボックスアドレスを解決することもあります。アドレスはプロセス ID でキーイングされ、プロセス ID はランタイムディレクトリを共有するコンテナ間で繰り返されるためです。2番目に開始したセッションは、使用中のアドレスを引き継ぐ代わりに隣接するアドレスを取得するため、どちらも到達不能になりません。ピアは影響を受けません。ピアはセッションレジストリからセッションのアドレスを読み取り、派生しません。

`send_message` の呼び出しは、メッセージが別のセッションに渡されたことのみを確認します。その後の状態はレシートとして後から届きます。保留、拒否、拒絶、期限切れ、または宛先間違い（宛先が変更された — 再度エージェントを一覧表示）の場合、または保留後に解放された場合、送信セッションのトランスクリプトに通知が表示されます（`Message to <name>: …`）。拒否（declined）、拒絶（refused）、ドロップ（dropped）は3つの異なる応答です。拒否は誰かがメッセージをレビューしてノーと言ったことを意味します。拒絶はそのセッションの `agents.crossSessionInbound` が `refuse` であり、誰もそれを全く見ていないことを意味します。ドロップは、それらのいずれよりも前にインボックスがメッセージを拒否したことを意味します（下記参照）。最初のドロップは即座に回答され、残りは数秒ごとに1つのレシートにまとめられ、それぞれが代表するメッセージに名前を付けます。そのため、連続しても1行ずつではなく数行で済みます。送信したモデルには通知されません。別のセッションが返信した場合、その返信はクロスセッションメッセージとして届きます。

### フラッド対策

セッションは、1つの送信者から一度に最大30メッセージを受け付け、その後2秒ごとに1メッセージを受け付けます。また、すべての送信者合わせて一度に最大32メッセージを受け付け、その後1秒ごとに1メッセージを受け付けます。2番目の制限が存在する理由は、送信者は自分自身に名前を付けるためです。その名前をローテーションすると、最初の制限からは新しい許可量が得られますが、2番目の制限からは得られません。2番目の制限が最初の制限よりわずかに大きいのは、受け付けられたメッセージごとにレシートが生成され、セッションは一度にそれほど多くのレシートを送信できないためです。別のセッションからのメッセージで、30秒以内にその送信者の前のメッセージを一言一句繰り返すものも拒否されます。1つの文でループするモデルは毎回新しいメッセージIDを発行するため、テキストがそれを捕捉します。セッションが開始したスクリプトからのメッセージと信頼されたコントローラーからのメッセージは、繰り返しチェックから免除されます。同じ行を2回報告するフックは2つの事実を報告しており、「continue」を2回言う人は2回意味しているためです。どちらもレート制限の対象となります。最後に、受け付けられたものの、セッションにすでに50メッセージが待機しているためにキューに入れられないメッセージも拒否されます。

この方法で拒否されたメッセージは、保留されず、モデルに表示されず、記録も残らないため、送信者は後で再試行して成功できます。受信セッションは、送信者ごとに1分間に1回まで、その行が代表するもののカウントとともに、トランスクリプトにその旨を表示します。送信セッションは、バーストコストとなったすべてのメッセージに名前を付ける1つのレシートを受け取り、そのトランスクリプトには、まだ重要な内容を後の1つのメッセージにまとめて再送信するよう記載されます。

送信側は結果を待ちません。各セッションは各アドレスに送信したものを追跡し、受信者がドロップするような送信を拒否するため、モデルはメッセージが書かれた後ではなく前にバッチ処理するように指示されます。また、受信者は拒否するつもりのメッセージに接続を消費しません。

### インボックス認証とスクリプトによる注入

各セッションのインボックスにはセッションごとのトークンが必要です。接続は、メッセージが読み取られる前に最初の行でそれを提示する必要があり、セッションは同じレジストリレコードを通じて自動的にトークンを交換します。トークンサポートなしのビルドのセッションは、新しいセッションから受信できますが、それへの送信は破棄されます。

セッションは自身のインボックスアドレスとトークンを `QWEN_CODE_MESSAGING_SOCKET` および `QWEN_CODE_MESSAGING_TOKEN` として子プロセスにエクスポートするため、セッションが実行するスクリプトやフックがそれにメッセージを送信できます。これはどこにも公開されない2番目の_子_トークンです。セッションが開始したプロセスのみがそれを保持できるため、それで届いたメッセージは別のセッションのものではなく、セッション自身のものとして認識されます。
...
各注入に新しい `msgId` を与えてください。受信ゲートはすでに処理済みの ID を記憶しているため、1つを再利用するフックは初回に配信され、それ以降のすべての実行でサイレントに重複排除されます。

同じ _テキスト_ を繰り返すことは問題ありません — 上記の繰り返しチェックはセッション自身のプロセスには適用されません — が、レート制限は適用されるため、ループ内のフックは他のフラッドと同様にドロップされます。

注入されたメッセージもまだインバウンドゲートを通過し、ユーザーからのものではないとマークされますが、ゲートはそれがセッション自身のプロセスから来たことを知っています。モードパリティのデフォルトでは、レビューなしで配信されます（同じ位置のピアは保留されます）が、`agents.crossSessionInbound` の明示的な `hold` または `refuse` は他のものと同様にそれに適用されます。モデルはそれを `<cross_session_message from="own process" origin="own-process">` として、それがセッションが実行したスクリプトまたはフックから来たものであり、ユーザーからのものではないという通知付きで受け取ります。

### 信頼されたコントローラー

上記のルールは、どのレビュークラスにいるかを明示しない送信者からのメッセージを保留します。そして Qwen Code セッションでないプログラムには、明示するクラスがありません。未知の相手には正しいデフォルトですが、あなたが選んだプログラムには正しくありません。音声フロントエンド、ディクテーションブリッジ、あなた自身の指示を中継する自動化デーモンは、すべてのメッセージが駐車され、それぞれを手動で承認することは目的に反します。

そのようなプログラムにトークンを発行して配信を許可します。

```bash
qwen sessions controllers add --label voice-bridge
```

トークンは1回だけ表示され、どこにも保存されません。Qwen ホーム下のファイルはその SHA-256 ハッシュのみを保持するため、後でそのファイルを読み取るものはトークンを提示できません。コマンドがトークンを表示したときに、コントローラー自身の設定に配置してください。

コントローラーは他の送信者と同じ方法でトークンを提示します — 接続の最初の行として — セッションレジストリからソケットパスを取得します（`qwen sessions ps --json` はライブセッションごとに1つのレコードを出力し、`ipcPath` がアドレスです）。
...
付与されたトークンで届いたメッセージは、どちらの側がどのレビュークラスにあっても、メッセージごとのレビューなしで配信されます。ただし、明示的な設定には従います。`agents.crossSessionInbound` の `hold` は他のものと同様にそれを駐車し、`refuse` はそれを拒否します。付与は1つのセッションではなくあなたの Qwen ホームに属するため、コントローラーは実行中のどのセッションにも到達し、セッションは接続ごとにファイルを再読み取りします。1つを発行または取り消すと、次の接続で有効になり、再起動は不要です。

```bash
qwen sessions controllers list          # ID、ラベル、追加された時期
qwen sessions controllers remove c_1a2b # 1つを取り消す
```

`/peers controllers` と `/peers revoke <id>` はセッション内から同じことを行います。付与を通じて届いたメッセージは `Message from a trusted controller (voice-bridge)` として表示され、`hold` 設定がそれを駐車した場合は `/peers` に `[controller] voice-bridge` として表示されます。

モデルはそのようなメッセージを `<cross_session_message from="controller" origin="controller" controller="voice-bridge">` として、あなた自身の指示を中継しているという通知付きで受け取ります。そして他のすべての起点に適用されるのと同じ2つの禁止があります。許可設定、QWEN.md、または設定をメッセージのリクエストによって編集することはできず、メッセージを保留中の確認プロンプトに対するあなたの承認として扱うことはできません。コントローラーは次に何をすべきかを言うことができます。あなたの代わりにプロンプトに答えることはできません。

トークンを保持する人は誰でもそのコントローラーとして送信できるため、他の認証情報と同様に扱ってください。1つのプログラムに与え、共有設定に入れず、そのプログラムが完了したら取り消してください。

### ACP で駆動されるセッション

`qwen --acp` の子は、自身がホストする各セッションを登録します — デーモンがプロセスを生成した場合は `serve` として、エディタや他のクライアントが `qwen --acp` を直接駆動している場合は `headless` として — セッションは `qwen sessions ps` や他のセッションの `list_agents` に他のセッションと同様に表示されます。送信可能です。モデルが `send_message` を呼び出して、開いているターミナルに到達できます。それら複数つが1つのプロセスと1つのインボックスを共有するため、送信者は意図するセッションを名前で指定する必要があります — すべての Qwen Code セッションはそれを自動的に行います。

そのようなセッションに送信されたメッセージは、保留ではなく拒否されます。保留は人間への質問であり、駆動されるセッションの代わりに保留メッセージリストを監視している人はいません。送信者は期限切れを待つ代わりに即座に通知されます。そのようなセッションで保留されたメッセージがどこに表示されるべきかは、まだ決定されていません。

セッションは、自身の設定で `agents.crossSessionMessaging` がオンの間のみ登録されます。オフの場合は非表示のままです。誰もメッセージを送信できないセッションを一覧表示する唯一の理由は、決して応答しないアドレスを宣伝することになるからです。

### Qwen Code セッションでないプログラム

上記のすべてはセッション間で機能しますが、そのどれもが特定のセッションに固有のものではありません。自身のレジストリレコードを書き込み、同じ方法でインボックスをバインドするプログラムは、`qwen sessions ps` と `list_agents` に一覧表示され、`send_message` から名前でアドレス指定でき、送信したものの配信レシートを受け取ります。音声フロントエンド、リレー、ビルドウォッチャーなどです。リストでそれが何であるかが分かるように、`kind: "external"` を記録する必要があります。

[Cross-Session Protocol](./cross-session-protocol.md) は、それを作成するための契約です。レコードスキーマと liveness の判断方法、ソケットパスとフレーミング、認証行、すべてのフレームフィールド、レシート状態とその遷移、およびレシーバーがモデルがメッセージを見る前にメッセージに対して行う処理について記述しています。

Node プログラムはこれらすべてを手作業で書く必要はありません。`@qwen-code/sdk/peer` がその契約を実装しています。`PeerEndpoint.start({ name })` がレコードを公開してインボックスをバインドし、`list()` と `send()` がセッションを名前でアドレス指定し、`onMessage` が送信されたメッセージを受け取ります。
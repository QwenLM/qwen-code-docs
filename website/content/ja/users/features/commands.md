...
> [!note]
>
> HTML エクスポートを開くと、`unpkg.com` からその Qwen Code バージョン専用のレンダラーが読み込まれます。バージョンがまだ公開されていない場合やレンダラーにアクセスできない場合は、ファイルに読み込みエラーが表示されます。Markdown、JSON、JSONL エクスポートは自己完結型です。

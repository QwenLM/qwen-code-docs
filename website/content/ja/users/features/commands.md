...
| `/peers` | このマシンの他の Qwen Code セッションから保留されているメッセージを確認する | `/peers`、`/peers accept <id>`、`/peers deny all` |
...
| `/goal` | ゴールを設定する — 条件が満たされるまで作業を継続する（[Goals](./goals.md) を参照） | `/goal <condition>`、`/goal clear` |
...
有効にすると、あるセッションのモデルは `list_agents` で他のセッションを発見できます。各セッションは `sessions` の下に、`qwen sessions ps --json` が記録する `name` で表示されます（テーブルビューでは長い名前が切り詰められる場合があります）。`send_message` でその名前を `to` として指定してメッセージを送信します。2つのセッションが同じ名前を共有する場合、`list_agents` はそれぞれに短い `[ref]` を付けて表示し、送信時にはそれを含める必要があります（`name [ref]`）。どちらとも解釈可能な裸の名前は、推測ではなく拒否されます。`list_agents` はセッション自身の名前も `self` の下に報告します。また、`to: "*"` は引き続き「自分の Agent Team のチームメート」を意味し、他のセッションには届きません。

メッセージは別のセッションから来たものとして受信セッションに届きます。そのユーザーからのものではなく、そこであなたの権限を一切持ちません。受信セッションは自身の権限設定の範囲内でのみメッセージを処理します。ユーザーは `agents.crossSessionInbound`（`accept`、`hold`、`refuse`）で受信メッセージの処理方法を選択できます。未設定の場合、受信セッションがまだ各アクションをレビューするモード（デフォルトまたはプランモード）の場合、または両方のセッションがアクションごとのレビューなしにアクションを適用するモードの場合はメッセージが配信されます。それ以外の場合、メッセージはレビューのために保留されます。保留されたメッセージは、受信セッションで `/peers` を使用して一覧表示および解放されます。

`send_message` の呼び出しは、メッセージが別のセッションに渡されたことのみを確認します。その後の状態はレシートとして後から届きます。保留、拒否、期限切れ、または宛先間違い（宛先が変更された — 再度エージェントを一覧表示）の場合、または保留後に解放された場合、送信セッションのトランスクリプトに通知が表示されます（`Message to <name>: …`）。送信したモデルには通知されません。別のセッションが返信した場合、その返信はクロスセッションメッセージとして届きます。
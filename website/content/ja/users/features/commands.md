...
`send_message` の呼び出しは、メッセージが別のセッションに渡されたことのみを確認します。その後の状態はレシートとして後から届きます。保留、拒否、拒絶、期限切れ、または宛先間違い（宛先が変更された — 再度エージェントを一覧表示）の場合、または保留後に解放された場合、送信セッションのトランスクリプトに通知が表示されます（`Message to <name>: …`）。拒否（declined）と拒絶（refused）は異なる応答です。拒否は誰かがメッセージをレビューしてノーと言ったことを意味、拒絶はそのセッションの `agents.crossSessionInbound` が `refuse` であり、誰もそれを全く見ていないことを意味します。送信したモデルには通知されません。別のセッションが返信した場合、その返信はクロスセッションメッセージとして届きます。

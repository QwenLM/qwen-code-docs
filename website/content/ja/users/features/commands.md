...
2つのセッションが同じインボックスアドレスを解決することもあります。アドレスはプロセス ID でキーイングされ、プロセス ID はランタイムディレクトリを共有するコンテナ間で繰り返されるためです。2番目に開始したセッションは、使用中のアドレスを引き継ぐ代わりに隣接するアドレスを取得するため、どちらもの到達不能になりません。ピアは影響を受けません。ピアはセッションレジストリからセッションのアドレスを読み取り、派生しません。
...
セッションは、1つの送信者から一度に最大30メッセージを受け付け、その後2秒ごとに1メッセージを受け付けます。また、すべての送信者合わせて一度に最大32メッセージを受け付け、その後1秒ごとに1メッセージを受け付けます。2番目の制限が存在する理由は、送信者は自分自身に名前を付けるためです。その名前をローテーションすると、最初の制限からは新しい許可量が得られますが、2番目の制限からは得られません。2番目の制限が最初の制限よりわずかかに大きいのは、受け付けられたメッセージごとにレシートが生成され、セッションは一度にそれほど多くのレシートを送信できないためです。別のセッションからのメッセージで、30秒以内にその送信者の前のメッセージを一言一句繰り返すものも拒否されます。1つの文でループするモデルは毎回新しいメッセージIDを発行するため、テキストがそれを捕捉します。セッションが開始したスクリプトからのメッセージと信頼されたコントローラーからのメッセージは、繰り返しチェックから免除されます。同じ行を2回報告するフックは2つの事実を報告しており、「continue」を2回言う人は2回意味しているためです。どちらもレート制限の対象となります。最後に、受け付けられたものの、セッションにすでに50メッセージが待機しているためにキューに入れられないメッセージも拒否されます。
...
トークンを保持する人は誰でもそのコントローラーとして送信できるため、他の認証情報と同様に扱ってください。1つのプログラムに与え、共有設定に入れないでもそのプログラムが完了したら取り消してください。

### ACP で駆動されるセッション

`qwen --acp` の子は、自身がホストする各セッションを登録します — デーモンがプロセスを生成した場合は `serve` として、エディタや他のクライアントが `qwen --acp` を直接駆動している場合は `headless` として — セッションは `qwen sessions ps` や他のセッションの `list_agents` に他のセッションと同様に表示されます。送信可能です。モデルが `send_message` を呼び出して、開いているターミナルに到達できます。それらの複数つが1つのプロセスと1つのインボックスを共有するため、送信者は意図するセッションを名前で指定する必要があります — すべての Qwen Code セッションはそれを自動的に行います。

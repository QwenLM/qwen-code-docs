...
- `security.allowedHttpHookUrls` ホワイトリストは独立して適用されます。管理環境では、このフラグをホワイトリストと組み合わせて、意図した内部エンドポイントのみが到達可能になるようにします。Workspace（プロジェクト）設定のホワイトリストは、User、System、または SystemDefaults のいずれかのスコープでホワイトリストが設定されていない場合にのみ尊重されます。それ以外の場合は無視され、警告としてログに記録されるため、リポジトリはフックがデータを送信する場所を絞り込むことはできますが、設定したホワイトリストを置き換えることはできません（空のホワイトリストは「すべて許可」を意味します）。
- HTTP フックはリダイレクトに追随しません。3xx レスポンスは他の非 2xx ステータスと同様に扱われます: 非ブロッキングなフックの失敗であり、リダイレクト先には決してアクセスしません。
...
最終的なフック出力に空でない `additionalContext` が含まれている場合、Qwen はまず値をサニタイズし、それを別のテキストパートとしてモデルに送信します。
...
このタグは、モデルとトランスクリプトのコンシューマーに、このパートが設定されたフックからのものであり、ユーザープロンプトからではないことを伝えます。これは出所マーカーであり、認証、認可、または一般的な信頼境界ではありません。

この追加コンテキストを持つ `UserQuery` について、セッション JSONL レコードはタグ付けされたパートを含むモデルバインドされたパートを保持し、以下の `systemPayload` を追加します。
...
機密性の高いプロンプトテレメトリ属性（有効な場合）および managed auto-memory の呼び出しは、どちらもフック前のプロンプトを使用します。`UserPromptSubmit` で追加されたコンテキストは含まれません。
...
フックは削除側のランタイムの通常のセッションフィールド（`session_id`、`transcript_path`、`cwd`）を使用します。ACP 経由では、削除側のランタイムは独自のトランスクリプトを持たないため、`transcript_path` は空です。`SessionDelete` は現在、インタラクティブな `/delete` フローと ACP の明示的な `deleteSession` メソッドで発生します。デーモン REST バッチ削除と内部クリーンアップでは発生しません。コマンドフックは、ディスパッチ後に Qwen が終了した場合でも完了まで残されます。その stdout と stderr は無視され、Qwen のパイプから独立しています。
...
コマンドフックは、ディスパッチ後に Qwen が終了した場合でも完了まで残されます。その stdout と stderr は無視され、Qwen のパイプから独立しています。
...
**注**: PostCompact は、公式の decision モードでサポートされているイベントリストには含まれて**いません**。`decision` フィールドやその他の制御フィールドは制御効果を生成せず、ログ記録の目的でのみ使用されます。
...
> **注**: `elicitation_dialog` タイプは定義されていますが、現在実装されていません。
...
非同期フックは Qwen プロセスにスコープされます。キャプチャされた出力はメモリ内の非同期フックレジストリを通じて配信されるためです。POSIX では、Qwen は終了時にまだ実行中の非同期フックプロセスツリーを回収します。ただし、終了後の fire-and-forget 完了が明示的に保証されているイベントタイプは除きます。Windows では、ルートプロセスが終了した後に子孫ツリーを再構築できないため、完全な親終了時の回収には Job Object または子孫追跡が必要です。

**機能:**

- 決定制御を返すことはできません（操作はすでに発生しています）
- 結果は、上記で文書化された出力が無視される fire-and-forget イベントタイプを除き、次の会話ターンで `systemMessage` または `additionalContext` を介して注入されます
...
コア/ヘッドレスパスでは、`prompt` には元のユーザー入力ではなく、生成または展開された内容が含まれる場合があります。ACP では展開前のリクエストテキストから始まるため、それを読んでも添付ファイルの本文や完全なモデル入力を検査することにはなりません。`UserPromptSubmit` はすべてのモデル送信をカバーするわけではありません。ソースの出所が必要な場合に `submitted_prompt` から `prompt` に黙ってフォールバックしないでください。

サポートされた送信テキストの検証を行い、長いプロンプトに対してコンテキストを提供する `UserPromptSubmit` フック。ヘッドレス送信や明示的に宣言された ACP/デーモン送信でも実行され、TUI 専用ではありません。ソースの出所が利用できない呼び出しはスキップします。ブロッキング結果は、これらの非 TUI パスを含む、影響を受ける呼び出しを停止します。キーワードチェックは例示であり、完全な DLP ポリシーではありません。
...
- フック出力の JSON フォーマットが正しいことを確認する
- 意図しないフック実行を避けるために、特定の matcher パターンを使用する
- `--debug` モードを使用して、フックのマッチングと実行の詳細情報を確認する。プロンプトフックの入力はセッションデバッグログに書き込まれる可能性があるため、適切なアクセスと保持の制御を適用する。
- すべてのフックを一時的に無効にする: 設定で `"disableAllHooks": true` を追加する
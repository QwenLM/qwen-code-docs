...
- **Auto Mode（デフォルト）**: デフォルトのすぐに使える体験 — LLM 分類器が安全なアクションを自動承認し、リスクのあるものをブロックし、中断を最小限に抑えながら安全網を維持します
- **Ask Permissions に切り替え**: すべてのファイル編集とシェルコマンドに手動承認が必要な場合
...
// .qwen/settings.json
{
  ...
  "permissions": {
    "autoMode": {
      ...
      // オプション: MCP ツール呼び出しを名前のみのみ分類器に送信します
      // (引数はデフォルトで転送されます)。

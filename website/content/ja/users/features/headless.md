...
# このプロジェクトの最も最近のセッションを継続して、新しいプロンプトを実行
qwen --continue -p "テストを再度実行し、失敗を要約してください"
...
# ファイルに追記
qwen -p "さらに詳細を追加" >> docker-explanation.txt
...
```bash
result=$(cat api/routes.js | qwen -p "これらのルート用にOpenAPI仕様を生成してください" --output-format json)
...
for file in src/*.py; do
    echo "Analyzing $file..."
    result=$(cat "$file" | qwen -p "潜在的なバグを見つけて改善提案をしてください" --output-format json)
...
echo "$(date): $total_tokens トークン, $tool_calls 回のツール呼び出し ($tools_used) を以下のモデルで使用: $models_used" >> usage.log

...
**Aone Code:** オリジンが `gitlab.alibaba-inc.com` 上にあるクローンをレビューする場合、そのクローン内から `/review` を実行します。プラットフォームがリモートから検出され、読み取りサブコマンドは `a1` CLI に基づいて動作します。ターゲット番号はグローバル MR ID です。`fetch-pr` は `refs/merge-requests/<id>/head` をフェッチして worktree + diff を構築するため、worktree のエージェントレビューは変更されません。このフェーズでは、すべての Aone 実行はコンテキスト利用不可であり、いくつかのフローがスキップされます（`github.com` の同名リポジトリにヒットする代わりに）。`pr-context`/`comment-status`/`presubmit` は Aone のバックイングを持たないため、判定は `COMMENT` にキャップされます。`test-plan` はバックイングなし、Agent 0 はスキップ、`publish-assets` の書き込みはスキップされます。`--comment` は `a1` CLI を通じてレビューを**投稿**します。インラインの発見ごとに1つのコメント、その後サマリーコメントです。Aone にはネイティブの request-changes 状態がないため、その判定ではサマリーコメントにブロッキングヘッダーが付き、実際に投稿されたインライン Critical はディスカッションゲートを通じてマージをブロックします（ディスカッションが未解決のまま）。インライン Critical が投稿されていない場合、ヘッダーは助言的なもので、機械的にマージをブロックするものはありません。ネイティブの `a1 repo mr approve` は Approve 判定用に配線されていますが、このフェーズでは発火しません。コンテキスト利用不可のキャップにより、すべての Aone の判定は Comment に保たれます。再ラウンドに関する2つの注意点: まだ重複排除のバックイングがないため、2回目の `--comment` ラウンドでは有効な発見ごとに新しいコメントとして再投稿され、自身PRの検出には Aone のバックイングがありません。`docs/design/2026-08-15-review-aone-provider.md` を参照してください。

...
**GitHub Enterprise:** `github.com` 以外のホストのPR URLをレビューする場合、そのホストのすべてのGitHub呼び出しがルーティングされます。レビューサブコマンド（`fetch-pr`、`pr-context`、`comment-status`、`presubmit`）は `--host` を受け取り、コード内で設定します。そのため、ホストの忘れによってレビューがサイレントに `github.com` にリターゲットされることはありません。
...
| 反復逆監査 (ステップ 5) | 2-5 (3A); ラウンド数 × チャンク数 (3B) | 2回連続のドライラウンドで停止（キャップ5）。3Bはラウンドごとにチャンクごとに1人の監査者をファンアウト                        |
| **合計**                        | **~17-23 (~15-22)**            | 3A同一リポジトリ: ~17-23（典型的には~17-19）。クロスリポジトリまたはローカル/ファイル: ~15-22。3Bはチャンクに応じてスケール（DESIGN.md参照） |

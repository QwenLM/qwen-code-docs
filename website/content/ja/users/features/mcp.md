...
Qwen Code は、[Model Context Protocol (MCP)](https://modelcontextprotocol.io/introduction) を通じて外部のツールやデータソースに接続できます。MCP サーバーにより、Qwen Code はあなたのツール、データベース、API にアクセスできるようになります。
...
> [!tip]
>
> 「すぐに始めるためのワンコマンド」をお探しの場合は、[クイックスタート](#quick-start)に進んでください。
...
3. 同じプロジェクトで Qwen Code を再起動するか（またはまだ実行されていない場合は起動して）、そのサーバーからのツールを使用するようモデルに指示してください。
...
> [!tip]
>
> 高度な設定レイヤー（システムデフォルト／システム設定および優先順位ルール）については、[Settings](../configuration/settings) を参照してください。
...
| `timeout`               | 数値<br>（デフォルト：600,000）   | リクエストタイムアウト（ミリ秒単位、デフォルト：600,000ms = 10分）                                                                                                                                                                                               |
...
| `<commandOrUrl>`    | 実行するコマンド（`stdio`用）またはURL（`http`/`sse`用）。        | —                  | `/usr/bin/python` or `http://localhost:8` |

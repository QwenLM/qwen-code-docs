...
- `isolation: "worktree"` を使用するエージェントはデフォルトのバックグラウンド動作に従います。クリーンアップはエージェントが完了を報告したときに実行されます。インラインで結果を受け取るには `run_in_background: false` を設定してください。呼び出し元所有の `working_dir` で起動したエージェントは、そのライフサイクルが外部で管理されるため、デフォルトでフォアグラウンドのままです。

...
- `isolation: "worktree"` には非フォークの `subagent_type` が必要です — フォークされたサブエージェント（`subagent_type: "fork"`）は親の会話コンテキスト全体を再利用するため、それらを分離すると意図と作業ツリーが分断されます。
- `isolation: "worktree"` を使用するエージェントはデフォルトのバックグラウンド動作に従います。クリーンアップはエージェントが完了を報告したときに実行されます。インラインで結果を受け取るには `run_in_background: false` を設定してください。呼び出し元所有の `working_dir` の起動はフォアグラウンドで実行されます。そのライフサイクルが外部で管理されるためです。明示的な `run_in_background: true` リクエストは拒否され、設定されたバックグラウンドのデフォルト（サブエージェント定義の `background: true`）はトップレベルで拒否され、ネストされている場合はフォアグラウンドにダウングレードされます。

...
ゴールの作成、編集、または再開には信頼されたワークスペース（`/trust`）が必要です。Goal state がジャーナルに記録される前に記録されたセッション（2026-07-29 より前の Qwen Code リリース）はゴールなしで再開されます。履歴内の `/goal` カードは履歴として表示され、`/goal` はゴールが設定されていないことを報告し、`/goal set` で再度開始します。Web Shell およびその他のデーモンクライアントでは、リプレイされた履歴の後の末尾カードに、古いゴールが駆動されていないことが示されます。ヘッドレスでの利用方法は [ヘッドレスモード](./headless.md#run-a-persistent-goal) を参照してください。

ゴールがターンを請求した後、フッターのピルとすべてのステータスカードには、許可されたウィンドウに対して消費した量が `1.2k/30.0m` の形式で表示されます。この数値は、ゴール自身のターンのモデル呼び出し、直接のフォアグラウンドサブエージェント、および検証ツールチェックをカウントします。ネストされたサブエージェントとバックグラウンドサブエージェント、無関係なサイドクエリ、cron ターン、および通知ターンは除外されます。使用可能な結果を返さなかった失敗したチェックはカウントできません。すでに記録された使用量を復元しても再カウントされません。ウィンドウは [`model.goalTokenBudget`](../configuration/settings.md) で設定されます。ウィンドウを使い切ったゴールを再開すると、すでに消費した分に加えて新しいウィンドウが付与されるため、数値はリセットされず `30.0m/60.0m` のように表示されます。予算がないゴールは消費量のみを表示します。まだターンを請求していないゴールは数値を一切表示しません。
...
ゴールはもう、ターン間で記録された証拠をチェックポイントの主張に圧縮しません。検証ツールはトランスクリプトを直接読み取るため、チェックポイントが供給するものは何もありません。また、長いターンの最後に実行されていたサイドモデルチェックとその停滞制限も実行されません。`model.goalCheckpointTimeoutSeconds` 設定は非推奨です。このキーは既存の設定ファイルが読み込めるように引き続き受け付けられますが、その値は無視されます。フッターのピル、ステータスカード、ヘッドレスの `/goal` 出力、Web Shell のステータスストリップとゴールダイアログ、および `get_goal` は、チェックポイントがまだ実行されていたビルドで記録されたセッションを含め、チェックポイント行や停滞カウントを表示しなくなりました。そのゴールの停止理由は引き続き停止理由を示します。
...
検証ツールは独自にコマンドを実行したりファイルを読み取ったりすることはありません。このゴールのレコードのみを確認し、最新のレコードから要求がいっぱいになるまで（256 000 バイト、単一のレコードは 8 000 を超えると途中で切り詰められます）後ろ向きに読み取ります。短い終了ターンは前のターンの余地を残します。長いターンは単独で要求を満たし、検証ツールには古いレコードがいくつ除外されたかが伝えられ、証明がその中にある可能性がある提案を拒否します。

- 表示されたアシスタントの出力、ツールの結果、およびあなた自身のメッセージが証拠としてカウントされます。objective のテキストとモデルの非表示の推論は証拠になりません。完了が提案されるずっと前に実行されたチェックは見えなくなっている可能性があり、再度実行する必要があります。

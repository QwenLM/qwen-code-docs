...
ゴールは、宣言された条件が満たされるまで Qwen Code をターンを超えて稼働させ続けます。`/goal <objective>` で設定すると、セッションは自律的に稼働を続けます。モデルが objective の完了またはブロックを提案すると、独立した検証ツールがゴールのトランスクリプトの直近の部分からその提案を判定します。検証ツールが受け入れた時点、またはゴールが一時停止、クリア、もしくは制限によって停止された時点でセッションは終了します。
...
ゴールの作成、編集、または再開には信頼されたワークスペース（`/trust`）が必要です。ゴールの状態がジャーナルに記録される前に記録されたセッション（Qwen Code リリース 2026-07-29 より前）は、ゴールなしで再開されます。履歴内の `/goal` カードは履歴として表示され、`/goal` はゴールが設定���れていないことを報告し、`/goal set` で再度開始します。Web Shell やその他のデーモンクライアントでは、リプレイされた履歴の後の末尾カードに、古いゴールが駆動されていないことが表示されます。ヘッドレスでの利用方法は [ヘッドレスモード](./headless.md#run-a-persistent-goal) を参照してください。

ゴールがターンを請求した後、フッターのピルとすべてのステータスカードには、許可されたウィンドウに対して消費した量が `1.2k/30.0m` の形式で表示されます。この数値はゴール自身のターン、直接のフォアグラウンドサブエージェント、および検証ツールチェック内のモデル呼び出しをカウントします。ネストされたサブエージェントおよびバックグラウンドサブエージェント、無関係なサイドクエリ、cron ターン、および通知ターンは除外されます。使用可能な結果を返さなかった失敗したチェックはカウントできません。すでに記録された使用量を復元しても再カウントされません。ウィンドウは [`model.goalTokenBudget`](../configuration/settings.md) で設定されます。ウィンドウを使い切ったゴールを再開すると、すでに消費した分に加えて新しいウィンドウが付与されるため、数値は最初からやり直すのではなく `30.0m/60.0m` と表示されます。予算がないゴールは消費量のみを表示します。まだターンを請求していないゴールは数値を一切表示しません。

ターンまたはアクティブ時間の予算が設定されている場合、ステータスカードには対応する使用量も予算に対して表示されます。たとえば `3/20 turns` や `12m 3s/30m` です。ヘッドレステキストでは `3 of 20 turns` や `12m 3s of 30m active` と表記されます。使用量がゼロの場合は非表示のままで、一時停止または停止されたゴールはこれらの数値を保持します。
...
ゴールは、記録した証拠をターン間でチェックポイントの主張に圧縮しなくなりました。検証ツールはトランスクリプトを直接読み取るため、チェックポイントが供給するものはなく、長いターンの最後に実行されていたサイドモデルチェックとその停滞制限も実行されません。`model.goalCheckpointTimeoutSeconds` 設定は非推奨です。このキーは既存の設定ファイルが読み込めるように引き続き受け付けられますが、その値は無視されます。フッターのピル、ステータスカード、ヘッドレスの `/goal` 出力、Web Shell のステータスストリップとゴールダイアログ、および `get_goal` は、まだチェックポイントを実行していたビルドで記録されたセッションを含め、チェックポイント行や停滞カウントを表示しなくなりました。そのゴールの停止理由は引き続き停止理由を表示します。
...
すべての一時停止はその理由を示します。あなたが中断した、`/goal pause` を実行した、セッションのトークン制限が次のモデルリクエストをブロックした、ターンが失敗した、検証ツールが提案を判定できなかった（タイムアウト、モデル呼び出しの失敗、または objective が証拠のための余地を残さなかった）、または 3 ターン連続で検証ツールが判定できる何も記録せず提案もなかった（`get_goal`、`update_goal` などのゴールの記録操作は進捗としてカウントされません）といった理由です。制限によって停止されたゴールは、代わりにその制限の理由を保持します。

## ゴールの判定方法

検証ツールは独自にコマンドを実行したりファイルを読み取ったりすることはありません。トランスクリプトにすでに存在するもののみを確認します。トランスクリプト内のこのゴールの記録を、直近のものから逆方向に、リクエストがいっぱいになるまで読み取ります（256,000 バイト、単一の記録は 8,000 を超えると途中で切り詰められます）。短いクロージングターンは前のターンに余地を残します。長いターンはリクエストを単独で満たし、検証ツールには何件の古い記録が除外されたかが伝えられ、証明がその中にあるかもしれない提案を拒否します。

- 表示されたアシスタントの出力、ツールの結果、およびあなた自身のメッセージが証拠としてカウントされます。objective のテキストとモデルの非表示の推論は証拠になりません。完了が提案されるずっと前に実行されたチェックは視界から外れている可能性があり、再度実行する必要があります。

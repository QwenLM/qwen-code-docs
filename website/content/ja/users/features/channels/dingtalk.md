...
- `per_turn`: メインプロンプトが終了するとすぐに、メインのステータスカードがターンの最後の空でないアシスタントの応答で完了します。それ以降の各バックグラウンド通知ターンは、それぞれの最後の空でないアシスタントの応答を保持し、別の完了カードとして送信します。

`per_turn` および `per_task` におけるバックグラウンドシェル、モニター、およびワークフローの出力には、種類、ステータス、および利用可能な場合はタスクラベルを含む見出しが付きます。`per_response` では、応答本文がそのまま配信されます。バックグラウンドエージェントの応答は、すべてのモードで元の本文を保持します。

デフォルトの `per_turn` モードでは、バックグラウンドタスクがメインカードの有効期間を延長することはなく、後続のコールバックがそれを上書きすることもできません。たとえば、メイン結果に 11 の個別のバックグラウンド通知ターンが続く場合、メイン結果カードと 11 のフォローアップ結果カードが生成されます。そのタスクに関連するバックグラウンド作業を待機して 1 つの最終結果を受け取るには、`per_task` を選択してください。結果はアシスタントから提供され、追加の要約は生成されず、中間の応答は連結されません。
...
インタラクティブステータスカードはネイティブなカード表現を提供します。ステータスカードが利用できないか、すべてのインタラクティブカードが無効になっている場合、同じ出力ポリシーが通常のメッセージを通じて適用されます。`per_task` はタスク全体を待機し、`per_response` は各完了応答を送信し、`per_turn` はターンごとに 1 つの結果を送信します。カードのコンテンツ制限を超えるバックグラウンド結果も通常のメッセージにフォールバックします。プラットフォームのメッセージ長制限により、長いテキストが分割される場合があります。ファイルと画像の配信は既存のルールを保持します。

この設定は、DingTalk の会話返信とそれに関連するバックグラウンドフォローアップに適用されます。同じ会話内の無関係なタスクをマージすることはありません。チャンネルループと Webhook 実行は既存の表示を保持します。

`per_task` は、そのプロンプトに関連付けられたエージェント、バックグラウンドシェル、モニター、およびワークフローの作業を待機します。これには、それらの通知ターンによって開始された作業も含まれます。一時停止されたタスクと長時間実行されるモニターは、完了するかキャンセルするまでオープン状態を維持します。将来のスケジュールされた実行と独立して管理されるデーモン子セッションは別の作業であり、このタスク境界には含まれません。

Todo Stop Guard が同じセッションでキューに入れられたメッセージに譲る場合、待機中の `per_task` リクエストはキューに入れられたメッセージを開始できるようにキャンセルとして終了します。保持された応答は成功したタスク結果として配信されません。関連するバックグラウンド作業はセッションで引き続き利用可能です。

中断された、または 10 分経過後も終了していないスタンドアロンのバックグラウンドターンは、部分的としてマークされた結果を配信する場合があります。これにより完了したメインカードが再開されることはありません。

受け入れられるのは `per_task`、`per_response`、`per_turn` のみです。未公開の `final_only` と `process_and_result` の値はエイリアスではありません。目的のモードに置き換えてください。`outputMode` を削除すると、`per_turn` のデフォルトに戻ります。個別のバックグラウンド集約トグルはありません。
...
- **参照メッセージ** — ユーザーのメッセージに引用（返信）すると、その引用テキストがエージェントのコンテキストとして含まれます。リッチテキストの引用はテキストの順序を保持し、埋め込み画像を添付します。引用されたメッセージが画像、ファイル、音声、または動画メッセージの場合、ボットは直接送信された場合と同じ方法でそれをダウンロードして添付します。ボットの応答の引用はまだサポートされていません。

...
**生成ファイル:** エージェントに完了したローカルファイルを明示的に送信するよう指示すると、ファイルを DingTalk のネイティブ添付ファイルとして返すことができます。ファイルは空でなく、20 MB 以下であり、設定されたワークスペース内またはシステムの一時ディレクトリ内に存在する必要があります。1 回の応答で送信できるファイルは最大 5 つです。アップロードまたは配信の失敗は、代わりに最終テキスト内で報告されます。

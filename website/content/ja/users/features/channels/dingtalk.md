...
`"atSender": true` を設定すると、ボットがグループメッセージのトリガーとなったメンバーを @メンションします。デフォルトではオフで、DingTalk スタッフ ID を持つエージェントの応答にのみ適用されます。メンション付きの応答はプレーンテキストを使用するため @が表示されます。メンションのない応答は Markdown フォーマットを使用します。

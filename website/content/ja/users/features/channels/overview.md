...
このモードでは、`qwen serve` が所有するワークスペースグループ化されたチャネルワーカープロセスが起動します。ワーカーは SDK を介してデーモンに接続し、同じチャネルアダプタを使用します。デーモンプロセスとは分離されているため、チャネルアダプタがクラッシュしてもデーモンはクラッシュしません。明示的な `--channel` 選択が優先され、準備できない場合はデーモンの起動が失敗します。フラグなしで起動した場合、信頼されたプライマリワークスペースの `serve.channels` 設定が復元されます。セカンダリワークスペースは独自の `serve.channels` を独立して復元しません。いずれのソースもない場合、デーモンは最初の `qwen channel set` までチャネルアダプタをロードせず、チャネルサービスの PID リースを予約しません。

自動復元は、無効な起動設定やワーカー起動前に発生した検証またはリースの失敗を、関連しない設定を保持しながらスキップします。ワーカーの起動が失敗した後、デーモンはクリーンアップが成功した後にのみ続行します。グローバルなランタイム起動タイムアウトまたは未確認のワーカー停止は、通常の起動失敗パスに従います。ワーカーの終了が未確認の間、サービスリースは保持されたままです。チャンネルが復元されない場合は、`serve.channels` を識別するメッセージについてデーモンログを確認してください。

保存された起動名は空でない必要があり、先頭または末尾に空白を含んではならず、安全でない制御文字や不可視文字を含んではなりません。無効なエントリは個別にスキップされ、配列インデックスでログに記録されます。起動時にインスタンスの名前変更や設定の書き換えは行われません。チャンネルの起動トグルは保存された設定を反映し、ランタイム状態はチャンネルが実行中かどうかを示します。
...
`--daemon-url` なしでは、`qwen channel status` と `qwen channel stop` はスタンドアロンの pidfile 動作を維持します。`--daemon-url` バリアントはデーモンマネージャーをクエリまたは停止します。ランタイムの選択は設定に書き込まれず、デーモンの再起動をまたいで生存しません。準備ができたワーカーが予期せず終了した場合、デーモンは実行を継続し、`/daemon/status` にチャネルワーカーの警告を報告します。

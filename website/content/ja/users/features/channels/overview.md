...
チャンネルは通常のレスポンス配信パスを使用します。共有配信レイヤーが完了したレスポンスを送信し、アダプタはインタラクティブカードをその場で更新するなど、ネイティブなプログレッシブ表示を提供する場合があります。プラットフォームのメッセージ長制限により、長いレスポンスが分割される可能性があります。

廃止された `blockStreaming`、`blockStreamingChunk`、および `blockStreamingCoalesce` 設定はサポートされなくなったため、チャンネル設定から削除できます。これらは配信に影響しません。チャンネル設定管理は、これらのフィールドの新しく追加または変更された値を拒否します。変更されていない保存値は、編集でチャンネルの `type` が保持される場合に保持または削除されます。チャンネルの `type` を変更するには、まずこれらのフィールドを削除する必要があります。

...
| `multiSession`           | いいえ           | 1 つのチャットに最大 8 つのオーナー スコープの名前付きタスクを保持します。デーモン管理モード、`sessionScope: "user"` が必要で、Webhook、グループ履歴のバックフィル、および有効なチャンネルループがないことが条件です                                      |
| `messagePrefix`          | いいえ           | 先行する ` @mentions` の後にこの正確な大文字小文字を区別するプレフィックスで始まるユーザーメッセージのみをディスパッチします。プレフィックスとそれに続く空白はディスパッチ前に削除されます                                            |
...
`messagePrefix` が設定されている場合、すべてのユーザー作成メッセージはプレフィックスと空でないペイロードで始まる必要があります。たとえば `/review inspect #123` のようになります。プレフィックスとその前のメンションのみが削除されます。ユーザーがプレフィックスの後に入力したメンションは変更されずにエージェントに届きます。共有コマンドとエージェントコマンドも同じルールを使用します（`/review /help`、`/review /clear` など）。Telegram の登録済みコマンドメニューアクションは、プレフィックスなしで引き続き利用できます。設定されたプレフィックスがそれらのいずれか本身である場合を除きます。その場合、プレフィックスが優先され、そのコマンドもプレフィックス付きで送信する必要があります（`/new /new`）。添付ファイルには、プラットフォームがサポートしている場合に一致するキャプションが必要です。キャプションのない Telegram、Feishu、WeChat、DingTalk、WeCom のメディアメッセージは引き続き実行され、プレースホルダーテキストはグループ履歴として引用されません。ネイティブ todo、Webhook、およびプロバイダー生成の割り当てやレビューリクエストイベントも、チャットメッセージではなくシステムイベントであるため、プレフィックスなしで引き続き実行されます。

プレフィックスを有効にする前に知っておくべき 2 つの意図的な動作があります。DingTalk または WeCom が文字起こしを補完する音声メッセージは、ユーザーが話したテキストとしてカウントされるため、他のメッセージと同様にプレフィックスを持つ必要があり、そうでない場合は破棄されます。文字起こしされていない音声メモのみがキャプションなしメディアとして実行されます。また、プレフィックスはペアリングの前にチェックされるため、不明な送信者や未承認のグループからの最初の接触もプレフィックスを持つ必要があります。この順序がないと、忙しいグループ内のプレフィックスなしのメッセージすべてがペアリング応答を引き起こし、これはまさにプレフィックスが存在する理由であるノイズです。新しいユーザーには帯域外でプレフィックスを伝えるか、ペアリングチャンネルをプレフィックスなしにしてください。
...
- **`thread`** — 既存の設定のために保持されているレガシーなスレッド/トピックルーティング。
...
名前付き結果は元のタスクを識別します。ダイレクトチャットでは `[task]` を使用し、グループチャットでは `[sender · task]` を使用します。名前付きテキスト権限プロンプトには、正確なリクエスト ID と対応する `/approve <id>`、`/approve-always <id>`、`/deny <id>` コマンドも表示されます。ラベルは表示のみを目的としており、モデルのトランスクリプトには保存されません。

1 つのタスクが選択された状態のまま次の通常メッセージを受け取りますが、他の名前付きタスクは共有ワークスペースで並行して実行を継続できます。別のタスクを作成または選択しても、以前の作業がキャンセルされたりターゲットを変更したりすることはありません。遅れて到着する結果には元のタスクラベルが保持されます。ビジーなタスクは閉じることができませんが、アクティブなプロンプトは既存のチャンネルキャンセル動作を通じて `/session cancel [<name>]` でキャンセルできます。独立してキューに入れられたターンはキャンセルされませんが、`collect` ディスパッチモードでは、キャンセルされたプロンプトの後にバッファリングされたフォローアップは既存の動作によって破棄されます。メディアの準備はターゲット指定されません。ベアの権限コマンドは選択されたタスクにのみ適用されますが、明示的なリクエスト ID で所有する非アクティブなタスクに応答できます。1 つのタスクが次の通常メッセージを受信するために選択されたままになりますが、他の名前付きタスクは並行して実行を継続する場合があります。`/session new <name>` は設定されたワークスペースを共有し、`/session new <name> --worktree` はデーモンワークスペースの `.qwen/worktrees/` ディレクトリ配下にそのタスク用の分離されたチェックアウトを作成します。デー��ンは再起動後にタスクを再開する前に永続化されたワークツリーの所有者を検証します。不足、変更、または外部の所有権記録は、タスクを共有ワークスペースにサイレントに移動する代わりに fail closed（失敗時は拒否）します。別のタスクを作成または選択しても、以前の作業はキャンセルもリターゲットもされず、遅れた結果は元のタスクラベルを保持します。ビジーなタスクは閉じることができませんが、アクティブなプロンプトは既存のチャンネルキャンセル動作を通じて `/session cancel [<name>]` でキャンセルできます。独立してキューに入れられたターンはキャンセルされませんが、`collect` ディスパッチモードでは、キャンセルされたプロンプトの後にバッファリングされたフォローアップは既存の動作によって破棄されます。メディアの準備はターゲット指定されません。ベアの権限コマンドは選択されたタスクにのみ適用されますが、明示的なリクエスト ID で所有する非アクティブなタスクに応答できます。`/clear`、`/new`、および `/reset` も選択されたワークツリータスクで機能します。タスクは新しい会話を取得しますが、ワークツリーとファイルは保持されます。ビジーなワークツリータスクはプロンプトが完了するまでリセットを拒否し、ワークツリーレコードが損傷しているタスクはファイルに触れずに失敗を報告します。チャンネルメモリは名前付きタスクではなくチャットにスコープされたままです。
...
ユーザーペアリングと同じ CLI コマンドでグループを承認します。保留中のリクエストには、グループとそれを開始したメンバーが識別されます。
...
- `/btw <question>` — 現在のタスクを中断せずにサイド質問をする。テキストのみの質問で最大 4096 文字、サイド質問サポートを持つエージェント接続が必要
- `/session new <name> --worktree` — 独自の Git ワークツリーにタスクを作成して選択。デーモン管理の名前付きタスクモードのみ
- `/session use <name>` — 開いているタスクを選択、または閉じたタスクを再開
- `/session cancel [<name>]` — 選択されたタスクのアクティブなプロンプトをキャンセルするか、別の所有タスクを指定します。独立してキューに入れられたターンはキャンセルされませんが、`collect` モードのフォローアップでキャンセルされたプロンプトの後にバッファリングされたものは既存のキャンセル動作によって破棄されます。メディアの準備はターゲット指定されません
...
その他のすべてのスラッシュコマンド（例: `/compress`, `/summary`）はエージェントに転送されます。名前付きタスクコマンドはモードが有効な場合にのみ登録されるため、既存の設定では `/sessions` はエージェントに参照可能です。

名前付きタスクコマンドは、すべてのチャネルタイプ（Telegram, WeChat, QQ, DingTalk, WeCom, Feishu, GitHub）で機能します。`/cancel` は現在 Telegram でのみ登録されており、ループの作成には現在のアダプタとターゲットのプロアクティブ配信サポートが必要です。

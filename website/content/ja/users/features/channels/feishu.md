...
デーモンで検出されたコンタクトに ID の代わりにユーザー名とグループ名を表示するには、以下の権限をオプションで有効にします。

- `contact:user.basic_profile:readonly` — ユーザーの表示名の読み取り
- `im:chat:readonly` — グループ名の読み取り

これらのオプションの権限がなくてもメッセージは動作し、検出されたコンタクトは Feishu のユーザー ID とチャット ID をそのままラベルとして保持します。
...
1. チャンネル設定で `groupPolicy` を `"allowlist"`、`"pairing"`、または `"open"` に設定する
2. ボットを Feishu グループに追加する
3. グループ内でボットを @メンションするとレスポンスが返る
4. `groupPolicy: "pairing"` を使用している場合、応答が開始される前にグループのペアリングリクエストを一度承認する
...
- `groupPolicy` が `"allowlist"`、`"pairing"`、または `"open"` に設定されているか確認する（デフォルトは `"disabled"`）
- `"pairing"` を使用している場合、グループのペアリングリクエストが承認されていることを確認する

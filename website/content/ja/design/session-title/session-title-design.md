...
`ChatRecordingService.recordCustomTitle()` は `Promise<boolean>` を返す。`true` は、`custom_title` レコードがレコーダーの順序付き JSONL 書き込みを完了し、インメモリのタイトルとソースが更新され、タイトルオブザーバーに通知されたことを意味する。書き込みキューへの受け入れのみを意味するわけではない。ファイル作成の失敗、既存のレコーダーの失敗、先にキューに入った書き込みの失敗、またはタイトル書き込み自体の拒否は `false` を返し、以前のタイトルとオブザーバーの状態は変更されないままとなる。レコーダーの正規の `flush()` は引き続きスティッキーなライター失敗を報告する。

オブザーバーは永続化後にのみ実行される。すでに書き込まれたタイトルは失敗したリネームとして遡及的に分類できないため、オブザーバーの例外は隔離される。コールバックはまた、永続化されたレコードに記録されたセッション ID を受け取る。コンシューマーはコールバック時に可変の Config セッションを読み取ってはならない。これにより、`/clear`、再開、またはブランチによってセッションが切り替わった後に、退出しつつあるレコーダーからの遅延書き込みが新しいプロンプトバーをリネームしてしまうことを防ぐ。

バックグラウンドの自動タイトルは同じ非公開の永続化パスを使用するが、ベストエフォートのままである。公開呼び出しは明示的なタイトルリクエストであり、`/rename --auto` やリモートコントロールのサーフェスからの `recordCustomTitle(title, 'auto')` が含まれる。
...
`maybeTriggerAutoTitle` は以下の条件をこの正確な順序でチェックする — それぞれが残りを短絡させ、安価なチェックが最初に実行される。

1. `currentCustomTitle` が設定されている → スキップ。手動/以前の自動を決して上書きしない。
2. ライターが縮退している → スキップ。fail-stop したレコーダーはタイトルを永続化できないため、これ以上高速モデルトークンを消費しない。
3. 明示的タイトルの書き込みが保留中 → スキップ。ユーザーおよびリモートの意図は、その JSONL 書き込みが確定する前でも優先される。
4. `autoTitleController !== undefined` → スキップ。一度に1回の試行。
5. `autoTitleAttempts >= 3` → スキップ。上限が総無駄を制限。
6. `autoTitleDisabledByEnv()` → スキップ。`QWEN_DISABLE_AUTO_TITLE=1` 明示的オプトアウト。
7. `!config.isInteractive()` → スキップ。ヘッドレス `qwen -p` / CI はワンショットセッションで高速モデルトークンを消費しない。
8. `!config.getFastModel()` → スキップ。高速モデルなし → 何もしない。
...
公開されている `recordCustomTitle()` の呼び出しはすべて、保留中明示的カウンターをインクリメントし、JSONL の待ちに入る前にアクティブなバックグラウンドコントローラーを中断する。これには `/rename --auto` も含まれるため、モデルが生成した明示的リネームは、リテラルなユーザータイトルと同じ優先度を持つ。バックグラウンドタスクは、生成後と永続化直前の両方で、カウンター、中断シグナル、現在のタイトル、ライターの状態をチェックする。並行する明示的リネームはライターによって直列化され、そのインメモリ結果も同じ順序でコミットされる。バックグラウンドタイトルがすでにキューに入った後に明示的タイトルが届いた場合、通常のライターの順序付けにより、明示的タイトルが最終レコードとなる。

明示的タイトルの書き込みが保留中の間、`finalize()` は以前にキャッシュされたタイトルを再アンカーしない。さもなければ、その古いレコードがリネームの後ろにキューに入り、新しいタイトルの反映が成功した後に JSONL の末尾になってしまう可能性がある。タイトルアンカーの集計は論理的なライターキューの順序に従う。未解決のタイトル書き込みの後ろにキューに入った子孫レコードはカウントされるが、しきい値による再アンカーは最終的なキャッシュタイトルが判明するまで延期されるため、シャットダウンも 32KB アンカーパスも、成功したリネームの後ろに古いタイトルを追加できない。

プロセス間の再読み取りは、引き続き、別の CLI プロセスによって書き込まれた手動タイトルを保護する。これは compare-and-swap ではない。最終チェックから書き込みまでのウィンドウにおける外部リネームは、許容される競合のままとなる。

### ブランチタイトルの順序

`/branch` は、ソースレコーダーをコピーする前に確定してフラッシュする。次に暫定のフォークを読み込み、一意のブランチタイトルを算出し、`SessionService` を通じてそのタイトルを永続化し、フォークを再読み込みして、その最終的な末尾とタイトルキャッシュに追加されたレコードを含める。これらの手順が成功した後でのみ、コアのレコーダーと UI を切り替える。ソースのフラッシュ失敗、タイトル失敗、または最終再読み込み失敗は、不完全なフォークを削除し、元のセッションをアクティブなままにする。`forkSession()` はまた、呼び出しが成功を報告する前に新しい JSONL の書き込みまたはクローズが失敗した場合、排他的に作成したターゲットを削除するため、呼び出し元はフォークを作成済みとマークする前のウィンドウで部分的なファイルを漏洩させることがない。

### デーモン境界

ライブ ACP のタイトル操作は `recordCustomTitle()` を待機するため、後続の無関係な書き込み失敗をリネームのせいにし得る 2 回目の `flush()` なしで、正確な `persisted` または `success` の結果を報告する。デーモンのメタデータ PATCH は意図的に楽観的なままとなっている。ライブ状態を更新してイベントを即座に発行する一方、子プロセスの永続化はベストエフォートであり、false の結果はデバッグログに記録される。強整合な HTTP メタデータ更新は本設計の範囲外である。

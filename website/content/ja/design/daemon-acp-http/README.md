...
> **過去の決定記録。** この設計は登録制マルチワークスペースランタイムより前のものである。ベアラートークンとシングルワークスペースバインドを信頼境界として参照している箇所は置き換えられた：現在、1 つのベアラートークンが登録済みのすべてのワークスペースをカバーし、ワークスペースの信頼は ACL ではなく実行ゲートである。現在の所有権と分離の契約については

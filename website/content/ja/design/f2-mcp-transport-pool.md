...
> **過去の決定の記録。** 元々の `1 daemon = 1 workspace` という前提は、マルチ
> ワークスペースのランタイムによって置き換えられた。実装されたプールは引き続き
> 同じ隔離境界を持つが、その境界は現在、1 つの `WorkspaceRuntime` である。
> セッションはランタイム内で共有され、ランタイム間で共有されることは決してない。
> 現在の所有権の契約については

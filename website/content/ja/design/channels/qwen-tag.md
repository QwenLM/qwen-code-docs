...
> **過去の決定記録。** このドラフトにおける「ワークスペースごとに1プロセス / ワークスペースごとに1デーモン」という前提はすでに置き換えられています。名前付きのデーモン管理チャネルは、所有するワークスペースごとにグループ化され、所有するランタイムごとに1つの worker を持つようになりました。`--channel all` は引き続きプライマリ専用です。単一のグローバルトークンと、個人単位の ID が存在しない点は、現在も制限として残っています。[`../daemon-multi-workspace-hardening.md`](../daemon-multi-workspace-hardening.md) を参照してください。

**ステータス:** Historical draft (v2)
**日付:** 2026-06-25
**著者:** (qwen-code)

...
`qwen serve` はデフォルトではローカルデーモンであり、設定を誤ると露出対象となります。そのセキュリティモデルは**階層化**されており、設定ミスがあっても fail closed するようになっています。

1. **バインド** — ループバック以外のバインドには常にベアラーが付随します。オペレーターが提供する場合と、起動時に一度だけ生成・表示される一時的な 128-bit ベアラーの場合があります。起動が拒否されるのは、提供されたトークンソースが明示的に空/空白の場合、またはリクエストされた `localhost` がループバック外に解決されトークンソースが解決されない場合のみです（この場合はトークンが生成されません）。
2. **ベアラー認証** — `bearerAuth` ミドルウェアは、定数時間の SHA-256 比較で通常の API ルートを保護します（`/health` を除く）。通常のループバックバインドでは `/health` が例外です（`require_auth` を設定すると、そのエンドポイントもベアラーの背後に配置されます）。チャネル Webhook の入力は、`x-qwen-webhook-secret` で認証される別の pre-bearer ルートです。Web Shell のドキュメントおよびアセットルートは、すべてのモードで pre-auth のままです。
3. **ホストヘッダー許可リスト** — ループバックでは、`localhost`、`127.0.0.1`、`[::1]`、`host.docker.internal`、または正確なバインドされたループバックアドレス（ポート付き）のみが受け入れられます。ポート 80 または 443 でリッスンしている場合は、対応するポートなしの形式も受け入れられます。この許可リストは DNS リバインディングから防御します。Local Control LAN リスナーは例外であり、プライマリバインドに関わらず常にアドバタイズされたオーソリティの Host チェックを強制します。
...
5. **ルートごとのミューテーションゲート** — strict ルートにはオペレーター権限が必要です。トークンなしのループバックプライマリリスナーは信頼されます。ベアラー認証されたリクエストおよびペアリングされた Local Control リクエストも資格を持ちます。信頼された権限なくこのゲートに到達したトークンなしのプライマリリクエストには、固有の `code: 'token_required'` エラーが返されます。不足または無効な設定済み資格情報およびペアリングされていない Local Control 資格情報は、リスナースコープのベアラーミドルウェアによってプレーンな `401 Unauthorized` で事前に拒否されます。
...
- 安全でない構成での起動を拒否する。
- 通常の API リクエストを、設定時のベアラー経由でゲートする（ループバックの `/health` 例外が適用される）。チャネル Webhook の入力は独立した共有シークレットゲートの背後に保持し、ループバックの Host チェックとブラウザの Origin チェックを認証済みルートおよび例外ルートの前に保持する。
- Wave 4 ルートがオプトインするルートごとのミューテーションゲートを提供する。
- SSE イベント経由で可視化されるプロバイダー OAuth フローを駆動するデバイスフローレジストリをホストする。

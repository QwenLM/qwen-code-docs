...
| `--session-restore-timeout-ms <n>`      | positive integer             | `60000`                                                                           | ACP セッションのロード/再開のタイムアウト（ミリ秒）。このフラグが省略された場合、明示的に指定された initialize タイムアウトは予算を引き上げますが、デフォルト値より下げることはありません。                                                                            |
...
| `sessionRestoreTimeoutMs`     | ACP セッションのロード/再開のタイムアウト。優先順位: 明示的な restore 値。それ以外の場合、明示的な initialize 値はデフォルトの 60000 を引き上げますが下げることはありません。それ以外の場合は 60000。 |
...
| `initializeTimeoutMs`, `sessionRestoreTimeoutMs`, `maxSessions`, `eventRingSize`, `permissionResponseTimeoutMs`, `maxPendingPermissionsPerSession` | リソース上限のバインド。                                                                      |

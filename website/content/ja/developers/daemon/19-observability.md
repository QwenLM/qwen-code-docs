...
| `DaemonLogger` 構造化ファイルログ         | `serve/daemon-logger.ts`                       | 安定した、サイズローテーションされる `daemon.log` に追記します。アクティブな記録済みのサンプリング済み OTel スパンが存在する状態で出力された呼び出し元の `info` / `warn` / `error` レコードには `trace_id` と `span_id` が含まれます。ファイルレコードには `runId` と PID も含まれます。起動時に選択された安定/フォールバックパスを出力します。フルステータスはヘルス、問題、およびファイルコピー損失カウンタを公開します。 |
...
| Web Shell UI                                | `GET /` (`packages/cli/src/serve/web-shell-static.ts`) | バンドルされた Web Shell アセットから提供されるブラウザ UI: チャット、セッションリスト、ワークスペースインスペクター、権限 UX。ループバックでは、`http://127.0.0.1:4170/` が SDK コードを書かずにエンドツーエンドの検証を最も迅速に行うパスです。登録ルールは [`02-serve-runtime.md`](./02-serve-runtime.md) にあります。 |
...
- **DaemonLogger のファイルログは構造化テキストであり**、`trace_id`、`span_id`、`route`、`sessionId`、`clientId` の各フィールドを正規表現で検索または抽出できます。呼び出し元の `info` / `warn` / `error` レコードにトレースフィールドが含まれるのは、ログ呼び出し時にアクティブな記録済みのサンプリング済み OTel スパンが存在する場合のみです。`raw` レコード、ブートレコード、ファイルドロップのサマリ、およびアクセスログの抑制サマリは意図的にこれらを省略します。相関はベストエフォートです。エクスポーターの障害により、サンプリングされたトレースがバックエンドで利用できなくなる可能性があります。これらの高カーディナリティ識別子は診断検索用であり、メトリクスラベルや集計用ではありません。`QWEN_SERVE_DEBUG` の stderr ログは非構造化テキストのままです。
- **受け入れられたプロンプト、コンティニュエーション、およびキャンセルのミュテーションにはライフサイクルログがあります。** `prompt enqueued`、`continuation enqueued`、および `cancel sent` には、該当する場合は `sessionId`、`promptId` が含まれ、提供された場合は `clientId` が含まれます。プロンプトコンテンツはログに記録されません。各独立したコントローラーには、個別の安定したクライアント ID を使用してください。意図的に ID を共有するコントローラーは、これらのレコードで区別できません。

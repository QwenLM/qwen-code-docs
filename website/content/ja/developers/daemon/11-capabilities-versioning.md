...
バッチセッションカタログ: `session_catalog_batch` は、最大 20 の登録済みワークスペースに対して `POST /sessions/catalog` を独立してアドバタイズします。ワークスペースごとのカーソル、明示的なエントリエラー、およびオプションのグループカタログをサポートします。このタグは無条件であり、単一ワークスペースのデーモンも含まれます。一度検出し、存在しない場合は既存のワークスペース修飾リスト/グループリクエストにフォールバックします。サポートは `session_list`、`multi_workspace_sessions`、または `workspace_qualified_rest_core` から推論できません。バッチは ACP を開始せず、内部ワークスペースを除外します。

ワークスペースのミューテーション（Wave 4 以降）: `workspace_memory`, `workspace_agents`, `workspace_agent_generate`, `workspace_acp_preheat`, `workspace_tool_toggle`, `workspace_skill_settings_toggle`, `workspace_skill_settings_batch_toggle`, **`workspace_settings`** (conditional), `workspace_permissions`, `workspace_init`, `workspace_github_setup`, `workspace_trust`, `workspace_mcp_restart`, `workspace_mcp_manage`, `workspace_file_read`, `workspace_file_bytes`, `workspace_file_read_cursor`, `workspace_file_write`, `workspace_file_upload`, **`workspace_reload`** (conditional)。2 つの Skill 設定タグは、廃止されたカタログ検証済みの `workspace_skill_toggle` および `workspace_skill_batch_toggle` タグを置き換えます。

...
# 12. ワークが drain した後、アイドル状態の ACP 子プロセスを少なくとも 60 秒間再利用可能に保つ
...
| `--hostname <host>`                     | string                         | `127.0.0.1`                                  | 非ループバックにはトークンが必要              | バインドアドレス。ループバックの値: 任意の `127.0.0.0/8` アドレス、`localhost`、`::1`、`[::1]`。`localhost` は 1 回だけ解決されてピン留めされ、信頼モードは起動完了前に実際のリスナーアドレスを検証します。`[::1]` のブラケットは自動的に削除されます。`host:port` 形式の入力は拒否され、`--port` を使用するようガイダンスが表示されます。                                    |
...
| `--enable-session-shell`                | boolean                        | `false`                                      | ベアラまたは信頼されたループバック               | 直接の `POST /session/:id/shell` 実行を有効にします。呼び出し元はセッションにバインドされた `X-Qwen-Client-Id` も送信する必要があります。                                                                                                        |
...
| `--allow-origin <pattern>`              | repeatable string              | none                                         | -                                        | デフォルトの Origin 拒否を置き換える CORS 許可リスト。ワイルドカードおよび非ループバックの HTTP(S) オリジンにはトークンが必要です。                                                                                                                                         |
...
| `--channel-idle-timeout-ms <n>`         | 非負の整数                       | `0`                                          | -                                        | ランタイムのワークが drain した後の ACP 子プロセスの自動回収遅延。プリヒートのみは初回使用のために保持されます。アクティブなキープアライブウィンドウが設定された遅延を延長する場合があります。残りの遅延がより長い方が優先されます。                                                                                                                               |
| `--initialize-timeout-ms <n>`           | number                         | `10000`                                      | -                                        | ACP 子プロセスの起動デッドライン（チャネルファクトリ + initialize ハンドシェイク）およびデフォルトのリクエストタイムアウト（ミリ秒）。                                                                                                                                                                                                                   |
...
| トークンなしの非ループバック HTTP(S) `--allow-origin`                           | `Refusing to start with --allow-origin ... but no bearer token configured`                          |

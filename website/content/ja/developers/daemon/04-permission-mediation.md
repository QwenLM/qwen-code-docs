...
**M = 2** の場合、分割投票（A が X を選択、B が Y を選択）は、投票者のキャンセル、セッションのキャンセル、またはオプションの対話タイムアウトによってのみ解決できます。どのオプションも全会一致に達しないためです。`permissionResponseTimeoutMs` はデフォルトで無効になっています。設定すると、未解決の分割投票はそのデッドラインで `{cancelled, timeout}` として解決されます。投票進行パスは、該当する動作をオペレーター向けに stderr にログ出力します。

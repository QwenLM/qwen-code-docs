...
新規または更新された設計ドキュメントには、同じディレクトリおよび PR 内に完全な英語版（`<name>.md`）と簡体字中国語版（`<name>.zh-CN.md`）を含める必要があります。構造、決定事項、制約、受け入れ基準を一致させ、相互の言語リンクを追加してください。設計ドキュメントの要件については、[英語](../design/README.md) または [简体中文](../design/README.zh-CN.md) を参照してください。

...
以下は、`GEMINI_SANDBOX_PROXY_COMMAND` 環境変数で使用できるプロキシスクリプトの例です。このスクリプトは、`example.com:443` への `HTTPS` 接続のみを許可し、他のすべてのリクエストを拒否します。
...
// :::8877 をリッスンし、example.com への HTTPS 接続のみを許可するプロキシサーバーの例。
// `GEMINI_SANDBOX_PROXY_COMMAND=scripts/example-proxy.js` を設定して、サンドボックスと一緒にプロキシを実行します。
// サンドボックス内で（シェルモードまたはシェルツール経由で）`curl https://example.com` でテストできます。
...
const server = http.createServer((req, res) => {
  // HTTPS の CONNECT 以外のリクエストをすべて拒否
  console.log(

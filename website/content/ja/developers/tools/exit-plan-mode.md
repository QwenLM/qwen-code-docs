...
`exit_plan_mode` は以下の引数を受け取ります。

- `plan` (string, 必須): 承認を得るためにユーザーに提示する実装計画。Markdown 形式で、実装手順を簡潔に記述してください。
- `originalRequest` (string, オプション): 元のリクエスト。plan-required なチームメイトがリーダーに承認を求める際に使用されます。
- `researchSummary` (string, オプション): plan-required なチームメイトのリーダーレビュー用の調査コンテキスト。
...
- **Restore Previous**: 計画を承認し、プランモードの前にアクティブだったモードを復元する
...
このツールは、明示的な判断が受け入れられ実行が開始された後にのみ承認モードを変更します。権限ルール、自動承認モード、および permission-allow フックは、ユーザーに代わってメインセッションのプラン終了を承認することはできません。
...
- **fail closed（失敗時は拒否）**: キャンセル、期限切れの承認、ホスト障害、および移行障害はプランモードのままになります

...
`read_file` は、指定されたファイルの内容を読み取って返します。このツールはテキストファイルと、現在のモデルがサポートするモダリティのメディアファイル（画像、PDF、音声、動画）を処理します。テキストファイルの場合は、特定の行範囲を読み取ることができます。サポートされていない PDF の場合はテキスト抽出が試行され、以下で説明する vision bridge のフォールバックが使用されます。サポートされていないその他のメディアファイルは、役立つエラーメッセージを返します。その他のバイナリファイルタイプは通常スキップされます。

- **ツール名:** `read_file`
- **表示名:** ReadFile
- **ファイル:** `read-file.ts`
- **パラメータ:**
  - `file_path` （文字列、必須）: 読み取るファイルの絶対パス。
  ...
  - `pages` （文字列、オプション）: PDF の場合、1始まりのページまたは閉じたページ範囲（`"3"` や `"20-25"` など）。1回のリクエストで最大20ページまで。
- **動作:**
  ...
  - テキストのみのプライマリモデルでの PDF: まずテキスト抽出が試行されます。抽出が失敗した場合、またはリクエストされた（または実際の）単一ページが12Kトークンのテキスト予算を超える場合、設定された vision bridge が自動的に、リクエストされた開始ページから最大4ページをレンダリングしてトランスクリプト化します。リクエストされた範囲は、実際のドキュメントの終端がわかっている場合にクリップされます。結果はトランスクリプトされた範囲と、残りの既知のページ数（ページ数が不明な場合は追加ページが存在する可能性があること）を識別します。通常の複数ページのテキストオーバーフローでは、vision に切り替える代わりに narrower な `pages` 範囲を要求します。
  - vision bridge による PDF トランスクリプションは損失を伴い、信頼されていないマシン生成コンテンツとしてマークされます。ツール結果はレンダリングされた画像ではなくテキストを含み、ユーザー向けの TUI、ACP、非インタラクティブな構造化出力、およびエクスポート表示は、既知の場合には vision モデルとエンドポイントを識別します。bridge が失敗した場合、正確な元の PDF 抽出エラーがモデルに返され、ユーザー表示には引き続き bridge の試行が開示されます。

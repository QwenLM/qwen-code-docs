...
スタンドアロンアーカイブは、デフォルトでクラシック Node.js フレーバーで提供されます。リポジトリ変数 `OPENTUI_PREVIEW_RELEASE_ENABLED` を `true` に設定すると、bun/OpenTUI プレビューフレーバーもビルドされます。このフレーバーのアーカイブには `-opentui-preview` サフィックスが付き、ランチャーは `QWEN_TUI_RENDERER` のデフォルトを `opentui` に設定します。この変数を未設定のままにすると、そのフレーバーは無効になります。

この変数は、新しいリリースで何をビルドするかだけを決定します。リリースを Aliyun OSS にミラーリングするワークフローはこれを読み取りません。そのワークフローは任意のタグに対して再ディスパッチでき、フレーバーが存在する前にカットされたタグには検証するプレビューアーカイブがないため、そのリリースが実際に出荷したアーカイブからフレーバーを導出します。
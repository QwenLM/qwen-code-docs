...
이 도구는 기본적으로 비활성화되어 있습니다. `settings.json`에서 활성화하고 Qwen Code를 다시 시작하세요:
...
- **옵트인:** `tools.todoWrite.enabled`를 `true`로 설정하고 도구를 사용하기 전에 Qwen Code를 다시 시작하세요.
- **활성화 시 자동 사용:** AI 어시스턴트는 복잡한 작업 중에 todo 목록을 관리합니다.

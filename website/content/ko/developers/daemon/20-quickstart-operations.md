...
# 12. 작업 드레인 후 유휴 ACP 자식을 최소 60초간 재사용 가능하게 유지
...
| `--hostname <host>`                     | string                         | `127.0.0.1`                                      | 비루프백은 토큰 필수                     | 바인드 주소. 루프백 값: 모든 `127.0.0.0/8` 주소, `localhost`, `::1`, `[::1]`; `localhost`는 한 번 resolve되어 고정되며, 신뢰 모드에서는 시작 완료 전 실제 리스너 주소를 검증. `[::1]` 대괄호는 자동 제거; `host:port` 입력은 `--port`를 사용하라는 안내와 함께 거부.                                                                                                                                                              |
...
| `--enable-session-shell`                | boolean                        | `false`                                          | Bearer 또는 신뢰된 루프백                | 직접 `POST /session/:id/shell` 실행을 활성화. 호출자는 세션에 바인딩된 `X-Qwen-Client-Id`도 전송해야 함.                                                                                                                                                                                                              |
...
| `--allow-origin <pattern>`              | 반복 가능 string               | none                                             | -                                        | 기본 Origin 거부를 대체하는 CORS 허용 목록. 와일드카드 및 비루프백 HTTP(S) 출처는 토큰 필수.                                                                                                                                                                                                                                                          |
...
| `--channel-idle-timeout-ms <n>`         |음이 아닌 정수                   | `0`                                              | -                                        | 런타임 작업 드레인 후 ACP 자식 자동 회수 지연. 일반 예열은 첫 사용을 위해 유지됨. 활성 킵얼라이브 창이 설정된 지연을 연장할 수 있으며, 더 긴 남은 지연이 우선.                                                                                                                                                                                                                                                                 |
| `--initialize-timeout-ms <n>`           | number                         | `10000`                                          | -                                        | ACP 자식 시작 기한 (채널 팩토리 + 초기화 핸드셰이크) 및 기본 요청 타임아웃(ms).                                                                                                                                                                                                                                                                    |
...
| 비루프백 HTTP(S) `--allow-origin` 토큰 없음                                   | `Refusing to start with --allow-origin ... but no bearer token configured`                          |
...
정적 셸은 모든 실행 모드에서 `bearerAuth` **이전에** 마운트됩니다 — 브라우저는 주소창 탐색이나 `<script src>` 하위 리소스에 `Authorization` 헤더를 첨부할 수 없으므로, 게이팅하면 UI가 깨집니다. API 호출은 토큰 없는 기본 리스너에서 신뢰 루프백 권한을 사용하거나 인증 모드에서 설정된 bearer를 첨부합니다. 비루프백 바인드에서 셸은 `--allow-origin <origin>`이 전달되지 않으면 읽기 전용입니다 — 동일 출처 POST는 `Origin` 헤더를 전달하며 CORS 벽이 거부(403)합니다 — 루프백 외 바인드에는 `--allow-origin`을 전달하세요.

...
| `multiSession`                           | `sessionScope: 'user'`를 위한 daemon 전용 이름 지정 작업. 소유자 카탈로그는 워크스페이스/channel 상태 디렉토리 아래에 영속화됩니다. 작업은 동시에 실행될 수 있으며, 작업별 Git worktree를 옵트인할 수 있고, 취소 및 권한 명령은 정확한 작업과 연관된 상태를 유지하고, 결과 및 인터랙티브 표면은 소스 작업을 식별합니다. Webhook, 그룹 히스토리 백필, 루프는 계속 제외됩니다. |

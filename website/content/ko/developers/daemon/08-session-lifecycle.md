...
비어 있거나 손상되었거나 고아 상태인 일반 트랜스크립트 파일도 대화로 로드할 수 없는 경우에도 이러한 생명주기 작업의 대상이 될 수 있습니다. 소유권 안전 검사는 의도적으로 실패를 닫고 운영자의 개입을 요구할 수 있습니다. 작성자가 인증된 핸드오프 증명을 봉인한 후에 변경된 파일은 운영자가 봉인된 잠금과 변경된 바이트를 해결할 때까지 `SessionTranscriptChangedError`로 실패합니다. 경계 있는 소유권 읽기 창을 초과하는 JSON 형태의 첫 물리적 레코드는 레코드가 복구되거나 축소될 때까지 `SessionTranscriptIdentityUnavailableError`로 실패합니다. 비객체 접두사를 가진 과도하게 큰 손상 레코드는 대상 자격을 유지합니다. 파싱 가능한 복구 레코드에는 문자열 `sessionId`와 `cwd` 소유권 필드가 포함되어야 하며, 혼합 로컬/외부 아카이브 상태도 실패를 닫습니다. `session_storage_conflict_repair`가 광고될 때 아카이브와 언아카이브는 `resolveConflicts: true`를 받습니다. 아카이브는 아카이브된 사본을 유지하고 언아카이브는 활성 사본을 유지합니다. 이 옵션이 없으면 활성/아카이브 충돌은 두 영속 사본 중 어느 것도 이동, 제거, 덮어쓰기하지 않으며 배치 `errors` 배열에서 반환됩니다. 아카이브는 여전히 충돌을 분류하기 전에 활성 세션을 엄격히 종료하며, 이로 인해 대기 중인 레코드가 활성 트랜스크립트로 플러시될 수 있습니다. 워크스페이스 한정 생명주기 라우트는 이제 이전의 HTTP `409 session_conflict` 응답 대신 HTTP `200` 배치 엔벨로프를 사용합니다.

...
| `turn_error`          | S->C      | turn이 실패함                                                                                                       | `sessionId, message, code?, promptId?`; 동일한 `promptId` 상관 메커니즘. 에이전트 측 `-32603` 실패 시 `message`는 에이전트가 제공한 경우 제공자 고유의 세부 정보를 전달하며(`data.details` / `data.message` / 중첩된 `data.error.message`), 일반 JSON-RPC 텍스트가 아닌 값이다. |

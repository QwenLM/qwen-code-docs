...
명시적 Extension 활성화 새로고침: `extension_activation_explicit_refresh`는 단일 및 배치 활성화 작업이 활성 세션을 직접 새로고침하지 않고 지속성 있는 정책 커밋에서 완료됨을 의미합니다. 즉각적인 적용이 필요한 클라이언트는 활성화 성공을 기다린 후 변경 사항을 즉시 적용해야 하는 각 워크스페이스에 대해 워크스페이스 런타임 새로고침 작업을 제출해야 합니다. 전역 기본 배치는 해당 워크스페이스가 이름에 대한 정확한 재정의(또는 레거시 경로 규칙 일치)를 보유하고 있지 않은 한 각 워크스페이스가 상속하는 기본 활성화를 변경하며, 모든 런타임을 커버하는 단일 새로고침이 없으므로 호출자가 새로고침하지 않은 워크스페이스는 다음 30초 세대 조정기 통과에서만 수렴합니다. 이 태그가 없는 데몬은 이미 활성화 작업에 새로고침을 포함하므로, 호출자는 해당 작업에서 호환성 새로고침을 제출하면 안 됩니다.

...
**M = 2**의 경우, 분할 투표(A는 X 선택, B는 Y 선택)는 요청별 타임아웃으로만 해결될 수 있습니다: 어떤 옵션도 만장일치에 도달하지 못하므로, 요청은 `permissionResponseTimeoutMs`(기본 5분)까지 대기한 후 `{cancelled, timeout}`으로 해결됩니다. 투표 진행 경로는 이 "만장일치이면 분할 투표 타임아웃" 동작을 운영자를 위해 stderr에 기록합니다.

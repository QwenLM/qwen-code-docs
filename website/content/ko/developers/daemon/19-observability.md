...
| `DaemonLogger` 구조화된 파일 로그           | `serve/daemon-logger.ts`                       | 안정적이며 크기 기반 순환되는 `daemon.log`에 추가. 파일 레코드에 `runId`와 PID가 포함됨. 부트 시 선택된 안정적/폴백 경로를 출력; 전체 상태는 헬스, 문제, 파일 복사 손실 카운터를 노출.                                                                                                                                                                    |
...
| `/demo` 디버그 콘솔                         | `GET /demo` (`packages/cli/src/serve/demo.ts`) | 브라우저에서 접근 가능한 단일 페이지 콘솔: 채팅, 이벤트 로그, 워크스페이스 검사기, 권한 UX. 루프백에서 `http://127.0.0.1:4170/demo`는 SDK 코드 작성 없이 가장 빠른 종단간 검증 경로. 등록 규칙은 [`02-serve-runtime.md`](./02-serve-runtime.md)에 있음.                                                                                                      |

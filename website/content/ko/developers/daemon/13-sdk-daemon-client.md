...
- 주석 전용 프레임과 알 수 없는 이벤트 타입(`DaemonEvent`로 전달됨. SDK 소비자는 `asKnownDaemonEvent`를 통해 하류에서 좁힙니다).
...
최종 작업 결과는 정렬된 `results`를 포함합니다. `enabled` 또는 `disabled` 설정 시 대상이 설치되어 있을 필요는 없습니다. 데몬이 이름 선언을 저장하고 해당 이름의 Extension이 나중에 설치될 때 해당 활성화 정책을 보존합니다. 변경된 모든 대상은 하나의 Extension Store 세대를 공유합니다. `extension_activation_explicit_refresh`가 광고되면, 활성화 작업은 내구성 있는 정책 커밋 후에 활성 세션을 새로고침하지 않고 완료됩니다. 즉시 적용이 필요한 호출자는 세션이 변경 사항을 즉시 적용해야 하는 각 워크스페이스에 대해 `workspace.refreshExtensionRuntime()`를 제출해야 합니다. 새로고침은 별도의 작업이며 대기하거나 백그라운드에 둘 수 있습니다. 전역 기본 배치는 해당 워크스페이스에 이름에 대한 정확한 오버라이드가 없거나 레거시 경로 규칙과 일치하지 않는 한, 등록된 모든 워크스페이스가 상속하는 기본 활성화를 변경하며, 모든 런타임을 커버하는 단일 새로고침은 없습니다. 워크스페이스 배치는 선택된 신뢰 런타임만 변경합니다. 이전 데몬은 이미 활성화 작업 내부에서 새로고침하므로, 클라이언트는 해당 기능이 없는 한 추가 새로고침을 제출하면 안 됩니다. 30초 세대 조정기는 호출자가 새로고침하지 않은 워크스페이스에 대한 독립적인 최종 수렴 경로로 남아 있습니다. 워크스페이스 `inherit`은 정확한 오버라이드를 제거하지만 알 수 없는 이름에 대한 선언을 생성하지 않습니다. 모두 알 수 없는 이름의 제거는 no-op로 성공합니다. 단일 활성화 메서드는 설치된 대상만 유지합니다.

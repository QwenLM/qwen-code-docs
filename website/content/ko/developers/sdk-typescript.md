...
- Node.js >= 22.0.0
- [Qwen Code](https://github.com/QwenLM/qwen-code) >= 0.4.0 (안정) 설치 및 PATH에서 접근 가능

> **nvm 사용자를 위한 참고 사항**: nvm으로 Node.js 버전을 관리하는 경우, SDK가 Qwen Code 실행 파일을 자동으로 감지하지 못할 수 있습니다. `pathToQwenExecutable` 옵션을 `qwen` 바이너리의 전체 경로로 명시적으로 설정해야 합니다.
...
| `coreTools`              | `string[]`                                               | -                | 레거시 `coreTools` / CLI `--core-tools` 허용목록 시맨틱스를 사용합니다. 지정되면 일치하는 핵심 도구만 세션에 등록됩니다. 이는 settings.json의 `permissions.allow`와 별개입니다. 후자는 시작 시 레지스트리 레벨 허용목록도 활성화합니다: 최소 하나의 유효한 허용 규칙이 구성되어 있으면(잘못된 항목은 계산되지 않음), 허용 또는 ask 규칙으로 커버되지 않는 내장 도구가 등록되지 않습니다(MCP 도구, `--json-schema` `structured_output` 계약, 계획 모드 라이프사이클 도구, `task_stop`, `tool_search`, `computer_use__*` 패밀리는 면제; 재시작 필요, #9827). SDK `allowedTools` 파라미터만으로는 허용목록을 활성화할 수 없지만, 허용목록이 활성 상태인 동안에는 해당 규칙이 유효 허용 집합에 병합되어 커버리지에 기여하므로 커버된 내장 도구가 등록된 상태로 유지됩니다. 예시: `['read_file', 'edit', 'run_shell_command']`. |
...
| `allowedTools`           | `string[]`                                               | -                | 자동 승인을 위해 settings.json의 `permissions.allow`와 동일합니다. 일치하는 도구는 `canUseTool` 콜백을 우회하여 자동으로 실행됩니다. 도구가 확인을 요구할 때만 적용됩니다. settings.json의 `permissions.allow`와 달리, 이 파라미터만으로는 레지스트리 허용목록을 활성화하지 않습니다; 단, settings 제공 허용목록이 활성 상태인 동안에는 `allowedTools` 규칙이 유효 허용 집합에 병합되어 커버리지에 기여하므로 커버된 내장 도구가 등록된 상태로 유지됩니다. `excludeTools`와 동일한 패턴 매칭을 지원합니다. 예시: `['Bash(git status)', 'Bash(npm test)']`.                                                                                                                                                                                                                                       |

...
| `coreTools`              | `string[]`                                               | -                | 레거시 `coreTools` / CLI `--core-tools` 허용목록 시맨틱스를 사용합니다. 지정되면 일치하는 핵심 도구만 세션에 등록됩니다. 이는 settings.json의 `permissions.allow`와 별개입니다. 후자는 시작 시 레지스트리 레벨 허용목록도 활성화합니다: 최소 하나의 유효한 허용 규칙이 구성되어 있으면(잘못된 항목은 계산되지 않음), 허용 또는 ask 규칙으로 커버되지 않는 내장 도구는 deferred로 전환됩니다 — `tool_search`를 통해 등록 및 로드 가능하지만, 스키마는 즉시 모델 요청에서 제외됩니다(MCP 도구, `--json-schema` `structured_output` 계약, 계획 모드 라이프사이클 도구, `task_stop`, `tool_search`, `computer_use__*` 패밀리는 면제; 재시작 필요, #9827, #10075). SDK `allowedTools` 파라미터만으로는 허용목록을 활성화할 수 없지만, 허용목록이 활성 상태인 동안에는 해당 규칙이 유효 허용 집합에 병합되어 커버리지에 기여하므로 커버된 내장 도구가 등록된 상태로 유지됩니다. 예시: `['read_file', 'edit', 'run_shell_command']`. |

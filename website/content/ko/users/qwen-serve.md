...
- ❌ **자동 생성 데몬 토큰** — alpha는 BYO-token(`openssl rand -hex 32` 하나면 충분). 자동 생성 + 토큰 저장소 인프라는 v0.16.x로 연기됩니다.
...
호출하세요. 라우트는 워크스페이스 `skills.disabled` 및 `skills.enabled`를
...
| `--memory-project-scope <mode>`         | `git-root`         | 프로젝트 메모리 분할 모드. `git-root`(기본값)는 동일한 Git 루트로 확인된 워크스페이스 간에 메모리를 공유합니다; `workspace`는 정확히 등록된 워크스페이스 디렉토리별로 메모리를 키잉하여 각 데몬 워크스페이스가 자체 격리된 메모리를 갖습니다. 제공 시 `QWEN_CODE_MEMORY_PROJECT_SCOPE`를 재정의합니다; 인식되지 않는 env 값은 일회성 경고와 함께 무시되고 `git-root`로 폴백합니다. `workspace`로 전환하면 기존 git-root 프로젝트 메모리가 마이그레이션되지 않습니다 — 다시 전환할 때까지 해당 항목이 표시되지 않습니다.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
| `--max-journal-events <n>`              | `10000`            | 비행 중인 라이브 저널(현재 미완료 턴)에 보유된 원시 이벤트 수의 세션당 상한. 초과하면 가장 오래된 저널 항목이 삭제되고 `history_truncated` 마커가 앞에 추가됩니다. 양의 안전 정수여야 합니다.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| `--max-journal-bytes <n>`               | `8388608`          | 비행 중인 라이브 저널의 세션당 바이트 상한. 초과하면 가장 오래된 저널 항목이 삭제됩니다(최소 하나의 항목은 항상 유지). 양의 안전 정수여야 합니다. 기본값 8 MiB.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
- **Host 헤더 허용 목록** — **루프백** 바인드에서 데몬은 `Host:`가 `localhost:port` / `127.0.0.1:port` / `[::1]:port` / `host.docker.internal:port`와 일치하는지 확인합니다(RFC 7230에 따라 대소문자 구분 없음). 일치하지 않는 `Host`는 `403`을 받습니다.

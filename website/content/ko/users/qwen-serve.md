...
- ✅ **철회 가능한 Local Control 페어링 토큰** — `--local-control`은 데몬이 소유하는 별도의 LAN 페어링 토큰을 생성합니다. 일반 데몬 토큰 저장은 BYO-token을 유지합니다.
...
토큰을 수동으로 생성하지 않고 인증된 단일 사용자 실행을 하려면 명시적으로 옵트인하세요:

```bash
qwen serve --open-with-auth
```

이 루프백 전용 모드는 `--token`이나 `QWEN_SERVER_TOKEN`이 토큰을 제공하지 않을 때 256비트 베어러 토큰을 생성하고, 열린 Web Shell에 `#token=` URL 프래그먼트로 전달합니다. 셸은 프래그먼트를 제거하고 해당 탭의 `sessionStorage`에 자격 증명을 유지합니다; 새로고침은 작동하지만 탭을 닫거나 데몬을 재시작하면 자격 증명이 손실됩니다. CI, SSH 또는 자동 열기를 사용할 수 없는 다른 환경에서는 데몬이 시작되고 프래그먼트 URL이 출력되어 수동으로 열 수 있습니다. 출력된 URL은 시크릿을 포함합니다.

이 플래그는 기본적으로 꺼져 있으며, 브라우저 열기 동작을 포함하고 Web Shell, 빌드된 Web Shell 자산 및 루프백 바인드가 필요합니다. 일반적인 `qwen serve --open`은 루프백에서 토큰 없이 유지됩니다. 인증된 열기 모드에서 일반 API 라우트는 베어러 없는 다른 로컬 클라이언트를 거부하며, 정적 Web Shell 자산과 루프백 `/health`는 `--require-auth`도 설정되지 않은 한 기존 사전 인증 동작을 유지합니다. 여러 클라이언트나 다시 열 수 있는 Web Shell의 경우 안정된 공유 토큰을 대신 사용하세요:
...
데몬 지원 `qwen channel set` 및 `qwen channel reload`, 그리고 `--daemon-url`과 함께 사용하는 `status` 및 `stop`은 `--open-with-auth`가 생성한 토큰을 발견할 수 없습니다. `QWEN_SERVER_TOKEN`을 사용하고 동일한 값을 `--token`으로 해당 클라이언트에 전달하거나, 인증된 열기 모드를 생략하세요.
...
`--open-with-auth`는 CLI 소유의 편의 기능이며 또 다른 데몬 토큰 소스가 아닙니다: 해당 옵션이 정의되어 있으면(비어 있더라도) `--token`을 선택하고, 그렇지 않으면 `QWEN_SERVER_TOKEN`을 선택한 다음 선택된 값을 트리밍하고 결과가 비어 있는 경우에만 생성합니다. 데몬은 생성된 값을 영속화하거나 `QWEN_SERVER_TOKEN`으로 내보내지 않으며, 기존 내부 인증된 자식 핸드오프는 변경되지 않습니다. Web Shell은 브라우저 사본를 수신 탭의 `sessionStorage`에만 저장합니다; 이 모드는 탭 간 또는 외부 클라이언트 자격 증명 발견 메커니즘을 추가하지 않습니다. 토큰은 독립적으로 철회할 수 없으며 클라이언트 아이덴티티에 연결되지 않습니다. 소유는 다른 베어러 토큰과 동일한 데몬 권한을 부여합니다. [인증된 Web Shell 실행 설계](../design/2026-08-22-serve-open-with-auth.md) 및 [#4514](https://github.com/QwenLM/qwen-code/issues/4514)의 관련 향후 작업을 참조하세요.
...
| `--local-control`                       | `false`            | 선택된 하나의 비공개 IPv4 인터페이스에서 데몬 소유 철회 가능한 페어링 토큰, 터미널 QR 코드, 정확한 브라우저 오리진 및 최선 노력 절전 억제로 Web Shell을 공유합니다. `--token`, `--allow-origin` 및 `--port 0`과 조합 가능합니다; `--no-web` 및 비기본 `--hostname`과 충돌합니다. 여러 LAN 후보가 있을 때 `--local-control-address`를 사용하세요. 음성 입력과 같은 보안 컨텍스트 브라우저 API를 위해서는 `--tls-cert` + `--tls-key`를 추가하세요.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
| `--memory-budget-mb <n>`                | cgroup/호스트의 50% | 전체 데몬 프로세스 트리에 대한 총 메모리 예산(MB). 설정되지 않으면 cgroup 제한 또는 호스트 메모리의 50%로 파생됩니다; 어쨌든 유효 값은 해석된 사용 가능 메모리로 제한되며, 구성된 값과 유효 값 모두 보고됩니다. `qwen --acp` 자식의 크기를 변경하지 않습니다; 현재 유일한 소비자는 적응형 라이브 저널 성장입니다: 유효 예산의 5%로 파생된 데몬 전체 성장 풀(`1024` MB로 상한; `insufficientMemory`를 보고하는 호스트에서는 풀이 0이고 적응형 성장이 비활성화)이 모든 워크스페이스 브리지에 공유됩니다 — `--max-journal-bytes` 참조. 해석된 수치는 `GET /daemon/status`의 `limits.memory` 아래에, 등록된 및 라이브 자식 수와 `runtime.memory` 아래의 자식당 권장 공유와 함께 나타납니다. 최소값에 비해 너무 작은 호스트는 클램프 업 대신 `insufficientMemory`를 보고합니다; 파생 비율이 50%이므로 ~2 GB 미만의 호스트는 이를 트리거합니다. 이러한 호스트에서 `--memory-budget-mb 1024`를 명시적으로 전달하여 파생 값을 재정의하세요(플래그는 여전히 경고를 지우기 위해 최소 1024 MB의 사용 가능 메모리가 필요). `[1024, 1048576]` 범위의 정수여야 합니다. |
...
| `--compacted-replay-max-bytes <n>`      | `4194304`          | `POST /session/:id/load`가 반환하는 유한 스냅샷의 보유 재생 이벤트에 대한 세션당 바이트 상한. 상한은 `compactedReplay`에 적용됩니다; 현재 비행 중인 `liveJournal`은 `--max-journal-events`와 `--max-journal-bytes`(적응형 성장이 높일 수 있는 기본 상한 — `--max-journal-bytes` 참조)로 별도로 제한됩니다. 값은 양의 안전 정수여야 합니다; 잘못된 값은 부팅 시 실패하며 하드 상한은 256 MiB입니다. 오래된 보유 재생이 삭제되면 스냅샷은 `history_truncated`로 시작됩니다. 이것은 디스크 트랜스크립트를 제한하지 않습니다.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| `--max-journal-events <n>`              | `10000`            | 현재 미완료 턴의 비행 중 `liveJournal`에 보유된 재생 항목의 세션당 기본 상한. 연속되는 호환 텍스트 또는 사고 청크는 항목을 공유하며, 항목당 최대 256개의 소스 이벤트가 가능합니다; 다른 이벤트 경계는 보존됩니다. 초과하면 데몬이 먼저 적응형 성장을 시도합니다(`--max-journal-bytes` 참조); 여유가 부여되지 않거나 부여가 초과분을 커버하지 못하면 가장 오래된 항목이 삭제되고 `history_truncated` 마커가 앞에 추가됩니다. 마커의 `truncatedEvents`와 `retainedEvents` 카운트는 소스 이벤트를 설명합니다. 양의 안전 정수여야 합니다. 이 플래그(또는 `--max-journal-bytes`)를 고정하면 적응형 성장이 비활성화됩니다.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| `--max-journal-bytes <n>`               | `8388608`          | 비행 중 `liveJournal`의 세션당 기본 바이트 상한으로, 호환 청크가 재생 항목을 공유하더라도 직렬화된 소스 이벤트로부터 계산됩니다. 턴이 상한을 초과하면 적응형 성장이 세션의 상한을 두 배로 높입니다(세션당 하드 상한 256 MiB까지, 남은 풀 여유로 제한). 데몬의 모든 라이브 세션에 걸쳐 부여된 성장은 데몬의 유효 메모리 예산의 5%로 크기가 지정된 공유 성장 풀에 맞습니다(`--memory-budget-mb` 값, 전달된 경우; 해석된 사용 가능 메모리로 상한; `1024` MB로 제한; `insufficientMemory`를 보고하는 호스트에서는 풀이 0이고 적응형 성장이 비활성화). 성장은 수요에 따라 발생하며 풀이 허용하는 범위 내에서만 이루어집니다; 거절되거나 풀이 소진되거나 부여가 초과분을 커버하지 못하면 가장 오래된 항목이 전체적으로 삭제됩니다(최소 하나의 항목은 항상 유지되므로 보유된 꼬리가 상한보다 훨씬 작을 수 있습니다). 이 플래그(또는 `--max-journal-events`)를 고정하면 적응형 성장이 비활성화됩니다. 양의 안전 정수여야 합니다. 기본값 8 MiB. |
...
| `--allow-origin <pat>`                  | —                  | T2.4 ([#4514](https://github.com/QwenLM/qwen-code/issues/4514)). 브라우저 webui 클라이언트를 위한 크로스 오리진 허용 목록. 반복 가능. 각 값은 `*`(모든 오리진 — 베어러 토큰이 구성되지 않으면 부팅 거부; `/health`도 루프백에서 기본적으로 사전 인증이므로 `--require-auth`가 루프백에서 권장; Web Shell 정적 자산은 모든 모드에서 사전 인증 상태를 유지하므로 `--no-web`을 전달하여 제거) 또는 표준 URL 오리진(`<scheme>://<host>[:<port>]`, 후행 슬래시/경로/사용자 정보/쿼리 없음). **서브도메인 와일드카드(`https://*.example.com`)는 의도적으로 지원되지 않습니다** — 각 서브도메인을 명시적으로 나열하거나 구성된 토큰(완전한 강화를 위해 `--require-auth`)과 함께 `*`를 사용하세요. 매칭된 오리진은 CORS 응답 헤더(`Access-Control-Allow-Origin`, `Vary: Origin`, 메서드, 헤더, max-age 및 노출된 `Retry-After`)를 받습니다; 매칭되지 않은 오리진은 여전히 현재 월과 동일한 인벨롭으로 403을 받습니다. `Origin: null`(샌드박스 iframe, file:// 문서)은 `*`에서도 항상 거부됩니다. `caps.features.allow_origin`으로 사전 점검. 루프백 자체 오리진 히트는 영향을 받지 않습니다. |
...
| `--open-with-auth`                      | `false`            | 루프백에서 베어러 인증과 함께 Web Shell을 엽니다. 활성화된 Web Shell과 빌드된 자산이 필요합니다. 구성된 토큰을 재사용하거나 프로세스 수명의 256비트 베어러를 생성하여 Web Shell 프래그먼트로 전달합니다. 브라우저를 사용할 수 없는 환경에서는 시크릿을 포함하는 수동 URL을 출력합니다. 다른 클라이언트는 동일한 명시적으로 구성된 공유 토큰이 필요합니다.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |

> **메모리 프로젝트 범위 주의사항.**
>
> - **데몬 vs. 독립 실행형 CLI.** 플래그 또는 데몬 시작 환경이
>   해당 데몬이 소유한 모든 런타임에 대해 하나의 동결 범위를
>   설정합니다. 워크스페이스 `.env` 또는 `settings.env`는 등록된
>   워크스페이스 하나에 대해 이를 재정의할 수 없습니다. 독립 실행형
>   `qwen` TUI는 여전히 git-root 범위를 기본값으로 사용합니다; 두
>   진입점을 일관되게 유지하려면 이를 시작하는 셸 또는 서비스
>   환경에서 `QWEN_CODE_MEMORY_PROJECT_SCOPE`를 export하세요.
...
모든 관리 데몬 ACP 세션은 `--external-tool-guard-mode`와 독립적으로, 기능 광고 없이 모델 셸 명령에 대한 내장 실행 전 가드를 적용합니다. 데몬은 바인딩된 워크스페이스와 세션의 현재 유효 작업 디렉토리를 소유하며, 둘 다 신뢰된 세션 상태에서 제공되고 ACP 자식으로부터 수락되지 않습니다.

가드는 셸 명령줄을 실행하는 도구 — `run_shell_command` 및 `monitor` — 를 검사하고, 저장소 위치가 세션의 유효 작업 디렉토리 외부로 해석되는 경우 실행 전에 변경 가능한 Git 명령을 거부합니다. 재배치는 리터럴 형태의 `git -C <path>`, `git --git-dir[=]<path>`, `git --work-tree[=]<path>`, 선행 `GIT_DIR`/`GIT_WORK_TREE`/`GIT_COMMON_DIR`/`GIT_INDEX_FILE` 할당(`export`/`declare`/`readonly`를 통해 이루어진 경우 포함, 이는 체인의 이후 모든 명령의 환경에 유지됨), 디렉토리 이동 래퍼 플래그(`env -C`, `sudo -D`) 및 동일한 명령 체인의 초기 `cd`, `pushd` 또는 `popd` 내장 명령에 대해 인식됩니다. 일반적인 래퍼 접두사(`sh -c`, `bash -c`, `eval`, `sudo`, `nohup`, `timeout`, `exec`, `command`, `builtin`, `env`, 경로 한정 `git` 바이너리 및 `{ …; }` / `! …` 셸 구문)은 해제되어 동일한 정책이 내부 Git 호출에 적용되며, `$(…)` 또는 백틱 대체 본문은 자체 명령으로 분석됩니다.

자체 worktree에 고정된 서브에이전트는 세션의 디렉토리가 아닌 해당 worktree로 제한되며, 데몬이 실행 디렉토리를 배치할 수 없는 셸 호출은 거부됩니다.

상대 대상은 표준 경로 해석을 포함한 후 명령의 유효 시작 디렉토리(존재하는 경우 `arguments.directory`, 그렇지 않으면 세션의 현재 유효 작업 디렉토리)에서 해석됩니다(.git gitfile 리디렉션, 심볼릭 링크 및 worktree별 관리 디렉토리 포함). 실행 전에 완전히 해석할 수 없는 재배치 대상(동적 대상(`$VAR`, 백틱, `~`, 글로브), 아직 존재하지 않는 경로, 읽을 수 없는 간접 참조)은 변경 가능 또는 분류 불가능한 서브명령에 대해 거부됩니다. 해석할 수 없는 재배치 대상은 서브명령과 관계없이 — 읽기 전용 서브명령도 — 거부됩니다. 서브명령이 작은 검증된 읽기 전용 집합(`rev-parse`, `cat-file`) 중 하나인 재배치 명령은 대상이 해석되면 계속 허용됩니다. 단, 명령이 명령 실행 `-c` 구성을 가지거나 `--output`, `--textconv` 또는 `--filters` 플래그가 있는 경우는 제외됩니다(이러한 플래그는 파일을 작성하거나 대상 저장소의 구성된 드라이버를 실행함). 인식된 재배치가 없는 명령은 기존 동작을 유지합니다. 거부는 최종이며 해석된, 동적 또는 해석 불가능한 저장소 위치에 대해 `Daemon shell guard denied a mutating Git command…`로, 명령을 구문 분석할 수 없거나 페이로드를 해석할 수 없거나 인식되지 않은 프로그램이 재배치 Git 명령을 실행할 수 있는 경우 `Daemon shell guard denied a shell command…`로 모델에 보고됩니다.

가드는 위의 리터럴 형태로 작성된 Git 재배치에 대해 신뢰할 수 있습니다 — 이 제어가 존재하는 잘못된 대상 명령 — 하지만 이를 방어하기 위해 작성된 셸 텍스트에 대해서는 **최선 노력이며 경계가 아닙니다**. 정적 판독자에게 재배치를 숨기는 구성은 통과할 수 있으며 새로운 구성이 계속 발견될 것입니다. 이를 이유로 데몬에게 더 넓은 신뢰를 부여하지 마세요. 스크립트 파일을 해석하거나, 명령 간 환경 변수 값을 추적하거나, heredoc 본문을 분석하지 않습니다(heredoc 내부의 Git 형태 텍스트는 셸이 절대 실행하지 않더라도 거부될 수 있음). `/fork` 및 에이전트 지원 워크스페이스 메모리 remember/dream은 내장 가드 아래에서 계속 사용 가능하며, 외부 프로바이더 모드가 활성 상태일 때만 제한됩니다. 선택적 외부 도구 가드는 추가 정책이며 내장 정책이 허용한 후에만 동일한 요청을 받습니다.
...
이슈 [#4514](https://github.com/QwenLM/qwen-code/issues/4514) T2.9는 15초 하트비트 + AbortSignal이 커버하지 않는 장기 실행 / 원격 배포 격차를 닫는 두 개의 옵트인 플래그를 제공합니다. 공유 권한 응답 타임아웃도 여기에 표시됩니다. 세 개 모두 기본값이 꺼져 있습니다.
...
| `--permission-response-timeout-ms <n>` | —                                   | `0`     | 데몬 모드에서 일반 권한 및 `ask_user_question` 응답에 대한 공유 벽시계. `0` 또는 플래그 생략은 무기한 대기합니다; 양의 정수는 둘 다에 마감을 부과합니다. 투표자 취소, 세션 취소 및 데몬 종료은 타이머가 비활성화된 경우에도 대기 중인 상호 작용을 해결합니다. |

프롬프트 및 writer 플래그는 밀리초 단위의 양의 정수를 허용합니다; `0`, `NaN`, 비정수 또는 음수 값은 부팅 시 명확한 오류 메시지와 함께 거부됩니다. 권한 응답 타임아웃은 `0` 또는 양의 정수를 허용합니다. 환경 변수 기반 두 마감의 경우 명시적 `ServeOptions` 필드가 env 값보다 우선합니다. SDK 소비자는 프롬프트 및 writer 동작에 의존하기 전에 일치하는 기능 태그를 사전 점검해야 합니다.

### 에이전트 텍스트 쓰기의 새 파일 모드

에이전트 텍스트 쓰기(`write_file`, `edit`, `notebook_edit` 및 셸 도구의 시뮬레이션된 sed 편집기)는 데몬의 원자적 작성자를 통해 게시되며, 기존 대상의 모드를 보존하고 **새** 파일의 경우 데몬 프로세스의 umask를 무시하고 소유자 전용 `0600`을 기본값으로 사용합니다. 이 실패 폐쇄 기본값은 의도적입니다: 새로 생성된 에이전트 파일은 감독 umask가 얼마나 관대하든 실수로 그룹/월드 읽기 가능이 되지 않습니다.

배포 규칙이 umask 기반인 운영자(예: `UMask=0002`가 있는 systemd 유닛, 공유 그룹 저장소)는 다음을 통해 새 파일을 표준 POSIX 처리로 옵트인할 수 있습니다:

| 환경 변수                  | 값                  | 기본값  | 수행 내용                                                                                                                                                                                                                                                                                                                                                                                                                    |
| -------------------------- | ------------------- | ------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `QWEN_SERVE_NEW_FILE_MODE` | `owner` \| `system` | `owner` | `system`은 새 파일을 `0o666 & ~umask`로 생성하므로 에이전트가 생성한 파일은 머신의 다른 프로세스와 마찬가지로 데몬 프로세스의 umask를 따릅니다. `owner`는 umask와 독립적인 `0600` 기본값을 유지합니다. 값은 대소문자를 구분하지 않습니다; 리터럴 `0600`은 `owner`의 별칭으로 허용됩니다(다른 8진 모드는 지원되지 않음), 다른 값은 stderr 경고와 함께 거부되고 `0600` 기본값이 유지됩니다. |
...
두 가지 구조적 선택이 Stage 1 / 1.5 / 2 메인 라인 로드맵의 명시적 비목표입니다. 사용 사례가 이 중 하나에 의존한다면, 우리를 기다리기보다는 이에 맞춰 계획하세요.

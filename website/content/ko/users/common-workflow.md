...
Qwen Code가 코드 리뷰, 인시던트 트러블슈팅, 감사 또는 프로덕션 데이터 조사와 같은 고위험 작업을 수행할 때, 확신에 차 있지만 잘못된 결론은 결론이 없는 것보다 더 큰 비용을 초래합니다. 팀의 검증 정책을 QWEN.md(참조: [Memory](./features/memory.md))에 인코딩하여 모든 세션에 적용되게 할 수 있으며, 아무의 기본값도 변경하지 않아도 됩니다.

아래 규칙은 멀티 에이전트 배포의 프로덕션 회고에서 adaptación된 것으로, 확신에 차 있지만 잘못된 결론이 반복적으로 동일한 실패 모드에서 기인했습니다: 코드를 쿼리하는 대신 코드에서 상태를 추론하고, 하나의 충족된 조건을 증명으로 간주하며, 이전 진술을 사실로서 반복하는 것입니다.

**1. 프로젝트 QWEN.md에 검증 정책 추가**

이 템플릿을 저장소 루트의 `QWEN.md`에 넣고 커밋하여 전체 팀에 정책이 적용되도록 합니다:

```markdown
# 검증 정책

조사, 트러블슈팅, 리뷰 및 결론을 도출하는 모든 작업에서 이 규칙을 필수로 간주합니다:

1. 결론 내리기 전에 검증하라. 데이터나 시스템 상태에 대한 주장을 권한 있는 소스(데이터베이스, API, 로그, 명령 출력)의 증거로 뒷받침하라. 코드 읽기만으로 도출된 결론은 "미검증 추론"으로 표시해야 한다.
2. 전체 체인을 검증하라. 인과 체인을 열거하고 모든 링크를 확인하라; 하나의 충족된 필요 조건이 결론을 증명하지는 않는다.
3. 대화 기록을 사실이 아닌 단서로 간주하라. 이전 트랜스크립트의 진술—자신의 이전 주장 포함—은 단서이다. 사실로서 반복하기 전에 소스에서 다시 검증하라.
4. 순서대로 트러블슈팅하라: 자신의 최근 변경 사항을 먼저, 그 다음 문서와 알려진 이슈, 그리고 마지막으로 외부 의존성을 확인하라.
5. 리뷰에서 100% 증거 규칙. 리뷰 및 감사 작업에서 증거 없이 결론을 보고하지 말라; 모든 주장에 검증 가능한 증거(파일:행, 쿼리 결과, 로그 발췌)를 첨부하라.
```

**2. 올바른 범위 선택**

어떤 파일이 누구에게 적용되는지는 [Where to create QWEN.md](./features/memory.md#where-to-create-qwenmd)를 참조하세요 — 프로젝트 루트의 `QWEN.md` 사본을 커밋하여 전체 팀에 정책이 적용되도록 합니다.

> [!tip]
>
> - 규칙을 구체적이고 실행 가능하게 유지하세요: "모든 주장을 쿼리 결과로 뒷받침하라"가 "주의하라"보다 더 효과적입니다.
> - 정책을 짧게 유지하세요 — QWEN.md는 간결할 때 더 안정적으로 준수됩니다(참조: [Memory](./features/memory.md)).
> - 규칙을 스택에 맞게 적응시키세요: 팀이 권한 있는 소스로 간주하는 실제 쿼리 도구, 대시보드 또는 로그 명령을 명시하세요.

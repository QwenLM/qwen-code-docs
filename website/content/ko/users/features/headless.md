...
> [!note]
>
> - `Learning`은 사용자에게 코드 작성을 요구하고 응답을 기다리므로 헤드리스 런에서는 건너뛴다.
> - 알 수 없는 스타일 이름은 경고를 출력하며 기본 스타일로 런을 계속한다.
> - `--output-style`은 `--system-prompt` 또는 `QWEN_SYSTEM_MD`가 내장 프롬프트를 대체하면 효과가 없다 — 스타일은 내장 프롬프트에만 적용된다.

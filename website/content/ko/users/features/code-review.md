...
| -------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------- | ------------------------------------- | ---------------- |
| `low`    | diff에 대한 3-6개의 방향 인라인 각도(diff 크기에 따라 조절)와 갭 스윕 및 under-floor 재패스 — 서브에이전트 없음, 빌드/테스트 없음, 프로젝트 규칙 없음                 | 10 (미검증)         | 없음                                  | 절대 안 함       |
...
단계 3C: low 노력: 3-6개 인라인 각도 + 갭 스윕 + under-floor 재패스
                                                               [0 서브에이전트 호출]
...
| 에이전트 6: Undirected Audit      | 3개 병렬 페르소나 (공격자 / 3am-oncall / 유지보수자) — 교차 차원 이슈를 잡음 — 그리고 counter-frame 감사(6d, high 노력의 PR 대상): PR 설명을 읽어서만 지정된 주제를 제외하고 병합 다음 날 동기화 이벤트를 재생                                                                                                                                                |
...
| `prose-exec`: Prose 실행            | diff가 지시 파일(`SKILL.md`, `AGENTS.md`/`CLAUDE.md`/`QWEN.md`, 에이전트 또는 명령 정의, `prompts/` 파일)을 건드리고 리뷰에 트리가 있을 때: 일회용 복사본에서 변경된 지시를 실행하고 실행 결과와 산문이 약속하는 것 사이의 갭을 기록                                                                                                                          |

세 Correctness 에이전트는 **절차적**입니다: 각 에이전트는 diff를 어떻게 순회하는지(라인별 / 삭제된 라인 / 교차 파일 엣지)로 정의되며, 버그 분류로 정의되지 않습니다 — 따라서 커버리지가 중복되지 않고 상호 보완적입니다. 두 개의 추가 전용 각도(1d/1e)가 언어 함정 체크리스트와 래퍼/프록시 라우팅을 라인별 순회로부터 분리합니다: 체크리스트 패턴 매칭과 구조적 라우팅 기대는 서로 다른 주의 모드이며, 순회에 포함되면 그 리듬에 의해 희석됩니다. 동일한 추론이 **코드 품질을 세 개로 분할**합니다(3a/3b/3c): 6개 항목 체크리스트를 가진 하나의 에이전트는 한 항목을 처리합니다 — 무겁게 재작성된 파일에서 측정하면, 8개 항목 체크리스트를 가진 하나의 에이전트는 5개 결함 중 1개를 찾았지만 같은 모델이 세 방향으로 분할되면 모두 5개를 찾았습니다 — 따라서 품질 체크리스트는 질문이 실제로 달라지는 지점에서 분할됩니다. 모든 에이전트는 병렬로 실행됩니다(에이전트 1은 3개 절차적 변형과 2개 전용 각도를 실행, 에이전트 3은 3개 체크리스트 슬라이스를 실행, 에이전트 6은 3개 페르소나 변형과 counter-frame 감사 6d를 동시에 실행하여, 동일 저장소 PR 리뷰에 대해 총 최대 17개 병렬 작업 — 에이전트 1e는 diff가 래핑 타입을 신호할 때만 실행 — diff의 도메인에서 필요할 때 0-2개의 에이전트 8 파인더 추가 — 따라서 실제로 16-19개, diff가 지시 파일을 건드릴 때 하나 더(`prose-exec`) 추가; 에이전트 0과 6d는 로컬 diff 및 파일 경로 리뷰에서 건너뛰어져 14-17개 실행; 교차 저장소 경량 모드도 에이전트 1c와 7을 건너뛰어 PR 정체성과 함께 14-17개, 없이 12-15개 실행).
...
전용 이미지 호스트 저장소를 권장하며; 포크나 임시 저장소도 작동합니다. 리뷰 대상 저장소는 피하세요 — 여기에 푸시된 이미지 브랜치는 모든 클론이 가져오는 도달 가능한 객체가 됩니다. 이미지는 `pr-assets/<pr>-review` 브랜치에 콘텐츠 해시된 이름으로 저장되며, 댓글은 **커밋 고정** URL로 참조합니다 — 브랜치가 나중에 이동해도 불변이며, GitHub Enterprise에서도 변경 없이 작동합니다.

GitHub 트리거 리뷰(PR 리뷰 워크플로)의 경우, 같은 이름의 **저장소 변수**에서 동일한 변수가 연결되며, 워크플로는 전용 외부 호스트만 통과시킵니다: 변수가 설정되지 않거나 리뷰 중인 저장소로 설정되면(트리밍 및 대소문자 정규화 후 비교되므로 같은 저장소의 패딩되거나 대소문자가 변경된 철자는 자체 대상으로 간주됨), 워크플로는 빈 값을 전달하고 게시가 거부됩니다 — 아무것도 변경되지 않으며, 리뷰는 증거를 산문과 로컬 아티팩트 경로로 유지합니다. 저장소의 Actions 변수에 `QWEN_REVIEW_ASSETS_REPO`를 별도 이미지 호스트 저장소로 설정한 유지관리자는 리뷰 댓글이 캡처 PNG를 포함할 수 있도록 합니다. 외부 대상은 자체 보존을 관리합니다; 시각적 정리 워크플로는 이전 Git 기반 게시자가 이 저장소에 남긴 기록 `pr-assets/*` 브랜치만 비웁니다.
...
`target`은 PR 번호, PR URL 또는 파일 경로입니다; 로컬 작업 트리를 리뷰하려면 생략하세요. 명령은 이 빌드의 자체 CLI를 비대화형으로 실행하며(stdin이 닫힌 상태로, 슬래시 명령어 감지가 유지됨), 자식의 진행 상황을 **stderr**로 스트리밍하고, 판정을 **stdout**에 출력합니다 — 또는 `--json`과 함께 전체 결과 객체를 출력합니다. 판정은 `compose-review`가 작성하는 아티팩트에서 읽힙니다(skill이 판정 권한으로 취급하는 동일한 JSON). 모델의 산문에서 파싱되지 않습니다.
...
| `candidateFloor` | `low`가 한 번의 결정적 재검 패스를 owe하는 경우 | `min(changed files, 4)`                                            |
...
바닥이 왜 거기 있는가: 9라인의 오타 수정에서, 6개의 인라인 순회는 아무것도 없는 5개의 순회이며, 스윕 — 첫 번째 패스가 도달하지 못한 것을 찾는 새로운 리더 — 는 첫 번째 패스가 모두 도달했을 때 찾을 것이 없습니다. low 노력 패스가 `candidateFloor` 아래로 유지되면, 가장 큰 변경된 소스 파일의 모든 커버 가능한 hunk(커버 가능한 모든 종류의 가장 큰 파일로 폴백)와 모든 커버 가능한 제거된 블록에 대해 한 번의 결정적 두 번째 검토를 수행합니다. 커버 불가능한 청크에 닿는 파일은 대상 선택에서 제외되며, 패스는 항상 대상, 새 후보 수, 커버 불가능한 청크를 이름으로 명시하는 영수증을 내보냅니다; 커버 가능한 파일이 없을 때도 커버 가능한 제거된 블록을 확인하고 누락된 대상을 공개합니다. 바닥은 할당량이 아닌 중지 신호입니다: 깔끔한 diff는 재패스 후에도 발견을 보고하지 않을 수 있습니다. 에이전트 8의 바닥은 실질적인 것입니다: "하나의 도메인이 diff를 지배한다"는 것은 판단이며, 40라인에 대해 만들어진 판단은 우세한 도메인을 매번 찾습니다. 40라인은 보통 한 가지 것이기 때문입니다.
...
| 단계                             | LLM 호출                       | 참고                                                                                                            |
| -------------------------------- | ------------------------------ | --------------------------------------------------------------------------------------------------------------- |
| 리뷰 에이전트 (단계 3)           | 17 (+0-2)                      | 병렬 실행; 에이전트 1e는 diff가 래핑 타입을 신호할 때만 (없으면 16); 교차 저장소는 에이전트 1c와 7 건너뛰기 (PR 정체성 포함 15, 없이 13 — 에이전트 0과 6d가 함께 떨어짐); 로컬/파일은 에이전트 0과 counter-frame 감사 6d 건너뛰기 (15); 지시 파일을 건드리는 diff에서 하나 더(`prose-exec`), 리뷰에 트리가 있을 때 |
...
| **합계**                         | **~20-31 (~16-29)**            | 3A 동일 저장소: ~20-31 (일반적 ~20-22); 교차 저장소 또는 로컬/파일: ~16-29 (하한은 정체성 없는 교차 저장소 명단 13); 에이전트 1e가 로스터에 없으면 하나 감소, `prose-exec`가 필요하면 하나 증가; 3B는 청크에 따라 확장 (DESIGN.md 참조) |

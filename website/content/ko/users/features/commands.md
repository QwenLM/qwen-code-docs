...
> [!note]
>
> HTML 내보내기를 열면 `unpkg.com`에서 해당 Qwen Code 버전의 렌더러와 스타일시트를 로드합니다. 해당 버전이 아직 배포되지 않았거나 두 파일 중 어느 하나에 도달할 수 없는 경우, 파일에 로드 오류가 표시됩니다. Markdown, JSON, JSONL 내보내기는 자체 완결적입니다.
...
| `/model --vision`     | 텍스트 전용 메인 모델용으로 이미지를 텍스트로 변환하는 vision-bridge 모델을 설정              | `/model --vision <model-id>`                                                                              |
...
- fire-and-forget 방식이라 세션을 사용할 수 있게 두는 `/btw`와 달리, `/advisor`는 리뷰가 반환될 때까지 입력을 차단합니다; 전체 컨텍스트 창에 걸쳐 강력한 리뷰어를 사용하면 수십 초가 걸릴 수 있습니다
...
리뷰는 헤더에 해결된 리뷰어 모델 이름을 표시하는 테두리 박스로 렌더링됩니다. 알 수 없는 `advisorModel`은 미리 검증되지 않습니다 — 제공자가 거부하면 `/advisor`가 실패를 보고하므로 모델 이름을 확인하세요; 해결할 수 없는 별칭 선택자(예: 빠른 모델이 구성되지 않은 `fast`)만 기본 모델로 폴백합니다. Advisor 요청은 구성된 모델 폴백을 사용하지 않습니다.
...
현재 이 기계에 등록된 Qwen Code 세션을 나열합니다. `sessions list`는 저장된 트랜스크립트를 탐색하는 반면("무엇을 작업했는가"), 이것은 라이브 프로세스 레지스트리를 탐색합니다("지금 무엇이 실행 중인가"). 종료된 세션이 남긴 기록은 발견될 때 정리됩니다. 일회성 `qwen -p` 실행은 등록되지 않으므로 표시되지 않습니다.
...
KIND는 세션을 등록한 주체를 나타냅니다 — 터미널 사용자는 `tui`, Qwen Code 세션이 아닌 프로그램(음성 프론트엔드, 릴레이)은 `external`, 다른 프로그램이 구동하는 세션은 `headless` 또는 `serve`입니다. 여러 `serve` 또는 `headless` 행이 하나의 PID를 공유할 수 있습니다: `qwen --acp` 자식은 하나의 프로세스에서 모든 세션을 호스팅하며 — 데몬이 생성한 경우 `serve`, 클라이언트가 직접 구동하는 경우 `headless` — 각각 별도로 등록됩니다. 이는 NAME, DIRECTORY와 마찬가지로 자기 보고입니다: 여기 있는 모든 필드는 해당 세션 프로세스가 직접 작성한 것이며, 세션이 수행할 수 있는 작업은 이에 의존하지 않습니다. 레코드 형식과 자체 프로그램 등록 방법은 [Cross-Session Protocol](./cross-session-protocol.md)을 참조하세요.
...
### 프로그램이 ACP를 통해 구동하는 세션

`qwen --acp` 자식은 호스팅하는 각 세션을 등록합니다 — 데몬이 프로세스를 생성한 경우 `serve`로, 편집기나 다른 클라이언트가 `qwen --acp`를 직접 구동하는 경우 `headless`로 — 세션은 `qwen sessions ps`와 다른 세션의 `list_agents`에 다른 세션과 마찬가지로 나타납니다. 메시지를 보낼 수 있습니다: 모델이 `send_message`를 호출하여 열어둔 터미널에 도달할 수 있습니다. 여러 세션이 하나의 프로세스와 하나의 inbox를 공유하므로, 발신자는 의도하는 세션의 이름을 지정해야 합니다 — 모든 Qwen Code 세션이 자동으로 수행하는 방식입니다.

하나로 _보내는_ 메시지는 보류 대신 거절됩니다. 보류는 사람에게 묻는 질문이며, 구동되는 세션을 대신해 보류된 메시지 목록을 감시하는 사람은 없습니다. 따라서 발신자는 만료를 기다리지 않고 즉시 통보를 받습니다. 보류된 메시지가 해당 세션에서 어떻게 표면으로 올라와야 하는지는 아직 정해지지 않았습니다.

세션은 자체 설정에 `agents.crossSessionMessaging`이 켜져 있는 동안에만 등록됩니다. 이 설정이 꺼져 있으면 보이지 않습니다. 아무도 메시지를 보낼 수 없는 세션을 나열하는 유일한 이유는 응답하지 않는 주소를 광고하는 것뿐이기 때문입니다.
...
Node 프로그램은 이 중 어느 것도 수작업으로 작성할 필요가 없습니다: ` @qwen-code/sdk/peer`가 계약을 구현합니다. `PeerEndpoint.start({ name })`은 레코드를 게시하고 inbox를 바인딩하며, `list()`와 `send()`는 이름으로 세션을 주소 지정하고, `onMessage`는 그들이 보내는 것을 수신합니다.

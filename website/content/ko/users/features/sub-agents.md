...
인터랙티브 TUI와 ACP 세션에서 백그라운드 에이전트, 셸, 모니터, 워크플로우의 완료 알림은 세션이 유휴 상태가 되면 모델 턴으로 전달되는 공유 큐를 사용합니다. 이 큐는 최대 20개의 알림만 보관하므로 노이즈가 많은 프로듀서가 무한정 백로그를 누적할 수 없습니다. 헤드리스 CLI의 로컬 큐는 이 규칙의 제한을 받지 않습니다.

21번째 알림이 도착하면 Qwen Code는 중간 모니터 펄스를 먼저 제거합니다 — 모니터의 다음 폴이 이를 대체합니다 — 그리고 그 외에는 가장 오래된 큐잉된 알림을 제거합니다. 에이전트 결과, 워크플로우 결과, 예약된 프롬프트는 인터랙티브 TUI에서 절대 제거되지 않습니다. 이를 대체하려는 알림은 대신 삭제되며, 터미널 결과만 큐잉되어 있을 때 도착하는 펄스도 마찬가지로 삭제됩니다.

삭제된 알림은 조용히 버려지지 않고 보고됩니다. 요약은 라이브 트랜스크립트에서 다음 알림 이전에 나타납니다. ACP는 해당 턴의 모델 입력에도 이를 접두사로 추가합니다. TUI는 다음 알림 배치까지 이를 보관하므로 cron 프롬프트는 슬래시, 셸, `@` 전처리를 통해 변경 없이 통과합니다. 데몬 알림은 확인 전에 기록되므로 리로드 후 내구성 기록이 이후 오버플로 요약보다 앞에 올 수 있습니다. ACP는 세션이 지워지거나 전환되거나 클라이언트가 알림 턴을 취소 또는 선점하면 보류 중인 요약을 삭제할 수 있습니다. 알림을 삭제해도 해당 작업이 중지되거나 삭제되지 않으며, 완료된 작업은 결과를 유지합니다. 요약은 확인할 작업이 있을 때 `/tasks`와 작업 출력 파일을 가리킵니다. 삭제된 예약된 프롬프트는 전달된 적이 없으며 재시도되지 않습니다. 기록되었지만 실시간으로 전달되지 못한 데몬 알림은 세션 트랜스크립트에서 계속 사용 가능하고 손실된 알림과 별도로 보고됩니다.

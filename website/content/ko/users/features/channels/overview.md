...
하나의 작업이 다음 일반 메시지를 받기 위해 선택된 상태로 유지되지만, 다른 명명된 작업은 공유 작업 디렉토리에서 계속 동시에 실행될 수 있습니다. 다른 작업을 생성하거나 선택해도 이전 작업이 취소되거나 대상이 변경되지 않으며, 늦은 결과는 원래 작업 레이블을 유지합니다. 바쁜 작업은 닫을 수 없지만 활성 프롬프트는 기존 Channel 취소 동작을 통해 `/session cancel [<name>]`으로 취소할 수 있습니다. 독립적으로 큐에 넣어진 턴은 취소되지 않지만, `collect` 디스패치 모드에서 취소된 프롬프트 뒤에 버퍼된 팔로우업은 해당 기존 동작에 의해 삭제됩니다. 미디어 준비는 대상이 지정되지 않습니다. 기본 권한 명령어는 선택된 작업에만 적용되지만, 명시적 요청 ID는 소유한 비활성 작업에 응답할 수 있습니다. 작업별 worktree는 Part 4에서 계획됩니다. 채널 메모리는 명명된 작업이 아닌 채팅에 범위가 지정됩니다.

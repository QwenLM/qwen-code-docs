...
의도적인 두 가지 동작이 있으며 접두사를 켜기 전에 알아두어야 합니다. DingTalk이나 WeCom이 트랜스크립트를 채워 넣은 음성 메시지는 사용자가 말한 텍스트로 간주되므로 다른 메시지와 마찬가지로 접두사를 포함해야 하며, 그렇지 않으면 삭제됩니다. 트랜스크립트 없는 음성 메모만 캡션 없는 미디어로 실행됩니다. 그리고 접두사는 페어링 전에 확인되므로 알 수 없는 발신자나 승인되지 않은 그룹의 첫 연락도 접두사를 포함해야 합니다. 이 순서가 없으면 바쁜 그룹의 모든 접두사 없는 메시지가 페어링 응답을 유발하며, 이는 접두사가 억제하려는 정확히 그 노이즈입니다. 새 사용자에게 접두사를 별도 경로로 알리거나 페어링 채널에 접두사를 설정하지 마세요.

...
명명된 결과는 원래 작업을 식별합니다. 직접 채팅은 `[task]`를 사용하고 그룹 채팅은 `[sender · task]`를 사용합니다. 명명된 텍스트 권한 프롬프트는 정확한 요청 ID와 해당하는 `/approve <id>`, `/approve-always <id>`, `/deny <id>` 명령어도 표시합니다. 레이블은 프레젠테이션 전용이며 모델 트랜스크립트에 저장되지 않습니다.

하나의 작업이 다음 일반 메시지를 받기 위해 선택된 상태로 유지되지만 다른 명명된 작업도 계속 동시에 실행될 수 있습니다. `/session new <name>`은 설정된 워크스페이스를 공유하는 반면 `/session new <name> --worktree`는 데몬 워크스페이스의 `.qwen/worktrees/` 디렉토리 아래에 해당 작업의 격리된 체크아웃을 생성합니다. 데몬은 재시작 후 작업을 다시 열기 전에 저장된 worktree 소유자를 확인합니다. 누락, 변경 또는 외부 소유권 기록은 작업을 공유 워크스페이스로 조용히 이동하는 대신 실패로 처리합니다. 다른 작업을 생성하거나 선택해도 이전 작업이 취소되거나 대상이 변경되지 않으며 늦은 결과는 원래 작업 레이블을 유지합니다. 바쁜 작업은 닫을 수 없지만 활성 프롬프트는 기존 Channel 취소 동작을 통해 `/session cancel [<name>]`으로 취소할 수 있습니다. 독립적으로 큐에 넣어진 턴은 취소되지 않지만 `collect` 디스패치 모드에서 취소된 프롬프트 뒤에 버퍼된 팔로우업은 해당 기존 동작에 의해 삭제됩니다. 미디어 준비는 대상이 지정되지 않습니다. 기본 권한 명령어는 선택된 작업에만 적용되지만 명시적 요청 ID는 소유한 비활성 작업에 응답할 수 있습니다. `/clear`, `/new`, `/reset`도 선택된 worktree 작업에서 작동합니다. 작업은 새 대화를 얻지만 worktree와 파일은 유지됩니다. 바쁜 worktree 작업은 프롬프트가 완료될 때까지 재설정을 거부하며 worktree 기록이 손상된 작업은 파일을 건드리지 않고 실패를 보고합니다. 채널 메모리는 명명된 작업이 아닌 채팅에 범위가 지정됩니다.
...
- `/btw <question>` — 현재 작업을 중단하지 않고 측면 질문을 합니다. 최대 4096자의 텍스트 전용 질문이며 측면 질문 지원을 가진 에이전트 연결이 필요합니다
...
- `/session new <name> --worktree` — 자체 Git worktree에서 작업을 생성하고 선택합니다. 데몬 관리 명명된 작업 모드 전용
- `/session use <name>` — 열려 있는 작업을 선택하거나 닫힌 작업을 다시 열기
- `/session cancel [<name>]` — 선택된 작업의 활성 프롬프트를 취소하거나 다른 소유한 작업의 이름을 지정합니다. 독립적으로 큐에 넣어진 턴은 취소되지 않지만 `collect` 모드에서 취소된 프롬프트 뒤에 버퍼된 팔로우업은 기존 취소 동작에 의해 삭제됩니다. 미디어 준비는 대상이 지정되지 않습니다
...
이 모드는 `qwen serve`가 소유하는 워크스페이스 그룹 채널 워커 프로세스를 시작합니다. 워커는 SDK를 통해 데몬에 다시 연결하고 동일한 채널 어댑터를 사용합니다. 워커는 데몬 프로세스와 분리되어 있으므로 채널 어댑터 충돌이 데몬을 충돌시키지 않습니다. 명시적 `--channel` 선택이 우선하며 준비될 수 없으면 데몬 시작이 실패합니다. 플래그 없이 시작하면 신뢰된 기본 워크스페이스의 `serve.channels` 설정이 복원됩니다. 보조 워크스페이스는 자체 `serve.channels`를 독립적으로 복원하지 않습니다. 어느 쪽 소스도 없으면 데몬은 첫 `qwen channel set`까지 채널 어댑터를 로드하거나 리스를 예약하지 않습니다.

자동 복원은 유효하지 않은 시작 설정과 워커 시작 전에 발생하는 검증 또는 리스 실패를 건너뛰면서 관련 없는 설정은 보존합니다. 워커 시작 실패 후 데몬은 정리가 성공한 경우에만 계속 실행됩니다. 글로벌 런타임 시작 타임아웃 또는 확인되지 않은 워커 중지도 일반 시작 실패 경로를 따르며, 워커 종료가 확인되지 않는 동안에는 서비스 리스가 유지됩니다. 채널이 복원되지 않을 때 `serve.channels`를 식별하는 메시지에 대해서는 데몬 로그를 확인하세요.

저장된 시작 이름은 비어 있으면 안 되며, 선행 또는 후행 공백이 없어야 하고, 안전하지 않은 제어 문자나 보이지 않는 문자를 포함하면 안 됩니다. 유효하지 않은 항목은 개별적으로 건너뛰어지며 배열 인덱스로 기록됩니다. 시작은 인스턴스 이름을 변경하거나 구성을 다시 작성하지 않습니다. 채널 시작 토글은 저장된 설정을 반영하며, 런타임 상태는 채널이 실행 중인지 여부를 보여줍니다.

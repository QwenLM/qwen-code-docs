...
`privatePolicy`는 다른 IM 채널과 동일한 방식으로 작동합니다:

- `disabled`: 페어링 요청을 생성하지 않고 모든 개인 메시지를 무시합니다.
- `allowlist`: 최상위 `allowedUsers`에 있는 사용자만 개인 메시지를 보낼 수 있습니다. 기업 환경에서 권장되는 기본값입니다.
- `pairing`: 봇을 사용하기 전에 사용자가 페어링해야 합니다.
- `open`: 로봇에게 메시지를 보낼 수 있는 모든 사람이 사용할 수 있습니다.

그룹의 경우 `groupPolicy`를 `"allowlist"`, `"pairing"` 또는 `"open"`으로 설정하세요. `"pairing"`에서는 그룹의 첫 번째 멘션이 페어링 요청을 생성하며, 응답이 시작되기 전에 한 번 승인해야 합니다. `groupPolicy: "pairing"`에서 액세스는 그룹 단위로 부여됩니다: 그룹이 승인되면 **해당 그룹의 모든 멤버**가 봇을 사용할 수 있으며, 그룹의 `senders: "allowlist"`와 `allowedUsers`로 제한할 수 있습니다. `privatePolicy`와 최상위 `allowedUsers`는 승인된 그룹의 멤버를 제어하지 않습니다. WeCom은 지능형 로봇을 멘션한 그룹 메시지만 전달하므로, 전달된 모든 그룹 콜백은 멘션된 것으로 처리됩니다. `requireMention` 설정은 멘션되지 않은 그룹 메시지에 대한 응답을 활성화할 수 없습니다. 해당 메시지가 봇에게 전달되지 않기 때문입니다.
...
사용자는 텍스트, 음성 메시지(음성 인식 텍스트 포함), 이미지, 텍스트와 이미지 혼합, 파일 및 비디오를 보낼 수 있습니다. 이미지는 이미지 첨부파일로 에이전트에게 전달됩니다. 파일과 비디오는 임시 로컬 경로에 다운로드되어 에이전트가 파일 도구로 읽을 수 있습니다.

...
그룹의 경우 `groupPolicy`를 `"allowlist"`, `"pairing"` 또는 `"open"`으로 설정하세요. `"pairing"`에서는 그룹의 첫 번째 멘션이 페어링 요청을 생성하며, 응답이 시작되기 전에 한 번 승인해야 합니다. `groupPolicy: "pairing"`에서 액세스는 그룹 단위로 부여됩니다: 그룹이 승인되면 **해당 그룹의 모든 멤버**가 봇을 사용할 수 있으며, `senderPolicy`와 `allowedUsers`는 승인된 그룹의 멤버를 제한하지 않습니다. WeCom은 지능형 로봇을 멘션한 그룹 메시지만 전달하므로, 전달된 모든 그룹 콜백은 멘션된 것으로 처리됩니다. `requireMention` 설정은 멘션되지 않은 그룹 메시지에 대한 응답을 활성화할 수 없습니다. 해당 메시지가 봇에게 전달되지 않기 때문입니다.

...
완료된 그룹 @ 또는 직접 메시지 답장은 첫 번째 전송 전에 체크포인트에 저장되며, 작업을 다시 실행하지 않고 백그라운드에서 재시도됩니다. 백오프는 5초로 시작하여 두 배로 증가하며 최대 5분까지 캡됩니다. 답장은 16번의 전송 실패 후 포기됩니다. 연결이 끊어진 채널이나 읽을 수 없는 pairing 승인으로 인한 로컬 지연은 32회의 연속 확인이라는 별도 제한을 사용하므로 전송 시도 예산을 소모하지 않습니다. 답장은 발신자, 그룹 또는 직접 메시지 인증이 명확히 취소된 경우 즉시 삭제됩니다. 큐는 최대 100개의 답장을 보관하며, 가득 차면 시도 횟수와 관계없이 가장 오래된 항목을 제거합니다. 구성된 DWS 프로필을 전환하면 큐가 비워집니다. 이러한 삭제는 채널 stderr에 보고되며 채팅 알림으로 전송되지 않습니다.

12,000코드 포인트를 초과하는 큐에 담긴 IM 답장은 잘리며, 수신자가 보는 메시지에 `[Response truncated for DWS delivery.]`가 추가됩니다. 문서 댓글 및 네이티브 todo 댓글 답장은 이 큐 재시도 또는 잘림 경로를 사용하지 않습니다. 각 턴은 하나의 댓글 전송 호출을 수행하고, 알 수 없는 결과는 무시되며, 명확한 실패는 기존 인바운드 턴 재시도 정책으로 전파됩니다. 종료 리액션은 작업 완료를 표시하며 답장 전달을 표시하지 않으므로, 재시도된 IM 답장은 해당 리액션이 나타난 후에 도착할 수 있습니다.
...
로컬 확인을 위해 다른 계정에서 직접 메시지를 보내고, 필요하면 pairing을 승인한 후, 작업이 실행되는 동안 구성된 시작 리액션이 나타나는지 확인하세요. 종료 리액션이 구성된 경우, 작업 완료 후 시작 리액션을 대체하는지 확인하세요. 답장 전달은 이후에 완료될 수 있습니다. 그런 다음 @언급 알림이 활성화된 문서 댓글을 추가하세요. 채널은 알림 메시지에 리액션하고, 문서를 읽은 후 원본 문서 댓글에 최종 응답을 게시해야 합니다. 알림이 비활성화된 댓글은 작업을 생성하지 않아야 합니다.

...
`startReaction`은 수락된 작업이 실행되는 동안 추가되는 이모지 문자 또는 DingTalk 리액션 이름입니다. 생략되거나 빈 값이면 기본값 `🤔`이 사용됩니다. `endReaction`은 작업이 완료, 실패 또는 취소된 후 이를 대체합니다. 생략되거나 빈 값이면 종료 리액션이 비활성화됩니다.
...
일반 직접 메시지도 같은 방식으로 복구됩니다. 5초 기록 확인이 실시간 스트림에서 누락된 직접 메시지를 다시 구동하며, 두 경로 모두에서 대화 및 메시지 ID로 중복 제거됩니다.
...
로컬 확인을 위해 다른 계정에서 직접 메시지를 보내고, 필요하면 pairing을 승인한 후, 작업이 실행되는 동안 구성된 시작 리액션이 나타나는지 확인하세요. 종료 리액션이 구성된 경우, 이후 시작 리액션을 대체하는지 확인하세요. 그런 다음 @언급 알림이 활성화된 문서 댓글을 추가하세요. 채널은 알림 메시지에 리액션하고, 문서를 읽은 후 원본 문서 댓글에 최종 응답을 게시해야 합니다. 알림이 비활성화된 댓글은 작업을 생성하지 않아야 합니다.

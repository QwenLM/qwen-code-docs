...
- [Agent Arena 사용 시기](#agent-arena-사용-시기)
- [Arena 세션 시작](#arena-세션-시작)
- [에이전트와 상호작용](#에이전트와-상호작용): 디스플레이 모드 및 탐색 포함
- [결과 비교 및 승자 선택](#결과-비교-및-승자-선택)
- [모범 사례](#모범-사례)
...
> [!note]
> **분할 창 디스플레이 모드는 향후 지원될 예정입니다.** tmux 기반 및 iTerm2 기반 분할 창 레이아웃을 지원할 계획입니다. 각 에이전트가 자체 터미널 팬을 가져와 진정한 나란히 보기를 제공합니다. 현재는 in-process 탭 전환만 사용할 수 있습니다.
...
Arena 설정은 [settings.json](../configuration/settings.md)의 `agents.arena` 아래에 중첩됩니다. 스키마는 `maxRoundsPerAgent`와 `timeoutSeconds`를 허용하지만, CLI는 현재 `/arena`에 사용되는 구성을 빌드할 때 두 값 모두를 삭제합니다. 따라서 두 필드 중 어느 것을 설정해도 `/arena` 실행을 제한하지 않습니다.

| 설정                             | 설명                                                                                                 | 기본값                     |
| :------------------------------- | :--------------------------------------------------------------------------------------------------- | :------------------------- |
| `agents.arena.worktreeBaseDir`   | Arena worktree의 사용자 지정 기본 디렉토리. 절대 경로를 사용하세요. `~`는 전개되지 않습니다.        | Qwen 홈의 `arena` 디렉토리 |
| `agents.arena.maxRoundsPerAgent` | 스키마에서 허용되지만 현재 `/arena`로 전달되지 않으므로 설정해도 실행에 영향을 주지 않습니다.        | 설정되지 않음              |
| `agents.arena.timeoutSeconds`    | 스키마에서 허용되지만 현재 `/arena`로 전달되지 않으므로 설정해도 실행에 영향을 주지 않습니다.        | 설정되지 않음              |
...
Arena는 여러 모델을 실행할 만큼 중요한 결정일 때 빛을 발합니다:
...
- Qwen 홈 디렉토리 아래 `arena` 디렉토리에 대한 쓰기 권한이 있는지 확인합니다 (기본 worktree 베이스 디렉토리)
...
- 더 적은 수의 에이전트를 사용하거나 지연 시간이 낮은 모델을 선택합니다
- 에이전트가 너무 오래 걸리면 Arena 실행을 수동으로 중지합니다
...
| **격리**          | 완전: 분리된 Git worktree                              | 공유 작업 목록을 가진 인프로세스 팀원                | 워커당 경량의 일시적 컨텍스트                            |

...
- `isolation: "worktree"`를 사용하는 에이전트는 기본 백그라운드 동작을 따르며, 에이전트가 완료를 보고하면 정리가 실행됩니다. 인라인 결과를 원하면 `run_in_background: false`를 설정하세요. 호출자 소유의 `working_dir` 실행은 수명이 외부에서 관리되므로 기본적으로 포그라운드로 유지됩니다.

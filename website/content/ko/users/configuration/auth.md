...
- **Alibaba ModelStudio**: 공식 권장 설정. **Coding Plan**(개인 개발자용 · 주간 할당량 포함), **Token Plan**(팀 및 회사용 · 전용 엔드포인트의 사용량 기반 과금) 또는 **Standard API Key**(기존 ModelStudio API 키로 연결) 중 하나를 선택하는 하위 메뉴를 엽니다. Token Plan과 Standard API Key는 추가 설정 없이 내장 [`web_search` 도구](../../developers/tools/web-search.md)를 활성화합니다.
...
| `wireApi`          | 아니오 | OpenAI 호환 요청 형식: `chat-completions` 또는 `responses`. 생략 시 제공자 프로토콜을 상속합니다.                                        |

...
├── types.ts            ← WorkspaceRequestContext + 子服务接口
├── index.ts            ← facade 工厂 (createDaemonWorkspaceService)
├── fileService.ts      ← 封装 fsFactory
├── authService.ts      ← 封装 DeviceFlowRegistry
├── agentsService.ts    ← 封装 SubagentManager
├── memoryService.ts    ← 封装 memory 文件操作
└── __tests__/
    ├── fileService.test.ts
    ├── authService.test.ts
...
> `listWorkspaceSessions` / `recordHeartbeat` / `getHeartbeatState` / `updateSessionMetadata` 保留在 bridge 中——它们访问 bridge 内部的 `byId` session map，属于 session 作用域的操作。
...
- `queryWorkspaceStatus` / `invokeWorkspaceCommand`（新暴露，封装 channel + timeout + error，供 service callback 委托调用）
- 权限仲裁 mediator
...
无外部包消费者（已验证在 `packages/cli/src/serve/` 和 `packages/acp-bridge/src/` 之外无引用），内部重构安全。
...
`qwen/workspace/...`（与现有的 `qwen/control/...` 区分）：

- `qwen/control/...` = daemon 向 child 转发命令（南向，经 AcpSessionBridge）
- `qwen/workspace/...` = daemon 本地工作区操作（北向，终止于 DaemonWorkspaceService）

> 待 chiga0 确认。如果更改命名空间，只需替换方法名前缀，不影响整体架构。
...
在能力声明（Capabilities advertise）时，需在 `_meta.qwen.methods` 中声明这些方法。
...
| 文件                                                      | 用途                                               |
| --------------------------------------------------------- | -------------------------------------------------- |
| `serve/workspace-service/types.ts`                        | `WorkspaceRequestContext` + 子 service 接口        |
| `serve/workspace-service/index.ts`                        | Facade 工厂                                        |
...
| `serve/workspace-service/__tests__/fileService.test.ts`   | 单元测试                                           |
| `serve/workspace-service/__tests__/authService.test.ts`   | 单元测试                                           |
| `serve/workspace-service/__tests__/agentsService.test.ts` | 单元测试                                           |
| `serve/workspace-service/__tests__/memoryService.test.ts` | 单元测试                                           |
...
| 文件                                                          | 变更                                                                                                                                                                                |
| ------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `acp-bridge/src/bridge.ts`                                    | 移除 8 个 workspace 方法（initWorkspace / setWorkspaceToolEnabled / 5 个 status getters / restartMcpServer）；新暴露 `queryWorkspaceStatus` + `invokeWorkspaceCommand`；重命名工厂函数 |
| `acp-bridge/src/bridgeTypes.ts`                               | 接口重命名 `HttpAcpBridge` → `AcpSessionBridge`；移除 8 个 workspace 方法签名；新增 `queryWorkspaceStatus` + `invokeWorkspaceCommand` 签名                                            |
| `acp-bridge/src/bridgeOptions.ts`                             | 更新 JSDoc 引用                                                                                                                                                                     |
| `acp-bridge/src/status.ts`                                    | 更新错误消息中的类名                                                                                                                                                                |
| `cli/src/serve/httpAcpBridge.ts` → 重命名为 `acpSessionBridge.ts` | re-export 更新                                                                                                                                                                      |
| `cli/src/serve/runQwenServe.ts`                               | 构造 `DaemonWorkspaceService`，注入 callback，传给 routes 和 /acp handler                                                                                                           |
| `cli/src/serve/server.ts`                                     | routes 从直连 `fsFactory`/`DeviceFlowRegistry` 改为调用 `workspace.file.*` / `workspace.auth.*`                                                                                       |
| `cli/src/serve/workspaceAgents.ts`                            | 业务逻辑迁移至 `agentsService.ts`；原文件变为 route handler 薄壳（构建 ctx → 调用 service）                                                                                             |
...
| `/acp` handler（`acp-integration/` 或 `serve/` 内）           | 新增北向方法分发                                                                                                                                                     |
...
REST API 接口（路径、HTTP 方法、请求/响应 JSON schema）保持不变。`sdk-typescript` 中的 `DaemonClient` / `DaemonSessionClient` 无需任何改动。

验证方式：现有的 `packages/sdk-typescript/test/unit/DaemonClient.test.ts` 和 `DaemonSessionClient.test.ts` 在本 PR 中必须零修改通过。
...
> JSON-RPC error codes 遵循 [ACP error code registry](https://spec.acpprotocol.org)（标准范围 -32000 ~ -32099 为 server-defined application errors）。具体的 code 值在实现时需与 `/acp` 现有的 error 映射逻辑（`acp-integration/errorCodes.ts`）对齐。
...
| 层                | 测试类型                                                                | 覆盖目标                                                       |
| ----------------- | ----------------------------------------------------------------------- | -------------------------------------------------------------- |
| 子 service 单元测试 | Jest，mock fsFactory / DeviceFlowRegistry / SubagentManager / callbacks | 业务逻辑正确性 + trust gate 拒绝非法 clientId                  |
| Route 集成测试    | 现有 route test 改为经 service 调用（验证 HTTP 接口不变）               | 回归保障，REST 路径不 break                                    |
| E2E 等价验证      | 启动真实 serve + HTTP 请求                                              | REST 和 `/acp` 对同一操作返回等价结果；trust gate 两端一致拒绝 |
### E2E 验证矩阵

- 文件读写：REST `GET /file` 与 `/acp` `qwen/workspace/fs/read` → 结果一致
- Agent CRUD：REST `POST /workspace/agents` 与 `/acp` `qwen/workspace/agents/create` → 行为一致
- 信任网关拦截：无效 clientId 在两条路径下均返回 403
- Workspace 初始化：验证 fsFactory 链路跑通 + 生成 audit trail
...
- REST route handler 改为调用 service
- bridge 瘦身（迁出 8 个 workspace 方法）+ 新增暴露 2 个 child 委托方法
- `HttpAcpBridge` 重命名为 `AcpSessionBridge`
- 新增 `/acp` northbound ext methods（27 个）
...
- workspace-scoped EventBus（属于 PR 24 范围）
- workspace-scoped ClientRegistry（属于 PR 24 范围）
- L2 ↔ L3 拆分（将 `ClientSideConnection` 从 bridge 中拆出）
- 将 REST 改造为 `/acp` compat shim（长期方向）
- channels standalone 模式统一（独立部署形态问题）
- `listWorkspaceSessions` / `recordHeartbeat` / `getHeartbeatState` / `updateSessionMetadata` 迁移（属于 session-scoped，保留原位）
- `publishWorkspaceEvent` / `knownClientIds` 的 ownership 转移（属于 session 衍生基础设施，保留由 bridge 持有，service 通过 callback 消费）

---

## 12. 待 chiga0 确认的决策点

1. `/acp` northbound 命名空间：`qwen/workspace/...` 或其他（如复用 `qwen/control/...`）
2. 重命名是否放在同一个 PR：倾向于放在同一个 PR，但可根据反馈拆分

> 以上两点如需调整，仅影响命名和 commit 边界，不影响架构。
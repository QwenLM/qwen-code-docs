...
- 发送方必须是已处于 ACP session 通道内且具备发送 `extNotification` 能力的运行时或 extension bridge。ACP `Session.ts` 可以发送该通知；`coreToolScheduler.ts` 本身不能直接向 daemon 主会话发送该通知。
- `BridgeClient` 在现有 `extNotification` 处理分支中按 notification name 进行 demux：命中 `qwen/notify/session/artifact-event` 后读取 payload，转换为 `SessionArtifactInput[]`，再进入统一 ingest pipeline。
- Bridge 必须从 notification transport context 派生 `source: 'hook'`，payload 中的 `source` 仅作为兼容性提示；如果 payload source 与 transport context 不一致，bridge 会将其覆盖为 `hook` 并记录 debug/warning。Notification payload 不能设置 `trustedPublisher`；如果携带 `storage: 'published'`，则按普通 untrusted input 校验失败处理。

注意：`qwen/notify/session/artifact-event` 仅作为 explicit artifacts 的传输 envelope，不应形成第二套 store/validation/dedupe 管道。`BridgeClient` 必须将 `_meta.artifacts`、hook artifacts 与 `artifact-event.artifacts` 统一转换为 `SessionArtifactInput[]`，调用同一个 `ingestArtifacts()` / `SessionArtifactStore.upsertMany()`，复用同一套 validation、normalization、enrichment、eviction 和 `artifact_changed` 发布逻辑。ACP 主会话当前没有 PostToolBatch callsite，不能把 `coreToolScheduler.ts` 的 batch hook 当作 daemon artifacts 面板的默认来源；若后续需支持 daemon 主会话 batch artifacts，必须先增加真实调用点和测试。非 ACP 运行时如果没有 artifact notification sink，则不能声明支持 daemon hook artifacts。
...
- IDE 插件将当前打开的预览 URL 加入产物区。
- WebUI 用户手动添加资源链接。
- Channel plugin 或外部集成在任务过程中登记平台资源。

与 hook 输出的区别：

- hook 输出适用于 agent 执行链路内部。
- POST route 适用于 daemon client / UI / 外部集成。
- POST body 必须经过统一的 artifact validation，不允许使用任意本机绝对路径或不支持的 URL scheme。
...
identity 仅描述资源位置，不包含 `source`。tool、hook、client 对同一 URL 或路径的登记将合并为一条 artifact，避免右侧面板重复展示同一资源。V1 不维护 `provenance[]`、信任级别或 retention class；首次成功登记者拥有该 artifact 的展示字段和来源审计字段，后续同 identity 登记仅表示“同一资源被再次观察到”。
...
如果输入同时携带多个 primary locator，V1 将直接拒绝，而不是尝试按优先级猜测 identity。这样可以避免一个 artifact 先按 `workspacePath` 去重、后续又按 `url` 去重而产生重复。

`storage: 'published'` 是唯一例外：它必须携带 `url` 作为 primary locator，可额外携带 `managedId` 作为 managed reference。published identity 仍按 `url` 计算；`managedId` 仅用于未来下载/预览，不参与去重。该例外仅接受带有内部 `trustedPublisher: true` 的输入；hook、client POST、`record_artifact` 或普通工具返回 `storage: 'published'` 时均按校验失败处理。

对外 id：

- 使用 identity 的 sha256 前 12 位。
...
- 使用 `path.resolve(workspaceCwd, input)` 获取绝对路径。
- 校验 resolved path 必须位于 workspace 内：`path.relative(workspaceCwd, resolved)` 不能以 `..` 开头，且不能是绝对路径。
- 如果目标已存在，使用 `fs.realpath` 检查 symlink 最终目标是否仍在 workspace 内；若 symlink 指向 workspace 外则拒绝。
- 如果目标不存在，registration 可以保留该 artifact，但初始 `status` 必须是 `missing`；不能因为 `realpath` 失败就跳过 symlink containment 检查。后续 GET TTL refresh 时必须重新执行相同的 containment + realpath 检查。
- 如果 refresh 时发现路径已变成指向 workspace 外的 symlink，artifact 将保留但 `status` 变为 `missing`，并清除 best-effort `sizeBytes`；V1 绝不会将该路径报告为 `available`。
- 输出统一使用 POSIX slash，去除开头的 `./`。
- 不做大小写折叠；即使 macOS 默认文件系统大小写不敏感，identity 仍按字符串区分，避免跨平台行为不一致。

`normalizedManagedId`：

- 输入首先 trim ASCII whitespace。
- trim 后不能为空，长度不超过 200 个字符。
- 拒绝 ASCII 控制字符。
...
- 使用 WHATWG `new URL(input)` 解析，禁止使用字符串 `startsWith('http')` 等宽松判断。
- 除 `ArtifactTool` trusted published URL 外，普通 link artifact 仅允许 `http:` / `https:`。
- `url` 字段保存清理后的可点击 URL，供 client 打开；不要将 identity 用 URL 反写为可点击 URL。
- identity 另用内部 `identityUrl` 计算，不作为 public 字段返回。
- scheme 和 host 转为小写。
- 默认端口归一化：不保留 `https:443` / `http:80`。
- 保留 fragment；在 hash-routed SPA 中，fragment 可能是资源 identity 的一部分。
- 保留 query 参数原始顺序；部分平台对 query 顺序敏感，V1 不做 query sort。
- 拒绝或清除 `username` / `password`，不将 URL userinfo 存入 artifact store。

去重行为：

- 首次登记：`created`
- 同 identity 再次登记：`updated`
- `createdAt` 保持不变。
- `updatedAt` 更新，但不参与 eviction 排序。
- 同一次 `upsertMany()` 内先按 identity 合并输入；同 identity 的 owner 由 `receivedSeq` 最小的输入决定，若无 `receivedSeq` 则使用输入数组顺序。`BridgeClient` 不应将不同 transport event 的 artifacts 无序合并；若必须合并，必须先分配 `receivedSeq` 再排序。每个最终 identity 仅在 `changes[]` 中产生一条 change。若该 identity 在本批之前不存在则为 `created`，否则为 `updated`。
- 展示字段 `title`、`description`、`source`、`toolCallId`、`toolName`、`hookName`、`extensionId`、`clientId` 采用 first-writer-wins 策略，不被后续同 identity 输入覆盖。
- 资源本体字段允许安全升级：同 URL identity 从 `external_url` 升级到 `published` 时，可更新 `storage`、补充 `managedId`、更新 `kind` / `mimeType` / `sizeBytes`，并允许 publisher 覆盖 `title` / `description`，避免占位 link 标题永久遮蔽真实发布物。该升级仅接受带有内部 `trustedPublisher: true` 的 `storage: 'published'` 输入。
- `managedId` 从空补齐为 published managed reference 是允许的；已有 `managedId` 不被后续普通输入覆盖。
- `status` 和 `sizeBytes` 是 daemon 的 best-effort 派生字段，可随 workspace stat 或 published artifact enrichment 刷新。
- `metadata` 保存首次登记时通过校验的小对象；后续同 identity 仅 `source: 'tool'` 或 `source: 'client'` 的输入可进行受控富化：只添加不存在的 key，不覆盖已有 key，合并后重新校验 primitive-only 与 4KB 总大小。hook 对已存在 artifact 的 metadata 富化默认忽略。若合并后超限，仅丢弃本次 metadata 富化并记录 warning，artifact 的其它安全升级仍可继续。
- client POST 同 identity 不覆盖展示字段，也不改变 `retentionSource`；它仅将内部 `clientRetained` 置为 `true`，用于表达用户手动保留意图。
...
- `receivedSeq`：输入接收顺序，仅用于同批 deterministic coalescing，不作为 public 字段返回。

配额与保留策略：

- 每个 session 最多 200 个 artifacts。
- V1 使用 soft source reservation，reservation 按内部 `retentionSource` 归属：
  - `tool`: 100
  - `client`: 50
  - `hook`: 50
- reservation 是最低保留额度，不是硬上限；未使用额度可被其它来源借用，直到达到全局 200 上限。
- 新建 artifact 导致总量超过 200 时，按以下顺序选择 eviction candidate。本批 `upsertMany()` 新创建的 artifact 默认不进入候选池；eviction 优先在本批开始前已存在的 artifacts 中选择候选。因此，本批新登记的 missing artifact 可能会在满 store 中挤掉仍然 live 的旧 artifact，这是 V1 为保证当前显式产物可见性作出的设计选择。
  ...
  4. 如果所有 artifact 均为 `clientRetained === true`，则裁剪最旧的 client-retained artifact。
- eviction 在使用 cached `missing` 优先级前，必须对即将作为候选的 workspace artifacts 进行 best-effort status refresh / containment check；如果刷新后为 `available`，则不能继续将其作为 missing 优先裁剪。刷新失败时保留原 cached 状态。
- `clientRetained` 是最后裁剪偏好，不是无限 pin，也不突破 200 全局上限或 soft reservation。当所有 artifact 均为 client-retained 时，仍按最旧的 client-retained artifact 进行裁剪。
- 如果裁减旧 artifact 后，本批新创建 artifact 自身仍超过剩余容量，store 必须在生成 `changes[]` 前按 `receivedSeq` / 输入顺序保留前 N 个新 identity，丢弃超出的本批输入并记录 warning/diagnostics。被丢弃的新输入不进入 store，不产生 `created` 或 `removed` change，因此同一次 mutation 内同一 identity 不会出现 `created` 后又 `removed` 的情况。
- “最旧”排序使用 `(createdAt, insertSeq)`，`insertSeq` 是 store 内部单调递增序号，用于稳定同毫秒或同批输入的 tiebreaker。
- 同 identity 重复登记会刷新 `updatedAt`，但 eviction 不参考 `updatedAt`；因此其它来源不能通过高频重复登记将旧 artifact 固定在保留集合中。
- 返回 `createdAt` 升序结果。
- 裁剪必须为每个被移除的 artifact 发送 `artifact_changed` / `removed`。V1 不提供其它裁剪事件。
- reservation 数值、`retentionSource`、`clientRetained` 与 `insertSeq` 是 V1 实现细节，不是 wire protocol 字段；后续可在不改变 API shape 的前提下调整默认值，或增加更细粒度的 per-producer quota。
...
- Client SSE 断线重连后应重新 `GET /session/:id/artifacts` 进行 snapshot sync。
- V1 不要求额外的 `artifacts_reset` event；如果后续支持 session 继续存在但 artifact store 被清空的运行模式，再增加 `artifacts_reset` 或等价的 snapshot-invalidated event。
- 历史恢复、跨进程持久化和 session load replay 属于后续阶段。

## 8. 内部实现链路

以下 Phase 是同一 V1 完整能力的工程实施顺序，不代表对外拆分为多个版本。实现 PR 可按 Phase 拆分，但合并后的设计基准是一项完整的 session artifacts 能力。
...
- `packages/core/src/tools/artifact/artifact-tool.ts`
  ...
  - 使用 `storage: 'published'`，不将 qwen home 本机路径作为 `workspacePath` 暴露。

Phase A 优先接入 `ToolResult.artifacts` 和 `ArtifactTool`；`record_artifact` 在 Phase D 接入，但仍属于同一个 V1 完整能力。
...
- `packages/cli/src/acp-integration/session/Session.ts`
  ...
  - PostToolUse hook artifacts 独立于工具成功/失败进行收集，用于 error trace / dashboard 等失败诊断产物。
  - 失败路径的 hook artifacts 不能依赖成功的 result metadata；必要时直接调用 bridge artifact ingest。
...
- 所有输入均转换为同一个 `SessionArtifactInput[]`。
- 基于 transport context 分配 `source`、`receivedSeq`。`trustedPublisher` 仅由 completed `ArtifactTool` session update 的 bridge-side ingest option 分配；`BridgeClient` 不得根据 artifact payload 字段或普通 `_meta.artifacts` 内容进行推断。
- 统一调用 `ingestArtifacts()` / `SessionArtifactStore.upsertMany()`，不为 notification artifacts 建立第二套 validation 或 dedupe。
...
- stat 成功且 containment / realpath check 仍通过：如果此前 cache 为 `missing`，GET 返回 `status: 'available'`。
- 如果 refresh 发现 symlink escape 或 workspace containment 失败，GET 返回 `status: 'missing'`，不返回新的 `sizeBytes`。
- GET 可静默刷新 status cache，但不得因读请求发布 `artifact_changed`；V1 status 对 SSE 客户端是最终一致的。
...
SDK 的单数 add 映射到 bridge 的复数 mutation：`addSessionArtifact(a)` 包装为 `addSessionArtifacts(sessionId, [a])`，返回完整的 `DaemonSessionArtifactMutationResult`，不丢弃 eviction 产生的 removed changes。

### 8.6 Phase D: record_artifact 显式注册
...
- 新增 `packages/core/src/tools/record-artifact.ts`
  ...
  - 不接受 `storage: 'published'` 或 `url + managedId` 的 published 例外。
  - 输出 `ToolResult.artifacts`，复用 V1 store/event/list 链路。
- `Config.createToolRegistry`
  - 通过 feature-gated 或 skill/extension opt-in 进行注册，避免给所有 session 增加模型可见的 tool。

### 8.7 Phase E: hook artifacts 显式注册
...
- `packages/core/src/hooks/hookAggregator.ts`
  - `mergeWithOrLogic()` 对 `artifacts` 进行多 hook concat，不采用 last-writer-wins 策略。
- `packages/core/src/core/toolHookTriggers.ts`
...
- ACP session V1 不声明 PostToolBatch artifacts 支持；如果产品要求 daemon 主会话支持 batch artifacts，必须在 ACP Session 中增加真实的 PostToolBatch callsite，而不是依赖 `coreToolScheduler.ts` 的非 daemon 主会话路径。
- 其他运行时如果已有 batch-level artifact notification，可通过 `qwen/notify/session/artifact-event` 发送给 bridge。
- BridgeClient 从 `qwen/notify/session/artifact-event` 提取 batch-level artifacts，走同一套 validation 和 upsert 流程。

### 8.8 Phase F: client POST / SDK add 显式注册
...
- `packages/cli/src/serve/server.ts`
  ...
  - DELETE 调用 bridge 的 `removeSessionArtifact()`；如果 artifact 已不存在，则返回空 `changes[]`，不发布 SSE。
  - 发布 `artifact_changed`，先发布 created/updated，再发布 removed。
- artifact add 不新增单数 bridge mutation；所有新增入口都走 `addSessionArtifacts()` / `upsertMany()`，避免 validation、coalescing、eviction 行为发生漂移。artifact remove 使用单独的 `removeSessionArtifact()`，因为它按 server-assigned artifact id 删除，不参与 input validation / identity coalescing。
...
- 必须使用 WHATWG `new URL(input)` 解析并检查 `parsed.protocol`，禁止基于字符串前缀进行判断。
- 存储前拒绝或清除 `parsed.username` / `parsed.password`，避免 URL credential 泄漏。
- `record_artifact` / hook / client POST 不允许 `file://`。
- `ArtifactTool` 返回的 `file://` published URL 保持例外，因为它来自已授权的 publish；remote daemon 场景应优先使用远端 publisher 的 `https:` URL。
- Daemon 不 fetch URL。
- Client 展示 host。
- URL 不自动打开。
- Client 不得因为 `kind: 'image' | 'video' | 'audio' | 'html'` 就自动把 external URL 填入 `<img>`、`<video>`、`<audio>`、`iframe` 或类似会发起网络请求的预览元素。V1 对 external URL 只展示图标、标题、host 和点击入口；远程预览必须等用户显式点击，或后续通过单独的 preview capability 与 sandbox 策略启用。
- Client 应对 loopback、RFC 1918、link-local、metadata service 等私网地址进行 warning 或 block；Daemon V1 不解析 DNS，不承担 SSRF 防护的最终判断。

### 9.2 Path

- 对外只返回 `workspacePath`，它必须是 workspace-relative path。
- workspace 外的 path 不作为 file artifact 暴露。
- `record_artifact` / hook / client POST 如果传入 `workspacePath`，必须在 workspace 内。
...
- Daemon validation 必须进行长度限制、trim、拒绝 ASCII 控制字符；不要把子串黑名单当作 XSS 安全边界。
- 所有可能进入 UI 的文本字段，包括 `title`、`description`、`metadata` string value、`toolName`、`hookName`、`extensionId`、`clientId`，client 都必须作为 untrusted text 渲染或进行 HTML escape，禁止通过 `innerHTML` 直接插入。

### 9.5 Anti-spam

- 每个 session 最多 200 个 artifacts。
- soft reservation 默认 `tool: 100`、`client: 50`、`hook: 50`，未使用额度可被其他来源借用。
...
V1 提供 hook artifact 后，extension 可在 `qwen-extension.json` 或 `hooks/hooks.json` 中提供 PostToolUse hook：
...
这适合企业插件：把“如何从工具结果拼接业务 URL”的逻辑固化在 extension 中，而不是写进每个 prompt。
...
- completed `ArtifactTool` session update 会通过 bridge-side ingest option 设置内部 `trustedPublisher: true`；`record_artifact`、其他 tool result、hook payload、client POST 不会设置，BridgeClient 也不能通过 artifact payload 字段推断。
...
- 同时携带多个 primary locator 的普通 artifact 被拒绝；只有 `trustedPublisher: true` 且 `storage: 'published'` 允许 `url + managedId`，identity 只按 `url` 计算。

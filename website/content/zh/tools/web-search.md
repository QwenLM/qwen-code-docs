...
使用 `web_search` 通过 Tavily API 执行网页搜索。该工具在可能的情况下会返回一个简洁的答案及来源。
...
2. **Environment variable**: 在环境变量或 `.env` 文件中设置 `TAVILY_API_KEY`
...
- **引用：** 来源链接以编号列表的形式附加在结果中。
- **API key：** 通过 settings.json、环境变量、.env 文件或命令行参数配置 `TAVILY_API_KEY`。如果未配置，则该工具不会被注册。
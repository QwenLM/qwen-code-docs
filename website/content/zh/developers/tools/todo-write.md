...
该工具默认禁用。在 `settings.json` 中启用它并重启 Qwen Code：
...
启用该工具后，AI 助手可能会在复杂的多步骤任务中使用它。如果你希望了解助手对你请求的规划方案，也可以要求它创建一个待办事项列表。
...
- **手动启用：** 将 `tools.todoWrite.enabled` 设置为 `true` 并重启 Qwen Code，然后才能使用该工具。
- **启用后自动使用：** AI 助手在复杂任务期间自动管理待办事项列表。

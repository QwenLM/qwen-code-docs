...
`exit_plan_mode` 接受以下参数：

- `plan`（字符串，必需）：你要呈现给用户审批的实施计划。应为简洁的、Markdown 格式的计划，描述实施步骤。
- `originalRequest`（字符串，可选）：原始请求，用于需要计划审批的 teammate 向其 leader 请求批准时使用。
- `researchSummary`（字符串，可选）：需要计划审批的 teammate 的调研上下文，供 leader 审查使用。
...
该工具仅在用户的显式决定被接受且执行开始后才会更改审批模式。权限规则、自动审批模式和权限允许 hook 不能代替用户批准主会话的计划退出。
...
- **Fail closed**：取消、过期的批准、宿主故障和转换失败均保持在计划模式

...
**注意：** 此工具是 opt-in 的，默认禁用，因为 `glob` 在大多数情况下已涵盖目录列表。要启用它，请在设置中将 `tools.listDirectory.enabled` 设置为 `true`，或在 `coreTools` 允许列表（`--core-tools` / `tools.core`）中显式列出 `list_directory`。
...
`read_file` 读取并返回指定文件的内容。此工具处理文本文件以及当前模型支持的模态的媒体文件（图像、PDF、音频、视频）。对于文本文件，它可以读取特定的行范围。不支持的 PDF 会尝试文本提取以及下文描述的有界 vision bridge 回退；其他不支持的媒体文件会返回一条有用的错误消息。其他二进制文件类型通常会被跳过。

- **工具名称：** `read_file`
- **显示名称：** ReadFile
- **文件：** `read-file.ts`
- **参数：**
  - `file_path`（字符串，必填）：要读取的文件的绝对路径。
  - `offset`（数字，可选）：对于文本文件，从 0 基数的行号开始读取。需要设置 `limit`。
  - `limit`（数字，可选）：对于文本文件，最大读取行数。如果省略，则读取默认最大值（例如 2000 行）或在可行时读取整个文件。
  - `pages`（字符串，可选）：对于 PDF，1 索引的页码或闭合页码范围，例如 `"3"` 或 `"20-25"`。一次请求最多包含 20 页。
- **行为：**
  ...
  - 对于使用纯文本主模型的 PDF：首先尝试文本提取。如果提取失败，或者显式请求的（或实际的）单页仍超过 12K token 的文本预算，则配置的 vision bridge 会自动渲染并转录从请求的起始页开始的最多四页。当已知文档实际页数时，请求的范围会被裁剪到文档末尾。结果会标识已转录的范围以及已知剩余的页数，或者当页数不可用时，说明可能还存在其他页。普通的多页文本溢出仍会要求提供更窄的 `pages` 范围，而不会切换到 vision。
  - Vision bridge PDF 转录是有损的，并被标记为不受信任的机器生成内容。工具结果包含文本而非渲染图像，其面向用户的 TUI、ACP、非交互式结构化输出和导出显示会在已知时标识 vision 模型和端点。如果 bridge 失败，原始 PDF 提取错误会精确返回给模型，而用户显示仍会披露 bridge 尝试。

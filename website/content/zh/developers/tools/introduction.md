...
- **[Web 抓取工具](./web-fetch.md)（`web_fetch`）：** 用于从 URL 获取内容。
- **[Web 搜索工具](./web-search.md)（`web_search`）：** 用于搜索互联网。
- **[多文件读取工具](./multi-file.md)（`read_many_files`）：** 专用于从多个文件或目录读取内容的工具，通常由 `@` 命令调用。
- **[记忆工具](./memory.md)（`save_memory`）：** 用于跨会话保存和回忆信息。
- **[Todo 写入工具](./todo-write.md)（`todo_write`）：** 用于在编码会话中创建和管理结构化任务列表。
- **[任务工具](./task.md)（`task`）：** 用于将复杂任务委派给专门的子代理（subagent）。
...
- **[沙箱机制](../sandbox.md)**：沙箱将模型及其产生的更改与你的环境隔离，以降低潜在风险。
...
在浏览器中打开 `http://127.0.0.1:4170/` 即可获取 Web Shell UI：聊天、会话列表和工作区检查。此无 token 的 loopback 主监听器授予本地调用方完整的操作员 API 权限，同时保留工作区信任、会话所有权、client-id、权限、功能、验证和资源检查。`createServeApp()` 在 `bearerAuth` **之前**挂载打包的 Web Shell 资源（`packages/cli/src/serve/web-shell-static.ts`），因此 Shell 本身无需 token 即可加载；其自身的 API 调用在配置了 bearer 时会携带 bearer——启动 daemon 时使用 `--open`（将 token 放在 URL fragment 中，永远不会发送到服务器）或在启用认证时手动追加 `#token=…`。`--no-web` 退出 Web Shell，使 daemon 仅提供 API。
...
# 12. 在运行时工作 drain 后保持空闲 ACP 子进程至少 60 秒可复用
...
| `--hostname <host>`                     | string                         | `127.0.0.1`                                  | 非 loopback 需要 token              | 绑定地址。Loopback 值：任意 `127.0.0.0/8` 地址、`localhost`、`::1`、`[::1]`；`localhost` 会被解析一次并固定，信任模式会在启动完成前验证实际监听地址。`[::1]` 的方括号会自动去除；如果输入 `host:port` 格式会被拒绝，并提示使用 `--port`。                                    |
...
| `--enable-session-shell`                | boolean                        | `false`                                      | Bearer 或受信任的 loopback               | 启用直接的 `POST /session/:id/shell` 执行。调用方还必须发送与会话绑定的 `X-Qwen-Client-Id`。                                                                                                        |
...
| `--allow-origin <pattern>`              | repeatable string              | none                                         | -                                        | 替换默认 Origin 拒绝策略的 CORS 允许列表。通配符和非 loopback HTTP(S) 源需要 token。                                                                                                                                         |
...
| `--channel-idle-timeout-ms <n>`         | non-negative integer           | `0`                                          | -                                        | 运行时工作 drain 后 ACP 子进程的自动回收延迟。首次使用前会保留普通预热。活跃的 keepalive 窗口可能会延长配置的延迟；以较长的剩余延迟为准。                                                                                                                               |
| `--initialize-timeout-ms <n>`           | number                         | `10000`                                      | -                                        | ACP 子进程启动截止时间（channel factory + initialize 握手）和默认请求超时（毫秒）。                                                                                                                                                               |
...
| 非 loopback HTTP(S) `--allow-origin` 但未配置 token                           | `Refusing to start with --allow-origin ... but no bearer token configured`                          |
...
静态 Shell 在所有启动模式下都挂载在 `bearerAuth` **之前**——浏览器无法为地址栏导航或 `<script src>` 子资源附加 `Authorization` 头，因此对其进行门控只会破坏 UI。API 调用在无 token 的主监听器上使用受信任的 loopback 权限，或在认证模式下附加配置的 bearer。在非 loopback 绑定下，Shell 是只读的，除非传递 `--allow-origin <origin>`——同源 POST 携带的 `Origin` 头会被 CORS 墙拒绝（403）——因此在 loopback 以外的任何绑定都需要传递 `--allow-origin`。

...
| `turn_error`          | S->C      | 轮次失败                                                                                                       | `sessionId, message, code?, promptId?`；使用相同的 `promptId` 关联机制。对于 agent 侧的 `-32603` 失败，当 agent 提供了详细信息时（`data.details` / `data.message` / 嵌套的 `data.error.message`），`message` 携带的是提供者自身的详情，而非通用的 JSON-RPC 文本。 |

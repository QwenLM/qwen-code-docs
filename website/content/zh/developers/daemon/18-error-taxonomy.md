...
| `file_too_large`            | 413  | 大文本缺少有限行数限制、大文本不是 UTF-8，或写入超过 `MAX_WRITE_BYTES`（5 MiB）。            | 对大 UTF-8 文本使用有限行数限制，使用字节窗口，或拆分写入。                                                                       |
| `hash_mismatch`             | 409  | 乐观并发 `expectedSha256` 失败，或文件在稳定读取期间发生了变更。                              | 重新读取文件并使用新版本/哈希重试。                                                                                               |
...
| 类                                       | HTTP | 原因                                                                                           | 修复措施                                                                                                                                                                  |
| ---------------------------------------- | ---- | ---------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `SessionNotFoundError`                   | 404  | sessionId 不在 `byId` 中（`code: "session_not_found"`），或会话正在关闭（`code: "session_closing"`）。                | 对于 `session_not_found`：重新创建或附加；会话可能已被回收。对于 `session_closing`：等待并重试；并发关闭正在进行中。DELETE 路由将 `session_closing` 视为幂等成功。         |
...
| `RestoreInProgressError`                 | 409  | `loadSession`、`resumeSession` 或 `POST /session` 上调用方提供的 id 与已拥有相同 id 的另一注册项冲突。                                                                                                    | 等待公布的延迟后重试请求的 restore 或 spawn；废弃清理携带基于预算的回退时间。                                                      |

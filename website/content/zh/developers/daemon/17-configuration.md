...
| `--open-with-auth`                      | boolean                    | `false`                                   | 在环回上以 bearer 认证打开 Web Shell。需要启用 Web Shell 和已构建的资源。复用已选择的 `--token` / `QWEN_SERVER_TOKEN`，或在监听前生成进程生命周期的 256 位 bearer。在浏览器不符合条件的环境中，启动并打印包含密钥的 fragment URL。不是 `ServeOptions` 或 SDK 设置。                                           |

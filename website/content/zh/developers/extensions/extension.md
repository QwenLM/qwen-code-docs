...
- `excludeTools`: 要从模型中排除的工具名称数组。你也可以为支持的工具指定特定命令的限制，比如 `run_shell_command` 工具。例如，`"excludeTools": ["run_shell_command(rm -rf)"]` 将阻止 `rm -rf` 命令。请注意，这与 MCP 服务器的 `excludeTools` 功能不同，后者可以在 MCP 服务器配置中列出。**重要：** 在 `excludeTools` 中指定的工具将在整个对话上下文中被禁用，并会影响当前会话中的所有后续查询。

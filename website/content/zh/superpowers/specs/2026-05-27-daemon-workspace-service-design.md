...
> JSON-RPC error codes 遵循 [ACP error code registry](https://spec.acpprotocol.org)（标准范围 -32000 ~ -32099 为服务端定义的应用错误）。具体 code 值在实现时对齐 `/acp` 现有 error 映射逻辑（`acp-integration/errorCodes.ts`）。
...
| Route integration | 现有 route 测试改为经 service（验证 HTTP surface 不变）                | 回归保障，REST 路径不 break                                    |

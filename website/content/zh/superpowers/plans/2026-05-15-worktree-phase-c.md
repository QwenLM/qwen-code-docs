# Worktree Phase C 实现计划

> **对于代理型开发者：** 必需的子技能：使用超能力：subagent-driven-development（推荐）或 superpowers:executing-plans 来逐步实现本计划。步骤使用复选框 (`- [ ]`) 语法进行跟踪。

**目标：** 为 worktree 添加会话持久化、hooksPath 初始化、Footer 状态展示和退出对话框，使 worktree 在 `--resume` 后可恢复，用户始终知道自己在哪个隔离环境中。

**架构：** 新增 `WorktreeSession` sidecar JSON 文件（与 JSONL session 文件并存），`EnterWorktree` 写入、`ExitWorktree` 清除；CLI 层通过 `useWorktreeSession` hook 监听文件变化并同步到 `UIState.activeWorktree`；Footer 读取该字段内置渲染 worktree 行；`WorktreeExitDialog` 在检测到活跃 worktree 时拦截第二次 Ctrl+C。

**技术栈：** TypeScript, React (Ink), Node.js `fs.watch`, `simple-git`, Vitest
...
- 新建：`packages/core/src/services/worktreeSessionService.ts`
- 新建：`packages/core/src/services/worktreeSessionService.test.ts`
- 修改：`packages/core/src/services/sessionService.ts`

- [ ] **第 1 步：新建 `worktreeSessionService.ts`**
...
- [ ] **第 4 步：修复 `writeWorktreeSession` 中的 require 调用**
...
- [ ] **第 6 步：在 `SessionService` 中新增 `getWorktreeSessionPath()`**
...
- 修改：`packages/core/src/services/gitWorktreeService.ts:1133-1158`（`createUserWorktree`）
- 修改：`packages/core/src/services/gitWorktreeService.test.ts`
...
- [ ] **第 3 步：在 `createUserWorktree` 中追加 hooksPath 配置**
...
- [ ] **第 4 步：对 `createAgentWorktree` 做相同修改**

找到 `createAgentWorktree` 方法，在其 `git worktree add` 之后添加相同的 hooksPath 代码块（完整代码与第 3 步相同，`slug` 来自 agent worktree 的参数）。
...
- 修改：`packages/core/src/tools/enter-worktree.ts`
- 修改：`packages/core/src/tools/enter-worktree.test.ts`
...
- [ ] **第 3 步：修改 `enter-worktree.ts`，在成功创建后写入 session**
...
- 修改：`packages/core/src/tools/exit-worktree.ts`
- 修改：`packages/core/src/tools/exit-worktree.test.ts`
...
- 新建：`packages/cli/src/ui/hooks/useWorktreeSession.ts`
- 修改：`packages/cli/src/ui/contexts/UIStateContext.tsx`
- 修改：`packages/cli/src/ui/AppContainer.tsx`

- [ ] **第 1 步：在 `UIStateContext.tsx` 新增 `activeWorktree` 字段**
...
- [ ] **第 2 步：新建 `useWorktreeSession.ts`**

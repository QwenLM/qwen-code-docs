...
- ✅ **Fresh Local Control token** — `--local-control` 为该进程生成一个 token。通用守护进程 token 存储仍需自带 token。
...
| `--local-control`                       | `false`         | 在每个非环回 IPv4 接口上共享经过身份验证的 Web Shell，使用临时的每进程 token、带标签的终端二维码、精确的浏览器 origin、固定端口和尽力而为的睡眠抑制。与 `--token`、`--allow-origin`、`--no-web`、`--port 0` 和非默认的 `--hostname` 冲突；添加 `--tls-cert` + `--tls-key` 以解锁安全上下文浏览器 API（如语音输入）。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
| `--max-journal-events <n>`              | `10000`            | 每个会话在飞行中 `liveJournal`（当前未完成的 turn）中保留的重放条目数量上限。连续的兼容文本或思考 chunk 共享一个条目，每个条目最多 256 个源事件；其他事件边界会被保留。超出时，最旧的条目会被丢弃，并在前面加上 `history_truncated` 标记。标记的 `truncatedEvents` 和 `retainedEvents` 计数描述的是源事件。必须是正安全整数。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| `--max-journal-bytes <n>`               | `8388608`          | 每个会话的飞行中 `liveJournal` 字节上限，从序列化的源事件计算，即使兼容的 chunk 共享一个重放条目。超出时，最旧的条目会被整体丢弃（始终至少保留一个条目），因此保留的尾部可能远小于上限。必须是正安全整数。默认为 8 MiB。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |

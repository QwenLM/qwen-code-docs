...
然后客户端在普通 API 请求上发送 `Authorization: Bearer $QWEN_SERVER_TOKEN`。`/health` **仅在普通环回绑定上**被豁免，因此 pod 内的 k8s/Compose 存活探针不需要凭据。在非环回绑定（`--hostname 0.0.0.0` 等）上，`/health` 与其他普通 API 路由一样需要 token — 否则攻击者可以探测任意地址以确认守护进程的存在。频道 webhook 入口在每个模式下都是独立的，使用其配置的 `x-qwen-webhook-secret` 而不是守护进程 bearer。使用 `/capabilities` 来端到端验证你配置的 token 是否正确：
...
- **TLS 与认证正交** —— HTTPS 加密传输层；当配置了 bearer token 时，它会门控普通 API 路由，除了普通环回绑定上的 `/health`。传递 `--require-auth` 也可以门控环回 `/health`。频道 webhook 入口始终使用自己的共享密钥。非环回绑定无论是否使用 TLS 都需要 token。
...
- **IPv6 通配符绑定会回拨到该主机实际分配的环回地址** — `--hostname ::`（或 `[::]`）绑定 IPv6 socket，当 IPv6 可用时空的 `--hostname` 也会绑定（不可用时 Node 回退到绑定 `0.0.0.0`）。该 socket 是双栈的（Node 在其上固定 `IPV6_V6ONLY=0`，因此 `net.ipv6.bindv6only` sysctl 不会改变这一点），两个环回地址通常都能到达它 — 但完全没有 IPv4 的主机只有 `::1`，而绑定了 `::` 但其环回上没有 `::1` 的主机（例如 `net.ipv6.conf.lo.disable_ipv6=1`）只有 `127.0.0.1`。当该主机分配了 `[::1]` 时 worker 会被发送到 `[::1]`，否则发送到 `127.0.0.1`。IPv6 通配符绑定的服务证书应在其 SAN 中包含两个环回地址（`mkcert localhost 127.0.0.1 ::1` 可以覆盖）；启动信任诊断会检查 worker 将拨号的确切 URL 并指出差距。`--hostname 0.0.0.0` 不变，仍然需要 `127.0.0.1`。

...
当未提供 `--token` 和 `QWEN_SERVER_TOKEN` 时，非环回监听器会生成一个 128 位的临时 bearer（22 个 URL 安全字符），并在启动时打印一次。显式标志值优先于环境变量；在非环回绑定上，显式空值仍然 fail closed 并拒绝启动，而在环回地址上它会覆盖 `QWEN_SERVER_TOKEN` 并使绑定保持 `--require-auth` 下描述的受信任无 token 模式。默认环回行为不变。

在通配符绑定上，启动时会列出来自实际接口的无 token 本地/网络 URL，以及——对于生成的 token，或在交互式终端上对于你自己的 token——一个**包含机密的二维码**，对应一个标记的私有 LAN 地址。通配符枚举仅列出与 Local Control 通告的相同私有/链路本地 LAN 地址群（RFC 1918 / RFC 3927 IPv4，加上双栈绑定上的 fc00::/7 ULA IPv6）；用于排除 VPN 适配器和容器桥接的软件接口过滤器是一个**名称启发式**（[#9158](https://github.com/QwenLM/qwen-code/issues/9158)），而非保证，因此 k8s overlay 名称（`cbr0`、`cali*`、`vxlan.*`）可能会漏过。显式非环回绑定则精确打印和生成你所选地址的二维码——包括可路由的地址。在容器内部，枚举的地址是容器自身的接口，因此要访问已发布的端口需要替换为主机地址；分配了 pty 的容器视为交互式终端，因此其日志仍然包含机密。扫描二维码即可在 URL 片段中携带 bearer 打开内置 Web Shell。浏览器会从 URL 中移除凭据并将其保留在每标签的 `sessionStorage` 中。或者，打开打印的 URL 并输入 token；缺失或过期的凭据会显示重试表单。生成的 token 在重启时轮换。`--no-web` 省略二维码，管道/重定向的 stdout 永远不会将稳定的运维 token 重新发布为二维码。地址不意味着防火墙或 NAT 可达性；不会自动发现公网 IP。

内置 Web Shell 的同源 **HTTP** 请求无需 `--allow-origin` 即可工作；其他浏览器 origin 仍然需要该白名单，`Origin: null` 仍然被拒绝。基于 WebSocket 的功能（终端、语音）以及通过 TLS 终止反向代理访问守护进程的浏览器仍然需要显式的 `--allow-origin <origin>` 条目——转发的 header 永远不会被信任为同源。任何重写 `Host` header 的纯 HTTP 中间件也是如此：nginx 默认的 `proxy_set_header Host $proxy_host` 和 k8s Ingress 都会使守护进程看到一个不再匹配浏览器 `Origin` 的 `Host`。为浏览器实际看到的 origin 传入 `--allow-origin <origin>`，或配置代理原样转发 `Host`。在**非环回**绑定上，仅端口转换（`docker -p 8080:4170`）无需额外配置——同源检查将 `Origin` 与转发的 `Host` 进行比较，从不查询监听端口。在默认的**环回**绑定上则不然：DNS 重绑定 Host 白名单仅接受守护进程自身的端口，因此端口转换隧道（`ssh -L 8080:localhost:4170`）会对每个请求（包括 shell 文档）返回 `403 Invalid Host header` 被拒绝，且 `--allow-origin` 无法覆盖它——请转发相同端口（`ssh -L 4170:localhost:4170`）或绑定非环回地址。Bearer 授予完整的守护进程 API 权限，包括代码执行。将终端 token 和二维码截图视为机密。HTTP 未加密；现有的 `--tls-cert` 和 `--tls-key` 选项提供使用你自己证书的 HTTPS。
...
默认绑定为 `127.0.0.1:4170`。Bearer 认证**关闭**，主监听器是受信任的，因此任何能访问该端口的本地进程都可以使用完整的运维 API，包括以守护进程用户身份执行代码。特定于路由的工作区信任、会话所有权、`X-Qwen-Client-Id`、权限、特性、验证和资源检查仍然适用。守护进程将当前工作目录注册为主工作区；使用绝对路径 `--workspace /path/to/dir` 可覆盖，重复该标志可注册额外的隔离运行时。
...
默认注册限制为 256。设置 `QWEN_SERVE_MAX_WORKSPACES=25 qwen serve --workspace /path/to/project` 可降低该值；接受的值是 1 到 256 的十进制整数。无效值会导致启动失败。这是一个守护进程启动设置：项目 `.env` 和项目 `settings.env` 无法更改它。嵌入调用方可以设置 `ServeOptions.maxRegisteredWorkspaces`，其优先级高于启动环境。

该限制计入主工作区、已注册的次要工作区和用户创建的临时工作区。内部 Conversations 运行时不受此限制。动态注册在达到容量时返回 `409 workspace_limit_reached`。`GET /capabilities` 和 `GET /daemon/status` 暴露 `limits.maxRegisteredWorkspaces`。

在默认注册容量 256 下，标准守护进程默认为固定的全局会话上限 800，包括仅启动一个工作区的守护进程。相同的会话默认值适用于任何高于 25 的配置注册容量。`--max-total-sessions` 仍然可以覆盖它；`0` 禁用它。在 25 或更低的容量下，现有默认值保持不变：一个启动或恢复的工作区无限制，多个工作区则从每工作区上限和该数量派生总数。注册更改不会重新计算总数。这不会更改 ACP 子进程内存策略，也不保证 800 个安全并发会话。

频道控制独立支持最多 25 个工作区所有者，暴露为 `limits.maxChannelControlWorkspaces`。转换也会计入为恢复而保留的所有者；在容量达到上限时，在添加新所有者之前先成功停止旧所有者上所有选定的频道。超量的转换返回 `409 channel_control_workspace_limit_reached`。`--channel all` 继续仅选择主工作区。频道 SDK 超时不变。

持久化注册文件保留 schema 版本 1，在 8 MiB 内支持最多 255 条次要记录。原始记录（包括过期别名）会消耗持久化槽位直到被遗忘。如果完整的合法启动集超过配置的限制，启动失败并保持文件不变。要降低容量，请以前一个容量重启，遗忘足够的注册，并减少显式 `--workspace` 参数，然后再以更低限制重启。在降级到旧版二进制之前，请备份文件并使用新版二进制将其缩小到最多 24 条次要记录和 256 KiB。否则旧版二进制会拒绝该存储并仅以显式工作区启动；它们的存储变更在写入前会失败。

### 调整会话实时状态轮询

Web Shell 默认每 **5 秒**刷新工作区会话实时状态。要使用不同的间隔，请在启动前设置守护进程环境变量：

```bash
QWEN_SESSION_LIVE_STATE_POLL_INTERVAL_MS=10000 qwen serve --web
```

该值为毫秒整数，范围从 `1000` 到 `2147483647`。缺失或无效的值回退到 `5000`。该间隔适用于该守护进程服务的所有工作区；更改后请重启守护进程并重新加载任何打开的 Web Shell 页面。本地会话操作和返回到可见标签页仍然可以立即刷新。这仅更改实时状态轮询，不更改完整的会话目录轮询。

守护进程在 `/capabilities` 中将有效间隔发布为 `sessionLiveStatePollIntervalMs`。Web Shell 使用该值，在连接到省略该值的旧版守护进程时回退到五秒。SDK 消费者可以在选择自己的轮询计划时读取该值。
...
此模式为实验性且由守护进程管理。它不会取代独立的 `qwen channel start` 命令：不带 `--daemon-url` 时，现有的 `qwen channel start`、`stop` 和 `status` 行为保持独立。使用 `qwen serve --channel` 时，显式选择优先，守护进程会在监听之前预留频道服务租约，如果初始 worker 无法就绪则启动失败。不带标志启动时，守护进程从受信任的主工作区恢复 `serve.channels`。如果两个来源都未选择频道，重量级频道运行时保持未加载状态，也不会预留频道服务租约，直到第一次运行时 PUT。次要工作区不会独立恢复自己的 `serve.channels`；请在启动后通过运行时 API 启用这些频道。如果已就绪的 worker 后来崩溃，守护进程会继续运行，在有界重启策略下重新启动它，并在 `GET /daemon/status` 中报告其状态（包括 `channel_worker_exited` 警告）。

自动恢复会跳过无效的启动字段，或者在 worker 启动前跳过验证或租约错误，记录其 `serve.channels` 来源，并保持不相关的设置生效。在 worker 启动失败后，守护进程仅在清理成功后才继续。全局运行时启动超时和未确认的 worker 停止保留正常的启动失败行为；在 worker 终止未确认期间，服务租约保持持有。请检查守护进程日志以了解跳过或失败的恢复。

存储的启动名称必须非空，没有前导或尾随空格，且不包含不安全的控制字符或不可见字符。无效条目会按数组索引单独跳过并记录。启动既不会将它们修剪为不同的实例名称，也不会重写设置。频道启动开关反映持久化配置，而运行时状态报告频道是否正在运行。

运行时控制通过 `GET`、`PUT` 和 `DELETE /workspace/channel` 暴露；SDK 辅助方法为 `getChannelWorkerControl()`、`setChannelWorkerSelection()` 和 `stopChannelWorker()`。PUT/DELETE/reload 使用严格变更门控：受信任的环回主调用方可以无 token 使用它们，而其他部署模式需要运维凭据。运行时选择是故意临时性的：PUT 不会编辑设置或启动选项，重启会回到显式的 `qwen serve --channel` 选择，否则回到受信任主工作区的 `serve.channels`，或者在两者都不存在时回到禁用状态。显式 CLI 和运行时命名选择会按首次出现顺序修剪和去重；顺序会被保留，因为第一个频道可能影响共享模型选择。

守护进程支持的 `qwen channel set` 和 `qwen channel reload`，以及带 `--daemon-url` 的 `status` 和 `stop`，无法发现由 `--open-with-auth` 生成的 token。请使用 `QWEN_SERVER_TOKEN` 并通过 `--token` 向这些客户端传递相同的值，或者省略已认证打开模式。
...
守护进程还为客户端 UI 和运维人员暴露只读的运行时快照：`GET /daemon/status`、`GET /workspace/mcp`、`GET /workspace/skills`、`GET /workspace/providers`、`GET /workspace/env`、`GET /workspace/preflight`、`GET /workspace/:id/session-info`、`GET /session/:id/status`、`GET /session/:id/context`、`GET /session/:id/supported-commands`、`GET /session/:id/tasks`、`GET /session/:id/lsp`、`GET /session/:id/resources` 和 `GET /session/:id/transcript`。
...
`GET /session/:id/resources` 返回该实时会话自身 Config 中经过净化的 Skill 和 MCP 快照。当会话可能从 worktree 或其他有效目录运行时（其资源与基础工作区不同），请使用它。工作区拥有的 MCP 身份验证、池、预算和发现错误详情在此会话视图中被省略。调用前请检查 `session_resources` capability。
...
要按名称更新工作区 Skill 设置，请在检查 `workspace_skill_settings_toggle` capability 后调用 `POST /workspace/skills/:name/enable` 并传入 `{ "enabled": true | false }`。要更改多个名称，请检查 `workspace_skill_settings_batch_toggle` 并调用 `POST /workspace/skills/enable` 并传入 `{ "skillNames": ["review", "deploy"], "enabled": false }`；它会一起处理每个结构上有效的名称，在最多一次锁定的设置写入中持久化所有产生的声明更改，并在有任何更改时刷新活跃 ACP 会话一次。这些路由写入工作区的 `skills.disabled` 和 `skills.enabled` 而不咨询已加载的 Skill 目录，因此名称可以在安装前或 Extension 不活跃时被禁用。启用会移除匹配的工作区禁用并记录显式的 `skills.enabled` 选择加入，即使在安装之前也是如此，因此此条目可以覆盖 Extension 内部的 Skill 禁用。重复相同的声明是空操作（`changed: false`）。它们的路径和请求 body 不变；特定于设置的 capability 取代了已退役的目录验证 Skill 切换标签。批次的 `errors` 数组保留用于线路兼容性，对于结构上有效的名称为空。从更高作用域继承的硬 `skills.disabled` 条目对于有效可用性仍然具有权威性，但不会阻止工作区记录或移除其自己的声明。Skill 状态单元格暴露 `disabledReason`（`hard`、`default` 或 `inactive_extension`）和可选的 `lockedScope`。不受信任的工作区写入仍然被拒绝。`deferred` 响应表示在活跃性检查时没有子进程存活，或者已更改的请求在必需的刷新期间丢失了其子进程/会话；当 `changed` 为 true 时，持久化的声明在子进程启动时生效。声明是否实际更改由 `changed` 报告（对于批次请求，在每个结果上）。`skills.disabled` 同时禁用手动和模型使用，不同于 `disable-model-invocation: true`（后者保留直接 `/skill-name` 调用可用）。对于 V2 Extension 批次，请检查 `extension_batch_activation_v2`：`PUT /extensions/activation` 更改全局默认值，而 `PUT /workspaces/:workspace/extensions/activation` 更改选定工作区的精确覆盖并接受 `"inherit"` 来清除它们。两者都接受 `extensionNames` 中的名称；`enabled` 和 `disabled` 可以在安装前声明，而对未知名称的 `inherit` 是空操作。每个请求返回一个可轮询的操作。
...
Write/edit 是**严格变更路由**：无 token 的受信任环回主监听器授予运维权限；没有凭据的非受信任嵌入会返回 `token_required`。替换和编辑需要来自完整快照 `GET /file`（或全窗口 `GET /file/bytes`）的最新 `expectedHash`。部分大文件窗口不能用作乐观并发 token。`create` 永远不会覆盖。允许对忽略路径进行显式写入，但会被审计。二进制写入、删除/移动/mkdir 以及递归父目录创建不属于此接口范围。
...
该选项默认为 `false`，需要重启，在安全模式、裸模式和审批 `plan` 模式下被强制关闭。它仅在内存中：从磁盘加载 Todo 状态或重启守护进程不会武装它。在 settings.json 中设置 `tools.todoWrite.enabled` 并重启 Qwen Code，然后新的普通 prompt 必须成功运行自己的顶级 `todo_write`；retry/continue 和实时客户端重新附加保留当前的内存中工作链。成功更改会话工作目录会清除它，因此旧 Todo 无法在新工作区中恢复。

## 身份验证

对于环回地址之外的任何访问，每个 API 请求都需要 bearer token。为可重用的客户端提供一个稳定的 token，或者不提供 token 而使用守护进程生成并打印的临时 token：
...
然后客户端在普通 API 请求上发送 `Authorization: Bearer <token>` — 导出了 `$QWEN_SERVER_TOKEN` 时使用它，或者使用守护进程生成时在启动时打印的 bearer（该路径从不设置环境变量，因此请从启动输出中复制该值）。`/health` **仅在普通环回绑定上**被豁免，因此 pod 内的 k8s/Compose 存活探针不需要凭据。在非环回绑定（`--hostname 0.0.0.0` 等）上，`/health` 与其他普通 API 路由一样需要 token — 否则攻击者可以探测任意地址以确认守护进程的存在。频道 webhook 入口在每个模式下都是独立的，使用其配置的 `x-qwen-webhook-secret` 而不是守护进程 bearer。使用 `/capabilities` 来端到端验证你配置的 token 是否正确：

> **加固的环回（`--require-auth`）。** 默认的环回无 token 行为适用于单用户笔记本电脑，但在共享开发主机、CI 运行器或多租户工作站（任何本地用户都可以 `curl 127.0.0.1:4170`）上是不安全的。传递 `--require-auth` 可使 bearer token 在每个路由上都是强制的 — 包括 `/health` 和 `/capabilities` — 即使绑定到 `127.0.0.1`。频道 webhook 入口保持通过 `x-qwen-webhook-secret` 独立认证。Web Shell 文档和资源路由保持预认证；使用 `--no-web` 移除它们。当没有 token 来源解析时启动失败，在环回地址上这意味着 `--token`、`QWEN_SERVER_TOKEN` 或 `--open-with-auth` — 最后一个会在启动前安装自己生成的 token，因此 `--require-auth --open-with-auth` 可以启动。生成取决于 `--hostname` 的拼写：环回拼写（`127.0.0.0/8`、`localhost`、`::1`、`[::1]`）从不生成，而其他任何拼写 — 包括恰好解析为环回的 DNS 名称 — 会生成一个满足 `--require-auth` 的临时 bearer 并在启动时打印（当 socket 绑定到环回时仅打印 token），每次重启时轮换，因此可重用客户端请固定 `--token` 或 `QWEN_SERVER_TOKEN`。启用该标志后，**未身份验证**的客户端无法读取 `/capabilities` 来发现需要身份验证；发现表面就是 401 响应 body 本身。身份验证后，`caps.features.require_auth` 标签是部署已加固的 post-auth 确认（对审计/合规 UI 很有用）：
>
> ```bash
...
- **TLS 与认证正交** —— HTTPS 加密传输层；当配置了 bearer token 时，它会门控普通 API 路由，除了普通环回绑定上的 `/health`。传递 `--require-auth` 也可以门控环回 `/health`。频道 webhook 入口始终使用自己的共享密钥。非环回绑定无论是否使用 TLS 都需要 token — 你传入的 token，或者守护进程生成并在启动时打印的临时 token。
- **作用域仅限 TLS 终端** —— 不支持自动生成，不支持 ACME / Let's Encrypt。这主要是为了局域网/开发环境的便利；对于面向互联网的部署，请在反向代理处终止 TLS（参见下方的威胁模型）。
- **频道 worker 通过 `https://` 回连守护进程** —— 因此它们也需要信任服务证书。自签名证书（或携带自身根证书的完整链）无需额外操作：守护进程会将其注入每个 worker 的 `NODE_EXTRA_CA_CERTS`。上述 mkcert 流程是 **CA 签发的**，因此仅靠叶子证书无法锚定链 — 在使用 `--channel` 启动前，请在守护进程的启动环境中导出 `NODE_EXTRA_CA_CERTS="$(mkcert -CAROOT)/rootCA.pem"`。运维人员设置的值会与守护进程证书**合并**，而非替换。缺少该设置时，守护进程启动显示正常，但每个频道 worker 会因 `UNABLE_TO_VERIFY_LEAF_SIGNATURE` 而不断重启循环；守护进程日志会在启动时指出此差距。
- **IPv6 通配符绑定会回拨到该主机实际分配的环回地址** — `--hostname ::`（或 `[::]`）绑定 IPv6 socket。（空的 `--hostname` 永远不会到达监听器：启动会在绑定前将其作为运维错误拒绝，因此未设置 `HOSTNAME` 的 compose 文件会大声失败，而不是静默通配符化。）该 socket 是双栈的（Node 在其上固定 `IPV6_V6ONLY=0`，因此 `net.ipv6.bindv6only` sysctl 不会改变这一点），两个环回地址通常都能到达它 — 但完全没有 IPv4 的主机只有 `::1`，而绑定了 `::` 但其环回上没有 `::1` 的主机（例如 `net.ipv6.conf.lo.disable_ipv6=1`）只有 `127.0.0.1`。当该主机分配了 `[::1]` 时 worker 会被发送到 `[::1]`，否则发送到 `127.0.0.1`。IPv6 通配符绑定的服务证书应在其 SAN 中包含两个环回地址（`mkcert localhost 127.0.0.1 ::1` 可以覆盖）；启动信任诊断会检查 worker 将拨号的确切 URL 并指出差距。`--hostname 0.0.0.0` 不变，仍然需要 `127.0.0.1`。
- **原地轮换 `--tls-cert` 需要重启守护进程** —— 守护进程提供的是启动时读取的字节，因此在重启之前，重新生成的 worker 可能加载比守护进程呈现的更新的内容，导致握手失败。
...
| `--hostname <addr>`                     | `127.0.0.1`     | 绑定接口。除环回地址外的任何绑定都需要 token；当 `--token` 和 `QWEN_SERVER_TOKEN` 都未提供 token 时，会生成一个临时 token 并在启动时打印。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
| `--token <str>`                         | —               | Bearer token。回退到 `QWEN_SERVER_TOKEN` 环境变量（会自动去除首尾空格 —— 方便使用 `$(cat token.txt)`）。非环回绑定在两个来源都未提供时会生成临时 token。显式空值在非环回绑定上仍然拒绝启动；在环回地址上它会覆盖环境变量并选择受信任的无 token 模式（或在 `--require-auth` 下直接拒绝）。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| `--require-auth`                        | `false`         | 如果没有 bearer token 则拒绝启动，即使在环回地址上也是如此。加固了 `127.0.0.1` 的开发者默认配置，适用于共享开发主机 / CI 运行器 / 多租户工作站等任何本地用户都能访问监听器的场景。该快速失败仅限环回：环回启动仅需要三个 token 来源之一 — `--token`、`QWEN_SERVER_TOKEN` 或 `--open-with-auth`（后者在启动前安装自己生成的 token；生成在环回地址上被抑制）— 而在非环回绑定上，生成的临时 token 即可满足该标志。同时将 `/health` 置于 bearer 保护之下。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| `--enable-session-shell`                | `false`            | 启用直接会话 shell 执行。在配置了 bearer token 或受信任环回模式下有效；调用仍然需要有效的会话绑定 `X-Qwen-Client-Id`。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
| `--max-total-sessions <n>`              | `800` 或派生值   | 可选的非负整数，跨所有已注册工作区运行时的守护进程级新会话创建上限。它适用于新子会话、会话恢复和分支/fork 创建的会话；附加到现有活跃会话不消耗名额。设置为 `0` 表示无限制。当注册容量高于 25 时默认为 800（包括默认的 256）。在 25 或更低的容量下，一个启动或恢复的工作区无限制；多个工作区则从每工作区上限和该数量派生总数。后续动态注册不会重新计算。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
| `--session-restore-timeout-ms <n>`      | `60000`            | ACP 会话加载/恢复截止时间（毫秒）。必须是正整数，最大到 `2147483647`；`0` 无效。如果省略，默认值为 60 秒，当显式提供的 `--initialize-timeout-ms` 更大时会提升到该值；更短的初始化超时不会降低恢复预算。SDK 和 Web Shell 会分别增加 10 秒和 15 秒的客户端余量。超时返回可重试的 `504 session_restore_timeout`；这并不意味着守护进程本身已退出。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| `--allow-origin <pat>`                  | —               | T2.4 ([#4514](https://github.com/QwenLM/qwen-code/issues/4514))。浏览器客户端的跨域白名单。可重复。每个值为 `*`（任意 origin —— 在环回绑定上，如果未配置 bearer token 则启动时拒绝；在非环回绑定上，生成的临时 token 满足这些守卫，因此拒绝仅发生在环回地址上；建议在环回地址上使用 `--require-auth`，这样 `/health` 也受 bearer 门控，因为默认情况下它在环回地址上是预认证的；Web Shell 静态资源在任何模式下都保持预认证，因此传入 `--no-web` 以移除它们）或规范的 URL origin（`<scheme>://<host>[:<port>]`，无尾部斜杠 / 路径 / 用户信息 / 查询参数）。没有 token 时，在环回绑定上 HTTP(S) origin 必须是环回地址；在非环回绑定上，生成的 token 满足相同的守卫。远程 HTTP(S) origin 在每个 API 路由上使用 bearer 进行身份验证；预认证例外仅包括 Web Shell 文档/资源路由、MCP App 沙箱、频道 webhook 入口（使用其自己的 `x-qwen-webhook-secret` 进行身份验证），以及环回绑定上的 `/health`（在其他地方受 bearer 门控）。浏览器扩展在本地保留无 token 访问。故意不支持子域名通配符（`https://*.example.com`）—— 请显式列出每个子域名，或者使用 `*` 并配置 token（以及使用 `--require-auth` 进行完全加固）。匹配的 origin 会收到 CORS 响应头（`Access-Control-Allow-Origin`、`Vary: Origin`、methods、headers、max-age 以及暴露的 `Retry-After`）；不匹配的 origin 仍会收到 403，并带有与当前 wall 相同的信封。`Origin: null`（沙箱 iframe、file:// 文档）总是被拒绝，即使在 `*` 下也是如此。通过 `caps.features.allow_origin` 进行预检。环回地址自身的 origin 命中不受影响。 |
| `--web` / `--no-web`                    | `true`          | 在守护进程根目录提供构建好的 Web Shell SPA（`GET /`、`/assets/*` 以及 `GET /session/<id>` 文档导航）。这些入口点注册在 bearer 认证网关**之前** —— 浏览器无法将 token 附加到 `<script>` 子资源或地址栏导航，shell 不携带任何机密。API 请求遵循守护进程的正常权限策略：配置的 token 门控普通 API 路由（环回 `/health` 除外，除非设置了 `--require-auth`），频道 webhook 入口始终使用自己的共享密钥，无 token 的受信任环回主监听器具有完整的运维访问权限。所有其他路径的 SPA 深度链接回退位于 bearer 中间件之后。在非环回地址绑定时，stderr 会输出一行警告，提示 UI 可在无认证的情况下访问。对于纯 API 守护进程，请使用 `--no-web`。当构建时省略了 Web Shell 资源时，此参数无效（守护进程会记录一条 breadcrumb 并以纯 API 模式运行）。                                                                                                                                                                                                                                                                                                          |
| `--open`                                | `false`         | 监听器启动后，在默认浏览器中打开守护进程 URL 处的 Web Shell（当 token 可解析时（无论是配置的还是生成的），会在 URL 片段中追加 `#token=` —— 片段永远不会发送到服务器，从而避免 token 出现在访问日志和 Referer 头中）。如果使用 `--no-web`，或者在没有浏览器的无头 / CI / SSH 环境中，则此操作无效。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
> **调整负载参数。** `--max-sessions` 是每个工作区的新会话上限。`--max-total-sessions` 是守护进程级的新会话上限；省略时默认值为 800（基于默认注册容量 256）。另外三个层级也会限制负载——在为高并发部署调整大小时，请将它们一起调优：
>
...
>
> 这些上限会相互影响：每个运行时受 `--max-sessions` 限制，而 `--max-total-sessions` 限制其聚合。有效会话上限是任何有限守护进程级上限和聚合每运行时上限中的较低者（如果每工作区上限无限制，则将该聚合视为无限制）。如果两者都不是有限的 —— 在默认注册容量下需要 `--max-total-sessions 0` —— 则没有有限会话上限。有限上限 × 64 个 subscriber × 256 个 frame 是 EventBus 层最坏情况下的内存中占用，再乘以 `--max-pending-prompts-per-session` 限制准入层接受的 prompt 工作量。默认大小假设是单用户/小团队负载；对于更大的部署，请逐步提高（并观察 RSS）。
...
- **仅限环回** — `127.0.0.0/8`、`localhost` 或 `::1`；无需认证。
- **`localhost` 只解析一次、固定并在监听后验证** — 如果其实际监听地址不在 `127.0.0.0/8` 或 `::1` 范围内，无 token 启动会关闭监听器并失败。
- **`--hostname 0.0.0.0` 需要 token** — 当两个 token 来源都未提供时会生成一个；显式空来源仍然 fail closed。
- **`LOOPBACK_BINDS` 包含 IPv6** —— `::1` 和 `[::1]` 在无 token 规则下被视为 loopback。
- **Host header 白名单** —— 在 **loopback** 绑定上，daemon 会检查 `Host:` 是否匹配 `localhost:port` / `127.0.0.1:port` / `[::1]:port` / `host.docker.internal:port` 或精确绑定的环回地址和端口（根据 RFC 7230 §5.4 不区分大小写）以防御 DNS rebinding。当监听端口 80 或 443 时，也接受相应的无端口形式，因为浏览器会省略 scheme 默认端口。这支持完整的 IPv4 环回范围（`127.0.0.0/8`）而不会接受不相关的 Host。**非 loopback 绑定（`--hostname 0.0.0.0`）故意绕过 Host 白名单** —— 运维人员已经选择了暴露面，因此 bearer-token 网关是唯一的身份验证层；反向代理 / SNI / client cert pinning 是运维人员的责任，而不是 daemon 的责任。如果你需要在非 loopback 绑定上实现基于 Host 的隔离，请在前端代理处终止 TLS 并检查 Host。
- **CORS 默认拒绝任何浏览器 Origin** —— 返回 `403` JSON。传递 **`--allow-origin <pattern>`**（可重复，T2.4 #4514）以允许特定的浏览器 origin。每个值要么是字面量 `*`（任何 origin——在 **loopback** 绑定上，如果未配置 bearer token 则启动时拒绝，而非 loopback 绑定会启动因为 token 生成会提供一个；建议在 loopback 上使用 `--require-auth` 以进行完全加固，因为默认情况下 `/health` 在 loopback 上是预认证的 — 注意 Web Shell 静态资源（`/`、`/assets/*`、`/session/:id` 文档导航）在任何模式下都在 bearer 之前挂载，即使在 `--require-auth` 下也保持预认证，所以当残留浏览器暴露面很重要时使用 `--no-web`），要么是规范的 URL origin（`<scheme>://<host>[:<port>]`，无尾随斜杠/路径/userinfo）。没有 token 时，在 loopback 绑定上 HTTP(S) 条目必须使用 loopback 主机；非 loopback 绑定总是携带 token — 提供的或生成的 — 正是因为远程托管的浏览器 origin 可以通过运维 API 以守护进程用户身份执行代码。显式的浏览器扩展 origin 保留其无 token 的本地自动化路径。匹配的 origin 会收到正确的 CORS 响应头（`Access-Control-Allow-Origin: <echoed>`、`Vary: Origin`，以及标准的 methods / headers / max-age 和暴露的 `Retry-After`）；不匹配的 origin 仍会收到 403，并使用与默认墙相同的 envelope。`caps.features.allow_origin` 是有条件广播的，因此 SDK / webui client 可以在发出跨域请求之前预检 daemon 是否支持。示例：`qwen serve --allow-origin http://localhost:3000 --allow-origin http://localhost:5173`。Loopback 自身 origin 的请求不受影响——一个单独的 Origin 剥离 shim 会处理它们，无论 `--allow-origin` 如何配置。内置远程 Web Shell 的同源 **HTTP** 请求也无需 `--allow-origin` 即可工作：在非 loopback 主监听器上，等于直接 socket scheme 加规范化 `Host` 权限的 `Origin` 会经过 bearer 认证，然后在 CORS 墙之前被剥离，而其现有的公共静态路由保持未认证。仍有两种排除情况需要 `--allow-origin` — 转发的 header 永远不会建立同源信任，因此 TLS 终止反向代理的 `https` 浏览器 origin 不是同源的（重写 `Host` header 的纯 HTTP 中间件也不是，例如 nginx 默认的 `proxy_set_header Host $proxy_host` 或 k8s Ingress；补救方法是为浏览器看到的 origin 传入 `--allow-origin <origin>`，或让代理原样转发 `Host` — 仅端口转换无需任何操作，因为只比较转发的 `Host`，仅剥离 scheme 默认的 `:80`/`:443`），以及 WebSocket 升级路由（终端、语音）保留单独的 CSWSH 门控，仅允许 loopback origin、白名单 origin 和 Local Control 监听器自身的 origin。
...
Issue [#4514](https://github.com/QwenLM/qwen-code/issues/4514) T2.9 提供了两个可选标志，弥补 15 秒心跳 + AbortSignal 未覆盖的长时间运行/远程部署差距。共享的权限响应超时也在此处列出。三者均默认关闭。
...
会话附件（Web Shell 通过 `POST /session/:id/attachments` 上传的文件和图片）默认存储在工作区的运行时临时目录下：`<runtimeBaseDir>/tmp/<projectHash>/attachments`，按会话键控为 `session-<sessionId>`。希望附件持久化到运行时临时目录之外（例如专用卷）的运维人员可以覆盖根目录：

| 环境变量                              | 值   | 默认值 | 功能                                                                                                                                                                                                |
| ------------------------------------- | ------ | ------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `QWEN_SERVE_SESSION_ATTACHMENTS_ROOT` | 路径   | 未设置   | 将会话附件存储在此目录下，而不是默认的运行时临时目录。接受绝对路径、相对于守护进程 cwd 的路径，或针对主目录展开的 `~` / `~/…`。 |

范围和限制：

- **单向迁移。** 设置环境变量后，新附件仅写入配置的根目录下。未命中配置根目录的读取和删除会回退到默认的运行时临时目录，因此在切换**之前**上传的附件仍然可读，并且在默认回退目录可写时仍可删除 — 如果无法 unlink 旧副本（例如只读的回退卷），则会暴露错误而不是报告成功。反向操作 — 在附件已写入配置根目录后移除环境变量 — 会使这些附件无法访问；对于给定工作区，请保持该变量稳定。
- **每会话布局。** 文件在两个位置中都位于 `<root>/<projectHash>/attachments/session-<sessionId>/` 下，其中 `<projectHash>` 是默认运行时临时目录使用的相同工作区哈希；回退查找使用默认目录中的相同会话布局。指向同一配置根目录的两个工作区彼此保持隔离。
- **删除清理。** 删除会话时，其附件目录会从配置的根目录和默认回退目录中同时移除。归档会话会保留其附件，以便在取消归档后仍然存在。
- 守护进程在启动时读取该变量；更改后请重启守护进程。该目录必须可被守护进程进程写入。
...
不受信任的次要工作区在 Web Shell 中显示为 `untrusted` 和 `read-only`。它可以展开以查看持久化会话目录，但目前无法在 Web Shell 中选择或打开、恢复、用于创建会话或完整导出。REST API 遵循现有的有界文件系统读取策略，还暴露其持久化会话组目录，以及当 `workspace_persisted_transcript` 被公布时，通过有界的工作区限定分页器暴露其活跃持久化转录。前进和游标读取不会启动 ACP 子进程；活跃会话的向后首页可能会启动一个以刷新持久化尾部。完整的工作区限定导出需要受信任的工作区和单独的 `workspace_session_export` capability。在使用执行、变更或导出功能之前，请先信任该工作区并重启守护进程。不受信任的主工作区在 Web Shell 中保持禁用状态。
...
| `GET /workspaces/:workspace/session/:id/transcript`     | 客户端需要从选定工作区获取活跃持久化转录。前进和游标页面保持在守护进程本地；活跃会话的向后首页可能会启动 ACP 并加载工作区设置以刷新持久化尾部。已注册的不受信任次要工作区可以使用此只读路径。Capability 标签：`workspace_persisted_transcript`。                                                                                                                          |
...
对于已注册的工作区，请使用 `client.workspaceById(workspaceId).getSessionTranscriptPage(sessionId, { cursor, limit })` 或 `/workspaces/:workspace/session/:id/transcript`。工作区限定方法始终使用原生 REST，即使 SDK 客户端有可替换的 ACP 传输。其游标仅限守护进程生命周期，守护进程重启后必须从第一页重新开始。活跃会话的向后首页可能会启动 ACP 并加载工作区设置以在读取前刷新持久化尾部。
...
`limit` 计算的是活跃聊天记录数，而不是发出的重放帧数；一条记录可以产生多个 `session_update` 事件。向后页面最多可扩展到 `3 * limit` 条记录，以保留 turn 和工具调用/结果边界。第一个响应冻结 JSONL 快照大小，并在 `hasMore` 为 true 时返回 `nextCursor`。后续页面忽略第 1 页之后的追加，但如果文件被删除、截断、替换、归档或以其他方式与冻结的游标冲突，则返回 `409`。非常大的快照会在索引之前返回 `413 transcript_too_large`，以便守护进程不会在请求路径上扫描无限制的转录文件。

对于通过旧版单数路由的重复分页，请将 `--channel-idle-timeout-ms` 设置为正值。在默认的 `0` 下，空闲工作区的 ACP 子进程 — 以及它持有的进程内转录索引缓存 — 会在每页之后被回收，因此每页都会重新生成子进程并通过重新扫描整个冻结前缀来重建索引（每页 `O(snapshotSize)`）。正值超时会在游标遍历期间保持子进程活跃，以便它重用缓存的转录索引和重放配置。工作区限定的前进和游标页面保持在守护进程本地；只有活跃会话的向后首页会跨越刷新屏障。
...
**`--require-auth` 与开发便利性。** 设备流路由使用严格变更门控。无 token 的受信任环回默认设置可以直接启动和管理流程；没有凭据的非受信任嵌入返回 `401 token_required`。在共享主机、CI 运行器、远程开发机器或本地进程不相互信任的任何地方，设置受管理的 `QWEN_SERVER_TOKEN` 并传递 `--require-auth`。
...
严格门控（需要受信任的环回主监听器或运维凭据）。通过活动的 `McpClientManager` 立即连接服务器并发现其工具。
...
| `401`  | `token_required`          | 非受信任的无 token 请求被严格门控拒绝                                                           |

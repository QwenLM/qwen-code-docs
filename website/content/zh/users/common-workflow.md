...
当 Qwen Code 执行高风险工作时——代码审查、事故排查、审计或调查生产数据——一个自信但错误的结论比没有结论的代价更大。你可以在 QWEN.md 中编码团队的验证策略（参见[记忆](./features/memory.md)），这样它就适用于每个会话，而无需更改任何人的默认设置。

以下规则改编自多代理部署的生产回顾，其中自信但错误的结论反复追溯到相同的失败模式：从代码推断状态而不是查询它，将一个满足的条件视为证明，以及将早期陈述作为事实重复。

**1. 在项目 QWEN.md 中添加验证策略**

将此模板放在仓库根目录的 `QWEN.md` 中并提交，这样策略就适用于整个团队：

```markdown
# 验证策略

在调查、故障排除、审查以及任何得出结论的任务中，将这些规则视为强制性规则：

1. 在得出结论前进行验证。用来自权威来源（数据库、API、日志、命令输出）的证据支持关于数据或系统状态的声明。仅从阅读代码得出的结论必须标记为"未验证的推断"。
2. 验证完整链条。枚举因果链并检查每个环节；一个满足的必要条件不能证明结论。
3. 将对话历史视为线索，而非事实。早期转录中的陈述——包括你自己之前的断言——都是线索。在将它们作为事实重复之前，在源头重新验证它们。
4. 按顺序进行故障排除：首先是你最近的更改，然后是文档和已知问题，最后才是外部依赖。
5. 审查中的 100% 证据规则。在审查和审计任务中，没有证据就不报告结论；将可验证的证据（文件:行号、查询结果、日志摘录）附加到每个声明。
```

**2. 选择合适的范围**

参见[在哪里创建 QWEN.md](./features/memory.md#where-to-create-qwenmd)了解哪个文件适用于谁——提交项目根目录的 `QWEN.md` 副本，这样策略就覆盖整个团队。

> [!tip]
>
> - 保持规则具体且可操作："用查询结果支持每个声明"比"小心"更有效。
> - 保持策略简短——当 QWEN.md 简洁时，它被更可靠地遵循（参见[记忆](./features/memory.md)）。
> - 根据你的技术栈调整规则：命名你的团队视为权威来源的实际查询工具、仪表板或日志命令。
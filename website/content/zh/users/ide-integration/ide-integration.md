...
目前，唯一支持的 IDE 是 [Visual Studio Code](https://code.visualstudio.com/) 以及其他支持 VS Code 扩展的编辑器。如需为其他编辑器构建支持，请参阅 [IDE Companion 扩展规范](../ide-integration/ide-companion-spec)。

## 功能特性

- **工作区上下文感知：** CLI 能自动识别您的工作区信息，从而提供更相关和准确的响应。该上下文包括：
  - 您工作区中**最近访问的 10 个文件**。
  - 您当前的光标位置。
  - 您所选中的文本（最多 16KB；超出部分将被截断）。

- **原生差异对比：** 当通义千问建议代码修改时，您可以在 IDE 的原生差异查看器中直接查看变更内容。这使您可以无缝地审查、编辑，并接受或拒绝建议的更改。

- **VS Code 命令支持：** 您可以直接从 VS Code 命令面板（`Cmd+Shift+P` 或 `Ctrl+Shift+P`）访问通义千问代码功能：
  - `Qwen Code: Run`：在集成终端中启动一个新的 Qwen Code 会话。
  - `Qwen Code: Accept Diff`：接受活动差异编辑器中的更改。
  - `Qwen Code: Close Diff Editor`：拒绝更改并关闭活动的差异编辑器。
...
- **对于 VS Code 的衍生版本：** 为了支持 VS Code 的衍生编辑器，该扩展也发布在 [Open VSX Registry](https://open-vsx.org/extension/qwenlm/qwen-code-vscode-ide-companion) 上。请根据你所使用编辑器的说明，从该注册表安装扩展。
...
启用后，Qwen Code 将自动尝试连接到 IDE 伴侣扩展。
...
如果已连接，此命令将显示它所连接的 IDE，以及它所知道的最近打开的文件列表。
...
你也可以在差异视图中**直接修改建议的更改内容**，然后再决定是否接受。

如果你在 CLI 中选择了‘Yes, allow always’，那么后续的修改将不再在 IDE 中显示，而是会自动接受。
...
- **消息：** `🔴 已断开连接：无法连接到 [IDE 名称] 的 IDE 伴侣扩展。请确保该扩展正在运行，并尝试重新启动终端。要安装该扩展，请运行 /ide install。`
  - **原因：** Qwen Code 找不到必要的环境变量（`QWEN_CODE_IDE_WORKSPACE_PATH` 或 `QWEN_CODE_IDE_SERVER_PORT`）来连接到 IDE。这通常意味着 IDE 伴侣扩展未运行或未正确初始化。
  - **解决方案：**
    1. 确保你已在 IDE 中安装了 **Qwen Code Companion** 扩展并已启用。
    2. 在 IDE 中打开一个新的终端窗口，以确保它获取到了正确的环境。
...
- **消息：** `🔴 已断开连接：目录不匹配。Qwen Code 运行的位置与 [IDE 名称] 中打开的工作区位置不同。请从项目根文件夹所在的同一目录运行 CLI。`
  ...
  - **解决方案：** 使用 `cd` 命令进入 IDE 中打开的同一目录，并重新启动 CLI。

- **消息：** `🔴 已断开连接：要使用此功能，请在 [IDE 名称] 中打开一个工作区文件夹，然后重试。`
  - **原因：** IDE 中未打开任何工作区。
  - **解决方案：** 在 IDE 中打开一个工作区，并重新启动 CLI。

### 通用错误

- **消息：** `在您当前的环境中不支持 IDE 集成。要使用此功能，请在以下受支持的 IDE 中运行 Qwen Code：[IDE 列表]`

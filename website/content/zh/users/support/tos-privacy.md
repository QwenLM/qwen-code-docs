...
1. **Qwen OAuth** — 使用你的 qwen.ai 账号登录（享有每日免费额度）
2. **Alibaba Cloud Coding Plan** — 使用阿里云提供的 API Key
3. **API Key** — 使用你自己的 API Key

对于每种身份验证方法，根据底层服务提供商的不同，适用的服务条款和隐私声明也可能有所不同。
...
当你使用 qwen.ai 账号进行身份验证时，适用以下服务条款和隐私声明文档：

- **服务条款：** 你的使用行为受 [Qwen 服务条款](https://qwen.ai/termsservice) 约束。
- **隐私声明：** 数据的收集与使用方式详见 [Qwen 隐私政策](https://qwen.ai/privacypolicy)。

有关身份验证配置、额度及支持功能的详细信息，请参阅 [身份验证配置](../configuration/settings)。

## 2. 如果你使用 Alibaba Cloud Coding Plan

当你使用阿里云提供的 API Key 进行身份验证时，将适用阿里云的服务条款与隐私声明。
...
> [!important]
>
> 使用 Alibaba Cloud Coding Plan 时，你将受阿里云的条款和隐私政策约束。请查阅其官方文档，了解有关数据使用、保留及隐私实践的具体细节。

## 3. 如果你使用自己的 API Key

当你使用其他提供商的 API Key 进行身份验证时，适用的服务条款和隐私声明取决于你所选择的提供商。

> [!important]
>
> 使用你自己的 API Key 时，你将受所选 API 提供商的条款和隐私政策约束，而非 Qwen Code 的条款。请查阅该提供商的文档，了解有关数据使用、保留及隐私实践的具体细节。

Qwen Code 支持多种兼容 OpenAI 的提供商。详细信息请参阅你所选提供商的服务条款和隐私政策。
...
- **API Key：** 除你所选 API 提供商收集的数据外，Qwen Code 不会额外收集任何数据。

## 常见问题解答 (FAQ)

### 1. 我的代码（包括提示词和回答）会被用于训练 AI 模型吗？

你的代码（包括提示词和回答）是否会被用于训练 AI 模型，取决于你的身份验证方法以及你所使用的具体 AI 服务提供商：
...
- **API Key**：数据使用完全取决于你所选的 API 提供商。各提供商均有自己的数据使用政策。请查阅你所选提供商的隐私政策和服务条款。

**重要提示**：Qwen Code 本身不会将你的提示词、代码或回答用于模型训练。任何用于训练目的的数据使用均受你所验证的 AI 服务提供商的政策约束。

### 2. 什么是使用统计？选择退出控制的是什么？

**使用统计** 设置用于控制 Qwen Code 为改善用户体验和产品质量而进行的可选数据收集。
...
“使用统计”设置仅控制 Qwen Code 自身的数据收集行为。它不会影响你所选的 AI 服务提供商（Qwen、OpenAI 等）根据其自身隐私政策可能收集的数据。

### 3. 如何在不同身份验证方法之间切换？

你可以随时在 Qwen OAuth、Alibaba Cloud Coding Plan 和你自己的 API Key 之间切换：

1. **启动时**：在提示时选择你偏好的身份验证方法
2. **在 CLI 中**：使用 `/auth` 命令重新配置身份验证方法
3. **环境变量**：配置 `.env` 文件以实现 API Key 自动身份验证

有关详细说明，请参阅 [身份验证配置](../configuration/settings#environment-variables-for-api-access) 文档。
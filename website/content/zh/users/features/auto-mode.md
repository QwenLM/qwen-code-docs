...
当分类器阻止某个操作时，工具调用会失败并显示：

- **`Blocked by auto mode policy: <reason>`** — 分类器判定该操作不安全。原因来自分类器的阶段 2。

此消息后跟着一行指导说明，告诉 agent **被拒绝的具体操作**绝不能通过其他工具、Shell 间接调用、生成的脚本、别名、符号链接、配置更改、钩子、命令文件、MCP 配置、编码的有效载荷或等效路径来完成。**不相关的安全工作和真正更安全的替代方案仍然被允许**——只有试图通过不同途径完成相同被拒绝意图的尝试才会被阻止。
...
- 如果分类器 API 无法访问、超时、超出上下文窗口或返回无效响应，当前操作会立即回退到手动审批。确认提示推荐使用 Default Mode，并提供 **Switch to Default Mode and allow once** 以及 Allow once 和 Reject 选项。切换仅影响当前运行时会话；不会更改你保存的设置。

- 在**连续 3 次策略阻止**后，下一次工具调用将回退到标准的手动审批提示。这可以捕获 agent 不断尝试被禁止命令的微小变体的情况。
- 在**连续 2 次不可用**结果（分类器 API 故障）后，后续调用会跳过已知故障的分类器，直接进入手动审批。第一次不可用结果就会提示；此阈值避免了反复等待分类器重试。

会话保持在 Auto Mode，除非你显式选择切换选项。只有回退调用会经过手动审批。当你批准回退调用或切换模式时，计数器会重置。

如果你发现自己经常触发回退，最可能的原因是分类器 API 中断或 hints 需要调整。在排查问题时，请切换到 Default Mode。

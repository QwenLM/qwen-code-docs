...
| `/compress` | 用摘要替换聊天历史以节省 Tokens                          | `/compress`                           |
| `/resume`   | 恢复之前的对话会话                                       | `/resume`                             |
| `/restore`  | 将文件恢复到工具执行前的状态                             | `/restore` (list) 或 `/restore <ID>` |
...
| `<project>/git/commit.toml`  | `/git:commit`     | `/git:commit 消息`    |
...
| 字段          | 必需   | 描述                                     | 示例                                       |
| ------------- | ------ | ---------------------------------------- | ------------------------------------------ |
| `prompt`      | 必需   | 发送给模型的提示内容                     | `prompt = "请分析代码: {{args}}"`          |
...
示例：Git Commit Message 生成

```

# git/commit.toml
description = "根据暂存的更改生成 Commit message"
prompt = """
请根据以下 diff 生成一个 Commit message：
diff
!{git diff --staged}
"""
```

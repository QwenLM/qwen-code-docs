...
本文档详细说明了 Qwen Code 支持的所有命令，帮助你高效管理会话、自定义界面并控制其行为。
...
| `/context` | 显示上下文窗口使用明细 | `/context` |
| → `detail` | 显示各项上下文使用明细 | `/context detail` |
| `/history` | 控制历史显示偏好和可见性 | `/history collapse-on-resume`, `/history expand-on-resume`, `/history expand-now` |
| `/diff` | 打开交互式 diff 查看器，显示未提交的更改和每轮对话的 diff。使用 ←/→ 在当前 git diff 和各个对话轮次之间切换，使用 ↑/↓ 浏览文件 | `/diff` |
| `/theme` | 更改 Qwen Code 视觉主题 | `/theme` |
| `/vim` | 开启/关闭输入区的 Vim 编辑模式 | `/vim` |
...
| `/terminal-setup` | 配置终端多行输入的快捷键绑定 | `/terminal-setup` |
...
- 可用的内置 UI 语言：`zh-CN`（简体中文）、`en-US`（英语）、`ru-RU`（俄语）、`de-DE`（德语）、`ja-JP`（日语）、`pt-BR`（巴西葡萄牙语）、`fr-FR`（法语）、`ca-ES`（加泰罗尼亚语）
...
| → `auto` | 分类器评估审批（自主模式） | `/approval-mode auto` |
...
| `/model --vision` | 设置视觉桥接模型，用于为纯文本主模型转录图像 | `/model --vision <model-id>` |
| `/effort` | 设置具备思考能力模型的推理努力程度 | `/effort`（打开选择器），`/effort high`（low/medium/high/xhigh/max；根据提供商进行映射和限制） |
...
| `/forget` | 从自动记忆中移除匹配项 | `/forget <query>` |
...
| `/tasks` | 列出后台任务 | `/tasks` |
| `/workflows` | 检查 workflow 运行状态 | `/workflows`, `/workflows <runId>` |
| `/lsp` | 显示 LSP 服务器状态 | `/lsp` |
| `/trust` | 管理文件夹信任设置 | `/trust` |

> [!warning]
>
> 请仅从你信任的来源安装扩展（`/extensions install`）。扩展可以捆绑 MCP 服务器、skill 和命令，它们以与 Qwen Code 本身相同的权限运行——可以访问你的文件、API key 和对话数据。`/extensions install` 不会提示确认。

> [!warning]
>
> `auto-edit`、`auto` 和 `yolo` 审批模式会绕过工具执行的审批提示。在 `yolo` 模式下，所有操作——包括 shell 命令、文件写入和网络请求——都会在未经确认的情况下运行。请仅在受信任、沙盒化或一次性的环境中使用这些模式。
...
| `/loop` | 按定期计划运行提示 | `/loop 5m check the build` |
| `/simplify` | 审查近期更改并直接应用安全的清理编辑 | `/simplify`, `/simplify focus on duplication` |
| `/qc-helper` | 解答有关 Qwen Code 使用和配置的问题 | `/qc-helper how do I configure MCP?` |
...
| 模式                 | 行为                                     |
| -------------------- | -------------------------------------------- |
| 交互模式          | 在上方显示 Composer 并进行 Markdown 渲染 |
| 非交互模式      | 返回文本结果：`btw> question\nanswer` |
| ACP (Agent Protocol) | 返回 stream_messages 异步生成器      |

> [!tip]
>
> 当你需要快速获得答案而不想打断主任务时，请使用 `/btw`。它在澄清概念、核实事实或在专注于主要工作流时获取快速解释方面特别有用。

### 1.7 会话回顾 (`/recap`)

`/recap` 命令会生成当前会话的简短“上次中断时的进度”摘要，以便你可以恢复旧对话，而无需翻阅大量历史记录。
...
- 使用配置的快速模型（`fastModel` 设置）（如果可用），否则回退到主会话模型。对于回顾来说，一个小巧、低成本的模型就足够了。
- 最近的对话（最多 30 条消息，仅限文本——工具调用和工具响应会被过滤掉）会连同精简的系统提示词一起发送给模型。
- 回顾内容会以暗色显示，并带有 `❯` 前缀，以便与真实的助手回复区分开来。
- 如果有模型回合正在进行或另一个命令正在处理，则会以内联错误拒绝。如果没有可用的对话，或者底层生成失败，`/recap` 会显示一条简短的信息提示而不是回顾内容——手动命令始终会返回某些内容。
...
❯ 重构 loopDetectionService.ts 以解决由无界的 streamContentHistory 和 contentStats 引起的长时间会话 OOM 问题。下一步是实现选项 B（带有 FNV-1a 的 LRU 滑动窗口），等待确认。
```

> [!tip]
>
> 通过 `/model --fast <model>`（例如 `qwen3-coder-flash`）配置快速模型，使 `/recap` 更快且成本更低。将 `general.showSessionRecap` 设置为 `true` 以启用自动触发；手动 `/recap` 命令始终有效，不受此设置影响。

### 1.8 Diff 查看器 (`/diff`)

`/diff` 命令打开一个交互式 diff 查看器，显示未提交的更改和每轮对话的 diff。使用 ←/→ 在当前 git diff 和各个对话回合之间切换，使用 ↑/↓ 浏览文件，按 Enter 查看内联 diff。
...
- **T1, T2, T3, …** — 每轮 diff，每个修改了文件的模型回合对应一个标签页。最近的回合排在最前面。每个标签页显示原始提示词的预览以提供上下文。

文件列表显示每个文件的统计信息（增加/删除的行数），并为特殊状态（`new`、`deleted`、`untracked`、`binary`、`truncated`、`oversized`）添加标签。在文件上按 Enter 可查看其带有语法高亮代码块的内联 diff。
...
在无头（`--prompt`）或非交互上下文中，`/diff` 会打印工作树与 HEAD 对比的纯文本摘要。不支持每轮导航。
...
| `/stats`         | 打开交互式使用统计仪表板（会话、活动和效率选项卡）                                       | `/stats` 或 `/usage`                                                                |
...
| `/config`        | 通过点路径键获取或设置任何设置（写入用户设置）                                                              | `/config`（列出所有），`/config <key>`，`/config <key>=<value>`                      |
...
| → `cpu-profile`  | 记录 CPU 配置文件以用于 Chrome DevTools 分析                                                                              | `/doctor cpu-profile [--duration <seconds>]`                                        |
| → `rollback`     | 将独立 CLI 二进制文件回滚到上一版本（仅限独立安装；对话历史请使用 `/rewind`） | `/doctor rollback`                                                                  |
...
> [!note]
>
> `/config` 通过点路径键读取和写入单个设置（例如 `general.vimMode`），作为交互式 `/settings` 编辑器的补充。不带参数（或 `--help`）运行 `/config` 会列出每个可设置的键及其类型和当前值。`/config <key>` 打印当前值——但对于布尔键，它会切换该值。`/config <key>=<value>` 设置该值。更改会写入用户设置（`~/.qwen/settings.json`）。只有 `boolean`、`string`、`number` 和 `enum` 设置可以通过这种方式更改——`array` 和 `object` 设置必须直接在 `settings.json` 中编辑。敏感值（API 密钥、令牌、基础 URL）在输出中会被掩码处理，并且禁止将 `tools.approvalMode` 设置为 `yolo`。
...
> [!note]
>
> 自定义命令现在使用 Markdown 格式，并带有可选的 YAML frontmatter。TOML 格式已弃用，但为了向后兼容仍受支持。当检测到 TOML 文件时，将显示自动迁移提示。
...
| 提示词主体   | 必需 | 发送给模型的提示词内容             | frontmatter 之后的任何 Markdown 内容 |
### TOML 文件格式（已弃用）
...
| 字段          | 必填 | 描述                               | 示例                                       |
| ------------- | ---- | ---------------------------------- | ------------------------------------------ |
| `prompt`      | 必填 | 发送给模型的 prompt 内容           | `prompt = "Please analyze code: {{args}}"` |
| `description` | 选填 | 命令描述（在 /help 中显示）        | `description = "Code analysis tool"`       |
...
“还有更多会话”的提示信息通过 stderr 输出，因此通过管道传递给 `jq` 仍然是安全的。

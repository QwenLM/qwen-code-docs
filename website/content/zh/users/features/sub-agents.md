...
在交互式 TUI 和 ACP 会话中，来自后台代理、shell、监控和工作流的完成通知共享一个队列，该队列在会话空闲时排入模型轮次。这些队列最多容纳 20 条通知，因此高产出者不会积累无限积压。无头 CLI 的本地队列不受此规则限制。

当第 21 条通知到达时，Qwen Code 会优先驱逐临时监控心跳 — 监控的下一次轮询会取代它 — 否则驱逐最旧的排队通知。代理结果、工作流结果和定时提示在交互式 TUI 中永远不会被驱逐；如果一条通知会 displacing 其中任何一个，则该通知被丢弃，当只有终端结果排队时，到达的心跳也会被丢弃。

被丢弃的通知会被报告而不是静默丢弃。摘要出现在实时转录中的下一条通知之前。ACP 还会将其前缀添加到该轮次的模型输入中；TUI 将其停放在下一个通知批次中，以便 cron 提示仍然不变地通过斜杠、shell 和 `@` 预处理。守护程序通知在被确认之前被记录，因此在重载后，其持久记录可以出现在后续的溢出摘要之前。如果会话被清除或切换，或者客户端取消或抢占通知轮次，ACP 可以丢弃待处理的摘要。丢弃通知永远不会停止或删除其任务，已完成的任务保留其结果；当有可检查的任务时，摘要指向 `/tasks` 和任务输出文件。被丢弃的定时提示从未被传递，也不会重试。已记录但无法实时传递的守护程序通知仍会在会话转录中可用，并与丢失的通知分开报告。

## 代理工作目录

对于命名的常规子代理，`working_dir` 将代理固定到当前仓库中的现有 git worktree。相对路径从当前目录解析，worktree 必须已向 git 注册并位于仓库内部。

`working_dir` 不能与 `subagent_type: "fork"` 组合。未命名的调用者拥有的 `working_dir` 启动在前台运行，因为 Qwen Code 不拥有该 worktree 的生命周期：显式的 `run_in_background: true` 请求会被拒绝，而配置的后台默认值（子代理定义中的 `background: true`）在顶级会被拒绝，在嵌套时会被降级为前台运行。如果同时提供 `working_dir` 和 `isolation: "worktree"`，Qwen Code 会重用调用者拥有的 worktree，而不是创建新的。Workflow 脚本故意更严格：同时收到 `workingDir` 和 `isolation` 的 workflow `agent()` 调用会被拒绝，而不是在忽略 `isolation` 的情况下运行。
...
- **敏感信息**：避免在子代理配置中包含密钥或凭据。
- **生产环境**：考虑为生产环境与开发环境分别设置不同的子代理。

## 限制

以下是对子代理配置的软性警告（不强制执行硬性限制）：

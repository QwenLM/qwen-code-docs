...
> [!tip]
>
> 如果你在寻找“快速上手的一条命令”，请直接跳转到 [快速开始](#quick-start)。

## 快速开始

Qwen Code 会从 `settings.json` 中的 `mcpServers` 字段加载 MCP 服务器。你可以通过以下任一方式配置服务器：
...
> [!tip]
>
> 如需了解高级配置层（系统默认值/系统设置及优先级规则），请参阅 [设置](../configuration/settings)。

## 配置服务器

### 选择传输方式

| Transport | 适用场景                                                       | JSON 字段                               |
| --------- | ----------------------------------------------------------------- | ------------------------------------------- |
| `http`    | 推荐用于远程服务；适用于云端 MCP 服务器 | `httpUrl`（+ 可选的 `headers`）            |
...
两种方法都会在 `settings.json` 中生成相同的 `mcpServers` 条目——选择你习惯的方式即可。
...
| `timeout`              | number<br>(default: 600,000) | 请求超时时间（毫秒）（默认：600,000ms = 10 分钟）                                                                                                                                                                                                 |
| `trust`                | boolean<br>(default: false)  | 设为 `true` 时，跳过该服务器的所有工具调用确认提示（默认：`false`）                                                                                                                                                                              |
| `includeTools`         | array                        | 从此 MCP 服务器包含的工具名称列表。指定后，仅此处列出的工具对该服务器可用（白名单行为）。若未指定，默认启用该服务器的所有工具。                                       |
| `excludeTools`         | array                        | 从此 MCP 服务器排除的工具名称列表。列出的工具将不对模型可用，即使服务器已暴露它们。<br>注意：`excludeTools` 优先级高于 `includeTools`——如果工具同时出现在两个列表中，它将被排除。 |
| `targetAudience`       | string                       | 你尝试访问的 IAP 保护应用上列入白名单的 OAuth Client ID。与 `authProviderType: 'service_account_impersonation'` 配合使用。                                                                                                         |
...
| `--include-tools`   | 要包含的工具的逗号分隔列表。                         | 包含所有工具 | `--include-tools mytool,othertool`        |
| `--exclude-tools`   | 要排除的工具的逗号分隔列表。                         | 无               | `--exclude-tools mytool`                  |

...
- 使用 `isolation: "worktree"` 的代理遵循默认后台行为；清理在代理报告完成时运行。设置 `run_in_background: false` 以获取内联结果。未命名的调用者拥有的 `working_dir` 启动在前台运行；显式的后台执行会被拒绝，而配置的后台执行（子代理定义中的 `background: true`）在顶级会被拒绝，在嵌套时会被降级为前台运行，因为其生命周期由外部管理。

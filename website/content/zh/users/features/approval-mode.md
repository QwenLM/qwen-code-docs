...
| -------------- | --------------------------- | --------------------------- | ------------------------------------------------------------------------------------------------------ | ---------- |
| **Plan**​      | ❌ 仅只读分析               | ❌ 不执行                   | • 代码探索 <br>• 规划复杂变更 <br>• 安全的代码审查                                                     | 最低       |
| **Default**​   | ✅ 需要手动批准             | ✅ 需要手动批准             | • 新的/不熟悉的代码库 <br>• 关键系统 <br>• 团队协作 <br>• 学习与教学                                   | 低         |
| **Auto-Edit**​ | ✅ 自动批准                 | ❌ 需要手动批准             | • 日常开发任务 <br>• 重构和代码改进 <br>• 安全的自动化                                                 | 中等       |
| **YOLO**​      | ✅ 自动批准                 | ✅ 自动批准                 | • 可信的个人项目 <br>• 自动化脚本/CI/CD <br>• 批量处理任务                                              | 最高       |

### 快速参考指南

- **以计划模式开始**：在进行更改前，非常适合用于理解项目
- **在默认模式下工作**：这是大多数开发工作的平衡选择
- **切换到自动编辑模式**：当你需要进行大量安全的代码更改时
- **谨慎使用YOLO模式**：仅适用于受信任的自动化操作，并且要在受控环境中

> [!tip]
>
> 在会话期间，你可以通过 **Shift+Tab** 快速切换模式。终端状态栏会显示你当前所处的模式，因此你始终知道Qwen Code拥有哪些权限。

## 1. 使用计划模式进行安全的代码分析

计划模式指示Qwen Code通过**只读**操作来分析代码库并制定计划，非常适合用于探索代码库、规划复杂变更或安全地审查代码。
...
```
我需要将我们的认证系统重构为使用 OAuth2。创建一个详细的迁移计划。
```

Qwen Code 会分析当前实现并制定全面的计划。通过以下后续问题进行优化：
...
默认模式是与 Qwen Code 协作的标准方式。在此模式下，你可以完全控制所有潜在风险操作——Qwen Code 在进行任何文件更改或执行 Shell 命令之前都会请求你的批准。

### 何时使用默认模式

- **初接触代码库时**：当你正在探索一个不熟悉的项目并希望格外谨慎时
- **关键系统**：当处理生产代码、基础设施或敏感数据时
- **学习与教学**：当你希望了解 Qwen Code 执行的每一步时
- **团队协作**：当多人共同处理同一代码库时
...
你可以使用 **Shift+Tab** 在会话期间切换权限模式。如果你当前处于其他模式，按下 **Shift+Tab** 最终会循环回到默认模式，此时终端底部将不再显示任何模式指示器。

**以默认模式启动新会话**

默认模式是启动 Qwen Code 时的初始模式。如果你已更改模式并希望返回默认模式，可以使用以下命令：
...
在运行无头命令时，默认模式是默认行为。你也可以显式指定它：

```
qwen --prompt "分析此代码中潜在的错误"
```
...
```
我需要在我们的应用中添加用户头像。这些图片应该存储在一个 S3 存储桶中，并将 URL 保存到数据库。
```

Qwen Code 将分析你的代码库并提出一个计划。然后它会在以下操作之前请求批准：

...
`$QWEN_HOME` 默认为 `~/.qwen`。文件名即为写入者的 PID，不包含其他内容；如果记录的 `pid` 字段与其文件名不一致，则该记录会被忽略。
...
- **守护进程管理的会话。** 目前只有交互式 UI 进行注册，因此 `qwen serve` 驱动的会话不在注册表中、无法被寻址、也无法发送。`serve` 和 `headless` 类型为其保留。

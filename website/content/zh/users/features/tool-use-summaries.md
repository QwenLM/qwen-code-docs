...
Qwen Code 可以在每个工具批次完成后生成一个简短的、类似 git 提交主题的标签，概括该批次完成的工作。该标签内联显示：对于主视图中已完成的工具组，它替换通用的 `Tool × N` 标题；当组被强制展开时（在 `Ctrl+O` 展开详情模式下，或对于错误/用户发起的批次），它显示为组下方的一行暗淡的 `● <label>`。
...
在主转录中，已完成的可折叠批次折叠为单个带标签的行——摘要替换通用的 `Tool × N` 标题：
...
完整的逐工具输出只需按一个键：按 `Ctrl+O` 切换展开详情模式。

### 展开详情模式（`Ctrl+O`）和强制展开的组

当一个组被强制展开时——在 `Ctrl+O` 展开详情模式下，或在主视图中的错误/用户发起的批次——每个工具单独渲染，摘要显示为组下方的一行暗淡徽章：
...
工具批次最终确定后，Qwen Code 会向配置的 fast 模型发起一次fire-and-forget 调用，内容包括：
...
标签是使用 [fast model](./followup-suggestions#fast-model)生成的——与你为提示建议和投机执行配置的模型相同。通过以下方式配置：
...
1. **每个批次一次生成，两种显示模式共享。** Fast 模型调用在 `handleCompletedTools` 中恰好发生一次，当工具批次最终确定时。之后切换 `Ctrl+O` **不会**触发新的调用——两种模式都读取最初捕获的同一个 `tool_use_summary` 历史条目。2. **切换或恢复会话时不会回填。** 在功能启用之前（或在你打开设置之前，或在恢复的会话中——`ChatRecordingService` 不会持久化摘要条目）完成的 `tool_group` 永远不会获得标签。没有“扫描现有历史”的步骤。如果在会话中途打开此设置，只有**未来**的批次会显示标签；较旧的组保持默认渲染，没有任何指示表明缺少标签。
3. **仅限主代理批次。** 触发点位于主会话的轮次循环（`useLlmStream`）中，因此：
...
主视图已将已完成的可折叠批次折叠为单个带标签的行（`✓  Read 4 text files`）——摘要承担了旧的逐工具列表的工作。要查看完整的逐工具详情，按 `Ctrl+O` 切换展开详情模式，每个工具单独渲染，摘要作为组下方的尾部 `● <label>` 行。
...
对于小型同类型批次（例如 `Read × 3`），展开的 `● <label>` 行可能读起来像对可见工具行的重述；如果这符合你通常的工作流程，你可以通过 `experimental.emitToolUseSummaries: false` 完全关闭摘要。
...
- [展开详情模式](../configuration/settings#ui) — 按 `Ctrl+O` 内联展开所有工具输出；摘要在主视图中替换已完成组的通用工具组标题。

...
已完成的群组 @ 或私聊回复在首次发送前会被检查点标记，并在后台重试而无需重新运行任务。退避从五秒开始，翻倍增长，上限为五分钟。回复在 16 次失败的投递尝试后被放弃。由频道断开或配对审批不可读引起的本地延迟使用单独的 32 次连续检查限制，因此不会消耗传输尝试预算。当发送者、群组或私聊授权被明确撤销时，回复会立即被丢弃。队列最多容纳 100 条回复，满时会驱逐最旧的条目，无论其尝试计数如何；切换配置的 DWS 配置文件会清除队列。这些丢弃会在频道 stderr 上报告，不会作为聊天通知发送。

超过 12,000 个代码点的排队 IM 回复会被截断，并在收件人看到的消息末尾追加 `[Response truncated for DWS delivery.]`。文档评论和原生待办评论回复不使用这个排队重试或截断路径：每个轮次进行一次评论发送调用，未知结果会被吞下，明确的失败会传播到现有的入站轮次重试策略。结束反应标记任务完成，而不是回复投递，所以重试的 IM 回复可能在结束反应出现之后才到达。

...
频道从该通知中提取文档 ID、评论键和请求内容。它会读取被引用的文档作为上下文，在任务运行时添加钉钉的 `暗中观察` 眼睛反应，并在原始文档评论下回复。当实时 DWS 事件流包含该卡片时使用实时流；每五秒的增量历史检查覆盖当前事件流遗漏的卡片。
...
进行本地验证时，从另一个账号发送私聊消息，如需要则批准配对，并验证任务运行时眼睛反应是否出现。然后添加一条启用了 @提及通知的文档评论。频道应该响应通知消息、读取文档，并在原始评论下发布最终答案。禁用通知的评论不应产生任何任务。

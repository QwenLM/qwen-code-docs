...
> [!note]
>
> 此行为适用于标准无头 CLI 运行。ACP 驱动的会话（IDE 集成、Web Shell）通过 `sessionGoalControl` 扩展方法驱动相同的 Goal 运行时，并以携带 `_meta.goalState` 的 `session/update` 通知接收每次状态变更，而非 `goal_state` 流事件。
...
使用 `--output-style` 为本次运行选择一种[输出风格](./output-styles)——内置或自定义。风格是叠加在内置 prompt 上的命名指令块，用于改变回答的撰写方式——`Concise` 以结果开头并省略前言和叙述，`Proactive` 直接开始工作而非提出建议，`Explanatory` 在此过程中添加关于代码库的简短说明。它会覆盖 `general.outputStyle` 设置；`default` 表示不选择任何风格。
...
| `--output-style`             | 本次运行的输出风格（`Concise`、`Proactive`、`Explanatory`、`Learning`、自定义风格名称，或 `default` 表示无）；覆盖 `general.outputStyle`                                                                                                                                                                                                                                                                                                | `qwen -p "query" --output-style Concise`                                 |

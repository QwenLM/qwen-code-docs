...
> [!warning]
>
> **同一认证类型内的重复模型 ID：** 目前不支持在同一 `authType` 下定义多个具有相同 `id` 的模型（例如，在 `openai` 中有两个条目都使用 `"id": "gpt-4o"`）。如果存在重复项，**第一个出现的获胜**，后续的重复项会被跳过并显示警告。请注意，`id` 字段既用作配置标识符，也用作发送到 API 的实际模型名称，因此使用唯一 ID（例如 `gpt-4o-creative`、`gpt-4o-balanced`）并不是可行的解决方法。这是一个已知限制，我们计划在未来的版本中解决。
...
| `qwen-oauth` | Qwen OAuth（硬编码，无法在 `modelProviders` 中被覆盖）                                   |
...
| `qwen-oauth`           | [`openai`](https://www.npmjs.com/package/openai) 配合自定义提供商 (DashScope 兼容)            |
...
大多数本地推理服务器（vLLM、Ollama、LM Studio 等）提供 OpenAI 兼容的 API 端点。使用 `openai` 认证类型和本地 `baseUrl` 来配置它们：
...
对于不需要认证的本地服务器，你可以为 API 密钥使用任何占位符值：
...
百炼编码方案提供了一组为编码任务优化的预配置 Qwen 模型。此功能适用于拥有百炼 API 访问权限的用户，并提供了简化的设置体验以及自动模型配置更新。

### 概述

当你使用 `/auth` 命令通过百炼代码计划 API 密钥进行身份验证时，Qwen Code 会自动配置以下模型：
...
> ```
>
> 然后确保如果使用项目级设置，此文件已添加到你的 `.gitignore` 中。
...
如果你希望手动配置 Coding Plan 模型，可以像任何兼容 OpenAI 的提供商一样将它们添加到你的 `settings.json` 中：
...
有效的 auth/model/credential 值是按字段选择的，遵循以下优先级（首先出现的生效）。你可以将 `--auth-type` 与 `--model` 结合使用，直接指向提供程序条目；这些 CLI 标志在其他层级之前运行。
...
> [!warning]
> 
> **弃用 `security.auth.apiKey` 和 `security.auth.baseUrl`：** 不推荐通过 `settings.json` 中的 `security.auth.apiKey` 和 `security.auth.baseUrl` 直接配置 API 凭据。这些设置在历史版本中用于通过 UI 输入的凭据，但凭据输入流程已在 0.10.1 版本中移除。这些字段将在未来版本中完全移除。**强烈建议迁移到 `modelProviders`** 来进行所有模型和凭据配置。在 `modelProviders` 中使用 `envKey` 引用环境变量，以安全地管理凭据，而不是在设置文件中硬编码凭据。

## 生成配置分层：不可渗透的提供者层

配置解析遵循严格的分层模型，其中有一个关键规则：**modelProvider 层是不可渗透的**。

### 工作原理

1. **当选择了 modelProvider 模型时**（例如，通过 `/model` 命令选择一个提供程序配置的模型）：
   - 提供程序的整个 `generationConfig` 被**原子性地**应用
   - **提供程序层完全不可穿透** —— 较低层级（CLI、环境变量、设置）完全不参与 generationConfig 解析
   - 在 `modelProviders[].generationConfig` 中定义的所有字段都使用提供程序的值
   - 提供程序**未定义的**所有字段都被设置为 `undefined`（不会从设置中继承）
...
- `samplingParams.max_tokens` = **undefined**（提供者中未定义，且提供者层不会从设置继承 — 未提供的字段会被明确设置为 undefined）
...
- 具有完整、原子的配置包
- 选择后，其配置作为不可渗透的层应用
- 在 `/model` 命令列表中显示，带有完整的元数据（名称、描述、功能）
- 推荐用于多模型工作流和团队一致性
...
- **使用提供者模型（Provider Models）**：当你在团队中共享标准模型、需要一致的配置或希望防止意外覆盖时
- **使用运行时模型（Runtime Models）**：当快速测试新模型、使用临时凭证或处理临时端点时
...
- 没有 `modelProviders` 时，解析器会混合 CLI/env/settings 层，创建运行时模型。这对于单提供程序设置来说是可以的，但在频繁切换时会很麻烦。当多模型工作流程很常见时，请定义提供程序目录，以便切换保持原子性、源归因和可调试性。
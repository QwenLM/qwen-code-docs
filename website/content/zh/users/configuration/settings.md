...
| `model.goalCheckpointTimeoutSeconds`               | integer | 单次 Goal 证据检查点调用的上限时间，单位为秒。长时间运行的 Goal 会定期通过侧模型调用将其证据压缩为检查点声明；未能在时限内完成的调用会被放弃作为不确定的检查——检查点停滞条纹被保留而非递增——后续轮次会重试。未设置时使用内置默认值 180。必须是 1 到 900 之间的整数；其他值会在启动时被拒绝。该调用是流式的，因此每请求的传输超时（`model.generationConfig.timeout`，默认 120 秒）仅约束连接和首次响应，超过 900 的值会被拒绝，因为超过默认的流生命周期守卫后，是那个守卫而非此设置结束调用。900 的上限是固定的：提高 `QWEN_STREAM_MAX_LIFETIME_MS` 不会提升它。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | `180`       |

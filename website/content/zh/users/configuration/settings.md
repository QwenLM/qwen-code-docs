...
| `model.goalCheckpointTimeoutSeconds`               | integer | 单次 Goal 证据检查点调用的上限时间，单位为秒。长时间运行的 Goal 会定期通过侧模型调用将其证据压缩为检查点声明。声明超出总字节预算或包含超出每声明字符限制的调用会进行一次纠正重试，两次调用共享此上限。未能在时限内完成的调用会被放弃作为不确定的检查；仅在证据窗口溢出时计入检查点停滞限制，而未溢出的检查保留连胜并在后续轮次重试。未设置时使用内置默认值 180。必须是 1 到 900 之间的整数；其他值会在启动时被拒绝。该调用是流式的，因此每请求的传输超时（`model.generationConfig.timeout`，默认 120 秒）仅约束连接和首次响应，超过 900 的值会被拒绝，因为超过默认的流生命周期守卫后，是那个守卫而非此设置结束调用。900 的上限是固定的：提高 `QWEN_STREAM_MAX_LIFETIME_MS` 不会提升它。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | `180`       |
...
| `tools.core`    | `tools.eager` (+ `permissions.deny`) | 不会自动迁移到 `permissions.allow`，后者是纯自动批准，无法重现白名单限制（#10075）。`tools.eager` 延迟未列出的默认 eager 工具（它们仍可通过 `tool_search` 加载）；`permissions.deny` 完全移除工具。两者都无法保留非空 `tools.core` 白名单对核心工具集的 fail-closed 保证：未来版本中添加的内置工具会注册直到被显式拒绝，因此 deny 列表必须在每个版本重新审计。空 `tools.core` 列表视为未设置，不禁用任何工具。 |
...
`allowDynamicHeaderValues` 只是同意开关。_哪些_主机接收该值以及_ header 名称是什么_由 header 所在位置决定——`modelProviders[].generationConfig.customHeaders`，请参阅 [`customHeaders` 中的动态值](./model-providers.md#dynamic-values-in-customheaders)。
...
设置 `QWEN_UPDATE_BASE_URL` 以使用自定义 HTTPS 发布根目录进行独立更新：
...
URL 必须使用 HTTPS，且不能包含凭据、查询字符串或片段。周围的空白和尾部斜杠会被移除。空或仅包含空白的值会保留内置的下载源及其回退顺序。当设置了自定义根目录时，失败的下载不会回退到内置源。

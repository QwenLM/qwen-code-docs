...
- `.qwen/skills/` 下的 [Skill](../features/skills)（每个 skill 是一个包含 `SKILL.md` 的目录）。
...
| `general.outputStyle` | string | 输出样式的名称，决定响应的撰写方式：内置样式（`Concise`、`Proactive`、`Explanatory`、`Learning`）或[自定义样式](../features/output-styles#custom-styles)的名称——即其 frontmatter 中的 `name`，默认为去掉 `.md` 的文件名（不区分大小写）。留空或设置为 `default` 使用默认样式。`--output-style` 可在单次运行中覆盖它。在会话中使用 `/output-style` 更改，该操作也会持久化选择；手动编辑此文件在下次启动时生效。在 `--bare` 和 `--safe-mode` 中被忽略。请参阅[输出样式](../features/output-styles)。 | `undefined` |
...
| `review.severityFloor` | enum | 未指定 `--severity-floor` 时 PR `/review` 发布的最低严重级别：`"auto"`（轮次自适应默认值——建议通过第 5 轮发布，从第 6 轮起仅发布 Critical，否则可发布的高置信度建议以及在新表面上 fail closed 的 Critical 会被记录并延迟，第 2-5 轮延迟自上一轮以来未更改的代码上的新建议；低置信度和 Nice-to-have 的发现仅限终端），`"critical"`（从第 1 轮开始采用该策略），或 `"suggestion"`（每轮都发布建议；关闭收敛策略）。非 PR 目标没有轮次，会忽略此设置。 | `"auto"` |
...
| `ui.brand.name`                       | string           | Web Shell 在其侧边栏、欢迎标题、关于面板和浏览器标签页标题中展示的产品名称。被清理为单行并限制为 80 个字符。仅从操作员作用域（User、System、SystemDefaults）读取——工作区 `.qwen/settings.json` 无法更改 shell 的品牌。会被替换改变的占位符值（`$VAR`/`${VAR}` 且变量已设置）会被拒绝，并在 daemon 的 stderr 上输出警告，因为替换源是进程范围的，而工作区可以提供它；无法解析的占位符会原样保留，因此拼写错误的变量会显示为字面文本而不是静默回退。无法从浏览器内的设置页面编辑。终端横幅有自己独立的设置 `ui.customBannerTitle`。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | `""`          |
| `ui.brand.logoPath`                   | string           | 用作 Web Shell 侧边栏徽标和浏览器 favicon 的 SVG 文件路径。前导 `~` 会被展开，相对路径相对于声明它的设置文件所在目录解析。必须是常规文件（不是符号链接，且不能通过多个硬链接到达），磁盘上和 UTF-8 解码后均不超过 32 KiB，其根元素是带命名空间的 `<svg>`（`xmlns` 属性，或前缀绑定根上的 `xmlns:svg` 绑定，使其可作为图像渲染）。会解析的环境变量占位符会被拒绝，与 `ui.brand.name` 相同。仅从操作员作用域读取。被拒绝的文件会在 daemon 的 stderr 上记录警告并回退到内置徽标。没有可用 `viewBox` 或显式正宽度/高度的根 `<svg>` 会被接受但会记录提示信息，前缀绑定根其无前缀元素没有默认命名空间绑定的情况也是如此，因为浏览器可能在侧边栏的固定大小下渲染为空白。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | `""`          |
...
| `ui.customAsciiArt`                   | string \| object | 替换横幅中的 QWEN ASCII 徽标。接受内联字符串（同时用于两个宽度层级）、`{ "path": "./brand.txt" }`（相对路径相对于所属设置文件的目录解析；在 POSIX 上使用 `O_NOFOLLOW` 在启动时读取一次，上限为 64 KB），或 `{ "small": ..., "large": ... }` 以进行宽度感知选择。经过清理；每个层级上限为 200 行 × 200 列。                                                                                                                                                                | `undefined`   |
...
| `ui.compactMode`                      | boolean          | **已全面退役。** CLI 现在始终在主转录中显示紧凑的、基于类型的工具视图；按 `Ctrl+O` 切换展开详情模式（展开或折叠所有思考块和工具输出内联），而不是切换模式，Web shell 现在也固定为紧凑视图。保留此键仅为避免现有设置文件产生警告；接受写入但没有代码读取该值。                                                                                                                                                                                                                                                                                                                                                             | `false`       |
| `ui.showToolCallDetails`                | boolean          | 内联显示工具参数和结果。设置为 `false` 可将普通工具调用渲染为单行摘要。点击虚拟化历史记录中的行或按 `Ctrl+O` 可展开详情。审批提示、用户发起的 shell 命令和聚焦的交互式 shell 保持展开。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | `true`        |
| `ui.showToolCallArgs`                   | boolean          | 将工具调用渲染在单独的行上，并内联显示其完整的原始参数，而不是将读取/搜索/列表批次折叠为 `Read 3 files` 的基于类型的紧凑摘要。恢复按工具描述所省略的参数（例如 `Edit` 通常只显示文件名）。在调试 MCP 集成或工具 schema 时非常有用。参数行上限为 2 行换行（最多 1000 个字符），因此待处理调用的批次不会超出终端；按 `Ctrl+O` 可取消上限并同时展开结果输出。两种情况保持紧凑视图：运行中的并行子代理组（由实时代理名册管理——展开它们会使实时帧重新膨胀到超过终端高度 (#5798)）以及 daemon 附加的会话（不会跨 daemon 边界携带参数）。在这些情况下请使用 `Ctrl+O`。仅限 TUI——Web shell 不受影响。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | `false`       |
...
| `ui.compactInline`                    | boolean          | **已移除。** 与 `ui.compactMode` 一同退役——紧凑视图现在在 TUI 和 Web shell 中始终开启。旧设置会被静默忽略（无启动警告）。                                                                                                                                                                                                                                                                                                                                                                                                                                                               | `N/A`         |
| `ui.useTerminalBuffer`                | boolean          | 在应用内的可滚动视口中渲染对话历史记录，而不是使用终端的回滚缓冲区。在兼容的交互式终端中默认启用，以避免长会话中以及 Ctrl+O 后、Ctrl+E / Ctrl+F（展开）后、窗口大小调整后或 alt-tab 切回时的闪烁、滚动风暴和界面冻结。屏幕阅读器模式以及非交互式输出（如管道 stdout 或 CI）改用仅追加的终端输出。使用 `Shift+↑/↓`（行）、`PgUp`/`PgDn`（页）、`Ctrl+Home/End`（顶部/底部）或鼠标滚轮进行滚动。同时启用鼠标交互：点击菜单/对话框中的选项以选择它，悬停以高亮它，点击输入框以定位光标。启用时不使用主机终端的回滚缓冲区。拖动以选择视口中的文本（双击/三击选择词/行），释放时复制。要使用终端自身的选择，请在拖动时按住 Shift（在 macOS 上为 Option）。单击打开指针下的 http(s) 超链接（其他链接 scheme 会被复制到剪贴板），在链接或文本选择上右键会打开应用内上下文菜单。这些鼠标交互由 `ui.mouseTracking` 控制；禁用该设置可将鼠标完全交还给终端。                                                                                                                                | `true`        |
| `ui.showScrollbar`                    | boolean          | 在应用内的可滚动视口（虚拟化历史）中显示自动隐藏的滚动条。滚动时出现，空闲时淡出。禁用以完全隐藏。仅适用于交互式终端 UI。                                                                                                                                                                                                                                                                                                                                                                                                              | `true`        |
| `ui.mouseTracking`                    | boolean          | 启用应用内的 SGR 鼠标追踪，用于文本选择、文本输入中的点击定位、行悬停、历史项切换和视口滚动。启用时，终端将所有鼠标事件转发给应用，因此 Qwen Code 提供了终端无法原生执行的功能的等效替代：单击打开指针下的 http(s) 超链接（其他链接 scheme 会被复制到剪贴板），在链接或文本选择上右键会打开包含打开链接 / 复制链接地址 / 复制选择内容的应用内上下文菜单。禁用以将鼠标完全交还给终端（原生右键菜单和链接点击）；这会关闭所有应用内鼠标交互，并且在虚拟化历史中滚轮不再滚动转录——请改用 Shift+↑/↓、PgUp/PgDn 或 Ctrl+Home/End（配合 `ui.useTerminalBuffer: false` 以恢复原生终端回滚缓冲区）。仅适用于交互式终端 UI。                                                                                                    | `true`        |
...
| `model.goalTokenBudget`                            | integer | 每个新 Goal 启动时的自主花费窗口，以 Goal 计量器计算的 token 数为单位（`totalTokenCount` 对 Goal 在其自身轮次中进行的模型调用求和）。花费完窗口的 Goal 会获得一个收尾轮次进行交接，然后停止直到你恢复它，这会启动另一个窗口。未设置时使用内置默认值 30,000,000；`-1` 表示无限制。零、超过 300,000,000（默认值的 10 倍，防输入错误）的值、其他负数、小数或非数字值会在启动时被拒绝。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | `30000000`  |
| `model.goalCheckpointTimeoutSeconds`               | integer | 单次 Goal 证据检查点检查的上限时间，单位为秒。长时间运行的 Goal 会定期通过侧模型调用将其证据压缩为检查点声明；当检查进行一次纠正重试时，两次调用共享此上限（[Goals](../features/goals.md) 列出了哪些失败可获得一次重试）。未能在时限内完成的检查会被放弃并视为不确定；仅在证据窗口溢出时计入检查点停滞限制，而未溢出的检查保留连胜并在后续轮次重试。未设置时使用内置默认值 180。必须是 1 到 900 之间的整数；其他值会在启动时被拒绝。该调用是流式的，因此每请求的传输超时（`model.generationConfig.timeout`，默认 120 秒）仅约束连接和首次响应，超过 900 的值会被拒绝，因为超过默认的流生命周期守卫后，是那个守卫而非此设置结束调用。900 的上限是固定的：提高 `QWEN_STREAM_MAX_LIFETIME_MS` 不会提升它。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | `180`       |
| `model.goalMaxTurns`                               | integer | 每个新 Goal 启动时武装的 Goal 轮次窗口。每个已完成的 Goal 轮次都计入，包括用户驱动的轮次；用户轮次在达到上限时仍会被准入，但它们可能使下一次自主续接变成收尾轮次。达到上限的 Goal 会获得一个收尾轮次进行交接，然后作为用量受限停止，直到你恢复它，这会在已完成的轮次之上授权另一个窗口。未设置时 Goal 没有轮次上限，`-1` 明确表示如此——但退出仅对已花费上限的 Goal 生效，在随后的恢复或编辑时；仍在上限内的 Goal 保持上限。上限仅对在更改后创建的 Goal 武装，因此限制已记录的 Goal 意味着用 `/goal set` 替换它，这会在修订版 1 启动一个新 Goal，其计量器重置且其早期证据不再可引用，或者清除它并重新开始。零、超过 10,000 的值、其他负数、小数或非数字值会在启动时被拒绝。更改在重启后生效。 | (none)      |
| `model.goalMaxActiveMinutes`                       | integer | 每个新 Goal 启动时武装的活跃时间窗口，以 Goal 保持活跃期间的实际时间分钟数为单位，包括轮次间的等待和空闲时间。暂停、阻塞或停止的时间不计入，重启期间的停机时间也不计入；挂起的进程仍会被计费。达到上限的 Goal 会获得一个收尾轮次进行交接，然后作为用量受限停止，直到你恢复它，这会授权另一个从停止处开始计算的窗口。上限在轮次之间读取，而非通过计时器，因此 Goal 可能在停止前远超上限。活跃时间在记录的转换之间测量，因此被重启中断的轮次中的时间不计费。未设置时 Goal 没有时间上限，`-1` 明确表示如此——但退出仅对已花费上限的 Goal 生效，在随后的恢复或编辑时。上限仅对在更改后创建的 Goal 武装，因此限制已记录的 Goal 意味着用 `/goal set` 替换它，这会在修订版 1 启动一个新 Goal，其计量器重置且其早期证据不再可引用，或者清除它并重新开始。零、超过 10,080（一周）的值、其他负数、小数或非数字值会在启动时被拒绝。更改在重启后生效。                        | (none)      |
...
| `model.generationConfig`                           | object  | 传递给底层内容生成器的高级覆盖配置。支持请求控制，如 `timeout`、`maxRetries`、`retryInitialDelayMs`、`retryMaxDelayMs`、`enableCacheControl`、`splitToolMedia`（默认 `true`；将工具返回的媒体——包括内置 read_file 读取的图像——拆分为后续的用户消息，而不是违反规范的 `role: "tool"` 消息，以便 doubao / new-api / LM Studio 等严格的 OpenAI 兼容服务器能够识别；设置为 `false` 可恢复旧版嵌入工具的行为）、`toolResultContentFormat`（默认 `"parts"`；仅当旧版 OpenAI 兼容运行时的工具模板忽略文本内容部分时，才设置为 `"string"`）、`contextWindowSize`（覆盖模型的上下文窗口大小）、`modalities`（覆盖自动检测的输入模态）、`customHeaders`（API 请求的自定义 HTTP 标头）和 `extra_body`（OpenAI 兼容和 OpenAI Responses API 请求的额外 body 参数），以及 `samplingParams` 下的微调旋钮（例如 `temperature`、`top_p`、`max_tokens`）。留空则依赖提供商默认值。 | `undefined` |
...
**流式传输守卫（OpenAI 兼容和 Anthropic 提供商）：**

两个守卫约束流式响应，均可设置为 `0` 以禁用。Gemini 生成器未实现这两个守卫，因此下方的滴注式流式传输对 Gemini 模型没有上限。
...
`extra_body` 字段允许向发送到 API 的请求体中添加自定义参数。这对于标准配置字段未涵盖的特定于提供商的选项非常有用。**注意：此字段支持兼容 OpenAI 的提供商（`openai`、`qwen-oauth`）和 OpenAI Responses API（`openai-responses`）。对于 Anthropic 和 Gemini 提供商，此字段将被忽略。** 在 `openai-responses` 协议中，旧版的 `enable_thinking` 键会被转换为 `reasoning.effort` 而非原样转发——对于该提供商，请直接使用 `reasoning.effort`。对于提供商模型，请在 `modelProviders[].generationConfig.extra_body` 中定义 `extra_body`。对于没有匹配提供商条目的运行时模型，请在 `model.generationConfig.extra_body` 中定义。
...
| 设置 | 类型 | 描述 | 默认值 |
| --- | --- | --- | --- |
| `modelPricing` | object | 用于在 `/stats model` 中进行成本估算的可选按模型定价。示例：`{ "qwen3-coder": { "inputPerMillionTokens": 0.30, "outputPerMillionTokens": 1.20 } }`。 | `undefined` |
...
如果你在文件搜索（例如 `@` 补全）时遇到性能问题，尤其是在包含大量文件的项目中，可以按推荐顺序尝试以下解决方法：
...
| `tools.core` | string 数组 | **已弃用。** 将在下一版本中移除。非空列表将核心工具集（file、shell、search 及相关内置工具）限制为白名单：不在列表中的核心工具会被禁用（fail-closed）。该集合之外的工具——动态发现的工具（MCP、skill）和合成/系统内置工具（如 `agent`、`list_agents`、plan 模式生命周期工具、goal 工具、`task_stop`、`send_message` 和 `tool_search`）——设计上会绕过白名单；使用 `permissions.deny` 阻止工具的调用（对于 MCP 工具，它仍会列出但在运行时被拒绝），或使用 `tools.disabled` / 每服务器的 `excludeTools` 过滤器将其从注册表中彻底移除。空列表（`[]`）视为未设置，不禁用任何工具。`permissions.allow` 无法重现此限制——它是纯自动批准（#10075）。使用 `tools.eager` 限制初始发送哪些默认 eager 的工具 schema（未列出的工具被延迟而非禁用——它们仍可通过 `tool_search` 加载），使用 `permissions.deny` 彻底阻止工具。 | `undefined` | |
| `tools.exclude` | string 数组 | **已弃用。** 请改用 `permissions.deny`。要从发现中排除的工具名称。不会自动迁移；旧版设置在启动时仍然有效。 | `undefined` | |
...
| `tools.eager` | string 数组 | 默认 eager 的内置工具名称白名单，其 schema 仍有资格用于初始模型请求。未列出的非豁免工具会被延迟：仍然注册、列在 `/tools` 中、可调用，并可通过 `tool_search` 发现。默认已延迟的工具即使被列出也仍然是按需加载；使用 `tools.visible` 可以在启动时显示某个工具。`tool_search`、`structured_output`、plan 模式生命周期工具、`task_stop`、MCP 工具和 `computer_use__*` 工具不受影响，保持正常的加载行为。显式空列表（`[]`）是活跃的，会延迟每个非豁免的默认 eager 工具；省略该设置表示无限制。与 `tool_search` 配合使用：当 ToolSearch 未注册时（`tools.toolSearch.enabled: false`、`tool_search` deny 规则、或 DeepSeek 模型的自动退出），白名单仍然保留 schema，但没有东西可以加载它们，因此被降级的工具在该会话中不可达（它们仍列在 `/tools` 中，并会记录警告）。两个例外：恢复会话历史中引用的被降级工具会重新发送其 schema 而不显示警告，列在 `tools.visible` 中的被降级工具会提前声明。如果你想要移除它们，请使用 `permissions.deny`，或保持 ToolSearch 开启。不可用的条目（空或格式错误）会被丢弃并显示警告，其余列表保持活跃。后续作用域替换早期列表。需要重启。 | `undefined` | |
| `tools.allowed` | string 数组 | **已弃用。** 请改用 `permissions.allow`。绕过确认对话框的工具名称。不会自动迁移；旧版设置在启动时仍然有效。 | `undefined` | |
...
| `tools.discoveryCommand` | string | 用于工具发现的运行命令。当 `tools.eager` 白名单激活时，未在其中命名的已发现工具会被注册为延迟工具：它仍列在 `/tools` 中并可通过 `tool_search` 加载，但其 schema 不会在初始模型请求中发送。 | `undefined` | |
...
| `tools.toolSearch.threshold` | number | 用作会话启动预算的上下文窗口百分比，用于预加载普通延迟工具（内置和 MCP 均包括）。当它们的组合 schema 在此预算内时，会提前声明它们，而不是通过 ToolSearch 按需加载。被 `tools.eager` 降级的工具不在此预加载范围内，仍然按需加载。设置为 `0` 则始终按需加载延迟工具。 | `10` | 需要重启：是 |
| `tools.listDirectory.enabled` | boolean | 启用内置 `list_directory` 工具。默认禁用，因为 `glob` 在大多数情况下已涵盖目录列表；当在 `coreTools` 允许列表中显式列出时（`--core-tools` / `tools.core`），该工具也会自动重新启用。 | `false` | 需要重启：是 |
| `tools.todoWrite.enabled`            | boolean           | 启用内置 `todo_write` 工具及其 system-prompt 指导。默认禁用。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | `false`     |                                                                                                                                                                                                                                                                                                                             |
> [!note]
>
> **从 `tools.core` / `tools.exclude` / `tools.allowed` 迁移：** 这些旧版设置已**弃用**，但不会自动迁移；它们在启动时继续有效。手动将 `tools.allowed` 和 `tools.exclude` 迁移到 `permissions.allow` 和 `permissions.deny`。`tools.core` 没有精确的替代；见下表。
...
| 设置 | 类型 | 描述 | 默认值 |
| --- | --- | --- | --- |
| `agents.builtin.exploreModel` | string | 内置 Explore 子代理的模型选择器。使用 `inherit` 继承主会话模型，`fast` 使用 `fastModel`，或模型 ID，或 `authType:model-id` 选择器。自定义的同名 Explore agent 保留其自身的模型配置。需要重启。 | `inherit` |
...
| `agents.crossSessionInbound` | enum | 入站跨会话消息的处理方式：`accept` 投递消息，`hold` 暂存消息供 `/peers` 审查而不让模型处理，`refuse` 让此会话退出。未设置时表示[用户创建的控制器](../features/commands.md#trusted-controllers)和此会话自身的子进程自动投递，而其他会话使用[审查级别对等](../features/commands.md#6-messaging-another-running-session)；其他消息被暂存以待审查。工作区只能收紧此设置（`hold` 或 `refuse`，当比操作员设置的值或未设置时的默认值更严格时）；有效的未识别值会暂存所有消息。 | `undefined` |
...
| 设置项             | 类型             | 描述                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | 默认值     |
| ------------------- | ---------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------- |
| `permissions.allow` | array of strings | 自动批准的工具调用规则（无需确认）。在所有作用域（用户 + 项目 + 系统）中合并。此键是纯自动批准：它永远不会移除、降级或隐藏工具，每个内置工具都保持注册，无论规则覆盖哪些工具；默认延迟的工具（如 `task_stop` 或 `monitor`）保持其通常的通过 `tool_search` 按需加载（#10075）。要将工具的 schema 排除在初始模型请求之外，请使用 `tools.eager`；要彻底阻止工具，请使用整工具 `permissions.deny` 规则——MCP 工具不受基于 deny 的移除影响（参见 `permissions.deny` 行）：请使用每服务器的 `excludeTools` / `tools.disabled` 过滤器来隐藏它们。例外：在 AUTO 审批模式下，危险的 allow 规则会被暂存而非激活，因此会话中途的移除无法触及暂存——退出 AUTO 模式时暂存的规则会被恢复并继续自动批准，直到会话重启。 | `undefined` |
| `permissions.ask`   | array of strings | 始终需要用户确认的工具调用规则。优先级高于 `allow`。与 `allow` 一样，这不会影响工具是否被注册。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | `undefined` |
...
| --------------- | ------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `tools.allowed` | `permissions.allow`                  | 不会自动迁移；在启动时仍然有效                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| `tools.exclude` | `permissions.deny`                   | 不会自动迁移；在启动时仍然有效                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| `tools.core`    | `tools.eager` (+ `permissions.deny`) | 不会自动迁移到 `permissions.allow`，后者是纯自动批准，无法重现白名单限制（#10075）。`tools.eager` 延迟未列出的默认 eager 工具（它们仍可通过 `tool_search` 加载）；`permissions.deny` 从注册表中完全移除内置工具（MCP 工具保持列出但在运行时被拒绝——使用 `tools.disabled` / 每服务器的 `excludeTools` 过滤器来彻底移除它们）。两者都无法保留非空 `tools.core` 白名单对核心工具集的 fail-closed 保证：未来版本中添加的内置工具会注册直到被显式拒绝，因此 deny 列表必须在每个版本重新审计。空 `tools.core` 列表视为未设置，不禁用任何工具。 |
...
| 设置项                  | 类型             | 描述                                                                                                                                                                                                                                                                                                                 | 默认值     |
| ------------------------ | ---------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------- |
| `slashCommands.disabled` | array of strings | 要隐藏并拒绝执行的 slash command 名称。与最终命令名称进行不区分大小写的匹配（对于扩展命令，这是消歧后的形式，例如 `myext.deploy`），但 [Skill](../features/skills) 命令在**任意一种**拼写形式下都会被门控——其注册名称（`rust:pdf`）或其 `SKILL.md` 作者使用的名称（`pdf`）——因此在该前缀存在之前编写的条目仍然会门控它。**在各作用域中作为并集合并**，因此工作区设置可以添加但不能移除用户或系统设置中定义的条目。 | `undefined` |
...
| `skills.disabled` | array of strings | 硬性禁用的 skill 名称。不区分大小写匹配，**在各设置作用域中作为并集合并**，因此项目设置无法覆盖用户或系统条目。隐藏的 skill 不会出现在 `<available_skills>` 中，也不会作为 `/<name>` slash commands 出现。扩展 skill 在任意一种拼写形式下都会被匹配——其注册名称（`rust:pdf`）或其 `SKILL.md` 作者使用的名称（`pdf`）——因此在该前缀存在之前编写的条目仍然会生效。 | `undefined` |
| `skills.defaultDisabled` | array of strings | 默认禁用但可通过 `skills.enabled` 启用的 skill 名称。不区分大小写匹配，在各设置作用域中作为并集合并。 | `undefined` |
| `skills.enabled` | array of strings | 显式启用。覆盖匹配的 `skills.defaultDisabled` 条目，对于扩展 skill，还会覆盖所属扩展的自身默认值和该工作区为其存储的启用状态。不区分大小写匹配，在各设置作用域中作为并集合并——仅针对 skill 的注册名称，因此扩展 skill 需要 `rust:pdf`：单独的 `pdf` 永远不会作为授权匹配——它只会取消拼写完全相同的 `skills.defaultDisabled` 条目（根据下方的相同拼写规则），一旦取消，该工作区存储的启用状态将决定，否则使用所属扩展的自身默认值。此设置无法覆盖 `skills.disabled` 或重新启用被 `skills.disabledLevels` 排除的层级中的 skill。 | `undefined` |
...
每个列表包含字面 skill 名称，在修剪后不区分大小写匹配，不支持 glob。`skills.enabled` 仅在两个条目拼写完全相同时才会取消 `skills.defaultDisabled` 条目，因为该步骤比较的是条目本身而不是将它们解析为 skill——因此必须同时取消默认禁用和显式启用的扩展 skill 在两个列表中都写作 `rust:pdf`。请参阅[扩展 Skills](../features/skills#extension-skills) 了解为什么上面两个列表接受任意一种拼写而这个列表不接受。
...
| 设置 | 类型 | 描述 | 默认值 |
| --- | --- | --- | --- |
| `experimental.sessionWorkflow` | boolean | 显示 daemon Web Shell 会话工作流 DAG，并将现有的 `plan` 审批模式展示为 **Plan & Review**。这仅改变展示方式：不会添加审批模式、改变 Todo 执行行为或调度依赖项。更改无需重启即可生效。 | `false` |
...
| `experimental.todoStopGuard` | boolean | 当当前 work chain 成功写入了未完成的顶级 Todo 列表时，允许 daemon 和 ACP 会话在模型自然停止后继续。需要 `tools.todoWrite.enabled`。最多添加两次连续的 primary-model 调用（无新用户输入）；轮次中的用户输入会开启新的两次尝试阶段。进程重启后不会恢复，在 safe、bare 和 Approval `plan` 模式下强制关闭。需要重启。 | `false` |
...
| `telemetry.logPrompts`                      | boolean | 是否在日志中包含用户提示词内容以及 API 请求/响应文本。                                                                                                                                                                                                                       |           |
...
⚠️ **与安全相关。** 控制 Qwen Code 在出站 LLM API 请求中写入哪些客户端关联数据——这是一个独立于 `telemetry.*` 的同意决策，后者管理流入你自己可观测性后端的数据。所有值默认关闭。

| 设置                                           | 类型    | 描述                                                                                                                                                      | 默认值  |
| ---------------------------------------------- | ------- | --------------------------------------------------------------------------------------------------------------------------------------------------------- | ------- |
| `outboundCorrelation.propagateTraceContext`    | boolean | 在出站 `fetch` 请求上注入 W3C `traceparent`，并在 shell 子进程中作为 `TRACEPARENT` 环境变量。需要 `telemetry.enabled: true`。              | `false` |
| `outboundCorrelation.allowDynamicHeaderValues` | boolean | 允许 `customHeaders` 值包含运行时占位符（如 `${session_id}`），按请求展开。关闭时，此类值会被丢弃而非发送。 | `false` |
...
`allowDynamicHeaderValues` 只是同意开关。_哪些_主机接收该值以及_该 header 的名称_由 header 所在位置决定——`modelProviders[].generationConfig.customHeaders`，请参阅 [`customHeaders` 中的动态值](./model-providers.md#dynamic-values-in-customheaders)。
这种作用域划分就是为什么这里没有主机白名单：你在编写提供商的 `baseUrl` 时已经选择了端点，不应该发送该 header 的提供商直接不携带它。

如果提供商条目有占位符而此设置为关闭，header 会被丢弃，并在启动时打印一个命名两者的警告——因此拒绝你请求的网关不应是一个静默的谜团。

**隐私说明：** 展开的值是一个稳定的每会话标识符。只在你已经发送 prompt 内容的提供商上启用它。
...
| `QWEN_TELEMETRY_INCLUDE_SENSITIVE_SPAN_ATTRIBUTES`   | 设置为 `true` 或 `1` 以将原样的用户提示词、系统提示词、工具 I/O 和模型响应附加到原生 OTel span 属性（并在 log-to-span 桥接 span 上保留 `function_args`、`error`、`error.message` 和 `error_message`，以及当 `telemetry.logPrompts` 也启用时的 `prompt` / `request_text` / `response_text`）。任何其他值均禁用此功能。                                                                                                                                                             | 覆盖 `telemetry.includeSensitiveSpanAttributes` 设置。⚠️ 会将敏感数据流式传输到你的 OTLP 后端。                                                                                                                                                                                                                                                                                                                                                                  |
...
| `SEATBELT_PROFILE`                                   | （macOS 特定）切换 macOS 上的 Seatbelt (`sandbox-exec`) 配置文件。                                                                                                                                                                                                                                                                                                                                        | `permissive-open`：（默认）限制对项目文件夹（及其他几个文件夹，请参阅 `packages/cli/src/serve/sandbox-macos-permissive-open.sb`）的写入，但允许其他操作。`strict`：使用默认拒绝操作的严格配置文件。`<profile_name>`：使用自定义配置文件。要定义自定义配置文件，请在项目的 `.qwen/` 目录中创建一个名为 `sandbox-macos-<profile_name>.sb` 的文件（例如，`my-project/.qwen/sandbox-macos-custom.sb`）。 |
...
设置 `QWEN_UPDATE_BASE_URL` 以使用自定义 HTTPS 发布根 URL 进行独立更新：
...
对于版本 `0.23.0`，更新器会从 `<base-url>/v0.23.0/` 下载平台归档文件、`SHA256SUMS` 和 `SHA256SUMS.sig`。请使用与官方发布相同的文件名和目录结构来托管这些文件。现有的校验和和签名检查仍然适用，包括在需要签名时的 `QWEN_REQUIRE_SIGNATURE=1`。

URL 必须使用 HTTPS，且不能包含凭据、查询字符串或片段。周围的空白和尾部斜杠会被移除。空或仅包含空白的值会保留内置的下载源及其回退顺序。当设置了自定义根 URL 时，失败的下载不会回退到内置源。

在启动 shell 或用户级 `.env` 文件中配置此变量。它会从项目 `.env` 和 `.qwen/.env` 文件以及所有作用域的顶层 `settings.json` `env` 部分中被拒绝。用户级 `.env` 值在启动时加载；更改后请重启 Qwen Code。

此设置适用于 `qwen update`、`/update` 和自动独立更新。它不会更改 npm 注册表的版本发现。它独立于安装器的 `QWEN_INSTALL_BASE_URL`，后者直接指向版本特定的目录。
...
| `--prompt` | `-p` | 用于直接向命令传递 prompt。这将以非交互模式调用 Qwen Code。 | 你的 prompt 文本 | 如需脚本示例，请使用 `--output-format json` 标志以获取结构化输出。 |
...
| `--output-style`             |       | 选择本次运行中决定响应撰写方式的输出样式。                                                                                                                                                                                                                                 | 样式名称                                       | `Concise`、`Proactive`、`Explanatory`、`Learning`、[自定义样式](../features/output-styles#custom-styles)的名称，或 `default` 表示无样式（不区分大小写）。覆盖 `general.outputStyle` 设置。未知名称会打印警告并以默认样式启动会话。当 `--system-prompt` 或 `QWEN_SYSTEM_MD` 替换内置 prompt 时无效。示例请参见 [Headless Mode](../features/headless)。                                                                                                                                                                                                           |
...
| `--disabled-slash-commands` | | 要隐藏/禁用的 slash command 名称（逗号分隔或重复）。与 `slashCommands.disabled` 设置和 `QWEN_DISABLED_SLASH_COMMANDS` 环境变量合并。与最终命令名称进行不区分大小写的匹配，Skill 命令同样适用两种拼写规则。 | 命令名称 | 示例：`qwen --disabled-slash-commands "auth,mcp,extensions"` |
...
| `--screen-reader` | | 启用屏幕阅读器模式，该模式会调整 TUI 以更好地兼容屏幕阅读器。 | | |
| `--version` | | 显示 CLI 的版本。 | | |
| `--openai-logging` | | 启用 OpenAI API 调用的日志记录，用于调试和分析。 | | |
| `--openai-logging-dir` | | 设置 OpenAI API 日志的自定义目录路径。 | 目录路径 | 此标志会覆盖 `settings.json` 中的 `enableOpenAILogging` 设置。 |
## 上下文文件（分层指令上下文）

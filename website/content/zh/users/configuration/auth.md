...
- **要求**：需要从 [阿里云百炼](https://bailian.console.aliyun.com/cn-beijing/?tab=globalset#/efm/coding_plan) 购买有效的编码计划订阅。
...
有关 Coding Plan 的更多详情，包括订阅选项和故障排除，请参阅[完整的 Coding Plan 文档](https://bailian.console.aliyun.com/cn-beijing/?tab=doc#/doc/?type=model&url=3005961)。
...
Qwen Code 将自动从找到的**第一个** `.env` 文件加载环境变量（变量**不会**在多个文件间合并）。
...
通过环境变量使用 OpenAI 兼容的 API 方法：

- 至少设置 `OPENAI_API_KEY`。
- 可选地设置 `OPENAI_BASE_URL` 和 `OPENAI_MODEL`。

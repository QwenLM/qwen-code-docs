...
Criar, editar ou retomar um Goal requer um workspace confiável (`/trust`). Uma sessão gravada antes de o estado do Goal ser registrado em diário (versões do Qwen Code anteriores a 29-07-2026) é retomada sem Goal: os cartões `/goal` em seu histórico são mostrados como histórico, `/goal` reporta que nenhum Goal está definido, e `/goal set` o inicia novamente. No Web Shell e outros clientes daemon, um cartão final após o histórico reproduzido diz que o Goal antigo não está sendo executado. O uso headless é coberto em [Modo Headless](./headless.md#run-a-persistent-goal).

Assim que um Goal fatura um turno, a pill do rodapé e cada cartão de status mostram o que ele gastou em relação à janela permitida, como `1.2k/30.0m`. O número conta as chamadas de modelo nos próprios turnos do Goal, seus subagentes diretos em foreground e suas verificações do verificador. Subagentes aninhados e em background, consultas laterais não relacionadas, turnos de cron e turnos de notificação são excluídos. Uma verificação falha que não retorna resultado utilizável não pode ser contada. Restaurar uso já registrado não o conta novamente. A janela é definida por [`model.goalTokenBudget`](../configuration/settings.md); retomar um Goal que gastou sua janela concede outra por cima do que já foi gasto, então o número lê `30.0m/60.0m` em vez de recomeçar. Um Goal sem orçamento mostra apenas o que gastou. Um Goal que ainda não faturou um turno não mostra números.

Quando orçamentos de turno ou tempo ativo estão configurados, os cartões de status também mostram o uso correspondente em relação ao seu orçamento, por exemplo `3/20 turns` e `12m 3s/30m`. O texto headless escreve isso como `3 of 20 turns` e `12m 3s of 30m active`. Uso zero permanece oculto, e Goals pausados ou interrompidos retêm esses números.

Mais duas janelas estão disponíveis e desativadas por padrão: [`model.goalMaxTurns`](../configuration/settings.md) limita quantos turnos um Goal pode finalizar, incluindo turnos do Goal dirigidos pelo usuário, e [`model.goalMaxActiveMinutes`](../configuration/settings.md) limita o tempo de parede enquanto ele permanece ativo em um processo em execução, incluindo esperas e tempo ocioso entre turnos. O tempo enquanto o Goal está pausado, bloqueado ou interrompido não conta para a segunda, nem o tempo de inatividade durante uma reinicialização; um processo meramente suspenso ainda é cobrado. Qualquer que seja a janela que acabar primeiro, o Goal recebe o mesmo único turno de encerramento para passar o trabalho e então para até você retomá-lo, e retomar autoriza outra janela por cima do que já foi usado. Apenas a janela que acabou se move; as outras permanecem onde estavam, e a opção `-1` para desistir igualmente apenas remove um teto de um Goal que já o gastou. A janela de tempo é lida entre turnos em vez de aplicada por um timer, então um Goal pode executar bem além dela antes de parar: um turno já em execução nunca é interrompido, e uma janela que acaba enquanto nenhum turno está em execução não é percebida até o próximo turno terminar. O tempo ativo é contado entre transições registradas, então o tempo em um turno que uma reinicialização interrompeu não é cobrado. Alterações em qualquer configuração entram em vigor após reinicialização. Ambas as janelas armam apenas em um Goal criado após a alteração, então limitar um Goal já registrado significa substituí-lo com `/goal set` -- um novo Goal na revisão 1, com seus medidores de turno, token e tempo ativo reiniciados e sua janela de evidência começando na substituição, então a evidência que o Goal antigo registrou não é mais citável -- ou limpá-lo e começar novamente. Uma retomada ou edição nunca adiciona um teto que não foi armado na criação.
...
Goals não comprimem mais evidência registrada em afirmações de checkpoint entre turnos: o verificador lê a transcrição diretamente, então não há nada para um checkpoint alimentar, e a verificação de modelo lateral que costumava executar no final de turnos longos, junto com seu limite de estagnação, não executa. A configuração `model.goalCheckpointTimeoutSeconds` está obsoleta: a chave ainda é aceita para que arquivos de configurações existentes carreguem, e seu valor é ignorado. A pill do rodapé, os cartões de status, a saída headless de `/goal`, a faixa de status do Web Shell e o diálogo de Goals, e `get_goal` não mostram mais uma linha de checkpoint ou contagem de estagnação, incluindo para uma sessão gravada por uma build que ainda executava checkpoints; o motivo de parada desse Goal ainda diz por que parou.
...
Cada pausa declara seu motivo: que você o interrompeu, que você executou `/goal pause`, que o limite de tokens da sessão bloqueou a próxima requisição ao modelo, que o turno falhou, que o verificador não pôde julgar uma proposta (expirou, sua chamada de modelo falhou, ou o objetivo não deixou espaço para evidência), ou que três turnos consecutivos não registraram nada que o verificador pudesse julgar e nenhuma proposta — leituras de contabilidade do Goal (`get_goal`, `update_goal`) não contam como progresso. Um Goal parado por um limite mantém o motivo desse limite.

## Como um Goal é julgado

O verificador nunca executa comandos nem lê arquivos por conta própria. Ele só vê o que já está na transcrição: os registros deste Goal, lidos do mais recente para trás até que a requisição esteja cheia (256.000 bytes, com qualquer registro único cortado no meio após 8.000). Um turno final curto deixa espaço para os turnos antes dele; um longo preenche a requisição por si só, e o verificador é informado de quantos registros mais antigos foram deixados de fora e rejeita uma proposta cuja prova possa estar entre eles:

- Saídas visíveis do assistente, resultados de ferramentas e suas próprias mensagens contam como evidência. O texto do objetivo e o raciocínio oculto do modelo não contam. Uma verificação executada muito antes da conclusão ser proposta pode ter saído de vista e precisa ser executada novamente.

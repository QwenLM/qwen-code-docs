...
| -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------- | ------------------------------------- | ---------------- |
| `low`    | 3-6 ângulos inline direcionados sobre o diff (escalados pelo tamanho do diff) mais um gap sweep e uma re-passagem under-floor — sem subagentes, sem build/teste, sem regras do projeto       | 10 (não verificados)  | Nenhum                                | Nunca            |
...
| `high`   | Pipeline completo: até 16 agentes paralelos → verificação em shards → auditoria reversa iterativa                                                                  | Ilimitado (verificado) | Approve / Request changes / Comment | Com `--comment` |

O `/review` resolve o esforço nesta ordem: um `--effort` explícito, o último nível digitado explicitamente para este projeto, a configuração `review.effort` do operador, e então o padrão embutido do alvo (**high** para revisões de PR, **medium** para revisões locais e de arquivo). Quando um nível memorizado se aplica, o `/review` o anuncia antes de começar o trabalho; digite um novo `--effort` para substituí-lo. Um `--comment` efetivo força o high (comentários publicados devem sobreviver à verificação) — em um alvo que não é PR, `--comment` é ignorado com um aviso e **não** altera o esforço. O medium mantém os agentes de segurança e cobertura de testes e o build/teste, e descarta as personas adversariais, os especialistas em armadilhas de linguagem e roteamento de wrapper/proxy (Agentes 1d/1e), os finders especializados e a auditoria reversa — então um Crítico sutil que apenas o segundo olhar revelaria pode passar; use `--effort high` para revisões sensíveis à segurança ou pré-release. Apenas o `low` é não verificado. O isolamento de worktree se aplica a revisões de PR no mesmo repositório; PRs entre repositórios são executados em modo leve (apenas diff, sem worktree ou build/teste). A passagem low é rotulada como não verificada, não emite veredito e nunca escreve no cache de revisão incremental, então uma execução posterior com `--effort high` nunca é ignorada como "já revisado"; o medium é verificado, mas seu Approve é limitado a Comment, porque nada olhou duas vezes para o que a primeira passagem deixou passar. A mecânica de obtenção do diff é idêntica em todos os níveis — revisões de PR sempre usam o worktree isolado e a mesma resolução de base, então a revisão nunca é contra a base errada. Uma diferença de escopo permanece: o cache incremental é apenas do high, então uma re-revisão high pode cobrir apenas os novos commits (`lastCommitSha..HEAD`), enquanto low/medium sempre revisam o diff completo do PR.
...
| Agente 1a: Varredura linha a linha  | Percorre cada hunk mais sua função envolvente: condições erradas, off-by-one, `await` faltando, casos de borda, race conditions                                                                                                                                                                                                                                            |
...
| Agente 1d: Varredura de armadilhas da linguagem | Carrega o checklist de armadilhas clássicas da linguagem do diff (`==` coerção, armadilhas de valores falsy, captura de variável de loop, defaults mutáveis, escritas em nil-map, concatenação SQL, aritmética DST) e faz pattern-match de cada hunk contra ele                                             |
| Agente 1e: Roteamento de wrapper/proxy | Para cada tipo que o diff adiciona ou modifica e envolve outro (cache, proxy, decorator, adapter): cada método roteia através da instância envolvida, e o wrapper encaminha todo método que os chamadores usam. Escalonado apenas quando o diff sinaliza um tipo de wrapping                                  |
...
| Agente 6: Auditoria Não Direcionada | 3 personas paralelas (atacante / plantão das 3am / mantenedor) — captura problemas entre dimensões — mais a auditoria counter-frame (6d, alvos de PR em esforço high): lê a descrição do PR apenas para EXCLUIR seus tópicos nomeados e re-executar seu incidente motivador no dia após o merge            |
...
| `prose-exec`: Execução de prosa     | Quando o diff toca um arquivo de instruções (um `SKILL.md`, `AGENTS.md`/`CLAUDE.md`/`QWEN.md`, uma definição de agente ou comando, um arquivo `prompts/`) e a revisão tem uma árvore: EXECUTA as instruções alteradas em uma cópia descartável e registra a lacuna entre a execução e o que a prosa promete  |

Os três agentes de Correção são **procedurais**: cada um é definido por como percorre o diff (linha a linha / linhas excluídas / arestas entre arquivos), não por uma taxonomia de bugs — então sua cobertura é complementar em vez de sobreposta. Dois ângulos dedicados adicionais (1d/1e) separam o checklist de armadilhas de linguagem e o roteamento de wrapper/proxy da caminhada linha a linha: um pattern-match de checklist e uma expectativa de roteamento estrutural são modos de atenção diferentes, e incorporados à caminhada eles foram diluídos pelo ritmo dela. O mesmo raciocínio divide a **qualidade de código em três** (3a/3b/3c): um agente segurando um checklist de seis itens finaliza um item — medido em um arquivo fortemente reescrito, um agente segurando um checklist de oito itens encontrou 1 de 5 defeitos e o mesmo modelo dividido em três partes encontrou todos os 5 — então o checklist de qualidade é cortado onde as perguntas genuinamente diferem. Todos os agentes executam em paralelo (o Agente 1 lança 3 variantes procedurais e 2 ângulos dedicados, o Agente 3 lança 3 fatias do checklist, e o Agente 6 lança 3 variantes de persona e a auditoria counter-frame 6d concorrentemente, totalizando até 17 tarefas paralelas para revisões de PR no mesmo repositório — o Agente 1e executa apenas quando o diff sinaliza um tipo de wrapping — mais 0-2 finders do Agente 8 quando o domínio do diff os exigir — então 16-19 na prática, e mais uma (`prose-exec`) quando o diff toca um arquivo de instruções; os Agentes 0 e 6d são ignorados em revisões de diff local e caminho de arquivo, que executam 14-17; o modo leve entre repositórios também ignora os Agentes 1c e 7, executando 14-17 com a identidade do PR e 12-15 sem ela).
...
- Etapas que **modificam** código para medir algo — os mutantes do probe de eficácia de testes e o probe de um achado específico pelo verificador — cada uma executa em seu próprio worktree descartável ao lado (`…-probe`, `…-scratch-<agent>`), para que o experimento de um agente não seja visível para os outros que leem a árvore compartilhada. Como salvaguarda, todo agente em cada wave também é informado de quais caminhos (se houver) diferem do commit em revisão no momento em que foi lançado, e que uma falha confinada a esses caminhos não é um achado. Todas essas árvores são varridas junto com o worktree no final da revisão.
...
| ------------------------------------------------------------------ | ----------------- | -------------------------------- |
| Revisão por LLM (Agentes 0, 1a, 1b, 1d, 1e, 2-6 + verificação + auditoria reversa iterativa) | ✅        | ✅                               |
...
Duas coisas para saber. Com apenas o padrão embutido do alvo, uma continuação mantém o **esforço** gravado da execução interrompida. Um `--effort` explícito, um nível memorizado pelo projeto, a configuração `review.effort` do operador, ou um `--comment` efetivo fornece um nível obrigatório; se diferir da execução interrompida, a retomada é recusada e uma nova execução começa nesse nível, porque esforço diferente é trabalho diferente. E se o head do PR se moveu enquanto a revisão estava fora, a retomada recusa (`head-moved`) e a execução nova revisa os novos commits — que é o que você quer, e conta como a única reinicialização desta revisão.
...
Aponte para um repositório onde você possa fazer push — um repositório dedicado para hospedar imagens é o destino recomendado; um fork ou repositório temporário também funciona. Evite o próprio repositório em revisão: branches de imagens enviados lá se tornam objetos alcançáveis que todo clone baixa. As imagens vão para o branch `pr-assets/<pr>-review` com nomes de hash de conteúdo, e os comentários as referenciam por URL com **pin de commit** — imutável mesmo que o branch se mova depois, e funcionando inalteradamente no GitHub Enterprise.

Para revisões disparadas pelo GitHub (o workflow de revisão de PR), a mesma variável é configurada a partir de uma **variável do repositório** com o mesmo nome, e o workflow passa apenas um host externo dedicado: com a variável não configurada, ou configurada para o repositório em revisão (comparado após trim e normalização de caixa, então grafias com padding ou caixa diferente do mesmo repositório contam como alvo próprio), o workflow passa um valor vazio e a publicação recusa — nada muda, e a revisão mantém suas evidências como prosa e caminhos de artefatos locais. Um mantenedor que configura `QWEN_REVIEW_ASSETS_REPO` nas variáveis de Actions do repositório para um repositório dedicado de hospedagem de imagens permite que comentários de revisão incorporem PNGs de captura. Um destino externo gerencia sua própria retenção; o workflow de limpeza de visuais apenas drena os branches históricos `pr-assets/*` que publicadores anteriores baseados em Git deixaram neste repositório.
...
A correspondência de modelo também controla o escopo incremental, não apenas o skip: "limpar até o commit em cache" é o veredito do modelo anterior, então quando novos commits chegaram desde a revisão em cache, uma incompatibilidade de modelo nunca faz escopo para `lastCommitSha..HEAD` — o range é o diff completo, notando "A rodada anterior foi revisada por qwen3-coder. Executando revisão completa com gpt-4o." — a menos que uma âncora certificada pelo modelo em execução seja recuperada da última revisão publicada (abaixo), que então define o range. Os achados da rodada anterior ainda são transportados para serem reavaliados; apenas a âncora não. O mesmo gate vincula a âncora recuperada do marcador de ledger da última revisão publicada quando o cache está ausente ou sua âncora é inutilizável (CI, outro clone): ela define o range incremental apenas se o modelo em execução a certificou — um marcador certificado por um modelo diferente, ou sem modelo (uma revisão publicada com `review.attribution` desativado, ou uma anterior ao campo), volta ao diff completo. Uma rodada que não fechou limpa publica seu marcador sem uma âncora (não pode certificar um range), mas essa perda não é persistente quando a lista de trabalho da rodada sobreviveu inteira: a recuperação avança a âncora a partir do marcador anterior mais recente que sua própria conta publicou com uma, então uma única rodada não limpa não força mais todas as rodadas posteriores a reler o diff completo — a próxima rodada faz escopo `anchor..HEAD`, que recupera o range que a rodada não limpa não conseguiu certificar. O enxerto de uma rodada com limite de tamanho é recusado (achados descartados cairiam fora do escopo enxertado e seriam aposentados silenciosamente), então rodadas posteriores continuam relendo o diff completo até que um marcador completo chegue.

O cache é armazenado em `.qwen/review-cache/` e rastreia tanto o SHA do commit quanto o ID do modelo. Certifique-se de que este diretório esteja no seu `.gitignore` (uma regra mais ampla como `.qwen/*` também funciona). No GitHub, se o commit em cache foi removido por um rebase ou force-push, o sistema volta a fazer uma revisão completa; o Aone julga a âncora em cache de forma diferente — veja seu parágrafo abaixo. Apenas revisões de esforço high consultam ou escrevem o cache — uma passagem rápida com `--effort low|medium` nunca conta como "já revisado".
...
**Aone Code:** para um clone cuja origem está em `gitlab.alibaba-inc.com`, execute o `/review` de dentro desse clone — a plataforma é detectada a partir do remote e os subcomandos funcionam, com suporte do CLI `a1` (pelo menos 0.1.90 — uma instalação mais antiga é recusada no momento da autenticação com uma mensagem de upgrade) — o número alvo é o id global do MR. O `fetch-pr` busca `refs/merge-requests/<id>/head` e constrói o worktree + diff, então a revisão por agentes do worktree não é alterada, e o `test-plan` também funciona — ele lê a descrição do MR através do mesmo reader. O `pr-context` também tem suporte: lê os metadados do MR, os threads de discussão e os resumos anteriores do qwen publicados (o machine ledger se recupera a partir deles), então uma execução Aone vê a discussão existente do MR exatamente como uma execução GitHub vê a de um PR. O `comment-status` e o `presubmit` também são suportados pelo `a1` (o `presubmit` completamente: detecção de PR próprio, drift do head, CI do merge-gate e deduplicação de comentários existentes), então rodadas repetidas de `--comment` deduplicam contra os comentários existentes do MR em vez de re-publicá-los (um thread que a plataforma marca como desatualizado — sua linha não mapeia mais após um amend — continua re-publicável), e a detecção de PR próprio também funciona. A escrita do `publish-assets` é ignorada. O `--comment` **publica** a revisão através do CLI `a1`: um comentário por achado inline, depois o comentário de resumo. O Aone não tem estado nativo de request-changes — nesse veredito, o comentário de resumo carrega um cabeçalho de bloqueio, e quaisquer Críticos inline que foram realmente publicados bloqueiam o merge através do gate de discussão enquanto suas discussões permanecem não resolvidas (quando nenhum Crítico inline é publicado, o cabeçalho é consultivo e nada bloqueia mecanicamente o merge). Os comentários publicados não carregam flag de comentário de IA — o `a1` não pode definir uma — então o gate de merge `ai_comment` dedicado do repositório não os rastreia. O `a1 repo mr approve` nativo dispara para um veredito Approve quando a execução leu o contexto do MR (o mesmo gate que o GitHub; uma execução sem contexto disponível permanece limitada a Comment). A re-revisão incremental segue o modelo de atualização AGit-Flow: uma atualização faz AMEND do único commit do CR no lugar, orfanizando o head que a rodada anterior revisou — então a âncora em cache é julgada SEM ascendência (o teste de âncora-antes-do-head falharia para toda atualização), e a re-revisão escopa o diff do PR para os arquivos que a atualização tocou em vez de voltar para uma revisão completa; uma atualização que também fez rebase para uma master mais recente mantém esse escopo apenas enquanto o drift do rebase permanecer nos arquivos do CR — drift tocando qualquer outro arquivo volta para a revisão completa, e nenhum byte de drift entra no escopo publicado de qualquer forma. Veja `docs/design/2026-08-15-review-aone-provider.md`.
...
| `candidateFloor`    | Quando o `low` deve uma re-passagem determinística de recall | `min(arquivos alterados, 4)`                                       |
...
Por que os pisos são onde estão: em uma correção de typo de nove linhas, seis caminhadas inline são cinco caminhadas sobre o nada, e o sweep — um leitor fresco buscando o que a primeira passagem não alcançou — não tem nada para buscar quando a primeira passagem alcançou tudo. Quando uma passagem de esforço low fica abaixo de `candidateFloor`, ela dá uma segunda olhada determinística em cada hunk cobrível no maior arquivo fonte alterado (recorrendo ao maior arquivo cobrível de qualquer tipo) e cada bloco removido cobrível. Arquivos tocando um chunk incobrível são excluídos da seleção de alvo, e a passagem sempre emite um recibo nomeando seu alvo, contagem de novos candidatos e quaisquer chunks incobríveis; quando nenhum arquivo é cobrível, ela ainda verifica os blocos removidos cobríveis e divulga o alvo faltante. O piso é um sinal de parada, não uma cota: um diff limpo ainda pode não reportar achados após essa re-passagem. O piso do Agente 8 é o substantivo: "um domínio domina o diff" é um julgamento, e um julgamento feito sobre quarenta linhas encontra um domínio dominante toda vez, porque quarenta linhas geralmente são uma coisa só.
...
| ------------------------------------ | ------------------------------ | --------------------------------------------------------------------------------------------------------------- |
| Agentes de revisão (Etapa 3)         | 17 (+0-2)                      | Executados em paralelo; Agente 1e apenas quando o diff sinaliza um tipo de wrapping (16 sem ele); entre repositórios ignora Agentes 1c e 7 (15 enquanto o plano carrega a identidade do PR, 13 sem ela — Agentes 0 e 6d caem juntos); local/arquivo ignora Agente 0 e a auditoria counter-frame 6d (15); mais uma (`prose-exec`) em um diff que toca um arquivo de instruções, quando a revisão tem uma árvore |
...
| **Total**                            | **~20-31 (~16-29)**            | 3A mesmo repositório: ~20-31 (típico ~20-22); entre repositórios ou local/arquivo: ~16-29 (o piso é o roster entre repositórios sem identidade de 13); um a menos quando o Agente 1e não está no roster, um a mais quando `prose-exec` é devido; 3B escala com chunks (ver DESIGN.md) |

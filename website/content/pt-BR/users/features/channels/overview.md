...
Este modo inicia processos de worker de canal agrupados por workspace pertencentes ao `qwen serve`. Os workers se conectam de volta ao daemon através do SDK e usam os mesmos adaptadores de canal. Eles são separados do processo do daemon, portanto, uma falha no adaptador de canal não derruba o daemon. Uma seleção explícita de `--channel` tem precedência e falha a inicialização do daemon se não puder ficar pronta. Em uma inicialização sem flags, a configuração `serve.channels` do workspace primário confiável é restaurada. Workspaces secundários não restauram independentemente suas próprias configurações `serve.channels`. Sem nenhuma das duas fontes, o daemon não carrega adaptadores de canal nem reserva a concessão até o primeiro `qwen channel set`. A restauração automática ignora configurações de inicialização inválidas e falhas de validação ou de concessão que ocorram antes dos workers iniciarem, preservando configurações não relacionadas. Após a falha de inicialização de um worker, o daemon continua somente quando a limpeza for bem-sucedida. Um timeout global de inicialização do runtime ou uma parada de worker não confirmada ainda seguem o caminho normal de falha de inicialização; a concessão do serviço permanece mantida enquanto a terminação do worker não estiver confirmada. Consulte o log do daemon para mensagens que identifiquem `serve.channels` quando um canal não for restaurado.

Os nomes de inicialização armazenados devem ser não vazios, não ter espaços em branco no início ou no fim, e não conter caracteres de controle ou invisíveis inseguros. Entradas inválidas são ignoradas individualmente e registradas pelo índice no array. A inicialização não renomeia instâncias nem reescreve a configuração. O toggle de inicialização do canal reflete a configuração salva; o estado de runtime mostra se o canal está em execução.

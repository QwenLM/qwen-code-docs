...
> [!note]
>
> Este comportamento se aplica a execuções headless padrão da CLI. Sessões conduzidas por ACP (integrações de IDE, Web Shell) conduzem o mesmo runtime do Goal através do método de extensão `sessionGoalControl`, e recebem cada mudança de estado como uma notificação `session/update` contendo `_meta.goalState` em vez de um evento de stream `goal_state`.
...
### Escolher um Estilo de Saída

Use `--output-style` para escolher um [estilo de saída](./output-styles) — integrado ou personalizado — para esta execução. Um estilo é um bloco nomeado de instruções sobreposto ao prompt integrado que muda como a resposta é escrita — `Concise` começa com o resultado e remove preâmbulos e narração, `Proactive` começa a trabalhar em vez de propor, `Explanatory` adiciona notas curtas sobre o código ao longo do caminho. Ele sobrescreve a configuração `general.outputStyle`; `default` não seleciona nenhum estilo.
...
> [!note]
>
> - `Learning` pede que você escreva parte do código e aguarda uma resposta, então é ignorado em execuções headless.
> - Um nome de estilo desconhecido imprime um aviso e a execução continua com o estilo padrão.
> - `--output-style` não tem efeito quando `--system-prompt` ou `QWEN_SYSTEM_MD` substitui o prompt integrado — um estilo só é sobreposto ao prompt integrado.
...
Para saída JSON e stream-JSON, valores textuais de `tool_result.content` são limitados a 65.536 bytes UTF-8 após a serialização da string JSON. Valores maiores são emitidos como pré-visualizações determinísticas de início/fim. O mesmo limite se aplica a sessões stream-JSON persistentes, transportes SDK, resultados de ferramentas de subagentes e Dual Output. O modo texto ainda imprime apenas a resposta final, retendo internamente apenas a pré-visualização limitada. Esse limite não restringe uma sessão JSON inteira, um evento JSONL, entrada de ferramenta ou mensagem parcial.
...
| `--output-style`             | Estilo de saída para esta execução (`Concise`, `Proactive`, `Explanatory`, `Learning`, o nome de um estilo personalizado ou `default` para nenhum); sobrescreve `general.outputStyle`                                                                                                                                                                                                                                                                                 | `qwen -p "query" --output-style Concise`                                 |
...
Para detalhes completos sobre todas as opções de configuração, arquivos de configurações e variáveis de ambiente disponíveis, consulte o [Guia de Configuração](../configuration/settings).
...
| `--max-tool-calls`    | `model.maxToolCalls`         | Chamadas de ferramentas de nível superior cumulativas despachadas pelo loop de execução principal (conta sucessos _e_ falhas — o modelo ainda consome tokens em erros). Veja "Escopo" abaixo para isenções de subagentes / saída estruturada. |
...
- **Ambiente isolado e confiável (runner de CI efêmero, contêiner):** `qwen -p "..." --yolo --max-session-turns N --max-wall-time 10m --output-format json`. Defina um orçamento de turnos e um orçamento de tempo real para que um agente travado não consuma todos os minutos do seu CI, e capture `--output-format json` para auditoria de uso / chamadas de ferramentas pós-execução.
- **Máquina local ou infraestrutura compartilhada:** passe também `--sandbox` (ou defina `QWEN_SANDBOX=1`) para que as ferramentas shell / write / edit sejam executadas dentro da imagem do sandbox.
...
> [!important]
> O retry persistente requer uma **opt-in explícita**. Apenas `CI=true` **não** o ativa — transformar silenciosamente um job de CI de falha rápida em um job de espera infinita seria perigoso. Sempre defina `QWEN_CODE_UNATTENDED_RETRY` explicitamente na configuração do seu pipeline.
...
Essas mensagens mantêm os runners de CI ativos e permitem que você monitore o progresso. Elas não aparecem no stdout, então a saída JSON canalizada para outras ferramentas permanece limpa.

...
- ✅ **Tokens Fresh Local Control** — `--local-control` gera um token para esse processo. O armazenamento geral de tokens do daemon continua sendo BYO-token.
...
| `--local-control`                       | `false`         | Compartilha o Web Shell autenticado em cada interface IPv4 não-loopback com um token fresco por processo, códigos QR etiquetados no terminal, origens de navegador exatas, uma porta fixa e inibição de suspensão de melhor esforço. Conflita com `--token`, `--allow-origin`, `--no-web`, `--port 0` e `--hostname` não padrão; adicione `--tls-cert` + `--tls-key` para APIs de navegador de contexto seguro como entrada de voz.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |

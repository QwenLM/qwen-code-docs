...
`qwen-code-action` é uma GitHub Action que integra o [Qwen Code] ao seu fluxo de desenvolvimento por meio da [Qwen Code CLI]. Ela atua tanto como um agente autônomo para tarefas rotineiras críticas de codificação quanto como um colaborador sob demanda ao qual você pode delegar rapidamente trabalho.

Use-a para realizar revisões de pull requests no GitHub, triagem de issues, análise e modificação de código, e muito mais utilizando o [Qwen Code] de forma conversacional (por exemplo, `@qwencoder fix this issue`) diretamente dentro dos seus repositórios do GitHub.

- [qwen-code-action](#qwen-code-action)
  - [Visão Geral](#visão-geral)
  - [Recursos](#recursos)
  - [Início Rápido](#início-rápido)
    - [1. Obtenha uma Chave de API do Qwen](#1-obtenha-uma-chave-de-api-do-qwen)
    - [2. Adicione-a como um Segredo do GitHub](#2-adicione-a-como-um-segredo-do-github)
    - [3. Atualize seu .gitignore](#3-atualize-seu-gitignore)
    - [4. Escolha um Fluxo de Trabalho](#4-escolha-um-fluxo-de-trabalho)
    - [5. Experimente](#5-experimente)
  - [Fluxos de Trabalho](#fluxos-de-trabalho)
    - [Qwen Code Dispatch](#qwen-code-dispatch)
    - [Triagem de Issues](#triagem-de-issues)
    - [Revisão de Pull Request](#revisão-de-pull-request)
    - [Assistente da Qwen Code CLI](#assistente-da-qwen-code-cli)
  - [Configuração](#configuração)
    - [Entradas](#entradas)
    - [Saídas](#saídas)
    - [Variáveis do Repositório](#variáveis-do-repositório)
    - [Segredos](#segredos)
  - [Autenticação](#autenticação)
    - [Autenticação do GitHub](#autenticação-do-github)
  - [Extensões](#extensões)
...
- **Colaboração sob demanda**: Acione fluxos de trabalho nos comentários de issues e pull requests mencionando o [Qwen Code CLI] (por exemplo, `@qwencoder /review`).
- **Extensível com Ferramentas**: Aproveite os recursos de chamada de ferramentas dos modelos [Qwen Code] para interagir com outras CLIs como o [GitHub CLI] (`gh`).
- **Personalizável**: Utilize um arquivo `QWEN.md` em seu repositório para fornecer instruções e contexto específicos do projeto ao [Qwen Code CLI].
...
Obtenha sua chave de API no [DashScope] (plataforma de IA da Alibaba Cloud)
...
1. Copie os fluxos de trabalho pré-construídos do diretório [`examples/workflows`](./examples/workflows) para o diretório `.github/workflows` do seu repositório. Nota: o fluxo de trabalho `qwen-dispatch.yml` também deve ser copiado, pois ele aciona a execução dos fluxos de trabalho.
...
Esta action fornece diversos workflows pré-construídos para diferentes casos de uso. Cada workflow foi projetado para ser copiado para o diretório `.github/workflows` do seu repositório e personalizado conforme necessário.

### Qwen Code Dispatch

Este workflow atua como um dispatcher central para o Qwen Code CLI, roteando requisições para
o workflow apropriado com base no evento de gatilho e no comando fornecido
no comentário. Para um guia detalhado sobre como configurar o workflow de dispatch, vá para
a
[documentação do workflow Qwen Code Dispatch](./examples/workflows/qwen-dispatch).

### Triagem de Issues

Esta ação pode ser usada para triar automaticamente issues do GitHub ou em horário agendado.
Para um guia detalhado sobre como configurar o sistema de triagem de issues, vá para a
[documentação do workflow de Triagem de Issues do GitHub](./examples/workflows/issue-triage).

### Revisão de Pull Requests

Esta ação pode ser usada para revisar automaticamente pull requests quando eles são
abertos. Para um guia detalhado sobre como configurar o sistema de revisão de pull requests,
vá para a [documentação do workflow de Revisão de PR do GitHub](./examples/workflows/pr-review).

### Assistente CLI do Qwen Code

Este tipo de ação pode ser usado para invocar um assistente de IA conversacional de propósito geral do Qwen Code
dentro dos pull requests e issues para realizar uma ampla gama de
tarefas. Para um guia detalhado sobre como configurar o fluxo de trabalho CLI de propósito geral do Qwen Code,
acesse a [documentação do fluxo de trabalho do Assistente Qwen Code](./examples/workflows/qwen-assistant).
...
- <a name="__input_settings"></a><a href="#user-content-__input_settings"><code>settings</code></a>: _(Opcional)_ Uma string JSON escrita em `.qwen/settings.json` para configurar as definições do projeto do CLI.
...
Recomendamos definir os seguintes valores como variáveis do repositório para que possam ser reutilizados em todos os workflows. Alternativamente, você pode defini-los inline como entradas de ação em workflows individuais ou para substituir valores no nível do repositório.
...
| `QWEN_CLI_VERSION` | Controla qual versão do Qwen Code CLI é instalada.         | Variável | Não         | Fixar a versão do CLI      |
...
1. Vá para **Settings > Secrets and variables > Actions > New variable** do seu repositório.
...
| ----------------- | ---------------------------------------------- | ----------- | ------------------------------------------ |
| `QWEN_API_KEY`    | Sua chave de API do Qwen do DashScope.         | Sim         | Necessário para todos os fluxos de trabalho que chamam o Qwen. |
| `APP_PRIVATE_KEY` | Chave privada do seu GitHub App (formato PEM). | Não         | Usando um GitHub App personalizado.        |

Para adicionar um segredo:

1. Vá para **Configurações > Secrets e variáveis > Actions > Novo segredo do repositório** do seu repositório.
2. Insira o nome e o valor do segredo.
3. Salve.

Para mais informações, consulte a
[documentação oficial do GitHub sobre criação e uso de segredos criptografados][secrets].
...
Para instruções detalhadas sobre como configurar e usar as extensões, acesse a
...
- **Proteção do Repositório:** Implementar proteção de branches e tags, e restringir aprovadores de pull requests.
...
Para um guia completo sobre como proteger seu repositório e fluxos de trabalho, consulte nossa [**documentação de Melhores Práticas**](./docs/best-practices.md).

## Personalização

Crie um arquivo [QWEN.md] na raiz do seu repositório para fornecer
contexto e instruções específicas do projeto para o [Qwen Code CLI]. Isso é útil para definir
...
Contribuições são bem-vindas! Confira o [**Guia de Contribuição**](./CONTRIBUTING.md) do Qwen Code CLI para mais detalhes sobre como começar.

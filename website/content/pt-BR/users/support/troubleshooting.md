...
- **Erro: `UNABLE_TO_GET_ISSUER_CERT_LOCALLY` ou `unable to get local issuer certificate`**
  - **Causa:** Você pode estar em uma rede corporativa com um firewall que intercepta e inspeciona o tráfego SSL/TLS. Isso geralmente exige que um certificado raiz personalizado da autoridade certificadora seja confiável para o Node.js.
  - **Solução:** Defina a variável de ambiente `NODE_EXTRA_CA_CERTS` com o caminho absoluto do arquivo do certificado raiz corporativo.
    - Exemplo: `export NODE_EXTRA_CA_CERTS=/caminho/para/seu/certificado-corporativo.crt`
...
- **O Qwen Code não está sendo executado no modo interativo em ambientes "CI"**
  ...
  - **Causa:** O pacote `is-in-ci` verifica a presença de `CI`, `CONTINUOUS_INTEGRATION` ou qualquer variável de ambiente com prefixo `CI_`. Quando alguma dessas é encontrada, ele sinaliza que o ambiente é não interativo, impedindo que a CLI seja iniciada no modo interativo.
...
- **Modo DEBUG não funciona a partir do arquivo .env do projeto**
  - **Problema:** Definir `DEBUG=true` em um arquivo `.env` do projeto não ativa o modo de depuração para a CLI.
...
## Companion da IDE não está conectando
...
- **Depuração via CLI:**
  ...
  - Verifique os logs do CLI, geralmente encontrados em um diretório específico do usuário ou em um diretório de cache.
...
- **Depuração do núcleo:**
  ...
  - Utilize ferramentas de depuração do Node.js (por exemplo, `node --inspect`) caso precise percorrer passo a passo o código do lado do servidor.

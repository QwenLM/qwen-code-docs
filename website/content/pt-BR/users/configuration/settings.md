...
| 2     | Arquivo padrão do sistema | Configurações padrão em todo o sistema que podem ser substituídas por outros arquivos de configuração |
...
- [Habilidades de Agente](../features/skills) (experimental) sob `.qwen/skills/` (cada Habilidade é um diretório contendo um arquivo `SKILL.md`).
...
| `general.gitCoAuthor`           | boolean | Adicionar automaticamente um trailer Co-authored-by às mensagens de commit do git quando commits são feitos através do Qwen Code. | `true`      |
...
| `model.openAILoggingDir`                             | string  | Caminho personalizado para diretório de logs da API OpenAI. Se não especificado, o padrão é `logs/openai` no diretório de trabalho atual. Suporta caminhos absolutos, caminhos relativos (resolvidos a partir do diretório atual) e expansão de `~` (diretório home).                                                                                                               | `undefined` |
...
| `context.discoveryMaxDirs`                          | número                     | Número máximo de diretórios para pesquisar memória.                                                                                                                                                                                                                                                                                                                   | `200`       |
...
| `context.fileFiltering.enableRecursiveFileSearch`   | booleano                   | Se deve habilitar a pesquisa recursiva por nomes de arquivos na árvore atual ao completar prefixos `@` no prompt.                                                                                                                                                                                                                                                    | `true`      |
...
1. **Use `.qwenignore`:** Crie um arquivo `.qwenignore` na raiz do seu projeto para excluir diretórios que contenham um grande número de arquivos dos quais você não precisa referenciar (por exemplo, artefatos de compilação, logs, `node_modules`). Reduzir o número total de arquivos pesquisados é a maneira mais eficaz de melhorar o desempenho.
...
| `tools.allowed`                      | array de strings  | Uma lista de nomes de ferramentas que irão pular o diálogo de confirmação. Isso é útil para ferramentas nas quais você confia e usa frequentemente. Por exemplo, `["run_shell_command(git)", "run_shell_command(npm test)"]` irá pular o diálogo de confirmação para executar quaisquer comandos `git` e `npm test`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | `undefined` |                                                                                                                                                                                                                                                      |
...
| `tools.useBuiltinRipgrep`            | boolean           | Usa o binário ripgrep embutido. Quando definido como `false`, o comando `rg` do sistema será usado em vez disso. Esta configuração só é eficaz quando `tools.useRipgrep` é `true`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | `true`      |                                                                                                                                                                                                                                                      |
...
| `mcp.excluded`      | array de strings | Uma lista de negação de servidores MCP a excluir. Um servidor listado tanto em `mcp.excluded` quanto em `mcp.allowed` é excluído. Observe que isso será ignorado se `--allowed-mcp-server-names` estiver definido.                                                              | `undefined` |
...
| `mcpServers.<NOME_DO_SERVIDOR>.url`     | string           | A URL de um servidor MCP que utiliza Server-Sent Events (SSE) para comunicação.                                                                                                                                                                                    | Sim      |
| `mcpServers.<NOME_DO_SERVIDOR>.httpUrl` | string           | A URL de um servidor MCP que utiliza HTTP transmissível para comunicação.                                                                                                                                                                                          | Sim      |
...
| `mcpServers.<NOME_DO_SERVIDOR>.includeTools` | array de strings | Lista de nomes de ferramentas a incluir deste servidor MCP. Quando especificado, somente as ferramentas listadas aqui estarão disponíveis a partir deste servidor (comportamento de lista branca). Se não especificado, todas as ferramentas do servidor são habilitadas por padrão. | Sim      |
...
| `telemetry.logPrompts`   | boolean | Indica se o conteúdo dos prompts do usuário deve ou não ser incluído nos logs.                 |         |
...
Para variáveis relacionadas à autenticação (como `OPENAI_*`) e a abordagem recomendada de `.qwen/.env`, veja **[Autenticação](../configuration/auth)**.
...
| `DEBUG` ou `DEBUG_MODE`          | (geralmente usado por bibliotecas subjacentes ou pelo próprio CLI) Defina como `true` ou `1` para habilitar registro detalhado de depuração, útil para solução de problemas. | **Nota:** Essas variáveis são automaticamente excluídas dos arquivos `.env` do projeto por padrão para evitar interferência no comportamento do CLI. Use arquivos `.qwen/.env` se precisar definir essas variáveis especificamente para o Qwen Code.                                                                                                                                                                                                                                                               |
...
- O sandbox é habilitado quando se usa `--yolo` ou `--approval-mode=yolo` por padrão.

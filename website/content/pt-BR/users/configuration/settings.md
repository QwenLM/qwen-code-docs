...
> [!tip]
>
> **Autenticação / API keys:** A autenticação (API Key, Alibaba Cloud Coding Plan) e as variáveis de ambiente relacionadas à autenticação (como `OPENAI_API_KEY`) estão documentadas em **[Authentication](../configuration/auth)**.
...
| `general.sessionRecapAwayThresholdMinutes` | number | Minutos que o terminal deve ficar desfocado antes que um resumo automático seja acionado ao receber foco. Usado apenas quando `showSessionRecap` está habilitado. | `5` |
...
| `general.dynamicCommandTranslation` | boolean | Habilita a tradução por IA das descrições dinâmicas de comandos slash. Quando desativado, os comandos dinâmicos mantêm suas descrições originais e ignoram as chamadas do modelo de tradução. | `false` |
...
| `ui.hideBanner` | boolean | Oculta o logotipo ASCII inicial e o painel de informações. As dicas e a entrada de chat ainda são renderizadas, a menos que `ui.hideTips` também esteja definido. | `false` |
...
| `ui.customAsciiArt` | string \| object | Substitui o logotipo ASCII do QWEN no banner. Aceita uma string inline (usada para ambas as faixas de largura), `{ "path": "./brand.txt" }` (caminhos relativos são resolvidos em relação ao diretório do arquivo de configuração proprietário; lido uma vez na inicialização com `O_NOFOLLOW` no POSIX, limitado a 64 KB) ou `{ "small": ..., "large": ... }` para seleção consciente da largura. Sanitizado; limitado a 200 linhas × 200 colunas por faixa. | `undefined` |
| `ui.showLineNumbers` | boolean | Mostra números de linha nos blocos de código na saída da CLI. | `true` |
...
| `ui.history.collapsePreviewCount` | number | Número de turnos de usuário mais recentes a serem mantidos visíveis quando `ui.history.collapseOnResume` está habilitado. `0` recolhe todo o histórico restaurado por padrão; `-1` mostra todo o histórico restaurado. | `0` |
| `ui.compactMode` | boolean | Oculta a saída da ferramenta e o pensamento para uma visualização mais limpa. Alterne com `Ctrl+O` durante uma sessão ou através da caixa de diálogo Configurações. Os prompts de aprovação da ferramenta nunca são ocultados, mesmo no modo compacto. A configuração persiste entre as sessões. | `false` |
...
| `ui.showResponseTokensPerSecond` | boolean | Mostra uma estimativa ao vivo de tokens/seg ao lado do contador de tokens de resposta enquanto o modelo está transmitindo. Esta é uma dica de velocidade de geração, não um ETA ou porcentagem de conclusão. Tem efeito na próxima sessão. | `false` |
...
| `model.chatCompression.enableScreenshotTrigger`    | boolean | Quando `true`, a auto-compactação também é acionada assim que o número de imagens retornadas por ferramentas acumuladas no histórico atinge `screenshotTriggerThreshold`, independentemente do uso de tokens — voltado para sessões de uso de computador onde capturas de tela frequentes diluem a atenção do modelo. Conta apenas imagens retornadas dentro de resultados de ferramentas, não imagens coladas pelo usuário. Substituição via variável de ambiente: `QWEN_COMPACT_SCREENSHOT_TRIGGER` (`1`/`true`/`0`/`false`).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | `true`      |
| `model.chatCompression.screenshotTriggerThreshold` | number  | Contagem de imagens retornadas por ferramentas no qual ou acima do qual o gatilho de captura de tela é acionado (apenas quando `enableScreenshotTrigger`). A compactação reseta a contagem — as imagens sobreviventes são reincorporadas como partes de nível superior, que o gatilho não conta — portanto, não será acionado novamente imediatamente. Substituição via variável de ambiente: `QWEN_COMPACT_SCREENSHOT_THRESHOLD`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | `50`        |
...
| `model.skipStartupContext`                         | boolean | Ignora o envio do contexto do workspace na inicialização (resumo do ambiente e confirmação) no início de cada sessão. Habilite isso se você preferir fornecer o contexto manualmente ou quiser economizar tokens na inicialização.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | `false`     |
| `model.enableOpenAILogging`                        | boolean | Habilita o log de chamadas da API OpenAI para depuração e análise. Quando habilitado, as requisições e respostas da API são registradas em arquivos JSON.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | `false`     |
...
Quando nem `samplingParams.max_tokens` nem `QWEN_CODE_MAX_OUTPUT_TOKENS` está definido, o Qwen Code geralmente usa o limite de saída declarado pelo modelo selecionado como o limite de saída padrão da requisição. Se a resposta ainda atingir esse limite, o Qwen Code pode tentar novamente com um limite maior (usando um piso de 64K) e então recuperar nas rodadas de continuação.
...
| `tools.shell.enableInteractiveShell`  | boolean           | Usa `node-pty` para uma experiência de shell interativa. O fallback para `child_process` ainda se aplica.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | `true`      |                                                                                                                                                                                                                                                                                                                             |
| `tools.core`                          | array de strings  | **Descontinuado.** Será removido na próxima versão. Use `permissions.allow` + `permissions.deny` em vez disso. Restringe as ferramentas integradas a uma lista de permissões. Todas as ferramentas que não estiverem na lista serão desativadas.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | `undefined` |                                                                                                                                                                                                                                                                                                                             |
| `tools.exclude`                       | array de strings  | **Descontinuado.** Use `permissions.deny` em vez disso. Nomes de ferramentas para excluir da descoberta. Migrado automaticamente para o formato `permissions` no primeiro carregamento.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | `undefined` |                                                                                                                                                                                                                                                                                                                             |
| `tools.allowed`                       | array de strings  | **Descontinuado.** Use `permissions.allow` em vez disso. Nomes de ferramentas que ignoram o diálogo de confirmação. Migrado automaticamente para o formato `permissions` no primeiro carregamento.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | `undefined` |                                                                                                                                                                                                                                                                                                                             |
...
| `"WebFetch(api.example.com)"` | Busca dados de `api.example.com` e todos os seus subdomínios            |
...
Controla quais comandos slash estão disponíveis no CLI. Útil para restringir a superfície de comandos em implantações multi-tenant ou corporativas.

| Configuração                  | Tipo             | Descrição                                                                                                                                                                                                                                                                                                                 | Padrão     |
| ------------------------ | ---------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------- |
| `slashCommands.disabled` | array of strings | Nomes de comandos slash para ocultar e recusar a execução. Correspondência sem distinção entre maiúsculas e minúsculas com o nome final do comando (para comandos de extensão, esta é a forma desambiguada, ex.: `myext.deploy`). **Mesclado como uma união entre os escopos**, então as configurações do workspace podem adicionar, mas não remover entradas definidas nas configurações do usuário ou do sistema. | `undefined` |

A mesma lista de bloqueio também pode ser fornecida via flag de CLI `--disabled-slash-commands`
(separada por vírgulas ou repetida) e a variável de ambiente `QWEN_DISABLED_SLASH_COMMANDS`;
os valores de todas as três fontes são unidos.

**Exemplo — bloquear comandos integrados para uma implantação em sandbox:**
...
ou `QWEN_CODE_SYSTEM_SETTINGS_PATH`), os usuários não podem reduzir a lista de bloqueio a partir
de seu próprio escopo, e os comandos desativados não aparecerão no autocomplete ou
serão executados quando digitados.

> [!note]
> Esta configuração controla apenas comandos slash (ex.: `/auth`, `/mcp`). Ela não
...
| `mcp.allowed`       | array of strings | Uma allowlist de servidores MCP permitidos. Permite especificar uma lista de nomes de servidores MCP que devem ser disponibilizados para o modelo. Isso pode ser usado para restringir o conjunto de servidores MCP aos quais conectar. Observe que isso será ignorado se `--allowed-mcp-server-names` estiver definido. | `undefined` |
| `mcp.excluded`      | array of strings | Uma denylist de servidores MCP para excluir. Um servidor listado tanto em `mcp.excluded` quanto em `mcp.allowed` é excluído. Observe que isso será ignorado se `--allowed-mcp-server-names` estiver definido.                                                                                           | `undefined` |
...
> [!warning]
> **Recurso Experimental**: O suporte a LSP é atualmente experimental e desativado por padrão. Habilite-o usando a flag de linha de comando `--experimental-lsp`.
...
| `advanced.excludedEnvVars`     | array of strings | Variáveis de ambiente para excluir do contexto do projeto. Especifica variáveis de ambiente que devem ser excluídas de serem carregadas a partir de arquivos `.env` do projeto. Isso impede que variáveis de ambiente específicas do projeto (como `DEBUG=true`) interfiram no comportamento do CLI. Variáveis de arquivos `.qwen/.env` nunca são excluídas.       | `["DEBUG","DEBUG_MODE"]` |
...
| Configuração                             | Tipo    | Descrição                                                                                                                                                                                                                                                                                          | Padrão |
| ----------------------------------- | ------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------- |
| `experimental.cron`                 | boolean | Habilita ferramentas de cron/loop na sessão (`cron_create`, `cron_list`, `cron_delete`) para que o modelo possa criar prompts recorrentes. Pode ser desativado via variável de ambiente `QWEN_CODE_DISABLE_CRON=1`. Requer reinicialização.                                                                                  | `true`  |
| `experimental.agentTeam`            | boolean | Habilita ferramentas de colaboração de agent-team (`team_create`, `task_create`, `task_update`, `send_message`, etc.) para coordenação multi-agente. Também pode ser habilitado via `QWEN_CODE_ENABLE_AGENT_TEAM=1`. Requer reinicialização.                                                                                   | `false` |
...
| `experimental.emitToolUseSummaries` | boolean | Gera um rótulo curto baseado em LLM após a conclusão de cada lote de chamadas de ferramentas. Consulte [Tool-Use Summaries](../features/tool-use-summaries). Requer que um modelo rápido esteja configurado (`fastModel`); caso contrário, é ignorado silenciosamente. Pode ser substituído por sessão com `QWEN_CODE_EMIT_TOOL_USE_SUMMARIES=0` ou `=1`. | `true`  |
...
| `mcpServers.<SERVER_NAME>.args`         | array of strings | Argumentos para passar para o comando.                                                                                                                                                                                                                                  | Sim      |
| `mcpServers.<SERVER_NAME>.env`          | object           | Variáveis de ambiente para definir para o processo do servidor.                                                                                                                                                                                                               | Sim      |
...
| `mcpServers.<SERVER_NAME>.headers`      | object           | Um mapa de cabeçalhos HTTP para enviar com requisições para `url` ou `httpUrl`.                                                                                                                                                                                                 | Sim      |
...
| `mcpServers.<SERVER_NAME>.includeTools` | array of strings | Lista de nomes de ferramentas para incluir deste servidor MCP. Quando especificado, apenas as ferramentas listadas aqui estarão disponíveis a partir deste servidor (comportamento de allowlist). Se não especificado, todas as ferramentas do servidor são habilitadas por padrão.                                        | Sim      |
| `mcpServers.<SERVER_NAME>.excludeTools` | array of strings | Lista de nomes de ferramentas para excluir deste servidor MCP. As ferramentas listadas aqui não estarão disponíveis para o modelo, mesmo que sejam expostas pelo servidor. **Nota:** `excludeTools` tem precedência sobre `includeTools` - se uma ferramenta estiver em ambas as listas, ela será excluída. | Sim      |
#### telemetry
...
| `QWEN_RUNTIME_DIR`                                 | Substitui o diretório de saída de tempo de execução (conversas, logs, todos). Quando não definida, o padrão é o diretório `QWEN_HOME`.                                                                                                                                                                                                                                                                         | Use isso para separar dados efêmeros de tempo de execução da configuração persistente. Útil quando `QWEN_HOME` está em um sistema de arquivos compartilhado/lento.                                                                                                                                                                                                                                                                                                                 |
...
| `QWEN_TELEMETRY_OTLP_ENDPOINT`                     | Define o endpoint OTLP para a telemetria.                                                                                                                                                                                                                                                                                                                                                                      | Substitui a configuração `telemetry.otlpEndpoint`.                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
| `CODE_ASSIST_ENDPOINT`                             | Especifica o endpoint para o servidor de code assist.                                                                                                                                                                                                                                                                                                                                                          | Isso é útil para desenvolvimento e testes.                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| `QWEN_CODE_MAX_OUTPUT_TOKENS`                      | Substitui o limite padrão máximo de tokens de saída por resposta. Quando não definido, o Qwen Code usa o limite de saída declarado pelo modelo e, se uma resposta for truncada, escala automaticamente (piso de 64K) e se recupera entre os turnos. Defina isso com um valor específico (por exemplo, `16000`) para usar um limite fixo — útil para backends self-hosted com capacidade restrita que desejam uma reserva de slots por solicitação menor. | Tem precedência sobre o padrão de limite do modelo, mas é substituído por `samplingParams.max_tokens` nas configurações. Desabilita a escala automática quando definido. Exemplo: `export QWEN_CODE_MAX_OUTPUT_TOKENS=16000`                                                                                                                                                                                                                                                        |
...
| `QWEN_CODE_PROFILE_STARTUP_NO_HEAP`                | Defina como `1` junto com `QWEN_CODE_PROFILE_STARTUP=1` para pular os snapshots `process.memoryUsage()` por checkpoint. Útil ao medir o próprio overhead de Heisenberg do profiler.                                                                                                                                                                                                                            | Desativado por padrão. Os snapshots de heap custam ~50 µs cada (bem abaixo de 1% do tempo total de inicialização), então a maioria dos usuários deve deixar isso como está.                                                                                                                                                                                                                                                                                                        |
...
Para a seleção da imagem do sandbox, a precedência é: `--sandbox-image` > `QWEN_SANDBOX_IMAGE` > `tools.sandboxImage` > imagem padrão integrada.
...
| `--system-prompt` | | Substitui o prompt do sistema da sessão principal integrado para esta execução. | Seu texto de prompt | Arquivos de contexto carregados, como `QWEN.md`, ainda são anexados após essa substituição. Pode ser combinado com `--append-system-prompt`. |
...
| `--acp` | | Habilita o modo ACP (Agent Client Protocol). Útil para integrações de IDE/editor como [Zed](../integration-zed). | | Estável. Substitui a flag obsoleta `--experimental-acp`. |
...
| `--openai-logging` | | Habilita o registro de chamadas da API OpenAI para depuração e análise. | | Esta flag substitui a configuração `enableOpenAILogging` em `settings.json`. |
...
- **Chamadas de Ferramentas:** Registramos os nomes das ferramentas que são chamadas, se elas tiveram sucesso ou falha, e quanto tempo levam para executar. Não coletamos os argumentos passados para as ferramentas nem nenhum dado retornado por elas.

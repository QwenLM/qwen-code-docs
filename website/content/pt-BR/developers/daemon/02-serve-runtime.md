...
- O `denyBrowserOriginCors` rejeita **todas** as requisições que carregam `Origin`; o Web Shell de **loopback** funciona porque outro middleware remove primeiro os valores correspondentes de mesma origem do loopback — binds fora do loopback exigem `--allow-origin` para os XHRs do shell.

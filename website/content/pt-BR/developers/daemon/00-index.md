...
- **ACP child** - o `qwen --acp` filho que hospeda o runtime do agente de um workspace. A produção tenta pré-aquecer a bridge primária e tenta novamente no primeiro uso após falha; um secundário confiável inicia seu filho sob demanda, enquanto um secundário não confiável não o faz. A bridge proprietária multiplexa sessões e clientes nesse filho.

...
No modo gerenciado por daemon, cada canal mapeia o tráfego de chat recebido para sessões do daemon sob um `SessionScope` configurável (`user`, `chat_thread` ou `single`). O valor legado `thread` do Channel permanece legível e editável para configurações existentes, mas novas configurações do Web Shell não o oferecem; isso é separado do próprio controle de criação de sessão `single`/`thread` da bridge do daemon. Com `sessionScope: "user"` e `multiSession: true`, o `ChannelBase` adiciona um catálogo persistido de sessões nomeadas, indexado por canal, chat e remetente, enquanto o `SessionRouter` mantém a sessão selecionada como sua rota de compatibilidade. Carregamentos exatos de sessões nomeadas nunca usam o caminho legado de load-or-replace. Um turno nomeado reserva sua sessão exata antes da preparação assíncrona e permanece vinculado após alterações de seleção posteriores sem reassociar a rota de compatibilidade. Tarefas nomeadas podem ser executadas concorrentemente, `/session cancel [<name>]` atinge apenas o prompt ativo verificado e comandos de permissão de texto simples consideram apenas a tarefa selecionada. `/session new <name> --worktree` também solicita isolamento Git worktree gerenciado pelo daemon. O daemon retorna o caminho canônico do worktree mais `worktreeState: "persisted-v1"`; a bridge valida essa atestação antes de registrar qualquer rota, e o reattach repete a mesma verificação. Turnos nomeados também capturam um rótulo de origem de tarefa apenas para entrega: mensagens diretas usam `[task]`, grupos usam `[sender · task]`, e prompts de permissão de texto exatos incluem o ID da requisição. O rótulo não é adicionado à resposta do modelo ou à transcrição. O adaptador delega para o `DaemonChannelBridge`, que por sua vez delega para o `DaemonSessionClient` do SDK (consulte [`13-sdk-daemon-client.md`](./13-sdk-daemon-client.md)). Cada canal nomeado deve resolver para um workspace registrado e confiável. O worker usa o cwd canônico desse runtime, `QWEN_DAEMON_WORKSPACE` e a sobreposição de ambiente; a resolução de propriedade nunca faz fallback para o primário.
...
O `ChannelWorkerManager` de longa duração possui a seleção consolidada do daemon e os supervisores agrupados por workspace. Uma seleção explícita com `--channel` tem precedência e permanece fail-fast. Na inicialização sem flag, a configuração `serve.channels` do workspace primário confiável é restaurada; isso carrega dinamicamente o runtime de canal, reserva o pidfile do serviço, resolve a propriedade do workspace e inicia os workers selecionados. Workspaces secundários não restauram independentemente suas próprias configurações `serve.channels`. Sem nenhuma fonte de inicialização, o primeiro `PUT /workspace/channel` com gate estrito realiza inicialização preguiçosa. `GET /workspace/channel` lê o snapshot do manager e `DELETE /workspace/channel` o interrompe de forma idempotente. Os helpers do SDK são `getChannelWorkerControl()`, `setChannelWorkerSelection()` e `stopChannelWorker()`; a entrada CLI é `qwen channel set` mais as variantes remotas `status` e `stop`.

Nomes de inicialização armazenados devem ser não vazios, não ter espaços em branco no início ou no fim e não conter caracteres de controle inseguros ou invisíveis. Entradas inválidas são ignoradas individualmente e registradas por índice do array; o daemon não as ajusta para outro nome de instância nem reescreve as configurações. Os argumentos do worker usam `--channel=<value>`, o que mantém um traço inicial no nome. O gerenciamento de canal continua reportando as configurações de inicialização persistidas e o estado de runtime real.
...
Um campo de inicialização inválido ou um erro de validação ou de lease antes dos workers iniciarem ignora a restauração automática e registra sua fonte `serve.channels` sem descartar configurações não relacionadas. Após uma falha na inicialização do worker, o daemon continua apenas se a limpeza for bem-sucedida. Os timeouts globais de inicialização do runtime e as paradas de worker não confirmadas seguem o caminho existente de falha de inicialização; o lease do serviço permanece mantido enquanto a terminação do worker não é confirmada. Inspecione o log do daemon para tentativas de restauração ignoradas ou com falha.

Falhas no `connect()` do adaptador são reportadas separadamente dos erros de ciclo de vida do worker. O worker envia cada falha delimitada e com credenciais redigidas pelo IPC de startup e aguarda o reconhecimento do supervisor antes de tentar o próximo adaptador. Um worker parcialmente conectado permanece em execução e expõe `startupFailures` em seu snapshot. Se todo adaptador em uma tentativa dinâmica falhar, a resposta `502 channel_worker_start_failed` carrega falhas tentadas anotadas com workspace enquanto `state` reflete o resultado do rollback; respostas GET subsequentes não retêm a tentativa. A inicialização explícita com `--channel` sem nenhum adaptador conectado continua sendo fail-fast. O `code` opcional do adaptador é apenas diagnóstico, e a `phase` atual é `connect`.
...
| `multiSession`                           | Tarefas nomeadas apenas do daemon para `sessionScope: 'user'`. O catálogo do proprietário é persistido abaixo do diretório de estado do workspace/canal; tarefas podem ser executadas concorrentemente, podem optar por worktrees Git por tarefa, comandos de cancelamento e permissão permanecem correlacionados à tarefa exata, e resultados e superfícies interativas identificam sua tarefa de origem. Webhooks, backfill de histórico de grupo e loops permanecem excluídos. |

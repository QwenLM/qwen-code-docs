...
- **`ServeMcpBudgetStatusCell.scope` é orientado por capabilities e compatível com versões futuras.** As células de snapshot expõem `budgets[]`, não um único campo `budget?`. Com `mcp_workspace_pool`, o runtime selecionado emite `scope: 'workspace'`; com o pool desabilitado ou indisponível, o gerenciador legado emite `scope: 'session'`. Consumidores devem tolerar valores adicionais desconhecidos de `scope` descartando-os em vez de falhar.

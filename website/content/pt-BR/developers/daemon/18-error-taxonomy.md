...
| `file_too_large`          | 413        | Texto grande sem limite finito de linhas, texto grande não é UTF-8, ou uma escrita excede `MAX_WRITE_BYTES` (5 MiB). | Use um limite finito de linhas para texto UTF-8 grande, use uma janela de bytes, ou divida a escrita.                                    |
| `hash_mismatch`           | 409        | `expectedSha256` de concorrência otimista falhou, ou um arquivo mudou durante uma leitura estável. | Releia o arquivo e tente novamente com a nova versão/hash.                                                                   |
...
| ------------------------------------- | ---- | ------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `SessionNotFoundError`                | 404  | sessionId não está em `byId` (`code: "session_not_found"`) ou a sessão está fechando (`code: "session_closing"`).                                                                                                   | Para `session_not_found`: recrie ou anexe; a sessão pode ter sido removida. Para `session_closing`: aguarde e tente novamente; um fechamento concorrente está em andamento. Rotas DELETE tratam `session_closing` como sucesso idempotente. |
...
| `RestoreInProgressError`              | 409  | `loadSession`, `resumeSession` ou um id fornecido pelo chamador em `POST /session` colide com outro registro que já possui o mesmo id.                                                                                                                                                                    | Aguarde o atraso anunciado e tente novamente o restore ou spawn solicitado; a limpeza abandonada carrega um backoff derivado do orçamento.                                                                                                                      |
...
| `restore_timeout`          | O load/resume de sessão ACP excedeu seu orçamento dedicado de restore.        |

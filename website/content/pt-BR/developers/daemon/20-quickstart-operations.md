...
Abra `http://127.0.0.1:4170/` em um navegador para obter a UI do Web Shell: chat, lista de sessões e inspeção de workspace. Este listener loopback sem token concede aos chamadores locais autoridade total da API do operador, preservando confiança de workspace, propriedade de sessão, client-id, permissão, feature, validação e verificações de recursos. `createServeApp()` monta os assets do Web Shell empacotados (`packages/cli/src/serve/web-shell-static.ts`) **antes** de `bearerAuth`, então o shell carrega sem token; suas próprias chamadas de API carregam o bearer quando um está configurado — inicie o daemon com `--open` (que coloca o token no fragmento da URL, nunca enviado ao servidor) ou adicione `#token=…` manualmente quando a autenticação estiver habilitada. `--no-web` desativa e deixa o daemon apenas com API.
...
| `--hostname <host>`                     | string                         | `127.0.0.1`                                      | Non-loopback requires token              | Endereço de bind. Valores de loopback: qualquer endereço `127.0.0.0/8`, `localhost`, `::1`, `[::1]`; `localhost` é resolvido uma vez e fixado, e o modo confiável verifica o endereço real do listener antes da inicialização completar. Os colchetes de `[::1]` são removidos automaticamente; a entrada `host:port` é rejeitada com orientação para usar `--port`.                                                                                                          |
...
| `--enable-session-shell`                | boolean                        | `false`                                          | Bearer or trusted loopback               | Habilita a execução direta de `POST /session/:id/shell`. Os chamadores também devem enviar um `X-Qwen-Client-Id` vinculado à sessão.                                                                                                                                                                                   |
...
| `--allow-origin <pattern>`              | repeatable string              | none                                             | -                                        | Allowlist CORS que substitui a negação padrão de Origin. Wildcard e origins HTTP(S) não-loopback requerem um token.                                                                                                                                                                                                                                          |
...
| `--channel-idle-timeout-ms <n>`         | non-negative integer           | `0`                                              | -                                        | Atraso automático de reaping do filho ACP após o trabalho do runtime ser drenado. O pré-aquecimento simples é preservado para o primeiro uso. Janelas de keepalive ativas podem estender o atraso configurado; o maior atraso restante prevalece.                                                                                                                                                                                                                    |
| `--initialize-timeout-ms <n>`           | number                         | `10000`                                          | -                                        | Prazo de inicialização do filho ACP (factory de canal + handshake de inicialização) e timeout padrão de requisição (ms).                                                                                                                                                                                                                                       |
...
| `context.fileName`          | string                                                             | Controla qual arquivo o `POST /workspace/init` escreve através do `contextFilename` do workspace-service.                                                                 |
...
| `--allow-origin` HTTP(S) não-loopback sem token                               | `Refusing to start with --allow-origin ... but no bearer token configured`                          |
...
O shell estático é montado **antes** de `bearerAuth` em qualquer modo de inicialização — um navegador não pode anexar um header `Authorization` a uma navegação na barra de endereços ou a um sub-recurso `<script src>`, então protegê-lo simplesmente quebraria a UI. As chamadas de API usam a autoridade do loopback confiável no listener primário sem token ou anexam o bearer configurado nos modos autenticados. Em um bind não-loopback, o shell é somente leitura, a menos que `--allow-origin <origin>` seja passado — POSTs same-origin carregam um header `Origin` que a barreira CORS rejeita (403) — então use `--allow-origin` para qualquer bind além do loopback.
...
- **`await blockForever()` é intencional.** Se `yargs.parse()` for resolvido, o nível superior da CLI cai no ponto de entrada da TUI interativa (`llm.tsx`). SIGINT / SIGTERM saem através do caminho `onSignal` do `runQwenServe`.
...
Nota: ao chamar `createServeApp` diretamente, o padrão é `fsFactory.trusted = false`. O `writeTextFile` do ACP no lado do agente é rejeitado como `untrusted_workspace`, e um aviso no stderr é impresso uma vez. Injete `deps.fsFactory` com confiança explícita, injete `deps.bridge` ou aceite o comportamento padrão restrito por confiança.

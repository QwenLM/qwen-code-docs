...
| `replay_complete`       | Sentinela sem ID emitido após o término do loop de replay `Last-Event-ID`, para replay limpo e caminhos de ring-evicted, mesmo quando `data.replayedCount === 0`. **Sem `id`**                                                             | `replayedCount: number`; permite que os consumidores removam a UI de catch-up de forma determinística sem timeout.                                                                                                                                                                                                                                |
...
| Tipo                 | Direção | Gatilho                                                              | Principais campos do payload                                                                                                                               |
| -------------------- | ------- | -------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `extensions_changed` | S->C    | Trabalho de instalação/atualização de extensão em segundo plano concluído ou alteração de status | `refreshed, failed, status?: 'installed' \| 'enabled' \| 'disabled' \| 'updated' \| 'uninstalled' \| 'failed', source?, name?, version?, error?`. Um daemon que anuncia `extension_activation_explicit_refresh` confirma a ativação sem transmiti-la, então uma ativação bem-sucedida não emite mais `enabled`/`disabled`; estes vêm de daemons mais antigos, e os mais novos convergem a ativação através de um broadcast de refresh sem status. |

...
Cada controlador independente deve usar um ID distinto e estável. O Web Shell preserva o prefixo histórico `webui_` para compatibilidade. Um host e um Web Shell incorporado devem compartilhar um ID apenas quando intencionalmente atuam como um único controlador lógico; uma vez compartilhado, os logs do daemon não conseguem distinguir qual deles originou uma requisição.
...
Para uma sessão worktree Part 4A persistida, a restauração é uma extensão deste ciclo de vida com gate de integridade. O sidecar identifica explicitamente a raiz do workspace solicitada, o checkout deve estar canonicamente contido abaixo do diretório `.qwen/worktrees/` correspondente, e seu marker deve ser um arquivo regular de link único contendo o ID exato da sessão restaurada. O daemon realoca um filho restaurado ocioso somente após essas verificações; um filho ativo é aceito apenas quando seu cwd reportado já é igual ao worktree, e um filho ativo cujo cwd reportado está ausente ou em outro lugar faz fail closed em vez de ser realocado sob seu prompt — exceto para uma restauração a frio que não pôde adiar seu restore-prompt (`suppressWorktreeContextRestore` desligado, então o bridge disparou a pergunta re-pendurada em vez de estacioná-la): essa forma mantém o resultado pré-4B, retornando metadados canônicos de worktree sem `worktreeState`, não realocado, com a sessão sobrevivendo. As respostas realocadas e aceitas retornam metadados canônicos de worktree com `worktreeState: "persisted-v1"`. Um sidecar com `supersededBy` nunca é restaurado: a rota retorna `409 worktree_session_superseded` com o id da sessão substituta, uma classificação decidida apenas a partir desse link antes de qualquer leitura de marker, então os chamadores redirecionam e reparam seu controle somente quando um load desse id tiver sucesso — uma transferência interrompida pré-commit nomeia uma substituta que não é a proprietária do marker, não pode ser ela mesma restaurada, e é coletada pelo reset re-tentado; uma substituta restaurada cujo link `supersedes` concorda com o sidecar antigo enquanto o marker nunca se moveu — ou está ausente — retorna `409 worktree_reset_interrupted`, cuja reparação é retentar o reset contra a sessão substituída; e um marker ausente sem esse par de links concordantes retorna `409 worktree_marker_missing`, cuja reparação é resetar a tarefa em vez de retentar a restauração, porque nenhum caminho de restauração recria um marker. A classificação de interrupção é verificada primeiro. Um estado Part 4A inválido desanexa uma conexão existente ou mata uma restauração a frio com `requireZeroAttaches`; um sidecar ausente da mesma forma não produz atestação. Sempre que a fonte de restauração efetiva é de propriedade do Channel, a rota suprime a limpeza best-effort do agente ACP para estado Part 4A ou sidecar não classificável, então a falha de validação preserva a evidência incerta do checkout. Os metadados de fonte persistida têm precedência; quando ausentes, a requisição load/resume fornece a fonte efetiva. Um sidecar legado estruturalmente válido sem `workspaceCwd` em vez disso retém a restauração best-effort existente do agente: ele deve identificar a raiz do workspace solicitada ou o nível superior do repositório Git, é verificado por contenção sem atestação de marker, pode ser limpo pelo agente, e pode retornar `worktree` sem `worktreeState`. Além desse caso explícito de compatibilidade legada, apenas sessões cuja fonte de restauração efetiva não é de propriedade do Channel retêm a limpeza best-effort existente antes da validação da rota.

A transferência de propriedade de worktree (`POST /session/:id/worktree-reset`, anunciada por `session_worktree_reset_v1`) estende este ciclo de vida para resets de tarefas do Channel: o daemon cria uma substituta fresca com escopo de thread no workspace raiz, a realoca no checkout verificado, vincula o par de sidecars (`supersededBy` na sessão antiga primeiro, depois `supersedes` na substituta), inverte o marker para a substituta sob um lock de rota por checkout e uma barreira de admissão que protege a admissão de prompts mais os outros sete writers que iniciam trabalho no checkout ou movem o cwd da sessão (rewind, alteração de cwd, branch, fork, shell, controle de objetivo, ação de tarefa de workflow), enquanto os caminhos de release e stop permanecem desprotegidos por design, e então corta os registros de cliente e a associação de worktree em memória da sessão substituída. O corte reporta se a sessão substituída realmente desapareceu: um sobrevivente cujo filho ainda mantém trabalho em segundo plano mantém a barreira armada, é registrado em log, e é reportado ao chamador como `supersededSessionLive: true` em vez de ser encoberto pela resposta de sucesso. Um writer protegido admitido na sessão substituída durante a transferência é recusado com `409 worktree_reset_active`; a taxonomia completa de falhas, incluindo quais janelas de crash um retry reverte e quais fazem fail closed para reparo pelo operador, está documentada com a rota em `qwen-serve-protocol.md`.
...
- Tags de capability: `session_create`, `session_id_override`, `session_scope_override`, `session_load`, `session_resume`, `unstable_session_resume` (alias depreciado), `session_list`, `session_info`, `session_close`, `session_metadata`, `session_set_model`, `client_identity`, `client_heartbeat`, `session_recap`, `session_generation`, `session_btw`, `session_context_usage`, `session_tasks`, `session_monitor_tool_correlation`, `session_stats`, `session_lsp`, `session_resources`, `session_status`, `non_blocking_prompt`.

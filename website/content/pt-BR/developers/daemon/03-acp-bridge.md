...
Cada `WorkspaceRuntime` ativo possui uma instância de `HttpAcpBridge`. A produção tenta pré-aquecer o processo filho primário confiável para compatibilidade; secundários confiáveis abrem seu `AcpChannel` no primeiro comando de runtime ou Session, e secundários não confiáveis não conseguem iniciar o ACP. Rotas primárias legadas mantêm seu comportamento de compatibilidade existente. Dentro do runtime, a bridge fornece sessões multiplexadas sobre o canal, `EventBus`es por sessão, um `MultiClientPermissionMediator`, um adaptador `BridgeFileSystem` e auxiliares orientados a ACP (`spawnOrAttach`, `loadSession`, `resumeSession`, `sendPrompt`, `cancelSession`, `respondToPermission`, além de RPCs extMethod para status do workspace e reinício do MCP). Bridges e filhos nunca são compartilhados entre runtimes de workspace.
...
- `defaultEntry` é a Session lógica reutilizável sob `sessionScope: 'single'`. O fechamento de Session remove apenas seu lease de Session. Após todo o trabalho de Session, restauração, controle de workspace, descoberta, autenticação e operações de runtime ser drenado e nenhuma janela de keepalive de ensure explícito estar pendente, um `channelIdleTimeoutMs` omitido ou zero recolhe o processo filho imediatamente. O pré-aquecimento simples é preservado para o primeiro uso e não arma esse recolhimento imediato por si só. Um valor positivo ou janela de keepalive ativa atrasa o recolhimento; o atraso remanescente maior vence.
...
| `@qwen-code/qwen-code-core` — `ApprovalMode`, `TrustGateError`                     | `packages/channels/base/` (planejado, F4)        |
...
| `channelIdleTimeoutMs`                        | `0`                                                | Atraso de auto-reap; o pré-aquecimento simples sobrevive à sua primeira leitura de status, e o keepalive ativo pode estendê-lo.                                                           |

...
- `working_dir` (string, opcional): Fixe um agente explicitamente nomeado e não-fork em uma git worktree registrada existente dentro do repositório atual. Lançamentos sem nome executam em foreground porque o chamador possui o ciclo de vida da worktree (veja `run_in_background`); um teammate nomeado fixado em uma executa concorrentemente e deve ser desligado antes que a worktree seja removida. Se ambos `working_dir` e `isolation` forem fornecidos, `working_dir` tem precedência.

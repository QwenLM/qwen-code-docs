...
| `permission_mode`          | `default`, `plan`, `auto-edit`, `auto`, `yolo`             | Modo de aprovação de execução de ferramentas. `auto` permite que um classificador LLM aprove chamadas de ferramentas; `yolo` aprova automaticamente todas as ferramentas; use apenas em ambientes confiáveis ou em sandbox. |

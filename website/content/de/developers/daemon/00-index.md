...
- [`13-sdk-daemon-client.md`](./13-sdk-daemon-client.md) - TypeScript SDK: `DaemonClient`, `DaemonSessionClient`, `DaemonAuthFlow`, SSE-Parser, Event-Reducer, `ui/*`-Transkript-Layer.
- [`14-cli-tui-adapter.md`](./14-cli-tui-adapter.md) - Gemeinsamer UI-Transkript-Layer und die Beziehung zum Legacy-CLI-TUI-Daemon-Adapter.
...
- **ACP child** - Der `qwen --acp`-Child, der die Agent-Runtime eines Workspace hostet. Die Produktion versucht, den vertrauenswürdigen primären Child aus Kompatibilitätsgründen vorzuwärmen; vertrauenswürdige sekundäre Workspaces starten bei ihrem ersten Runtime-Kommando oder ihrer ersten Session, und nicht vertrauenswürdige sekundäre Workspaces starten kein ACP. Legacy-Primary-Routen behalten ihr bestehendes Kompatibilitätsverhalten bei. Die owning Bridge multiplext Sessions und Clients auf diesen Child.
...
| ----------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------ |
| Bootstrap und HTTP-Assembly         | `packages/cli/src/serve/run-qwen-serve.ts`, `packages/cli/src/serve/server.ts`, `packages/cli/src/serve/routes/health.ts`, `packages/cli/src/serve/web-shell-static.ts`                                                                                                | [`02`](./02-serve-runtime.md), [`20`](./20-quickstart-operations.md)                                                   |
...
| Gemeinsamer UI-Transkript-Layer     | `DaemonUiEventType`, `DaemonSessionProvider`, `packages/web-shell/client/daemon/`                                                                                                                                                                                           | [`13`](./13-sdk-daemon-client.md), [`14`](./14-cli-tui-adapter.md), [`../daemon-ui/README.md`](../daemon-ui/README.md) |
...
- **Web-UI-Produktdetails** - Der gemeinsame Transkript-Layer und die Web-UI-Daemon-Entry-Points werden hier behandelt, aber das Produkt-UI-Layout wird in `docs/developers/daemon-ui/` und den Adapter-Design-Notizen verfolgt.
...
| Auth / Netzwerk-Guardrails| Token-loser Loopback gewährt dem primären Listener lokale Operator-Autorität; Non-Loopback erfordert Bearer; `--require-auth` erweitert Bearer auf Loopback und `/health`; Host-Allowlist und Standard-CORS-Deny sind aktiv. | [`12`](./12-auth-security.md), [`17`](./17-configuration.md)              |
...
| Event-Schema    | `EVENT_SCHEMA_VERSION = 1`; 53 bekannte Event-Typen; ID-lose Subscriber-Synthetic-Frames; `_meta.serverTimestamp` gestempelt durch `EventBus.publish()` (mit `formatSseFrame()`-Fallback für Synthetic-Frames). | [`09`](./09-event-schema.md), [`10`](./10-event-bus.md)                                                           |
| Capabilities    | `SERVE_PROTOCOL_VERSION = 'v1'`; registrierte und Conditional-Tags werden durch die maßgebliche Registry definiert.                                                                                                                             | [`11`](./11-capabilities-versioning.md)                                                                           |
| Session-Shell   | `POST /session/:id/shell` existiert hinter `--enable-session-shell`, Bearer- oder Trusted-Loopback-Autorität und session-gebundener `X-Qwen-Client-Id`; Capability-Tag ist conditional.                                               | [`11`](./11-capabilities-versioning.md), [`17`](./17-configuration.md), [`20`](./20-quickstart-operations.md)     |
...
| Gemeinsame UI-Transkript-Layer   | SDK `daemon/ui/*` normalisiert Daemon-Events in 42 semantische UI-Event-Typen, reduziert sie zu Transkript-Blöcken und stellt Renderer und Conformance-Helper bereit. | [`14`](./14-cli-tui-adapter.md), [`../daemon-ui/README.md`](../daemon-ui/README.md), [`../daemon-ui/MIGRATION.md`](../daemon-ui/MIGRATION.md) |
| Web-Shell-Daemon-Consumer       | `packages/web-shell/client/daemon/` konsumiert den SDK-Transkript-Store über React-Provider und Adapter.                                                         | [`14`](./14-cli-tui-adapter.md), [`../daemon-client-adapters/web-shell.md`](../daemon-client-adapters/web-shell.md)                                 |

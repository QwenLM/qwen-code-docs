...
| `--memory-budget-mb <n>`                | integer in `[1024, 1048576]`   | 50% des Cgroup/Host-Speichers                | Nur Beobachtung                          | Gesamtspeicherbudget für den Daemon-Prozessbaum, begrenzt auf den aufgelösten verfügbaren Speicher. Berichtet unter `limits.memory`; in eine Partition modelliert, auf die nichts angewendet wird.                                                                                                                                                  |
...
| Web-Shell-statische Asset-Bereitstellung                                                       | `packages/cli/src/serve/web-shell-static.ts`            | `mountWebShellAssets()`                                                        |

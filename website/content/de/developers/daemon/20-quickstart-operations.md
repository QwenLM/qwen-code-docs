...
| `--memory-budget-mb <n>`                | integer in `[1024, 1048576]`   | 50% des Cgroup/Host-Speichers                | Nur Beobachtung                          | Gesamtspeicherbudget für den Daemon-Prozessbaum, begrenzt auf den aufgelösten verfügbaren Speicher. Berichtet unter `limits.memory`; dimensioniert kein Child. |
...
| `QWEN_CODE_MEMORY_PROJECT_SCOPE`    | `workspace` schlüsselt Projektspeicher nach dem genauen Workspace-Verzeichnis auf; jeder andere Wert behält den `git-root`-Geltungsbereich bei (nicht erkannte Werte warnen einmal). `--memory-project-scope` hat Vorrang. |
...
Der Daemon konstruiert jede Workspace-Runtime aus den zusammengeführten Einstellungen und dem Umgebungs-Overlay des jeweiligen Workspaces. Prozessglobale Listener/Auth-Optionen werden einmal aufgelöst, während runtime-spezifische Dienste und ACP-Children das Snapshot der besitzenden Runtime erhalten. Fehlerhafte Einstellungen folgen dem dokumentierten Start-Fallback- oder Fehlerverhalten für die betroffene Runtime; sie dürfen nicht dazu führen, dass die Einstellungen eines anderen Workspaces wiederverwendet werden.

Boot ruft `loadSettings(boundWorkspace)` einmal pro Runtime auf:

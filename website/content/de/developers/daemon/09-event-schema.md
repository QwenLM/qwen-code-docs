...
Skill-Toggle-APIs hängen optionale `mutation: { id, kind: 'skill_toggle', skills: [{ name, enabled }], activation, sessionsRefreshed, sessionsFailed }` an. Jedes `skills.disabled` / `skills.enabled`-Event desselben Requests teilt sich eine Mutations-ID. Andere Settings-Writes lassen `mutation` weg. Workspace-Service-Writes enthalten `scope`; einige andere Emitter (zum Beispiel Session-Model-Switches) lassen es weg. Der SDK-Normalizer setzt fehlenden `scope` standardmäßig auf `'workspace'`.
...
| Typ                  | Richtung | Trigger                                                              | Wichtige Payload-Felder                                                                                                                               |
| -------------------- | -------- | -------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------ |
| `extensions_changed` | S->C     | Hintergrund-Installation/Aktualisierung von Extensions abgeschlossen oder Statusänderung | `refreshed, failed, status?: 'installed' \| 'enabled' \| 'disabled' \| 'updated' \| 'uninstalled' \| 'failed', source?, name?, version?, error?`. Ein Daemon, der `extension_activation_explicit_refresh` bewirbt, committet die Aktivierung ohne sie zu broadcasten, sodass eine erfolgreiche Aktivierung nicht mehr `enabled`/`disabled` emittiert; diese stammen von älteren Daemons, und neuere konvergieren die Aktivierung über einen statuslosen Refresh-Broadcast. |
...
| `turn_complete`       | S->C     | Ein Turn wurde erfolgreich abgeschlossen                                                                                       | `sessionId, stopReason, promptId?, branchPoint?`. `promptId` verknüpft mit nicht-blockierenden Prompt-Antworten (`202`). In Frage kommende abgeschlossene Turns enthalten `branchPoint: { assistantRecordUuid, checkpointUuid }`. |
| `turn_error`          | S->C     | Ein Turn ist fehlgeschlagen                                                                                                       | `sessionId, message, code?, promptId?`; derselbe `promptId`-Korrelationsmechanismus. Für agentseitige `-32603`-Failures trägt `message` die eigenen Details des Providers, wenn der Agent welche angegeben hat (`data.details` / `data.message` / verschachtelt `data.error.message`), nicht den generischen JSON-RPC-Text.                                                                                                                   |
...
- `settings_changed` / `settings_reloaded` - werden von `asKnownDaemonEvent` erkannt; der Session-Reducer pflegt keine dedizierten View-State-Felder dafür. Skill-Toggle-`settings_changed`-Events tragen optionale `mutation`-Metadaten, damit Hosts Skill-spezifische Änderungen inkrementell anwenden können, statt den Task neu zu laden. Andere UIs können das Event weiterhin als Refresh-Signal behandeln.

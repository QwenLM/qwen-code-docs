...
Jeder unabhängige Controller sollte eine eigene, stabile ID verwenden. Web Shell bewahrt das historische `webui_`-Präfix aus Kompatibilitätsgründen. Ein Host und eine eingebettete WebShell sollten sich eine ID nur teilen, wenn sie absichtlich als ein logischer Controller agieren; sobald sie geteilt wird, können Daemon-Logs nicht unterscheiden, welcher der beiden einen Request ausgelöst hat.
...
Für eine persistierte Part-4A-Worktree-Session ist die Wiederherstellung eine integritätsgesicherte Erweiterung dieses Lifecycles. Der Sidecar identifiziert explizit das angeforderte Workspace-Root, der Checkout muss kanonisch unter dem entsprechenden `.qwen/worktrees/`-Verzeichnis enthalten sein, und sein Marker muss eine Single-Link-Regular-File sein, die die exakt wiederhergestellte Session-ID enthält. Der Daemon reloziert ein idle wiederhergestelltes Child erst nach diesen Prüfungen; ein aktives Child wird nur akzeptiert, wenn sein gemeldetes Cwd bereits dem Worktree entspricht, und ein aktives Child, dessen gemeldetes Cwd fehlt oder sich woanders befindet, schlägt fail-closed statt unter seinem Prompt reloziert zu werden – außer bei einem Cold Restore, der seinen Restore-Prompt nicht aufschieben konnte (`suppressWorktreeContextRestore` ausgeschaltet, sodass die Bridge die erneut eingehängte Frage abgefeuert hat, statt sie zu parken): diese Form behält das Pre-4B-Ergebnis bei, gibt kanonische Worktree-Metadaten ohne `worktreeState` zurück, nicht reloziert, wobei die Session überlebt. Die relozierten und akzeptierten Responses geben kanonische Worktree-Metadaten mit `worktreeState: "persisted-v1"` zurück. Ein Sidecar mit `supersededBy` wird niemals wiederhergestellt: die Route gibt `409 worktree_session_superseded` mit der Ersatz-Session-ID zurück, eine Klassifizierung, die allein aus diesem Link vor jedem Marker-Read entschieden wird, sodass Caller umleiten und ihr Bookkeeping erst heilen, sobald ein Load dieser ID erfolgreich war – ein Pre-Commit-unterbrochener Transfer benennt einen Ersatz, der nicht der Marker-Owner ist, nicht selbst wiederhergestellt werden kann und durch den wiederholten Reset gereapt wird; ein wiederhergestellter Ersatz, dessen `supersedes`-Link mit dem alten Sidecar übereinstimmt, während der Marker sich nie bewegt hat – oder fehlt – gibt `409 worktree_reset_interrupted` zurück, dessen Reparatur darin besteht, den Reset gegen die ersetzte Session zu wiederholen; und ein fehlender Marker ohne dieses übereinstimmende Link-Paar gibt `409 worktree_marker_missing` zurück, dessen Reparatur darin besteht, den Task zurückzusetzen statt den Restore zu wiederholen, da kein Restore-Pfad einen Marker neu erstellt. Die unterbrochene Klassifizierung wird zuerst geprüft. Ungültiger Part-4A-State trennt ein bestehendes Attachment oder tötet einen Cold Restore mit `requireZeroAttaches`; ein fehlender Sidecar liefert ebenfalls keine Attestierung. Wann immer die effektive Restore-Quelle Channel-owned ist, unterdrückt die Route das Best-Effort-Cleanup des ACP-Agenten für Part-4A- oder nicht klassifizierbaren Sidecar-State, sodass ein Validierungsfehler die unsicheren Checkout-Beweise bewahrt. Persistierte Quell-Metadaten haben Vorrang; wenn sie fehlen, liefert der Load/Resume-Request die effektive Quelle. Ein strukturell gültiger Legacy-Sidecar ohne `workspaceCwd` behält stattdessen das bestehende Best-Effort-Agenten-Restore bei: er muss entweder das angeforderte Workspace-Root oder dessen Git-Repository-Top-Level identifizieren, wird ohne Marker-Attestierung auf Containment geprüft, kann vom Agenten bereinigt werden und kann `worktree` ohne `worktreeState` zurückgeben. Abgesehen von diesem expliziten Legacy-Kompatibilitätsfall behalten nur Sessions, deren effektive Restore-Quelle nicht Channel-owned ist, das bestehende Best-Effort-Cleanup vor der Route-Validierung.

Worktree-Ownership-Transfer (`POST /session/:id/worktree-reset`, beworben durch `session_worktree_reset_v1`) erweitert diesen Lifecycle für Channel-Task-Resets: der Daemon spawnt einen frischen Thread-scoped Ersatz im Root-Workspace, reloziert ihn in den verifizierten Checkout, verlinkt das Sidecar-Paar (`supersededBy` zuerst auf der alten Session, dann `supersedes` auf dem Ersatz), kippt den Marker auf den Ersatz unter einem Per-Checkout-Route-Lock und einer Admission-Barriere, die Prompt-Admission plus die sieben anderen Writer fence, die im Checkout Arbeit aufnehmen oder die Session-Cwd bewegen (Rewind, Cwd-Änderung, Branch, Fork, Shell, Goal-Control, Workflow-Task-Action), während die Release- und Stop-Pfade designbedingt nicht gefenced sind, und trennt dann die Client-Registrierungen und In-Memory-Worktree-Assoziation der ersetzten Session. Die Trennung berichtet, ob die ersetzte Session tatsächlich verschwunden ist: ein Überlebender, dessen Child noch Hintergrundarbeit hält, hält die Barriere aktiv, wird geloggt und wird dem Caller als `supersededSessionLive: true` gemeldet, statt von der Success-Response überdeckt zu werden. Ein gefenced Writer, der während des Transfers auf der ersetzten Session zugelassen wird, wird mit `409 worktree_reset_active` abgelehnt; die vollständige Fehler-Taxonomie, einschließlich welche Crash-Windows ein Retry zurückrollt und welche fail-closed für Operator-Reparatur gehen, ist bei der Route in `qwen-serve-protocol.md` dokumentiert.
...
Leere, beschädigte und verwaiste reguläre Transkriptdateien bleiben für diese Lifecycle-Operationen qualifiziert, auch wenn sie nicht als Konversationen geladen werden können. Ownership-Sicherheitsprüfungen können absichtlich fail-closed ausfallen und einen Operator-Eingriff erfordern. Eine Datei, die geändert wurde, nachdem ein Writer seinen zertifizierten Handoff-Nachweis versiegelt hat, schlägt mit `SessionTranscriptChangedError` fehl, bis der Operator das versiegelte Lock und die geänderten Bytes auflöst. Ein JSON-förmiger erster physischer Datensatz, der das begrenzte Ownership-Lesefenster überschreitet, schlägt mit `SessionTranscriptIdentityUnavailableError` fehl, bis der Datensatz repariert oder verkleinert wird; übergroße beschädigte Datensätze mit einem Nicht-Objekt-Präfix bleiben qualifiziert. Ein parsbarer wiederhergestellter Datensatz muss String-`sessionId`- und `cwd`-Ownership-Felder enthalten, und gemischte lokale/fremde Archiv-Zustände fallen ebenfalls fail-closed aus. Wenn `session_storage_conflict_repair` beworben wird, akzeptieren Archive und Unarchive `resolveConflicts: true`: Archive behält die archivierte Kopie, während Unarchive die aktive Kopie behält. Ohne diese Option verschieben, entfernen oder überschreiben Active/Archive-Konflikte keine der persistenten Kopien und werden im Batch-`errors`-Array zurückgegeben. Archive schließt weiterhin strikt eine Live-Session, bevor es den Konflikt klassifiziert, was Warteschlangen-Datensätze in das aktive Transkript flushen kann. Workspace-qualifizierte Lifecycle-Routen verwenden jetzt dieses HTTP-`200`-Batch-Envelope statt ihrer früheren HTTP-`409 session_conflict`-Response.

...
| `file_too_large`          | 413       | Großem Text fehlt ein endliches Zeilenlimit, großer Text ist nicht UTF-8, oder ein Schreibvorgang überschreitet `MAX_WRITE_BYTES` (5 MiB). | Verwende ein endliches Zeilenlimit für großen UTF-8-Text, verwende ein Bytefenster oder teile den Schreibvorgang. |
| `hash_mismatch`           | 409       | Optimistische Nebenläufigkeit `expectedSha256` fehlgeschlagen oder eine Datei hat sich während eines Stable-Reads geändert. | Lese die Datei erneut und wiederhole mit der neuen Version/dem neuen Hash. |
...
| ------------------------------------- | ---- | ---------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `SessionNotFoundError`                | 404  | sessionId nicht in `byId` (`code: "session_not_found"`) oder die Session wird gerade geschlossen (`code: "session_closing"`). | Für `session_not_found`: neu erstellen oder anhängen; die Session könnte bereits bereinigt worden sein. Für `session_closing`: warten und erneut versuchen; ein gleichzeitiges Schließen läuft. DELETE-Routen behandeln `session_closing` als idempotenten Erfolg. |
...
| `RestoreInProgressError`              | 409  | `loadSession`, `resumeSession` oder eine aufruferseitig angegebene ID bei `POST /session` kollidiert mit einer anderen Registrierung, die dieselbe ID bereits besitzt. | Warte die angegebene Verzögerung ab und wiederhole den angeforderten Restore oder Spawn; die Cleanup-Phase verlassener Restores trägt einen vom Budget abgeleiteten Backoff.         |
...
| `restore_timeout`         | ACP-Session-Load/Resume hat sein dediziertes Restore-Budget überschritten. |

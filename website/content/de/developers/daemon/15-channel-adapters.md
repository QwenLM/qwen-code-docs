...
Der langlebige `ChannelWorkerManager` besitzt die feste Daemon-Auswahl und die nach Workspace gruppierten Supervisoren. Eine explizite `--channel`-Auswahl hat Vorrang und bleibt fail-fast. Bei einem Boot ohne Flags wird die `serve.channels`-Einstellung des vertrauenswürdigen Primary-Workspaces wiederhergestellt; dies lädt dynamisch die Channel-Runtime, reserviert die Service-PID-Datei, löst die Workspace-Ownership auf und startet die ausgewählten Worker. Sekundäre Workspaces stellen ihre eigenen `serve.channels` nicht unabhängig wieder her. Ohne eine dieser beiden Startup-Quellen führt das erste strikt-gatede `PUT /workspace/channel` eine Lazy-Initialisierung durch. `GET /workspace/channel` liest den Manager-Snapshot und `DELETE /workspace/channel` stoppt ihn idempotent. SDK-Helper sind `getChannelWorkerControl()`, `setChannelWorkerSelection()` und `stopChannelWorker()`; der CLI-Einstieg ist `qwen channel set` plus die Remote-Varianten `status` und `stop`.

Gespeicherte Startup-Namen dürfen nicht leer sein, kein führendes oder abschließendes Whitespace enthalten und keine unsicheren Steuer- oder unsichtbaren Zeichen enthalten. Ungültige Einträge werden individuell übersprungen und nach Array-Index protokolliert; der Daemon trimmt sie weder in einen anderen Instanznamen noch schreibt er die Settings um. Worker-Argumente verwenden `--channel=<value>`, wodurch ein führendes Minuszeichen im Namen erhalten bleibt. Das Channel-Management meldet weiterhin die persistierten Startup-Einstellungen und den tatsächlichen Runtime-State.

Ein ungültiges Startup-Feld oder ein Validierungs- oder Lease-Fehler vor dem Start der Worker überspringt die automatische Wiederherstellung und protokolliert seine `serve.channels`-Quelle, ohne andere Einstellungen zu verwerfen. Nach einem fehlgeschlagenen Worker-Startup fährt der Daemon nur fort, wenn die Bereinigung erfolgreich ist. Globale Runtime-Startup-Timeouts und unbestätigte Worker-Stopps folgen dem bestehenden Startup-Failure-Pfad; die Service-Lease bleibt gehalten, während die Worker-Terminierung unbestätigt ist. Untersuche das Daemon-Log für übersprungene oder fehlgeschlagene Wiederherstellungsversuche.
...
| Knob                                     | Effekt                                                                                                                              |
| ---------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------- |
| `sessionScope`                           | `'user'` (Sender + Chat), `'chat_thread'` (Channel + chatId + threadId) oder `'single'` (eine gemeinsame Session pro Channel). Das veraltete `'thread'` bleibt erhalten, wenn es bereits konfiguriert ist, wird für neue Web-Shell-Konfigurationen aber nicht mehr angeboten.                                                                                                                                                              |
| `multiSession`                           | Nur im Daemon verfügbare benannte Tasks für `sessionScope: 'user'`. Der Owner-Katalog wird unterhalb des Workspace-/Channel-State-Verzeichnisses persistiert; Tasks können nebenläufig laufen, können sich für eigene Git-Worktrees pro Task entscheiden, Abbruch- und Berechtigungsbefehle bleiben exakt dem jeweiligen Task zugeordnet, und Ergebnisse sowie interaktive Oberflächen weisen ihren Ursprungs-Task aus. Webhooks, das Nachladen von Gruppenverläufen und Loops bleiben ausgeschlossen. |
| `approvalMode`                           | `'auto'` (automatisch antworten) / `'prompt'` (UI rendern).                                                                                                                                                                                                                                                                                                                                                 |
| `allowlist?: string[]`                   | Erlaubte Sender-IDs; fehlt der Eintrag, ist der Zugang offen.                                                                                                                                                                                                                                                                                                                                                               |
| `denylist?: string[]`                    | Gesperrte Sender-IDs.                                                                                                                                                                                                                                                                                                                                                                                |

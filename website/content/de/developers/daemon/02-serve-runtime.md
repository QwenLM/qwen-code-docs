...
- Unsichere oder ungültige Startkonfigurationen ablehnen: Non-Loopback-Bind ohne Token, `--require-auth` ohne Token, Wildcard- oder Non-Loopback-HTTP(S)-`--allow-origin` ohne Token, `mcpBudgetMode='enforce'` ohne positives `mcpClientBudget`, ein nicht existierender oder kein Verzeichnis-`--workspace` sowie ungültige Timeout- oder Rate-Limit-Werte.
...
| `hostAllowlist(bind, getPort)` | Auf Loopback validieren, dass `Host` zu `localhost`, `127.0.0.1`, `[::1]`, `host.docker.internal` oder der exakt gebundenen Loopback-Adresse gehört, plus dem tatsächlichen Port; Port-lose Formen werden auf den Ports 80 und 443 akzeptiert. | Schutz gegen DNS-Rebinding. Der Vergleich ist case-insensitive und wird pro Port gecacht. Der Local-Control-LAN-Listener erzwingt immer seine Advertised-Authority-Hostprüfung, unabhängig vom primären Bind. |
...
8. **CORS-/Timeout-/Rate-Limit-Validierung**: Wildcard- und Non-Loopback-HTTP(S)-`--allow-origin`-Werte erfordern ein Token; Prompt-, Writer-, Channel-Idle-, Session-Idle-, Reaper- und Rate-Limit-Window-Werte schlagen bei Ungültigkeit sofort fehl (fail fast).

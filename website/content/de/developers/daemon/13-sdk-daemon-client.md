...
Das terminale Operationsergebnis enthält sortierte `results`. Targets müssen beim Setzen von `enabled` oder `disabled` nicht installiert sein: Der Daemon speichert eine Namensdeklaration und behält diese Aktivierungsrichtlinie bei, wenn später eine Extension mit diesem Namen installiert wird. Alle geänderten Targets teilen sich eine Extension-Store-Generation. Wenn `extension_activation_explicit_refresh` angekündigt wird, enden Aktivierungsoperationen nach dem dauerhaften Policy-Commit ohne aktive Sessions zu refreshen. Ein Caller, der sofortige Anwendung benötigt, sollte dann `workspace.refreshExtensionRuntime()` für jeden Workspace absetzen, dessen Sessions die Änderung umgehend anwenden müssen; der Refresh ist eine separate Operation und kann abgewartet oder im Hintergrund belassen werden. Ein globaler Standard-Batch ändert die Standardaktivierung, die jeder registrierte Workspace erbt, es sei denn, dieser Workspace hat ein exaktes Override für den Namen (oder matcht eine Legacy-Pfad-Regel), und er hat keinen einzigen Refresh, der jede Runtime abdeckt; ein Workspace-Batch ändert nur die ausgewählte vertrauenswürdige Runtime. Ältere Daemons refreshen bereits innerhalb der Aktivierungsoperation, daher dürfen Clients den zusätzlichen Refresh nicht einreichen, es sei denn, die Capability liegt vor. Der 30-Sekunden-Generation-Reconciler bleibt ein unabhängiger Eventual-Convergence-Pfad für Workspaces, die der Caller nicht refreshed hat. Workspace `inherit` löscht das exakte Override, erstellt aber keine Deklaration für einen unbekannten Namen; ein All-Unknown-Clear gelingt als No-Op. Einzelne Aktivierungsmethoden bleiben auf installierte beschränkt.
...
Der erste Produktions-Consumer ist `packages/web-shell/client/daemon/` über Reacts `DaemonSessionProvider`. Siehe [`14-cli-tui-adapter.md`](./14-cli-tui-adapter.md) für die detaillierte Architektur, das Glossar, die Selektoren-Tabelle und die Beziehung zum Legacy `DaemonTuiAdapter`.
...
Wenn `workspace_session_live_state` angekündigt wird, liest `client.getWorkspaceSessionLiveState(workspaceCwd)` oder die skopierten `client.workspaceById(workspaceId).getSessionLiveState()` / `client.workspaceByCwd(workspaceCwd).getSessionLiveState()` den speicherbasierten Live-Session-Snapshot plus die Katalogversion des ausgewählten vertrauenswürdigen Workspace und gibt `DaemonWorkspaceSessionLiveState` zurück (`{ v: 1, catalogVersion: DaemonSessionCatalogVersion, sessions: DaemonSessionLiveState[] }`). Diese Methoden verwenden immer natives REST mit Bearer-Authentifizierung und einem kodierten Workspace-Selektor, bewahren die optionale Client-Identität und verwenden das bestehende Kurz-Request-Timeout. Sie rufen nicht `requireCapability()` auf – ein Capability-Probe bei jedem Poll würde das Anfragevolumen verdoppeln – daher pre-flighten Consumer `workspace_session_live_state` einmal aus ihren bereits geladenen Capabilities und fallen auf das bestehende Katalog-Polling zurück, wenn das Tag fehlt. Leite die Unterstützung nicht von `workspace_qualified_rest_core` ab. Jeder `DaemonSessionLiveState` trägt einen optionalen `updatedAt`-Aktivitätswasserstand, der es einem Consumer ermöglicht, die Aktualität einer bereits vorhandenen Katalogzeile zu aktualisieren, statt den Katalog nach einem abgeschlossenen Turn neu zu laden; er fehlt vor dem ersten laufenden Turn-Terminal in der aktuellen Bridge und nach einem Daemon- oder Runtime-Austausch, daher muss ein Consumer sein bestehendes Katalog-Fallback für einen fehlenden Wert beibehalten, statt Abwesenheit als nicht unterstützt zu behandeln.

...
| `logPrompts`                      | `QWEN_TELEMETRY_LOG_PROMPTS`                         | `--telemetry-log-prompts` / `--no-telemetry-log-prompts` | Benutzer-Prompt-Inhalte und API-Request/Response-Text in Telemetrie-Logs einschließen                                                                                                              | `true`/`false`    | `true`                  |
...
2. **Log-to-Span-Bridge-Spans** (werden verwendet, wenn HTTP-Traces ohne Log-Endpunkt exportiert werden) behalten `function_args`, `error`, `error.message` und `error_message`, sowie `prompt`, `request_text` und `response_text` wenn `logPrompts` ebenfalls aktiviert ist, anstatt diese Attribute zu verwerfen.
...
- `qwen-code.api_request`: Ausgehende Anfrage an die LLM-API.
  - **Attribute**: `model` (string), `prompt_id` (string), `request_text` (string, optional — enthält Request-Inhalt nur, wenn `log_prompts_enabled` true ist; Log-to-Span-Bridge-Spans benötigen zusätzlich `includeSensitiveSpanAttributes`; undurchsichtiges `thoughtSignature`-Provider-Payload wird eingeschlossen, wobei seine Policy-Entscheidung in #11682 nachverfolgt wird), `subagent_name` (string, optional)

- `qwen-code.api_response`: Von der LLM-API empfangene Antwort.
  - **Attribute**: `response_id` (string), `model` (string), `status_code` (int/string, optional), `duration_ms` (int), `input_token_count` (int), `output_token_count` (int), `cached_content_token_count` (int), `thoughts_token_count` (int), `total_token_count` (int), `prompt_id` (string), `auth_type` (string, optional), `response_text` (string, optional — enthält sichtbaren Antwortinhalt nur, wenn `log_prompts_enabled` true ist; Log-to-Span-Bridge-Spans benötigen zusätzlich `includeSensitiveSpanAttributes`; es trägt keinen Inhalt für interne Prompt-IDs oder Antworten ohne sichtbaren Text), `subagent_name` (string, optional)

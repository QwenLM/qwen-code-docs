...
- `working_dir` (string, optional): Einen explizit benannten, Nicht-Fork-Agenten an einen vorhandenen registrierten Git-Worktree im aktuellen Repository anheften. Der Aufrufer besitzt den Worktree-Lifecycle, daher läuft dieser Modus im Vordergrund. Wenn sowohl `working_dir` als auch `isolation` angegeben sind, hat `working_dir` Vorrang.

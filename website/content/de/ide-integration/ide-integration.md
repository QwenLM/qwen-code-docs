...
Derzeit wird als einzige IDE [Visual Studio Code](https://code.visualstudio.com/) sowie andere Editoren unterstützt, die VS Code-Erweiterungen nutzen können. Informationen zum Aufbau der Unterstützung für weitere Editoren findest du in der [IDE Companion Extension Spec](./ide-companion-spec.md).
...
- **Native Diff-Ansicht:** Wenn Qwen Code-Änderungen vorschlägt, kannst du die Änderungen direkt in der nativen Diff-Ansicht deiner IDE betrachten. So kannst du die vorgeschlagenen Änderungen nahtlos überprüfen, bearbeiten und entweder akzeptieren oder ablehnen.
...
Wenn du Qwen Code in einem unterstützten Editor ausführst, erkennt es automatisch deine Umgebung und fordert dich auf, eine Verbindung herzustellen. Wenn du mit "Ja" antwortest, wird das notwendige Setup automatisch ausgeführt, einschließlich der Installation der Companion-Extension und der Aktivierung der Verbindung.

### 2. Manuelle Installation über CLI

Falls du die Aufforderung zuvor abgelehnt hast oder die Extension manuell installieren möchtest, kannst du den folgenden Befehl innerhalb von Qwen Code ausführen:

```
/ide install
```

Dadurch wird die richtige Extension für deine IDE gefunden und installiert.
...
> HINWEIS:
> Die „Qwen Code Companion“-Extension kann weiter unten in den Suchergebnissen erscheinen. Wenn du sie nicht sofort findest, versuche nach unten zu scrollen oder nach „Newly Published“ zu sortieren.
>
...
- Um die Verbindung zur IDE zu aktivieren, führe Folgendes aus:
  ```
  /ide enable
  ```
- Um die Verbindung zu deaktivieren, führe Folgendes aus:
  ```
  /ide disable
  ```

Wenn aktiviert, wird Qwen Code automatisch versuchen, sich mit der IDE-Companion-Erweiterung zu verbinden.

### Status prüfen

Um den Verbindungsstatus zu prüfen und den Kontext anzuzeigen, den die CLI von der IDE erhalten hat, führe Folgendes aus:
...
- **In einem Docker-Container:** Wenn du Qwen Code innerhalb eines Docker-(oder Podman-)Containers ausführst, kann die IDE-Integration sich weiterhin mit der VS Code-Erweiterung verbinden, die auf deinem Host-Rechner läuft. Der CLI ist so konfiguriert, dass er automatisch den IDE-Server unter `host.docker.internal` findet. Normalerweise sind keine besonderen Konfigurationen erforderlich, aber du solltest sicherstellen, dass dein Docker-Netzwerksetup Verbindungen vom Container zum Host zulässt.
...
- **Nachricht:** `🔴 Disconnected: Failed to connect to IDE companion extension for [IDE Name]. Please ensure the extension is running and try restarting your terminal. To install the extension, run /ide install.`
  ...
  - **Lösung:**
    ...
    2. Öffne ein neues Terminalfenster in deiner IDE, um sicherzustellen, dass die richtige Umgebung übernommen wird.

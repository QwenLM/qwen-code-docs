...
> [`gen-ai-arms-field-alignment.md`](./gen-ai-arms-field-alignment.md) ersetzt die Verwendung von `gen_ai.provider.name=qwen-code` und das temporäre `gen_ai.agent.id` in diesem Dokument. Das hier beschriebene Design für Lebenszyklus, Identität, Parenting und Verknüpfung von `qwen-code.subagent.*` bleibt gültig.
...
| QwenLogger-RUM-Erfassung (Aliyun-interne Statistiken)                                           | `qwen-logger.ts:573-574`                         | Beibehalten – RUM sieht keine OTel-Spans, nur LogRecords                                   |

...
Verwende `read_many_files`, um Inhalte aus mehreren Dateien zu lesen, die durch Pfade oder Glob-Patterns spezifiziert werden. Das Verhalten dieses Tools hängt von den übergebenen Dateien ab:
...
**Hinweis:** `read_many_files` sucht nach Dateien anhand der angegebenen Pfade oder Glob-Patterns. Ein Verzeichnispfad wie `"/docs"` liefert ein leeres Ergebnis zurück; das Tool benötigt ein Pattern wie `"/docs/*"` oder `"/docs/*.md"`, um die relevanten Dateien zu identifizieren.
...
- `exclude` (list[string], optional): Glob-Patterns für Dateien/Verzeichnisse, die ausgeschlossen werden sollen (z. B. `["**/*.log", "temp/"]`). Diese werden zu den Standard-Ausschlussmustern hinzugefügt, wenn `useDefaultExcludes` auf `true` gesetzt ist.
- `include` (list[string], optional): Zusätzliche Glob-Patterns, die explizit eingeschlossen werden sollen. Diese werden mit `paths` zusammengeführt (z. B. `["*.test.ts"]`, um Testdateien gezielt hinzuzufügen, wenn diese zuvor allgemein ausgeschlossen wurden, oder `["images/*.jpg"]`, um bestimmte Bildtypen einzuschließen).
...
- `useDefaultExcludes` (boolean, optional): Legt fest, ob eine Liste von Standard-Ausschlussmustern angewendet werden soll (z. B. `node_modules`, `.git`, binäre Dateien, die keine Bilder oder PDFs sind). Standardwert ist `true`.
...
- Für Bilder und PDFs: wenn diese explizit per Name oder Erweiterung angefordert wurden (z. B. `paths: ["logo.png"]` oder `include: ["*.pdf"]`), liest der Tool die Datei und gibt ihren Inhalt als base64-kodierten String zurück.
- Der Tool versucht, andere Binärdateien (also solche, die weder gängigen Bild-/PDF-Typen entsprechen noch explizit angefordert wurden) durch Prüfung auf Null-Bytes im Anfangsinhalt zu erkennen und zu überspringen.
...
Lese die Haupt-README, alle Markdown-Dateien im `docs` Verzeichnis und ein bestimmtes Logo-Bild, ausgenommen eine spezifische Datei:

```
read_many_files(paths=["README.md", "docs/**/*.md", "assets/logo.png"], exclude=["docs/OLD_README.md"])
```

Lese alle JavaScript-Dateien, aber schließe Testdateien explizit ein sowie alle JPEGs in einem `images` Ordner:
...
- **Umgang mit Binärdateien:**
  ...
  - **Andere Binärdateien:** Das Tool versucht, andere Arten von Binärdateien zu erkennen und überspringt diese, indem es den Anfang der Datei auf Null-Bytes prüft. Solche Dateien werden aus der Ausgabe ausgeschlossen.
...
- **Pfadangaben:** Stelle sicher, dass Pfade und Glob-Muster korrekt relativ zum Zielverzeichnis des Tools angegeben sind. Bei Bild-/PDF-Dateien sollten die Muster spezifisch genug sein, um sie einzuschließen.
- **Standardmäßig ausgeschlossene Pfade:** Beachte die standardmäßig ausgeschlossenen Muster (wie `node_modules`, `.git`) und verwende `useDefaultExcludes=False`, wenn du diese überschreiben musst – aber tue dies mit Vorsicht.
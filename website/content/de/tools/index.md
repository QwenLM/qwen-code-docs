...
Qwen Code enthält integrierte Tools, die das Modell verwendet, um mit deiner lokalen Umgebung zu interagieren, Informationen abzurufen und Aktionen auszuführen. Diese Tools erweitern die Fähigkeiten der CLI und ermöglichen es, über die reine Textgenerierung hinauszugehen sowie bei einer Vielzahl von Aufgaben zu unterstützen.

## Überblick über Qwen Code Tools

Im Kontext von Qwen Code sind Tools spezifische Funktionen oder Module, die das Modell anfordern kann, um sie auszuführen. Wenn du zum Beispiel das Modell bittest, „die Inhalte von `my_document.txt` zu zusammenfassen“, wird es wahrscheinlich erkennen, dass es diese Datei lesen muss, und dann die Ausführung des `read_file`-Tools anfordern.

Die Kernkomponente (`packages/core`) verwaltet diese Tools, stellt deren Definitionen (Schemas) dem Modell zur Verfügung, führt sie bei Anforderung aus und gibt die Ergebnisse an das Modell zurück, damit diese weiter verarbeitet und in eine nutzerfreundliche Antwort umgewandelt werden können.
...
- **Befehle ausführen:** Mit Tools wie `run_shell_command` kann das Modell Shell-Befehle ausführen (mit entsprechenden Sicherheitsmaßnahmen und Nutzerbestätigung).
...
Um die Qwen Code Tools zu verwenden, übergibst du einen Prompt an die CLI. Der Prozess funktioniert wie folgt:
...
Viele Tools, insbesondere solche, die dein Dateisystem ändern oder Befehle ausführen können (`write_file`, `edit`, `run_shell_command`), sind mit Sicherheit im Hinterkopf konzipiert. Qwen Code wird typischerweise:
...
- **Sandboxing nutzen:** Alle Tools unterliegen Einschränkungen, die durch Sandboxing erzwungen werden (siehe [Sandboxing in Qwen Code](../sandbox.md)). Das bedeutet, wenn du in einer Sandbox arbeitest, müssen alle Tools (einschließlich MCP-Server), die du verwenden möchtest, _innerhalb_ der Sandbox-Umgebung verfügbar sein. Um beispielsweise einen MCP-Server über `npx` auszuführen, muss die `npx`-Executable innerhalb des Docker-Images der Sandbox installiert oder in der `sandbox-exec`-Umgebung verfügbar sein.
...
- **[MCP servers](./mcp-server.md)**: MCP-Server fungieren als Brücke zwischen dem Modell und deiner lokalen Umgebung oder anderen Services wie APIs.
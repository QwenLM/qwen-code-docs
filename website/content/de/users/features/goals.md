...
Zwei weitere Fenster sind verfügbar und standardmäßig ausgeschaltet: [`model.goalMaxTurns`](../configuration/settings.md) begrenzt, wie viele Turns ein Goal abschließen darf, einschließlich vom Benutzer gesteuerte Goal-Turns, und [`model.goalMaxActiveMinutes`](../configuration/settings.md) begrenzt die Echtzeit, während es in einem laufenden Prozess aktiv bleibt, einschließlich Warte- und Leerlaufzeit zwischen Turns. Zeit, während das Goal pausiert, blockiert oder gestoppt ist, zählt nicht gegen das zweite, ebenso wenig wie Ausfallzeit über einen Neustart hinweg; ein Prozess, der lediglich suspendiert ist, wird weiterhin belastet. Welches Fenster zuerst erschöpft ist, das Goal bekommt denselben einzigen Wind-Down-Turn zur Übergabe und stoppt dann, bis du es fortsetzt, und das Fortsetzen autorisiert ein weiteres Fenster oben auf das bereits verbrauchte. Nur das erschöpfte Fenster bewegt sich; die anderen bleiben, wo sie waren, und der `-1`-Opt-Out nimmt ebenfalls nur eine Obergrenze von einem Goal, das sie bereits verbraucht hat. Das Zeitfenster wird zwischen Turns gelesen statt durch einen Timer erzwungen, sodass ein Goal weit über es hinauslaufen kann, bevor es stoppt: ein bereits laufender Turn wird niemals unterbrochen, und ein Fenster, das erschöpft ist während kein Turn läuft, wird erst beim Ende des nächsten Turns bemerkt. Aktive Zeit wird zwischen aufgezeichneten Übergängen gezählt, sodass Zeit in einem Turn, den ein Neustart unterbrochen hat, nicht belastet wird. Änderungen an beiden Einstellungen wirken nach einem Neustart. Beide Fenster scharfen nur bei einem Goal, das nach der Änderung erstellt wurde, daher bedeutet die Begrenzung eines bereits protokollierten Goals, es durch `/goal set` zu ersetzen -- ein neues Goal bei Revision 1, mit zurückgesetzten Turn-, Token- und Aktivzeit-Messern und seinem Beleg-Fenster, das bei der Ersetzung beginnt, sodass Belege, die das alte Goal aufgezeichnet hat, nicht mehr zitierbar sind -- oder es zu löschen und von vorne anzufangen. Ein Fortsetzen oder Bearbeiten fügt niemals eine Obergrenze hinzu, die bei der Erstellung nicht scharf gemacht wurde.
...
| `Budget:`    | Beraterische Modell-Anweisung, wann aufgegeben werden soll, z. B. "stop as blocked after 20 turns". Um eine durchzusetzen, setze `model.goalMaxTurns` oder `model.goalMaxActiveMinutes` in den Einstellungen, nicht hier. |
...
`Budget` ist eine Anweisung an das Modell, wann es aufgeben und einen Blocker melden soll; das Modell kann sie befolgen oder auch nicht. Um die Runtime selbst bei einer Turn-Anzahl oder Dauer stoppen zu lassen, setze [`model.goalMaxTurns`](../configuration/settings.md) oder [`model.goalMaxActiveMinutes`](../configuration/settings.md). Beides in das Ziel zu schreiben konfiguriert diese nicht und ändert nicht das Token-Budget des Goals.

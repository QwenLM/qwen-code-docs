...
- Die `security.allowedHttpHookUrls`-Whitelist gilt weiterhin unabhängig davon. In verwalteten Umgebungen sollte dieser Schalter mit einer Whitelist kombiniert werden, sodass nur die beabsichtigten internen Endpunkte erreichbar sind. Eine Whitelist in Workspace- (Projekt-)Einstellungen wird nur berücksichtigt, wenn kein User-, System- oder SystemDefaults-Scope eine setzt; andernfalls wird sie ignoriert und als Warnung protokolliert, sodass ein Repository eingrenzen kann, wohin seine Hooks Daten senden, aber niemals eine von dir konfigurierte Whitelist ersetzen kann (eine leere Whitelist bedeutet "alle erlauben").
- HTTP-Hooks folgen niemals Redirects. Eine 3xx-Antwort wird wie jeder andere Nicht-2xx-Status behandelt: ein nicht-blockierender Hook-Fehler, und das Redirect-Ziel wird niemals kontaktiert.

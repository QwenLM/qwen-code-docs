...
Der Headless-Modus bietet eine Oberfläche ohne grafische Elemente für Qwen Code, die:

- Eingaben über Befehlszeilenargumente oder stdin akzeptiert
- Strukturierte Ausgaben zurückgibt (Text oder JSON)
- Datei-Umleitung und Weiterleitung unterstützt
- Automatisierungs- und Skript-Arbeitsabläufe ermöglicht
...
Gesprächskontext aus dem aktuellen Projekt in Headless-Skripten wiederverwenden:
...
# Eine bestimmte Sitzungs-ID direkt fortsetzen (ohne Benutzeroberfläche)
qwen --resume 123e4567-e89b-12d3-a456-426614174000 -p "Führen Sie den nachfolgenden Refactoring durch"
```

> [!note]
>
> - Sitzungsdaten sind projektspezifisches JSONL unter `~/.qwen/projects/<bereinigtes-cwd>/chats`.
> - Stellt Gesprächsverlauf, Tool-Ausgaben und Chat-Komprimierungs-Checkpoints wieder her, bevor die neue Eingabeaufforderung gesendet wird.
...
Das Stream-JSON-Format gibt JSON-Nachrichten sofort aus, sobald sie während der Ausführung auftreten, wodurch eine Echtzeitüberwachung ermöglicht wird. Dieses Format verwendet zeilengetrenntes JSON, bei dem jede Nachricht ein vollständiges JSON-Objekt in einer einzelnen Zeile ist.

```bash
qwen -p "Erkläre TypeScript" --output-format stream-json
```

Ausgabe (Streaming während des Auftretens von Ereignissen):
...
Der Parameter `--input-format` steuert, wie Qwen Code die Eingabe aus der Standardeingabe verarbeitet:
...
> **Hinweis:** Der Stream-json-Eingabemodus befindet sich derzeit in der Entwicklung und ist für die Integration in SDKs vorgesehen. Dazu muss `--output-format stream-json` gesetzt sein.
...
| `--continue`                 | Die letzte Sitzung für dieses Projekt fortsetzen    | `qwen --continue -p "Setzen wir dort fort, wo wir aufgehört haben"`      |
...
```bash
cat src/auth.py | qwen -p "Überprüfen Sie diesen Authentifizierungscode auf Sicherheitsprobleme" > security-review.txt
```

### Commit-Nachrichten generieren

```bash
result=$(git diff --cached | qwen -p "Schreiben Sie eine prägnante Commit-Nachricht für diese Änderungen" --output-format json)
...
```bash
result=$(cat api/routes.js | qwen -p "Generieren Sie ein OpenAPI-Spec für diese Routen" --output-format json)
echo "$result" | jq -r '.response' > openapi.json
```

### Stapelweiser Code-Analysis

```bash
for file in src/*.py; do
    echo "Analysiere $file..."
    result=$(cat "$file" | qwen -p "Mögliche Fehler finden und Verbesserungsvorschläge machen" --output-format json)
...
```bash
result=$(git log --oneline v1.0.0..HEAD | qwen -p "Erstellen Sie Versionshinweise aus diesen Commits" --output-format json)

...
| -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------- | ----------------------------------- | ---------------- |
| `low`    | 3–6 gezielte Inline-Blickwinkel über dem Diff (skaliert nach Diff-Größe) plus ein Gap-Sweep und ein Under-Floor-Re-Pass – keine Subagents, kein Build/Test, keine Projekt-Regeln                                                                            | 10 (unverifiziert)  | Keines                              | Niemals          |
...
Schritt 3C: low Effort: 3-6 Inline-Blickwinkel + Gap-Sweep + Under-Floor-Re-Pass
                                                               [0 Subagent-Aufrufe]
...
| Agent 6: Undirected Audit         | 3 parallele Personas (Angreifer / 3am-oncall / Maintainer) – erkennt übergreifende Probleme – plus der Counter-Frame-Audit (6d, PR-Targets bei High-Effort): liest die PR-Beschreibung nur, um seine nominierten Themen auszuschließen und den motivierenden Vorfall am Tag nach dem Merge zu replayen                                                                                                                                                                                                     |
...
| `prose-exec`: Prose-Ausführung        | Wenn das Diff eine Instruktionsdatei berührt (eine `SKILL.md`, `AGENTS.md`/`CLAUDE.md`/`QWEN.md`, eine Agent- oder Befehlsdefinition, eine `prompts/`-Datei) und das Review einen Tree hat: FÜHRT die geänderten Anweisungen in einer Einweg-Kopie aus und dokumentiert die Lücke zwischen dem Lauf und dem, was die Prosa verspricht  |

Die drei Correctness-Agents sind **prozedural**: jeder ist dadurch definiert, wie er das Diff durchläuft (Zeile für Zeile / gelöschte Zeilen / Cross-File-Kanten), nicht durch eine Bug-Taxonomie – ihre Abdeckung ist also komplementär statt überlappend. Zwei weitere dedizierte Blickwinkel (1d/1e) spalten die Sprach-Fallstricke-Checkliste und das Wrapper/Proxy-Routing aus dem Zeile-für-Zeile-Durchlauf ab: ein Checklist-Pattern-Match und eine strukturelle Routing-Erwartung sind unterschiedliche Aufmerksamkeitsmodi, und in den Durchlauf eingebettet wurden sie durch seinen Rhythmus verwässert. Dieselbe Begründung teilt **Codequalität in drei** (3a/3b/3c): ein Agent mit einer sechs Punkte umfassenden Checkliste erledigt einen Punkt – gemessen an einer stark umgeschriebenen Datei, ein Agent mit einer acht Punkte umfassenden Checkliste fand 1 von 5 Defekten, und dasselbe Modell, auf drei Teile aufgeteilt, fand alle 5 – die Qualitäts-Checkliste wird also dort geschnitten, wo die Fragen sich wirklich unterscheiden. Alle Agents laufen parallel (Agent 1 startet 3 prozedurale Varianten und 2 dedizierte Blickwinkel, Agent 3 startet 3 Checklisten-Scheiben und Agent 6 startet 3 Persona-Varianten und den Counter-Frame-Audit 6d gleichzeitig, was bis zu 17 parallele Tasks für Same-Repo-PR-Reviews ergibt – Agent 1e läuft nur, wenn der Diff einen Wrapping-Typ signalisiert – plus 0-2 Agent-8-Finder, wenn die Diff-Domäne sie erfordert, also praktisch 16-19, und einen weiteren (`prose-exec`), wenn das Diff eine Instruktionsdatei berührt; Agent 0 und 6d werden bei Local-Diff- und File-Path-Reviews übersprungen, die 14-17 laufen; der Cross-Repo-Lightweight-Modus überspringt auch Agents 1c und 7 und läuft mit 14-17 mit der PR-Identität und 12-15 ohne sie).
...
Verweise auf ein Repository, in das du pushen kannst – ein dediziertes Image-Host-Repository wird empfohlen; ein Fork oder Scratch-Repo funktioniert ebenfalls. Vermeide das Repository, das gerade reviewed wird: Bild-Branches, die dorthin gepusht werden, werden zu reachable Objects, die jeder Clone fetcht. Bilder landen auf dem `pr-assets/<pr>-review`-Branch mit Content-gehashten Namen, und Kommentare referenzieren sie über **Commit-gepinnte** URLs – unveränderlich, selbst wenn der Branch sich später bewegt, und unverändert funktional auf GitHub Enterprise.

Für GitHub-getriggerte Reviews (der PR-Review-Workflow) wird dieselbe Variable aus einer **Repository-Variable** desselben Namens gespeist, und der Workflow gibt nur einen dedizierten externen Host durch: bei nicht gesetzter Variable oder wenn sie auf das Repository unter Review zeigt (nach Trimmen und Case-Normalisierung verglichen, sodass gepaddete oder groß-/kleingeschriebene Schreibweisen desselben Repositories als Self-Targeting zählen), übergibt der Workflow einen leeren Wert und das Veröffentlichen verweigert – nichts ändert sich, und das Review behält seine Nachweise als Prosa und lokale Artefaktpfade. Ein Maintainer, der `QWEN_REVIEW_ASSETS_REPO` in den Actions-Variablen des Repos auf ein separates Image-Host-Repository setzt, ermöglicht es Review-Kommentaren, Capture-PNGs einzubetten. Ein externes Ziel verwaltet seine eigene Retention; der Visuals-Cleanup-Workflow räumt nur die historischen `pr-assets/*`-Branches auf, die frühere Git-basierte Publisher in diesem Repository hinterlassen haben.
...
| `candidateFloor` | Wann `low` einen deterministischen Recall-Re-Pass schuldet | `min(changed files, 4)`                                            |
...
Warum die Floors dort sind, wo sie sind: bei einem Neun-Zeilen-Typo-Fix sind sechs Inline-Durchläufe fünf Durchläufe über nichts, und der Sweep – ein frischer Leser, der jagt, was der erste Durchlauf nicht erreicht hat – hat nichts zu jagen, wenn der erste Durchlauf alles erreicht hat. Wenn ein Low-Effort-Durchlauf unter `candidateFloor` bleibt, macht er einen deterministischen zweiten Blick auf jeden coverbaren Hunk in der größten geänderten Quelldatei (ausweichend auf die größte coverbare Datei jeglicher Art) und jeden coverbaren entfernten Block. Dateien, die einen nicht coverbaren Chunk berühren, werden von der Zielauswahl ausgeschlossen, und der Durchlauf gibt immer ein Receipt aus, das sein Ziel, die New-Candidate-Anzahl und alle nicht coverbaren Chunks nennt; wenn keine Datei coverbar ist, prüft er trotzdem die coverbaren entfernten Blöcke und legt das fehlende Ziel offen. Der Floor ist ein Stopp-Signal, keine Quote: ein sauberes Diff kann auch nach diesem Re-Pass keine Ergebnisse melden. Agent 8s Floor ist der substanzielle: "eine Domäne dominiert das Diff" ist ein Urteil, und ein Urteil, das über vierzig Zeilen gefällt wird, findet jedes Mal eine dominante Domäne, weil vierzig Zeilen normalerweise alles eine Sache sind.
...
| -------------------------------- | ------------------------------ | -------------------------------------------------------------------------------------------------------------- |
| Review-Agents (Schritt 3)        | 17 (+0-2)                      | Laufen parallel; Agent 1e nur wenn der Diff einen Wrapping-Typ signalisiert (16 ohne ihn); Cross-Repo überspringt Agents 1c und 7 (15 während der Plan die PR-Identität trägt, 13 ohne sie – Agent 0 und 6d entfallen zusammen); Local/File überspringt Agent 0 und den Counter-Frame-Audit 6d (15); einen mehr (`prose-exec`) bei einem Diff, das eine Instruktionsdatei berührt, wenn das Review einen Tree hat |
...
| **Gesamt**                       | **~20-31 (~16-29)**            | 3A Same-Repo: ~20-31 (typisch ~20-22); Cross-Repo oder Local/File: ~16-29 (der Floor ist das identitätslose Cross-Repo-Roster von 13); ein weniger wenn Agent 1e nicht rostered, einen mehr wenn `prose-exec` geschuldet wird; 3B skaliert mit Chunks (siehe DESIGN.md) |

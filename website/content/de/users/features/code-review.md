...
**Aone Code:** Für einen Clone, dessen Origin auf `gitlab.alibaba-inc.com` liegt, führe `/review` innerhalb dieses Clones aus – die Plattform wird aus der Remote erkannt und die Lese-Subcommands funktionieren, unterstützt von der `a1`-CLI – die Zielnummer ist die globale MR-ID. `fetch-pr` holt `refs/merge-requests/<id>/head` und baut den Worktree + Diff, sodass das Agent-Review des Worktrees unverändert bleibt. In dieser Phase ist jeder Aone-Lauf kontextlos und mehrere Flows werden übersprungen (statt das gleichnamige Repo auf github.com zu treffen): `pr-context`/`comment-status`/`presubmit` haben keine Aone-Unterstützung (Fazit gedeckelt auf `COMMENT`), `test-plan` ist nicht unterstützt, Agent 0 wird übersprungen, und der `publish-assets`-Schreibvorgang wird übersprungen – mit `--comment` ebenfalls verweigert, ist ein Aone-Lauf in dieser Phase read-only gegenüber der Plattform; Ergebnisse landen in der Terminalausgabe und im gespeicherten Report. Siehe `docs/design/2026-08-15-review-aone-provider.md`.

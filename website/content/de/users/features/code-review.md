...
**GitHub Enterprise:** Das Reviewen einer PR-URL auf einem Nicht-`github.com`-Host routet jeden GitHub-Aufruf an diesen Host – die Review-Subcommands (`fetch-pr`, `pr-context`, `comment-status`, `presubmit`) akzeptieren `--host` und setzen ihn im Code, sodass ein vergessener Host das Review nicht stillschweigend auf `github.com` umleiten kann.

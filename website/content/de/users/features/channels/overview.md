...
Ein Task bleibt ausgewählt, um die nächste normale Nachricht zu empfangen, aber andere benannte Tasks können weiterhin gleichzeitig im gemeinsamen Arbeitsverzeichnis laufen. Das Erstellen oder Auswählen eines anderen Tasks bricht frühere Arbeiten nicht ab oder leitet sie um, und verspätete Ergebnisse behalten ihr ursprüngliches Task-Label. Ein beschäftigter Task kann nicht geschlossen werden, aber seine aktive Eingabeaufforderung kann mit `/session cancel [<name>]` über das bestehende Channel-Abbruchverhalten abgebrochen werden. Unabhängig Warteschlangen-Turns werden nicht abgebrochen, aber im `collect`-Dispatch-Modus werden Follow-ups, die hinter der abgebrochenen Eingabeaufforderung gepuffert sind, durch dieses bestehende Verhalten verworfen. Medienvorbereitung wird nicht gezielt durchgeführt. Nackte Berechtigungsbefehle gelten nur für den ausgewählten Task, während eine explizite Anfrage-ID einem besitzlosen inaktiven Task antworten kann. Pro-Task-Worktrees sind für Teil 4 geplant. Channel-Speicher bleibt auf den Chat bezogen, nicht auf einen benannten Task.
...
- `/session cancel [<name>]` – Die aktive Eingabeaufforderung des ausgewählten Tasks abbrechen oder einen anderen besitzlosen Task benennen; unabhängig Warteschlangen-Turns werden nicht abgebrochen, aber im `collect`-Modus gepufferte Follow-ups hinter der abgebrochenen Eingabeaufforderung werden durch das bestehende Abbruchverhalten verworfen; Medienvorbereitung wird nicht gezielt durchgeführt

...
| `messagePrefix`          | Nein             | Nur Benutzernachrichten dispatchen, die mit diesem exakten, case-sensitiven Präfix beginnen, nach führenden ` @mentions`; das Präfix und der folgende Whitespace werden vor dem Dispatch entfernt                                            |
...
Zwei Verhaltensweisen sind absichtlich und sollten bekannt sein, bevor du das Präfix aktivierst. Eine Sprachnachricht, deren Transkript DingTalk oder WeCom ausfüllt, zählt als Text, den der Benutzer gesprochen hat, also muss sie wie jede andere Nachricht das Präfix tragen und wird sonst verworfen – nur eine nicht transkribierte Sprachnachricht läuft als captionlose Medien. Und das Präfix wird vor dem Pairing geprüft, daher muss Erstkontakt von einem unbekannten Absender oder einer nicht genehmigten Gruppe ebenfalls das Präfix tragen; ohne diese Reihenfolge würde jede nicht-präfixierte Nachricht in einer belebten Gruppe eine Pairing-Antwort auslösen, was genau das Rauschen ist, das das Präfix unterdrücken soll. Sage neuen Benutzern das Präfix auf anderem Wege, oder lasse Pairing-Channels ohne Präfix.
...
Benannte Ergebnisse identifizieren ihren Ursprungstask: Direkte Chats verwenden `[task]`, während Gruppenchats `[sender · task]` verwenden. Benannte Textberechtigungsaufforderungen zeigen auch die genaue Anfrage-ID und die entsprechenden `/approve <id>`, `/approve-always <id>` und `/deny <id>` Befehle an. Das Label ist nur zur Darstellung und wird nicht im Modelltranskript gespeichert.

Ein Task bleibt ausgewählt, um die nächste normale Nachricht zu empfangen, aber andere benannte Tasks können weiterhin gleichzeitig laufen. `/session new <name>` teilt den konfigurierten Workspace, während `/session new <name> --worktree` einen isolierten Checkout für diesen Task unter dem `.qwen/worktrees/`-Verzeichnis des Daemon-Workspaces erstellt. Der Daemon überprüft den persistierten Worktree-Owner vor dem Wiedereröffnen des Tasks nach einem Neustart; ein fehlender, geänderter oder fremder Ownership-Datensatz schlägt fail-closed statt den Task still in den gemeinsamen Workspace zu verschieben. Das Erstellen oder Auswählen eines anderen Tasks bricht frühere Arbeiten nicht ab oder leitet sie um, und verspätete Ergebnisse behalten ihr ursprüngliches Task-Label. Ein beschäftigter Task kann nicht geschlossen werden, aber seine aktive Anfrage kann mit `/session cancel [<name>]` über das bestehende Channel-Abbruchverhalten abgebrochen werden. Unabhängig in Warteschlange eingereihte Turns werden nicht abgebrochen, aber im `collect`-Dispatch-Modus werden Follow-ups, die hinter der abgebrochenen Anfrage gepuffert sind, durch das bestehende Abbruchverhalten verworfen. Medienvorbereitung wird nicht gezielt durchgeführt. Nackte Berechtigungsbefehle gelten nur für den ausgewählten Task, während eine explizite Anfrage-ID einem owned inaktiven Task antworten kann. `/clear`, `/new` und `/reset` funktionieren ebenfalls bei einem ausgewählten Worktree-Task: der Task erhält eine frische Konversation, während sein Worktree und seine Dateien behalten werden. Ein beschäftigter Worktree-Task verweigert den Reset, bis seine Anfrage abgeschlossen ist, und ein Task, dessen Worktree-Datensatz beschädigt wurde, meldet den Fehlschlag, ohne Dateien anzurühren. Channel-Speicher bleibt auf den Chat bezogen, nicht auf einen benannten Task.
...
- `/btw <question>` – Eine Seitenfrage stellen, ohne die aktuelle Aufgabe zu unterbrechen; reine Textfragen bis 4096 Zeichen, erfordert eine Agenten-Verbindung mit Seitenfragen-Unterstützung
...
- `/session new <name> --worktree` – Einen Task in einem eigenen Git-Worktree erstellen und auswählen; nur im daemon-verwalteten benannten Task-Modus
- `/session use <name>` – Einen offenen Task auswählen oder einen geschlossenen Task wieder öffnen
- `/session cancel [<name>]` – Die aktive Anfrage des ausgewählten Tasks abbrechen oder einen anderen owned Task benennen; unabhängig in Warteschlange eingereihte Turns werden nicht abgebrochen, aber im `collect`-Modus gepufferte Follow-ups hinter der abgebrochenen Anfrage werden durch das bestehende Abbruchverhalten verworfen; Medienvorbereitung wird nicht gezielt durchgeführt
...
Dieser Modus startet workspace-gruppierte Channel-Worker-Prozesse, die `qwen serve` gehören. Worker verbinden sich über das SDK zurück mit dem Daemon und verwenden dieselben Channel-Adapter. Sie sind vom Daemon-Prozess getrennt, sodass ein Absturz eines Channel-Adapters nicht den Daemon zum Absturz bringt. Eine explizite `--channel`-Auswahl hat Vorrang und lässt den Daemon-Start fehlschlagen, wenn sie nicht bereit werden kann. Bei einem Start ohne Flags wird die `serve.channels`-Einstellung des vertrauenswürdigen primären Workspaces wiederhergestellt. Sekundäre Workspaces stellen ihre eigenen `serve.channels` nicht unabhängig wieder her. Ohne eine dieser Quellen lädt der Daemon keine Channel-Adapter und reserviert nicht die Lease bis zum ersten `qwen channel set`.

Die automatische Wiederherstellung überspringt ungültige Starteinstellungen sowie Validierungs- oder Lease-Fehler, die vor dem Start der Worker auftreten, und behält dabei nicht betroffene Einstellungen. Nach einem fehlgeschlagenen Worker-Start fährt der Daemon nur fort, wenn die Bereinigung erfolgreich war. Ein globales Laufzeit-Start-Timeout oder ein unbestätigter Worker-Stopp folgt weiterhin dem normalen Startfehlerpfad; die Service-Lease bleibt gehalten, solange die Worker-Beendigung unbestätigt ist. Prüfe das Daemon-Log auf Nachrichten, die `serve.channels` identifizieren, wenn ein Channel nicht wiederhergestellt wird.

Gespeicherte Startnamen dürfen nicht leer sein, kein führendes oder nachfolgendes Whitespace enthalten und keine unsicheren Steuer- oder unsichtbaren Zeichen enthalten. Ungültige Einträge werden individuell übersprungen und nach ihrem Array-Index protokolliert. Der Start benennt keine Instanzen um und schreibt die Konfiguration nicht um. Der Channel-Start-Toggle spiegelt die gespeicherte Einstellung wider; der Laufzeitstatus zeigt an, ob der Channel läuft.

...
Ein **einfacher Klick** auf einen http(s) OSC-8-Hyperlink im Viewport öffnet ihn in deinem Standardbrowser. Nicht-http(s)-Links (zum Beispiel `mailto:`) werden stattdessen in die Zwischenablage kopiert. **Rechtsklick** auf einen Link oder eine aktive Textauswahl öffnet ein In-App-Kontextmenü mit **Link öffnen**, **Link-Adresse kopieren** oder **Auswahl kopieren**. Das Menü kann mit den Pfeiltasten navigiert, mit `Enter` ausgeführt und mit `Esc` oder Klick außerhalb geschlossen werden. Setze `ui.mouseTracking` auf `false`, um zu verhindern, dass qwen-code die Maus vollständig erfasst; dadurch werden das native Rechtsklick-Menü des Terminals, OSC-8-Hyperlink-Klicks und die Textauswahl durch Klicken und Ziehen wiederhergestellt, aber der In-App-Viewport reagiert dann nicht mehr auf die Maus — verwende in diesem Fall die obigen Tastaturkürzel zum Scrollen.

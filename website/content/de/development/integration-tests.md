...
### Test-Ausgaben behalten

Du kannst die temporären Dateien, die während eines Testlaufs erstellt wurden, zur Überprüfung aufbewahren. Das ist besonders nützlich, um Probleme mit Dateisystemoperationen zu debuggen.

Um die Testausgaben zu behalten, setze die Umgebungsvariable `KEEP_OUTPUT` auf `true`.

```bash
KEEP_OUTPUT=true npm run test:integration:sandbox:none
```

Wenn die Ausgaben behalten werden, gibt der Test-Runner den Pfad zum eindeutigen Verzeichnis des Testlaufs aus.
...
Wenn du `VERBOSE=true` und `KEEP_OUTPUT=true` im selben Befehl verwendest, wird die Ausgabe sowohl in der Konsole gestreamt als auch in einer Logdatei innerhalb des temporären Testverzeichnisses gespeichert.
...
Um die Codequalität und Konsistenz sicherzustellen, werden die Integrationstest-Dateien als Teil des Haupt-Build-Prozesses gelintet. Du kannst den Linter auch manuell ausführen und automatisch Fehler beheben lassen.
...
Du kannst das Flag `:fix` hinzufügen, um alle behebbaren Linting-Fehler automatisch zu korrigieren:

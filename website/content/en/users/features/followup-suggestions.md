...
The interactive CLI and the daemon decide this separately, and they do not apply the same conditions: the CLI gates generation itself, in each of its renderers, while the daemon gates it server-side for every client attached to the session.

...
Используйте `save_memory`, чтобы сохранять и вспоминать информацию между вашими сессиями в Qwen Code. С помощью `save_memory` вы можете указать CLI запомнить ключевые детали между сессиями, обеспечивая персонализированную и целенаправленную помощь.
...
Инструмент добавляет переданный `fact` в ваш файл контекста в домашней директории пользователя (по умолчанию `~/.qwen/QWEN.md`). Имя этого файла можно изменить через параметр `contextFileName`.

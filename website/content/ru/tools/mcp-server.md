...
MCP сервер — это приложение, которое предоставляет инструменты и ресурсы CLI через Model Context Protocol, позволяя ему взаимодействовать с внешними системами и источниками данных. MCP сервера выступают в роли моста между моделью и вашей локальной средой или другими сервисами, такими как API.
...
- **Доступ к ресурсам:** Читать данные из определенных ресурсов (хотя CLI в основном ориентирован на выполнение инструментов).
...
- **Stdio Transport:** Запускает дочерний процесс и общается через stdin/stdout
...
Вы можете настроить MCP-серверы в вашем файле `settings.json` двумя основными способами: через объект `mcpServers` на верхнем уровне для определения конкретных серверов и через объект `mcp` для глобальных настроек, которые управляют обнаружением и выполнением серверов.
...
- **`mcp.allowed`** (массив строк): Список разрешенных имен MCP-серверов. Если задан, то будут подключаться только серверы из этого списка (соответствующие ключам в объекте `mcpServers`).
- **`mcp.excluded`** (массив строк): Список исключенных имен MCP-серверов. Серверы из этого списка подключаться не будут.
...
Каждая конфигурация сервера поддерживает следующие параметры:

#### Обязательные (один из следующих)

- **`command`** (string): Путь к исполняемому файлу для транспорта Stdio
- **`url`** (string): URL endpoint'а SSE (например, `"http://localhost:8080/sse"`)
- **`httpUrl`** (string): URL endpoint'а HTTP streaming
...
- **`includeTools`** (string[]): Список имен инструментов, которые нужно включить с этого MCP-сервера. Если указано, то будут доступны только перечисленные здесь инструменты (поведение allowlist). Если не указано, по умолчанию включаются все инструменты с сервера.  
- **`excludeTools`** (string[]): Список имен инструментов, которые нужно исключить с этого MCP-сервера. Указанные здесь инструменты будут недоступны модели, даже если они предоставлены сервером. **Примечание:** `excludeTools` имеет приоритет над `includeTools` — если инструмент присутствует в обоих списках, он будет исключен.  
- **`targetAudience`** (string): OAuth Client ID, добавленный в allowlist IAP-защищенного приложения, к которому вы пытаетесь получить доступ. Используется с `authProviderType: 'service_account_impersonation'`.  
- **`targetServiceAccount`** (string): Email адрес Google Cloud Service Account, от имени которого будет выполняться имперсонация. Используется с `authProviderType: 'service_account_impersonation'`.
...
Для серверов, поддерживающих автоматическое обнаружение OAuth, вы можете опустить конфигурацию OAuth и позволить CLI обнаружить её автоматически:
...
- Обнаружит OAuth эндпоинты из метаданных сервера
- Выполнит динамическую регистрацию клиента, если это поддерживается
- Обработает OAuth поток и управление токенами
...
2. **Обнаружение OAuth** находит endpoint'ы авторизации и токенов
...
**Важно:** Аутентификация через OAuth требует, чтобы ваша локальная машина могла:
...
- **`authProviderType`** (string): Определяет провайдер аутентификации. Может принимать одно из следующих значений:
  ...
  - **`service_account_impersonation`**: CLI будет выступать от имени Google Cloud Service Account для аутентификации на сервере. Это удобно при работе с сервисами, защищенными IAP (в частности, это было разработано для сервисов Cloud Run).
...
#### Имперсонализация Service Account

Чтобы выполнить аутентификацию на сервере с помощью Service Account Impersonation, необходимо установить значение `authProviderType` как `service_account_impersonation` и указать следующие свойства:

- **`targetAudience`** (string): OAuth Client ID, добавленный в allowlist IAP-защищенного приложения, к которому вы пытаетесь получить доступ.
- **`targetServiceAccount`** (string): Email адрес Google Cloud Service Account, от имени которого будет выполняться имперсонализация.

CLI будет использовать ваши локальные Application Default Credentials (ADC) для генерации OIDC ID токена для указанного service account и аудитории. Этот токен затем будет использоваться для аутентификации на MCP сервере.

#### Инструкция по настройке

1. **[Создайте](https://cloud.google.com/iap/docs/oauth-client-creation) или используйте существующий OAuth 2.0 client ID.** Чтобы использовать существующий OAuth 2.0 client ID, выполните шаги из раздела [How to share OAuth Clients](https://cloud.google.com/iap/docs/sharing-oauth-clients).
2. **Добавьте OAuth ID в allowlist для [программного доступа](https://cloud.google.com/iap/docs/sharing-oauth-clients#programmatic_access) к приложению.** Поскольку Cloud Run пока не поддерживается как тип ресурса в gcloud iap, вы должны добавить Client ID в allowlist на уровне проекта.
3. **Создайте service account.** [Документация](https://cloud.google.com/iam/docs/service-accounts-create#creating), [Ссылка на Cloud Console](https://console.cloud.google.com/iam-admin/serviceaccounts)
4. **Добавьте service account и пользователей в политику IAP** на вкладке "Security" самого сервиса Cloud Run или через gcloud.
5. **Предоставьте всем пользователям и группам**, которые будут иметь доступ к MCP Server, необходимые разрешения для [имперсонации service account](https://cloud.google.com/docs/authentication/use-service-account-impersonation) (т.е. роль `roles/iam.serviceAccountTokenCreator`).
...
#### MCP Server на базе HTTP с Custom Headers
...
### SSE MCP Server с имперсонацией Service Account
...
## Подробное погружение в процесс Discovery

Когда Qwen Code запускается, он выполняет discovery MCP серверов через следующий детализированный процесс:
...
4. **Очистка имен:** Имена инструментов приводятся в соответствие с требованиями Qwen API:
   ...
   - Имена длиннее 63 символов обрезаются с заменой середины на `___`

### 3. Разрешение конфликтов

Если несколько серверов предоставляют инструменты с одинаковыми именами:
...
- **Обновление статуса:** Финальный статус серверов устанавливается как `CONNECTED` или `DISCONNECTED`
...
- **Список серверов:** Все сконфигурированные MCP серверы
...
**Симптомы:** Сервер подключен, но инструменты недоступны
...
4. **Переменные окружения:** Проверьте, что необходимые environment variables передаются корректно

### Советы по отладке

1. **Включите режим отладки:** Запустите CLI с флагом `--debug`, чтобы получить подробный вывод
2. **Проверьте stderr:** stderr сервера MCP записывается в лог (INFO-сообщения фильтруются)
3. **Изолируйте тестирование:** Протестируйте ваш MCP-сервер отдельно перед интеграцией
4. **Настройка поэтапно:** Начните с простых инструментов, прежде чем добавлять сложную функциональность
...
- **Совместимость с песочницами:** При использовании песочницы убедитесь, что MCP-серверы доступны внутри этой среды
...
- **Очистка свойств:** Система автоматически удаляет определенные свойства схем (`$schema`, `additionalProperties`) для совместимости с Qwen API
- **Санитизация имен:** Имена инструментов автоматически очищаются для соответствия требованиям API
...
Инструменты MCP не ограничены возвратом простого текста. Вы можете возвращать rich, multi-part контент, включая текст, изображения, аудио и другие бинарные данные в одном ответе инструмента. Это позволяет создавать мощные инструменты, которые могут предоставлять модели разнообразную информацию за один шаг.
...
3. Выводит понятное и удобное сообщение в CLI, указывающее, что были получены и текст, и изображение.

Это позволяет создавать сложные инструменты, которые могут предоставлять богатый мультимодальный контекст модели Qwen.

## MCP Prompts как Slash Commands

Помимо инструментов, MCP серверы могут предоставлять предопределенные prompts, которые можно выполнять как slash команды внутри Qwen Code. Это позволяет создавать ярлыки для часто используемых или сложных запросов, которые можно легко вызывать по имени.
...
Это можно добавить в `settings.json` под ключом `mcpServers` следующим образом:
...
Этот транспорт используется для серверов, которые используют потоковую передачу через HTTP.
...
Чтобы посмотреть все настроенные MCP-серверы, используйте команду `list`. Она отображает имя каждого сервера, его конфигурацию и статус подключения.

...
Используйте `read_many_files`, чтобы прочитать содержимое нескольких файлов, указанных по путям или с помощью glob-паттернов. Поведение этого инструмента зависит от переданных файлов:

- Для текстовых файлов инструмент объединяет их содержимое в одну строку.
- Для изображений (например, PNG, JPEG), PDF, аудио (MP3, WAV) и видео (MP4, MOV) файлов он читает данные и возвращает их в виде base64-кодированных данных, при условии, что они явно запрошены по имени или расширению.
...
**Примечание:** `read_many_files` ищет файлы по указанным путям или glob-паттернам. Путь к директории, такой как `"/docs"`, вернёт пустой результат; инструмент требует паттерн, например `"/docs/*"` или `"/docs/*.md"`, чтобы определить нужные файлы.

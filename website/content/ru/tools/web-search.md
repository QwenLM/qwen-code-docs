...
- **Возвращаемый ответ:** Инструмент `web_search` возвращает краткий ответ, если он доступен, с списком ссылок на источники.
- **Цитирование:** Ссылки на источники добавляются в виде пронумерованного списка.
- **API key:** Настройте `TAVILY_API_KEY` через settings.json, переменные окружения, файлы .env или аргументы командной строки. Если ключ не настроен, инструмент не будет зарегистрирован.
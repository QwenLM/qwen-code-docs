...
Используйте `web_fetch` для получения контента по указанному URL и его обработки с помощью AI модели. Инструмент принимает URL и prompt в качестве входных данных, извлекает содержимое страницы, конвертирует HTML в markdown и обрабатывает контент с учетом prompt, используя небольшую и быструю модель.

### Аргументы

`web_fetch` принимает два аргумента:

- `url` (string, обязательный): URL, по которому нужно получить контент. Должен быть полностью корректным URL, начинающимся с `http://` или `https://`.
- `prompt` (string, обязательный): Prompt, описывающий, какую информацию вы хотите извлечь из содержимого страницы.
...
- **Обработка контента:** Инструмент напрямую загружает контент и обрабатывает его с помощью AI модели, конвертируя HTML в читаемый текстовый формат.
- **Качество вывода:** Качество результата зависит от четкости инструкций в промпте.
- **MCP инструменты:** Если доступен предоставляемый MCP инструмент web fetch (начинающийся с "mcp__"), предпочтительно использовать именно его, так как он может иметь меньше ограничений.
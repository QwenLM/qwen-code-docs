...
- **Название инструмента:** `list_directory`
- **Отображаемое имя:** ReadFolder
- **Файл:** `ls.ts`
- **Параметры:**
  - `path` (string, обязательный): Абсолютный путь к каталогу для получения списка.
...
- **Поведение:**
  ...
  - Сортирует записи так, что сначала идут каталоги, затем — по алфавиту.
...
- **Поведение:**
  - Для текстовых файлов: Возвращает содержимое. Если используются `offset` и `limit`, возвращается только указанный диапазон строк. Указывает, было ли содержимое обрезано из-за ограничений по количеству или длине строк.
...
- **Вывод:** (`llmContent`):
  ...
  - Для изображений/PDF-файлов: Объект, содержащий `inlineData` с полями `mimeType` и `data` в формате base64 (например, `{ inlineData: { mimeType: 'image/png', data: 'base64encodedstring' } }`).
...
- **Поведение:**
  - Записывает предоставленный `content` по указанному `file_path`.
...
- **Вывод (`llmContent`):** Сообщение вида: `Найдено 5 файл(ов), соответствующих "*.ts" в директории src, отсортировано по времени модификации (новые первыми):\nsrc/file1.ts\nsrc/subdir/file2.ts...`
- **Подтверждение:** Нет.

## 5. `search_file_content` (SearchText)

`search_file_content` ищет регулярное выражение в содержимом файлов указанной директории. Может фильтровать файлы по glob-паттерну. Возвращает строки с совпадениями вместе с путями к файлам и номерами строк.

- **Имя инструмента:** `search_file_content`
- **Отображаемое имя:** SearchText
- **Файл:** `grep.ts`
- **Параметры:**
  - `pattern` (string, обязательный): Регулярное выражение (regex), которое нужно найти (например, `"function\s+myFunction"`).
  - `path` (string, опциональный): Абсолютный путь к директории для поиска. По умолчанию — текущая рабочая директория.
  ...
  - `maxResults` (number, опциональный): Максимальное количество совпадений, которые будут возвращены, чтобы избежать переполнения контекста (по умолчанию: 20, максимум: 100). Используйте меньшие значения для широких запросов, большие — для точных.
- **Поведение:**
  - Использует `git grep`, если доступно в Git-репозитории, для повышения скорости; иначе использует системную команду `grep` или поиск на основе JavaScript.
  ...
  - По умолчанию ограничивает результаты 20 совпадениями, чтобы избежать переполнения контекста. При обрезке результатов выводится предупреждение с рекомендациями по уточнению поиска.
- **Вывод (`llmContent`):** Отформатированная строка с совпадениями, например:
  ...
  ПРЕДУПРЕЖДЕНИЕ: Результаты обрезаны, чтобы избежать переполнения контекста. Чтобы увидеть больше результатов:
  - Используйте более точный паттерн для уменьшения количества совпадений
  - Добавьте фильтры файлов через параметр 'include' (например, "*.js", "src/**")
  - Укажите более узкий 'path' для поиска в подкаталоге
  - Увеличьте значение параметра 'maxResults', если вам нужно больше совпадений (текущее: 20)
  ```
...
`edit` заменяет текст в файле. По умолчанию заменяется одно вхождение, но можно заменить несколько вхождений, если указать `expected_replacements`. Этот инструмент предназначен для точных, целенаправленных изменений и требует значительного контекста вокруг `old_string`, чтобы убедиться, что изменения вносятся в правильное место.
...
- **Параметры:**
  ...
  - `old_string` (string, обязательный): Точный текст, который нужно заменить.
    ...
    **ВАЖНО:** Эта строка должна однозначно идентифицировать экземпляр, который нужно изменить. Она должна включать как минимум 3 строки контекста _до_ и _после_ целевого текста, точно соответствующего пробелам и отступам. Если `old_string` пустая, инструмент попытается создать новый файл по пути `file_path` с содержимым `new_string`.
...
- **Поведение:**
  - Если `old_string` пустая и файл по пути `file_path` не существует, создается новый файл с содержимым `new_string`.
  - Если `old_string` задана, инструмент читает файл по пути `file_path` и пытается найти ровно одно вхождение `old_string`.
  - Если одно вхождение найдено, оно заменяется на `new_string`.
  - **Повышенная надежность (многоэтапная коррекция редактирования):** Чтобы значительно повысить вероятность успешного редактирования, особенно когда `old_string`, предоставленная моделью, может быть не совсем точной, инструмент использует механизм многоэтапной коррекции.
    ...
    - Этот процесс самокоррекции пытается определить уникальный сегмент, который модель хотела изменить, делая операцию `edit` более устойчивой даже при незначительных неточностях в начальном контексте.
- **Условия сбоя:** Несмотря на механизм коррекции, инструмент завершится ошибкой, если:
  ...
  - `old_string` не найдена в файле даже после попыток коррекции.
  - `old_string` найдена несколько раз, и механизм самокоррекции не может определить однозначное совпадение.
- **Вывод (`llmContent`):**
...
Эти инструменты файловой системы предоставляют основу, позволяющую Qwen Code понимать и взаимодействовать с локальным контекстом вашего проекта.
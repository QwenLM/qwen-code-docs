...
| ----------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------- |
| **Авто**    | После срабатывания `recordAssistantTurn`. Пропускается, если задан существующий или ожидающий явный заголовок, есть сбой записи, выполняется другая попытка, достигнут лимит, неинтерактивный режим, отключено окружением или нет быстрой модели. | `ChatRecordingService.maybeTriggerAutoTitle` — fire-and-forget |
...
| `packages/cli/src/ui/commands/renameCommand.ts`      | `/rename --auto`, sentinel-парсер, карта сообщений о причинах ошибок.                       |
...
### Контракт долговечного результата

`ChatRecordingService.recordCustomTitle()` возвращает `Promise<boolean>`.
`true` означает, что запись `custom_title` завершила упорядоченную запись
JSONL рекордера, заголовок и источник в памяти обновлены, а наблюдатель
заголовка уведомлён. Это не означает просто принятия в очередь записи. Сбой
создания файла, существующий сбой рекордера, сбой более ранней записи в
очереди или собственное отклонение записи заголовка возвращают `false` и
оставляют прежний заголовок и состояние наблюдателя без изменений.
Канонический `flush()` рекордера по-прежнему сообщает о «прилипшем» сбое
писателя.

Наблюдатель запускается только после персистентности. Исключения наблюдателя
изолируются, поскольку уже записанный заголовок нельзя задним числом
классифицировать как неудачное переименование. Колбэк также получает ID
сессии, зафиксированный в сохранённой записи; потребители не должны читать
изменяемую сессию Config в момент колбэка. Это предотвращает ситуацию, когда
отложенная запись от уходящего рекордера переименовывает новую строку промпта
после того, как `/clear`, возобновление или ветвление переключили сессию.

Фоновый автозаголовок использует тот же приватный путь персистентности, но
остаётся без гарантий (best-effort). Публичные вызовы — это явные запросы
заголовка, включая `recordCustomTitle(title, 'auto')` из `/rename --auto` и
поверхностей удалённого управления.
...
При возобновлении конструктор `ChatRecordingService` вызывает `sessionService.getSessionTitleInfo(sessionId)`, чтобы прочитать **оба** поля — заголовок и его источник. Без гидратации источника повторное добавление записи в `finalize()` (которое выполняется при каждом событии жизненного цикла сессии) переписывало бы auto как manual при каждом цикле возобновления — молчаливо убирая приглушение как визуальную подсказку.
...
`maybeTriggerAutoTitle` проверяет эти условия в точном порядке — каждое коротко замыкает остальные, так что дешёвые выполняются первыми:

1. `currentCustomTitle` задан → пропустить. Никогда не перезаписывать ручной / предыдущий автоматический.
2. Писатель деградировал → пропустить. Рекордер в режиме fail-stop не может сохранить заголовок, поэтому не тратить ещё токены быстрой модели.
3. Ожидается запись явного заголовка → пропустить. Намерение пользователя и удалённого управления побеждает ещё до того, как его запись в JSONL завершится.
4. `autoTitleController !== undefined` → пропустить. Одна попытка за раз.
5. `autoTitleAttempts >= 3` → пропустить. Лимит ограничивает общую трату ресурсов.
6. `autoTitleDisabledByEnv()` → пропустить. `QWEN_DISABLE_AUTO_TITLE=1` — явный отказ.
7. `!config.isInteractive()` → пропустить. Headless `qwen -p` / CI никогда не тратит токены быстрой модели на одноразовую сессию.
8. `!config.getFastModel()` → пропустить. Нет быстрой модели → ничего не делаем.

### Почему лимит 3, а не 1

Первый ответ ассистента может быть чистым вызовом инструмента без видимого пользователю текста (например, модель начинает с `grep`). `tryGenerateSessionTitle` возвращает `{ok: false, reason: 'empty_history'}` в этом случае. Без окна повторных попыток вся возможность получить заголовок для сессии была бы сожжена на первом ходе, ещё до того, как пользователь сказал что-то интересное. Лимит 3 покрывает распространённый случай «первый ход — шум», одновременно ограничивая бесконечные повторные попытки при постоянном сбое быстрой модели.
...
### Явное переименование попадает в середину генерации

Каждый публичный вызов `recordCustomTitle()` увеличивает счётчик ожидающих явных заголовков и прерывает активный фоновый контроллер до того, как тот начнёт ждать JSONL. Это включает `/rename --auto`, поэтому явное переименование, сгенерированное моделью, имеет тот же приоритет, что и буквальный заголовок пользователя. Фоновая задача проверяет счётчик, сигнал отмены, текущий заголовок и состояние писателя как после генерации, так и непосредственно перед персистентностью. Параллельные явные переименования сериализуются писателем; их результаты в памяти фиксируются в том же порядке. Если явный заголовок приходит после того, как фоновый заголовок уже поставлен в очередь, обычный порядок записи писателя делает явный заголовок финальной записью.

Пока ожидается запись явного заголовка, `finalize()` не закрепляет повторно предыдущий кэшированный заголовок. Иначе эта устаревшая запись могла бы встать в очередь за переименованием и стать хвостом JSONL после того, как новый заголовок успешно запишется. Учёт закрепления заголовка следует логическому порядку очереди писателя. Дочерние записи, поставленные в очередь за любым неразрешённым записыванием заголовка, учитываются, но повторное закрепление по порогу откладывается до тех пор, пока не станет известен финальный кэшированный заголовок, так что ни завершение процесса, ни путь закрепления по 32 КБ не могут добавить устаревший заголовок после успешного переименования.

Повторное чтение между процессами по-прежнему защищает ручные заголовки, записанные другим CLI-процессом. Это не compare-and-swap; внешнее переименование в финальном окне «проверка-запись» остаётся принятой гонкой.

### Порядок заголовков при ветвлении

`/branch` финализирует и сбрасывает (flush) исходный рекордер перед его копированием. Затем он загружает предварительный форк, вычисляет уникальный заголовок ветки, сохраняет этот заголовок через `SessionService` и перезагружает форк, чтобы его финальный хвост и кэш заголовков включали добавленную запись. Только после успешного выполнения этих шагов он переключает основной рекордер и UI. Сбой сброса исходного рекордера, сбой заголовка или сбой финальной перезагрузки удаляет незавершённый форк и оставляет исходную сессию активной. `forkSession()` также удаляет цель, созданную в эксклюзивном режиме, если запись или закрытие нового JSONL не удаётся до того, как вызов сможет сообщить об успехе, поэтому вызывающая сторона не может оставить частичный файл в окне до того, как отметит форк как созданный.

### Граница демона

Живые операции заголовка ACP дожидаются `recordCustomTitle()` и поэтому сообщают правдивый результат `persisted` или `success` без второго `flush()`, который мог бы приписать переименованию более поздний несвязанный сбой записи. PATCH метаданных демона остаётся намеренно оптимистичным: он обновляет живое состояние и сразу публикует событие, при этом персистентность дочернего процесса — по мере возможности (best-effort), а результат `false` записывается в debug-лог. Строго согласованные обновления метаданных по HTTP выходят за рамки этого дизайна.

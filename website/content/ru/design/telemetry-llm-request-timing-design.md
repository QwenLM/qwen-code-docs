...
> алиасы `gen_ai.usage.cached_tokens`,
> `gen_ai.server.time_to_first_token` и
> `gen_ai.usage.reasoning_tokens` из этого документа, а также заменяет алиасы
> LLM-спана `qwen-code.model`, `input_tokens`, `output_tokens` и
> `cached_input_tokens` стандартными атрибутами GenAI. Приватный атрибут
> спана `ttft_ms`, `ApiResponseEvent.ttft_ms`, `sampling_ms`, пропускная
> способность, `/stats` и описанные здесь метрики разбивки API-запросов
> остаются в силе. Документ выравнивания добавляет независимый стандартный
> атрибут `gen_ai.response.time_to_first_chunk` рядом с `ttft_ms`.
...
- **Не зависит от захвата контента GenAI**. Сжатие чата больше не сбрасывает
  глобальное для процесса состояние хэша чувствительных атрибутов, поскольку
  это состояние было удалено; тайминг запросов остаётся отдельной
  поверхностью.

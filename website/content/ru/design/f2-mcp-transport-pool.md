...
> **Историческая запись решения.** Исходная предпосылка `1 демон = 1 рабочее
> пространство` заменена мультипространственными рантаймами. Реализованный пул
> по-прежнему имеет ту же границу изоляции, но этой границей теперь является
> один `WorkspaceRuntime`: сессии разделяются внутри рантайма и никогда между
> рантаймами. Текущий контракт владения см. в

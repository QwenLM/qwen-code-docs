...
> - `QWEN_CODE_MEMORY_PROJECT_SCOPE=workspace`: разбивает память проекта по точной директории workspace (по умолчанию `git-root` — общий доступ по корневой директории Git). Значение проходит trim / нормализацию в нижний регистр; нераспознанные значения один раз выдают предупреждение и откатываются к `git-root`.
>   - Командная память (`getTeamAutoMemoryRoot`) по-прежнему разбивается по корневой директории Git: вложенные workspace внутри одного checkout продолжают разделять командную память — командная память и должна разделяться между workspace, этот переключатель её не меняет.
>   - Переключение scope не выполняет миграцию: после переключения на `workspace` память проекта, ранее записанная под ключом git-root, «теряется из виду» (а при обратном переключении не видно новое содержимое, записанное под ключом workspace).
>   - Ключи директорий генерируются `sanitizeCwd` (не буквенно-цифровые символы заменяются на `-`), поэтому соседние директории, различающиеся только пунктуацией (например, `feature_1` и `feature-1`), отображаются в одну директорию памяти; при разбивке `workspace` такие имена будут разделять память — этого следует избегать при именовании.
>   - Лимит ожидающих задач (`MAX_PENDING = 16`) задан **на каждую lane**: N workspace допускают 16·N одновременных задач памяти; общего лимита на уровне демона сейчас нет.

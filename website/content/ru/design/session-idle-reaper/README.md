...
| Накапливается 32 сессии (по умолчанию `maxSessions`)                                    | `SessionLimitExceededError` при новом `spawnOrAttach` — пользователь заблокирован           |
...
| Disconnect reaper (server.ts)              | Рукопожатие spawn           | Вытесняет сессии, чей владелец spawn'а отключился ВО ВРЕМЯ POST /session handshake |
...
| Проверка                  | Почему                                                                                                                       |
| ------------------------- | ---------------------------------------------------------------------------------------------------------------------------- |
| Нет активного prompt'а    | headless / автономный prompt (например, конвейер CLI, задача cron) может выполняться без подписчика SSE. Его вытеснение убило бы работу. |
...
| ----------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Reaper закрывает сессию, к которой headless-клиент собирается переподключиться      | 30-минутный TTL по умолчанию является щедрым; headless-клиенты должны отправлять heartbeats. Транскрипт на диске сохраняется — `session/load` восстанавливает его.                                      |
| `closeSession` внутри reaper'а вызывает исключение, нарушая цикл обхода             | Каждое закрытие выполняется в отдельном `.catch()` — сбой одного не блокирует другие                                                                                                                    |
| Обход reaper'ом `byId` во время одновременного `closeSession` из другого пути       | Итерация Map ES2015 допускает удаление текущего/предыдущего ключей. Двойное закрытие идемпотентно (`byId.get` возвращает undefined → `SessionNotFoundError`, перехватываемый `.catch()` reaper'а).        |
...
| Взаимодействие с таймером бездействия канала                                        | Когда последняя сессия вытесняется, `closeSession` уже вызывает `startIdleTimer` для канала. Дополнительной логики не требуется.                                                                         |
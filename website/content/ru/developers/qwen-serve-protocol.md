...
| `realtime_voice`                    | демон macOS WebShell включил Live Voice и нативную интеграцию Host.                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
Управление привязками доступно только для экземпляров, настроенных с политикой отправителя `pairing`:

- `GET .../channels/:name/pairing-requests`
- `POST .../channels/:name/pairing-requests/approve` с `{ "code": "..." }`
- `GET .../channels/:name/pairing-approvals`
- `DELETE .../channels/:name/pairing-approvals` с `{ "senderId": "..." }`

Все маршруты привязок требуют bearer-токен и используют `Cache-Control: no-store`. Запросы, одобрения и отзыва привязаны к выбранному экземпляру Channel и рабочему пространству. Снимок одобрений содержит ID отправителей, поскольку allowlist не сохраняет отображаемые имена отправителей. Отзыв неизвестного отправителя возвращает `404 channel_pairing_approval_not_found`.
...
События выдаются в виде стандартных фреймов EventSource. Демон записывает одну строку `data:` на фрейм (JSON не содержит встроенных символов новой строки после `JSON.stringify`); Сервер записывает одну строку `data:` на фрейм (JSON не содержит встроенных переносов строк после `JSON.stringify`); SSE-транспорт добавляет `\n\n` для разделения фреймов.
...
| `packages/cli/src/serve/routes/*.ts`                 | Сфокусированные группы маршрутов Express, включая сессию, SSE, аутентификацию рабочего пространства, статус и файлы |

...
`session_archive` анонсирует API архивации состояния каталогов v1: `POST /sessions/archive`, `POST /sessions/unarchive` и `GET /workspace/:id/sessions?archiveState=active|archived`. Архивированные сессии не могут быть загружены или возобновлены до тех пор, пока не будут разархивированы. `session_storage_conflict_repair` анонсирует аддитивную опцию запроса `resolveConflicts` и корзину ответа `resolvedConflicts`, описанные ниже.
...
`extension_local_path_install` анонсирует локальные для демона источники расширений как через `POST /workspace/extensions/install`, так и через `POST /extensions/install`. `source` должен быть абсолютным путём, существующим на хосте демона. Относительные пути по-прежнему не поддерживаются, чтобы cwd процесса демона не мог изменить источник или затенить сокращение `owner/repo` на GitHub. Существующая операция установки копирует расширение в управляемое хранилище; она не создаёт ссылку на источник. Клиенты должны предварительно проверять этот тег, поскольку старые демоны отклоняют локальные источники.
...
| `GET /workspaces/:workspace/extensions/:extensionId/state`         | `200` состояние ресурса рабочего пространства (`extension_state`)           |
| `PUT /workspaces/:workspace/extensions/:extensionId/state`         | `202` операция состояния ресурса рабочего пространства (`extension_state`)  |
...
#### Состояние ресурса рабочего пространства

Preflight `extension_state` независимо. Он поддерживает только навыки; он не подразумевает управление ресурсами MCP. Оба маршрута выбирают зарегистрированный рантайм рабочего пространства и никогда не переключаются на основной. GET следует существующим правилам доверия только для чтения; PUT требует доверенного рабочего пространства.
...
PUT принимает от 1 до 100 записей, включая пакет из одного элемента. Он отклоняет некорректно сформированные записи, дублирующиеся имена без учёта регистра и неподдерживаемые группы перед постановкой в очередь. Все имена должны принадлежать установленному целевому расширению, включая текущие отключённые навыки; некорректная цель завершает операцию без частичной записи. Одна заблокированная фиксация хранилища объединяет перечисленные переопределения для точного канонического рабочего пространства. Не перечисленные навыки и другие рабочие пространства сохраняются. Запись настроек, декларация будущего имени или неявная активация родительского расширения не выполняются.

GET возвращает `v: 1`, `workspaceId`, `workspaceCwd`, `extensionId`, `name` и `skills`. Каждый навык имеет `name`, `defaultEnabled`, nullable `workspaceEnabled`, `effectiveEnabled` и опциональные `disabledReason`/`lockedScope`. Опциональный `skillStates` манифеста поставляет значения по умолчанию; отсутствующие значения по умолчанию включены. Для активного родителя порядок приоритета: жёсткое отключение в настройках, явное включение в настройках, отключение по умолчанию в настройках, внутреннее переопределение рабочего пространства, значение по умолчанию манифеста. Внутреннее отключение использует `disabledReason: "default"` и никогда не блокирует настройки. Отключённый или удалённый родитель не может быть возрождён переопределением навыка.

Операция `set_extension_state` возвращает `status: "updated"` с упорядоченными `result.resourceStates.skills`; `result.states` сохраняет своё значение проверки обновлений. Сохранённое состояние не является доказательством того, что каждая сессия обновилась. Демон обновляет только навыки и их команды/контекст модели для целевого рантайма, включая бутстрап и живые сессии, без перезапуска нерелевантных MCP/LSP/хуков. Сбои обновления после фиксации генерируют предупреждения без отката состояния. Переопределения переживают перезапуск и обновления расширения и удаляются при удалении. Клиенты не должны переключаться на API настроек навыка, который записывает настройку с более высоким приоритетом.
...
Для начальной активации только в рабочем пространстве используйте `{ "scope": "workspace", "workspaceId": "target-workspace-id" }`; целевой объект должен существовать и быть доверенным. Установки демона принимают источники GitHub, Git и npm. Когда анонсирован `extension_local_path_install`, они также принимают абсолютный путь, существующий на хосте демона. `ref` не применяется к npm, `ref` и `autoUpdate` не применяются к локальным источникам, а `registry` применяется только к npm. `ref`, `autoUpdate`, `allowPreRelease` и `registry` необязательны.
...
| ----------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `require_auth`                      | демон был запущен с флагом `--require-auth` (или `requireAuth: true` через встроенный API). Bearer-токен обязателен на каждом обычном маршруте API, включая `/health` на loopback-биндах; аутентификация webhook-входа канала сохраняет свою независимую аутентификацию по общем�� секрету, а маршруты документов и ресурсов Web Shell остаются pre-auth.                                                                                                                                                                                                                         |
...
| `external_tool_guard`               | `qwen serve` завершил стартовое рукопожатие для `--external-tool-guard-mode=required`; каждый созданный канал ACP должен подтвердить установленный callback до создания сессии, и каждый поддерживаемый управляемый инструмент ACP верхнего уровня, достигающий границы выполнения, должен получить одно внешнее разрешение перед выполнением. Более ранние отказы разрешений/хуков не делают запрос к провайдеру. Вложенное выполнение AgentCore находится за пределами v1 и отклоняется, пока этот режим внешнего провайдера активен. Тег отражает только внешнего провайдера: независимо от него, каждый демон применяет встроенный гейт перемещения Git к управляемым инструментам, несущим командную строку shell (`run_shell_command` и `monitor`), поэтому отсутствие этого тега не означает отсутствие отказов перед выполнением. |
| `allow_origin`                      | T2.4 ([#4514](https://github.com/QwenLM/qwen-code/issues/4514)). Демон был запущен хотя бы с одним `--allow-origin <pattern>` (или `allowOrigins: [...]` через встроенный API). Cross-origin запросы с совпадающих origin получают корректные заголовки ответов CORS; несовпадающие origin по-прежнему получают стандартный 403. Настроенный список паттернов намеренно НЕ возвращается в `/capabilities`, чтобы не раскрывать набор доверенных origin неаутентифицированным читателям — браузерный клиент уже знает свой собственный origin. |
...
| `user_language_sync`                | демон был создан с доступным сохранением настроек (то же условие, что и для `workspace_settings`), поэтому регистрируется бессессионный маршрут `POST /language`.                                                                                                                                                                                                                                                                                                                                                |
...
| `session_shell_command`             | выполнение shell в сессии явно включено и действует при bearer-аутентификации или доверии loopback; вызовы по-прежнему требуют client id, привязанного к сессии.                                                                                                                                                                                                                                                                                                                                                  |
| `standalone_sessions_v1`            | демон установил полный рантайм автономных сессий, координатор жизненного цикла, долговременный журнал удалений, реализацию управляемого каталога и семейство маршрутов `/standalone/sessions`. Прямые встраивания без полного графа зависимостей опускают и маршруты, и этот тег.                                                                                                                                                                                                                                    |
| `standalone_session_options_v1`     | полный рантайм автономных сессий установлен (то же условие, что и для `standalone_sessions_v1`), поэтому на внутреннем рантайме Conversations регистрируется доступный только для чтения бессессионный маршрут `GET /standalone/session-options`.                                                                                                                                                                                                                                                               |
| `session_artifacts_persistence`     | сохранение артефактов сессии подключено для рантайма.                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| `session_sources`                   | сохранение источников сессии подключено для рантайма. Регистрирует метаданные-файлы рабочего пространства, загруженные вложения и HTTP(S)-ссылки через владельца живой сессии.                                                                                                                                                                                                                                                                                                                                    |
| `session_generation`                | доступны помощники генерации сессии.                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| `scheduled_task_session_reuse`      | активно долговременное управление сессиями запланированных задач, и каждый управляемый рантайм демона установил callback, позволяющий задаче явно привязаться к своей текущей существующей сессии.                                                                                                                                                                                                                                                                                                                |
...
| `multi_workspace_session_shell`     | зарегистрировано более одного рантайма рабочего пространства, и выполнение shell в сессии явно включено при bearer-аутентификации или доверии loopback; REST shell с единственным числом разрешает владеющий рантайм.                                                                                                                                                                                                                                                                                              |
...
| `native_directory_picker`           | хост демона может открыть нативный диалог выбора директории ОС (`osascript` на macOS, PowerShell на Windows, `zenity` на хосте Linux с дисплеем). Headless-хосты опускают тег, чтобы клиенты скрывали элемент Browse вместо отображения гарантированно неработающего диалога.                                                                                                                                                                                                                                    |
| `workspace_runtime`                 | каждый активный бридж рабочего пространства предоставляет авторитетный снимок жизненного цикла рантайма; смешанные или устаревшие инжектированные бриджи опускают тег.                                                                                                                                                                                                                                                                                                                                             |
| `workspace_skills_config_runtime`   | каждое активное рабочее пространство поддерживает авторитетный жизненный цикл рантайма и разделённые маршруты Skills config/runtime, используемые управлением навыками и композером новой сессии.                                                                                                                                                                                                                                                                                                                  |
| `workspace_local_open`              | хост демона может открыть директорию рабочего пространства в файловом менеджере ОС хоста (`open` на macOS, `explorer.exe` на Windows, `xdg-open` на хосте Linux с дисплеем). Headless-хосты опускают тег, чтобы клиенты скрывали элемент «Открыть локально» вместо отображения гарантированно неработающего диалога. Открытый путь — это всегда разрешённый зарегистрированный cwd рабочего пространства, через `POST /workspaces/:workspace/open`; маршрут принимает опциональное JSON-тело `{ "target": "terminal" }` (отсутствует/другое = папка) и отвечает `{ kind: 'workspace-local-open', opened: true, target }`, где `target` установлен в `folder` или `terminal`. |
| `workspace_local_terminal`          | хост демона может открыть окно терминала в директории рабочего пространства (`open -a Terminal` на macOS, `wt.exe` с фолбэком на `cmd.exe` на Windows, `gnome-terminal`/`konsole`/`xterm` на хосте Linux с дисплеем). Headless-хосты опускают тег, чтобы клиенты скрывали элемент «Открыть в терминале» вместо отображения гарантированно неработающего диалога. Обслуживается через `POST /workspaces/:workspace/open` с телом `{ "target": "terminal" }`.                                                                                                                                                                                |
...
| `web_terminal`                      | ACP HTTP включен, поэтому аутентифицированный эндпоинт Web Terminal доступен.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
**Аутентификация:** требуется при привязке не к loopback-интерфейсу и когда loopback усилен флагом `--require-auth`. При обычной привязке к loopback (`127.0.0.0/8`, `localhost`, `::1`, `[::1]`) `/health` регистрируется до bearer middleware, поэтому liveness-пробы k8s/Compose внутри пода не нужно передавать токен. При привязке не к loopback (`--hostname 0.0.0.0` и т.д.) или усиленном loopback маршрут регистрируется после bearer middleware и возвращает 401 без валидного токена — в противном случае неаутентифицированный вызывающий мог бы опрашивать произвольные адреса, чтобы подтвердить существование `qwen serve`, что представляет собой утечку информации с низким уровнем серьезности, которая плохо сочетается со сканированием портов. CORS deny + Host allowlist по-прежнему применяются к исключению для обычной привязки к loopback.
...
`limits.maxRegisteredWorkspaces` является аддитивным и сообщает разрешённый лимит регистрации пользователя (по умолчанию 256, настраивается от 1 до 256). `limits.maxChannelControlWorkspaces` сообщает независимый лимит владельца управления/восстановления равный 25 на стандартном демоне, даже когда каналы отключены. Пользовательские контроллеры анонсируют это поле только когда они его применяют. Эти поля присутствуют как в ответах загрузки, так и в ответах готовности; старые демоны могут их опускать. Контроллер каналов отклоняет переходные объединения владельцев сверх своего лимита с `409 channel_control_workspace_limit_reached` до построения кандидатов-воркеров; начальное объединение при запуске сверх лимита завершается ошибкой до публикации слушателя, поэтому оно не имеет HTTP-поверхности. Ёмкость регистрации не определяет тайм-аут канала SDK.

`limits.maxTotalSessions` является аддитивным. `null` означает, что эффективный лимит свежих сессий для всего демона отключен. Когда разрешённая ёмкость регистрации (по умолчанию 256, настраивается через `QWEN_SERVE_MAX_WORKSPACES` или встроенный `maxRegisteredWorkspaces`) превышает 25 и `--max-total-sessions` не указан, стандартный демон использует фиксированный общий лимит 800, даже с одним рабочим пространством при запуске. При ёмкости регистрации 25 или меньше, несколько рабочих пространств при запуске/восстановлении с конечным `maxSessionsPerWorkspace` выводят эффективный общий лимит один раз как `maxSessionsPerWorkspace * workspaceCount` по тому же количеству рабочих пространств при запуске и восстановлении; одно рабочее пространство при запуске или восстановлении сохраняет неограниченный лимит по умолчанию. Явные общие лимиты, включая отключённые значения, имеют приоритет. Последующая динамическая регистрация не пересчитывает общий лимит. Прямые встраивания `createServeApp` должны предоставить свою собственную общую политику допуска. Когда установлен, он ограничивает создание свежих сессий по всему демону и сообщает о сбоях общего лимита с существующей формой ошибки `session_limit_exceeded` плюс `scope: "total"`.
...
Запуск channel-воркера, управляемого демоном, по явному `qwen serve --channel
...` остаётся fail-fast и имеет приоритет над сохранёнными настройками запуска.
Загрузка без флагов восстанавливает `serve.channels` из доверенного основного рабочего пространства.
Вторичные рабочие пространства независимо не восстанавливают свои собственные `serve.channels`.
Без явного выбора или выбора основного рабочего пространства загрузка рантайма канала остаётся
ленивой.

Сохранённые имена запуска должны быть непустыми, не иметь ведущих или замыкающих пробелов
и не содержать небезопасных управляющих или невидимых символов. Некорректные записи
пропускаются по одной и логируются по индексу массива; запуск не обрезает их в
другие имена экземпляров и не переписывает настройки. Аргументы воркера используют
`--channel=<value>`, сохраняя ведущий дефис как часть имени.

Некорректное поле запуска или ошибка валидации или аренды перед запуском воркеров
пропускает автоматическое восстановление, с логом, идентифицирующим `serve.channels`, в то время как
несвязанные настройки остаются в силе. Неудачный запуск воркера позволяет демону
продолжить после успешной очистки. Глобальные тайм-ауты запуска рантайма и
неподтверждённые остановки воркера следуют существующему пути сбоя запуска; аренда
остаётся удерживаемой, пока завершение воркера не подтверждено. Проверьте логи демона для
пропущенных или неудачных восстановлений. Управление каналами сообщает сохранённые настройки запуска
и фактическое состояние рантайма.

После того как воркер достиг состояния ready, неожиданные завершения перезапускаются
супервизором serve в рамках ограниченной политики: до 3 попыток перезапуска в 5-минутном
окне, с задержкой 1 с, 5 с, затем 15 с. Воркер отправляет IPC-хартбиты каждые
15 с; если хартбит не наблюдается в течение 45 с, супервизор считает воркер
...
- `409 channel_service_conflict`, `channel_worker_not_enabled` или `channel_control_workspace_limit_reached`
...
Строгие записи из запроса основного рабочего пространства без токена, достигающего шлюза без полномочий доверенного loopback, возвращают `401 token_required` до запуска кода управления. Отсутствующие или некорректные настроенные учётные данные и несопряжённые учётные данные Local Control отклоняются ранее с простым `401 Unauthorized`.
Запросы основного рабочего пространства с доверенным loopback выполняются нормально.
После начала запроса отключение HTTP-клиента не отменяет транзакцию жизненного цикла; клиенты могут безопасно повторить тот же PUT.

Для `502 channel_worker_start_failed` ответ также может включать `startupFailures[]` и `startupFailuresTruncated`. Каждая ошибка добавляет доверенный `workspaceCwd` попыточного воркера. Эти поля описывают неудачную транзакцию, тогда как `state` описывает текущее состояние после отката; последующий GET не сохраняет неудачную попытку. Частично подключённый воркер вместо этого возвращает успех и раскрывает свои ошибки в снимке воркера. Явная загрузка `--channel` без подключённого адаптера завершается ошибкой запуска. Сбой запуска, полученный из настроек, логируется и позволяет демону продолжить после успешной очистки, в рамках глобального тайм-аута запуска рантайма. Невозможность подтвердить очистку сохраняет обычное поведение сбоя запуска и аренду сервиса.
...
Дескрипторы полей могут предоставлять метаданные вложенных объектов через `properties`. Числовые дескрипторы могут использовать `exclusiveMinimum` для открытых нижних границ. Строковые дескрипторы и дескрипторы секретов могут использовать `multiline` для запроса у клиентов многострочного текстового поля; типы дескрипторов допускают его только для полей верхнего уровня. Клиенты, которые не отображают рекламируемый тип поля, должны сохранять существующее значение конфигурации вместо его приведения или удаления, а клиент, отображающий поле `multiline` в однострочном элементе управления, должен сохранять сохранённое значение дословно вместо записи обратно значения ввода с удалёнными символами новой строки. Поля объектов не могут быть обязательными, а вложенные свойства не могут быть секретами или полями, разрешаемыми из окружения; эти протоколы управления остаются только на верхнем уровне. Вложенное свойство `required` применяется только пока его родительский объект присутствует в записи; пропуск родительского объекта оставляет его вложенные требования непроверенными. Записи заменяют сохранённое значение каждого поля целиком, поэтому сохранение объекта означает повторную отправку сохранённого объекта; демон не объединяет частичные объекты.

Записи конфигурации используют оптимистичную параллельность и строгий шлюз полномочий оператора:
...
Все маршруты привязок требуют строгих полномочий оператора и используют `Cache-Control: no-store`. Запросы, одобрения и отзыва привязаны к выбранному экземпляру Channel и рабочему пространству. Ожидающие запросы включают типизированный субъект пользователя или группы; групповые запросы также сохраняют отправителя, инициировавшего запрос. Снимки одобрений содержат `senderIds` и `groupIds`, поскольку allowlist не сохраняет отображаемые имена. Отзыв неизвестного пользователя или группы возвращает `404 channel_pairing_approval_not_found`.
...
> **`session_worktree_persistence_v1`** означает, что демон может сохранять и проверять владение worktree из части 4A. Успешные ответы создания worktree и ответы восстановления, чей потомок либо перемещён в режиме простоя, либо уже сообщает проверенный cwd worktree, несут метаданные `worktree` плюс `worktreeState: "persisted-v1"`. Устаревшее восстановление best-effort или холодное восстановление, чей промпт восстановления был запущен, а не припаркован (`suppressWorktreeContextRestore` выключен, поэтому маршрут не запросил отсрочку у бриджа) и которое поэтому сообщает об активном промпте без текущего cwd, может всё ещё возвращать `worktree` без этого подтверждения. Клиенты, запрашивающие изоляцию, должны предварительно проверять этот тег и верифицировать каждый ответ; одного объекта `worktree` недостаточно как доказательства долговременного владения.

> **`session_worktree_reset_v1`** означает, что демон поддерживает передачу владения worktree: `POST /session/:id/worktree-reset` перемещает владение checkout сохранённой сессии worktree на новую сессию-замену. Ответы восстановления получают три типизированные классификации 409 в дополнение к этому: `worktree_session_superseded` (sidecar сессии несёт ссылку `supersededBy` — классификация определяется исключительно по этой ссылке, до любого чтения маркера, и ссылка записывается до переключения маркера, поэтому `replacementSessionId` в теле является перенаправлением для проверки, а не доказательством владения: в прерванном состоянии до коммита он называет сессию, которая не является владельцем маркера, не может быть восстановлена сама и будет утилизирована повторным сбросом), `worktree_marker_missing` (маркер checkout отсутствует; сбросьте задачу для его воссоздания — повторная попытка восстановления не может этого сделать, потому что ни один путь восстановления не записывает маркер) и `worktree_reset_interrupted` (предыдущая передача завершилась сбоем в процессе выполнения, и ссылки sidecar совпадают; повторите сброс). Прерванная классификация проверяется первой: отсутствующий маркер, чьи ссылки `supersedes`/`supersededBy` совпадают, проявляется как `worktree_reset_interrupted`, а не как `worktree_marker_missing`. См. раздел маршрута ниже для протокола передачи и таксономии сбоев.
...
Брендинг продукта Web Shell для хостов, которые хотят использовать собственный бренд. Разрешается из `ui.brand` в областях настроек оператора. Ограничивается тегом возможности `web_shell_brand`; демон без этого маршрута отвечает 404.
...
> **Оба поля необязательны, и `{}` является нормальным ответом.** Отсутствие поля означает «использовать встроенный бренд клиента» — Web Shell отображает собственное имя и встроенный логотип. Клиенты не должны считать пустое тело ошибкой.

> **Обработчик всегда отвечает 200, даже если настроенный логотип был отклонён.** Отсутствующий файл, символическая ссылка, жёстко связанный файл, директория, не-SVG документ или содержимое более 32 КиБ возвращает тело без `logoDataUri`, а причина записывается в stderr демона как `qwen serve: GET /brand: ui.brand.logoPath …`. Клиент не может различить «бренд не настроен» и «логотип оператора был отклонён»; каналом для оператора является лог демона. Одно предупреждение мягче, чем отказ, и их два: корневой `<svg>` без пригодного `viewBox` и без положительных, не-процентных `width`/`height` (некорректный или нулевой viewBox считается непригодным), который браузер может отобразить пустым при фиксированном размере боковой панели; и корневой элемент с префиксом, чьи элементы без префикса не имеют привязки к пространству имён по умолчанию, что делает их невидимыми. Middleware перед обработчиком отвечает до его запуска — `401`, когда требуется и отсутствует bearer-аутентификация, `429`, когда включён опциональный ограничитель скорости. Демон в режиме дренирования не отклоняет этот маршрут: ограничитель скорости допускает запросы во время дренирования, поэтому обработчик продолжает отвечать 200, пока слушатель не закроется и клиент не увидит сбой соединения. Запуск не является 503 на этом маршруте: на стандартном пути с отложенным рантаймом запрос _удерживается_ до готовности рантайма и затем отвечает 200, поэтому сработать рано может только короткий таймаут клиента. 503 с `code: "daemon_runtime_starting"` достигает этого маршрута только в конфигурациях, которые отвечают до готовности рантайма (например, `--open`) и является повторяемым; 503 с `code: "daemon_runtime_failed"` является терминальным до перезапуска демона и не несёт `Retry-After` — не повторяйте его.

> **Настройки рабочего пространства никогда не участвуют.** Маршрут загружает настройки с `skipWorkspaceSettings`, поэтому `.qwen/settings.json` репозитория не может переименовать продукт или указать файл для чтения демоном и встраивания в каждый подключённый браузер. Читаются только System Defaults, User и System, в таком порядке приоритета. По той же причине значение бренда отклоняется с предупреждением в stderr демона, когда подстановка плейсхолдеров изменила бы его: подстановка берёт из общепроцессного окружения, которое `.qwen/.env` или блок `env` рабочего пространства заполняют первым при запуске. Неразрешимый плейсхолдер (переменная не установлена) сохраняется дословно, поэтому переменная с опечаткой отображается как буквальный текст, а не молча заменяется фолбэком.

> **`logoDataUri` должен отображаться как изображение, а не внедряться как разметка.** Демон не санирует прочитанный SVG. SVG, загруженный через `img` src или favicon href, не может выполнять скрипты; SVG, внедрённый в документ, может. Web Shell всегда назначает его только в контекст изображения, и именно этот инвариант делает отсутствие санитизатора безопасным.

> **Глобально для процесса.** Маршрут не принимает селектор рабочего пространства и не принимает id сессии: значение получается из глобальной конфигурации пользователя, поэтому оно одинаково для каждого рабочего пространства, которое обслуживает демон. Он регистрируется после `bearerAuth` и ограничителя скорости и перед фолбэком Web Shell SPA, поэтому отвечает JSON для любого заголовка `Accept`.
...
Ошибки включают `400 invalid_path` / `invalid_persist_flag` / `invalid_persist_target` / `invalid_display_name`, `409 workspace_exists` / `workspace_nested` / `workspace_limit_reached` / `workspace_registration_store_too_large`, `500 workspace_registration_store_error` / `runtime_creation_failed` и `501 persistence_not_available` / `not_implemented`.
...
`true`) и присутствует только как `false`, когда навык не может быть вызван
вручную. Это не влияет на маршруты переключения навыков только через настройки,
описанные ниже. `modelInvocable` независим: `false`
...
Для навыков, принадлежащих расширениям, `extensionName` — это каноническое имя манифеста,
и его безопасно использовать как идентификатор владельца. `extensionDisplayName` — это
опциональное локализованное значение для отображения, которое может быть не уникальным.
Новые клиенты должны отображать `extensionDisplayName ?? extensionName`; старые демоны
опускают поле отображаемого имени.
...
Возвращает каталог инструментов, о котором сообщает ACP-потомок основного рабочего пространства. Это устаревший маршрут только для основного рабочего пространства: он не имеет селектора рабочего пространства и не должен использоваться для определения инструментов неосновного рантайма.
...
Когда ACP-потомок не активен, маршрут всё равно возвращает `200` с `acpChannelLive: false`, пустым массивом `tools` и записью `not_started` в опциональном массиве `errors`. Неожиданные сбои бриджа используют стандартный ответ об ошибке бриджа `500`. Метод TypeScript SDK — `workspaceTools()`; выделенного тега возможности нет, поэтому клиенты, поддерживающие старые демоны, должны трактовать `404` как неподдерживаемый.
...
Возвращает метаданные для одного пути основного рабочего пространства. Query-параметр `path` обязателен.
...
`type` принимает значение `file`, `directory`, `symlink` или `other`. Маршрут использует границу файлов рабочего пространства и общую оболочку ошибок файловой системы, описанную выше. Метод TypeScript SDK — `fileStat()`.

#### `GET /list`

Выводит содержимое одного каталога основного рабочего пространства. Query-параметр `path` обязателен; `includeIgnored=1` (или `true`) включает записи, совпадающие с правилами игнорирования. Ответ ограничен 2000 записями и устанавливает `truncated: true`, если их больше.
...
`kind` каждой записи — `file`, `directory`, `symlink` или `other`. Метод TypeScript SDK — `dirList()`.

#### `GET /glob`

Находит пути внутри основного рабочего пространства. Query-параметр `pattern` обязателен. Опциональный `cwd` сужает поиск, `includeIgnored=1` (или `true`) включает игнорируемые пути, а `maxResults` — целое число от 1 до 50 000 (по умолчанию 5 000).
...
Совпадения указаны относительно рабочего пространства. Некорректные значения query возвращают `400`; ошибки доверия рабочего пространства, containment, отсутствующего пути и неожиданные сбои используют общую оболочку ошибок файловой системы. Метод TypeScript SDK — `glob()`.

#### `POST /file/write`

Создает или заменяет текстовый файл. Это строгий маршрут мутации: основной запрос без токена
через доверенный loopback авторизован. Основной запрос без токена, достигший гейта без полномочий
доверенного loopback, возвращает `401 { "code": "token_required" }`. Сбои учётных данных
настроенного токена и Local Control отклоняются раньше с простым `401 Unauthorized`; при
`--require-auth` глобальный bearer middleware отклоняет неаутентифицированные запросы до
выполнения маршрута.
...
Для сессии верхнего уровня `state` повторяет те же структуры ACP model/mode/config-option, которые используются в
`POST /session`, `POST /session/:id/load` и `POST /session/:id/resume`. Виртуальный id сессии с префиксом `subagent.` разрешается относительно родительского рантайма и возвращает пустой объект `state`.
...
Этот маршрут владельца живой сессии читает Config выбранной сессии через её собственное ACP-соединение. Он не объединяет общепроцессные статус-маршруты или статус-маршруты рабочего пространства, не инициализирует холодный рантайм, не подключает клиент и не переключается на основное рабочее пространство. Неизвестные и только сохранённые сессии возвращают существующий ответ `session_not_found`.

Вложенные объекты используют точные контракты статуса `GET /workspace/skills` и `GET /workspace/mcp`. Их поля `workspaceCwd` идентифицируют Config, создавшую снимок, и совпадают с верхнеуровневым значением. Существующие правила сокрытия данных по-прежнему применяются: учётные данные и заголовки MCP, значения окружения, тела навыков и сырые настройки никогда не появляются в ответе. Обогащения аутентификации MCP, пула, бюджета рабочего пространства и ошибок обнаружения рабочего пространства отсутствуют, потому что их состояние принадлежит рабочему пространству или ключируется по имени сервера, а не по сессии. Статус, обнаружение и учёт из собственного MCP-менеджера выбранной сессии остаются доступными.

### Жизненный цикл автономных сессий (`standalone_sessions_v1`)

Когда `/capabilities.features` содержит `standalone_sessions_v1`, демон предоставляет семейство маршрутов для всего процесса для автономных сессий верхнего уровня, принадлежащих его выделенному рантайму Conversations. Эти маршруты никогда не принимают селектор рабочего пространства и никогда не переключаются на основное рабочее пространство. Прямые встраивания, которые не могут построить полный граф зависимостей Conversations (владение, рантайм, каталог, жизненный цикл и журнал удалений), опускают и возможность, и все маршруты ниже.

| Маршрут                                          | Запрос                                                                                                                                                       | Успех                                                                                                                                          |
| ------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------- |
| `POST /standalone/sessions`                      | `{ "sessionId": "<UUID>", "modelServiceId"?: string, "approvalMode"?: ApprovalMode }`                                                                        | `200` с автономной сессией, `context: { "kind": "standalone" }` и её управляемым выходным каталогом без проекта. Создание происходит без промпта. |
| `GET /standalone/session-options`                | нет; любое поле query отклоняется с кодом 400                                                                                                                   | `200` с `{ v, initialized, current?, approvalMode?, providers, errors? }`; внутренний путь рабочего пространства и состояние ACP-канала опускаются    |
| `GET /standalone/sessions`                       | Query: `cursor?`, `size?` (1-100), `archiveState?` (`active` или `archived`)                                                                                  | `200 { sessions, nextCursor?, liveMergeFailed?, truncated? }`                                                                                  |
| `GET /standalone/sessions/:id`                   | нет                                                                                                                                                          | `202 { sessionId, state: "creating" }`, пока локальное создание выполняется, иначе `200` с точной сводкой.                                       |
| `POST /standalone/sessions/:id/load`             | Только существующие опции восстановления: `historyPageSize?`, `liveReplayMode?`, `hideInheritedHistory?`, `approvalMode?`; идентификация клиента остаётся в `X-Qwen-Client-Id`. | `200` восстановленная автономная сессия.                                                                                                        |
| `POST /standalone/sessions/:id/resume`           | Те же опции восстановления, что и для `load`.                                                                                                                 | `200` восстановленная автономная сессия без воспроизведения истории загрузки.                                                                    |
| `POST /standalone/sessions/:id/repair-directory` | Пустое тело или `{}`                                                                                                                                         | `200` с проверенным или воссозданным управляемым каталогом.                                                                                     |
| `PATCH /standalone/sessions/:id/metadata`        | `{ "displayName": string }`                                                                                                                                  | `200 { sessionId, displayName }`                                                                                                               |
| `GET /standalone/sessions/:id/export`            | Query: `format=html`, `format=md`, `format=json` или `format=jsonl` (по умолчанию `html`).                                                                    | Существующий тип содержимого экспорта, имя файла и тело.                                                                                        |
...
Тела должны быть JSON-объектами без неизвестных полей. ID — это значения RFC UUID v1-v5; демон канонизирует их в нижний регистр. Пакетные запросы содержат 1-100 строк и проверяются и дедуплицируются перед мутацией. Сбой пакета сообщается как `{ sessionId, code, message }` и не откатывает успешные операции для других ID. `fileCleanupPending` означает, что удаление транскрипта зафиксировано, но авторизованные журналом sidecar-или управляемый каталог должны быть повторены согласованием; сессия уже логически удалена.

Видимы только явные автономные транскрипты и документированная форма совместимости устаревшего верхнего уровня. Дочерние, живые, проектные, worktree, неоднозначные, нечитаемые или записанные в журнал удалений записи отклоняются. Создание продолжается, если HTTP-ответ отключается; зафиксированная сессия не удаляется, и клиент ответа отсоединяется. Восстановите точным GET с последующим load/resume вместо повторной попытки создания как подключения.

Архивация, разархивация, восстановление, переименование и удаление используют тот же допуск жизненного цикла для каждой сессии, что и load/resume и промпты. Удаление использует unlink транскрипта как точку долговременной фиксации и приватный журнал с атомарным управляемым каталогом для восстановления после сбоев. Восстановление воссоздаёт каталог, если транскрипт остался нетронутым, и завершает очистку, если транскрипт удалён; любое несовпадение идентичности, конфликтующий путь, чужой владелец или неоднозначный транскрипт возвращает структурированную ошибку с fail-closed.
...
| `worktree`       | нет         | Создаёт новую сессию-тред в пользовательском Git worktree. Опциональный `slug` проходит проверку имени worktree демона. Предварительно проверьте `session_worktree_persistence_v1`; клиенты не должны выводить долговременную изоляцию из более старых ответов в формате worktree. Создание worktree не является операцией подключения и не может комбинироваться с созданием ветки. Маршрут возвращает успех только после перемещения, создания эксклюзивного маркера владения, сохранения sidecar и финальной проверки генерации рантайма.                                                                                                                      |
...
`worktreeState` присутствует только для подтверждённой сессии worktree. Устаревшее восстановление best-effort может вернуть `worktree` без `worktreeState`. Холодное восстановление части 4A, чей промпт восстановления был запущен, а не припаркован (`suppressWorktreeContextRestore` выключен, поэтому маршрут не запросил отсрочку у бриджа), также возвращает непроверенные метаданные `worktree` до завершения промпта: потомок читается как активный без сообщённого cwd, перемещение невозможно при активном промпте, и маршрут сохраняет сессию, а не убивает только что восстановленную вызывающей стороной. Ни один из ответов не является доказательством долговременного корня выполнения. Клиент, запросивший изоляцию worktree, должен требовать `worktreeState: "persisted-v1"` и ожидаемый канонический путь перед маршрутизацией промпта; начальная загрузка может быть повторена после завершения активного промпта. При повторном подключении SDK результат разделяется по тому, несёт ли ответ всё ещё метаданные `worktree`: ответ, несущий их с отсутствующим подтверждением или изменённым путём, является терминальным для этого клиента — новое подключение отсоединяется и промпт не повторяется — тогда как ответ без объекта `worktree` вообще излечивается: клиент отбрасывает свой кэшированный захват worktree, сохраняет новое подключение и повторяет промпт в том же вызове. Излечение не блокирует эту повторную попытку, поэтому один ход всё ещё может выполниться в директории, которую клиент больше не может подтвердить (`exit_worktree`, который клиент никогда не видел, является обычной причиной, и клиент не может отличить её от очищенной ассоциации в памяти). Вызывающий сторонник изоляции должен поэтому повторно проверить подтверждение worktree перед отправкой промпта; отброшенный захват только заставляет этого вызывающего fail closed на его собственном гейте идентичности при следующей загрузке или выборе.
...
Передача владения checkout сохранённой сессии worktree части 4A новой сессии-замене. Pre-flight `session_worktree_reset_v1` — старые демоны возвращают `404` для этого маршрута. Этот маршрут — то, как Channel named-task `/clear` сбрасывает задачу worktree без удаления её файлов: замена получает совершенно новый разговор, а вытесненная сессия сохраняет свой транскрипт в каталоге, но больше никогда не может быть восстановлена.
...
`cwd` следует тем же правилам разрешения, что и `POST /session/:id/load`, и может быть опущен для основного рабочего пространства демона. Поля метаданных помечают сессию-замену теми же соглашениями области потока и источника, что и при создании worktree.

При успехе маршрут возвращает `200` с ответом create-shape сессии-замены — новый `sessionId`, `workspaceCwd`, `currentCwd`, равный проверенному каноническому пути worktree, метаданные `worktree` и `worktreeState: "persisted-v1"`. Вызывающие стороны отбрасывают свой дескриптор старого id и далее используют id замены; последующее восстановление старого id возвращает `worktree_session_superseded` с `replacementSessionId`, что клиенты также могут использовать для самостоятельного исправления устаревшего учёта — но только после успешной загрузки этого id. Классификация определяется исключительно по ссылке sidecar, и эта ссылка записывается до переключения маркера, поэтому прерванная передача до коммита возвращает замену, которая не является владельцем маркера, чьё собственное восстановление возвращает `worktree_reset_interrupted`, и которую затем утилизует повторный сброс.

Этот маршрут никогда не читает `X-Qwen-Client-Id`, поэтому замена запускается непривязанной — но ответ об успехе не является свободным от регистрации. Свежая передача создаёт owner-style `clientId` для этого запуска, регистрирует его на замене и сообщает его в теле; идемпотентное возобновление зафиксированной передачи не сообщает никакого. Созданный id не является привязкой (счётчик привязок замены не изменяется, поэтому `killSession` с `requireZeroAttaches` по-прежнему видит её как непривязанную), но живая регистрация клиента — это именно то, что удерживает демон от idle-очистки: отсоедините id, который вы не продолжаете использовать, иначе замена остаётся живой неопределённо долго — а замена, которая всё ещё жива без маркера, — это форма, которую последующий сброс отказывается разбирать. Относитесь к ответу как к идентичности замены и подключайтесь к ней через обычный `POST /session/:id/load` или поверхность `/resume`, когда требуется зарегистрированный клиент.

Передача сериализуется для каждого checkout относительно восстановлений worktree и других сбросов, отказывается выполняться, пока любая сессия занята (промпт в полёте или ожидающее взаимодействие), и устанавливает барьер допуска на вытесненной сессии: пока он установлен, ровно восемь писателей отклоняются с `worktree_reset_active` при допуске — `POST /session/:id/prompt`, `/rewind`, `/cd`, `/branch`, `/fork`, `/shell`, `/goal` и `/tasks/:taskId/workflow-action`. Остальные семь заблокированы, потому что каждый из них перемещает cwd сессии или начинает в ней работу без прохождения допуска промпта: shell-команда выполняется в эффективном cwd сессии, который для перемещённой сессии worktree является самим checkout; форк-агент выполняет свои инструменты в этом cwd; перемотка восстанавливает файлы относительно него; cd перемещает его, включая в поддиректорию checkout; branch мутирует сохранённую историю вытесненной сессии и создаёт производную сессию, пока владение в переходном состоянии; goal `resume` продвигает queued ход и ставит в очередь продолжение; а workflow action запускает сохранённый workflow или перезапускает живой запуск через собственный реестр инструментов сессии. `POST /session/:id/continue` не является девятым гейтом: он проводит принятое продолжение через заблокированный допуск промпта и отклоняется там. Этот список — весь забор, и забор — это не всё, что достигает потомка — пути release и stop остаются используемыми во время передачи по дизайну (отмена задачи, очистка goal, отсоединение клиента, убийство сессии), и внутренняя для демона постановка в очередь фонового уведомления — подтверждение завершения суб-сессии её родителю, которое не вызывает ни один HTTP-маршрут — также не заблокирована, потому что она ставит в очередь уведомление, а не начинает ход. Последний шаг передачи отключает регистрации клиентов вытесненной сессии, очищает её ассоциацию worktree в памяти и сообщает, действительно ли вытесненная сессия исчезла. Выживший — потомок, который всё ещё удерживает фоновую работу, поэтому idle-закрытие, которое запускает последнее отсоединение, отклоняется и откладывается — выявляется, а не замалчивается: демон логирует его, барьер остаётся установленным на этой записи как единственный оставшийся забор, и тело `200` несёт `supersededSessionLive: true`, чтобы вызывающий знал, что вытесненный id всё ещё жив и может быть повторно подключён внутри checkout, чьё владение только что переместилось. Безопасность при сбое — для каждого окна, и порядок записи определяет окна: ссылка `supersededBy` старого sidecar записывается первой, затем sidecar замены с `supersedes`, и маркер переключается последним. Сбой до переключения поэтому оставляет старую сессию авторитетной в одном из трёх форматов, которые повторная попытка различает — никаких ссылок вообще (замена — обычный сирота, и повторная попытка просто начинает свежую передачу), только обратная ссылка при отсутствующем sidecar замены (повторная попытка завершается fail closed с `worktree_reset_invalid_state` и оставляет прерванное состояние нетронутым для восстановления оператором) или обе ссылки присутствуют (повторная попытка откатывает частичную передачу и завершает свежую — если только прерванная замена не может быть удалена, потому что клиент всё ещё подключён к ней, в этом случае откат завершается fail closed с `worktree_reset_invalid_state` и сохраняет обе ссылки, поэтому последующая повторная попытка сходится, когда этот клиент исчезает). Сбой после переключения оставляет замену авторитетной, и повторная попытка возобновляется как no-op. Старая сессия никогда не удаляется, и checkout worktree никогда не удаляется сбросом.

**Ошибки:**

- `404` — `session_not_found`, когда запись сессии не сущ��ствует.
- `409` — `worktree_reset_unsupported`, когда сессия не имеет валидного sidecar части 4A (не сессия worktree).
- `409` — `worktree_reset_active`, когда сессия, участвующая в передаче, занята; повторите попытку после её урегулирования. Допуск любого писателя, которого охватывает барьер — промпт, перемотка, смена cwd, branch, fork, shell-команда, управление goal или действие workflow-задачи — на сессии во время передачи завершается с тем же кодом.
- `409` — `worktree_reset_invalid_state` для повреждённого sidecar, невалидного маркера, сбоя containment или несогласованной пары ссылок вытеснения. Не деструктивно для того, что начал этот запрос: любая частичная передача, начатая этим запросом, откатывается до того, как вызывающий увидит её, тогда как ветви возобновления fail closed — невалидный маркер, пара ссылок вытеснения, которая не согласована (включая обратную ссылку, чей sidecar замены отсутствует), неоднозначный владелец маркера и замена, которая всё ещё жива на этом демоне при отсутствующем маркере — оставляют предсуществующее прерванное состояние нетронутым для восстановления оператором, поэтому эта форма остаётся видимой для последующего сброса или восстановления. Повторная попытка против одного из этих состояний перечитывает его и возвращает тот же 409: она не сходится, поэтому состояние требует восстановления оператором, а не цикла повторных попыток. Конкретная причина записывается в лог демона, а не в провод.
- `500` — fail-closed внутренние ошибки (например, бридж без поддержки сброса или отклонение перемещения после отката); не повторяемы как есть. Один `500` отличается по природе: передача маркера, чей собственный пост-коммит хвост завершился сбоем _после_ того, как переключение надёжно зафиксировалось, сообщает о перемещении владения, никогда не запускает деструктивный откат и восстанавливается повторной попыткой сброса, который идемпотентно возобновляет зафиксированный id передачи.
...
Доверенные активные списки включают поля live-оверлея демона, такие как `clientCount`, `hasActivePrompt` и `activeWorkState`. Списки недоверенных вторичных и архивные списки предназначены только для хранилища: поля live-оверлея остаются отсутствующими или ложными, а архивные записи устанавливают `isArchived` в `true`. Возвращается пустой массив (а не 404), если сессий не существует — UI выбора сессии не должен выдавать ошибку только из-за того, что рабочее пространство неактивно.
...
`v` — версия схемы ответа. Каждый успешный ответ включает `Cache-Control: no-store`. `sessions` — это полный, непагинированный, неупорядоченный набор сессий, находящихся в живом состоянии в выбранном рантайме; пустой живой рантайм возвращает `200` с `sessions: []`. `clientCount`, `hasActivePrompt`, `isWaitingForPermission` и `isWaitingForUserQuestion` — обязательные поля провода, а отсутствующие необязательные значения бриджа проецируются в `0` или `false`. `activeWorkState` — wire-аддитивное поле, отсутствующее на старых демонах: `active` означает, что демон владеет неурегулированной работой или потомок отправил свежий непустой снимок удержания; `idle` выдаётся только для свежего пустого снимка, покрывающего каждую обязательную категорию; `unknown` означает, что согласованная отчётность устарела или неполна; а `unsupported` означает, что потомок не согласовал отчётность. Это не изменяет `hasActivePrompt`: фоновая оболочка, cron-ход или ожидающее уведомление терминала — это активная работа, не становящаяся фронтенд-промптом. Статические поля каталога, такие как отображаемое имя, время создания, организация и метаданные источника, намеренно исключены и остаются во владении полного каталога. Отсутствующая строка live-state очищает только изменчивые поля известной строки каталога; она никогда не удаляет строку сохранённого каталога.
...
- `400` — существующее поведение проверки селектора или `workspace_mismatch` для неизвестного, некорректного, вложенного или незарегистрированного селектора; маршрут никогда не разрешает неизвестный селектор в основной рантайм.
- `403` — `untrusted_workspace` для любого недоверенного рантайма, включая недоверенное основное рабочее пространство.
- `503` — `workspace_runtime_unavailable` с `Retry-After` для рантайма на этапе бутстрапа, перехода, дренирования, блокировки или удаления, или поколения рантайма, закрывающегося во время запроса.
- `500` — неожиданные локальные ошибки используют существующее маппирование ошибок бриджа.
...
`resolveConflicts` является опциональным и по умолчанию равен `false`. По умолчанию активные и архивные файлы с одинаковым id сообщаются в `errors`, и ни одна копия не перемещается, не удаляется и не перезаписывается. Архивирование живой сессии всё равно выполняет строгое закрытие, описанное выше, перед классификацией конфликта, поэтому это закрытие может сбросить очередь записей в активный транскрипт. При `resolveConflicts: true` архивация восстанавливает конфликт только когда обе копии являются обычными файлами транскриптов, которые выбранное рабочее пространство может обслуживать, включая принадлежащие ему пустые или повреждённые транскрипты. Она сохраняет архивную копию, удаляет активную копию и сообщает id в обоих `archived` и `resolvedConflicts`. Опция не обходит проверки владения; смешанное локальное/чужое или иначе неоднозначное владение сообщается в `errors`, и ни одна копия не перемещается. Записи `errors` имеют формат `{ "sessionId": "<uuid>", "error": "message" }`.

Архивированная сессия возвращается в `alreadyArchived` после того, как демон получает аренду писателя или обслуживания для согласования ожидающей очистки sidecar. Если другой процесс всё ещё удерживает аренду писателя, архивированный id сообщается в `errors` до тех пор, пока аренда не станет доступной.

Перемещение транскрипта или восстановление конфликта не откатывается, если последующая проверка владения очисткой завершается сбоем. В этом случае id может появиться только в `errors`, даже если состояние архива уже изменилось, и может быть пропущен в `archived` и `resolvedConflicts`. Повторите тот же запрос жизненного цикла для того же рабочего пространства, прежде чем считать ошибку доказательством того, что активная копия или конфликт остаются. Когда сервис может проверить идентичность сохранённого транскрипта и получить требуемую аренду писателя или обслуживания, повторная попытка сообщает авторитетное состояние `alreadyArchived` даже для пустых или повреждённых транскриптов, которые список сессий пропускает, и возобновляет ожидающую очистку sidecar. Транскрипты, чья сохранённая идентичность не может быть проверена, продолжают сообщать `errors` при повторной попытке и требуют ручной проверки; повторные попытки, которые не могут получить требуемую аренду, остаются в `errors` до её доступности.

Конфликты жизненного цикла являются результатами элементов пакета: маршруты без рабочего пространства и с указанием рабочего пространства возвращают HTTP `200` с конфликтом в `errors`. Это заменяет прежний HTTP `409 session_conflict` для маршрутов с указанием рабочего пространства; клиенты, вызывавшие этот маршрут, должны исследовать ответ пакета. Внутренние REST-пакеты рантайма сохраняют безопасное сообщение о конфликте, продолжая скрывать другие детали ошибок для каждой сессии.
...
`resolveConflicts` является опциональным и по умолчанию равен `false`. По умолчанию одновременные активные и архивные файлы JSONL создают конфликт в `errors`, и ни одна копия не перемещается, не удаляется и не перезаписывается; сессия только с активной копией возвращается в `alreadyActive`. При `resolveConflicts: true` разархивация сохраняет активную копию, удаляет архивную копию и сообщает id в обоих `unarchived` и `resolvedConflicts`. Выполняющееся в данный момент архивирование или разархивирование для того же id возвращает `409 session_archiving` до начала пакетной обработки.
...
Идемпотентно: возвращает `404` для неизвестных сессий. Конверт ошибки использует `code: "session_not_found"`; одновременное закрытие может вернуть `code: "session_closing"`, что клиенты могут трактовать как то же успешное терминальное состояние для этого маршрута.
...
Демон предоставляет пять маршрутов управления мутациями, которые позволяют удалённым клиентам изменять состояние среды выполнения без использования CLI хоста демона. Управление режимом одобрения сохраняет свой нестрогий гейт совместимости. Переключение инструментов, переключение навыков, инициализация рабочего пространства и перезапуск MCP используют строгий гейт мутации: проходят запросы от основного клиента с доверенным loopback, запросы с bearer-аутентификацией и сопряжённые запросы Local Control. Основной запрос без токена, достигший гейта без полномочий доверенного loopback, получает `401 {code: 'token_required'}`; отсутствующие или некорректные настроенные учётные данные и несопряжённые учётные данные Local Control отклоняются ранее с простым `401 Unauthorized`. Все пять:

- Принимают и помечают заголовок `X-Qwen-Client-Id` (цепочка аудита PR 7). Когда заголовок несёт доверенный id, демон выдаёт `originatorClientId` в соответствующем событии SSE, чтобы UI других клиентов могли подавлять эхо собственных мутаций.
- Выполняют pre-flight проверку каждой возможности для каждого тега перед предоставлением доступа. Демон, у которого отсутствует маршрут, возвращает `404`. Для маршрутов настроек навыков отсутствие тега может означать, что те же пути обслуживают устаревший контракт с проверкой по каталогу: маршрут для одной цели может вернуть HTTP `404 skill_not_found` или `409 skill_not_toggleable`, тогда как пакетный маршрут возвращает HTTP 200 с ошибками из каталога в `errors[]`. Не используйте зондирование маршрутов как проверку версии.
...
#### Разделение маршрутов Skills config и runtime

Тег возможности: `workspace_skills_config_runtime`.

`GET /workspace/config/skills` читает локальный для демона владелец глобальной конфигурации. `GET /workspaces/:workspace/config/skills` читает точную зарегистрированную конфигурацию рабочего пространства и не требует живого или доверенного рантайма. Оба возвращают обычную форму статуса Skills без обязательного `runtimeEpoch`:
...
`GET /workspace/runtime/skills` и `GET /workspaces/:workspace/runtime/skills` читают выбранный доверенный рантайм без его запуска. Живой каталог включает поколение рантайма, которое его создало:
...
Глобальная установка и удаление используют `POST /workspace/config/skills/install` и `DELETE /workspace/config/skills/:name?scope=global`. Установка, удаление и включение в рабочем пространстве используют `POST /workspaces/:workspace/config/skills/install`, `DELETE /workspaces/:workspace/config/skills/:name?scope=workspace` и `POST /workspaces/:workspace/config/skills/:name/enable`. Записи конфигурации фиксируют долговременное состояние перед планированием согласования рантайма. Их `activation` равен `deferred`, когда текущий рантайм не может быть обновлён, и `reconciling`, когда обновление поставлено в очередь; no-op переключение сохраняет активацию фасада вместо утверждения, что произошло обновление рантайма.

Удаление конфигурации возвращает `503 skills_config_unavailable` вместо окончательного ответа not-found, когда инвентарь навыков локальный для демона не может быть перечислен.

Единственный владелец мутации отклоняет область рабочего пространства с `400 workspace_scope_requires_qualified_workspace`; квалифицированные мутации отклоняют глобальную область с `400 global_scope_requires_singular_owner`. Квалифицированные записи требуют доверенного рабочего пространства. Устаревший инжектированный бридж возвращает `501 workspace_runtime_not_supported` для квалифицированных записей конфигурации, требующих координации рантайма. Неизвестные, удалённые, переходные и дренируемые рабочие пространства следуют стандартным ошибкам квалифицированного рабочего пространства и никогда не делают фолбэк на основной.

#### `POST /workspace/skills/:name/enable`

Тег возможности: `workspace_skill_settings_toggle`. Форма с указанием рабочего пространства — `POST /workspaces/:workspace/skills/:name/enable`.

Обновляет настройки навыков рабочего пространства для имени без обращения к загруженному каталогу навыков. Обрезанное имя из запроса передаётся в хранилище и возвращается в ответе. Включение любого имени записывает opt-in `skills.enabled` рабочего пространства, даже до установки; отключение удаляет этот opt-in и добавляет запись `skills.disabled` рабочего пространства. Существующие записи для навыков, которые больше не загружены, сохраняются, а дублирующиеся записи или записи с другим регистром для цели сворачиваются. Жёсткая запись `skills.disabled`, унаследованная от области более высокого уровня, остаётся авторитетной для эффективной доступности, но не предотвращает область рабочего пространства от записи или удаления своей собственной декларации. В остальном декларации рабочего пространства участвуют в обычном разрешении `skills.disabled > skills.enabled > skills.defaultDisabled` и могут переопределять записи `skills.defaultDisabled` или `skills.enabled` области более высокого уровня.

Это отличается от операции управляемого навыка ACP `qwen/skills/setEnabled` и поля frontmatter `disable-model-invocation`. Для активного родительского расширения эффективная доступность навыка следует `skills.disabled` > `skills.enabled` > `skills.defaultDisabled` > внутреннее переопределение рабочего пространства > `skillStates` манифеста > включён. Как жёсткое, так и стандартное отключение удаляют навык из доступности slash-команд/модели и отклоняют последующее выполнение навыка. `disable-model-invocation: true` сохраняет прямой вызов пользователем доступным и только скрывает навык от вызова моделью.
...
`activation` отражает живость дочернего процесса и любое требуемое обновление независимо от `changed`. Он равен `applied`, когда дочерний процесс ACP жив и любое требуемое обновление успешно, `deferred`, когда дочерний процесс не был жив при проверке живости или изменённый запрос теряет свой дочерний процесс/сессию во время требуемого обновления, и `partial`, когда хотя бы одно другое требуемое обновление завершается ошибкой. No-op поэтому может быть `applied` или `deferred` с `changed: false`; когда `changed` истинно и активация равна `deferred`, сохранённая декларация используется при запуске дочернего процесса. Занятые сессии включаются в требуемое обновление. Демон перезагружает настройки рабочего пространства для дочернего процесса ACP и каждой активной сессии, уведомляет потребителей SkillManager и отправляет `available_commands_update`. Запрос, уже отправленный модели, не переписывается; последующая валидация, снимки команд и контексты модели используют новое состояние. Если сохранение не удаётся, обновление или событие не выдаются. Если обновление сессии не удаётся, зафиксированная настройка сохраняется. Когда дочерний процесс возвращает результаты для каждой сессии, счётчики сессий точны. Если сам контроль обновления завершается ошибкой до возврата этих результатов, `sessionsFailed: 1` является консервативной нижней границей, указывающей на сбой запроса обновления.
...
Мутация повторно использует событие `settings_changed` в масштабе рабочего пространства для каждого изменённого ключа (`skills.disabled` и/или `skills.enabled`); она не добавляет новый тип события. Каждое из этих событий включает тот же объект `mutation`: `{ id, kind: 'skill_toggle', skills: [{ name, enabled }], activation, sessionsRefreshed, sessionsFailed }`. `id` коррелирует каждое событие настроек, созданное одним запросом переключения. `skills` перечисляет запрошенные имена и запрошенные значения включённости, декларации настроек рабочего пространства которых фактически изменились; настройка более высокой области может оставить эффективную доступность неизменной. Ячейки статуса навыков рабочего пространства включают опциональные поля `disabledReason: 'hard' | 'default' | 'inactive_extension'` и `lockedScope: 'system' | 'user' | 'systemDefaults'`.

#### `POST /workspace/skills/enable`

Тег возможности: `workspace_skill_settings_batch_toggle`. Форма с указанием рабочего пространства — `POST /workspaces/:workspace/skills/enable`.

Обновляет настройки навыков рабочего пространства для до 100 имён в одном запросе; лимит считает сырые записи `skillNames` до дедупликации. Имена обрезаются и дедуплицируются без учёта регистра с сохранением порядка первого вхождения и регистра. Демон не обращается к загруженному каталогу навыков. Он применяет все результирующие изменения деклараций не более чем в одной блокированной записи настроек и, когда что-то изменилось, обновляет активные сессии один раз. Включение всегда записывает явный opt-in `skills.enabled` рабочего пространства, включая ещё не установленные имена, поэтому оно может переопределить внутреннее отключение расширения. Повторение идентичной декларации остаётся no-op. Неожиданные сбои сохранения или генерации рантайма проваливают весь запрос.
...
Некорректно сформированные запросы возвращают HTTP 400 с `invalid_skill_names`, `invalid_skill_name` или `invalid_enabled_flag`. Аутентификация, доверие рабочего пространства, идентификация клиента, неожиданные сбои сохранения и сбои генерации рантайма проваливают весь запрос через стандартные гейты маршрута. `errors` остаётся в ответе для совместимости провода и пуст для структурно валидных имён. Batch-level `activation`, `sessionsRefreshed` и `sessionsFailed` описывают живость дочернего процесса и единое обновление живой сессии, общее для всех изменённых результатов. Пакет, в котором ни одна цель не изм��нилась, всё равно может ответить `applied`, когда дочерний процесс жив, или `deferred`, когда его нет, соответствуя ответу no-op для одного навыка, поэтому выводите фактические изменения из флага `changed` каждого результата. Когда хотя бы одна цель изменяется, демон выдаёт те же метаданные мутации `settings_changed`, что и маршрут для одного навыка; каждое событие `skills.disabled` / `skills.enabled` из этого запроса разделяет один `mutation.id`.

#### `POST /workspace/init`

Тег возможности: `workspace_init`. Чистый файловый ввод-вывод — без обращения к ACP, **без вызова LLM**.

Создает пустой `QWEN.md` (или переопределение `context.fileName` в настройках рабочего пространства) в корне основного рабочего пространства демона. Только механическое действие — для заполнения содержимого с помощью ИИ, выполните `POST /session/:id/prompt`.
...
Тег возможности: `user_language_sync` (условный — анонсируется только когда доступно сохранение настроек, то же условие, что и для `workspace_settings`). Bridge → ACP extMethod `qwen/control/user/language` для каждого доверенного рантайма с живым каналом.

Бессессионная синхронизация языка на уровне **пользователя** (issue #10234), для хостов, которым нужно переключить язык до существования какой-либо сессии (например, страница приветствия). Классификация владения: общепроцессная — она мутирует только глобальное состояние пользователя (`~/.qwen/settings.json`, глобальный `~/.qwen/output-language.md`) плюс обновление рантайма по мере возможности (best-effort), поэтому не принимает ни селектор рабочего пространства, ни id сессии. Зарегистрирован за нестрогим гейтом `mutate()`, соответствуя родственному `POST /session/:id/language`.

Процесс демона является единственным писателем: он сохраняет `general.language` (и, когда `syncOutputLanguage` истинно, `general.outputLanguage` плюс глобальный `output-language.md`) перед рассылкой. Каждый доверенный рантайм затем переключает свой собственный язык UI процесса и перезагружает настройки области пользователя с диска; когда `syncOutputLanguage` истинно, он также обновляет системную инструкцию каждой локальной сессии. Отличия от маршрута с областью сессии:

- Файлы output-language, привязанные к проекту, НЕ переписываются — сессия, чьё рабочее пространство имеет собственный `.qwen/output-language.md`, сохраняет это переопределение (область проекта побеждает при обновлении).

- Ноль сессий и ноль живых каналов рантайма — это **успех**, а не ошибка; рантайм без живого канала пропускается (не считается сбоем) и читает сохранённые файлы при следующем создании его канала.
- Рантаймы недоверенного рабочего пространства пропускаются, соответствуя позиции `403 untrusted_workspace` маршрута сессии.
...
`language` должен быть одним из кодов, перечисленных в `supportedLanguages` маршрута `/capabilities` (плюс `'auto'`); `syncOutputLanguage` по умолчанию равен `false`.
...
`language` — это разрешённый демоном UI-язык, поэтому запрос `'auto'` может вернуть конкретный язык; хосты должны читать сохранённую настройку `general.language` для состояния селектора. `outputLanguage` равен `null`, когда `syncOutputLanguage` был false. `refresh.runtimes` считает рантаймы, применившие переключение через живой канал; `refresh.sessions` суммирует обновления системной инструкции для каждой сессии; `refresh.failed` агрегирует сбои обновления для каждой сессии и сбои рантаймов (рассылка выполняется по мере возможности (best-effort) и никогда не проваливает запрос после успешного сохранения).

Ошибки:

- `400 {code: 'invalid_language', allowed: [...]}` — неизвестный код языка.
- `400 {code: 'invalid_sync_flag'}` — `syncOutputLanguage` не является булевым значением.
- `400 {code: 'invalid_client_id'}` — `X-Qwen-Client-Id` не является клиентом, известным какому-либо рантайму (заголовок необязателен; опустите его, когда сессия не существует).
- `500 {code: 'persist_error'}` — сбой записи пользовательских настроек или файла output-language; сбойный шаг и всё после него не выполнились (ранее применённые шаги, такие как запись `general.language`, не откатываются).
- `404` — демон predates маршрут или был собран без сохранения настроек (проверьте тег возможности).

Событие SSE (в масштабе рабочего пространства, на шинах сессий каждого рантайма): `language_changed` с `{language, outputLanguage, userLevel: true, originatorClientId?}`. Маркер `userLevel: true` отличает эту рассылку от события для каждой сессии маршрута сессии, которое несёт `sessionId` вместо этого.
...
Отправьте тот же голос, что документирован ниже, но направьте его через рантайм, владеющий
именованной живой сессией. Новые интеграции с несколькими рабочими пространствами должны использовать эту форму
вместо устаревшего общепроцессного маршрута. Pre-flight
`caps.features.session_permission_vote`.

Тело запроса, политики медиации, результаты и успешный ответ
идентичны `POST /permission/:requestId`. Опциональный
заголовок `X-Qwen-Client-Id` участвует в политиках designated и consensus.
Сбои используют стабильные значения `code`, где указано; некорректный ввод и проигранная
гонка ожидающего запроса могут опустить `code`:

- `400` — некорректное тело голоса (без `code`) или недействительная идентификация клиента
  (`invalid_client_id`), или `invalid_option_id`, когда выбранный вариант не
  был предложен. Перечитайте предложенные варианты вместо повторной попытки того же голоса.
- `403` — `permission_forbidden`, когда активная политика отклоняет голосующего, или
  `untrusted_workspace`, когда неосновное владеющее рабочее пространство не является доверенным.
  Неосновной владелец освобождён от этой проверки доверия, и голос может
  быть принят.
- `404` — `session_not_found`, когда живой владелец отсутствует, или без `code`, когда
  запрос не ожидает обработки.
- `500` — `cancel_sentinel_collision`, когда `allowedOptionIds` агента
  содержит зарезервированный sentinel `__cancelled__`, или `ambiguous_session_owner`,
  когда более одного рабочего пространства претендует на сессию.
- `501` — `permission_policy_not_implemented` для политики, которую эта сборка не
  реализует.
- `503` — `workspace_runtime_unavailable`, когда владеющий рантайм
  недоступен, или `daemon_draining`, когда демон больше не принимает
  работу.

Он никогда не повторяет попытку против основного бриджа. Метод TypeScript SDK —
...
> **Таймаут разрешения (отключен по умолчанию).** `permission_request`
...
> не отключится (отмена в середине промпта разрешает ожидающие разрешения как `cancelled`),
> (d) сессия не будет убита, (e) демон не завершит работу, **или
> (f) не сработает настроенный таймаут**. При срабатывании таймаута `requestPermission` агента
> разрешается как `{outcome: 'cancelled'}`, кольцо аудита записывает запись
...
> Общий таймаут настраивается через
> `BridgeOptions.permissionResponseTimeoutMs` или
> `qwen serve --permission-response-timeout-ms`. Его значение по умолчанию — `0`, поэтому
> обычные разрешения и `ask_user_question` ожидают решения человека бесконечно.
> Отмена голосующим, отмена сессии, очистка при отключении и
> завершение работы демона по-прежнему разрешают ожидающие взаимодействия как отменённые.
...
Строгий шлюз мутации: основные запросы без токена с доверенным loopback проходят. Основной запрос без токена, достигающий шлюза без полномочий доверенного loopback, получает `401 token_required`; отсутствующие или некорректные настроенные учётные данные и несопряжённые учётные данные Local Control отклоняются ранее с простым `401 Unauthorized`.
...
- `401 token_required` — строгий шлюз отклонил основной запрос без токена без полномочий доверенного loopback

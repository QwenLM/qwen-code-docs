...
Если параметр не задан, явные разовые запросы используют `child_process`; интерактивные сеансы TUI, ACP, stream-json ввода, stdin-only и file-input используют PTY. В сборках Windows **<= 19041** (до Windows 10 версии 2004) режим PTY переключается на `child_process`, поскольку старые реализации ConPTY имеют известные проблемы с надежностью (отсутствие вывода, зависания). Это соответствует той же границе, используемой в VS Code ([microsoft/vscode#123725](https://github.com/microsoft/vscode/issues/123725)). Если `node-pty` недоступен во время выполнения, инструмент также переключается на `child_process`.

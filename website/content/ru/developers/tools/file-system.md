...
**Примечание:** Этот инструмент является включаемым явно (opt-in) и отключён по умолчанию, поскольку `glob` в большинстве случаев покрывает вывод списка каталогов. Включите его, установив `tools.listDirectory.enabled` в `true` в настройках, или явно указав `list_directory` в allowlist `coreTools` (`--core-tools` / `tools.core`).
...
`read_file` читает и возвращает содержимое указанного файла. Этот инструмент обрабатывает текстовые файлы и медиафайлы (изображения, PDF, аудио, видео), модальность которых поддерживается текущей моделью. Для текстовых файлов может читать определённые диапазоны строк. Неподдерживаемые PDF пытаются извлечь текст, а также используется ограниченный vision фолбэк, описанный ниже; другие неподдерживаемые медиафайлы возвращают информативное сообщение об ошибке. Другие типы бинарных файлов обычно пропускаются.

- **Имя инструмента:** `read_file`
- **Отображаемое имя:** ReadFile
- **Файл:** `read-file.ts`
- **Параметры:**
  - `file_path` (строка, обязательный): Абсолютный путь к файлу для чтения.
  ...
  - `pages` (строка, опциональный): Для PDF — 1-индексная страница или закрытый диапазон страниц, например `"3"` или `"20-25"`. Запрос может содержать не более 20 страниц.
- **Поведение:**
  ...
  - Для PDF с текстовой основной моделью: сначала предпринимается попытка извлечения текста. Если извлечение не удаётся, или явно запрошенная (или фактическая) отдельная страница всё равно превышает бюджет текста в 12K токенов, настроенный vision бридж автоматически рендерит и транскрибирует не более четырёх страниц, начиная с запрошенной первой страницы. Запрошенный диапазон обрезается до фактического конца документа, когда он известен. Результат определяет транскрибированный диапазон и либо известные оставшиеся страницы, либо, когда количество страниц недоступно, указывает, что могут существовать дополнительные страницы. Обычное многостраничное переполнение текста по-прежнему запрашивает более узкий диапазон `pages` вместо переключения на vision.
  - Транскрипция PDF через vision бридж является потерей и помечается как ненадёжный машиногенерированный контент. Результат инструмента содержит текст, а не отрендеренные изображения, и его пользовательский TUI, ACP, неинтерактивный структурированный вывод и экспортные отображения определяют vision модель и endpoint, когда они известны. Если бридж не работает, точная исходная ошибка извлечения PDF возвращается модели, в то время как отображение для пользователя по-прежнему раскрывает попытку бриджа.

...
2. Эти скрипты сборки не включены в пакеты, выпускаемые npm.
3. Код содержит жестко заданные проверки пути, которые явно отклоняют запросы на сборку из сред, отличных от среды исходного кода.

Если вам нужны дополнительные инструменты внутри контейнера (например, `git`, `python`, `rg`), создайте собственный Dockerfile. Конкретная операция выглядит следующим образом
...
# 6. Для получения деталей символической ссылки можно посмотреть конкретный путь к исходному коду

...
После потери соединения текущий вызов воспроизводится только для доверенного сервера в доверенном рабочем пространстве, когда инструмент явно объявляет `idempotentHint: true` или объявляет `readOnlyHint: true` без конфликтующего `destructiveHint: true` или `idempotentHint: false`. Отсутствующие или конфликтующие аннотации, а также аннотации от недоверенного сервера или рабочего пространства, считаются небезопасными, поскольку сервер мог завершить побочный эффект до потери ответа. Авторы инструментов должны публиковать точные аннотации MCP; администраторы всё равно должны проверять их перед включением доверия к серверу.
...
- **`versionNegotiation`** (`"auto" | "legacy"`, по умолчанию: `"legacy"`): Для серверов Stdio значение `"auto"` включает зонд `server/discover` на одноразовом дочернем процессе.
- **`trust`** (логическое): Если `true`, обходит подтверждения вызовов инструментов для этого сервера в доверенном рабочем пространстве (по умолчанию: `false`)
...
- **Пользовательский URI перенаправления**: Используйте `--oauth-redirect-uri` или настройте `redirectUri` в settings.json, чтобы указать публичный URL, заканчивающийся на `/oauth/callback`. Направьте этот путь через reverse-proxy на `http://127.0.0.1:7777/oauth/callback` на машине с запущенным Qwen Code.
...
- Вы **должны** настроить пользовательский `redirectUri`, указывающий на общедоступный URL, заканчивающийся на `/oauth/callback`
- Завершайте TLS на reverse proxy и перенаправляйте только этот путь на `http://127.0.0.1:7777/oauth/callback`
...
- Headless-окружениях без доступа к браузеру
...
- **`redirectUri`** (строка): Пользовательский URI перенаправления. **Критически важно для удалённых развёртываний**: По умолчанию `http://localhost:7777/oauth/callback`. Для удалённого использования укажите публичный URL, заканчивающийся на `/oauth/callback`, и направьте его через reverse-proxy на локальный callback-слушатель. Можно настроить через `qwen mcp add --oauth-redirect-uri` или напрямую в settings.json.
...
- **Настройки доверия:** Опция `trust` обходит диалоги подтверждения инструментов только в доверенном рабочем пространстве. Используйте с осторожностью и только для серверов, которые вы полностью контролируете
...
- `--trust`: Доверять серверу (обходить подтверждения вызовов инструментов в доверенном рабочем пространстве).
...
- `--oauth-redirect-uri`: URI перенаправления OAuth (например, `https://your-server.com/oauth/callback`). По умолчанию `http://localhost:7777/oauth/callback` для локальных настроек. **Важно для удалённых развёртываний**: Используйте публичный URL, заканчивающийся на `/oauth/callback`, и направьте его через reverse-proxy на `http://127.0.0.1:7777/oauth/callback`.

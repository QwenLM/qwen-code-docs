...
Инструмент отключен по умолчанию. Включите его в `settings.json` и перезапустите Qwen
Code:
...
Когда инструмент включен, ИИ-ассистент может использовать его для сложных многошаговых задач. Вы также можете попросить ассистента создать список задач, если хотите увидеть запланированный подход к вашему запросу.
...
- **Включение явно:** Установите `tools.todoWrite.enabled` в `true` и перезапустите Qwen Code перед использованием инструмента.
- **Автоматическое использование при включении:** ИИ-ассистент управляет списками задач при выполнении сложных задач.

...
| `packages/web-shell`                                                                                             | Браузерный UI и React-адаптер демона, построенные на TypeScript SDK.                                                                                                                                   |
...
| `packages/desktop-shell`, `packages/vscode-ide-companion`, `packages/chrome-extension`, `packages/zed-extension` | Продуктовые и редакторские интерфейсы, адаптирующие Qwen Code к среде хоста.                                                                                                                           |
...
- `packages/web-shell` адаптирует состояние демона к React-провайдерам и хукам
  и обеспечивает браузерный опыт;
- IDE- и канальные пакеты преобразуют специфичные для хоста события в общие
  контракты клиента или бриджа.
...
`packages/web-shell` строит слой состояния React и браузерный UI на клиенте
демона. Другие клиенты, включая IDE-интеграции и управляемые демоном каналы,
переиспользуют те же SDK и контракты событий вместо импорта кода серверной
реализации.
...
- **Плагины каналов** адаптируют платформы обмена сообщениями к общей среде
  выполнения каналов.
- **SDK-клиенты** создают пользовательские локальные приложения или приложения на базе демона.

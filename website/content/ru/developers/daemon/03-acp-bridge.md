...
Каждая активная `WorkspaceRuntime` владеет одним экземпляром `HttpAcpBridge`. Продакшен пытается выполнить прогрев доверенного первичного дочернего процесса для совместимости; доверенные вторичные открывают свой `AcpChannel` при первой команде среды выполнения или Session, а недоверенные вторичные не могут запустить ACP. Устаревшие маршруты первичного процесса сохраняют свое существующее поведение совместимости. Внутри среды выполнения bridge предоставляет мультиплексированные сессии через канал, `EventBus` для каждой сессии (`EventBus`es), `MultiClientPermissionMediator`, адаптер `BridgeFileSystem` и ACP-ориентированные помощники (`spawnOrAttach`, `loadSession`, `resumeSession`, `sendPrompt`, `cancelSession`, `respondToPermission`, а также RPC-вызовы extMethod для статуса рабочего пространства и перезапуска MCP). Bridge и дочерние процессы никогда не используются совместно между средами выполнения рабочих пространств (workspace runtimes).
...
**Инвариант `isDying`**: любой путь завершения должен синхронно устанавливать `ChannelInfo.isDying = true` **до** ожидания `channel.kill()`. `ensureChannel` считает умирающий канал отсутствующим и создает новый. Без этого флага параллельный `spawnOrAttach`, поступающий во время окна корректного завершения SIGTERM (до 10 с), подключится к транспорту, который вот-вот закроется, и sessionId вызывающего будет возвращать 404 при всех последующих запросах. Завершение включает сбои инициализации, неудачные попытки запуска на пустом канале, истечение времени простоя рабочего пространства и shutdown; закрытие последней Session только освобождает её аренду и планирует политику простоя рабочего пространства.
...
- `defaultEntry` — это переиспользуемая логическая Session при `sessionScope: 'single'`. Закрытие Session удаляет только её аренду Session. После завершения дренирования всей работы Session, восстановления, управления рабочим пространством, обнаружения, аутентификации и операций среды выполнения и отсутствия ожидающего окна поддержания явного ensure, пропущенный или нулевой `channelIdleTimeoutMs` завершает дочерний процесс немедленно. Простой прогрев сохраняется для первого использования и сам по себе не активирует немедленное завершение. Положительное значение или активное окно поддержания задерживает завершение; побеждает большая оставшаяся задержка.
...
| `channelIdleTimeoutMs`                        | `0`                                                | Задержка автоматического завершения; простой прогрев переживает первое чтение статуса, а активное поддержание может его продлить.                                    |

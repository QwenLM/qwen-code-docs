...
- **Нормализация** (`normalizer.ts`): сопоставляет 53 известных типа событий wire-схемы демона (см. [`09-event-schema.md`](./09-event-schema.md)) с 43 семантическими событиями `DaemonUiEventType`, удобными для UI, такими как `assistant.text.delta`, `tool.update` и `session.metadata.changed`.
...
- Нормализует 53 wire-события демона в стабильный UI-словарь (`DaemonUiEventType`), чтобы рендерерам не приходилось проверять `rawEvent.data`.
...
`ui/types.ts` определяет 43 типа событий UI, сгруппированных по доменам.
...
`normalizeDaemonEvent` сопоставляет 53 известных wire-события демона с этим словарем. Неизвестные, не смоделированные или некорректные типы событий нормализуются в `debug` и сохраняют `rawEvent` для диагностики хоста.
...
flowchart LR
    ...
    D --> E["DaemonUiEvent<br/>(43 типа, удобных для UI)"]
...
`session.state_resync_required` сопоставляется с маркером "пропущенного диапазона" в транскрипте. UI-код может вызвать `formatMissedRange(state)` для рендеринга текста вроде "пропущены события X-Y". Редьюсер устанавливает `awaitingResync` и пропускает обычные дельта-события, пока код потребителя не перезагрузит ограниченное окно снимка повтора сессии и не снимет блокировку. Загруженный снимок может начинаться с `history_truncated`; этот маркер отображается только как статус и не должен запускать новый цикл ресинхронизации. См. [`10-event-bus.md`](./10-event-bus.md) для получения информации о семантике ring-eviction и `state_resync_required`.

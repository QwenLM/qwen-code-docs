...
| `workspaceCwd`                                        | `connect(options)`               | Используется в `POST /session`; должен совпадать с основным рабочим пространством демона или зарегистрированной средой выполнения мульти-воркспейс сессии. |
...
- **Несовпадение `workspaceCwd`** с зарегистрированными рабочими пространствами демона возвращает `400 workspace_mismatch` — показывайте это как понятную ошибку настройки, а не повторяйте попытки.

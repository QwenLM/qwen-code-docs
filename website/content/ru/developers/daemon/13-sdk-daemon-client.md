...
Для пакетных изменений выполните pre-flight `workspace_skill_settings_batch_toggle` и вызовите любой из вариантов клиента с тем же контрактом. Маршруты и тела запросов не изменились:
...
`DaemonSkillBatchToggleResult` содержит упорядоченные `results`, массив `errors` для совместимости и batch-level счётчики активации/обновления сессий. Текущие демоны обрабатывают каждое структурно валидное имя в порядке запроса, сохраняют все результирующие изменения объявлений вместе не более чем за одну заблокированную запись настроек, обновляют активные сессии один раз при любых изменениях и возвращают пустой массив `errors` без обращения к загруженному каталогу навыков. Включение регистрирует явное включение `skills.enabled` для рабочего пространства даже для ещё не установленных имён, поэтому оно может переопределить внутреннее отключение расширения; идентичное повторяющееся объявление остаётся no-op. Типы элементов ошибок остаются доступными, чтобы SDK мог декодировать ответы от старых демонов. Метод выбрасывает исключение при ответе с кодом, отличным от 200.
...
Результат завершающей операции содержит упорядоченные `results`. Цели не обязаны быть установлены при задании `enabled` или `disabled`: демон сохраняет объявление имени и сохраняет эту политику активации, когда расширение с таким именем устанавливается позже. Все изменённые цели разделяют одну генерацию Extension Store. Когда анонсируется `extension_activation_explicit_refresh`, операции активации завершаются после стойкого коммита политики без обновления активных сессий. Вызывающему коду, которому требуется немедленное применение, следует отправить `workspace.refreshExtensionRuntime()` для каждого рабочего пространства, чьи сессии должны немедленно применить изменение; обновление является отдельной операцией и может быть ожидано или оставлено в фоне. Глобальный пакет по умолчанию изменяет активацию по умолчанию, которую наследует каждое зарегистрированное рабочее пространство, если только это рабочее пространство не содержит точного переопределения для имени (или не соответствует устаревшему правилу пути), и не имеет единого обновления, покрывающего каждый runtime; пакет рабочего пространства изменяет только выбранный доверенный runtime. Старые демоны уже выполняют обновление внутри операции активации, поэтому клиенты не должны отправлять дополнительное обновление, если возможность отсутствует. 30-секундный согласователь генераций остаётся независимым путём окончательной сходимости для рабочих пространств, которые вызывающий код не обновил. `inherit` для рабочего пространства очищает точное переопределение, но не создаёт объявление для неизвестного имени; очистка всех неизвестных целей успешно выполняется как no-op. Единичные методы активации остаются только для установленных расширений.

Для внутренних переключателей навыков расширений в рабочем пространстве выполните pre-flight `extension_state` и используйте сгруппированные по ресурсам REST-методы. Они не записывают настройки навыков и не активируют отключённое родительское расширение:
...
`WorkspaceExtensionState` сообщает значения по умолчанию из манифеста, точные переопределения рабочего пространства и эффективное состояние с учётом настроек. Операция возвращает упорядоченные `resourceStates.skills` и может завершиться успешно с предупреждениями об обновлении. Поддерживается только группа `skills`. Не понижайте эти вызовы до `setWorkspaceSkillEnabled`, который записывает настройки с более высоким приоритетом.

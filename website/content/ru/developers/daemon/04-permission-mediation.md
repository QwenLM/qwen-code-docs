...
отменой голосующим, отменой сессии или опциональным таймаутом взаимодействия:
ни одна опция не достигает единогласия. `permissionResponseTimeoutMs` отключён по
умолчанию; при настройке неразрешённый разделённый голос разрешается как
`{cancelled, timeout}` по этому дедлайну. Путь продвижения голоса логирует
применимое поведение в stderr для операторов.

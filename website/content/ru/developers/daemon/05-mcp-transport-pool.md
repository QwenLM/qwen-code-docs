...
`McpTransportPool` (`packages/core/src/tools/mcp-transport-pool.ts`) — это пул уровня рабочей области в F2 (#4175 commit 5): несколько сессий ACP на одном демоне используют один транспорт на уникальный кортеж `(serverName + configFingerprint)`, вместо того чтобы каждая сессия порождала собственный дочерний процесс MCP. При включённом режиме пула каждый запущенный дочерний процесс ACP владеет независимым пулом (`QwenAgent.mcpPool`). Продакшен пытается выполнить прогрев основного дочернего процесса и повторяет попытку при первом использовании после сбоя; доверенный вторичный запускает свой дочерний процесс по требованию, тогда как недоверенный вторичный не запускает ни того, ни другого. Пул конструируется один раз при запуске агента с загрузочным `Config` среды выполнения и переживает жизненные циклы сессий. Записи ведут подсчёт ссылок на подключения сессий и закрываются после настраиваемого периода ожидания, когда счётчик ссылок достигает нуля.

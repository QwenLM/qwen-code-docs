...
| `file_too_large`           | 413       | Большой текст не имеет конечного лимита строк, большой текст не в UTF-8 или запись превышает `MAX_WRITE_BYTES` (5 МиБ). | Используйте конечный лимит строк для большого UTF-8 текста, используйте байтовое окно или разделите запись.              |
| `hash_mismatch`            | 409       | Ошибка оптимистичной блокировки `expectedSha256` или файл изменился во время стабильного чтения.                        | Перечитайте файл и повторите попытку с новой версией/хешем.                                                              |
...
| `io_error`                 | 503       | `ENOSPC` / `EIO` / `EBUSY` / `ETXTBSY` / `ENAMETOOLONG` / `EMFILE` / `ENFILE`.  | Оперативное исправление на уровне хоста (переполнен диск, исчерпание файловых дескрипторов); вызывайте ops, а не security. |
| `internal_error`           | 500       | Ошибка, не связанная с errno, достигла границы. | Откройте баг демона.                                     |
| `parse_error`              | 400 / 422 | Ошибка разбора тела запроса (400) или нарушение инварианта уровня сервиса (422). | Проверьте тело запроса; проверьте версию SDK.                                                       |

Различие между `io_error` и `permission_denied` сделано намеренно, чтобы пайплайны мониторинга могли маршрутизировать по `errorKind`; объединение ENOSPC в `permission_denied` привело бы к вызову команды реагирования на инциденты безопасности из-за проблемы `df -h`.

## Ошибки моста ACP (`packages/acp-bridge/src/bridgeErrors.ts`)

Типизированные классы, выбрасываемые мостом / посредником. Большинство несёт HTTP-статус через switch обработчика маршрута.

| Класс                                       | HTTP | Причина                                                                                                                                                                                                                                                                                                                                                                                              | Устранение                                                                                                                                                                                                             |
| ------------------------------------------- | ---- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `SessionNotFoundError`                      | 404  | `sessionId` отсутствует в `byId` (`code: "session_not_found"`) или сессия закрывается (`code: "session_closing"`).                                                                                                                                                                                                                                                                                                                                                                             | Для `session_not_found`: пересоздайте или подключитесь; сессия могла быть удалена. Для `session_closing`: подождите и повторите попытку; выполняется параллельное закрытие. DELETE-маршруты трактуют `session_closing` как идемпотентный успех.                  |
...
| `RestoreInProgressError`                    | 409  | `loadSession`, `resumeSession` или переданный вызывающим кодом id в `POST /session` конфликтует с другой регистрацией, уже владеющей тем же id.                                                                                                                                                                                        | Подождите объявленную задержку и повторите запрошенное восстановление или создание; очистка покинутого восстановления использует backoff, выведенный из бюджета.                                                         |
...
| `SessionRestoreTimeoutError`                | 504  | Загрузка/восстановление сессии ACP превысило выделенный бюджет восстановления.                                                                                                                                                                                                                                                                                                                      | Повторите после объявленной задержки; изучите трассировки этапа восстановления перед увеличением бюджета.                                                                                                              |
| `BridgeChannelQuarantinedError`             | 503  | `reason` — это `restore_cleanup_failed`, `restore_settlement_overdue`, `new_session_cleanup_failed` или `new_session_settlement_overdue`. Состояние settlement-overdue очищается после того, как поздний сбой урегулируется или поздний успех завершит очистку по точному ID; неубедительная очистка переходит в соответствующее состояние cleanup-failed. Состояния cleanup-failed длятся до тех пор, пока канал рабочего пространства не завершит дренирование. Тело 503 также содержит `retryAfterSeconds`. | Продолжайте использовать существующие сессии и повторите попытку после объявленной задержки; состояния cleanup-failed требуют переработки канала, тогда как состояния settlement-overdue могут восстановиться после урегулирования и завершения любой необходимой очистки.                                                                                                  |
...
| `restore_timeout`            | Загрузка/восстановление сессии ACP превысило выделенный бюджет восстановления. |
...
| `401`  | `{ error: '...', code: 'token_required' }`    | Строгий маршрут на ненадёжном встраивании без токена. Первичные запросы с доверенного loopback проходят. SDK отображают подсказку о настройке токена.                            |
...
- `packages/cli/src/serve/auth.ts` (тела ошибок аутентификации)

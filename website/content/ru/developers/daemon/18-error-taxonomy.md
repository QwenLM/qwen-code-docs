...
| ----------------------------------- | ---- | --------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `SessionNotFoundError`              | 404  | `sessionId` отсутствует в `byId` (`code: "session_not_found"`) или сессия закрывается (`code: "session_closing"`).                                                                         | Для `session_not_found`: создайте или подключите сессию заново; возможно, она была удалена. Для `session_closing`: подождите и повторите попытку; выполняется параллельное закрытие. DELETE-маршруты трактуют `session_closing` как идемпотентный успех. |

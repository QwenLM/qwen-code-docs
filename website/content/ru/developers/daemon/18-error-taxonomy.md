---
title: Таксономия ошибок и устранение неполадок
description: Закрытые объединения типов ошибок демона, виды ошибок HTTP-границы, ошибки моста ACP и обёртка SDK — с причинами и рекомендациями по устранению.
---
...
| `io_error`                 | 503       | `ENOSPC` / `EIO` / `EBUSY` / `ETXTBSY` / `ENAMETOOLONG` / `EMFILE` / `ENFILE`.  | Проверьте место на диске, состояние хранилища, дескрипторы файлов. **Отличается от `permission_denied`**, чтобы пайплайны мониторинга не вызывали команду реагирования на инциденты безопасности из-за "диск заполнен". |
| `internal_error`           | 500       | Ошибка, не связанная с errno, достигшая границы (`TypeError`, программная ошибка). | Сообщите об ошибке; поле `cause` в теле ответа может содержать исходное исключение.                                     |
| `parse_error`              | 400 / 422 | Ошибка разбора тела запроса (400) или нарушение инварианта уровня сервиса (422). | Проверьте синтаксис запроса; для 422 проверьте контракт маршрута.                                                       |

## Ошибки моста ACP (`packages/acp-bridge/src/bridgeErrors.ts`)

Каждый типизированный класс экспортируется из `bridgeErrors.ts` и поддерживает `instanceof` при использовании в том же процессе Node. Через сеть маршрутизация выполняется по `body.code` / `body.kind` / `body.errorKind`.

| Класс                                       | HTTP | Причина                                                                                                                                                                                                                                                                                                                                                                                              | Устранение                                                                                                                                                                                                             |
| ------------------------------------------- | ---- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `SessionNotFoundError`                      | 404  | Неизвестный `sessionId`.                                                                                                                                                                                                                                                                                                                                                                             | Проверьте id сессии; повторите `POST /session` или подключите сессию заново; возможно, она была удалена. Для `session_closing`: подождите и повторите попытку; выполняется параллельное закрытие. DELETE-маршруты трактуют `session_closing` как идемпотентный успех.                  |
...
| `BridgeChannelQuarantinedError`             | 503  | Очистка покинутого восстановления была неубедительной (`restore_cleanup_failed`) или покинутое восстановление не завершило полный бюджет после дедлайна (`restore_settlement_overdue`); в обоих случаях канал рабочего пространства отказывает в новых сессиях до завершения дренирования. Тело 503 содержит `reason` и `retryAfterSeconds`. | Продолжайте использовать существующие сессии, дождитесь переработки канала, затем повторите работу с новыми сессиями.                                                                                                  |

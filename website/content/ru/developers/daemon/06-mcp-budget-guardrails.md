...
- **`ServeMcpBudgetStatusCell.scope`** управляется возможностями (capabilities) и совместим с будущими расширениями. Ячейки снимка раскрывают `budgets[]`, а не одно поле `budget?`. При `mcp_workspace_pool` выбранная среда выполнения генерирует `scope: 'workspace'`; при отключённом или недоступном пуле устаревший менеджер генерирует `scope: 'session'`. Потребители должны допускать дополнительные неизвестные значения `scope`, игнорируя их, а не завершаясь с ошибкой.

...
| `--hostname <host>`                     | string                         | `127.0.0.1`                                  | Не-loopback требует токен                | Адрес привязки. Значения loopback: любой адрес из `127.0.0.0/8`, `localhost`, `::1`, `[::1]`; `localhost` разрешается один раз и фиксируется, а доверенный режим проверяет фактический адрес слушателя до завершения запуска. Скобки в `[::1]` удаляются автоматически; ввод `host:port` отклоняется с подсказкой использовать `--port`.                               |
...
| `--enable-session-shell`                | boolean                        | `false`                                      | Bearer или доверенный loopback           | Включает прямое выполнение `POST /session/:id/shell`. Вызывающая сторона также должна отправить привязанный к сессии `X-Qwen-Client-Id`.                                                                              |
...
| `--allow-origin <pattern>`              | repeatable string              | none                                         | -                                        | Белый список CORS, заменяющий стандартный запрет Origin.Wildcard и не-loopback HTTP(S) origin требуют токен.                                                                                                                                     |
...
| `--channel-idle-timeout-ms <n>`         | number                         | `0`                                          | -                                        | Задержка автоматического завершения ACP-потомка после завершения рабочей нагрузки runtime. Обычный прогрев сохраняется для первого использования. Окна активного keepalive могут расширять настроенную задержку; побеждает большая оставшаяся задержка.                                                                                                    |
| `--initialize-timeout-ms <n>`           | number                         | `10000`                                      | -                                        | Дедлайн запуска дочернего процесса ACP (фабрика каналов + рукопожатие initialize) и таймаут запроса по умолчанию (мс).                                                                                                                                          |
...
| Не-loopback HTTP(S) `--allow-origin` без токена                                 | `Refusing to start with --allow-origin ... but no bearer token configured`                          |
...
Статический shell монтируется **до** `bearerAuth` во всех режимах запуска — браузер не может добавить заголовок `Authorization` к навигации в адресной строке или подресурсу `<script src>`, поэтому проверка только сломала бы UI. API-вызовы используют полномочия доверенного loopback на основном слушателе без токена или прикрепляют настроенный bearer в аутентифицированных режимах. При привязке не к loopback shell доступен только для чтения, если не передан `--allow-origin <origin>` — одноисточниковые POST-запросы несут заголовок `Origin`, который CORS-стена отклоняет (403) — поэтому передавайте `--allow-origin` для любой привязки за пределами loopback.

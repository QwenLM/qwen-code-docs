...
| `--memory-budget-mb <n>`                | integer в `[1024, 1048576]`    | 50% памяти cgroup/хоста                      | Только наблюдение                        | Общий лимит памяти для дерева процессов демона, ограниченный доступной памятью. Отображается в `limits.memory`; не определяет размер дочерних процессов.                                                                                         |

...
- [`04-permission-mediation.md`](./04-permission-mediation.md) - `MultiClientPermissionMediator`, четыре политики, инвариант таймаута N1, sentinel отмены.
...
- [`12-auth-security.md`](./12-auth-security.md) - bearer-промежуточное ПО, белый список хостов, запрет CORS, гейт мутаций, `--require-auth`, исключение для `/health`, device flow.
...
- [`20-quickstart-operations.md`](./20-quickstart-operations.md) - кратчайший путь запуска, проверки через curl, карта маршрутов и границы встраивания (внутрипроцессный хостинг является внутренним; внешние интеграции используют `qwen serve --no-web` плюс протокол HTTP/SSE). Внутренний рецепт жизненного цикла `createServeApp` находится в [`02-serve-runtime.md`](./02-serve-runtime.md).

## Глоссарий

- **ACP** - Agent Client Protocol. JSON-RPC поверх stdio, используемый для связи между бриджем демона и дочерним процессом ACP. Это не HTTP-протокол, который клиенты используют для работы с демоном.
- **Дочерний процесс ACP** - дочерний процесс `qwen --acp`, размещающий среду выполнения агента одного рабочего пространства. В продакшене выполняется попытка прогрева доверенного основного дочернего процесса для совместимости; доверенные вторичные запускаются при первой команде выполнения или сессии, а недоверенные вторичные не запускают ACP. Устаревшие маршруты основного сохраняют существующее поведение совместимости. Владеющий бридж мультиплексирует сессии и клиенты на этот дочерний процесс.
...
- **Рабочее пространство** - директория, зарегистрированная при запуске демона, восстановленная из хранилища регистраций или добавленная динамически. `workspaceCwd` — устаревшее основное значение по умолчанию; `workspaces[]` — каталог изолированных сред выполнения и их метаданные доверия/удаления.
...
| ----------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| Загрузка и сборка HTTP              | `packages/cli/src/serve/run-qwen-serve.ts`, `packages/cli/src/serve/server.ts`, `packages/cli/src/serve/routes/health.ts`, `packages/cli/src/serve/web-shell-static.ts`                                                                                                | [`02`](./02-serve-runtime.md), [`20`](./20-quickstart-operations.md)                                                   |
| Бридж ACP и мультиплексирование сессий | `packages/acp-bridge/src/bridge.ts`, `packages/acp-bridge/src/bridgeTypes.ts`, `@qwen-code/acp-bridge`                                                                                                                                                               | [`03`](./03-acp-bridge.md), [`08`](./08-session-lifecycle.md)                                                        |
...
| Общий слой транскриптов UI          | `DaemonUiEventType`, `DaemonSessionProvider`, `packages/web-shell/client/daemon/`                                                                                                                                                                                             | [`13`](./13-sdk-daemon-client.md), [`14`](./14-cli-tui-adapter.md), [`../daemon-ui/README.md`](../daemon-ui/README.md) |
...
| Аутентификация / сетевые ограничения | Loopback без токена наделяет основной слушатель полномочиями локального оператора; не-loopback требует bearer; `--require-auth` распространяет bearer на loopback и `/health`; белый список хостов и запрет CORS по умолчанию активны. | [`12`](./12-auth-security.md), [`17`](./17-configuration.md)            |
...
| Возможности      | `SERVE_PROTOCOL_VERSION = 'v1'`; зарегистрированные и условные теги определяются авторитетным реестром.                                                                                                                            | [`11`](./11-capabilities-versioning.md)                                                                         |
| Оболочка сессии  | `POST /session/:id/shell` доступен при использовании `--enable-session-shell`, bearer-аутентификации или доверенного loopback и привязанного к сессии `X-Qwen-Client-Id`; тег возможности является условным.                             | [`11`](./11-capabilities-versioning.md), [`17`](./17-configuration.md), [`20`](./20-quickstart-operations.md)   |
...
| Потребитель daemon в Web Shell | `packages/web-shell/client/daemon/` потребляет хранилище транскриптов SDK через React-провайдеры и адаптеры. | [`14`](./14-cli-tui-adapter.md), [`../daemon-client-adapters/web-shell.md`](../daemon-client-adapters/web-shell.md) |
...
| Быстрый старт / эксплуатация | Описаны кратчайший путь запуска, рецепты запуска, проверки через curl, поведение аутентификации Web Shell, разделение маршрутов, поведение при завершении работы и границы встраивания. Внутренний рецепт жизненного цикла `createServeApp` находится в [`02`](./02-serve-runtime.md). | [`20`](./20-quickstart-operations.md) |
| Ошибки | Явные сбои при запуске, ошибки маршрутов, ошибки бриджа, EventBus, файловой системы и медиатора обобщены с указанием способов устранения. | [`18`](./18-error-taxonomy.md) |
...
- Схема событий v1 аддитивна. Новые известные типы событий должны добавляться в `DAEMON_KNOWN_EVENT_TYPE_VALUES`; старые SDK должны трактовать неизвестные типы как совместимые в прямом направлении.
...
- `_meta` Envelope и `data._meta` полезной нагрузки ACP различаются. Происхождение вызовов инструментов находится в полезной нагрузке ACP; временные метки отправки сервера находятся в Envelope SSE.

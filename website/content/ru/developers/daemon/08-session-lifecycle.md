...
Каждый независимый контроллер должен использовать уникальный стабильный ID. WebUI по умолчанию генерирует ID с префиксом `webui_`. Хост и встроенный WebShell должны разделять один ID только в том случае, если они намеренно действуют как один логический контроллер; после объединения логи журналов демона не смогут различить, какой из них инициировал запрос.
...
Для сохранённой сессии worktree Part 4A восстановление является расширением этого жизненного цикла с контролем целостности. Sidecar явно идентифицирует запрошенный корень рабочего пространства, checkout должен канонически содержаться в соответствующем каталоге `.qwen/worktrees/`, а его маркер должен быть обычным файлом с одной ссылкой, содержащим точный ID восстановленной сессии. Демон перемещает неактивный восстановленный дочерний процесс только после этих проверок; активный дочерний процесс принимается только когда его сообщённый cwd уже равен worktree, а активный дочерний процесс, чей сообщённый cwd отсутствует или находится в другом месте, завершается с ошибкой (fail closed) вместо перемещения под его промптом — за исключением холодного восстановления, которое не могло отложить restore-промпт (`suppressWorktreeContextRestore` выключен, поэтому бридж отправил повторно заданный вопрос вместо его парковки): этот случай сохраняет результат до-4B, возвращая канонические метаданные worktree без `worktreeState`, неперемещёнными, с выжившей сессией. Перемещённые и принятые ответы возвращают канонические метаданные worktree с `worktreeState: "persisted-v1"`. Sidecar с `supersededBy` никогда не восстанавливается: маршрут возвращает `409 worktree_session_superseded` с ID замещающей сессии — классификация, определяемая исключительно по этой ссылке до любого чтения маркера, поэтому вызывающие стороны перенаправляются и исправляют свой учёт только после успешной загрузки этого ID; прерванная передача pre-commit называет замену, которая не является владельцем маркера, не может быть восстановлена сама и уничтожается повторным сбросом; восстановленная замена, чья ссылка `supersedes` совпадает со старым sidecar, пока маркер не перемещался — или отсутствует — возвращает `409 worktree_reset_interrupted`, чей ремонт заключается в повторной попытке сброса против замещённой сессии; а отсутствующий маркер без совпадающей пары ссылок возвращает `409 worktree_marker_missing`, чей ремонт заключается в сбросе задачи, а не в повторной попытке восстановления, поскольку ни один путь восстановления не создаёт маркер заново. Классификация прерывания проверяется первой. Некорректное состояние Part 4A отсоединяет существующее подключение или уничтожает холодное восстановление с `requireZeroAttaches`; отсутствующий sidecar аналогично не даёт аттестации. Всякий раз, когда эффективный источник восстановления принадлежит Channel, маршрут подавляет best-effort очистку ACP-агента для состояния Part 4A или неклассифицируемого sidecar, поэтому ошибка валидации сохраняет неопределённые доказательства checkout. Сохранённые метаданные источника имеют приоритет; когда они отсутствуют, запрос load/resume предоставляет эффективный источник. Структу��но валидный устаревший sidecar без `workspaceCwd` вместо этого сохраняет существующее best-effort восстановление агентом: он должен идентифицировать либо запрошенный корень рабочего пространства, либо его Git-репозиторий верхнего уровня, проверяется на containment без аттестации маркером, может быть очищен агентом и может вернуть `worktree` без `worktreeState`. Кроме этого явного случая совместимости с устаревшим форматом, только сессии, эффективный источник восстановления которых не принадлежит Channel, сохраняют существующую best-effort очистку перед валидацией маршрута.

Передача владения worktree (`POST /session/:id/worktree-reset`, анонсируемая `session_worktree_reset_v1`) расширяет этот жизненный цикл для сбросов задач Channel: демон создаёт новый replacement с областью действия потока в корневом рабочем пространстве, перемещает его в проверенный checkout, связывает пару sidecar (`supersededBy` на старой сессии первым, затем `supersedes` на replacement), переключает маркер на replacement под блокировкой маршрута для каждого checkout и барьером допуска, который ограждает допуск промптов плюс семь других писателей, начинающих работу в checkout или перемещающих cwd сессии (rewind, смена cwd, ветка, форк, shell, управление целью, действие workflow-task), в то время как пути release и stop остаются неограждёнными по замыслу, а затем разрывает регистрации клиентов замещённой сессии и ассоциацию worktree в памяти. Отчёт о разрыве указывает, действительно ли замещённая сессия исчезла: выживший, чей дочерний процесс всё ещё удерживает фоновую работу, оставляет барьер активным, логируется и сообщается вызывающей стороне как `supersededSessionLive: true` вместо того, чтобы быть скрытым ответом об успехе. Ограждённый писатель, допущенный на замещённую сессию во время передачи, отклоняется с `409 worktree_reset_active`; полная таксономия сбоев, включая то, какие окна сброса откатывает повторная попытка и какие завершаются с ошибкой (fail closed) для ремонта оператором, документирована вместе с маршрутом в `qwen-serve-protocol.md`.
...
### Откат сессии (Session Rewind)

`GET /session/:id/rewind/snapshots` и `POST /session/:id/rewind` определяют runtime активного рабочего пространства владельца. Сохранённые сессии должны быть загружены или восстановлены перед откатом. Откат обрезает историю диалога и опционально восстанавливает файлы, отслеживаемые `edit` и `write_file`; он не отменяет shell-команды, Git-операции, скрипты или ручные изменения. Восстановление файлов работает по принципу best-effort, поэтому ответ может сообщить `rewound: false` и `filesFailed[]` после того, как история диалога уже была изменена. Вызовы SDK для отката всегда используют owner-aware REST, даже если клиент обычно использует ACP-транспорт, потому что мутация должна сохранять строгую REST-аутентификацию.
...
Пустые, повреждённые и осиротевшие обычные файлы транскриптов остаются доступными для этих операций жизненного цикла, даже если они не могут быть загружены как диалоги. Проверки безопасности владельца могут намеренно завершаться с ошибкой (fail closed) и требовать вмешательства оператора. Файл, изменённый после того, как писатель запечатал сертифицированное доказательство передачи, завершается с ошибкой `SessionTranscriptChangedError`, пока оператор не разрешит заблокированный замок и изменённые байты. Первая физическая запись в формате JSON, превышающая ограниченное окно чтения владельца, завершается с ошибкой `SessionTranscriptIdentityUnavailableError`, пока запись не будет исправлена или уменьшена; повреждённые записи чрезмерного размера с префиксом не-объекта остаются доступными. Восстановленная запись, пригодная для разбора, должна содержать строковые поля владения `sessionId` и `cwd`, а смешанные локальные/иностранные состояния архива также завершаются с ошибкой (fail closed). Когда анонсирован `session_storage_conflict_repair`, archive и unarchive принимают `resolveConflicts: true`: archive сохраняет архивную копию, тогда как unarchive сохраняет активную копию. Без этой опции конфликты активная/архивная копия не перемещают, не удаляют и не перезаписывают сохранённые копии и возвращаются в массиве `errors` батча. Archive по-прежнему строго закрывает активную сессию перед классификацией конфликта, что может сбросить queued записи в активный транскрипт. Маршруты жизненного цикла с квалификацией рабочего пространства теперь используют HTTP `200` batch-обёртку вместо предыдущего ответа HTTP `409 session_conflict`.
...
Для сохранённой сессии worktree Part 4A восстановление является расширением этого жизненного цикла с контролем целостности. Sidecar явно идентифицирует запрошенный корень рабочего пространства, checkout должен канонически содержаться в соответствующем каталоге `.qwen/worktrees/`, а его маркер должен быть обычным файлом с одной ссылкой, содержащим точный ID восстановленной сессии. Демон перемещает неактивный восстановленный дочерний процесс только после этих проверок; активный дочерний процесс принимается только когда его сообщённый cwd уже равен worktree, а активный дочерний процесс, чей сообщённый cwd отсутствует или находится в другом месте, завершается с ошибкой (fail closed) вместо перемещения под его промптом — за исключением холодного восстановления, которое не могло отложить restore-промпт (`suppressWorktreeContextRestore` выключен, поэтому бридж отправил повторно заданный вопрос вместо его парковки): этот случай сохраняет результат до-4B, возвращая канонические метаданные worktree без `worktreeState`, неперемещёнными, с выжившей сессией. Перемещённые и принятые ответы возвращают канонические метаданные worktree с `worktreeState: "persisted-v1"`. Sidecar с `supersededBy` никогда не восстанавливается: маршрут возвращает `409 worktree_session_superseded` с ID замещающей сессии — классификация, определяемая исключительно по этой ссылке до любого чтения маркера, поэтому вызывающие стороны перенаправляются и исправляют свой учёт только после успешной загрузки этого ID; прерванная передача pre-commit называет замену, которая не является владельцем маркера, не может быть восстановлена сама и уничтожается повторным сбросом; восстановленная замена, чья ссылка `supersedes` совпадает со старым sidecar, пока маркер не перемещался — или отсутствует — возвращает `409 worktree_reset_interrupted`, чей ремонт заключается в повторной попытке сброса против замещённой сессии; а отсутствующий маркер без совпадающей пары ссылок возвращает `409 worktree_marker_missing`, чей ремонт заключается в сбросе задачи, а не в повторной попытке восстановления, поскольку ни один путь восстановления не создаёт маркер заново. Классификация прерывания проверяется первой. Некорректное состояние Part 4A отсоединяет существующее подключение или уничтожает холодное восстановление с `requireZeroAttaches`; отсутствующий sidecar аналогично не даёт аттестации. Всякий раз, когда эффективный источник восстановления принадлежит Channel, маршрут подавляет best-effort очистку ACP-агента для состояния Part 4A или неклассифицируемого sidecar, поэтому ошибка валидации сохраняет неопределённые доказательства checkout. Сохранённые метаданные источника имеют приоритет; когда они отсутствуют, запрос load/resume предоставляет эффективный источник. Структурно валидный устаревший sidecar без `workspaceCwd` вместо этого сохраняет существующее best-effort восстановление агентом: он должен идентифицировать либо запрошенный корень рабочего пространства, либо его Git-репозиторий верхнего уровня, проверяется на containment без аттестации маркером, может быть очищен агентом и может вернуть `worktree` без `worktreeState`. Кроме этого явного случая совместимости с устаревшим форматом, только сессии, эффективный источник восстановления которых не принадлежит Channel, сохраняют существующую best-effort очистку перед валидацией маршрута.

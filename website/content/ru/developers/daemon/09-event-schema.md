...
API переключения навыков прикрепляют опциональные `mutation: { id, kind: 'skill_toggle', skills: [{ name, enabled }], activation, sessionsRefreshed, sessionsFailed }`. Все события `skills.disabled` / `skills.enabled` из одного запроса разделяют один mutation id. Остальные записи настроек опускают `mutation`. Записи сервиса рабочего пространства включают `scope`; некоторые другие эмиттеры (например, переключение модели сессии) опускают его. Нормализатор SDK подставляет отсутствующий `scope` как `'workspace'`.
...
| Тип                | Направление | Триггер                                                              | Ключевые поля полезной нагрузки                                                                                          |
| ------------------ | ----------- | -------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------ |
| `extensions_changed` | S->C      | Завершена фоновая установка/обновление расширения или изменен статус | `refreshed, failed, status?: 'installed' \| 'enabled' \| 'disabled' \| 'updated' \| 'uninstalled' \| 'failed', source?, name?, version?, error?`. Демон, анонсирующий `extension_activation_explicit_refresh`, применяет активацию без трансляции, поэтому успешная активация больше не генерирует `enabled`/`disabled`; эти события исходят от старых демонов, а новые сводят активацию к трансляции обновления без статуса. |
...
| `turn_error`        | S->C        | Сбой хода                                                                                                           | `sessionId, message, code?, promptId?`; тот же механизм корреляции `promptId`. Для `-32603` ошибок на стороне агента `message` содержит собственные детали провайдера, если агент их предоставил (`data.details` / `data.message` / вложенный `data.error.message`), а не общий текст JSON-RPC. |
...
- `settings_changed` / `settings_reloaded` — распознаются через `asKnownDaemonEvent`; редьюсер сессии не поддерживает выделенные поля состояния представления. События `settings_changed` с переключением навыков несут опциональные метаданные `mutation`, чтобы хосты могли применять изменения только навыков инкрементально, без перезагрузки задачи. Остальные UI могут по-прежнему обрабатывать это событие как сигнал обновления.

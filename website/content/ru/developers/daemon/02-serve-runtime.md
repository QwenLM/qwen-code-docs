...
| `createMutationGate` (per-route)  | Опциональный на уровне маршрута гейт для мутационных маршрутов, требующих токен даже на loopback.                                                                                                            | Возвращает `401 { code: 'token_required' }`. Не глобальный `app.use`; маршруты вызывают `mutate({ strict: true })` по мере необходимости.                                                             |

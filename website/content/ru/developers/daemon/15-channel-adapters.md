...
| `multiSession`                           | Именованные задачи только для демона при `sessionScope: 'user'`. Каталог владельца сохраняется в каталоге состояния рабочего пространства/канала; webhook, заполнение истории групп, циклы, переключение запущенных задач и worktree для каждой задачи исключены в Части 2. |

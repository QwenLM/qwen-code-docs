...
В режиме управления демоном каждый канал сопоставляет входящий чат-трафик с сессиями демона в рамках настраиваемой области `SessionScope` (`user`, `chat_thread` или `single`). Устаревшее значение `thread` остаётся читаемым и редактируемым для существующих конфигураций, но новые конфигурации Web Shell его не предлагают; это отдельно от собственного переключателя создания сессий `single`/`thread` бриджа демона. При `sessionScope: "user"` и `multiSession: true` `ChannelBase` добавляет персистентный каталог именованных сессий с ключом по каналу, чату и отправителю, тогда как `SessionRouter` сохраняет выбранную сессию как свой маршрут совместимости. Точные загрузки именованных сессий никогда не используют устаревший путь load-or-replace. Именованный ход резервирует свою точную сессию перед асинхронной подготовкой и остаётся привязанным после последующих изменений выбора без повторной привязки маршрута совместимости. Именованные задачи могут выполняться параллельно, `/session cancel [<name>]` нацелен только на подтверждённый активный промпт, а текстовые команды разрешений без параметров учитывают только выбранную задачу. `/session new <name> --worktree` дополнительно запрашивает изоляцию Git worktree под управлением демона. Демон возвращает канонический путь worktree плюс `worktreeState: "persisted-v1"`; бридж проверяет это подтверждение перед записью любого маршрута, а переподключение повторяет ту же проверку. Именованные ходы также захватывают метку источника задачи только для доставки: прямые сообщения используют `[task]`, группы используют `[sender · task]`, а точные текстовые промпты разрешений включают ID запроса. Метка не добавляется в ответ модели или транскрипт. Адаптер делегирует задачи `DaemonChannelBridge`, который, в свою очередь, делегирует их `DaemonSessionClient` из SDK (см. [`13-sdk-daemon-client.md`](./13-sdk-daemon-client.md)). Каждый именованный канал должен разрешаться в одно зарегистрированное доверенное рабочее пространство. Воркер использует канонический cwd этой среды выполнения, `QWEN_DAEMON_WORKSPACE` и оверлей окружения; разрешение владельца никогда не откатывается к основному.
...
Долгоживущий `ChannelWorkerManager` владеет подтверждённым выбором демона и сгруппированными по рабочим пространствам супервизорами. Явный выбор `--channel` имеет приоритет и остаётся fail-fast. При запуске без флагов восстанавливается настройка `serve.channels` доверенного основного рабочего пространства; это динамически загружает среду выполнения каналов, резервирует pidfile сервиса, определяет владельца рабочего пространства и запускает выбранные воркеры. Вторичные рабочие пространства не восстанавливают самостоятельно свои собственные `serve.channels`. Без какого-либо источника запуска первая строго фильтруемая `PUT /workspace/channel` выполняет ленивую инициализацию. `GET /workspace/channel` читает снимок менеджера, а `DELETE /workspace/channel` останавливает его идемпотентно. Хелперы SDK — `getChannelWorkerControl()`, `setChannelWorkerSelection()` и `stopChannelWorker()`; CLI-команда — `qwen channel set` плюс удалённые варианты `status` и `stop`.

Сохранённые имена запуска должны быть непустыми, не иметь начальных или конечных пробелов и не содержать небезопасных управляющих или невидимых символов. Некорректные записи пропускаются по одной и логируются по индексу в массиве; демон не обрезает их в имя другого экземпляра и не переписывает настройки. Аргументы воркера используют `--channel=<value>`, что сохраняет начальный дефис внутри имени. Управление каналами продолжает сообщать сохранённые настройки запуска и фактическое состояние среды выполнения.

Некорректное поле запуска или ошибка валидации либо аренды перед запуском воркеров пропускает автоматическое восстановление и логирует свой источник `serve.channels` без отбрасывания несвязанных настроек. После неудачного запуска воркера демон продолжается только если очистка прошла успешно. Глобальные таймауты запуска среды выполнения и неподтверждённые остановки воркеров следуют существующему пути ошибки запуска; аренда сервиса остаётся удерживаемой, пока завершение воркера не подтверждено. Просмотрите лог демона для пропущенных или неудачных попыток восстановления.
...
| `multiSession`                           | Именованные задачи только для демона при `sessionScope: 'user'`. Каталог владельца сохраняется в каталоге состояния рабочего пространства/канала; задачи могут выполняться параллельно, могут использовать per-task Git worktree, команды отмены и разрешений остаются привязанными к конкретной задаче, а результаты и интерактивные поверхности идентифицируют свою исходную задачу. Webhook, заполнение истории групп и циклы остаются исключёнными. |
...
- `packages/channels/plugin-example/` (референсный каркас плагина)

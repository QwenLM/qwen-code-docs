...
| `multiSession`                           | Именованные задачи только для демона при `sessionScope: 'user'`. Каталог владельца сохраняется в каталоге состояния рабочего пространства/канала; задачи могут выполняться параллельно, команды отмены и разрешений остаются привязанными к конкретной задаче, а результаты и интерактивные поверхности идентифицируют свою исходную задачу. Webhook, заполнение истории групп, циклы и worktree для каждой задачи остаются исключёнными. |

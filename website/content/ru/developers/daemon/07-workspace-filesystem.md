...
HTTP-маршруты файлов (`GET /file`, `GET /file/bytes`, `POST /file/write`, `POST /file/edit`, `GET /list`, `GET /glob`, `GET /stat`) используют эту границу. В продакшен-демоне делегированные вызовы ACP достигают WFS через внедрённый адаптер моста; общие вызывающие моста используют WFS только когда они внедряют такой адаптер. Продакшен-среды выполнения `qwen serve` на одном хосте объявляют `readTextFile: false`, поэтому все потребители `FileSystemService.readTextFile` в дочернем процессе используют обычный сервис файловой системы CLI; итоговые записи текста ACP `writeTextFile` остаются делегированными через WFS.
...
- Тот же документ объясняет, почему #8618 всё ещё воспроизводится для семейства записи и редактирования даже после этого изменения, и ограниченное чувство, в котором сохранённый путь чтения адаптера "fail closed".

---
title: Граница файловой системы рабочей области
description: "`WorkspaceFileSystem` — разрешение путей, политики размера/доверия, атомарные записи, аудит и типизированные ошибки для HTTP-маршрутов файлов демона и делегированных вызовов ACP."
---
...
sequenceDiagram
    ...
    else файл > 256 КиБ И явный аргумент окна
        FS->>FSP: open stable FileHandle
        FS->>FS: потоковое чтение запрошенных строк из того же inode
        FS->>FS: лимит вывода 256 КиБ и сканирования 8 МиБ; без полного хэша файла

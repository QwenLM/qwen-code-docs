...
- **Некоторые теги являются условными.** Теги из `CONDITIONAL_SERVE_FEATURES` анонсируются только при включении соответствующего переключателя развертывания. Наличие тега означает, что поведение существует.
...
`workspaceCwd` — это канонический путь основного рабочего пространства (см. [`02-serve-runtime.md`](./02-serve-runtime.md)). Текущие демоны используют `workspaces[]` как каталог зарегистрированных runtime; `multi_workspace_sessions` указывает, что активно более одного runtime. `policy.permission` — это активная политика медиатора.
...
Read-only снимки рабочего пространства: `workspace_mcp`, `workspace_skills`, `workspace_providers`, `workspace_acp_status`, `workspace_env`, `workspace_preflight`, `workspace_hooks`, `workspace_extensions`.
...
Локальная установка расширений: `extension_local_path_install` разрешает абсолютный путь на хосте демона в существующем поле `source` обоих маршрутов установки расширений. Он отделён от `extension_management_v2`, поскольку маршрут совместимости основного рабочего пространства также поддерживает его, и клиенты не должны отправлять локальные пути старым демонам.
...
Явное обновление активации расширений: `extension_activation_explicit_refresh` означает, что одиночные и пакетные операции активации завершаются на стойком коммите политики без прямого обновления активных сессий. Клиенты, которым требуется немедленное применение, должны дождаться успешной активации и отправить операцию обновления runtime рабочего пространства для каждого рабочего пространства, сессии которого должны немедленно применить изменение; глобальный пакет по умолчанию изменяет активацию по умолчанию, которую наследует каждое рабочее пространство, если только это рабочее пространство не содержит точного переопределения для имени (или не соответствует устаревшему правилу пути), и не имеет единого обновления, охватывающего каждый runtime, поэтому рабочие пространства, которые вызывающий не обновляет, сходятся только при следующем 30-секундном проходе согласователя поколений. Демоны без этого тега уже включают обновление в операцию активации, поэтому вызывающие не должны отправлять там обновление для совместимости.
...
Маршрутизация сессий с несколькими рабочими пространствами: **`multi_workspace_sessions`** (условный), **`multi_workspace_session_rewind`** (условный) и **`multi_workspace_session_shell`** (условный). Клиент может использовать rewind для основной сессии с `session_rewind`; для вторичной живой сессии дополнительно требуется `multi_workspace_session_rewind`. Shell использует эквивалентную пару `session_shell_command` + `multi_workspace_session_shell` для вторичной сессии. ACP-нативные клиенты продолжают использовать `_qwen.methods`, возвращаемые при initialize; никакой ACP rewind vendor method не анонсируется.
...
- Входные данные переключателей поступают из `runQwenServe` / `createServeApp`, включая аутентификацию, MCP, origin, промпт, настройки, shell, rate-limit, reload и состояние количества активных runtime рабочих пространств.

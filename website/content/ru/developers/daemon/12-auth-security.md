...
- **Токен не настроен** → middleware является no-op (режим разработчика на loopback). Исключение: **LAN-слушатель** Local Control имеет область действия на уровне слушателя и всегда требует учётные данные сопряжения (`CredentialStore.isOpen` никогда не возвращает `true` для `local-control`), поэтому он никогда не бывает открытым даже на демоне без токена.

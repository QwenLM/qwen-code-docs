...
Ниже приведён пример скрипта прокси, который можно использовать с переменной окружения `QWEN_SANDBOX_PROXY_COMMAND`. Этот скрипт разрешает только `HTTPS`-подключения к `example.com:443` и отклоняет все остальные запросы.

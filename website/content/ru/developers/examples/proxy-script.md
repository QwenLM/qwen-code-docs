...
Ниже приведен пример прокси-скрипта, который можно использовать с переменной окружения `QWEN_SANDBOX_PROXY_COMMAND`. Этот скрипт разрешает только `HTTPS`-соединения к `example.com:443` и отклоняет все остальные запросы.

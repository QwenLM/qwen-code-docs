# Краткое руководство по API-only DaemonClient (TypeScript)

Минимальный сквозной пример: запустите демон `qwen serve` в режиме API-only в другом терминале, затем управляйте им из Node-скрипта с помощью `DaemonClient` из SDK. См. также: [Руководство по режиму демона](../../users/qwen-serve.md) и [Справочник HTTP-протокола](../qwen-serve-protocol.md).
...
`--no-web` удаляет ассеты Web Shell и связанные с ними поверхности: `POST /workspace/local-control/enable` затем завершается с fail closed и `409 local_control_web_shell_unavailable` на каждой платформе, а на macOS маршруты `/live/*`, WebSocket `/live/host` и ключи настроек `experimental.liveVoice.*` не регистрируются — поэтому интеграция, управляющая Live-методами SDK, должна работать без `--no-web`. Это не переключатель профиля функций: маршруты сессии, запросов, рабочего пространства, разрешений и SSE не изменяются. Каждое значение `--workspace` должно быть абсолютным путём к каталогу. Первая рабочая область является основной и остаётся совместимым значением по умолчанию для запросов без `cwd`; `/capabilities.workspaces[]` — это каталог, который клиенты должны использовать при явном выборе среды выполнения.

Режим loopback без токена предназначен для рабочей станции с одним пользователем. На общем хосте установите `QWEN_SERVER_TOKEN` и добавьте `--require-auth`; привязки не к loopback требуют токен.
...
`expectedHash` — это SHA-256 от сырых байтов файла на диске. Режим `"replace"` и `editWorkspaceFile()` требуют его, чтобы устаревшие клиенты не перезаписали файл, который они только что не читали. Запись/редактирование принимают основной слушатель без токена в доверенном локальном контуре; в недоверенных развертываниях требуются bearer- или pairing-учетные данные.

...
| `coreTools`               | `string[]`                                     | -               | Использует устаревшую семантику белого списка `coreTools` / CLI `--core-tools`. Если указан, для сессии регистрируются только совпадающие основные инструменты. Это отдельно от `permissions.allow` в settings.json, который также активирует белый список на уровне реестра при запуске: когда настроено хотя бы одно действительное правило разрешения (некорректные записи не учитываются), встроенные инструменты, не покрытые ни одним правилом разрешения или запроса, не регистрируются (инструменты MCP, контракт `--json-schema` `structured_output`, инструменты жизненного цикла режима планирования, `task_stop`, `tool_search` и семейство `computer_use__*` освобождены; требуется перезапуск, #9827). Параметр SDK `allowedTools` не может активировать белый список самостоятельно, но пока белый список активен, его правила объединяются в эффективный набор разрешений и учитываются в покрытии, сохраняя зарегистрированными покрытые встроенные инструменты. Пример: `['read_file', 'edit', 'run_shell_command']`. |

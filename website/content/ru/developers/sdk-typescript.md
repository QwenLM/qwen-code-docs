...
| `coreTools`               | `string[]`                                     | -               | Использует устаревшую семантику белого списка `coreTools` / CLI `--core-tools`. Если указан, для сессии регистрируются только совпадающие основные инструменты. Это отдельно от `permissions.allow` в settings.json, который также активирует белый список на уровне реестра при запуске: когда настроено хотя бы одно действительное правило разрешения (некорректные записи не учитываются), встроенные инструменты, не покрытые ни одним правилом разрешения или запроса, понижаются до отложенных — регистрируются и загружаются через `tool_search`, но их схемы не попадают в запрос модели (инструменты MCP, контракт `--json-schema` `structured_output`, инструменты жизненного цикла режима планирования, `task_stop`, `tool_search` и семейство `computer_use__*` освобождены; требуется перезапуск, #9827, #10075). Параметр SDK `allowedTools` не может активировать белый список самостоятельно, но пока белый список активен, его правила объединяются в эффективный набор разрешений и учитываются в покрытии, оставляя покрытые встроенные инструменты зарегистрированными. Пример: `['read_file', 'edit', 'run_shell_command']`. |
...
| `allowedTools`            | `string[]`                                     | -               | Эквивалент `permissions.allow` в settings.json для автоутверждения. Совпадающие инструменты обходят callback `canUseTool` и выполняются автоматически. Применяется только когда инструмент требует подтверждения. В отличие от `permissions.allow` в settings.json, этот параметр сам по себе не активирует белый список реестра; однако, пока белый список из настроек активен, правила `allowedTools` объединяются в эффективный набор разрешений и учитываются в покрытии, поэтому покрытые встроенные инструменты остаются зарегистрированными (непокрытые понижаются до отложенных, а не удаляются, #10075). Поддерживает те же шаблоны, что и `excludeTools`. Пример: `['Bash(git status)', 'Bash(npm test)']`. |

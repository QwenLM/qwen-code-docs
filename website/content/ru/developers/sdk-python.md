...
| `permission_mode`          | `default`, `plan`, `auto-edit`, `auto`, `yolo`               | Режим подтверждения выполнения инструментов. `auto` позволяет LLM-классификатору одобрять вызовы инструментов; `yolo` автоматически одобряет все инструменты; используйте только в доверенной или изолированной среде. |

...
| `logPrompts`                      | `QWEN_TELEMETRY_LOG_PROMPTS`                         | `--telemetry-log-prompts` / `--no-telemetry-log-prompts` | Включать содержимое промптов пользователя и текст запросов/ответов API в логи телеметрии                                                                                                              | `true`/`false`    | `true`                  |
...
`gen_ai.tool.description` — нечувствительные статические метаданные реестра, выдаваемые независимо от `includeSensitiveSpanAttributes`. Это включает описания, предоставленные MCP-серверами и другими поставщиками инструментов рабочего пространства. Значение ограничено 4096 единицами UTF-16 и никогда не включает детали динамического вызова.
...
2. **Спаны моста log-to-span** (используются, когда HTTP-трейсы экспортируются без эндпоинта логов) сохраняют `function_args`, `error`, `error.message` и `error_message`, а также `prompt`, `request_text` и `response_text` при включённом `logPrompts`, вместо того чтобы отбрасывать эти атрибуты.
...
### Аффинити сессий Routify

LLM-запросы Qwen Code через пути провайдеров OpenAI-совместимого, DashScope, Anthropic,
Gemini и Vertex включают текущий ID сессии Qwen Code в заголовке `session_id` при
обращении по HTTPS напрямую к `routify.alibaba-inc.com`,
`routify-online.alibaba-inc.com` или `routify-pub.alibaba-inc.com`. ModelRouter Routify
использует это значение для аффинити сессий и маркировки трафика. Это
поведение не контролируется `telemetry.enabled` или
`outboundCorrelation.*`.

Начальное совпадение назначения намеренно узкое: Qwen Code не
прикрепляет заголовок к поддоменам, другим хостам `alibaba-inc.com` или другим конечным точкам LLM. Стандартное поведение перенаправления fetch по-прежнему применяется после этого
совпадения, поэтому ответ Routify может переслать заголовок путем перенаправления запроса.

ID сессии читается для каждого запроса, поэтому новая сессия, созданная
`/clear`, получает новое значение аффинити без пересборки клиента SDK. Gemini
требует явный Routify `baseUrl`, чтобы Qwen Code мог проверить
назначение.

`X-Qwen-Code-Request-Id` не реализован.

## Входящая корреляция (HTTP API демона)

HTTP API демона принимает стандартный заголовок W3C `traceparent` в каждом
запросе. Два потребителя читают его независимо:

- **Переназначение родительского спана запроса (телеметрия включена).** Когда SDK
  телеметрии инициализирован, валидный заголовок извлекается как удалённый
  родительский спан запроса, поэтому спаны демона прикрепляются под трейсом
  вызывающей стороны вместо создания нового. Путь пересылки `_meta` читает
  ту же цепочку родителей, поэтому спаны подпроцессов сессии, пересылаемые
  через запрос демона, также наследуют его.
- **Поле `traceId` в access-логе (оба режима).** Специализированный pre-auth
  middleware захватывает и разбирает заголовок в каждом запросе — включая
  те, которые обрываются на аутентификации (401), rate limiter (429),
  парсере тела JSON (400) или не совпадают ни с одним маршрутом (404) —
  и access-лог выдаёт trace id вызывающей стороны как поле `traceId`
  в camelCase. При отключённой телеметрии это поле является единственным
  способом связать строку лога демона с логами вызывающей стороны (или
  бэкендом трейсов), поэтому один сохранённый запрос работает для обоих
  режимов без настройки телеметрии.

Невалидный, но присутствующий заголовок отклоняется (спан остаётся без
родителя) и оставляет ограниченную по частоте DEBUG-метку
(`qwen-code.daemon.traceparent.invalid`), записывающую отклонённое значение,
чтобы сломанная кросс-сервисная связка была диагностируема только по логам
демона.

### Принудительный сэмплинг под входящими родителями

При стандартном сэмплере `parentbased_always_on` (и других parentbased
значениях по умолчанию), флаг `sampled=0` удалённого родителя — это
head-based решение на стороне вызывающего, а не запрос на отключение
телеметрии демона, поэтому извлечение принудительно устанавливает флаг
SAMPLED на входящих родителях. Единственный способ отключить это —
`OTEL_TRACES_SAMPLER=parentbased_always_off`, который уважает флаги
вызывающей стороны — обратите внимание, что он также отключает сэмплинг
корневых спанов для всего демона, а не только для запросов, связанных
с входящими родителями.

**Предупреждение:** постоянный `traceparent` (например, захардкоженный в
клиенте для нагрузочного тестирования) переназначает каждый запрос демона
в один единственный трейс; генерируйте свежий заголовок для каждого запроса.
...
- `qwen-code.api_request`: Исходящий запрос к API LLM.
  - **Атрибуты**: `model` (string), `prompt_id` (string), `request_text` (string, опционально — содержит содержимое запроса только когда `log_prompts_enabled` равен true; спаны моста log-to-span дополнительно требуют `includeSensitiveSpanAttributes`; включается непрозрачная полезная нагрузка провайдера `thoughtSignature`, решение по политике которой отслеживается в #11682), `subagent_name` (string, опционально)

- `qwen-code.api_response`: Ответ, полученный от API LLM.
  - **Атрибуты**: `response_id` (string), `model` (string), `status_code` (int/string, опционально), `duration_ms` (int), `input_token_count` (int), `output_token_count` (int), `cached_content_token_count` (int), `thoughts_token_count` (int), `total_token_count` (int), `prompt_id` (string), `auth_type` (string, опционально), `response_text` (string, опционально — содержит видимое содержимое ответа только когда `log_prompts_enabled` равен true; спаны моста log-to-span дополнительно требуют `includeSensitiveSpanAttributes`; не содержит содержимого для внутренних id промптов или ответов без видимого текста), `subagent_name` (string, опционально)
...
Следующие метрики определены, но **ещё не включены в продакшене**. Они будут активированы за специальным флагом конфигурации мониторинга производительности.

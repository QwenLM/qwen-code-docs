...
1. Включите телеметрию в `.qwen/settings.json` и задайте OTLP-эндпоинт:
   ...
   **Вариант B: протокол HTTP с эндпоинтами для каждого сигнала** (для бэкендов,
   ...
   `QWEN_TELEMETRY_USER_ID` как переменную окружения.
...
- `qwen-code.tool`: Оборачивает полный жизненный цикл инструмента (ожидание подтверждения + выполнение).
  - **Атрибуты**: `session.id`, опциональное расширение ARMS `gen_ai.user.id`, `gen_ai.operation.name` (`execute_tool`), `gen_ai.tool.name`, `gen_ai.tool.type` (`function`), `gen_ai.tool.call.id`, `tool.call_id`, `duration_ms`, `success`, `error`, `tool.failure_kind` (string, опционально — конкретная причина сбоя, например, "cancelled", "tool_error", "tool_exception", "timeout", "permission_denied", "pre_hook_blocked")

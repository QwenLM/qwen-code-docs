...
Еженедельные preview-релизы каждый вторник в 17:00 UTC для раннего доступа к будущим функциям.

### Nightly-релизы

Ежедневные nightly-релизы в 21:00 UTC для тестирования самых новых изменений.

## Расписание автоматических релизов

- **Nightly**: каждый день в 21:00 UTC
- **Preview**: каждый вторник в 17:00 UTC
...
- **Упрощённое управление зависимостями**: Выполнение `corepack pnpm install --frozen-lockfile` из корня проекта устанавливает зависимости всех пакетов в workspace и связывает их вместе, так что вам не нужно выполнять установку в каталоге каждого пакета.
- **Автоматическая привязка**: Пакеты внутри workspace могут зависеть друг от друга. При установке из корня pnpm автоматически создает симлинки между пакетами. Это означает, что когда вы вносите изменения в один пакет, эти изменения немедленно становятся доступны другим пакетам, которые от него зависят.

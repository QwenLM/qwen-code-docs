...
- **Ошибка: `Qwen OAuth free tier was discontinued on 2026-04-15`**
  ...
  - **Решение:** Переключитесь на другой метод аутентификации. Выполните `qwen` → `/auth` и выберите один из вариантов:
    - **API Key**: Используйте API key из Alibaba Cloud Model Studio ([Beijing](https://bailian.console.aliyun.com/) / [intl](https://modelstudio.console.alibabacloud.com/)). См. руководство по настройке API ([Beijing](https://bailian.console.aliyun.com/cn-beijing/?tab=doc#/doc/?type=model&url=3023091) / [intl](https://modelstudio.console.alibabacloud.com/ap-southeast-1?tab=doc#/doc/?type=model&url=2974721)).
    - **Alibaba Cloud Coding Plan**: Оформите подписку за фиксированную ежемесячную плату с увеличенными квотами. См. руководство по Coding Plan ([Beijing](https://bailian.console.aliyun.com/cn-beijing/?tab=coding-plan#/efm/coding-plan-index) / [intl](https://modelstudio.console.alibabacloud.com/?tab=coding-plan#/efm/coding-plan-index)).
...
- **В: Кастомизация (расширение, хук, skill, MCP-сервер или подагент), похоже, ломает Qwen Code. Как ее изолировать?**
  - О: Запустите Qwen Code с флагом `--safe-mode`, чтобы отключить все кастомизации — контекстные файлы, хуки, расширения, skills, MCP-серверы, пользовательские подагенты (загружаются только встроенные), правила разрешений, переопределения режима одобрения из настроек, функции памяти и настройки песочницы — для текущей сессии. Примечание: флаги CLI `--yolo` и `--approval-mode` продолжают действовать в безопасном режиме. Если проблема исчезает в безопасном режиме, включайте кастомизации по одной, чтобы найти виновника.
    ...
    - Примечание: «MCP-серверы» здесь означают серверы, настроенные в `settings.json` / проектном `.mcp.json` — локальное, фоновое состояние, для изоляции которого предназначен безопасный режим. MCP-серверы, которые вы явно передаёте для текущего вызова (например, `session/new` `mcpServers` у ACP-клиента для эмбеддингов или `--mcp-config`), не являются локальным/фоновым состоянием и продолжают работать в безопасном режиме.
...
- **Ошибка: "Operation not permitted", "Permission denied" или аналогичные.**
  ...
  - **Решение:** Подробнее см. в документации [Конфигурация: Песочница](../features/sandbox), включая информацию о том, как настроить конфигурацию песочницы.
...
- **Проблема: Прокрутка тачпадом в tmux изменяет историю промптов вместо прокрутки диалога**
  ...
  - **Решение:** Если режим чтения с экрана отключён, убедитесь, что `ui.useTerminalBuffer` включён; затем используйте `Shift+Up` / `Shift+Down` или колесико мыши, когда tmux передаёт события прокрутки приложению (требуется `ui.mouseTracking`). Если вы предпочитаете прокрутку хоста, настройте привязки мыши в tmux для событий прокрутки.

- **Правый клик не работает, ссылки не открываются, текст нельзя выделить в терминале**
  - **Проблема:** Нативные контекстные меню по правому клику, клики по OSC 8-гиперссылкам и нативное выделение текста в терминале перестают работать во время работы Qwen Code.
  - **Причина:** Когда `ui.mouseTracking` включён (по умолчанию), Qwen Code перехватывает все события мыши через SGR-отслеживание мыши для обеспечения выделения текста в приложении, позиционирования по клику, наведения на строки, переключения элементов истории и прокрутки вьюпорта. Терминал передаёт все события мыши приложению вместо их нативной обработки. Qwen Code предоставляет собственные замены, пока отслеживание включено: одинарный клик открывает http(s) OSC 8-гиперссылку, а правый клик по ссылке или выделенному тексту открывает контекстное меню в приложении.
  - **Решение:** Если вы предпочитаете нативную обработку терминалом, установите `"ui.mouseTracking": false` в вашем `settings.json`. Это отключает всё взаимодействие с мышью в приложении, включая открытие ссылок и контекстное меню в приложении. В Virtualized History (`ui.useTerminalBuffer: true`, по умолчанию) колесико больше не будет прокручивать транскрипт — используйте `Shift+↑/↓`, `PgUp/PgDn` или `Ctrl+Home/End`. Чтобы также восстановить нативную прокрутку терминала, установите `"ui.useTerminalBuffer": false`. Требуется перезапуск.
...
Если вы столкнулись с проблемой, которая не описана в этом _руководстве по устранению неполадок_, попробуйте поискать ее в [трекере Issues Qwen Code на GitHub](https://github.com/QwenLM/qwen-code/issues). Если не найдете похожей проблемы, создайте новый Issue на GitHub с подробным описанием. Pull requests также приветствуются!
...
- **Правый клик не работает, ссылки не открываются, текст нельзя выделить в терминале**
  - **Проблема:** Нативные контекстные меню по правому клику, клики по OSC 8-гиперссылкам (Ctrl+Click или обычный клик по URL) и нативное выделение текста в терминале перестают работать во время работы Qwen Code.
  - **Причина:** Когда `ui.mouseTracking` включён (по умолчанию), Qwen Code перехватывает все события мыши через SGR-отслеживание мыши для обеспечения выделения текста в приложении, позиционирования по клику, наведения на строки и прокрутки вьюпорта. Терминал передаёт все события мыши приложению вместо их нативной обработки.
  - **Решение:** Установите `"ui.mouseTracking": false` в `settings.json`, чтобы восстановить нативные контекстные меню по правому клику и кликабельные URL-ссылки. Это отключает всё взаимодействие с мышью в приложении. В Virtualized History (`ui.useTerminalBuffer: true`, по умолчанию) колесико больше не будет прокручивать транскрипт — используйте `Shift+↑/↓`, `PgUp/PgDn` или `Ctrl+Home/End`. Чтобы также восстановить нативную прокрутку терминала, установите `"ui.useTerminalBuffer": false`. Требуется перезапуск.

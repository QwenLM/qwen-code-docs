...
qwenVersion, ipcPath (когда доступен обмен сообщениями peer)
```
...
Вызов `send_message` только подтверждает, что сообщение передано в другую сессию. Что с ним произошло, приходит позже как квитанция: если оно было удержано, отклонено (declined), отказано (refused), просрочено или неправильно адресовано (адрес сменил владельца — перечислите агентов снова) — или освобождено после удержания — уведомление появляется в транскрипте отправляющей сессии (`Message to <name>: …`). Declined и refused — разные ответы: declined означает, что кто-то проверил сообщение и отклонил, тогда как refused означает, что `agents.crossSessionInbound` этой сессии установлен в `refuse` и никто его вообще не видел. Модель, отправившая его, не уведомляется; если другая сессия отвечает, ответ приходит как межсессионное сообщение.

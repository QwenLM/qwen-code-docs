...
| `/summary`    | Создает сводку проекта на основе истории разговора                        | `/summary`                           |
...
| `/approval-mode` | Изменить режим одобрения для использования инструментов | `/approval-mode <mode (auto-edit)> --project` |
| →`plan`          | Только анализ, без выполнения                 | Безопасный обзор                              |
| →`default`       | Требовать одобрения для правок                | Повседневное использование                    |
...
| Команда       | Описание                                              | Примеры использования              |
| ------------- | ----------------------------------------------------- | ---------------------------------- |
| `/help`       | Показать справочную информацию о доступных командах   | `/help` или `/?`                   |
| `/about`      | Показать информацию о версии                         | `/about`                           |
| `/stats`      | Показать подробную статистику текущей сессии          | `/stats`                           |
...
| `@<путь_к_каталогу>`| Рекурсивно прочитать все текстовые файлы в каталоге | `@docs/ Кратко изложи содержание этого документа`|
...
| Формат команды       | Описание                                                                 | Примеры                                  |
| -------------------- | ------------------------------------------------------------------------ | ---------------------------------------- |
| `!<команда_оболочки>` | Выполнить команду в подпроцессе оболочки                                | `!ls -la`, `!git status`                 |
...
Сохраняйте часто используемые запросы в виде команд-шорткатов для повышения эффективности работы и обеспечения согласованности.
...
| Команды проекта   | `<корневой каталог проекта>/.qwen/commands/`      | Специфичные для проекта, поддаются контролю версий | Высокий   | Совместное использование в команде, специфичные команды проекта |
...
| Инъекция команд shell        | `!{command}`       | Требуется динамический контент       | Перед выполнением требуется подтверждение |
...
| --- | --- | --- |
| Есть параметры | Добавляются в конец запроса (разделены двумя переводами строк) | `/cmd parameter` → Исходный запрос + параметр |
...
prompt = """
Проверить {{args}}, ссылаясь на стандарты:
...
| Практические аспекты | Рекомендуемый подход                | Чего избегать                               |
| -------------------- | ----------------------------------- | ------------------------------------------- |

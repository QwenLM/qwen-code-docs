...
| -------------------------- | ------------------------------------------------- | ---------------------------------------------------------------- |
| Слэш-команды (`/`)         | Мета-управление самим Qwen Code                   | Управление сессиями, изменение настроек, получение справки       |
| Команды с `@`              | Быстрая вставка содержимого локальных файлов в диалог | Анализ ИИ указанных файлов или кода в директориях                |
...
| `/editor`    | Открывает диалог для выбора поддерживаемого редактора | `/editor`                     |
...
| →`auto-edit`     | Автоматически подтверждает изменения        | Доверенное окружение                              |
| →`yolo`          | Автоматически подтверждает всё              | Быстрое прототипирование                          |
| `/model`         | Переключает модель, используемую в текущей сессии | `/model`                                      |
| `/model --fast`  | Устанавливает облегченную модель для подсказок | `/model --fast qwen3-coder-flash`             |
| `/extensions`    | Список всех активных расширений в текущей сессии | `/extensions`                                 |
| `/memory`        | Управление контекстом инструкций ИИ         | `/memory add Important Info`                  |
...
| `/loop`      | Запуск промпта по расписанию с заданным интервалом                | `/loop 5m check the build`                        |
...
| `Ctrl/cmd+T`       | Переключить описание инструментов | Управление MCP-инструментами |
...
| ---------------- | ------------------------------------------ | ------------------------------------ | --------- | -------------------------------------------------- |
| Пространство имен | Поддиректория создает команды с двоеточием | Улучшенная организация команд        |           |                                                    |
...
| Метод обработки              | Синтаксис        | Сценарии применения                  | Особенности безопасности               |
| ---------------------------- | ---------------- | ------------------------------------ | -------------------------------------- |
...
| Внутри Shell-команды | `prompt = "Search: !{grep {{args}} .}"` | `/search "hello"`     | Выполнение `grep "hello" .` |

#### 2. Обработка параметров по умолчанию

| Ситуация ввода | Метод обработки                                      | Пример                                         |
| -------------- | ---------------------------------------------------- | ---------------------------------------------- |
| Есть параметры | Добавляется в конец промпта (разделено двумя переносами строки) | `/cmd parameter` → Исходный промпт + параметр  |
...
| Практика           | Рекомендуемый подход              | Чего избегать                               |
| ------------------ | --------------------------------- | ------------------------------------------- |
...
| Обработка ошибок   | Используйте вывод ошибок Shell    | Игнорировать сбой выполнения                |
| Организация файлов | Организуйте по функциям в директориях | Все команды в корневой директории           |
| Поле описания      | Всегда предоставляйте четкое описание | Полагаться на автогенерируемое описание     |

#### Таблица напоминаний о функциях безопасности

| Механизм безопасности | Эффект защиты              | Действие пользователя      |
| --------------------- | -------------------------- | -------------------------- |
| Экранирование Shell   | Предотвращает инъекцию команд | Автоматическая обработка   |
| Подтверждение выполнения | Избегает случайного выполнения | Подтверждение в диалоге    |

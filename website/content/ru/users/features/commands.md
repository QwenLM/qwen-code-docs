...
| `/peers`              | Просмотр сообщений, ожидающих обработки из других сессий Qwen Code на этой машине                    | `/peers`, `/peers accept <id>`, `/peers deny all`                                                         |
...
Сообщение поступает в другую сессию с пометкой, что оно исходит от другой
сессии, а не от её пользователя, и не несёт никаких ваших полномочий
там: получающая сессия обрабатывает его только в рамках своих собственных
настроек разрешений. Её пользователь может выбрать, что происходит
с входящими сообщениями, через `agents.crossSessionInbound` (`accept`,
`hold` или `refuse`). Если значение не установлено, сообщение
доставляется, если получающая сессия всё ещё проверяет каждое действие
(режим default или plan), или если обе сессии находятся в режиме,
применяющем действия без проверки каждого действия; в противном случае
оно удерживается для проверки. Удержанные сообщения отображаются
и освобождаются через `/peers` в получающей сессии.

Вызов `send_message` только подтверждает, что сообщение передано
в другую сессию. Что с ним произошло, приходит позже как квитанция:
если оно было удержано, отклонено, просрочено или неправильно адресовано
(адрес изменил владельца — перечислите агентов снова) — или освобождено
после удержания — уведомление появляется в транскрипте отправляющей
сессии (`Message to <name>: …`). Модель, отправившая его, не
уведомляется; если другая сессия отвечает, ответ приходит как
межсессионное сообщение.
...
Протокол версии 2 ограничивает текстовые значения `tool_result.content` до 65 536 байт UTF-8 после сериализации в JSON-строку. Превышающие размер значения становятся детерминированными превью начала/конца; тип события и схема полей остаются неизменными. Это лимит поля, а не универсальный лимит размера кадра JSONL.

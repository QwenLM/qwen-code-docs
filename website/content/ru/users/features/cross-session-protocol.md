...
Для Node-программы ` @qwen-code/sdk/peer` реализует присоединяющуюся сторону
этой страницы — запись, inbox, строку аутентификации, фреймы и подтверждения —
на чистом Node, и его тесты запускают его против собственной реализации
Qwen Code в обоих направлениях. Он не применяет никакие правила §6 к своему
inbox: программа, которой нужны ограничения по скорости, удержания или окно
дубликатов, применяет их самостоятельно. Используйте его или читайте дальше,
чтобы написать свой.
...
inbox, который он сам привязывает (§2). На Linux `pidNs` обязателен: каждый
читатель сравнивает его со своим, поэтому запись без него никогда не
отображается в списке — и никогда не очищается. `procStart` тоже обязателен,
по другой причине: без него читатель возвращается к простой проверке живости
PID и не может отличить повторно использованный PID от процесса, записавшего
запись. Записывайте во временный файл в том
же каталоге и делайте `rename` поверх целевого; создавайте файл с правами
0600; отказывайтесь писать через symlink. Если `<pid>.json` уже содержит
что-то, что вы не можете доказать, что это оставлено более ранним процессом
с вашим PID — тот же `pidNs`, тот же boot id, другие тики запуска, —
запишите `<pid>-<8 hex>.json` вместо замены: читатели принимают оба имени,
и запись там может принадлежать живому процессу в другом пространстве имён
или на другой машине. Удаляйте запись при выходе.
...
используется. `PeerEndpoint.start()` в ` @qwen-code/sdk/peer` делает всё это
и удаляет запись при `close()`.

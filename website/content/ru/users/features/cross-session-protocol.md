...
`$QWEN_HOME` по умолчанию `~/.qwen`. Имя файла — это PID записывающего
процесса и ничего более; запись, у которой поле `pid` не совпадает с именем
файла, игнорируется.
...
| `pid`           | Идентификатор процесса записывающего. Должен совпадать с именем файла.                                                                                                                                                                                                                                   |
...
имя файла совпадает с `pid`; `pidNs` совпадает с reader's; boot id внутри
`procStart` совпадает с reader's (или `procStart` равен `null`); и PID жив с
теми же тиками запуска. Актуальная запись с `ipcPath` всё равно должна быть
набрана (dialed), прежде чем она будет объявлена достижимой — файл сокета
переживает сбой.
...
- **Сессии под управлением демона.** Сегодня регистрируется только
  интерактивный UI, поэтому сессия, которую ведёт `qwen serve`, отсутствует
  в реестре, не может быть адресована и не может отправлять. Типы `serve` и
  `headless` зарезервированы для него.
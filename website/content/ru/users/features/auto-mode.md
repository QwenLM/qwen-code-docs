...
Это полезно для продакшен-сред или сред с высоким уровнем безопасности, где требуется эшелонированная защита: даже кажущиеся безобидными команды проверяются классификатором перед выполнением. Обратной стороной является дополнительная задержка (~300 мс на каждый вызов оболочки только для чтения) и зависимость от доступности классификатора — если API классификатора недоступен, команды оболочки только для чтения также будут требовать ручного одобрения.
...
- Если API классификатора недоступно, истекает время ожидания, превышает контекстное окно или возвращает некорректный ответ, текущее действие немедленно возвращается к ручному одобрению. Подтверждение рекомендует Default Mode и предлагает **Switch to Default Mode and allow once** наряду с Allow once и Reject. Переключение затрагивает только текущую runtime-сессию; оно не изменяет ваши сохраненные настройки.
...
- После **2 последовательных результатов unavailable** (сбоев API классификатора) последующие вызовы пропускают заведомо сломанный классификатор и переходят сразу к ручному одобрению. Первый результат unavailable уже запрашивает подтверждение; этот порог предотвращает повторное ожидание попыток повтора классификатора.

Сессия остается в Auto Mode, если вы явно не выберете опцию переключения. Только возвратный вызов проходит через ручное одобрение. Счетчики сбрасываются, когда вы одобряете возвратный вызов или переключаете режимы.
...
- **MCP-инструменты оцениваются по их аргументам, а не по проверенному поведению.** Сторонние MCP-инструменты (`mcp__*`) никогда не попадают в быстрый список разрешений; каждый вызов с сервера, не помеченного `trust: true`, отправляется в классификатор с именем сервера, именем инструмента, самоотчётами сервера (`readOnlyHint` / `destructiveHint` / `idempotentHint` / `openWorldHint`) и ограниченной копией аргументов. Классификатору сообщается, что аннотации не проверены. Он не видит, что сервер фактически делает с вызовом, поэтому вводящее в заблуждение имя инструмента вместе с безобидными аргументами всё равно может пройти. Если вы доверяете конкретному MCP-инструменту, добавьте `permissions.allow: ["mcp__server__tool"]`, чтобы он полностью обходил классификатор; если вы хотите, чтобы классификатор видел только имя инструмента (например, когда он работает против другого провайдера, чем основная модель), установите `permissions.autoMode.mcp.forwardArguments: false` — большинство MCP-вызовов тогда будут консервативно блокироваться.
...
- `edit` / `write_file`: file_path плюс превью старого/нового контента в 300 символов. Полный контент не пересылается.
...
MCP-инструменты (`mcp__*`): пересылаются имя сервера, имя инструмента, аннотации сервера и аргументы вызова. Каждая строка (значение или ключ) обрезается до 2 000 символов, имена — до 200, весь payload имеет общий бюджет 16 000 символов, измеряемый по красиво отформатированной форме, которую получает классификатор, а вложенность и количество записей ограничены; каждое обрезание помечается на месте (`…[truncated N chars]` или `[omitted: …]`) и отмечается флагом `arguments_truncated: true` / `name_truncated: true`, чтобы классификатор никогда не принял пропуск за отсутствие. Исторические действия в транскрипте ограничены 4 000 символов каждое и 40 000 символов суммарно (сначала хранятся новейшие; у старых остаётся только имя инструмента). Аргументы — это то, что агент собирается отправить на сервер — правила классификатора по утечке данных и внешней записи могут применяться только к ним, и они уже были произведены основной моделью, поэтому их пересылка в классификатор на той же конфигурации модели не раскрывает ничего нового. Если ваш классификатор работает против другого провайдера, установите `permissions.autoMode.mcp.forwardArguments: false`, чтобы вернуть проекцию только по имени (ожидайте, что большинство MCP-вызовов будут блокироваться).

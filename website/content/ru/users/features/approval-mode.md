Qwen Code предлагает три различных режима разрешений, которые позволяют гибко контролировать взаимодействие ИИ с вашим кодом и системой в зависимости от сложности задачи и уровня риска.
...
Чтобы начать новый сеанс в режиме планирования, используйте команду `/approval-mode`, а затем выберите `plan`
...
Стандартный режим — это обычный способ работы с Qwen Code. В этом режиме вы сохраняете полный контроль над всеми потенциально опасными операциями — Qwen Code запросит ваше одобрение перед внесением любых изменений в файлы или выполнением команд оболочки.
...
```
Мне нужно добавить аватары пользователей в наше приложение. Изображения должны храниться в бакете S3, а URL-адреса сохраняться в базе данных.
```
...
### Установите режим по умолчанию как default
...
Режим YOLO предоставляет Qwen Code максимально возможные права, автоматически одобряя все вызовы инструментов, включая редактирование файлов и выполнение команд оболочки.
...
- **Пакетная обработка**: Задачи, требующие цепочек команд из нескольких шагов

> [!warning]
>
> **Используйте режим YOLO с осторожностью**: ИИ может выполнять любые команды с правами вашего терминала. Убедитесь, что:
>
...
qwen --prompt "Запустить набор тестов, исправить все неудачные тесты, затем зафиксировать изменения"

# Без участия человека ИИ будет:

# 1. Выполнять команды тестирования (автоматическое одобрение)

# 2. Исправлять неудачные тестовые случаи (автоматическое редактирование файлов)

# 3. Выполнять git commit (автоматическое одобрение)
```
...
Во время сессии Qwen Code используйте **Shift+Tab**, чтобы быстро переключаться между тремя режимами:

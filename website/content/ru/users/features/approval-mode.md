...
- **Auto Mode (по умолчанию)**: Опыт работы «из коробки» по умолчанию — LLM-классификатор автоматически одобряет безопасные действия и блокирует рискованные, минимизируя прерывания, но сохраняя страховку
- **Переключитесь на Ask Permissions**: Когда вам нужно ручное одобрение для каждого редактирования файла и shell-команды
...
// .qwen/settings.json
{
  ...
  "permissions": {
    "autoMode": {
      ...
      // Опционально: отправлять вызовы инструментов MCP в классификатор только по имени
      // (аргументы по умолчанию передаются).

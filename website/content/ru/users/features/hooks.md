...
| `timeout`       | `number`                 | Нет         | Таймаут в секундах, по умолчанию 60              |
...
`timeout` измеряется в секундах для хуков command, HTTP и prompt; зарегистрированные через SDK хуки function используют миллисекунды. Таймауты хуков command раньше указывались в миллисекундах, поэтому для хуков command значение `1000` или больше по-прежнему интерпретируется как миллисекунды, и существующие настройки продолжают работать. Для миграции найдите хуки command, у которых `timeout` равен `1000` или больше, и перепишите значение в секундах, например `10000` как `10`. Чтобы задать хуку command таймаут 1000 секунд или более, продолжайте указывать его в миллисекундах, например `1800000` для 30 минут. `timeout` хука command, не являющийся положительным числом, например `"30s"`, игнорируется, и применяется значение по умолчанию 60 секунд. При включённом логировании отладки (`QWEN_DEBUG_LOG_FILE=1`) каждый хук command с миллисекундным или проигнорированным `timeout` один раз за сессию упоминается в отладочном логе этой сессии.
...
**Назначение**: Выполняется перед поддерживаемыми вызовами модели для валидации, блокировки или обогащения их входных данных. На пути core/headless событие в настоящее время охватывает отправку `UserQuery`, `ToolResult` и `Hook`, тогда как отправки `Retry`, `Steer`, `Cron`, `Notification` и `Teammate` пропускаются. Таким образом, оно может возникать на путях продолжения, и `prompt` не должен считаться исходным пользовательским вводом. Путь сессии ACP имеет собственную политику вызова: повторы и вновь запущенные фоновые задачи всё ещё могут вызывать устаревшие хуки; ходы continue, restored-question и runtime-goal — нет.
...
`submitted_prompt` — опциональное поле. Оно присутствует на поддерживаемых интерактивных отправках TUI и отправках `UserQuery` первого хода в headless-режиме. На пути сессии ACP, используемом клиентами ACP, `serve` и хостами демона, новый ход должен нести явное объявление отправки. Отсутствующие, нестроковые, пустые или состоящие только из пробелов объявления опускают поле; значение никогда не восстанавливается из `prompt` или отображаемой метки. Повторы, продолжения и классифицированные по каналу ходы опускают его. Исключение канала включает как автоматизированные события, так и человеческие сообщения, переданные через адаптеры канала.

Web Shell предоставляет исходный текст композера на своей границе отправки. Передачи голоса Realtime не объявляют происхождение, потому что их текст запроса поступает из сгенерированных моделью аргументов инструмента. Другие клиенты ACP/SDK демона могут явно подключиться на каждый запрос с помощью `_meta: { "qwen.submittedPrompt": "original submitted text" }`, захватываемого до расширения ресурсов или только модели. Существующие клиенты без этого объявления продолжают запускать устаревшие хуки, но не запускают Auto Recall с гейтом по происхождению. Не добавляйте объявление глобально в транспорт SDK: запланированные задачи, запуски задач Live, порождения суб-сессий, написанные моделью межсессионные сообщения и повышенные сообщения в середине хода не должны приобретать его автоматически. Приватный ключ `qwen.daemon.submittedPrompt` зарезервирован для перехода демон-потомок и удаляется у внешних вызывающих сторон. Эти объявления являются предоставленным вызывающей стороной происхождением, а не доказательством авторства или авторизации человека.

На пути ACP начальный устаревший `prompt` — это текстовые блоки запроса, соединённые пробелом перед расширением ресурсов, вложений, слеш-команд или только модели. Он не раскрывает полный расширенный вход модели. `submitted_prompt` может равняться этому тексту, но поступает только из явного объявления и сохраняет свои исходные пробелы. На пути core/headless устаревший `prompt` представляет текущий привязанный к модели текст для вызова хука. Ни одно поле не является полной поверхностью проверки DLP.

Следующие правила композера применяются к интерактивному TUI, а не к клиентам ACP. Отложенный ввод может сохранять поле, когда его происхождение остаётся полным. Комбинированный пакет сохраняет происхождение только тогда, когда каждый его элемент имеет его; отредактированный, частично известный или иной неоднозначный ввод опускает поле. Навигация по промптам, командам и shell-истории, выбранные поисковые совпадения, восстановление stash'а после перезапуска и восстановление отката диалога также опускают его, потому что эти пути могут возвращать привязанный к модели текст без его исходного происхождения. Потребители, которым требуется отправленный пользователем текст, должны трактовать отсутствие как недоступное, а не переключаться на `prompt`.

После очистки или отправки восстановленного ввода или ввода без доступного происхождения, композер также очищает свою историю undo и redo. Это предотвращает восстановление развёрнутого текста через undo после потребления его маркера или sidecar.

Плейсхолдеры больших вставок остаются компактными в `submitted_prompt`; развёрнутое содержимое вставки появляется только в `prompt`. На этом пути TUI потребители должны трактовать поле как проекцию текста, а не как побайтовую запись ввода из буфера обмена. Клиенты ACP не имеют эквивалентного встроенного отслеживания происхождения Vim, плейсхолдеров вставок, истории или отката; они сами определяют, сохраняет ли восстановленный или отредактированный текст действительное объявление отправки.
...
- Занимает один из 10 слотов для параллельных асинхронных хуков, пока не завершится или не достигнет своего `timeout` (по умолчанию 60 секунд)
...
- Таймауты предотвращают зависание хуков (по умолчанию: 60 секунд для хуков command)

...
Цель настолько хороша, насколько хорош её критерий завершения. См. [Goals](./goals.md) для информации о том, что верификатор может и не может оценить, и используйте `qwen -p "/goal-draft <intent>"`, чтобы составить цель перед её установкой.
...
> [!note]
>
> Это поведение относится к стандартным запускам headless-режима через CLI. ACP-сессии (IDE-интеграции, Web Shell) управляют тем же рантаймом Goal через метод расширения `sessionGoalControl` и получают каждое изменение статуса как уведомление `session/update` с `_meta.goalState` вместо stream-события `goal_state`.
...
Используйте `--output-style`, чтобы выбрать [стиль вывода](./output-styles) — встроенный или пользовательский — для данного запуска. Стиль — это именованный блок инструкций, накладываемый на встроенный промпт, который изменяет способ написания ответа — `Concise` начинается с результата и опускает вступление и повествование, `Proactive` начинает работу вместо предложения, `Explanatory` добавляет короткие заметки о кодовой базе по ходу дела. Он переопределяет настройку `general.outputStyle`; `default` не выбирает никакой стиль.
...
| `--output-style`             | Стиль вывода для данного запуска (`Concise`, `Proactive`, `Explanatory`, `Learning`, имя пользовательского стиля или `default` — без стиля); переопределяет `general.outputStyle`                                                                                                                                                                                                                                                                          | `qwen -p "query" --output-style Concise`                                 |

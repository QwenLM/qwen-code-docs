...
## Пересланные записи чатов

Вы можете переслать боту серию сообщений из другого чата методом объединённой пересылки DingTalk — как отдельное сообщение или как сообщение, на которое вы отвечаете. Бот разворачивает запись в текст для агента: заголовок и краткое содержание записи становятся строкой заголовка, а каждое пересланное сообщение перечисляется в разделе `[Chat record messages]` в формате `Отправитель: сообщение`. Пересланное сообщение, тело которого не является текстом, отображается как плейсхолдер — `[image]`, `[file: <name>]`, `[audio]`, `[video]`.

Длинные записи **ограничиваются, и это объявляется**: не более 50 сообщений, не более 4000 символов суммарно и не более 500 символов на сообщение. Всё, что вырезано, сообщается агенту в том же тексте — строка `[N more message(s) not shown]` в конце для отброшенных сообщений и маркер ` [truncated]` на любом сокращённом сообщении. Таким образом, агент знает, что отвечает по неполной записи; если вам нужна вся запись, пересылайте её меньшими частями.

Запись, **на которую вы отвечаете**, цитируется, а не отправляется, а цитируемый текст ограничивается 500 симвомами на любом канале — поэтому запись отображается в рамках этого лимита в 500 символов вместо лимита в 4000 символов, и те же объявления об ограничениях применяются внутри неё. Ожидайте, что цитируемая запись будет содержать свой заголовок и первое сообщение или два; перешлите её как отдельное сообщение, чтобы агент получил всю запись целиком.

Поскольку пересланную запись пишут другие люди, всё, что из неё извлечено — заголовки, имена отправителей, тела сообщений, — нейтрализуется перед передачей агенту, поэтому пересланное сообщение не может выдать себя за инструкцию боту.

Многострочный формат выше — это то, что агент видит в личном чате. В группе всё сообщение нейтрализуется второй раз перед передачей агенту, который сворачивает его в одну строку и убирает квадратные скобки вокруг маркеров; содержимое и объявления об ограничениях одинаковы в обоих случаях.
...
- **Цитируемые сообщения** — Цитирование (ответ на) сообщения пользователя включает цитируемый текст как контекст для агента. Если цитируемое сообщение — это изображение, файл, аудио или видео, бот загружает и прикрепляет его так же, как при прямой отправке. Цитирование ответов бота пока не поддерживается.

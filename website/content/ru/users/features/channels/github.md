...
Обратите внимание, что при `groupPolicy: "pairing"` доступ предоставляется для каждого репозитория: после одобрения репозитория **любой пользователь GitHub** может управлять ботом через issues и pull requests этого репозитория. Весь трафик GitHub — это групповой трафик, поэтому `senderPolicy` и `allowedUsers` не ограничивают участников одобренного репозитория. Одобрения привязаны к полному имени репозитория (`owner/repo`), которое меняется при переименовании или переносе — отзовите устаревшие групповые одобрения после любого переименования, переноса или удаления репозитория.

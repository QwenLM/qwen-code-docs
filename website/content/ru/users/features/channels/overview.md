...
Часть 2 использует одну выбранную задачу за раз и общее рабочее пространство. Создание задачи или переключение с выбранной задачи отклоняется, пока эта задача всё ещё выполняется или ожидает разрешения, и занятая задача не может быть закрыта. Одновременное переключение между выполняющимися задачами, именованная отмена и метки задач запланированы на Часть 3; worktree на каждую задачу запланированы на Часть 4. Память канала привязана к чату, а не к именованной задаче.
...
- `/session cancel [<name>]` — Распознаётся, но отложено до Части 3. Дождитесь завершения выбранной задачи перед переключением; пользователи Telegram могут использовать `/cancel` для выбранной задачи

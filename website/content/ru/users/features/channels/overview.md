...
Этот режим запускает рабочие процессы каналов, сгруппированные по рабочим пространствам, принадлежащие `qwen serve`. Воркеры подключаются к демону через SDK и используют те же адаптеры каналов. Они отделены от процесса демона, поэтому сбой адаптера канала не приводит к сбою демона. Явный выбор `--channel` имеет приоритет и прерывает запуск демона, если не может быть приведён в готовность. При запуске без флагов восстанавливается настройка `serve.channels` доверенного основного рабочего пространства. Вторичные рабочие пространства не восстанавливают свою собственную настройку `serve.channels` самостоятельно. Без какого-либо из этих источников демон не загружает адаптеры каналов и не резервирует аренду до первого `qwen channel set`.

Автоматическое восстановление пропускает недействительные настройки запуска, а также сбои валидации или аренды, возникающие до запуска воркеров, сохраняя при этом несвязанные настройки. После сбоя запуска воркера демон продолжает работу только после успешной очистки. Глобальный таймаут запуска среды выполнения или неподтверждённая остановка воркера по-прежнему следуют обычному пути сбоя запуска; аренда сервиса остаётся удерживаемой, пока прекращение работы воркера не подтверждено. Проверьте лог демона на сообщения с упоминанием `serve.channels`, если канал не восстанавливается.

Сохранённые имена запуска должны быть непустыми, не содержать начальных или конечных пробелов и не содержать небезопасных управляющих или невидимых символов. Недействительные записи пропускаются по отдельности и логируются по их индексу в массиве. Запуск не переименовывает экземпляры и не переписывает конфигурацию. Переключатель запуска канала отражает сохранённую настройку; состояние выполнения показывает, запущен ли канал.

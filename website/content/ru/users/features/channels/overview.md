...
Платформы хостинга кода (начиная с [GitHub](./github)) также поддерживаются через адаптеры опроса — агент отслеживает уведомления и отвечает на @упоминания в issues и pull requests.
...
1. Настройте бота в вашем мессенджере (см. руководства для конкретных каналов: [Telegram](./telegram), [WeChat](./weixin), [QQ Bot](./qqbot), [DingTalk](./dingtalk), [WeCom](./wecom), [Feishu](./feishu), [GitHub](./github))
...
| ------------------------ | ---------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `type`                   | Да               | Тип канала: `telegram`, `weixin`, `qq`, `dingtalk`, `wecom`, `feishu`, `github` или пользовательский тип из расширения (см. [Плагины](./plugins))                    |

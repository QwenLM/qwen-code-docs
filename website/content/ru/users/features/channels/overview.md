...
| `multiSession`           | Нет              | Сохранять до восьми именованных задач с областью владельца в одном чате. Требуется режим под управлением демона, `sessionScope: "user"`, отсутствие вебхуков и дозаполнения истории группы, а также отсутствующие включённые циклы канала |
| `messagePrefix`          | Нет              | Отправлять только те сообщения пользователя, которые начинаются с этого точного префикса (с учётом регистра) после любых начальных ` @упоминаний`; префикс и следующий за ним пробел удаляются перед отправкой                                            |
...
Когда установлен `messagePrefix`, каждое сообщение пользователя должно начинаться с префикса и непустого содержимого, например `/review inspect #123`. Удаляются только префикс и упоминания перед ним; упоминание, набранное пользователем после префикса, достигает агента без изменений. Общие команды и команды агента используют то же правило (`/review /help`, `/review /clear` и так далее). Зарегистрированные действия меню команд Telegram остаются доступными без префикса — если только настроенный префикс не совпадает с одним из них, в этом случае префикс имеет приоритет и эту команду тоже нужно отправлять с префиксом (`/new /new`). Вложениям нужно подходящее описание, когда платформа его поддерживает; медиа-сообщения Telegram, Feishu, WeChat, DingTalk и WeCom без описания продолжают работать, и их текст-заглушка никогда не цитируется обратно как история группы. Нативные todo, вебхуки и генерируемые провайдером события назначения или запроса рецензии также продолжают работать без префикса, потому что это системные события, а не сообщения чата.

Два поведения являются намеренными, и о них стоит знать перед включением префикса. Голосовое сообщение, транскрипт которого заполняет DingTalk или WeCom, считается текстом, произнесённым пользователем, поэтому оно должно нести префикс, как и любое другое сообщение, и иначе отбрасывается — только нетранскрибированная голосовая заметка работает как медиа без описания. И префикс проверяется до сопряжения, поэтому первый контакт от неизвестного отправителя или неодобренной группы тоже должен нести префикс; без такого порядка каждое сообщение без префикса в активной группе вызывало бы ответ сопряжения, что именно и является шумом, для подавления которого префикс и существует. Сообщите новым пользователям префикс вне канала, или оставьте каналы сопряжения без префикса.
...
- **`chat_thread`** — Одна сессия на чат-тред/тему, общая для участников этого треда.
- **`thread`** — Устаревшая маршрутизация по тредам/темам, сохранённая для существующих конфигураций.
- **`single`** — Одна общая сессия для всех пользователей. Все участники ведут один общий диалог.

### Именованные задачи

Каналы под управлением демона могут хранить несколько именованных диалогов для одного пользователя в одном чате:
...
Каталог является приватным для конкретного канала, чата и отправителя. Имена задач используют 1–32 ASCII-буквы, цифры, подчёркивания или дефисы и уникальны без учёта регистра. Может быть открыто до восьми задач; закрытие задачи отсоединяет её без удаления транскрипта, поэтому повторный выбор позже открывает тот же самый диалог. ID сессий никогда не принимаются и не показываются в командах чата.

Именованные результаты идентифицируют свою исходную задачу: прямые чаты используют `[task]`, а групповые чаты используют `[sender · task]`. Именованные текстовые запросы разрешений также показывают точный ID запроса и соответствующие команды `/approve <id>`, `/approve-always <id>` и `/deny <id>`. Метка используется только для отображения и не сохраняется в транскрипте модели.

Одна задача остаётся выбранной для получения следующего обычного сообщения, но другие именованные задачи могут продолжать выполняться параллельно. `/session new <name>` использует настроенное рабочее пространство, а `/session new <name> --worktree` создаёт изолированную копию для этой задачи в каталоге `.qwen/worktrees/` рабочего пространства демона. Демон проверяет сохранённого владельца worktree перед повторным открытием задачи после перезапуска; отсутствующая, изменённая или чужая запись о владении вызывает fail closed вместо молчаливого перемещения задачи в общее рабочее пространство. Создание или выбор другой задачи не отменяет и не перенаправляет предыдущую работу, а поздние результаты сохраняют метку своей исходной задачи. Занятую задачу нельзя закрыть, но её активный промпт можно отменить с помощью `/session cancel [<name>]` через существующее поведение отмены канала. Независимо поставленные в очередь ходы не отменяются, но в режиме диспетчеризации `collect` любые последующие сообщения, буферизованные за отменённым промптом, отбрасываются этим существующим поведением. Подготовка медиа не таргетируется. Базовые команды разрешений применяются только к выбранной задаче, тогда как явный ID запроса может ответить неактивной задаче, принадлежащей пользователю. `/clear`, `/new` и `/reset` также работают для выбранной задачи с worktree: задача получает новый диалог, а её worktree и файлы сохраняются. Занятая задача с worktree отклоняет сброс до завершения своего промпта, а задача с повреждённой записью worktree сообщает о сбое, не затрагивая файлы. Память канала привязана к чату, а не к именованной задаче.

Этот режим недоступен в автономном `qwen channel start`, с вебхуками, с ненулевым `groupHistoryLimit` канала или группы, или с циклами канала. Если для этого канала уже существует включённый цикл, воркер демона отказывается запускаться, пока цикл не будет отключён.
...
Каналы используют обычный путь доставки ответов. Общий слой доставки отправляет завершённые ответы, а адаптеры могут предоставлять нативный прогрессивный вывод, например обновление интерактивной карточки на месте. Ограничения длины сообщений платформы всё ещё могут разбивать длинные ответы.

Устаревшие настройки `blockStreaming`, `blockStreamingChunk` и `blockStreamingCoalesce` больше не поддерживаются и могут быть удалены из конфигурации канала. Они не влияют на доставку. Управление настройками канала отклоняет новые добавленные или изменённые значения для этих полей. Незаписанное сохранённое значение сохраняется или удаляется, если редактирование сохраняет `type` канала; смена `type` канала требует предварительного удаления этих полей.
...
- `/btw <question>` — Задать побочный вопрос, не прерывая текущую задачу; текстовые вопросы до 4096 символов, требуется подключение к агенту с поддержкой побочных вопросов
- `/sessions [all]` — Список открытых именованных задач, или включить закрытые задачи; доступно только при `multiSession: true`
- `/session current` — Показать выбранную именованную задачу
- `/session new <name>` — Создать и выбрать задачу с общим рабочим пространством
- `/session new <name> --worktree` — Создать и выбрать задачу в собственном Git worktree; только для режима именованных задач под управлением демона
- `/session use <name>` — Выбрать открытую задачу или переоткрыть закрытую задачу
- `/session cancel [<name>]` — Отменить активный промпт выбранной задачи или назвать другую принадлежащую задачу; независимо поставленные в очередь ходы не отменяются, но последующие сообщения в режиме `collect`, буферизованные за отменённым промптом, отбрасываются существующим поведением отмены; подготовка медиа не таргетируется
- `/session close <name>` — Закрыть задачу без удаления её транскрипта
...
Все остальные слэш-команды (например, `/compress`, `/summary`) перенаправляются агенту. Команды именованных задач регистрируются только при включённом режиме, поэтому `/sessions` остаётся видимой для агента для существующих конфигураций.

Команды именованных задач работают во всех типах каналов (Telegram, WeChat, QQ, DingTalk, WeCom, Feishu, GitHub). `/cancel` в настоящее время регистрируется только Telegram, а создание циклов требует поддержки проактивной доставки для текущего адаптера и цели.

...
4. Если используется `groupPolicy: "pairing"`, одобрите запрос на сопряжение группы один раз перед началом ответов. Обратите внимание, что после одобрения группы **любой участник этой группы** может использовать бота; `senderPolicy` и `allowedUsers` не ограничивают участников одобренной группы.
...
- Проверьте, что `groupPolicy` установлено в `"allowlist"`, `"pairing"` или `"open"` (по умолчанию `"disabled"`).

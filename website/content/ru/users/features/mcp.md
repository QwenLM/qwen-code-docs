...
### Автоматическое согласование stdio

По умолчанию stdio-серверы используют однопроцессный устаревший поток инициализации. Для подключения к stdio-серверу, поддерживающему только современный протокол, включите автоматическое согласование протокола:
...
Автоматическое согласование запускает временную копию настроенного сервера перед запуском процесса сессии и может использовать до пяти секунд из бюджета обнаружения. Оставляйте политику legacy по умолчанию для серверов с не-идемпотентными побочными эффектами при запуске, эксклюзивными блокировками или PID-файлами, или с медленными рукопожатиями инициализации.
...
- **Доверие серверу** (`trust: true`): обходит запросы подтверждения для этого сервера, но только в доверенном рабочем пространстве (используйте с осторожностью).

### Воспроизведение при потере соединения

Qwen Code повторно подключается и воспроизводит текущий вызов MCP-инструмента только в том случае, если у сервера указано `trust: true`, рабочее пространство является доверенным, а инструмент явно объявляет либо `idempotentHint: true`, либо аннотацию только для чтения. Аннотации только для чтения конфликтуют с `destructiveHint: true` или `idempotentHint: false` и не воспроизводятся.

Вызовы с отсутствующими аннотациями, конфликтующими аннотациями, ненадежным сервером или ненадежным рабочим пространством не воспроизводятся после сбоя соединения. Qwen Code сообщает, что результат может быть неизвестен, поскольку сервер мог завершить операцию до потери ответа. Проверьте результат перед повторной попыткой. Такое консервативное поведение может отличаться от предыдущих версий, которые прозрачно повторяли вызовы без аннотаций.

Аннотации — это поведенческие подсказки, предоставляемые сервером, а не разрешения или граница авторизации. Настраивайте `trust: true` только для серверов, которыми вы управляете и чьи аннотации вы проверили.
...
- **Удаленные/облачные развертывания**: При запуске Qwen Code на удаленных серверах, облачных IDE или веб-терминалах перенаправление по умолчанию на localhost НЕ будет работать. Настройте `--oauth-redirect-uri` с публичным URL, заканчивающимся на `/oauth/callback`, затем направьте reverse-proxy с этого пути на `http://127.0.0.1:7777/oauth/callback` на машине, где запущен Qwen Code. Qwen Code не завершает TLS; это должен делать прокси.
...
Например, reverse proxy может перенаправлять только этот путь callback на локальный слушатель:
...
| `versionNegotiation`   | `"auto" \| "legacy"`<br>(по умолчанию: `"legacy"`) | Для stdio-серверов: `"auto"` включает согласование протокола на временном дочернем процессе. Значение по умолчанию `"legacy"` запускает только процесс сессии.                                                                                                               |
| `trust`                | boolean<br>(по умолчанию: false)  | Если `true`, обходит подтверждения вызова инструментов для этого сервера в доверенном рабочем пространстве (по умолчанию: `false`)                                                                                                                                                                              |
...
| `--trust`                   | Доверять серверу; пропуск подтверждений в доверенных рабочих пространствах.         | — (`false`)                            | `--trust`                                                          |

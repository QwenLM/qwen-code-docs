...
- Режим одобрения не установлен на `plan`
- Функция включена (по умолчанию включена — установите `ui.enableFollowupSuggestions` в `false`, чтобы отключить)

Предложения не будут появляться в неинтерактивном режиме CLI (например, в режиме headless/SDK). В демоне генерация происходит на стороне сервера и выполняется после каждого хода, удовлетворяющего условиям выше, поэтому headless- или SDK-клиент, который не может отображать предложения, должен установить `ui.enableFollowupSuggestions` в `false`, чтобы избежать затрат LLM на каждый ход.

...
**Aone Code:** для клона, чей origin находится на `gitlab.alibaba-inc.com`, запустите `/review` изнутри этого клона — платформа определяется по remote, и подкоманды работают через CLI `a1` — целевой номер является глобальным id MR. `fetch-pr` получает `refs/merge-requests/<id>/head` и строит worktree + diff, поэтому агентское ревью worktree не меняется. Каждый запуск Aone выполняется без контекста, и несколько потоков пропускаются (вместо обращения к одноимённому репозиторию на github.com): `pr-context`/`comment-status`/`presubmit` не имеют поддержки Aone (вердикт ограничен до `COMMENT`), `test-plan` не поддерживается, Агент 0 пропускается, и запись `publish-assets` пропускается. `--comment` **публикует** ревью через CLI `a1`: один комментарий на каждую inline-находку, затем сводный комментарий. У Aone нет нативного состояния request-changes — при таком вердикте сводный комментарий несёт блокирующий заголовок, и любые inline-Critical, которые были опубликованы, блокируют мерж через гейт обсуждений, пока их обсуждения не закрыты (когда inline-Critical не публиковались, заголовок носит рекомендательный характер и ничто не блокирует мерж механически). Нативная команда `a1 repo mr approve` подключена для вердикта Approve, но не срабатывает в этой фазе: ограничение недоступности контекста ограничивает каждый вердикт Aone до Comment. Два нюанса для повторных раундов: дедупликации пока нет, поэтому второй раунд `--comment` повторно публикует каждую всё ещё актуальную находку как новый комментарий, и обнаружение собственных PR не имеет поддержки Aone. См. `docs/design/2026-08-15-review-aone-provider.md`.

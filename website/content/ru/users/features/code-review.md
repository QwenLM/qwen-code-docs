...
- Сноска об авторстве модели и версии CLI в каждом комментарии (например, _— qwen3-coder via Qwen Code /review (v0.21.2)_); установите `review.attribution` в `false` в вашем пользовательском или системном `settings.json` (рабочий `.qwen/settings.json` игнорируется для настроек `review.*`), чтобы публиковать без неё

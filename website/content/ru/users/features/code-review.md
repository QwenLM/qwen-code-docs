...
Шаг 5:  Итеративный обратный аудит, fan-out по чанкам;
        остановка после 2 последовательных сухих раундов (лимит 5)
...
- Сноска об авторстве модели и версии CLI в каждом комментарии (например, _— qwen3-coder via Qwen Code /review (v0.21.2)_)
...
**GitHub Enterprise:** ревью PR по URL на хосте, отличном от `github.com`, направляет все вызовы GitHub на этот хост — подкоманды ревью (`fetch-pr`, `pr-context`, `comment-status`, `presubmit`) принимают `--host` и устанавливают его в коде, поэтому забытый хост не может молча перенаправить ревью на `github.com`.
...
| Итеративный обратный аудит (Шаг 5) | 2-5 (3A); раунды × чанки (3B) | Два последовательных сухих раунда для остановки (лимит 5); 3B делает fan-out по одному аудитору на чанк за раунд  |
| **Всего**                        | **~17-23 (~15-22)**            | 3A в том же репо: ~17-23 (типично ~17-19); кросс-репо или локальные/файловые: ~15-22; 3B масштабируется с чанками (см. DESIGN.md) |

...
- **Для ротации `--tls-cert` без перезапуска нужен перезапуск демона** — демон обслуживает байты, прочитанные при запуске, поэтому до перезапуска возрождённые воркеры могут загрузить более новое содержимое, чем представляет демон, и их рукопожатия завершатся ошибкой.

...
Для удаленного зеркалирования панели `/skills` из CLI вызовите `POST /workspace/skills/:name/enable` с `{ "enabled": true | false }`, проверив возможность `workspace_skill_toggle`. Маршрут обновляет `skills.disabled` и `skills.enabled` рабочего пространства по мере необходимости, отклоняет неизвестные, скрытые, неактивные из расширения, заблокированные в области с более высоким приоритетом и недоверенные цели, и немедленно обновляет активные сессии ACP. Включение навыка `skills.defaultDisabled` записывает канонический opt-in в `skills.enabled`; жесткая запись `skills.disabled`, унаследованная из области с более высоким приоритетом, по-прежнему не может быть переопределена. Ячейки статуса навыков предоставляют `disabledReason` (`hard`, `default` или `inactive_extension`) и опциональный `lockedScope`. Ответ `deferred` означает, что настройка была сохранена, пока дочерний процесс ACP не запущен; она применится при запуске дочернего процесса. `skills.disabled` отключает как ручное использование, так и использование моделью, в отличие от `disable-model-invocation: true`, который оставляет доступным прямой вызов `/skill-name`.
...
| `--memory-project-scope <mode>`         | `git-root`         | Режим разделения памяти проекта. `git-root` (по умолчанию) разделяет память между рабочими пространствами, разрешенными в один и тот же корень Git; `workspace` ключирует память по точному зарегистрированному каталогу рабочего пространства, так что каждое рабочее пространство демона получает свою изолированную память. Переопределяет `QWEN_CODE_MEMORY_PROJECT_SCOPE`, если указан; нераспознанное значение env игнорируется с одноразовым предупреждением и возвращается к `git-root`. Переключение на `workspace` не мигрирует существующую память проекта git-root — такие записи перестают быть видимыми, пока вы не переключитесь обратно.                                                                                                                                                                                                                                                                                                                                            |
...
| `--compacted-replay-max-bytes <n>`      | `4194304`          | Побайтовый лимит на живую сессию для сохранённых событий воспроизведения в ограниченном снимке, возвращаемом `POST /session/:id/load`. Лимит применяется к `compactedReplay`; текущий живой журнал `liveJournal` отдельно ограничивается `--max-journal-events` и `--max-journal-bytes`. Значения должны быть положительными безопасными целыми числами; невалидные значения вызывают сбой при запуске, а жёсткий потолок — 256 МиБ. Когда старые сохранённые события воспроизведения отбрасываются, снимок начинается с `history_truncated`. Это не ограничивает транскрипт на диске. |
| `--max-journal-events <n>`              | `10000`            | Лимит на количество сырых событий, сохраняемых в живом журнале в полете (текущий незавершенный ход) на сессию. При превышении самые старые записи журнала отбрасываются и добавляется маркер `history_truncated`. Должен быть положительным безопасным целым числом.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `--max-journal-bytes <n>`               | `8388608`          | Лимит байтов на сессию для живого журнала в полете. При превышении самые старые записи журнала отбрасываются (всегда сохраняется хотя бы одна запись). Должен быть положительным безопасным целым числом. По умолчанию 8 МиБ.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
>
> - **Демон vs. автономный CLI.** `--memory-project-scope` (и
>   `QWEN_CODE_MEMORY_PROJECT_SCOPE`) влияет только на среды выполнения под управлением демона.
>   Автономный TUI `qwen`, запущенный в том же каталоге, по-прежнему использует
>   область git-root, если переменная окружения не экспортирована глобально. Чтобы сохранить
>   согласованность обеих точек входа, зафиксируйте область в `.env` рабочего пространства или
>   `settings.env`, чтобы каждый процесс, читающий рабочее пространство, соглашался.

...
### Удалённый Web Shell и доступ с телефона

```bash
qwen serve --hostname 0.0.0.0
```

Если не указаны ни `--token`, ни `QWEN_SERVER_TOKEN`, слушатель не-loopback генерирует эфемерный 128-битный bearer (22 URL-безопасных символа) и печатает его один раз при запуске. Явные значения флага имеют приоритет над окружением; при не-loopback привязке явно пустое значение всё равно отказывает в запуске (fail closed), тогда как на loopback оно затеняет `QWEN_SERVER_TOKEN` и оставляет привязку в доверенном режиме без токена, описанном в разделе `--require-auth`. Стандартное поведение loopback не изменяется.

При wildcard-привязках запуск перечисляет локальные/сетевые URL без токена с реальных интерфейсов и — для сгенерированного токена или для вашего собственного токена при интерактивном терминале — **QR-код с секретом** для одного помеченного адреса приватной LAN. Перечисление wildcard перечисляет только ту же приватную/link-local LAN-популяцию, которую рекламирует Local Control (RFC 1918 / RFC 3927 IPv4, плюс fc00::/7 ULA IPv6 на dual-stack привязках); фильтр программных интерфейсов, который не пропускает VPN-адаптеры и контейнерные бриджи, — это **эвристика по имени** ([#9158](https://github.com/QwenLM/qwen-code/issues/9158)), а не гарантия, поэтому имена overlay k8s (`cbr0`, `cali*`, `vxlan.*`) могут проскочить. Явная не-loopback привязка вместо этого печатает и QR-кодует ровно тот адрес, который вы выбрали — включая маршрутизируемый. Внутри контейнера перечисленный адрес — это собственный интерфейс контейнера, поэтому для доступа к опубликованному порту нужно подставить адрес хоста; контейнер с pty-аллокацией считается интерактивным терминалом, поэтому его логи остаются с секретами. Сканируйте QR, чтобы открыть встроенный Web Shell с bearer во фрагменте URL. Браузер удаляет учётные данные из URL и хранит их в `sessionStorage` для каждой вкладки. Альтернативно, откройте напечатанный URL и введите токен; отсутствующие или просроченные учётные данные показывают форму повторной попытки. Сгенерированные токены ротируются при перезапуске. `--no-web` пропускает QR, а piped/redirected stdout никогда не публикует стабильный токен оператора как QR. Адреса не подразумевают доступность через firewall или NAT; публичный IP не обнаруживается автоматически.

Одноисточниковые **HTTP**-запросы встроенного Web Shell работают без `--allow-origin`; другие browser origin всё ещё нуждаются в этом allowlist, а `Origin: null` остаётся отклонённым. Функции на базе WebSocket (терминал, голос) и браузеры, достигающие демона через reverse proxy с терминацией TLS, всё ещё требуют явной записи `--allow-origin <origin>` — пересланным заголовкам никогда не доверяют для установления одноисточниковости. То же самое для любого plain-HTTP посредника, который переписывает заголовок `Host`: стандартный `proxy_set_header Host $proxy_host` nginx и k8s Ingress оба заставляют демон видеть `Host`, который больше не совпадает с `Origin` браузера. Передайте `--allow-origin <origin>` для origin, который на самом деле видит ваш браузер, или настройте прокси на дословную пересылку `Host`. При **не-loopback** привязке только трансляция порта (`docker -p 8080:4170`) ничего не требует — проверка одноисточниковости сравнивает `Origin` с пересланным `Host` и никогда не обращается к порту слушателя. При стандартной **loopback** привязке это не так: allowlist Host для защиты от DNS rebinding принимает только собственный порт демона, поэтому туннель с трансляцией порта (`ssh -L 8080:localhost:4170`) отклоняется с `403 Invalid Host header` для каждого запроса, включая документ оболочки, и `--allow-origin` не может это переопределить — пересылайте тот же порт (`ssh -L 4170:localhost:4170`) или привязывайтесь к не-loopback. Bearer предоставляет полные полномочия API демона, включая выполнение кода. Относитесь к токенам терминала и снимкам QR как к секретам. HTTP не зашифрован; существующие опции `--tls-cert` и `--tls-key` предоставляют HTTPS с вашим собственным сертификатом.
...
### Ёмкость регистрации рабочих пространств

Стандартный лимит регистрации — 256. Установите `QWEN_SERVE_MAX_WORKSPACES=25 qwen serve --workspace /path/to/project` для уменьшения; допустимые значения — десятичные целые числа от 1 до 256. Невалидные значения приводят к сбою запуска. Это настройка запуска демона: проект `.env` и проект `settings.env` не могут её изменить. Встроенные вызывающие стороны могут установить `ServeOptions.maxRegisteredWorkspaces`, который имеет приоритет над окружением запуска.

Лимит учитывает основное рабочее пространство, зарегистрированные вторичные рабочие пространства и созданные пользователем рабочие пространства для экспериментов. Внутренняя среда выполнения Conversations освобождена от лимита. Динамическая регистрация возвращает `409 workspace_limit_reached` при достижении ёмкости. `GET /capabilities` и `GET /daemon/status` предоставляют `limits.maxRegisteredWorkspaces`.

При стандартной ёмкости регистрации 256 стандартный демон по умолчанию использует фиксированный глобальный лимит сессий 800, включая демон, запущенный только с одним рабочим пространством. Тот же стандарт сессии применяется к любой настроенной ёмкости регистрации выше 25. `--max-total-sessions` всё ещё переопределяет его; `0` отключает его. При ёмкости 25 или менее остаётся существующий стандарт: одно запущенное или восстановленное рабочее пространство не ограничено, а несколько выводят общий лимит из лимита на рабочее пространство и этого количества. Изменения регистрации не пересчитывают общий лимит. Это не изменяет политику памяти дочернего процесса ACP и не обещает 800 безопасных одновременных сессий.

Управление каналами независимо поддерживает не более 25 владельцев рабочих пространств, что предоставляется как `limits.maxChannelControlWorkspaces`. Переход также учитывает владельцев, сохранённых для восстановления; при ёмкости успешно остановите все выбранные каналы на старом владельце перед добавлением нового владельца. Превышение размера перехода возвращает `409 channel_control_workspace_limit_reached`. `--channel all` продолжает выбирать только основное рабочее пространство. Таймаут SDK каналов не изменён.

Сохранённые файлы регистрации сохраняют схему версии 1 и поддерживают до 255 вторичных записей в пределах 8 МиБ. Сырые записи, включая устаревшие алиасы, потребляют постоянные слоты до забывания. Если полный валидный набор запуска превышает настроенный лимит, запуск завершается сбоем и оставляет файл без изменений. Для уменьшения ёмкости перезапустите с предыдущей ёмкостью, забудьте достаточно регистраций и уменьшите явные аргументы `--workspace` перед перезапуском с меньшим лимитом. Перед понижением до более старого бинарного файла создайте резервную копию файла и используйте более новый бинарный файл для уменьшения его до не более чем 24 вторичных записей и 256 КиБ. Более старые бинарные файлы иначе отклоняют это хранилище и запускаются только с явными рабочими пространствами; их мутации хранилища завершаются сбоем до записи.

### Настройка интервала опроса live-состояния сессии

Web Shell обновляет live-состояние сессий рабочего пространства каждые **5 секунд** по умолчанию. Для использования другого интервала установите переменную окружения демона перед его запуском:

```bash
QWEN_SESSION_LIVE_STATE_POLL_INTERVAL_MS=10000 qwen serve --web
```

Значение — целое число в миллисекундах, от `1000` до `2147483647`. Отсутствующие или невалидные значения возвращаются к `5000`. Интервал применяется ко всем рабочим пространствам, обслуживаемым этим демоном; перезапустите демон и перезагрузите любые открытые страницы Web Shell после его изменения. Локальные действия сессий и возврат на видимую вкладку всё ещё могут обновляться немедленно. Это изменяет только опрос live-состояния, а не полный опрос каталога сессий.

Демон публикует эффективный интервал как `sessionLiveStatePollIntervalMs` в `/capabilities`. Web Shell использует это значение и возвращается к пяти секундам при подключении к более старому демону, который его опускает. Потребители SDK могут читать это значение при выборе собственного графика опроса.
...
| `--hostname <addr>`                     | `127.0.0.1`     | Интерфейс привязки. Всё, что выходит за пределы loopback, требует токен; если ни `--token`, ни `QWEN_SERVER_TOKEN` не предоставляют токен, генерируется эфемерный токен, который печатается при запуске.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
| `--token <str>`                         | —               | Bearer-токен. Резервный источник — переменная окружения `QWEN_SERVER_TOKEN` (с удалением начальных и конечных пробелов — удобно для `$(cat token.txt)`). Привязка не к loopback без какого-либо источника генерирует эфемерный токен. Явно пустое значение всё равно отказывает в запуске при привязке не к loopback; на loopback оно затеняет переменную окружения и выбирает доверенный режим без токена (или полностью отказывает при `--require-auth`).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| `--require-auth`                        | `false`         | Отказ от запуска без bearer-токена, даже на loopback. Усиливает стандартную настройку разработчика `127.0.0.1` для общих dev-хостов / CI-раннеров / многопользовательских рабочих станций, где любой локальный пользователь может обратиться к слушателю. Этот fail-fast действует только на loopback: loopback запускается только с одним из трёх источников токена — `--token`, `QWEN_SERVER_TOKEN` или `--open-with-auth`, который устанавливает собственный сгенерированный токен перед запуском (генерация подавляется на loopback) — тогда как при привязке не к loopback сгенерированный эфемерный токен удовлетворяет флагу. Также защищает `/health` bearer-токеном.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| `--enable-session-shell`                | `false`         | Включить прямое выполнение оболочки сессии. Действует при настроенном bearer-токене или в режиме доверенного loopback; вызовы всё ещё требуют действительный `X-Qwen-Client-Id`, привязанный к сессии. |
...
| `--max-total-sessions <n>`              | `800` или производное | Опциональный неотрицательный целочисленный общедемонный лимит создания новых сессий по всем зарегистрированным средам выполнения рабочих пространств. Применяется к новым дочерним сессиям, восстановлению сессий и сессиям, созданным через branch/fork; подключение к существующей живой сессии не потребляет слот. Установите `0` для неограниченного режима. По умолчанию 800, когда ёмкость регистрации выше 25 (включая стандартные 256). При ёмкости 25 или менее одно запущенное или восстановленное рабочее пространство не ограничено; несколько выводят общий лимит из лимита на рабочее пространство и этого количества. Последующая динамическая регистрация не пересчитывает его.                                                                                                                                                                                                                                                                                                                                                                       |
...
- **`--hostname 0.0.0.0` требует токен** — один генерируется, если не предоставлен ни один источник токена; явно пустые источники всё равно завершаются ошибкой (fail closed).
...
- **CORS по умолчанию отклоняет любой Origin браузера** — возвращает `403` JSON. Передайте **`--allow-origin <pattern>`** (можно повторять, T2.4 #4514), чтобы пропустить определенные Origins браузеров. Каждое значение — это либо литерал `*` (любой origin — при привязке к **loopback** запуск отклоняется, если не настроен bearer-токен, тогда как привязка не к loopback запускается, потому что генерация токена предоставляет его; для полного усиления защиты рекомендуется `--require-auth` на loopback, так как `/health` по умолчанию остается pre-auth на loopback — обратите внимание, что статические ассеты Web Shell (`/`, `/assets/*`, навигации документов `/session/:id`) монтируются до bearer-аутентификации в любом режиме и остаются pre-auth даже при `--require-auth`, поэтому используйте `--no-web`, когда остаточная браузерная поверхность важна), либо канонический URL origin (`<scheme>://<host>[:<port>]`, без завершающего слэша / пути / userinfo). При привязке к **loopback** без токена записи HTTP(S) должны использовать loopback-хост; привязка не к loopback всегда несет токен — предоставленный или сгенерированный — именно потому, что удаленно размещенные browser origins могут выполнять код через API оператора от имени пользователя демона. Явные origins расширений браузера сохраняют свой путь локальной автоматизации без токена. Совпавшие origins получают правильные заголовки ответа CORS (`Access-Control-Allow-Origin: <echoed>`, `Vary: Origin`, а также стандартные методы / заголовки / max-age и выставленный `Retry-After`); несовпавшие origins по-прежнему получают 403 с тем же конвертом, что и при стандартной блокировке. `caps.features.allow_origin` анонсируется условно, чтобы SDK / webui клиенты могли предварительно проверить, поддерживает ли демон кросс-доменные запросы, перед их выполнением. Пример: `qwen serve --allow-origin http://localhost:3000 --allow-origin http://localhost:5173`. Запросы с loopback на себя (например, Web Shell UI) не затрагиваются — отдельная заглушка для удаления Origin обрабатывает их независимо от `--allow-origin`. Одноисточниковые **HTTP**-запросы встроенного удаленного Web Shell также работают без `--allow-origin`: при основном слушателе не к loopback `Origin`, равный схеме прямого сокета плюс нормализованные полномочия `Host`, проходит bearer-аутентификацию и затем удаляется перед стеной CORS, в то время как его существующие публичные статические маршруты остаются без аутентификации. Два исключения всё ещё требуют `--allow-origin` — пересланные заголовки никогда не устанавливают доверие одноисточниковости, поэтому `https` browser origin reverse proxy с терминацией TLS не является одноисточниковым (как и plain-HTTP посредник, переписывающий заголовок `Host`, например стандартный `proxy_set_header Host $proxy_host` nginx или k8s Ingress; решение — `--allow-origin <origin>` для origin, который видит ваш браузер, или прокси, пересылающий `Host` дословно — одна только трансляция порта ничего не требует, потому что сравнивается только пересланный `Host` с удалёнными портами по умолчанию схемы `:80`/`:443`), и маршруты обновления WebSocket (терминал, голос) сохраняют отдельный гейт CSWSH, который принимает только loopback origins, origins из allowlist и собственный origin слушателя Local Control.
...
`limit` — это целевое количество активных записей чата, а не отправленные фреймы воспроизведения; одна запись может создать несколько событий `session_update`. Обратная страница может расшириться до максимум `3 * limit` записей для сохранения границ ходов и вызовов инструментов/результатов. Первый ответ фиксирует размер снимка JSONL и возвращает `nextCursor`, пока `hasMore` истинно. Последующие страницы игнорируют добавления после страницы 1, но возвращают `409`, если файл удален, усечен, заменен, архивирован или иным образом конфликтует с фиксированным курсором. Очень большие снимки возвращают `413 transcript_too_large` перед индексацией, чтобы демон не сканировал неограниченные файлы транскриптов на пути запроса.

Для повторного постраничного просмотра через устаревший единичный маршрут установите `--channel-idle-timeout-ms` в положительное значение. При стандартном `0` неактивный дочерний процесс ACP рабочего пространства — и кэш индекса транскриптов в процессе, который он хранит — утилизируется после каждой страницы, поэтому каждая страница повторно порождает дочерний процесс и перестраивает индекс повторным сканированием всего фиксированного префикса (`O(snapshotSize)` на страницу). Положительный таймаут сохраняет дочерний процесс активным при обходе курсора, чтобы он повторно использовал свой кэшированный индекс транскриптов и конфигурацию воспроизведения. Прямые страницы и страницы с курсором с квалификацией рабочего пространства остаются локальными для демона; только обратная первая страница для живой сессии пересекает барьер сброса.
...
Строгий шлюз. Отключает сервер и удаляет его из оверлея времени выполнения. Идемпотентно — удаление имени, которое никогда не добавлялось, возвращает ответ с пропуском (не ошибку).

...
Привязка по умолчанию — `127.0.0.1:4170`. Bearer-аутентификация **отключена** для loopback, чтобы локальная разработка «просто работала». Демон регистрирует текущий рабочий каталог как основное рабочее пространство; используйте абсолютный путь `--workspace /path/to/dir`, чтобы переопределить его, и повторите флаг для регистрации дополнительных изолированных сред выполнения.

...
| `--local-control`                       | `false`         | Предоставить аутентифицированный Web Shell на каждом не-loopback IPv4-интерфейсе с свежим per-process токеном, маркированными QR-кодами терминала, точными browser origins, фиксированным портом и best-effort подавлением спящего режима. Конфликтует с `--token`, `--allow-origin`, `--no-web`, `--port 0` и нестандартным `--hostname`; добавьте `--tls-cert` + `--tls-key` для browser API безопасного контекста, таких как голосовой ввод.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |

...
Архивы должны быть полностью самодостаточными расширениями и иметь поддерживаемый корневой манифест: `qwen-extension.json` для нативного расширения Qwen или `plugin.json` для пакета [Agent Plugins v1](./agent-plugins.md).
...
Ваш npm-пакет должен включать поддерживаемый манифест в корне пакета: `qwen-extension.json` для нативного расширения Qwen или `plugin.json` для пакета [Agent Plugins v1](./agent-plugins.md). npm-тарбол — это просто ещё один механизм доставки.
...
Убедитесь, что выбранный корневой манифест и все указанные файлы пакета включены в публикуемый пакет (т.е. не исключены через `.npmignore` или поле `files` в `package.json`).

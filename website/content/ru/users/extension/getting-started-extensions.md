...
Навык расширения регистрируется под именем его владельца: навык выше станет `my-first-extension:code-analyzer`, поэтому вы запускаете его как `/my-first-extension:code-analyzer`. `skills.disabled` может заблокировать его как под этим именем, так и под простым `code-analyzer`, которое вы указали; `skills.enabled` возвращает его только под префиксным именем. См. [Навыки расширений](../features/skills.md#extension-skills).

### Добавление условных правил

Расширение также может содержать директорию `rules/`. Правило — это Markdown-файл, чей фронтматтер `paths:` указывает, к каким файлам оно применяется, и оно попадает в промпт только когда вызов инструмента затрагивает соответствующий файл:
...
**Правила расширения должны быть условными** — правило без `paths:` пропускается и упоминается в предупреждении при запуске. Базовое правило было бы частью каждого запроса, а именно это то, чего сноска в следующем шаге просит вас избегать. Правила появляются в промпте с пометкой владельца, как `my-first-extension:rules/charting.md`. См. [Правила](../features/rules.md).
...
> **Это самое затратное место для инструкций.** Контекстный файл расширения добавляется в системный промпт **каждого запроса каждой сессии, где расширение активно**, независимо от того, связана ли текущая работа с вашим расширением — никакой фильтра по релевантности и никаких ограничений на размер. В одном измеренном сеансе контекстные файлы девяти расширений составили 9 989 токенов — 21% всего, что запрос нёс до начала разговора. Включайте в контекстный файл только те факты, которые верны всегда — идентичность расширения, его словарь, жёсткое ограничение — а указания для конкретных сценариев размещайте в [навыке](../features/skills.md). Навык перечисляется только по имени и описанию — в том же измеренном сеансе 84 навыка в среднем занимали около 55 токенов каждый — а загружает своё содержимое только при вызове; навык с [фильтром по `paths:`](../features/skills.md#optional-gate-a-skill-on-file-paths-paths) даже не попадает в список, пока не будет затронут соответствующий файл. Команда `/context detail` показывает контекстный файл каждого расширения, чтобы вы видели, во сколько обходится ваш.
...
Снова перезапустите CLI. Теперь модель будет иметь контекст из вашего файла `QWEN.md` в каждой сессии, где расширение активно — и в каждом запросе этих сессий, поэтому сноска выше просит вас держать его кратким и использовать навыки для всего, что связано с конкретными сценариями.

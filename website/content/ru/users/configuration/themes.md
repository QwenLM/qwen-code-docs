...
Выбранные темы сохраняются в [конфигурации](../configuration/settings) Qwen Code, поэтому ваш выбор запоминается между сеансами.
...
Для любых значений цветов можно использовать либо шестнадцатеричные коды (например, `#FF0000`), **либо** стандартные имена цветов CSS (например, `coral`, `teal`, `blue`). Полный список поддерживаемых имен смотрите в разделе [CSS color names](https://developer.mozilla.org/en-US/docs/Web/CSS/color_value#color_keywords).
...
- Пользовательские темы могут быть установлены на уровне пользователя, проекта или системы и подчиняются тем же правилам [приоритета конфигурации](../configuration/settings), что и другие настройки.

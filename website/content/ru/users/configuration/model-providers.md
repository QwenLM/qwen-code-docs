...
Когда текстовая основная модель использует эту модель как настроенный vision-фолбэк, полный ход с изображениями остаётся на этом же провайдере, модели и эндпоинте на протяжении вызовов инструментов и повторов. Следующий независимый ход возвращается к основной модели, и каждый запрос модели получает только те модальности медиа, которые поддерживает его цель. Пропустите `agent` (или установите `false`), чтобы сохранить более безопасный поток транскрибации Vision Bridge.
...
### Динамические значения в `customHeaders`

Значение `customHeaders` может содержать плейсхолдер `${session_id}`, который
разворачивается для каждого запроса с текущим ID сессии Qwen Code. Используйте его для шлюзов,
которым требуется стабильный идентификатор на время разговора — OpenCode Go, например,
отклоняет запросы без `x-opencode-session`:
...
Поскольку значение разрешается для каждого запроса, а не встраивается в SDK-клиент,
`/new` и `/resume` ротируют его без перезапуска.

⚠️ **Требуется два шага.** Приведённая выше запись провайдера — это только половина —
плейсхолдер бездеятелен, пока вы также не включите
...
Пока вы этого не сделаете, значение, содержащее плейсхолдер, будет **отброшено**, а не отправлено,
а Qwen Code выведет предупреждение при запуске с указанием заголовка и этой настройки.
Заголовок никогда не отправляется с литеральным `${session_id}` в нём.

Переключатель является глобальным, поскольку это решение о согласии, отдельное от _того, куда_
отправляется значение: развёрнутое значение несёт текущее состояние сессии тому, кто его получает,
и переключатель контролирует только то, может ли `${session_id}` быть развёрнут. Он не
определяет, какой источник настроек предоставил заголовок.

**Примечание о конфиденциальности:** ID сессии — это стабильный идентификатор на время
разговора, поэтому любой хост, которому вы его отправляете, может группировать все запросы этого
разговора. Какие это хосты определяется тем, какие записи провайдеров несут
заголовок — отдельного списка хостов для синхронизации с вашим `baseUrl` нет.
...
- `customHeaders` - Пользовательские HTTP-заголовки (могут содержать `${session_id}`; см. [Динамические значения](#динамические-значения-в-customheaders))

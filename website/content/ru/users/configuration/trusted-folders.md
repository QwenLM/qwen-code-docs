...
- **Не доверять**: помечает папку как ненадежную. CLI будет работать в ограниченном режиме «безопасности».
...
2.  **Переменные окружения игнорируются**: CLI **не** загружает никакие файлы `.env` из проекта.
...
- **Просмотр всех правил доверия**: Чтобы увидеть полный список всех правил доверенных и недоверенных папок, вы можете проверить содержимое файла `~/.qwen/trustedFolders.json` в вашем домашнем каталоге.

## Процесс проверки доверия (для продвинутых пользователей)

Для продвинутых пользователей будет полезно знать точный порядок операций определения уровня доверия:

1.  **Сигнал доверия от IDE**: Если вы используете [интеграцию с IDE](../users/ide-integration/ide-integration), CLI сначала запрашивает у IDE информацию о том, является ли рабочее пространство доверенным. Ответ IDE имеет наивысший приоритет.

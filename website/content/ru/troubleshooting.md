...
- **Ошибка: Команда не найдена (при попытке запустить Qwen Code с помощью `qwen`).**
  ...
  - **Решение:**
    ...
    - Если вы установили `qwen` глобально, убедитесь, что директория с глобальными бинарниками `npm` находится в вашем `PATH`. Вы можете обновить его с помощью команды `npm install -g @qwen-code/qwen-code@latest`.

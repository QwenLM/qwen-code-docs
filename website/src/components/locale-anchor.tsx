...
export function LocaleAnchor(
  ...
  if (typeof href === "string" && href.length > 0) {
    try {
      // Next 配置了 trailingSlash: true，pathname 通常以 "/" 结尾。
      // 但 MDX 里的相对链接（../、./、foo）更接近“文件系统”语义：以当前页面“文件”所在目录为基准。
      // 因此这里把末尾 "/" 去掉，让 URL 解析把当前页当作“文件”，避免把当前 slug 当成目录层级。
      const basePathnameRaw = pathname || "/";
      const basePathname =
        basePathnameRaw !== "/" && basePathnameRaw.endsWith("/")
          ? basePathnameRaw.slice(0, -1)
          : basePathnameRaw;
      const base = `http://local${basePathname}`;
